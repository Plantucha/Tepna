...
(() => {
  ...
  var HOST_STYLE_PROPS = /* @__PURE__ */ new Set(['position', 'left', 'right', 'top', 'bottom', 'inset', 'width', 'height', 'z-index', 'transform']);
  ...
  function createExternalModules(onResolved) {
    ...
    function load(kind, url) {
      ...
      ready
        ...
        .then((src) => {
          ...
          console.info('[dc-runtime] x-import: loaded', url, '\u2014 exports:', Object.keys(module.exports), 'window globals:', Object.keys(globals));
          onResolved();
        })
        .catch((e) => {
          cache.set(url, {
    ...
    function resolve2(url, name) {
      ...
      if (!reportedMissing.has(key)) {
        reportedMissing.set(key, entry.error || 'no export named "' + name + '" (has: ' + Object.keys(mod).join(', ') + ')');
        console.error(
    ...
    function waitForGlobal(name) {
      ...
      const tick = () => {
        ...
        if (Date.now() - started >= GLOBAL_POLL_TIMEOUT_MS) {
          console.warn('[dc-runtime] x-import: global', JSON.stringify(name), 'never appeared on window after ' + GLOBAL_POLL_TIMEOUT_MS + 'ms');
          return;
        }
  ...
  function createRuntime(doc = document) {
    ...
    function ensureFetched(name) {
      ...
      fetch(url)
        .then((res) => {
          if (!res.ok) {
            console.error('[dc-runtime] sibling fetch for <' + name + '/> failed:', url, 'returned', res.status, '\u2014 the reference renders as an empty placeholder.');
            return '';
          }
          return res.text();
        })
        .then((t) => {
          ...
          if (!parsed) {
            console.error('[dc-runtime] sibling fetch for <' + name + '/>:', url, 'has no <x-dc> block \u2014 not a Design Component.');
            return;
          }
    ...
    function updateJs(name, src) {
      ...
      try {
        ...
        if (r.jsSeq !== seq) return;
        console.error('[dc-runtime] logic class eval FAILED for', name, '\u2014 the template renders with props only.', e);
  ...
  function loadReactUmd() {
    ...
    return Promise.all([loadScript(REACT_URL, REACT_SRI), loadScript(REACT_DOM_URL, REACT_DOM_SRI)]).then(() => void 0);
  }
  function init() {
