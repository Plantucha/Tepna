...
(function (global) {
  ...
  // ════════════════════════════════════════════════════════════════════════
  //  WAVEFORM-DERIVED SpO₂ TREND (owner-ordered ship, 2026-08-20 — experimental tier)
  ...
  /** Parse the O2Ring/ViHealth live SpO₂ CSV (`Time,Oxygen Level,Pulse Rate,Motion`, DMY stamps) into
   *  [{tMs, spo2}]. DMY per the Clock Contract's O2Ring rule; out-of-band SpO₂ (<60|>100) dropped. */
  function parseO2RingSpo2Csv(text) {
    var out = [];
    var lines = String(text || '').split(/\r?\n/);
    for (var i = 0; i < lines.length; i++) {
      var ln = lines[i];
      if (!ln || /^Time/i.test(ln)) continue;
      var p = ln.split(',');
      if (p.length < 2) continue;
      var m = /^(\d{2}):(\d{2}):(\d{2})\s+(\d{2})\/(\d{2})\/(\d{4})/.exec(p[0]);
      if (!m) continue;
      var tMs = Date.UTC(+m[6], +m[5] - 1, +m[4], +m[1], +m[2], +m[3]); // DMY, floating (Clock Contract)
      var v = +p[1];
      if (v >= 60 && v <= 100) out.push({ tMs: tMs, spo2: v });
    }
    return out;
  }
