...
(function () {
  ...
  function lockFresh() {
    try {
      var l = JSON.parse(localStorage.getItem('hrvconf_lock') || 'null');
  ...
  function lockBeat() {
    try {
      localStorage.setItem('hrvconf_lock', JSON.stringify({ id: RUN_ID, ts: Date.now() }));
    } catch (e) {}
  }
  function lockRelease() {
    try {
      var l = lockFresh();
      if (l && l.id === RUN_ID) localStorage.removeItem('hrvconf_lock');
    } catch (e) {}
  }
  ...
  async function run(resumeCk) {
    ...
    setStatus('booting ' + K + '× worker realms…', 'run');
    setProg(0);
    ...
    async function lane(rec) {
      while (!CANCEL) {
        ...
        if (m && m.result) {
          var res = m.result,
            prof = res.meta && res.meta.profile,
            age = prof ? prof.age : null;
          var gtN = {};
          ((res.meta && res.meta.groundTruth && res.meta.groundTruth.nights) || []).forEach(function (gn) {
            gtN[gn.n] = gn;
          });
    ...
    try {
      localStorage.setItem('hrvconf_ptPerSec', String(pdone / Math.max(0.001, elapsed)));
    } catch (e) {}
    setProg(1);
    if ($('cancel')) $('cancel').style.display = 'none';
    setStatus(
      (CANCEL ? 'cancelled — partial · ' : 'done · ') + PTS.length + ' nights · ' + (startSeed + pdone) + ' pt in ' + fmtETA(elapsed) + ' (' + (pdone / Math.max(0.001, elapsed)).toFixed(0) + ' pt/s)',
      CANCEL ? 'idle' : 'done'
    );
    try {
      updEta();
  ...
  (function () {
    async function tryResume() {
      try {
        ...
        if (lockHeldByOther()) {
          setStatus('another instance running ' + ck.nextSeed + '/' + ck.N + '… (watching)', 'run');
  ...
  function sevOf(a) {
    return a < 5 ? 'none' : a < 15 ? 'mild' : a < 30 ? 'mod' : 'severe';
  }
  ...
  function star(p) {
    return p == null ? '' : p < 0.001 ? '***' : p < 0.01 ? '**' : p < 0.05 ? '*' : 'n.s.';
  }
  var RESULT = null;
  function analyze() {
    var n = PTS.length;
    $('hN').textContent = n;
    if (!n) return;
    var ages = PTS.map(function (p) {
        return p.age;
      }),
      ahis = PTS.map(function (p) {
        return p.ahi;
      }),
    ...
    if (fit) {
      var labels4 = ['Intercept — rMSSD at mean age & AHI', 'age — per year (at mean AHI)', 'AHI — per event/h (at mean age)', 'age × AHI — effect modification'];
      ...
      for (var j = 0; j < 4; j++) {
        html +=
          '<tr' +
          (j === 3 ? ' style="color:var(--amber)"' : '') +
          '><td>' +
          labels4[j] +
          '</td>' +
          '<td class="num">' +
          fit.beta[j].toFixed(3) +
          '</td>' +
          '<td class="num">[' +
          fit.ci[j][0].toFixed(3) +
          ', ' +
          fit.ci[j][1].toFixed(3) +
          ']</td>' +
          '<td class="num">' +
          fit.t[j].toFixed(2) +
          '</td>' +
      ...
      html +=
        '<div class="muted" style="margin-top:10px;font-size:11.5px;line-height:1.65">' +
        'Full cohort, n = ' +
        fit.n +
        ' nights · R² = ' +
        fit.r2.toFixed(3) +
        ...
        ') · residual df = ' +
        fit.df +
        ' · 95% CI normal approx.<br>' +
        'Mean-centred predictors; r(age, AHI) = ' +
        rAA.toFixed(2) +
        ' (both retained — partial, not marginal, effects).<br>' +
        'Scaled: <span style="color:var(--blue)">' +
        perDecade.toFixed(1) +
        ' ms / decade</span> · ' +
        '<span style="color:var(--red)">' +
        per10.toFixed(1) +
        ' ms / 10 AHI</span> · ' +
        'interaction <span style="color:var(--amber)">' +
        (interScale >= 0 ? '+' : '') +
        interScale.toFixed(1) +
        ' ms / (decade × 10 AHI), p = ' +
        fmtP(interP) +
        '</span>.<br>' +
        verdict +
    ...
    var healthy = PTS.filter(function (p) {
      return p.ahi < 5;
    });
    var ref = ols(
      healthy.map(function (p) {
        return p.age;
      }),
      healthy.map(function (p) {
        return p.rmssd;
    ...
    var expectedFor = function (age) {
      return ref ? ref.slope * age + ref.intercept : mean(ys);
    };
    ...
    var rawScore = PTS.map(function (p) {
      return -p.rmssd;
    });
    var adjScore = PTS.map(function (p) {
      return -(p.rmssd - expectedFor(p.age));
    });
    var rRaw = roc(rawScore, pos),
      rAdj = roc(adjScore, pos);
    ...
    var sorted = PTS.slice().sort(function (a, b) {
      return a.rmssd - b.rmssd;
    });
    var flagged = sorted.slice(0, Math.round(n * 0.25));
    var falseOld = flagged.filter(function (p) {
      return p.ahi < 5;
    });
    ...
    RESULT = {
      n: n,
      model: fit
        ? { terms: ['intercept', 'age', 'ahi', 'age_x_ahi'], beta: fit.beta, se: fit.se, t: fit.t, p: fit.p, ci: fit.ci, r2: fit.r2, adjR2: fit.adjR2, df: fit.df, centered: { age: mAge, ahi: mAhi } }
        : null,
      corr_age_ahi: rAA,
      perDecadeAge: perDecade,
      per10AHI: per10,
      interaction_ms_per_decade_per10AHI: interScale,
      interaction_p: interP,
      ref: ref,
      aucRaw: rRaw.auc,
      aucAdj: rAdj.auc,
      misattrFrac: misattr,
      flaggedN: flagged.length,
  ...
  function ageColor(a) {
    var t = Math.max(0, Math.min(1, (a - 20) / 65));
    return 'hsl(' + Math.round(200 - t * 200) + ',70%,60%)';
  }
  ...
  function frame(ctx, P, w, h, xmin, xmax, ymin, ymax, xlab, ylab) {
    ...
    var fmtX = function (v) {
      return xmax - xmin <= 5 ? v.toFixed(1) : String(Math.round(v));
    };
    var fmtY = function (v) {
      return ymax - ymin <= 5 ? v.toFixed(1) : String(Math.round(v));
    };
    for (var i = 0; i <= 5; i++) {
      ...
      var px = P + ((w - P - 12) * i) / 5,
        py = h - P - ((h - P - 10) * i) / 5;
      ctx.fillText(fmtX(xv), px - 8, h - P + 16);
      ctx.fillText(fmtY(yv), 6, py + 4);
  ...
  function ptStyle() {
    var n = PTS.length;
    if (n <= 3000) return { r: 2.2, a: 0.5 };
    if (n <= 15000) return { r: 1.3, a: 0.32 };
    if (n <= 40000) return { r: 0.8, a: 0.2 };
    return { r: 0.6, a: 0.12 };
  }
  ...
  function drawVsAge(ref) {
    ...
    clearC(ctx, w, h);
    var ymax = 80;
    frame(ctx, P, w, h, 20, 85, 0, ymax, 'age (years)', 'measured rMSSD (ms)');
    var X = function (a) {
        return P + ((w - P - 12) * (a - 20)) / 65;
      },
      Y = function (v) {
        return h - P - ((h - P - 10) * v) / ymax;
      };
    var st = ptStyle();
    ctx.globalAlpha = st.a;
    PTS.forEach(function (p) {
      ctx.fillStyle = SEV_COLOR[p.sev];
      dot(ctx, X(p.age), Y(p.rmssd), st);
    });
    ctx.globalAlpha = 1;
    if (ref) {
      ctx.strokeStyle = '#fff';
      ...
      ctx.setLineDash([5, 4]);
      ctx.beginPath();
      ctx.moveTo(X(20), Y(ref.slope * 20 + ref.intercept));
      ctx.lineTo(X(85), Y(ref.slope * 85 + ref.intercept));
      ctx.stroke();
      ctx.setLineDash([]);
    }
    $('ageLegend').innerHTML =
      'Dashed = healthy (AHI&lt;5) age reference. <span class="chip" style="background:#39D98A"></span>none <span class="chip" style="background:#3DE0D0"></span>mild <span class="chip" style="background:#FFB84D"></span>mod <span class="chip" style="background:#FF6B7A"></span>severe. Apneic dots sit below the healthy age line at every age.';
  }
  function drawVsAhi() {
    var cv = $('vsAhi'),
      ctx = cv.getContext('2d'),
    ...
    ctx.globalAlpha = st.a;
    PTS.forEach(function (p) {
      ctx.fillStyle = ageColor(p.age);
      dot(ctx, X(p.ahi), Y(p.rmssd), st);
    });
    ctx.globalAlpha = 1;
  ...
  function drawRoc(rRaw, rAdj) {
    ...
    ctx.fillText('raw rMSSD  AUC ' + (rRaw.auc != null ? rRaw.auc.toFixed(3) : '—'), X(0.42), Y(0.18));
    ctx.fillStyle = '#3DE0D0';
    ctx.fillText('age-adjusted  AUC ' + (rAdj.auc != null ? rAdj.auc.toFixed(3) : '—'), X(0.42), Y(0.1));
  ...
  $('dlCsv').onclick = function () {
    ...
    PTS.forEach(function (p) {
      rows.push([p.age, p.ahi, p.rmssd, p.sev, p.cpap ? 1 : 0]);
    });
    dl(
      'hrv-confound-results.csv',
      rows
        .map(function (r) {
          return r.join(',');
        })
  ...
  $('dlStats').onclick = function () {
    dl(
      'hrv-confound-stats.json',
      JSON.stringify(
        Object.assign(
          {
            generated: new Date().toISOString(),
            framing:
              'SIMULATION. age→HRV and apnea→HRV couplings are generator inputs (literature-anchored, planted ADDITIVELY → interaction≈0), editable. Full-cohort model rMSSD~age×AHI fitted with SE/t/p/95%CI; the interaction term tests effect modification on the FULL N (no age-stratified subgroups). rMSSD measured by real PulseDex. Quantifies screening error + single age-adjustment recovery; NOT clinical validation.'
          },
          RESULT
        ),
        null,
  ...
  $('dlFig').onclick = function () {
    var ids = ['vsAge', 'vsAhi', 'roc'],
      gap = 16;
    var c0 = $('vsAge');
    var W = c0.width * 2 + gap * 3,
      H = c0.height * 2 + gap * 3;
    ...
    ctx.fillRect(0, 0, W, H);
    ctx.drawImage($('vsAge'), gap, gap);
    ctx.drawImage($('vsAhi'), gap * 2 + c0.width, gap);
    ctx.drawImage($('roc'), gap, gap * 2 + c0.height);
    out.toBlob(function (b) {
      var a = document.createElement('a');
      a.href = URL.createObjectURL(b);
      a.download = 'hrv-confound-figures.png';
      a.click();
    }, 'image/png');
  };
  ...
  function updEta() {
    ...
    if (rate && isFinite(rate) && rate > 0) {
      ...
      e.textContent =
        '\u2248 ' + (mm ? mm + 'm' + (ss < 10 ? '0' : '') + ss + 's' : ss + 's') + ' on this machine (' + rate.toFixed(0) + ' pt/s \u00b7 ' + (navigator.hardwareConcurrency || '?') + ' cores)';
    } else {
