...
(function (root) {
  ...
  function pdComputeResult(input) {
    ...
    /* ∅ CLOCK SEAM — the TWO-CLOCK discriminator, and the same one ECGDex states in its own words:
       "through a real BLE dropout both clocks keep ticking, so the device delta ≈ the phone delta;
       only a clock step makes them disagree". PulseDex has both clocks and never compared them: the
       RR INTERVAL is the device's measure of elapsed time between two beats, and the `tsMs` delta is
       the host's. A capture-side resync moves one and not the other.

       This matters because `rmssd`/`sdnn`/`pnn50` read the RR VALUES ONLY and never touch `tsMs`, so
       a clock step is INVISIBLE to them by construction — measured on a planted 7-year seam with
       physiological RR throughout, every one came back byte-identical to the continuous record while
       `coverage` collapsed 100 → 0 beside them. A number computable from broken input, reporting no
       problem (§∅). They are correct about the RR series and wrong about the RECORDING: the window
       no longer describes one continuous stretch of signal, so per the 2026-09-17 ruling a
       DISCONTINUITY refuses while reduced coverage annotates.

       The bound is NOT a new constant. 60000 ms is `ECG_RESYNC_BOUND_MS` = `PPG_RESYNC_BOUND_MS` =
       `MOTION_RESYNC_BOUND_MS`, shared because all these nodes read the same step in the same
       device's several files; the `ppgdex-clock-seam` parity group asserts this one equal to them. */
    const PD_RESYNC_BOUND_MS = 60000;
    let _seamN = 0,
      _seamMaxMs = 0;
    if (timedAxis && f.tsMs && f.tsMs.length >= N) {
      for (let i = 0; i + 1 < N; i++) {
        const hostDelta = f.tsMs[i + 1] - f.tsMs[i],
          devDelta = a[i];
        if (!isFinite(hostDelta) || !isFinite(devDelta)) continue;
        const disagree = Math.abs(hostDelta - devDelta);
        if (disagree > PD_RESYNC_BOUND_MS) {
          _seamN++;
          if (disagree > _seamMaxMs) _seamMaxMs = disagree;
        }
      }
    }
    const clockSeam = _seamN > 0;
    /* `meanRR`/`hr` are PER-BEAT averages, not window statistics over the axis, and they are kept
       deliberately: `classifyRecording` and `adaptEnvelopeNode` consume the duration immediately, and
       collapsing them is the DEEP-AUDIT-III §6.2 regression the envelope comment above already
       records paying for once. The VARIABILITY metrics are what span the seam, and those refuse —
       the same split PpgDex makes, where `_clockSeam` nulls the CVHR index with `reason:'clock-seam'`
       and leaves the rest of the record alone. */
    const meanRR = mean(a),
      sdnn = clockSeam ? null : std(a),
      rm = clockSeam ? null : rmssd(a),
      pn = clockSeam ? null : pnn50(a);
    ...
    return {
      ...
      hr,
      /* ∅ null, with the reason NAMED beside them — an absent HRV figure a reader cannot explain is
         only half of the refusal. `hrvReason` is additive and every consumer already guards
         `!= null` on these three (they were nullable via the < 10 beat path before this). */
      sdnn: sdnn == null ? null : +sdnn.toFixed(2),
      rmssd: rm == null ? null : +rm.toFixed(2),
      pnn50: pn == null ? null : +pn.toFixed(1),
      hrvReason: clockSeam ? 'clock-seam' : null,
      clockSeams: clockSeam ? { n: _seamN, maxDisagreeMs: Math.round(_seamMaxMs), boundMs: PD_RESYNC_BOUND_MS } : null,
      tp: sp.tp,
      hf: sp.hf,
      lf: sp.lf,
      vlf: sp.vlf,
  ...
  // modules destructure it explicitly (const { … } = window.PulseDex._bare) instead of depending
  ...
  PulseDex._bare = BARE;
  // mutable cross-file state, namespace-proxied — the DSP closure owns `lastResult`; the app
  // module bridges window.lastResult to this on its own page (the guarded window proxy below
  // keeps serving the non-namespaced classic realms).
  Object.defineProperty(PulseDex, 'lastResult', {
    configurable: true,
    get: function () {
      return lastResult;
    },
