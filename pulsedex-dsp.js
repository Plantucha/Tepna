...
const std    = a => { if(a.length<2) return 0; const m=mean(a); let s=0; for(let i=0;i<a.length;i++){ const d=a[i]-m; s+=d*d; } return Math.sqrt(s/(a.length-1)); }; // sample SD (÷N−1) — HRV Task Force / Kubios convention, unified fleet-wide 2026-06-24
const rmssd  = a => { const d=[]; for(let i=1;i<a.length;i++) d.push((a[i]-a[i-1])**2); return Math.sqrt(mean(d)); };
const pnn50  = a => { let n=0; for(let i=1;i<a.length;i++) if(Math.abs(a[i]-a[i-1])>50) n++; return n/(a.length-1)*100; };
const nn50c  = a => { let n=0; for(let i=1;i<a.length;i++) if(Math.abs(a[i]-a[i-1])>50) n++; return n; };
const minmax = a => { let mn=Infinity,mx=-Infinity; for(const v of a){ if(v<mn)mn=v; if(v>mx)mx=v; } return {mn,mx}; };
const mxdmn  = a => { const {mn,mx}=minmax(a); return mx-mn; };
const quant  = (a,q) => { const s=[...a].sort((x,y)=>x-y),i=(s.length-1)*q,l=Math.floor(i),h=Math.ceil(i); return +(s[l]+(s[h]-s[l])*(i-l)).toFixed(1); };
const modeV  = a => { const f={}; a.forEach(v=>{const k=Math.round(v/10)*10;f[k]=(f[k]||0)+1;}); return +Object.entries(f).sort((x,y)=>y[1]-x[1])[0][0]; };
...
function stressEst(sd,rm) { return Math.round(Math.max(0,Math.min(100, 107.68 - 0.2525*sd - 1.2295*rm))); }  // R²=0.92 vs Welltory Stress(HRV)
function hrvEst(sd,rm,pn)  { return Math.round(Math.max(0,Math.min(100, 1.494*rm - 13.37))); }  // calibrated to Welltory HRV Score: linear in rMSSD, R²=0.997
function energyEst(sd,rm)  { return Math.round(Math.max(0,Math.min(100, 1.27 + 0.4314*sd + 0.485*rm))); }  // R²=0.67 vs Welltory Energy(HRV)
function focusEst(sd,rm)   { return Math.round(Math.max(0,Math.min(100, -67.51 + 31.94*Math.log(sd)))); }  // R²=0.45 vs Welltory Focus (SDNN-driven)
function cohEst(rm,sd)     { return Math.round(Math.min(100,rm/sd*100*0.7)); }  // PulseDex-native — Welltory Coherence index is not derivable from summary HRV (R²≈0)
// VO2 / BP / cognitive
function vo2Base(hrRest,hrmax)  { return 15.3*hrmax/hrRest; }   // Uth-Sørensen (2004): VO₂max ≈ 15.3 × HRmax/HRrest
...
function altVO2Factor(elevM){ return elevM<=1500 ? 1 : Math.max(0.55, 1 - (elevM-1500)/300*0.01); }
...
function periodicBreathingIndex(a){
  const N=a.length; if(N<120) return null;
  const t=[]; let acc=0; for(let i=0;i<N;i++){t.push(acc/1000);acc+=a[i];}
  const dt=linfit(t,a), x=a.map((v,i)=>v-(dt.slope*t[i]+dt.intercept));
  const band=(lo,hi)=>{ let p=0; const nf=48, df=(hi-lo)/(nf-1);
    for(let k=0;k<nf;k++){ const w=2*Math.PI*(lo+k*df); let cs=0,ss=0,cc=0,sc2=0;
      for(let i=0;i<N;i++){const c=Math.cos(w*t[i]),s=Math.sin(w*t[i]);cs+=x[i]*c;ss+=x[i]*s;cc+=c*c;sc2+=s*s;}
      p+=(cs*cs/(cc||1)+ss*ss/(sc2||1))*0.5*df; }   // ×df → integrated power, bands comparable
    return p; };
  const pb=band(0.01,0.05), lf=band(0.04,0.15), tot=band(0.003,0.40)||1;
  const frac=Math.min(1,pb/tot);
  return { frac:+frac.toFixed(3), strong: frac>0.40 && pb>lf };   // PB dominates the low band
}
function siCalc(amo,mo,mx)   { return (mo&&mx) ? amo/(2*(mo/1000)*(mx/1000)) : 0; }  // Baevsky SI — Mo & MxDMn in SECONDS (Welltory units)
...
function efcIdx(en,fo,co)   { return en*0.4+fo*0.3+co*0.3; }
function crsIdx(co,rm,pn,st){ return st ? (co*rm*pn)/(st*1000) : 0; }
function absIdx(ps,sn)      { return (ps+sn) ? (ps-sn)/(ps+sn) : 0; }
...
function dfaAlpha1(a){
  ...
  for(let i=0;i<N;i++){ acc+=a[i]-m; y.push(acc); }            // integrated profile
  const logn=[], logF=[];
  for(let n=4;n<=16;n++){
    ...
    if(F>0){ logn.push(Math.log10(n)); logF.push(Math.log10(F)); }
  }
...
function sampEn(a, m, r){
  let N=a.length; if(N<m+2) return null;
  const MAXN=20000;
  if(N>MAXN){ const stride=Math.ceil(N/MAXN), dec=[]; for(let i=0;i<N;i+=stride) dec.push(a[i]); a=dec; N=a.length; }
  let B=0, A=0;
  for(let i=0;i<N-m;i++){
...
function fragmentation(a){
  ...
  let tot=0, shortNN=0, pasNN=0, altRun=0;
  runs.forEach(L=>{ tot+=L; if(L<3) shortNN+=L; });
  for(let i=0;i<runs.length;i++){
    if(runs[i]===1) altRun++;
    else { if(altRun>=4) pasNN+=altRun; altRun=0; }
  }
...
function prsaCapacity(a, sign){
  ...
  const X=[];
  for(let k=0;k<2*L+1;k++){ let s=0; win.forEach(w=>s+=w[k]); X.push(s/win.length); }
...
function lombScargle(a, nf){
  const N=a.length;
  const t=[]; let acc=0;
  for(let i=0;i<N;i++){ t.push(acc/1000); acc+=a[i]; }    // beat times in seconds
  const dt=linfit(t,a); const x=a.map((v,i)=>v-(dt.slope*t[i]+dt.intercept));  // linear detrend (Task Force) — stops slow drift leaking into VLF/LF
  const fLo=0.003, fHi=0.40; nf=nf||512; const df=(fHi-fLo)/(nf-1);
  ...
  let tp=0,vlf=0,lf=0,hf=0, peakF=0, peakP=0, gPeakF=0, gPeakP=0;
  for(let kf=0;kf<nf;kf++){
    ...
    for(let i=0;i<N;i++){
      const wt=w*(t[i]-tau), cw=Math.cos(wt), sw=Math.sin(wt);
      nC+=x[i]*cw; dC+=cw*cw; nS+=x[i]*sw; dS+=sw*sw;
    }
    const P=0.5*((nC*nC/(dC||1))+(nS*nS/(dS||1)));
    const e=P*df; tp+=e;
    if(P>gPeakP){ gPeakP=P; gPeakF=f; }                  // GLOBAL peak (whole band) — surfaces sub-HF CSR/PB
    if(f<0.04) vlf+=e;
    else if(f<0.15) lf+=e;
    else { hf+=e; if(P>peakP){ peakP=P; peakF=f; } }     // HF-only peak → respRate (RSA frequency)
  }
  const variance=x.reduce((s,v)=>s+v*v,0)/N;
  const sc = tp>0 ? variance/tp : 1;                      // calibrate ∫PSD = variance
  return {
    tp:Math.round(tp*sc), vlf:Math.round(vlf*sc),
    lf:Math.round(lf*sc), hf:Math.round(hf*sc),
...
function _pdIntervalColFromHeader(parts){
  ...
  for(const pat of pats){ for(let i=0;i<parts.length;i++){ if(pat.test(parts[i]||'')) return i; } }
  return -1;
}
function _pdIntervalColByRange(rows){
  if(!rows.length) return -1;
  let ncol=0; for(const r of rows) ncol=Math.max(ncol,r.length);
  let best=-1,bestScore=-1;
  for(let c=0;c<ncol;c++){
    const nums=[]; for(const r of rows){ const v=parseFloat(String(r[c]||'').replace(',','.')); if(isFinite(v)) nums.push(v); }
    ...
    if(med>=300 && med<=2000){
      ...
      if(inRange>bestScore){ bestScore=inRange; best=c; }
    }
  }
  return best;
}
...
function mergeMultipart(parsed){           // parsed = [{name,text}]
...
function parseRRInput(raw) {
  ...
  const vals = [];
  const tsMs = [];
  let t0Ms = null, offsetMin = null, tsOK = true;
  let prevTMs = null, dateAnchorMs = null;
  let sourceFormat = 'rr', intervalCol = -1, headerLine = null;

  if (delimHits >= 2) {
    const rows = nonEmpty.map(l => l.split(/[;\t]/));
    // header = first row whose 2nd field isn't numeric (a label row)
    for (const p of rows) { if (p.length >= 2 && !isFinite(parseFloat((p[1]||'').replace(',', '.')))) { headerLine = p; break; } }
    if (headerLine) {
      intervalCol = _pdIntervalColFromHeader(headerLine);
      if (/pp[\s_\-]*interval|\bppi\b/i.test(headerLine.join(';'))) sourceFormat = 'ppi';
    }
    if (intervalCol < 0) {                 // no usable header → pick by physiological range
      const dataRows = rows.filter(p => p !== headerLine && p.length >= 2 && isFinite(parseFloat((p[p.length-1]||'').replace(',', '.'))));
      intervalCol = _pdIntervalColByRange(dataRows);
    }
    ...
    for (const parts of rows) {
      ...
      if (p) {
        tsMs.push(p.tMs); prevTMs = p.tMs;
        if (dateAnchorMs == null) { const d = new Date(p.tMs);
          dateAnchorMs = Date.UTC(d.getUTCFullYear(), d.getUTCMonth(), d.getUTCDate()); }
        if (t0Ms === null) { t0Ms = p.tMs; offsetMin = p.offsetMin; }  // anchor + capture zone (Polar RR is zoned ISO)
      } else {
        tsMs.push(NaN); tsOK = false;      // missing stamp → NaN, NEVER fabricate now()
      }
    }
  }

  if (!vals.length) {                       // fallback: scrape bare RR numbers (no timestamps)
  ...
  let nUsable = 0;
  for (let i = 0; i < vals.length; i++) { const v = vals[i]; if (isFinite(v) && v >= 250 && v <= 3000) nUsable++; }
  const usable = nUsable >= 10;
  let reason = null;
  if (!vals.length) reason = (headerLine || delimHits >= 2)
        ? 'columns present but no interval rows — the device logged no usable beats'
        : 'no numeric intervals found in the file';
  ...
  return { vals, t0Ms: tsValid ? t0Ms : null, offsetMin: tsValid ? offsetMin : null,
           tsMs: tsValid ? tsMs : null, sourceFormat, intervalCol,
           nRaw: vals.length, nUsable: nUsable, usable: usable, reason: reason };
}
...
function artifactClean(vals){
  ...
  for(let i=0;i<n;i++){
    const lo=Math.max(0,i-W),hi=Math.min(n,i+W+1),seg=[];
...
function beatTimes(a,tsEpoch){
  ...
  if(tsEpoch&&tsEpoch.length===a.length&&isFinite(tsEpoch[0])){
    const t0=tsEpoch[0];for(let i=0;i<a.length;i++)t[i]=(tsEpoch[i]-t0)/1000;
  }else{let acc=0;for(let i=0;i<a.length;i++){t[i]=acc/1000;acc+=a[i];}}
  return t;
}
...
function classifyRecording(a,t0Ms,durSec){
  const n=a.length,hr=60000/mean(a),durMin=durSec/60;
  ...
  let mode,conf,why;
  if(durMin>=20*60){mode='continuous';conf=.95;why=(durMin/60).toFixed(1)+' h continuous record';}
  else if(durMin>=90){mode='overnight';conf=.9;why=(durMin/60).toFixed(1)+' h'+(hour!=null?' from '+String(hour).padStart(2,'0')+':00':'');}
  else if(hr>=100||rising>15){mode='exercise';conf=.75;why='mean HR '+hr.toFixed(0)+' bpm'+(rising>15?', HR rising '+rising.toFixed(0):'');}
  else if(durMin<=8&&(hour==null||hour<11)){mode='morning';conf=.7;why=durMin.toFixed(1)+' min'+(hour!=null?' at '+String(hour).padStart(2,'0')+':00':'')+' spot';}
  else{mode='spot';conf=.6;why=durMin.toFixed(1)+' min reading';}
...
function windowAnalysis(a,t,winSec){
  const N=a.length;let i=0;const tEnd=t[N-1];const wins=[],segs=[];
  for(let w0=0;w0<=tEnd;w0+=winSec){
    const w1=w0+winSec,seg=[];
    while(i<N&&t[i]<w1){seg.push(a[i]);i++;}
    if(seg.length>=20){            // need enough beats for a valid window
      const m=mean(seg);
      wins.push({tMin:+(w0/60).toFixed(1),hr:+(60000/m).toFixed(1),rmssd:+rmssd(seg).toFixed(1),
                 sdnn:+std(seg).toFixed(1),pnn:+pnn50(seg).toFixed(1),meanRR:+m.toFixed(1)});
      segs.push(seg);
    }
  }
  return {wins,segs};
...
function lineChartSVG(pts,color,medVal){
  const W=680,H=150,P={l:46,r:14,t:14,b:26},n=pts.length;
  if(!n)return '';
  let ymn=Infinity,ymx=-Infinity,xmn=Infinity,xmx=-Infinity;
  for(const p of pts){if(p.y<ymn)ymn=p.y;if(p.y>ymx)ymx=p.y;if(p.x<xmn)xmn=p.x;if(p.x>xmx)xmx=p.x;}
  ...
  const xt=[];for(let h=Math.ceil(xmn/60)*60;h<=xmx;h+=60)xt.push(h);
...
function _pdPearson(x,y){
  ...
  const d=Math.sqrt(sxx*syy); return d>0 ? +(sxy/d).toFixed(3) : null;
}
function _pdSeriesStats(a){
  if(!a||a.length<3) return null;
  const m=mean(a);
  return { n:a.length, meanRR:+m.toFixed(1), hr:+(60000/m).toFixed(1),
           sdnn:+std(a).toFixed(1), rmssd:+rmssd(a).toFixed(1), pnn50:+pnn50(a).toFixed(1) };
}
...
function compareIntervalSeries(primary, reference){
  ...
  const endTs = (vals, tsMs) => {
    ...
    const t=new Array(vals.length); let acc=0; for(let i=0;i<vals.length;i++){ acc+=vals[i]; t[i]=acc; } return t;
  };
  const haveAbs = !!(primary.tsMs && reference.tsMs && isFinite(primary.tsMs[0]) && isFinite(reference.tsMs[0]));
  const ta = endTs(A, primary.tsMs), tb = endTs(B, reference.tsMs);

  const matchA=[], matchB=[], diffs=[];
  if(haveAbs){
    ...
    for(let i=0;i<B.length;i++){
      ...
      const tol=Math.max(120, 0.30*Math.min(A[j],B[i]));
      if(dt<=tol){ matchA.push(A[j]); matchB.push(B[i]); diffs.push(B[i]-A[j]); }
    }
  } else {
    // no shared clock → assume 1:1 beat correspondence (both are the same recording)
    const m=Math.min(A.length,B.length);
    for(let i=0;i<m;i++){ matchA.push(A[i]); matchB.push(B[i]); diffs.push(B[i]-A[i]); }
  }
  const nM=diffs.length;
  const statsA=_pdSeriesStats(A), statsB=_pdSeriesStats(B);
  const mAll=Math.min(A.length,B.length);
  if(nM<5){
    return { matched:nM, matchRatePct:+(nM/mAll*100).toFixed(0), statsA, statsB, haveAbs,
             weak:true, note:'Too few beats line up — different recordings, or no shared clock.' };
  }

  // ── beat-level agreement (Bland–Altman) ──
  const bias=mean(diffs), sdDiff=std(diffs);
  ...
  const rmA=rmssd(matchA), rmB=rmssd(matchB), sdA=std(matchA), sdB=std(matchB);
  const dRMSSD=+(rmB-rmA).toFixed(1), dSDNN=+(sdB-sdA).toFixed(1);
  ...
  const kA=primary.kind||'auto', kB=reference.kind||'auto';
  ...
  const ba=[]; for(let i=0;i<nM;i++){ if(i%step===0) ba.push([+( (matchA[i]+matchB[i])/2 ).toFixed(0), +diffs[i].toFixed(1)]); }

  return {
    matched:nM, matchRatePct:+(nM/mAll*100).toFixed(0), haveAbs,
    primaryLabel:primary.label||'Signal A', referenceLabel:reference.label||'Signal B',
    refIsPulse,
    statsA, statsB,
    agreement:{ biasMs:+bias.toFixed(1), sdDiffMs:+sdDiff.toFixed(1),
      loaLoMs:+(bias-1.96*sdDiff).toFixed(1), loaHiMs:+(bias+1.96*sdDiff).toFixed(1),
      madMs:+mad.toFixed(1), within25Pct:+within25.toFixed(0), pearsonR:r, grade:agreeGrade },
    discrepancy:{ dRMSSD, dSDNN, rmssdRatio, pttvMs:pttv,
...
function _pdNormalizeRRInput(input){
  ...
  if(input && typeof input==='object'){
    ...
    if(vals) return { vals:vals.slice(),
...
function pdComputeResult(input){
  ...
  const a = cleanR.clean, N = a.length;
  if(N < 10) return null;
  ...
  if(f.tsMs && isFinite(f.tsMs[0]) && isFinite(f.tsMs[N-1])){
    const wall=(f.tsMs[N-1]-f.tsMs[0])/1000; let rrSum=0; for(let i=0;i<N;i++) rrSum+=a[i]/1000;
  ...
  const meanRR=mean(a), sdnn=std(a), rm=rmssd(a), pn=pnn50(a);
  const hr=+(60000/meanRR).toFixed(1);

  let win=null, dispRm=rm, dispSd=sdnn, dispHr=hr, dispPn=pn, dispMeanRR=meanRR, repSeg=a, winSpec=null;
  if(longRec){
    ...
    if(win.wins.length>=3){
      ...
      let bi=0,bd=Infinity; for(let i=0;i<rmA.length;i++){const d=Math.abs(rmA[i]-dispRm);if(d<bd){bd=d;bi=i;}}
      repSeg=win.segs[bi];
  ...
  const cSd=longRec?dispSd:sdnn, cRm=longRec?dispRm:rm, cPn=longRec?dispPn:pn;
  ...
  const _cid = (input && typeof input==='object' && typeof input.contentId==='string') ? input.contentId
  ...
  return {
    t0Ms:f.t0Ms, offsetMin:f.offsetMin, contentId:_cid,
    mode, modeLabel:MODE_LABEL[mode]||mode, longRec, durMin:+(durSec/60).toFixed(1),
    coverage, N, windows: win?win.wins:null,
    meanRR:+meanRR.toFixed(1), hr, sdnn:+sdnn.toFixed(2), rmssd:+rm.toFixed(2), pnn50:+pn.toFixed(1),
    tp:sp.tp, hf:sp.hf, lf:sp.lf, vlf:sp.vlf,
    stress, hrv, energy, focus, coherence:coh,
    sns:ans.sns, psns:ans.psns, snsBal:ans.snsBal, psnsBal:ans.psnsBal,
    sd1:sd1v, sd2:sd2v, lnrmssd:lnrm, lfhf:lfhfv
  };
}
...
function pdEventsFromResult(r){
  ...
  if(t0!=null){
    ...
    if(wins.length){
      const rms=wins.map(w=>w.rmssd).sort((a,b)=>a-b), med=rms[Math.floor(rms.length/2)]||1;
      wins.forEach(w=>{
        const tMs=t0+w.tMin*60000, st=stressEst(w.sdnn,w.rmssd);
        if(w.rmssd < 0.7*med && w.rmssd < 25)
          ev.push({tMs, t:_pdClockS(tMs), impulse:'hrv_drop', node:'PulseDex',
        ...
        if(st>=70)
          ev.push({tMs, t:_pdClockS(tMs), impulse:'stress_peak', node:'PulseDex',
            conf:+Math.max(0.4,Math.min(0.92,(st-50)/50)).toFixed(2), meta:{stress:st, rmssd:w.rmssd, sdnn:w.sdnn, position:null}});
      });
    } else if(r.rmssd!=null){
      if(r.stress>=70) ev.push({tMs:t0, t:_pdClockS(t0), impulse:'stress_peak', node:'PulseDex',
        conf:+Math.max(0.4,Math.min(0.92,(r.stress-50)/50)).toFixed(2), meta:{stress:r.stress, rmssd:r.rmssd, position:null}});
      else if(r.rmssd<20) ev.push({tMs:t0, t:_pdClockS(t0), impulse:'hrv_drop', node:'PulseDex',
        conf:0.55, meta:{rmssd:r.rmssd, sdnn:r.sdnn, position:null}});
    }
  }
...
function pdBuildNodeExport(r, opts){
  ...
  const schema = {
    name:'ganglior.node-export', version:'2.0', node:'PulseDex', nodeVersion:'1.0',
  ...
  return {
    ...
    schema,
    recording:{ source:'rr', contentId:r.contentId??null, startEpochMs:r.t0Ms??null, offsetMin:r.offsetMin??null,
      durationMin:r.durMin, mode:r.mode, modeLabel:r.modeLabel, longRecording:r.longRec,
      windows:(r.windows?r.windows.length:0), beats:r.N, coveragePct:r.coverage },
    hrv:{ time:{ rmssd:r.rmssd, sdnn:r.sdnn, pnn50:r.pnn50, hr:r.hr, meanRR:r.meanRR, lnRMSSD:r.lnrmssd },
      frequency:{ lf:r.lf, hf:r.hf, vlf:r.vlf, lfhf:r.lfhf }, poincare:{ sd1:r.sd1, sd2:r.sd2 } },
...
PulseDex.compute = function(input, opts){
  ...
  if(!r) return null;
  opts = opts || {};
  return pdBuildNodeExport(r, { provenance:(opts.provenance!==undefined?opts.provenance:null),
    kernel:opts.kernel||null, ingest:opts.ingest||undefined, generated:opts.generated });
};
...
if (!root.__DEX_NAMESPACED__) {
  Object.assign(root, {
    ...
    fmtDateTime, mean, std, rmssd, pnn50, nn50c, minmax, mxdmn,
    quant, modeV, amo50, sd1, sd2, lnR, lfHf, nu,
    ansBalance, stressEst, hrvEst, energyEst, focusEst, cohEst, vo2Base,
    vo2Adj, altVO2Factor, periodicBreathingIndex, siCalc, efcIdx, crsIdx, absIdx, linfit,
  ...
  Object.defineProperty(root, 'lastResult', { configurable: true,
    get: function () { return lastResult; }, set: function (v) { lastResult = v; } });
}
