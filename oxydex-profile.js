...
let UP = {
  ...
  hrmaxOverride: 0,
  hrRestOverride: 0,
  elevation: null, // metres above sea level, or null = NOT MEASURED (§∅); adjusts SpO₂ normal thresholds
...
function upLoad() {
  ...
  if (window.DexProfile) {
    try {
      ...
      UP.elevation = p.elevation != null && isFinite(p.elevation) ? p.elevation : null; // §∅ — absent stays absent
      UP.cpap = p.cpap || 'no';
    } catch (e) {}
    return;
  }
...
function upToDOM() {
  ...
  sv('profElevation', UP.elevation != null ? UP.elevation : '');
...
/* §∅ — NO elevation means NO adjustment, and that is a different state from "at sea level", even
   though both yield 0. `upSpo2Adj()` returns the number (0 either way, so no shipped threshold moves
   on this change); `upElevState()` says WHICH state produced it, and the disclosure below uses it so
   a reader can tell an unmeasured profile from a measured coast. */
function upElevState() {
  return UP.elevation == null || !isFinite(UP.elevation) ? 'unmeasured' : 'measured';
}
function upSpo2Adj() {
  if (upElevState() === 'unmeasured') return 0; // not measured ⇒ nothing to adjust by
  return Math.min(7, Math.round((UP.elevation / 1000) * 1.8));
}
...
function profileDerivedUpdate() {
  ...
  // Elevation — show current SpO₂ adjustment alongside formula. `_elv`/`_adj` were
  // shared with the dead `pd_elev` disclosure that DEEP-AUDIT-II #43 removed; this
  // profileElevSub sub still needs them, so they are declared locally here (the #43
  // removal dropped the shared declarations and left this line referencing undefined
  // names → a ReferenceError that broke profileDerivedUpdate — regression hotfix).
  var _elv = UP.elevation;
  var _adj = upSpo2Adj();
  /* Three states, three messages — an unmeasured profile must not read as a measured coast (§∅). */
  _sub(
    'profileElevSub',
    _elv == null
      ? '∼ not set — no altitude adjustment applied (enter your elevation; −1.8% SpO₂ per 1000 m, Roach 1998)'
      : _elv > 0
        ? '∼ Roach 1998: −1.8%/1000m · adj −' + _adj + '% (norm ≥' + (95 - _adj) + '%)'
        : '0 m = sea level (measured) · −1.8% SpO₂ per 1000m (Roach 1998)'
  );
}
