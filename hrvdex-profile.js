...
function inferFromData() {
  ...
  const hrs = useRows.map(r => r._hr).filter(v => v > 30 && v < 120).sort((a,b)=>a-b);
  ...
  function _ageSDNN(ms){ return null; }  // ANS-age helper retired 2026-06-23 (unused; kept as null stub)
  ...
  const _hrmaxV = (_pp.hrmax_manual > 0 && _pp.hrmax_manual >= 140 && _pp.hrmax_manual > _hrRestV + 45)
    ? _pp.hrmax_manual : _tanaka;
  ...
  if (DXP()) { try { DXP().prefillFrom({ hrRest: restingHR, vo2: vo2Est }); } catch(e){} }
  window._projVO2 = vo2Est;

  // Date range info
  const dates = allRows.map(r => r._date).filter(d => !isNaN(d)).sort((a,b)=>a-b);
  ...
  function setIfEmpty(id, val, isEstimate) {
    ...
    const defaultVals = {prof_age:'35',prof_weight:'75.0',prof_height:'175.0',
                         prof_sbp:'120',prof_dbp:'78',prof_vo2gt:'0',prof_hrmax:'0'};
  ...
  function _setSub(id, txt){ var el=document.getElementById(id); if(el){ el.textContent=txt; el.style.display=''; } }
  ...
  if(_lblAge){ _lblAge.style.display='none'; }
  ...
  if (DXP()) {
    ...
    try {
      ...
      if (DXP().renderPanel && !window._hrvPanel) {
        window._hrvPanel = DXP().renderPanel({ node:'hrvdex', mount:'dexProfilePanel', onChange:function(){ updateProfile(); } });
      }
      ...
      if (!DXP().isPristine()) {
        const p = DXP().get(), man = (DXP().getRecord().manual) || {};
        const set = (id,v) => { const el=document.getElementById(id); if(el!=null && v!=null && v!=='') el.value=v; };
        set('prof_age', p.age); const sx=document.getElementById('prof_sex'); if(sx) sx.value = p.sex==='F'?'F':'M';
        set('prof_weight', p.weight); set('prof_height', p.height);
        set('prof_sbp', p.sbp); set('prof_dbp', p.dbp);
        set('prof_vo2gt', man.vo2!=null?man.vo2:''); set('prof_hrmax', man.hrMax!=null?man.hrMax:'');
        set('prof_hrrest', man.hrRest!=null?man.hrRest:''); set('prof_elev', p.elevation!=null?p.elevation:'');
      }
    } catch(e) {}
    updateProfile();
    _hrvLoading = false;
...
function getProfile() {
  if (DXP()) {
    ...
    const detOr0 = field => { const mv = n(man[field],0); if(mv>0) return mv; const r = DXP().resolve(field); return (r.origin==='detected'&&r.v>0)?r.v:0; };
    return {
      age: n(p.age,35), sex: p.sex==='F'?'F':'M',
      weight: n(p.weight,75), height: n(p.height,175),
...
const POP_NORMS = {
  M: {
    '18': {w:79.8, h:177.4}, '30': {w:85.6, h:177.2},
    '40': {w:89.4, h:176.7}, '50': {w:91.4, h:175.8},
    '60': {w:87.9, h:174.6}, '70': {w:82.1, h:172.3}
  },
  F: {
    '18': {w:68.2, h:163.5}, '30': {w:73.4, h:163.2},
    '40': {w:76.8, h:162.7}, '50': {w:78.3, h:161.9},
    '60': {w:76.1, h:160.5}, '70': {w:71.4, h:158.2}
  }
};
...
function applyAgeNorms(useIdeal) {
  ...
  if(wLabel) { wLabel.textContent = useIdeal
      ? ('∼ ideal (BMI 22.5) ' + w + ' kg')
      : ('∼ CDC pop avg ' + norm.w + ' kg · ideal (BMI 22.5) ' + idealW + ' kg');
    wLabel.style.display=''; }
  if(hLabel) { hLabel.textContent = '∼ CDC pop avg ' + h + ' cm'; hLabel.style.display=''; }
...
function updateProfile() {
  ...
  if (DXP()) {
    if (!_hrvLoading && document.getElementById('prof_age')) try {
      const g = id => { const e=document.getElementById(id); return e?e.value:''; };
      const num = v => { const n=parseFloat(v); return isFinite(n)?n:null; };
      DXP().setManual('age', num(g('prof_age'))); DXP().setManual('sex', g('prof_sex'));
      DXP().setManual('weight', num(g('prof_weight'))); DXP().setManual('height', num(g('prof_height')));
      DXP().setManual('sbp', num(g('prof_sbp'))); DXP().setManual('dbp', num(g('prof_dbp')));
  ...
  const _hrRest0 = (typeof allRows !== 'undefined' && allRows.length > 0)
    ? Math.round(allRows.reduce((s,r)=>s+r._hr,0)/allRows.length) : 60;
  ...
  const hrRest = (typeof allRows !== 'undefined' && allRows.length > 0)
    ? Math.round(allRows.reduce((s,r)=>s+r._hr,0)/allRows.length)
    : 60;
  const hrr = hrmax - hrRest; // HR Reserve
  const z1_lo = Math.round(hrRest + 0.50 * hrr), z1_hi = Math.round(hrRest + 0.60 * hrr);
  const z2_lo = Math.round(hrRest + 0.60 * hrr), z2_hi = Math.round(hrRest + 0.70 * hrr);
  const z3_lo = Math.round(hrRest + 0.70 * hrr), z3_hi = Math.round(hrRest + 0.80 * hrr);
  const z4_lo = Math.round(hrRest + 0.80 * hrr), z4_hi = Math.round(hrRest + 0.90 * hrr);
  const z5_lo = Math.round(hrRest + 0.90 * hrr);
  ...
  const bap_full = (0.011 * hrRest + 0.014 * p.age + 0.008 * p.sbp + 0.014 * p.dbp
                    - 0.009 * p.weight - 0.009 * p.height - 0.27).toFixed(3);
  ...
  function vo2Percentile(vo2, age, sex) {
    ...
    const mPerc = {
      '20-29': [[31,5],[37,20],[42,40],[49,60],[55,80],[62,95]],
      '30-39': [[29,5],[35,20],[40,40],[46,60],[52,80],[59,95]],
      '40-49': [[25,5],[31,20],[36,40],[42,60],[49,80],[56,95]],
      '50-59': [[22,5],[27,20],[31,40],[37,60],[44,80],[51,95]],
      '60-69': [[18,5],[23,20],[27,40],[32,60],[38,80],[45,95]],
    };
    const fPerc = {
      '20-29': [[26,5],[31,20],[36,40],[41,60],[48,80],[55,95]],
      '30-39': [[24,5],[29,20],[33,40],[39,60],[45,80],[52,95]],
      '40-49': [[21,5],[25,20],[29,40],[34,60],[40,80],[47,95]],
      '50-59': [[18,5],[22,20],[26,40],[30,60],[37,80],[44,95]],
      '60-69': [[15,5],[19,20],[22,40],[27,60],[34,80],[40,95]],
    };
  ...
  const grp = (label, items) =>
    `<div class="pd-group"><span class="pd-group-label">${label}</span><div class="pd-group-grid">${items}</div></div>`;
  d.innerHTML =
    ...
    grp('Respiratory / fitness',
      ...
      di('VO₂ percentile', '~'+vo2Perc+'th', 'Cooper bands ('+(p.age<30?'20s':p.age<40?'30s':p.age<50?'40s':p.age<60?'50s':'60+')+' '+(p.sex==='M'?'M':'F')+')')
    );

  // Field hints: HRmax guard, resting HR, elevation
  const altFactor = p.elev<=1500 ? 1 : Math.max(0.55, 1 - (p.elev-1500)/300*0.01);
  const _set = (id,txt,est)=>{ const el=document.getElementById(id); if(!el) return; el.textContent=txt; el.classList.toggle('est',!!est); };
  ...
  if(pz){
    const zr = (label, lo, hi, cls) =>
      `<div class="prof-zone-row ${cls}"><span class="prof-zone-label">${label}</span><span class="prof-zone-val">${lo}–${hi} bpm</span></div>`;
...
function calcVo2Cat(vo2, age, sex) {
  ...
  const maleNorms = {
    '20-24':[37.1,42.2,46.2,52.1],'25-29':[36.2,40.0,44.2,49.4],
    '30-34':[34.6,38.5,42.4,47.4],'35-39':[32.5,36.7,40.8,45.4],
    '40-44':[31.1,35.4,38.9,43.7],'45-49':[28.7,32.3,35.9,40.5,49.5],
    '50-54':[26.1,30.9,33.8,37.4],'55-59':[24.5,28.0,31.4,34.3],
    '60-64':[23.3,25.6,27.8,31.2],'65-69':[19.8,22.7,25.6,27.9]
  };
  const femaleNorms = {
    '20-24':[31.0,35.2,38.6,43.9],'25-29':[29.4,33.4,36.7,41.0],
    '30-34':[27.7,31.4,34.4,38.6],'35-39':[25.8,29.5,32.5,37.7],
    '40-44':[24.5,27.8,30.9,35.0],'45-49':[22.8,26.2,29.0,32.3],
    '50-54':[20.8,24.0,26.7,29.4],'55-59':[20.0,22.0,24.3,26.6],
  };
  const ab = age<25?'20-24':age<30?'25-29':age<35?'30-34':age<40?'35-39':
