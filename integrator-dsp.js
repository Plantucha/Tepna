...
function fmtDayShort(ms){ var d=new Date(ms); return MONTHS[d.getUTCMonth()]+' '+d.getUTCDate(); }
...
function combineConf(confs){
  var prod = 1, any = false;
  for(var i=0;i<confs.length;i++){ var c=confs[i]; if(c==null||!isFinite(c)) continue; any=true; prod *= (1 - Math.max(0,Math.min(1,c))); }
  if(!any) return null;
...
function _mergeMs(ivs){
  ...
  var total=0, curS=a[0][0], curE=a[0][1];
  for(var i=1;i<a.length;i++){
    if(a[i][0] <= curE){ curE=Math.max(curE,a[i][1]); }
    else { total+=(curE-curS); curS=a[i][0]; curE=a[i][1]; }
  }
  total+=(curE-curS);
  return total;
}
...
function pearson(xs,ys){ var n=xs.length; if(n<3) return null;
  ...
  var sxy=0,sxx=0,syy=0; for(var i=0;i<n;i++){ var dx=xs[i]-mx, dy=ys[i]-my; sxy+=dx*dy; sxx+=dx*dx; syy+=dy*dy; }
  if(sxx===0||syy===0) return null; return +(sxy/Math.sqrt(sxx*syy)).toFixed(3); }
...
/* ════════════════════════════════════════════════════════════════════════
   ...
   ════════════════════════════════════════════════════════════════════════ */
const NODE_COLORS = {
  ECGDex:'#FF6B7A', OxyDex:'#58A6FF', GlucoDex:'#FFB84D',
  PulseDex:'#3DE0D0', PpgDex:'#B98AFF', HRVDex:'#39D98A', CPAPDex:'#14B8A6', Unknown:'#8C9DB3'
};
function nodeColor(n){ return NODE_COLORS[n] || NODE_COLORS.Unknown; }
...
function _eventsFromEnvelope(json, node){
  ...
  var raw = Array.isArray(json.ganglior_events) ? json.ganglior_events
          : (Array.isArray(json.fascia_events) ? json.fascia_events
          : (Array.isArray(json.events) ? json.events : []));
  // If no t0Ms but events carry absolute tMs, derive t0Ms from earliest.
  if(t0Ms == null){
    var abs = raw.map(function(e){ return (e&&typeof e.tMs==='number')?e.tMs:null; }).filter(function(v){return v!=null;});
  ...
  for(var i=0;i<raw.length;i++){
    ...
    events.push({ tMs:tMs, t:(e.t || fmtClockS(tMs)), impulse:e.impulse||'event',
      node:e.node||node, conf:(e.conf!=null?e.conf:null), sqi:(e.sqi!=null?e.sqi:null), meta:e.meta||{} });
  }
...
function adaptEnvelopeNode(json, node, filename){
  ...
  var t0Ms = r.t0Ms, events = r.events;
  ...
  var offsetMin = null;
  for(var i=0;i<events.length;i++){ if(events[i].meta && events[i].meta.offsetMin!=null){ offsetMin=events[i].meta.offsetMin; break; } }
  if(json.recording && json.recording.offsetMin!=null) offsetMin = json.recording.offsetMin;
  // node-specific summary for fusion
  var summary = {};
  var seriesOut = {};
  if(node==='ECGDex'){
    summary.autonomicInstabilitySlope = (json.hrvStability && json.hrvStability.mean_lnRMSSD_slope!=null) ? json.hrvStability.mean_lnRMSSD_slope
        : (json.reserved && json.reserved.autonomicInstabilitySlope!=null ? json.reserved.autonomicInstabilitySlope : null);
    ...
    var _ht = (json.hrv&&json.hrv.time)||{}, _hf=(json.hrv&&json.hrv.frequency)||{};
    ...
    if(!_sleepSuppressed && json.sleep && json.sleep.stageMinutes && json.sleep.totalSleepMin>0){
      var _sm=json.sleep.stageMinutes, _tot=json.sleep.totalSleepMin;
  ...
  if(node==='GlucoDex'){
    ...
    seriesOut.cells = (Array.isArray(ts.cells) ? ts.cells : [])
      .map(function(cl){ return { tMs: (cl.tMs!=null ? cl.tMs
              : (t0c!=null && cl.i!=null && cadMin!=null ? t0c + cl.i*cadMin*60000 : null)),
            v: cl.v, f: (cl.f!=null?cl.f:0) }; })
  ...
  return [{
    ...
    dateStr: t0Ms!=null?fmtDate(t0Ms):null, t0Ms:t0Ms, endMs:endMs, offsetMin:offsetMin,
    ...
    raw:{ ganglior_events: (json.ganglior_events || json.events || null) }, _src:filename
  }];
}

function _dig(o, path){ var c=o; for(var i=0;i<path.length;i++){ if(c==null) return null; c=c[path[i]]; } return (c==null?null:c); }
...
function _ecgPostureSeries(json, t0Ms){
  ...
  function push(tMin, pos){ if(pos==null) return; var tMs = (t0Ms!=null && tMin!=null) ? t0Ms + tMin*60000 : null; out.push({ tMs:tMs, tMin:tMin, pos:String(pos).toLowerCase() }); }
  if(Array.isArray(acc)){ acc.forEach(function(a){ push(a.tMin!=null?a.tMin:a.t, a.position||a.pos||a.bodyPosition); }); }
  ...
  if(Array.isArray(eps)){ eps.forEach(function(e){ if(e && e.position && e.position!=='unknown') push(e.tMin, e.position); }); }
  ...
  if(Array.isArray(json.ganglior_events)){
    json.ganglior_events.forEach(function(e){
      ...
      if(p){ var tMs=reconstructEventTMs(e,t0Ms); if(tMs!=null) out.push({ tMs:tMs, pos:String(p).toLowerCase() }); }
    });
  }
...
function adaptOxyDex(json, filename){
  ...
  var _topEvents = (json && !Array.isArray(json) && Array.isArray(json.ganglior_events) && json.ganglior_events.length)
    ? _eventsFromEnvelope({ ganglior_events:json.ganglior_events, startEpochMs:_topT0 }, 'OxyDex').events : null;
  ...
  if(_topEvents){
    ...
    _eventBuckets = nights.map(function(){ return []; });
    _topEvents.forEach(function(e){
      if(e.tMs==null) return;
      var best=-1, bestS=-Infinity;
      for(var wi=0; wi<_wins.length; wi++){ var w=_wins[wi];
        if(w.s==null || e.tMs < w.s) continue;
        if(w.e!=null && e.tMs > w.e) continue;
        if(w.s > bestS){ bestS=w.s; best=wi; }
      }
      // no window strictly contains it → attach to the latest night starting at/before it
      if(best<0){ for(var wj=0; wj<_wins.length; wj++){ var w2=_wins[wj]; if(w2.s!=null && e.tMs>=w2.s && w2.s>bestS){ bestS=w2.s; best=wj; } } }
  ...
  nights.forEach(function(n, ni){
    ...
    // 1) native PER-NIGHT ganglior events (legacy emit-shim that wrote events into a night)
      ...
      devs.forEach(function(d){
        ...
        events.push({ tMs:tMs, t:fmtClockS(tMs), impulse:'desat_event', node:'OxyDex',
          conf:+conf.toFixed(2), meta:{ depth:depth, nadir:d.nadir, durSec:d.duration, recovery:d.recovery } });
      });
      ...
      sevs.forEach(function(sp){
        ...
        events.push({ tMs:tMs, t:fmtClockS(tMs), impulse:'autonomic_arousal', node:'OxyDex',
          conf:+conf.toFixed(2), meta:{ peak:sp.peak, baseline:sp.baseline, rise:rise, mfm:sp.mfm, spo2:sp.spo2 } });
      });
    }
    events.sort(function(a,b){ return a.tMs-b.tMs; });
    var summary = {
      odi4: n.odi4?n.odi4.rate:null, minSpo2: stats.minSpo2, meanSpo2: stats.meanSpo2,
...
function normalizeFile(json, filename){
  var warnings = [];
  if(json == null){ return { recs:[], warnings:['Empty or unreadable JSON — skipped'] }; }
  // bus check (input only — never reject on bus name)
  var busVal = json.bus || (json.schema && json.schema.bus) || null;
  if(!busOK(busVal)){ warnings.push('Unknown bus "'+busVal+'" — accepting anyway (case-insensitive)'); }
  ...
  if(node!=='Unknown' && KNOWN_NODES.indexOf(node)<0){
    warnings.push('Node "'+node+'" is not registered in the Integrator (no color / summary adapter) — '
      +'it will load but be excluded from fusion. Add it to NODE_COLORS + the summary branch.');
  }
  // ── SUMMARY-SHAPE GUARD ──────────────────────────────────────────────────
  ...
  var _looksSummary = json && !Array.isArray(json)
     && !json.ganglior_events && !json.fascia_events && !json.events && !json.recording
     && json.t0Ms != null
     && (Array.isArray(json.windows) || json.nWindows != null || json.durMin != null);
  if(_looksSummary && node !== 'Unknown'){
    warnings.push('"'+(filename||node)+'" looks like a '+node+' SUMMARY export (flat metrics: '
...
function dedupeRecs(existing, incoming){
  ...
  incoming.forEach(function(nr){
    ...
    if(dup){ warns.push(nr.label+' looks like a duplicate of an already-loaded recording — skipped'); }
    else kept.push(nr);
  });
...
function overlapInterval(a, b){
  var wa=recWindow(a), wb=recWindow(b);
  if(!wa||!wb) return null;
  // same-day different-timezone: align on real instants only when BOTH offsets present AND differ
  var sa=wa.startMs, ea=wa.endMs, sb=wb.startMs, eb=wb.endMs, basis='wall-clock';
  if(a.offsetMin!=null && b.offsetMin!=null && a.offsetMin!==b.offsetMin){
    sa-=a.offsetMin*60000; ea-=a.offsetMin*60000; sb-=b.offsetMin*60000; eb-=b.offsetMin*60000; basis='utc-instant';
  }
  var s=Math.max(sa,sb), e=Math.min(ea,eb);
  if(e<=s) return null;
  return { startMs:Math.max(wa.startMs,wb.startMs), endMs:Math.min(wa.endMs,wb.endMs),
           overlapMin:+(((e-s)/60000)).toFixed(1), basis:basis };
}

/* ════════════════════════════════════════════════════════════════════════
   §5 FUSION RULES — each independently skippable; evidence-based.
   ════════════════════════════════════════════════════════════════════════ */
function _byNode(recs, node){ return recs.filter(function(r){ return r.node===node && !r.dateUnknown; }); }
function _eventsOfType(rec, types){ return rec.events.filter(function(e){ return types.indexOf(e.impulse)>=0; }); }
...
function pickHRAuthority(hrNodesLive){
  ...
  var ranked = hrNodesLive.filter(function(h){ return h && HR_AUTHORITY[h.node]!=null; })
    .slice().sort(function(a,b){ return HR_AUTHORITY[a.node]-HR_AUTHORITY[b.node]; });
  return ranked.length ? ranked[0] : null;
}
function corroborateDesat(desat, hrNodesLive){
  if(!desat) return desat;
  if(desat.artifact){ desat.verdict='artifact'; return desat; }   // already self-gated at the node
  var onset = (desat.tMs!=null ? desat.tMs : desat.onsetMs);
  var depth = (desat.depthPct!=null ? desat.depthPct
              : (desat.meta && desat.meta.depth!=null ? desat.meta.depth
              : (desat.meta && desat.meta.desatDepth!=null ? desat.meta.desatDepth : desat.depth)));
  ...
  if(hrNode){
    var w0 = onset, w1 = onset + CONSEQUENCE.SURGE_WIN_SEC*1000;
    var surges = hrNode.surges || [];
    var surge = (onset!=null) && surges.some(function(s){ var t=(s!=null&&s.tMs!=null)?s.tMs:s; return t!=null && t>=w0 && t<=w1; });
    if(expectSurge && !surge){ desat.verdict='artifact-no-consequence'; }   // depth demanded a surge; none came ⇒ drop
...
function fuseApneaEvents(recs, dtMs, gate){
  ...
  var raw=[];
  oxy.forEach(function(o){ cardiac.forEach(function(g){ var w=overlapInterval(o,g); if(w) raw.push([w.startMs,w.endMs]); }); });
  if(!raw.length) return { findings:[], confirmedAHI:null, confirmedAHIReportable:false, overlapHours:0,
    apneaAuthority:_deviceScoredAuthority(recs, null),
    matched:{desat:0,surge:0}, total:{desat:0,surge:0}, unmatched:{desat:[],surge:[]},
  ...
  for(var ri=1; ri<raw.length; ri++){
    var last=merged[merged.length-1], cur=raw[ri];
    if(cur[0] <= last[1]) last[1]=Math.max(last[1],cur[1]);   // overlap → extend
    else merged.push(cur.slice());                            // disjoint → new interval
  }
  var totHrs=0; merged.forEach(function(iv){ totHrs+=(iv[1]-iv[0])/3600000; });
  function inUnion(tMs){ for(var i=0;i<merged.length;i++){ if(tMs>=merged[i][0]-dtMs && tMs<=merged[i][1]+dtMs) return true; } return false; }
  ...
  function gather(recList, types){
    var pool=[], seen={};
    recList.forEach(function(r){ _eventsOfType(r,types).forEach(function(e){
      if(!inUnion(e.tMs)) return;
      var key=e.impulse+'@'+Math.round(e.tMs/1000);
      if(seen[key]) return; seen[key]=1;
      pool.push(e);
    }); });
    pool.sort(function(a,b){ return a.tMs-b.tMs; });
    return pool;
  }
  ...
  desats.forEach(function(d){
    ...
    if(best!=null){
      usedSurge.add(best);
      var s=surges[best];
      findings.push({ tMs:d.tMs, durSec:(d.meta&&d.meta.durSec)||null, type:'confirmed_apnea_event',
        conf:combineConf([effConf(d), effConf(s)]), nodes:['OxyDex', s.node||'ECGDex'],
        sources:[{node:'OxyDex',impulse:d.impulse,tMs:d.tMs,conf:d.conf,sqi:(d.sqi!=null?d.sqi:null),effConf:+(effConf(d)||0).toFixed(3)},
        ...
        note:'O₂ desaturation confirmed by a directionally-consistent autonomic surge ('
          +'−'+leadMaxSec+'s…+'+trailMaxSec+'s). Neither node alone can assert this.' });
  ...
  var unmatchedSurge = surges.filter(function(s,si){ return !usedSurge.has(si); });
  findings.sort(function(a,b){ return a.tMs-b.tMs; });
  ...
  var hrByNode={};
  surges.forEach(function(s){ var k=s.node||'ECGDex'; (hrByNode[k]=hrByNode[k]||[]).push(s.tMs); });
  var hrNodesLive = Object.keys(hrByNode).map(function(k){ return { node:k, surges:hrByNode[k] }; });
  findings.forEach(function(f){ f.verdict='confirmed'; });
  unmatchedDesat.forEach(function(d){ corroborateDesat(d, hrNodesLive); });
  var consequence = {
    hrWitness: (pickHRAuthority(hrNodesLive)||{}).node || null,
    confirmed: findings.length,
    noConsequence: unmatchedDesat.filter(function(d){ return d.verdict==='artifact-no-consequence'; }).length,
    unconfirmed: unmatchedDesat.filter(function(d){ return d.verdict==='unconfirmed-desat'; }).length
  };
  ...
  findings.forEach(function(f){ f.belowChance = belowChance; f.pSpurious = +pAtLeast.toFixed(3); });
  ...
  return { findings:findings, confirmedAHI:ahi, confirmedAHIReportable:(!belowChance && nConf>0),
    overlapHours:+totHrs.toFixed(2), apneaAuthority:_deviceScoredAuthority(recs, ahi),
    matched:{ desat:nConf, surge:nConf }, total:{ desat:desats.length, surge:surges.length },
    unmatched:{ desat:unmatchedDesat, surge:unmatchedSurge },
    consequence:consequence,
    nullModel:{ expectedConfirmed:+lambda.toFixed(2), pAtLeastObserved:+pAtLeast.toFixed(3),
      belowChance:belowChance, surgeRatePerHr:+(surgeRate*3600).toFixed(1), directionalWindowSec:winSec } };
}
...
function _deviceScoredAuthority(recs, confirmedIndex){
  ...
  for(var ci=0; ci<cpap.length; ci++){
    ...
    if(cs.residualAHI!=null){
      ...
      return { source:'device-scored', node:'CPAPDex', ahi:dev,
        components:{ central:cs.centralIndex!=null?cs.centralIndex:null,
...
function _poissonSf(k, lam){
  ...
  var cum=0, term=Math.exp(-lam);
  for(var i=0;i<k;i++){ cum+=term; term*=lam/(i+1); }
...
function labelPositionalApnea(recs, apneaResult){
  ...
  var posture = [];
  ecg.forEach(function(g){ if(g.summary && g.summary.posture) posture = posture.concat(g.summary.posture); });
  posture = posture.filter(function(p){ return p.tMs!=null; }).sort(function(a,b){return a.tMs-b.tMs;});
  // fall back to PpgDex limb-ACC posture when no chest-strap posture is available
  var src = 'chest-acc';
  if(!posture.length){
    var ppg = _byNode(recs,'PpgDex');
    ppg.forEach(function(g){ if(g.summary && g.summary.posture) posture = posture.concat(g.summary.posture); });
    posture = posture.filter(function(p){ return p.tMs!=null; }).sort(function(a,b){return a.tMs-b.tMs;});
  ...
  apneaResult.findings.forEach(function(f){
    ...
    if(p==null) unknown++; else if(/supine|back/.test(p)) supine++; else nonsupine++;
  });
  ...
  return { available:true, supine:supine, nonsupine:nonsupine, unknown:unknown, supineRate:rate!=null?+rate.toFixed(2):null,
    positional:positional, postureSource:src,
    note:'Provisional — body position is '+(src==='limb-acc'?'LIMB-worn ACC (Polar Sense; lower reliability — wrist/ankle orientation, not trunk)':'chest-ACC-derived')+', not PSG. '+
...
function _mageWin(v, sd){
  var n=v.length; if(n<4) return Math.round(sd);
  var tp=[0], dir=0, i;
  for(i=1;i<n;i++){
    var d=v[i]-v[tp[tp.length-1]];
    if(dir===0){ if(Math.abs(d)>0) dir=(d>0?1:-1); }
    else { var s=v[i]-v[i-1]; var sg=s>0?1:(s<0?-1:0); if(sg!==0 && sg!==dir){ tp.push(i-1); dir=sg; } }
  }
  tp.push(n-1);
  var exc=[];
  for(i=1;i<tp.length;i++){ var amp=Math.abs(v[tp[i]]-v[tp[i-1]]); if(amp>sd) exc.push(amp); }
  if(!exc.length) return Math.round(sd);
  var sum=0; for(i=0;i<exc.length;i++) sum+=exc[i]; return Math.round(sum/exc.length);
}
function glucoseMetricsInWindow(cgmRec, startMs, endMs, opts){
  ...
  var i, win=[];
  for(i=0;i<cells.length;i++){ if(cells[i].tMs>=startMs && cells[i].tMs<=endMs) win.push(cells[i]); }
  // value stats exclude compression artifacts (f===3); keep OK + gap-interp
  var vals=[], valsT=[];
  for(i=0;i<win.length;i++){ if(win[i].f===3) continue; if(win[i].v==null) continue; vals.push(win[i].v); valsT.push(win[i].tMs); }
  ...
  // mean / SD / CV
  var m=0; for(i=0;i<vals.length;i++) m+=vals[i]; m/=vals.length;
  var ss=0; for(i=0;i<vals.length;i++){ var dv=vals[i]-m; ss+=dv*dv; }
  var sd = vals.length>1 ? Math.sqrt(ss/(vals.length-1)) : 0;
  var cv = m>0 ? sd/m*100 : 0;
  // TIR 70–140 · nadir · time-below-70
  var tt=0, below=0, nadir=Infinity, nadirT=null;
  for(i=0;i<vals.length;i++){ var g=vals[i]; if(g>=70&&g<=140) tt++; if(g<70) below++; if(g<nadir){ nadir=g; nadirT=valsT[i]; } }
  // dawn rise: min(03:00–06:00) → max(06:00–08:00) by UTC hour
  var dawnNadir=Infinity, dawnMax=-Infinity, dawnT=null;
  for(i=0;i<vals.length;i++){ var h=new Date(valsT[i]).getUTCHours();
    if(h>=3&&h<6){ if(vals[i]<dawnNadir) dawnNadir=vals[i]; }
    else if(h>=6&&h<8){ if(vals[i]>dawnMax){ dawnMax=vals[i]; dawnT=valsT[i]; } } }
  var dawnRise = (isFinite(dawnNadir)&&isFinite(dawnMax)) ? Math.round(dawnMax-dawnNadir) : null;
  return {
    nMin:+nMin.toFixed(1), coverage:+coverage.toFixed(3), nCells:win.length,
    nocturnalMean:Math.round(m), nocturnalCV:+cv.toFixed(1),
    nadirValue:Math.round(nadir), nadirTimeMs:nadirT,
    dawnRise:dawnRise, dawnRiseTimeMs:dawnT,
    tir70_140:+(tt/vals.length*100).toFixed(1), mage:_mageWin(vals, sd),
...
function fuseAutonomicGlycemic(recs, dtMs, opts2){
  var ecg=_byNode(recs,'ECGDex'), glu=_byNode(recs,'GlucoDex');
  ...
  ecg.forEach(function(g){
    glu.forEach(function(c){
      ...
      var cv, dawn, coverage=null, windowed=false;
      if(hasCells){
        ...
        cv = gm.nocturnalCV; dawn = gm.dawnRise; coverage = gm.coverage; windowed = true;
      } else {
      ...
      pairs.push({ ecg:g.label, glu:c.label, overlapMin:win.overlapMin, slope:slope,
        glucoseCV:cv, dawnSurge:dawn, coverage:coverage, windowed:windowed });
    });
  });
...
function _tchRhoFromMotion(triplet, keys){
  var ms=triplet.map(function(s){ return { node:s.node, m:_tchAlignedMotion(s, keys) }; }).filter(function(x){ return x.m; });
  if(ms.length<2) return null;
  var rs=[]; for(var i=0;i<ms.length;i++) for(var j=i+1;j<ms.length;j++){ var r=_tchPearson(ms[i].m, ms[j].m); if(r!=null) rs.push(r); }
  if(!rs.length) return null;
  var pos=rs.map(function(r){ return Math.max(r,0); });
  var mean=pos.reduce(function(a,b){return a+b;},0)/pos.length;
...
function _tchHat(like, ptsFn, metric){
  ...
  var ws=like.filter(function(s){ return ptsFn(s).length>=12; });
  if(ws.length<3) return { ok:false, metric:metric, reason:'need ≥3 nodes with a per-epoch '+metric+' series; have '+ws.length, nodesWithSeries:ws.map(function(s){return s.node;}) };
  ...
  if(typeof TCH.screenTriplet==='function'){
    ...
    if(scr && scr.drop){
      return { ok:false, metric:metric, reason:'decorrelated node dropped — '+scr.reason,
               dropped:scr.drop, keptPair:scr.keptPair, corr:scr.corr,
               nodesWithSeries:ws.map(function(s){return s.node;}) };
    }
  }
  ...
  if(!r.ok){ r.metric=metric; return r; }
  r.metric=metric; r.coMotion={};
  ...
  r.levels={}; [best.A,best.B,best.C].forEach(function(s){ var mp={}; ptsFn(s).forEach(function(p){mp[p.tMin]=p.v;}); var vs=best.al.keys.map(function(kk){return mp[kk];}).filter(function(v){return v!=null;}); r.levels[s.node]=vs.length?+(vs.reduce(function(a,b){return a+b;},0)/vs.length).toFixed(1):null; });
...
function _tchConsensus(like){ return _tchHat(like, _rmssdPts, 'rmssd'); }
function fuseHRVConsensus(recs, dtMs){
  ...
  var groups=[];
  sources.forEach(function(s){ var placed=false;
    for(var i=0;i<groups.length;i++){ if(groups[i].some(function(o){ return overlapInterval(o,s); })){ groups[i].push(s); placed=true; break; } }
    if(!placed) groups.push([s]);
  });
  var blocks = groups.filter(function(g){ return g.length>=2; }).map(function(g){
    ...
    var likeWin = g.filter(function(s){ return ((s.summary&&s.summary.hrvWindow)||'wholeRecord') === win; });
    ...
    function spread(key){ var vs=like.map(function(s){return s.summary[key];}).filter(function(v){return v!=null;}); if(vs.length<2) return null;
      var mn=Math.min.apply(null,vs), mx=Math.max.apply(null,vs), md=median(vs);
    ...
    recs.forEach(function(rc){ if(hrLike.indexOf(rc)>=0 || rc.dateUnknown || _hrPts(rc).length<12) return;
      if(g.some(function(o){ return overlapInterval(o, rc); })) hrLike.push(rc); });
    var tchHR = _tchHat(hrLike, _hrPts, 'hr');
    var hrReconciled = null;
    if(tchHR && tchHR.ok && tchHR.levels){   // inverse-variance reconciled HR (weight ∝ 1/σ²)
    ...
    if(tchHR && tchHR.ok) note += ' HR-hat: '+tchHR.culprit+' is the noisiest HR estimator (σ≈'+(tchHR.sigma[tchHR.culprit]!=null?tchHR.sigma[tchHR.culprit].toFixed(1):'?')+' bpm, '+tchHR.method+(tchHR.rho?', ρ='+tchHR.rho:'')+')'+(hrReconciled!=null?'; reconciled HR '+hrReconciled+' bpm.':'.');
...
function fuseStagingConsensus(recs, remGapThresh){
  remGapThresh = remGapThresh==null ? 0.20 : remGapThresh;
  var src = recs.filter(function(r){ return !r.dateUnknown && r.summary && r.summary.remFraction!=null; });
  if(src.length < 2) return null;
  // group by temporal overlap (same night)
  var groups=[];
  src.forEach(function(s){ var placed=false;
    for(var i=0;i<groups.length;i++){ if(groups[i].some(function(o){ return overlapInterval(o,s); })){ groups[i].push(s); placed=true; break; } }
    if(!placed) groups.push([s]); });
  var blocks = groups.filter(function(g){ return g.length>=2; }).map(function(g){
    var vals = g.map(function(s){ return { node:s.node, remPct:+(s.summary.remFraction*100).toFixed(1), method:s.summary.stagingMethod||null }; });
    var fr = g.map(function(s){ return s.summary.remFraction; });
    var gap = Math.max.apply(null,fr) - Math.min.apply(null,fr);
    var disagree = gap > remGapThresh;
    return { window:fmtDayShort(g[0].t0Ms), nodes:g.map(function(s){return s.node;}),
      remByNode:vals, remGapPct:+(gap*100).toFixed(1), disagreement:disagree,
      note: disagree
        ? ('Single-signal sleep stages disagree by '+(gap*100).toFixed(0)+' pts of REM ('
            +vals.map(function(v){return v.node+' '+v.remPct+'%';}).join(' vs ')
...
function _pbObserver(rec){
  ...
  if(rec.node==='OxyDex' || rec.node==='CPAPDex'){
    ...
    var cs = pb.map(function(e){ return e.conf; }).filter(function(c){ return c!=null && isFinite(c); });
...
function fusePeriodicBreathing(recs){
  var src = (recs||[]).filter(function(r){ return !r.dateUnknown; })
    .map(function(r){ var o=_pbObserver(r); return o ? { rec:r, obs:o } : null; })
    .filter(function(x){ return x; });
  if(src.length < 2) return null;
  // group by temporal overlap (same night) — identical pattern to staging / HRV consensus
  var groups=[];
  src.forEach(function(s){ var placed=false;
    for(var i=0;i<groups.length;i++){ if(groups[i].some(function(o){ return overlapInterval(o.rec, s.rec); })){ groups[i].push(s); placed=true; break; } }
    if(!placed) groups.push([s]);
  });
  var blocks = groups.map(function(g){
    ...
    var byNode={};
    g.forEach(function(s){ var ex=byNode[s.obs.node]; if(!ex || (s.obs.episodes||0) > (ex.episodes||0)) byNode[s.obs.node]=s.obs; });
    var obs = Object.keys(byNode).map(function(k){ return byNode[k]; });
    var t0 = Math.min.apply(null, g.map(function(s){ return s.rec.t0Ms!=null?s.rec.t0Ms:Infinity; }));
    if(!isFinite(t0)) t0=null;
    var conf = combineConf(obs.map(function(o){ return (o.conf!=null?o.conf:0) * (PB_TIER_WEIGHT[o.tier]||0.6); }));
    var note = 'Periodic breathing corroborated across '+obs.length+' independent signals — '
      + obs.map(function(o){ return o.node+' ('+o.channel
          + (o.episodes!=null?', '+o.episodes+' episode'+(o.episodes===1?'':'s'):'')
          + (o.pbPct!=null?', '+o.pbPct+'% of night':'')
          + (o.cvhrIndex!=null?', CVHR '+o.cvhrIndex+'/h':'')+')'; }).join('; ')
      + '. Tier-weighted (device-scored > cardiac CVHR > oximetry proxy); a cross-signal corroboration, not a scored Cheyne–Stokes index.';
    return { t0Ms:t0, window:(t0!=null?fmtDayShort(t0):'date unknown'),
      observerNodes:obs.map(function(o){ return o.node; }), nObservers:obs.length,
      corroborated:obs.length>=2, conf:conf, sources:obs, note:note };
...
function runFusion(recs, opts){
  ...
  var gate = { leadMaxSec:(opts.leadMaxSec!=null?opts.leadMaxSec:15),
               trailMaxSec:(opts.trailMaxSec!=null?opts.trailMaxSec:60) };
  var dated = recs.filter(function(r){ return !r.dateUnknown; });
  // overall window across dated recs + pairwise overlaps
  var startMs=null,endMs=null;
  dated.forEach(function(r){ var w=recWindow(r); if(!w) return; if(startMs==null||w.startMs<startMs) startMs=w.startMs; if(endMs==null||w.endMs>endMs) endMs=w.endMs; });
  var pairs=[];
  for(var i=0;i<dated.length;i++) for(var j=i+1;j<dated.length;j++){
    var ov=overlapInterval(dated[i],dated[j]);
    pairs.push({ a:dated[i].label, an:dated[i].node, b:dated[j].label, bn:dated[j].node,
      aWin:recWindow(dated[i]), bWin:recWindow(dated[j]), overlap:ov });
  }
  var anyOverlap = pairs.some(function(p){ return p.overlap; });
  ...
  var ivs=[]; pairs.forEach(function(p){ if(p.overlap) ivs.push([p.overlap.startMs, p.overlap.endMs]); });
  ...
  var nodesExcluded = dated.filter(function(r){
    return !pairs.some(function(p){ return p.overlap && (p.a===r.label || p.b===r.label); });
  }).map(function(r){ return r.node + (r.dateStr?(' · '+r.dateStr):''); });
  ...
  var hrLive = dated.filter(function(r){ return HR_AUTHORITY[r.node]!=null && r.nEvents>0; })
    .map(function(r){ return { node:r.node, label:r.label, nEvents:r.nEvents }; });
  ...
  var hrSource = hrLive.length ? {
    ...
    available: hrLive.map(function(h){ return h.node; }),
    note: hrPick ? (hrPick.node === 'ECGDex'
  ...
  if(staging && staging.blocks){
    staging.blocks.forEach(function(b){
      ...
      findings.push({ tMs:(startMs||0), type:'staging_disagreement', conf:null,
        nodes:b.nodes, sources:[], meta:{ remGapPct:b.remGapPct, remByNode:b.remByNode },
        note:b.note });
    });
  }
  ...
  if(periodicBreathing && periodicBreathing.blocks){
    periodicBreathing.blocks.forEach(function(b){
      findings.push({ tMs:(b.t0Ms!=null?b.t0Ms:(startMs||0)), type:'periodic_breathing', conf:b.conf,
        nodes:b.observerNodes, sources:b.sources,
        meta:{ nObservers:b.nObservers, corroborated:b.corroborated, window:b.window,
          byNode:b.sources.map(function(o){ return { node:o.node, channel:o.channel, tier:o.tier, episodes:o.episodes, pbPct:o.pbPct, cvhrIndex:o.cvhrIndex, conf:o.conf }; }) },
        note:b.note });
    });
  }
  ...
  return {
    ...
    matchWindow:{ leadMaxSec:gate.leadMaxSec, trailMaxSec:gate.trailMaxSec,
      directionalWindowSec:gate.leadMaxSec+gate.trailMaxSec, unionPrefilterSec:dtMs/1000 },
    anyOverlap:anyOverlap,
    kernelAudit:kernelAudit,
    hrSource:hrSource,
    pairs:pairs, apnea:apnea, positional:positional, autoGly:autoGly, hrv:hrv, staging:staging, periodicBreathing:periodicBreathing,
    findings:findings,
    unmatched: apnea?apnea.unmatched:{desat:[],surge:[]},
    nodes: recs.map(function(r){ return { node:r.node, label:r.label, date:r.dateStr, window:recWindow(r), nEvents:r.nEvents, dateUnknown:r.dateUnknown }; })
  };
}
...
function buildFusionExport(recs, fusion){
  return {
    ...
    schema:{ name:BUS+'.fusion-export', version:'1.3', generated:fusion.generated,
      ...
      method:{
        confidence:'Each finding conf = noisy-OR of its sources: conf = 1 − ∏(1 − cᵢ), capped at 0.97, rounded to 3 dp. Per source, cᵢ = effConf = conf × (sqi ?? 1): the event-likelihood (scaled to surge magnitude / desat depth) attenuated by local signal quality (R7). Raw conf, sqi and effConf are retained in each finding’s sources[].',
        apneaMatch:'confirmed_apnea_event = SpO₂ desaturation ⟷ nearest unused autonomic surge from ECGDex|PpgDex, within an asymmetric directional window (surge may lead the nadir by ≤leadMaxSec, trail by ≤trailMaxSec; R5). One surge confirms at most one desat.',
        nullModel:'confirmedApneaIndex is published (reportable=true) only when the confirmed count exceeds a per-night Poisson chance expectation; otherwise findings carry belowChance=true + pSpurious and the index is withheld (R5).',
        window:'window.overlapMin / overlapUnionMin = merged-union minutes where ≥2 nodes coincide (NOT a sum of pairwise overlaps); intersectionMin = N-way all-node overlap; pairwiseSumMin retained for transparency; nodesExcluded = dated nodes overlapping nothing (R3).',
    ...
    findings: fusion.findings.map(function(f){ return {
      tMs:f.tMs, time: f.tMs!=null?fmtDateTime(f.tMs):null, type:f.type, conf:f.conf,
      belowChance: f.belowChance!=null?f.belowChance:undefined, pSpurious: f.pSpurious!=null?f.pSpurious:undefined,
      durSec:f.durSec||null, nodes:f.nodes, sources:f.sources, meta:f.meta, note:f.note }; }),
    unmatched:{ desat: fusion.unmatched.desat.map(function(e){ return { tMs:e.tMs, time:fmtDateTime(e.tMs), conf:e.conf, meta:e.meta }; }),
                surge: fusion.unmatched.surge.map(function(e){ return { tMs:e.tMs, time:fmtDateTime(e.tMs), conf:e.conf }; }) },
...
var GRADE_MIRROR = { odi4:'validated', minSpo2:'measured', rmssd:'validated', sdnn:'validated',
                     glucoseCV:'validated', residualAHI:'measured' };
...
window.IntegratorDSP = {
  BUS:BUS, parseTimestamp, reconstructEventTMs, fmtClock, fmtClockS, fmtDate, fmtDateTime, fmtDayShort,
  nodeColor, NODE_COLORS, normalizeFile, dedupeRecs, recWindow, overlapInterval, runFusion,
  buildFusionExport, combineConf, glucoseMetricsInWindow,
  corroborateDesat, pickHRAuthority,
  gradeFor, GRADE_MIRROR, GRADE_SOURCES
};
