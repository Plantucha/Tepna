...
function _tchHat(like, ptsFn, metric) {
  ...
  // The corner SOURCES, parallel to `_cornerIds`. A consumer cannot resolve a weight by node NAME:
  // `_cornerIds` disambiguates duplicates ("PpgDex 1"/"PpgDex 2"), so two corners of the same node
  // share a name and differ only by label. Published alongside so a lookup can match by IDENTITY,
  // which is the only key that is unique. DEEP-AUDIT-V-FOLLOWUPS F5 required this re-key; it was
  // stamped DONE with this half unbuilt, and the result was that BOTH PpgDex corners silently
  // dropped out of the inverse-variance mean and the published figure was the remaining corner
  // verbatim (measured 07-19: weights {0.0385, 0.4808, 0.4808}, values [45, 32.6, 31.7], published
  // 31.7 — ECGDex's own value — against an honest label-aligned mean of 32.64).
  var _cornerSrcs = [best.A, best.B, best.C];
    ...
    .map(function (g) {
      ...
      if (tch && tch.ok && rm && rm.values) {
        ...
        rm.values.forEach(function (o) {
          // BY IDENTITY, not by name. `tch.weights` is keyed by the disambiguated corner label, so
          // `tch.weights[o.node]` is undefined for every corner whose node name is shared — which is
          // exactly the finger/wrist PpgDex pair this fusion exists to combine.
          var _ci = tch.cornerSrcs ? tch.cornerSrcs.indexOf(o.src) : -1;
          var w = _ci >= 0 ? tch.weights[tch.cornerIds[_ci]] : null;
          if (w != null) {
            _acc += w * o.v;
            _ws += w;
          }
...
function fitClockDrift(aTimes, bTimes, opts) {
  ...
  var _h = (function () {
    ...
    /* ONLY WHEN THERE IS A TREND TO BE LINEAR ABOUT. A slope-ratio test is meaningless on a fit whose
       true drift is ~0: the two halves then measure noise around zero, their ratio is random and their
       signs are a coin flip, so the test refuses the CLEANEST case there is — a perfectly aligned pair
       with a constant offset. (Caught by the existing planted-1.5 s-offset fixture, which is exactly
       that case and which this gate failed on its first draft.) Require the excursion the fit CLAIMS
       to clear the scatter it sits in before asking about its shape; below that, curvature is not
       measurable and the leg stays silent rather than convicting. */
    var _resid = _wrapped && _wrapped.residRmsMs != null ? _wrapped.residRmsMs : null;
    if (_resid == null) return null;
    var _span = rows[rows.length - 1].tMs - rows[0].tMs;
    if (!(Math.abs(slope) * _span > 3 * _resid)) return null;
  ...
  return {
    ...
    /* CURVATURE — a linearity check the beat-match statistic structurally cannot make.
       `confident` used to certify a fit on correspondence alone: how often the blocks agree about
       WHICH beat pairs with which. That measures whether the pairing is real, not whether the offset
       walks in a straight line, so a smoothly DECELERATING clock walk certified as a linear drift
       (measured 2026-09-12: −137 ppm, confident:true, over a 0 → −2,030 ms decelerating walk; the
       same shape on 08-26). A line has the SAME slope in both halves of its own blocks; a walk that
       flattens does not. Split the fitted blocks, fit each half, and compare.
       The ratio bound is deliberately loose (1/3) and is NOT corpus-tuned — it is set to refuse the
       shape (a half that has flattened toward zero, or reversed) rather than to grade a fit. */
    _halves: _h,
    ...
    /* The search window BOUNDS what drift can be seen: a pair drifting faster than this walks out of
       ...
       number hides. */
    /* Over the FITTED BLOCK span, not A's full span. Using A's span understates the reach of a fit
       that only spans part of the recording, and the error is in the dangerous direction: on
       2026-09-12 A spans 387 min while the fitted blocks span 270, so reach read 129 ppm against a
    ...
    /* BOTH legs, and they answer different questions. Correspondence says the PAIRING is real; the
    ...
    reason:
      chance == null
        ? 'no control blocks'
        : medFrac < 2 * chance
          ? 'correspondence does not clear its own chance control'
          : _h && !_h.sameSign
            ? 'the two halves of the fit drift in OPPOSITE directions — not a constant rate'
            : _h && _h.ratio != null && _h.ratio < 1 / 3
              ? 'the fit CURVES: half-slopes ' + _h.firstPpm + ' / ' + _h.secondPpm + ' ppm (ratio ' + _h.ratio + ') — a walk, not a drift'
              : null,
    perBlock: rows
  };
}
...
function runFusion(recs, opts) {
  ...
  return {
    ...
    /* `beatCheck` rides here with its siblings. `detectClockSkew` computed it and `runFusion` then
       rebuilt `clockSkew` field-by-field WITHOUT it, so a beat-level cross-check on a verdict made
       by a coarse 30 s grid was computed on every fusion and discarded — no consumer anywhere.
       Wired rather than deleted because it joins a block that IS consumed (`fusion.clockSkew` is
       passed up at the end of runFusion); publishing it into an unread key would have recreated
       the defect one field over. */
    clockSkew: {
      findings: skew.findings,
      applied: skewApplied,
      pairs: skew.pairs,
      fits: skewFits,
      beatCheck: skew.beatCheck || null,
      vetoes: skew.vetoes && skew.vetoes.length ? skew.vetoes : undefined
    },
    hrSource: hrSource,
    pairs: pairs,
    apnea: apnea,
