...
function hrAgreement(sources, opts) {
  ...
  for (var p = 0; p < ref.pts.length; p++) {
    var t = ref.pts[p].t;
    var vals = [];
    for (var q = 0; q < idx.length; q++) {
      var hit = q === 0 ? ref.pts[p] : near(idx[q].pts, t);
...
function runFusion(recs, opts) {
  ...
  return {
    ...
    clockSkew: { findings: skew.findings, applied: skewApplied, pairs: skew.pairs, fits: skewFits },
    hrSource: hrSource,
    pairs: pairs,
    apnea: apnea,
