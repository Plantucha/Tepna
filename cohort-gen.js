...
(function (global) {
  ...
  var SEVERITY = [
    ...
    /* `lognorm` replaces the uniform draw for this stratum ONLY — see `ahiInStratum`. The bound is no
       longer a ceiling but a refusal guard well above anything observed (real max 286.9). */
    { key: 'severe', ahi: [30, 300], w: 1, lognorm: { shift: 29, mu: 3.076, sigma: 0.731 } }
  ];
  ...
  function rsaGainFor(rmssd) {
    return clamp(Math.sqrt(Math.max(0, rmssd * rmssd - 7 * 7)) / 19.15, 0.06, 4.35);
  }
  ...
  /* ── within-stratum AHI shape ──────────────────────────────────────────────
     A uniform draw inside a stratum has no skew and no tail BY CONSTRUCTION, and for the severe band
     that is not a small inaccuracy — it is the whole difference between the synthetic and real
     ODI-4/AHI relationship. Measured against SHHS1 (n=3008 severe nights, 2026-09-13):

         severe stratum   p25    MED    p75     p95     max
         real            39.2   50.7   67.7   101.2   286.9   right-skewed, mean 56.5 > median 50.7
         uniform[30,80]  42.5   55.0   67.5    77.5    80.0   mean == median by construction

     The quartiles very nearly agree; the TAIL does not, and 13.5 % of real severe nights sat above the
     old hard ceiling of 80 — unreachable, not merely under-sampled, and exactly where a detector
     saturates. So the severe band now draws a shifted log-normal fitted to the real median and p95
     (mu/sigma on AHI-29), and its upper bound becomes a refusal guard rather than a ceiling.

     ⚠️ THE OTHER THREE STRATA ARE DELIBERATELY LEFT UNIFORM. Measured on the same corpus, `mod` is
     already near-exact (real median 22.7 against a uniform 22.5) and `mild` is close (11.5 vs 10).
     `none` differs more (4.0 vs 2.5) but rests on 61 nights and reflects SHHS having few healthy
     participants clustered just under the cut — fitting that would import the cohort's RECRUITMENT,
     which is a property of who was enrolled and not of physiology.

     ⚠️ AND THE STRATUM MIX IS NOT FITTED EITHER, on purpose. `cohort-gen` is a coverage harness; fit
     the mix to SHHS and 1.2 % of patients are healthy and the none/mild path stops being exercised.
     There is also no single target to fit: the SAME 5136 nights are 58.6 % severe under SHHS's scored
     AHI and 3.1 % severe under a 4 % desaturation index. Fitting the mix would bake one scoring
     convention invisibly into every future experiment. Use post-stratification weights at analysis
     time instead (`tools/cohort-fit.mjs` computes them from whatever reference you name).

     ⚠️ EXACTLY ONE `rng()` CALL, like the uniform draw it replaces. This is load-bearing: adding a
     second would shift every subsequent draw for that patient, so every field after `baseAHI` would
     change for every seed and no previously generated cohort would reproduce. The normal variate is
     obtained by inverting the standard normal CDF on the single uniform instead. Non-severe patients
     are therefore byte-identical to v1.9. */
  function invNorm(u) {
    // Acklam's rational approximation, |error| < 1.15e-9 — ample here and dependency-free.
    if (!(u > 0 && u < 1)) return 0;
  ...
  function ahiInStratum(sev, u) {
    if (!sev.lognorm) return +(sev.ahi[0] + u * (sev.ahi[1] - sev.ahi[0])).toFixed(1);
    var L = sev.lognorm;
    var v = L.shift + Math.exp(L.mu + L.sigma * invNorm(u));
    return +clamp(v, sev.ahi[0], sev.ahi[1]).toFixed(1);
  }
  ...
  function arcNightAHI(arc, baseAHI, i, nNights, interventionNight) {
    ...
    switch (arc) {
      case 'improving':
        return baseAHI * (1 - 0.55 * t);
      case 'worsening':
        return baseAHI * (1 + 0.7 * t);
      case 'intervention':
        return i < interventionNight ? baseAHI * (0.9 + 0.2 * (i / Math.max(1, interventionNight))) : baseAHI * (0.18 + 0.05 * (nNights - i));
      default:
        return baseAHI; // flat (jitter added by caller)
    }
  }
  ...
  function sampleProfile(seed) {
    ...
    var baseAHI = ahiInStratum(sev, rng()); // ONE rng() call, as before — see `ahiInStratum`
    var arc = pick(rng, ARC);
    var nNights = 1 + Math.floor(rng() * 12); // variable 1–12 (longitudinal lane)
    var interventionNight = arc === 'intervention' ? clamp(1 + Math.floor(rng() * (nNights - 1)), 1, Math.max(1, nNights - 1)) : -1;
    ...
    var hasRR = chance(rng, 0.9); // drives ECGDex + PulseDex
    ...
    return {
      ...
      arc: arc,
      nNights: nNights,
      interventionNight: interventionNight,
      cpapState: cpapState,
      glycemic: glyc,
      nocturnalHypo: hypoFlag,
      dawnPhenomenon: dawnFlag,
  ...
  function buildNightConfigs(profile, rng) {
    ...
    for (var i = 0; i < profile.nNights; i++) {
      ...
      var _ahiJit = Math.max(0, ahi * (0.9 + rng() * 0.2)),
        _ahiCap = 95;
      ...
      cfgs.push({
        n: i + 1,
        date: profile.year + '-' + p2(profile.month) + '-' + p2(day),
        ...
        rsaGain: +rsaGainFor(rmssd).toFixed(3),
  ...
  function injectDropout(csv, frac, seed) {
    ...
    var lo = Math.round(nData * 0.2),
      hi = Math.round(nData * 0.8) - perBurst;
  ...
  function patient(seed, opts) {
    ...
    opts = opts || {};
    var only = opts.only
      ? { oxy: opts.only.indexOf('oxy') >= 0, rr: opts.only.indexOf('rr') >= 0, gluco: opts.only.indexOf('gluco') >= 0, hrv: opts.only.indexOf('hrv') >= 0 }
    ...
    var timelines = cfgs.map(function (cfg, i) {
      return S.masterTimeline(cfg, (seed + i * 7919) >>> 0);
    });
    ...
    var truthAHI = nights.map(function (nt) {
      return nt.cfg.ahi;
    });
    var groundTruth = {
      ...
      nights: nights.map(function (nt) {
        var apneas = nt.groundTruth.events.filter(function (e) {
        ...
        return {
          ...
          durSec: nt.groundTruth.durSec,
          ahiTruth: nt.cfg.ahi,
          cpap: nt.cfg.cpap,
          gluc: nt.cfg.gluc,
          rmssdTarget: nt.cfg.rmssd,
          nApnea: apneas.length,
          nPB: nt.groundTruth.periodicBreathing.length,
          eventsTMs: apneas.map(function (e) {
            return e.t0Ms;
          })
        };
      }),
  ...
  function buildCpapEdfSet(night, profile, seed) {
    ...
    var recDur = 60,
      R = Math.max(1, Math.round(durSec / recDur));
    ...
    var leakStart = Math.round(nH * (0.4 + rng() * 0.3)),
      leakLen = Math.round(nH * 0.12);
    ...
    var PLD = {
      ...
      signals: {
        'Press.2s': mkSig(
          nH,
          function (i) {
            return setPress + Math.sin(i / 60) * 0.2;
          },
          0.5,
          'cmH2O'
        ),
        ...
        'Leak.2s': mkSig(
          ...
          function (i) {
            var l = baseLeakLs;
            if (bigLeak && i >= leakStart && i < leakStart + leakLen) l = 0.55 + rng() * 0.2;
            return l;
          },
          0.5,
          'L/s'
    ...
    if (hasOxi) {
      for (var z = 0; z < n1; z++) {
        spo2[z] = 95 + Math.round(rng() * 2);
        pulse[z] = 56 + Math.round(rng() * 8);
      }
    } else {
      for (var z2 = 0; z2 < n1; z2++) {
        spo2[z2] = -1;
        pulse[z2] = -1;
      }
    }
