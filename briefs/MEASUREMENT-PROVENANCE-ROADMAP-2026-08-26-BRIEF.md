...
**Status:** PROPOSED (🟢 **RATIFIED — the schedule decision this brief was parked on WAS MADE on 2026-09-15.** `OWNER-DECISION-QUEUE-2026-09-03-BRIEF.md` D5: *"ALL THREE RATIFIED 2026-09-15"*, this roadmap among them. Stamped here **2026-09-17 (Kestrel)** — and the reason it needed stamping is the finding: this file carried **ZERO** mentions of the ratification for two days while its own header read *"the owner has not scheduled it"*, so every reader saw a programme waiting on a decision that had already been taken. The queue records the same failure twice before (D6, twelve days; D3, nine days), which is why the linkage is now proposed as a `docs-ledger` check rather than another note: a decision the queue marks ruled must be back-referenced by every brief it names, the same bidirectional shape check 8 already enforces for residue keys. **NEXT STEP — SPAWNED 2026-09-17: `MEASUREMENT-INSTANCE-CONTRACT-2026-09-17-BRIEF.md`.** It executes the §1+§2 measurement-instance contract brief, staged one node at a time, **OxyDex first** per §3. The MINOR bump and its staged fixture re-stamp are authorised work. ⚠️ The park reason that follows is retained as HISTORY and is FALSE as of 2026-09-15. HISTORY — parked 2026-09-02 — drain triage, Kestrel: a ROADMAP whose first unit, the §1+§2 measurement-block schema contract, is a MINOR schema bump that re-stamps every node's fixtures in stages — §📦 makes its timing the owner's call, and the owner has not scheduled it; none of the six done-when items is started and none can start before that decision. Owner: the owner (schedule) → Kestrel (spawns the §1+§2 contract brief when scheduled). Not to be executed during the drain) · **Created:** 2026-08-26 · **Follows:** `ACQ-EVIDENCE-CONTRACT-2026-08-24-BRIEF.md` (the acquisition half of this chain — 21/22 §21 criteria met, last one in flight) · **Relates:** CLAUDE.md §🔒 Clock Contract, §🎫 evidence ladder, §🔏 provenance gates, `docs/LEXICON.md`
