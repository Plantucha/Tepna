...
| PpgDex | rMSSD · meanHR · SpO₂-proxy where emitted | the seam-refusal (`clock-seam`) must map to *no block*, never a block over a discontinuity (§∅ ruling 2026-09-17). **BUILT 2026-09-22 (Osprey):** `hr` + `rmssd` whole-record blocks, `clock-seam ⇒ measurement: null` + `measurementReason`, planted through the real parse path; §10 PASS (`audits/MEASUREMENT-EMITTER-PERF-PPGDEX-2026-09-22.json`); the SpO₂-proxy is not emitted (PpgDex publishes none); found limit → residue `2026-09-22-ppgdex-fixture-spread-not-bundle-reproducible` |
