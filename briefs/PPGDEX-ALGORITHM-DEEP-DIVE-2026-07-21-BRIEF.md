...
> | **#2 ★ correctRR fills** | 🔶 **OPEN, and partly advanced** | `:1999-2002` still does `if (bad) { v = ref; nCorr++; runBad++; flags.push(1); }` then `out.push(v)` — a rejected interval IS still replaced by the running reference. But a `flags` array now exists and is returned (`:2011`), so #2's *gap-flag* half is partly present; what is missing is the EXCLUDE half. |
