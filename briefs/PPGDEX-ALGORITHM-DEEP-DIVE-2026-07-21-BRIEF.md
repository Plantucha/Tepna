...
> | **#2 ★ correctRR fills** | ✅ **LANDED #2333, 2026-09-08** | The fill is GONE: `correctRR` now EXCLUDES a rejected interval instead of substituting the running median of the last 7 accepted. Verified on `origin/main` 2026-09-11 — the quoted `v = ref` survives only inside a comment recording the old behaviour, and a new `ppgdex-dsp · correctrr-excludes` group plants a 29.0 %-rejected series and reconstructs the old filling to prove the difference. ⚠️ The row below this one described it as OPEN for three days after it shipped; the committed corpus could not express the change (measured correction rates 0.00 / 0.00 / 6.25 % against the ~28.8 % it targets), which is why no fixture moved to signal it. |
