...
**Status:** IN-PROGRESS (**code complete; §1's corpus run is NO LONGER data-blocked — re-verified 2026-09-02**: the 2026-09-01 triage said "locally only 6 dates intersect"; the merged corpus root holds **37**, each with a >1 MB `Polar_H10_*_ACC.txt` and a >100 kB `CPAP/*_BRP.edf` (80 ACC dates × 183 BRP dates, basename-parsed with size floors). **SIZED 2026-09-04 — 79 intersecting dates, 2026-06-06 → 2026-08-31, and the corpus is FULLY PAIRED: the intersection EQUALS the ACC count, so every real ACC night has a BRP night and ACC is the binding side completely.** ACC via the tool's own `RespAccAnalysis.sessionStamp` (566 files, 0 unreadable), synthetic excluded, BRP keyed by DIRECTORY: 232 dates. ⚠️ **FIVE VALUES WERE PRODUCED BEFORE THIS ONE — 37 → 71 → 77 → 76 → 79 — each from a DIFFERENT mechanism, and not one of them errored:** (1) 37, stale; (2) 71, a predicate scoped to `uploads/` while `smoketest-captures/` and `workshop-imports/` hold 241 of the 566 ACC files; (3) 77, built on a BRP count keyed by FILENAME — but `CPAP/YYYYMMDD/` is the night and **133 of 1027 BRP files (12 %) sit in a directory whose date differs from their filename** (post-midnight sessions), so filename-keying gives 223 and directory-keying 232; (4) 76, after dropping `20260513` — its ONLY ACC evidence was a single file under `uploads/synthetic/`, which would have made it the first pilot date and validated the pipeline against fabricated data in a real-data paper; (5) 79, correct. **Every one of the four wrong values looked exactly like a corpus size.** Do not hand-roll a corpus filter beside a tool that has one — §12 records that defect and it was rebuilt repeatedly here by readers of this brief. Also: each BRP exists as byte-identical duplicates across `uploads/`, `smoketest-captures/`, `vigil-archive/` and `Ecg nightly/`, so file counts are inflated and staging must de-dup by basename or `linkSync` throws EEXIST. Not parked — §1 is executable. §6A's attribution box is UNTICKED, see R3) · **Created:** 2026-07-21 · **Residue:** 2026-09-02-motiondex-respratemethod-unexported, 2026-09-02-motiondex-epoch-count-disagrees · **§6A CLOSED 2026-09-03 (#2136)** — `respRateMethod` now reaches a consumer (export `motiondex-dsp.js:1449` → adapter `integrator-dsp.js:644` → `fuseRespirationRate` `:3301`), gated by `seam-parity`. **ONE box remains open and it is the figures item, re-measured 2026-09-03: still 0 `<img>` across all three papers** (`acc-respiratory-rate`, `cpap-flow-reference`, `effort-typing-null`), 27 unrelated files in `papers/figures/`. So the brief is code-complete with a single DATA/figure deliverable outstanding — do not read "code complete" as "done".
