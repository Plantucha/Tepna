...
**Status:** DONE — 2026-09-20 (the one remaining item — figure regeneration — LANDED: three figures off the live canvases wired into `papers/acc-respiratory-rate.html`, headline reproduced bit-for-bit; two residues filed for what the regeneration exposed, neither a regen item. Previously IN-PROGRESS 2026-09-20 — NOT owner-blocked: the editorial half is executed 2026-09-07. ONE item remains and it is executable — regenerate the figures into `papers/figures/` with the tool; verified 2026-09-20: one resp/acc file there and no regen commit on that path. That is the whole remainder) · **Created:** 2026-07-21 · **Residue:** 2026-09-02-motiondex-respratemethod-unexported, 2026-09-02-motiondex-epoch-count-disagrees, 2026-09-20-sibling-papers-have-no-figure-source · **§6A CLOSED 2026-09-03 (#2136)** — `respRateMethod` now reaches a consumer (export `motiondex-dsp.js:1449` → adapter `integrator-dsp.js:644` → `fuseRespirationRate` `:3301`), gated by `seam-parity`. ~~**ONE box remains open and it is the figures item, re-measured 2026-09-03: still 0 `<img>` across all three papers** (`acc-respiratory-rate`, `cpap-flow-reference`, `effort-typing-null`), 27 unrelated files in `papers/figures/`. So the brief is code-complete with a single DATA/figure deliverable outstanding — do not read "code complete" as "done".~~ (superseded 2026-09-20 — see the head clause) · **Owner ruling 2026-09-07:** §1 — **retract and restate** the ACC-respiration figure with the current-code number — **Magpie**. The field half rides box day 4 (the Polar Logger session, `CAPTURE-HOST-FOLLOWUPS-II` V1/V2): scheduled, no date set. · **DRAIN 2026-09-11 (Brief runner) — RE-VERIFIED, UNCHANGED.** No commit since the 2026-09-07 execution. §1's paper-editorial half is confirmed landed in the tree: `papers/acc-respiratory-rate.html` carries the restated **18,856** epochs and **91.7 %** within 2 brpm. Status stays IN-PROGRESS for the remaining FIELD work, which no dev-checkout session can schedule.
...
- [x] Figures regenerated by that tool into `papers/figures/` — **LANDED 2026-09-20.** `tools/resp-acc-headless.mjs --figures` run on the 50-ACC / 192-CPAP-date stage (hardlinked, 49 paired, 14 scored, 8,057 epochs): three PNGs off the live canvases — Bland–Altman, coverage, per-night — plus `cohort-manifest.json`, wired as Figures 1–3 into `papers/acc-respiratory-rate.html`, served twins created by hand (build-docs walks `docs/`, so a NEW asset never enters its set until its twin exists). Headline reproduced bit-for-bit (MAE 1.10, 0.96–1.27, −0.53, 3.07, ±5.93, 90.8 %, r 0.351). ⚠️ The tool emits figures for the ACC paper ONLY; `cpap-flow-reference` and `effort-typing-null` still carry zero `<img>` because nothing produces one for them — a capability question, filed as residue, not a regeneration left undone. ⚠️ The regeneration also exposed that the retracted cohort description (26 nights / 172 h) survives in the paper's title, abstract, methods, statistics and limitations — filed as residue; only the two sentences the figures directly contradict were restated. — ORIGINAL ITEM: **THE ACTUAL BLOCKING ITEM.** Measured
