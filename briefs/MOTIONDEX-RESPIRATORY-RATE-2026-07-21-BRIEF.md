...
**Status:** IN-PROGRESS (🔴 **§1's PAPER-EDITORIAL HALF IS EXECUTED 2026-09-07 — the pre-fix figure is RETRACTED and RESTATED, owner-ruled the same day.** `papers/acc-respiratory-rate.html` published MAE **1.01** / **91.7 %** within 2 brpm over **18,856** epochs; those numbers predate #1042's cross-device alignment fix, whose own message records nine of sixteen nights scored against noise, and they were never reproducible from committed code. Reproduced 2026-09-07 by driving `resp-acc-analysis.html` through its own UI (`node tools/resp-acc-headless.mjs <stage> --url …`, 50 `Polar_H10_*_ACC.txt` hardlink-staged against the 192-night CPAP tree, EXIT=0, 0 console errors): **49 ACC/CPAP-paired nights, 8,057 epochs, MAE 1.10 (95 % CI 0.96–1.27), 90.8 % within 2 brpm, bias −0.53, RMSE 3.07, LoA ±5.93, r 0.351**; null baseline 1.64; coverage curve 0.84 @ 85 %, 0.63 @ 70 %; reference self-noise 0.74. Nothing hand-typed — every value read from that run. **The estimator is WORSE than published and still beats the null: the conclusion survives, the magnitude does not.** ⚠️ Two table rows are **WITHDRAWN rather than updated** — event-free epochs, and the shipped-predecessor comparison — because the current tool does not emit those configurations, and carrying a number forward under a corrected paper is the exact error the correction exists to fix. ⚠️ The paper also disagreed with ITSELF before this: the abstract said bias −0.46 while the table said +0.38. **§1's FIELD half — a re-measurement on a capture-box night — remains PARKED (box day 4) and is not claimed.** Prior: **code complete; §1's corpus run is NO LONGER data-blocked — re-verified 2026-09-02**: the 2026-09-01 triage said "locally only 6 dates intersect"; the merged corpus root holds **37**, each with a >1 MB `Polar_H10_*_ACC.txt` and a >100 kB `CPAP/*_BRP.edf` (80 ACC dates × 183 BRP dates, basename-parsed with size floors). Not parked — §1 is executable and owes a sizing first. ⚠️ **SUPERSEDED — SIZED 2026-09-06 (Magpie): §1's corpus run ALREADY RAN on 2026-09-05 and this sentence predates it by three days.** Row `2026-09-05-respacc-epochs-predate-alignment-fix` records the result: staged over 51 H10 ACC sessions / 24 CPAP-paired dates → **49 nights grouped · 19 confidently locked · 14 drift-consistent · 8,057 epochs · 71.5 h · MAE 1.10**. So §1 is EXECUTED-NOT-PUBLISHED, and what remains is an OWNER CALL, not a work-unit: current code reproduces NEITHER published figure and the paper still carries the pre-fix **MAE 1.01 / 91.6 % within 2 brpm**, so regenerating a published preprint headline is the owner's decision. **Do not re-derive the 37.** Re-measured on the merged corpus 2026-09-06: **74 H10 ACC dates (>1 MB) × 227 BRP dates (>100 kB) → 71 intersecting**, against the 24 CPAP-paired dates the 09-05 run actually used and the 37 this header still quotes from an older tree (80 × 183). A larger-cohort re-run is therefore possible and better-powered, but lands on the same owner gate. **BANDS, PRE-STATED so a re-run cannot set them afterwards:** validating = per-night MAE ≤ 2 brpm on CONFIDENTLY-LOCKED nights (the paper's own within-2-brpm claim); expected lock rate **35–50 %** of staged nights, from the only two post-fix runs there are (19/49 = 39 % on 2026-09-05, 7/16 = 44 % in #1042); **< 25 %** would say the alignment fix regressed and **> 60 %** that the corpus improved. ⚠️ A "how many produced a number" band belongs on LOCKED nights, NEVER on staged ones — the drift check refusing an off-model night is the tool working, not the run failing, and #1042 is why: nine of sixteen nights had been scored against NOISE (`recoverOffset` returning the argmax of a noise field at peak r 0.16–0.20 over a ±90-min search) while that same drift check had already flagged every one off-model and the code ignored its own verdict. An earlier proposed band of ≥ 30 of 37 (81 %) was set from this header before the 09-05 row existed and is unreachable by construction. Nothing needs building: `tools/resp-acc-headless.mjs` already drives the page end-to-end and its `--figures` writes the three canvases, which is the one open box. §6A's attribution box is UNTICKED, see R3) ⚠️ **DRAIN 2026-09-19 — 2 landings in `motiondex-dsp.js` / `papers/acc-respiratory-rate.html` since 2026-09-07.** **#2348** is the retraction this header already records. **#2609** is new to it: a clock seam now REFUSES `immobileFrac`. **Recorded, NOT assessed** — whether a seam refusal touches this brief's respiratory-rate done-whens is the owner's reading, and #2609 changed a different quantity. · **Created:** 2026-07-21 · **Residue:** 2026-09-02-motiondex-respratemethod-unexported, 2026-09-02-motiondex-epoch-count-disagrees · **§6A CLOSED 2026-09-03 (#2136)** — `respRateMethod` now reaches a consumer (export `motiondex-dsp.js:1449` → adapter `integrator-dsp.js:644` → `fuseRespirationRate` `:3301`), gated by `seam-parity`. **ONE box remains open and it is the figures item, re-measured 2026-09-03: still 0 `<img>` across all three papers** (`acc-respiratory-rate`, `cpap-flow-reference`, `effort-typing-null`), 27 unrelated files in `papers/figures/`. So the brief is code-complete with a single DATA/figure deliverable outstanding — do not read "code complete" as "done". · **Owner ruling 2026-09-07:** §1 — **retract and restate** the ACC-respiration figure with the current-code number — **Magpie**. The field half rides box day 4 (the Polar Logger session, `CAPTURE-HOST-FOLLOWUPS-II` V1/V2): scheduled, no date set. · **DRAIN 2026-09-11 (Brief runner) — RE-VERIFIED, UNCHANGED.** No commit since the 2026-09-07 execution. §1's paper-editorial half is confirmed landed in the tree: `papers/acc-respiratory-rate.html` carries the restated **18,856** epochs and **91.7 %** within 2 brpm. Status stays IN-PROGRESS for the remaining FIELD work, which no dev-checkout session can schedule.
