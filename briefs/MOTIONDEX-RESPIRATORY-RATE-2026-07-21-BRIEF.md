...
**Status:** IN-PROGRESS (**code complete; §1's corpus run is NO LONGER data-blocked — re-verified 2026-09-02**: the 2026-09-01 triage said "locally only 6 dates intersect"; the merged corpus root holds **37**, each with a >1 MB `Polar_H10_*_ACC.txt` and a >100 kB `CPAP/*_BRP.edf` (80 ACC dates × 183 BRP dates, basename-parsed with size floors). **SIZED 2026-09-04 — and the corpus GREW: the intersection is now 71, not 37.** Predicate stated because ours and the 09-02 count disagree and only one of them can be re-run: measured over `/srv/data/tepna-corpus/uploads` (the only local root holding big ACC — 325 files; `uploads/` and `workshop-imports/` hold none), `find -name 'Polar_H10_*_ACC.txt' -size +1M` → **72 ACC dates**, `find -iname '*_BRP.edf' -size +100k` → **232 BRP dates**, intersection **71**, spanning 2026-05-13 → 2026-08-31. Input volume **6.3 GB / 6795706558 B**, mean **91 MB per date**. ⚠️ The BRP side is what moved (183 → 232) — the nightly CPAP harvest has been running — while ACC reads 72 against the 09-02 figure of 80, and that 8-date gap is NOT reconciled: it may be a different root, a different size floor, or dates the harvest pruned. Do not average the two counts; re-run the predicate above. Not parked — §1 is executable. §6A's attribution box is UNTICKED, see R3) · **Created:** 2026-07-21 · **Residue:** 2026-09-02-motiondex-respratemethod-unexported, 2026-09-02-motiondex-epoch-count-disagrees · **§6A CLOSED 2026-09-03 (#2136)** — `respRateMethod` now reaches a consumer (export `motiondex-dsp.js:1449` → adapter `integrator-dsp.js:644` → `fuseRespirationRate` `:3301`), gated by `seam-parity`. **ONE box remains open and it is the figures item, re-measured 2026-09-03: still 0 `<img>` across all three papers** (`acc-respiratory-rate`, `cpap-flow-reference`, `effort-typing-null`), 27 unrelated files in `papers/figures/`. So the brief is code-complete with a single DATA/figure deliverable outstanding — do not read "code complete" as "done".
