...
**Status:** IN-PROGRESS (**code complete; §1's corpus run is NO LONGER data-blocked — re-verified 2026-09-02**: the 2026-09-01 triage said "locally only 6 dates intersect"; the merged corpus root holds **37**, each with a >1 MB `Polar_H10_*_ACC.txt` and a >100 kB `CPAP/*_BRP.edf` (80 ACC dates × 183 BRP dates, basename-parsed with size floors). **SIZED 2026-09-04 — 77 intersecting dates, 2026-05-13 → 2026-08-31**, sourced through the TOOL'S OWN RULE (`RespAccAnalysis.sessionStamp`, loaded as the tool loads it, fail-loud if unavailable) over `/srv/data/tepna-corpus`: 566 ACC files / 0 unreadable → **80 ACC dates**; 1029 BRP files → 223 BRP dates; intersection **77**, ACC's 80 the binding constraint. The 0-unreadable is direct evidence both filename layouts are handled — stronger than inferring it from §12. ⚠️ TWO hand-rolled predicates got this wrong first and §12 is about exactly that: one reported 71 (pattern fine, scoped to `uploads/` while `smoketest-captures/` and `workshop-imports/` hold 241 of the 566 files), another reported 47 by rebuilding the retired `_YYYYMMDD_HHMMSS_ACC` regex §12 records as the cause of a pre-flight lying about corpus size — the defect fixed on main as ONE RULE, NOT A SECOND COPY. Neither errored. Do not hand-roll a corpus filter beside a tool that has one. A `_[0-9]{8}` date pattern also admits `02849638`, the H10 device id, as a date — inert here, live for anyone reusing it. Not parked — §1 is executable. §6A's attribution box is UNTICKED, see R3) · **Created:** 2026-07-21 · **Residue:** 2026-09-02-motiondex-respratemethod-unexported, 2026-09-02-motiondex-epoch-count-disagrees · **§6A CLOSED 2026-09-03 (#2136)** — `respRateMethod` now reaches a consumer (export `motiondex-dsp.js:1449` → adapter `integrator-dsp.js:644` → `fuseRespirationRate` `:3301`), gated by `seam-parity`. **ONE box remains open and it is the figures item, re-measured 2026-09-03: still 0 `<img>` across all three papers** (`acc-respiratory-rate`, `cpap-flow-reference`, `effort-typing-null`), 27 unrelated files in `papers/figures/`. So the brief is code-complete with a single DATA/figure deliverable outstanding — do not read "code complete" as "done".
