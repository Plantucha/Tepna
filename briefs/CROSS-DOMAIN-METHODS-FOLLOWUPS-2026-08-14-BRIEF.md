...
**Status:** IN-PROGRESS — **Line refs reconciled 2026-09-25 against `origin/main`: the §1 withdrawal note (`WITHDREW`) is at `integrator-tch.js:42` and its companion (`carries ZERO information`) at `tools/tch-per-epoch-rho.mjs:120`, so the `:41`, `:116` and `:105` citations below have drifted — cite the identifiers.** re-verified 2026-09-25, Kestrel triage, docs only: the three ticks still hold on `origin/main` (`integrator-tch.js:41` carries the §1 withdrawal note; `tools/hostaxis-estimator-bakeoff.mjs`, `tools/eqc-run.mjs`, `tools/tch-per-epoch-rho.mjs` all exist) and both remainders are as blocked as on 09-02 — the `[~]` rides the next detector change, the `[⛔]` is a ResMed module the owner confirmed does not exist; NO CHANGE. Previously 2026-08-26 (⚠️ **the 2026-08-20 audit fixed the BOXES and left the STATUS contradicting them.** That audit found 3 of 5 Done-when boxes already satisfied and merely unticked, ticked them, and wrote *"nothing here is unstarted work"* — while the label stayed `PROPOSED`, which means exactly *unstarted*. Corrected to IN-PROGRESS 2026-08-26 after re-verifying all three ticks in source rather than trusting the audit: `integrator-tch.js:41` carries the §1 withdrawal note, `tools/tch-per-epoch-rho.mjs:116` the companion, and both `tools/hostaxis-estimator-bakeoff.mjs` and `tools/eqc-run.mjs` exist on `main`. **NOT DONE, and neither remaining item is work anyone can pick up:** one is `[~]` awaiting a real detector change to exercise the §7 protocol, the other `[⛔]` hardware-blocked on a ResMed oximeter module the owner confirmed does not exist — a purchase, not a cable.) · **Created:** 2026-08-14 · **DRAIN 2026-09-02 (Osprey) — re-verified; status deliberately LEFT at IN-PROGRESS.** Both remainders confirmed still blocked: one `[~]` awaits a real detector change to exercise the section-7 protocol (opportunistic — it cannot be scheduled, only ridden along with the next detector work), one `[BLOCKED]` on the same non-existent ResMed oximeter module as [[R5-HR-TRIPLET-REFERENCE-2026-07-12]]. ⚠️ **Not flipped to PROPOSED**, because this brief's own header records that exact flip as the 2026-08-20 error — PROPOSED means unstarted, and 3 of 5 boxes are verified done in source. **Owner: the OWNER for the hardware half; whoever next touches a detector for the other. Next step:** none schedulable. · **DRAIN 2026-09-06 (Magpie) — re-verified independently; status UNCHANGED at IN-PROGRESS, and a DONE-or-PROPOSED binary does not fit this brief.** Boxes on today's tree: **7 `[x]` · 1 `[~]` · 1 `[⛔]`** — not DONE (two remain), not PROPOSED (seven verify in source, and this header already records that flip as the 2026-08-20 error). ⚠️ **Both `file:line` references in this header have DRIFTED and now read as false negatives**: the §1 withdrawal note is at `integrator-tch.js:42` (was :41), the companion at `tools/tch-per-epoch-rho.mjs:105` (was :116). Cite the IDENTIFIER, not the line — and note a grep for `withdraw` misses BOTH, because the code says **WITHDREW**; search `WITHDREW` and `carries ZERO information`. Both notes, and `tools/hostaxis-estimator-bakeoff.mjs` + `tools/eqc-run.mjs`, confirmed present on `main`. · **DRAIN 2026-09-19 (Osprey): both remainders STILL BLOCKED — header unchanged. Surface derived (13 files): 7 landings since 2026-09-06, none of which unblocks the `[⛔]` hardware item (a ResMed oximeter module that does not exist). ⚠️ **The `[~]` item may now be RIDEABLE and I am flagging it rather than claiming it:** it awaits *"a real detector change to exercise the §7 protocol"*, and two candidates have since landed — #2540 (a publication threshold corrected in `capture-host/clock_offset.py`) and #2609 (a clock seam refusing `immobileFrac`). Whether either exercises §7's blind-analysis protocol needs a read of §7 against the diff, which this pass did NOT do. Next session: start there; it is the cheapest open thing in this brief.**
