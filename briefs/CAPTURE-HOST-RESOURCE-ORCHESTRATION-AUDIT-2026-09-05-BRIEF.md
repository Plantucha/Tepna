...
| `connected=False` during `_retry_sleep` | stall path today keeps `connected` true while sleeping; changing it moves `_LINK_EPOCH` semantics — residue row | unblocked 2026-09-25: the residue row `2026-09-05-retry-sleep-stale-connected` landed as #2274 — not executed |
