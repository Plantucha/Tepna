...
## 8 · What a stuck value means TO the ring — the vendor answer on record + a PRE-REGISTERED capture (2026-09-24, Finch)
...
- `docs/O2RING-FINGER-OFF-2026-09-19.md` (owner + Wren, pre-registered): **settled finger-off ⇒ exactly
  ...
  **3 are bracketed by pulsatile signal on both sides (worn)**: 221 / 411 / 3,968 samples (2026-08-05 ·
  ...
  (672 @ 100)**.
- The 2026-09-06 all-hands (fleet memory `zero-in-data-all-hands`; summarised in §8.2): **to the vendor
  SDK, 0 and 199 are signal and only 156 is special**. Wren's full-corpus co-timing found **~98 % of all
  0/199 occur while `sensorState` reads worn**. The ring's own state field therefore does not mark
  in-wear blanking, and run length is the only instrument.
...
Pre-stated confidence: S1 optical, moderate. The reason is reasoning, not a measurement: a torch
pressed to the finger should exceed what ambient cancellation absorbs, but nothing in the tree measures
that headroom for this ring. S5 `ACCRAW` ≥ 200, moderate (one LSB of accelerometer noise would break the run, in which
