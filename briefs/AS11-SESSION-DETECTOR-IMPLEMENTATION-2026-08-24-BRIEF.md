...
**Status:** DONE — 2026-09-25 (Kestrel triage, docs only. Every done-when is met on `origin/main` @ 60755013: increments 1–2 BUILT and in production (`capture.py` hook behind `as11_detector.enabled` default OFF, `config.example.yaml:458`; `/srv/tepna/SESSIONDETECT.csv` live at 40,263 rows on 09-19), the read-only property test-backed (`tests/test_as11_detector_read_only.py`), and **increment 3 CLOSED AS SATISFIED 2026-09-19 with a stated reopening criterion** — recorded at the TAIL of this line below, which a first triage pass today misread as "still an owner decision" from the head of the line; the cloud pre-verification (#3052) caught it. `capture-host-ci` is green on that sha. Residue, one row: `cpap_detect.py:8` and `:101` still say "the daemon hook / acting mode is a later increment", which is false now. ⚠️ Not stamped: a `settings_schema.py` row for `as11_detector.*` does not exist — a rollout detail, not a done-when. Previously PROPOSED, parked 2026-09-02 — increments 1–2 BUILT and running in production (83befab4 #1746; `/srv/tepna/SESSIONDETECT.csv` is live at 10195 rows, written today), and the boundary review is discharged. The remaining done-when item is increment 3: the acting-mode follow-up brief has never been spawned, and whether acting mode is in scope AT ALL is a lead/owner decision, not a measurement. ✅ **The READ-ONLY residue is CLOSED 2026-09-03**: `tests/test_as11_detector_read_only.py` enforces done-when item 3 across all three detector modules. It had been confirmed by reading and by nothing else — both `cpap_supervisor.py:13` and `cpap_shadow_runner.py:15` assert the property in a COMMENT, and a safety property whose only evidence is a comment regresses in one line while the comment still says it did not. The scan strips comments and KEEPS strings, because an AS11 operation is a method STRING handed to `as11_link.rpc()` — stripping literals would blind it to the very thing forbidden, and keeping comments would fail on the two sentences that promise the property. Verified by planting a real `EnterStandby` call into `cpap_supervisor` and watching that module's leg go red. **Owner:** Heron · **Next step:** none — increment 3 CLOSED AS SATISFIED 2026-09-19 with a stated reopening criterion (see increment 3 below) · ⚠ **RE-VERIFIED 2026-09-19 (Wren, box + tree):** nothing landed in the surface I checked since 09-03 (same paths as the parent; only #2151, the read-only test, on 09-03). BOX: `/srv/tepna/SESSIONDETECT.csv` is live — 40,263 rows, written 2026-09-19 12:08, 1,400–2,000 rows/day over the last eight days, 0–3 transitions/day. Increment 3 (acting mode) is still *"a later increment"* (`cpap_detect.py:101`) and remains a lead/owner decision, not checkable from code. One observation for that decision — CORRECTED the same day (Wren, T1 in host time): the production daemon already ACTS on therapy, and the witness it acts on is THIS detector — `auto_start` fires on the shadow poll's `Therapy` sighting (11/11 journal-bounded sessions since 08-31, sighting 3–17 s before the stream start). An earlier draft of this note said the detector "called the same night quiet"; that was a device-vs-host window error (parent brief, 2026-09-19 note). So increment 3 is not blocked on the detector being wrong — it is already the trigger; what "acting mode" would add beyond `auto_start` is the question to put to the lead) · **Residue:** 2026-09-25-cpap-detect-comments-say-later-increment · **Created:** 2026-08-24
...
4. ✅ **Increment 3 — CLOSED AS SATISFIED 2026-09-19 (lead ruling via Kestrel, on Wren's T1 measurement).** The detector already causes action: `auto_start` fires on the shadow poll's `Therapy` sighting — 11/11 journal-bounded sessions since 08-31, sighting 3–17 s before the stream start — so "make the detector act" is met by a mechanism this brief did not imagine, and a second actor on the same sighting would be a layer with no arbiter (owner's 2026-09-19 filter, question 5). **Reopening criterion, verbatim: an action at a therapy boundary that `auto_start` does not already take** — stopping capture at therapy end, triggering a spool pull, marking a session boundary for the folder. Name one and the increment is real; until then the box is closed, not unbuilt. The original scope, kept for the record: **acting mode (follow-up PR, after ≥1 clean shadow night):** the adapter calls
