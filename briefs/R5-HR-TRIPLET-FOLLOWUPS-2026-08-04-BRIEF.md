...
**Status:** IN-PROGRESS — 2026-09-17 (🟢 **§3's PARKED DECISION IS RULED AND EXECUTED — owner, 2026-09-17 (queue D3): switch to the MEAN, and make `hrStatMixed` comparability-keyed rather than name-keyed.** Both premises were verified in the source before the ask, because this header flagged its own read as second-hand: `oxydex-dsp.js` did ship `_median(b.hr)` with `hrStat: 'median-rate'`, and `integrator-dsp.js` did compute the flag from `_hrStatSet.length > 1`. Landed: OxyDex publishes `mean-rate`; the Integrator's decision is now a pure exported `hrStatMixed(stats)` keyed on a declared class map — `{rate-of-mean, mean-rate}` together because they were MEASURED to agree to 0.3σ, `median-rate` alone because it was measured not to — and it FAILS CLOSED twice, on an unrecognised name and on a null. It was extracted from the fusion body on purpose: the flag only publishes when the three-cornered hat resolves, which made the inline version untestable while being the one part worth testing. Two pre-existing groups were re-aimed rather than deleted, both having pinned the OLD behaviour deliberately — the epoch fixture was BUILT to discriminate (59 samples at 60 plus one at 600 ⇒ median 60, mean 69) and a group named *"cross-node epoch HR uses TWO different estimators"* asserted the very confound this removes. ⚠️ **No committed GATE-B fixture can express the change** — residue `2026-09-17-no-fixture-expresses-epoch-hrstat`; read "0 fixtures moved" as uninformative, not as export-inert. PRIOR STAMP: IN-PROGRESS — 2026-09-02 (statistic-naming BUILT across all producers and the Integrator consumer; the −0.299 bpm gap gate was upgraded from source-scan to **asserting the value** with an anti-vacuity leg. **PARKED on one owner decision**: §3's `median→mean` switch moves a published field and needs a ruling on `hrStatMixed` semantics — *per Heron's read, not independently re-verified*) · **Residue:** 2026-09-17-no-fixture-expresses-epoch-hrstat · **Created:** 2026-08-04 · **Spawned-by:** `R5-HR-TRIPLET-REFERENCE-2026-07-12-BRIEF.md` (bias item, executed) · **Affects:** every cross-node HR comparison — `oxydex-dsp.js` `oxyBuildEpochSeries`, `ecgdex-dsp.js` epoch builder, the HR three-cornered hat
