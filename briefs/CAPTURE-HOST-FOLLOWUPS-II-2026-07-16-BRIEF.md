...
**Status:** PROPOSED (parked 2026-09-02 — §2's V1–V5 remain the only open set and they are hardware-gated in three different ways, which is worth separating: **V1/V2** ⚠️ **THE HARDWARE GATE IS DISCHARGED — corrected 2026-09-19 (Heron).** The exports this clause says are needed ALREADY EXIST in the corpus and predate the claim: Verity `_GYRO` (`Polar_Sense_0C301E3F_20260621_060522_GYRO.txt` and a 06-09 sibling), Verity `_MAGN` (three files, 06-09 / 06-16 / 06-21) and an H10 `_ACC` (`Polar_H10_02849638_20260617_010616_ACC.txt`) — all June 2026, i.e. before this brief was created (07-16) and before it was parked (09-02). Names match the Polar-Sensor-Logger per-stream layout; I verified the FILES, not their provenance beyond that. So V1/V2 are **repo work now, not "phone + hardware, off the box"** — what remains is the byte-diff itself. ⚠️ **Why it read as absent, and it is a trap for the whole fleet:** `uploads/` is gitignored, so a corpus check run inside the mandated per-task worktree (§👥.1) finds nothing *by construction* — I made this exact error before catching it, and it must be run against the PRIMARY checkout or `$DEX_UPLOADS` (`docs/CORPUS-LOCATIONS.md`). Original text follows: need a Polar Sensor Logger export of a Verity `_GYRO`/`_MAG` and an H10 `_ACC` to byte-diff against our decoders (phone + hardware, off the box; the decoders themselves have synthetic known-answer coverage at `tests/test_polar_pmd.py:179-252`). **V4** needs a real NTP step on the box. **V5's apply path** needs an owner-authorized box deploy (`sudo bash deploy/enable-clock-control.sh`); V5's drift half is CLOSED — gate-backed by `deploy/check-system-files.sh` and surfaced nightly by `nightqc.system_file_drift`. ⚠ **V3 UNSUPPLIABLE, RETIRED 2026-09-02** (Kestrel deputy ruling, reversible by the owner): its residual asks for a byte-diff against a vendor PPI export, and Polar Sensor Logger never produced one — so no owner action closes it with the current vendor app. Retired rather than deleted, and rather than carried open forever; it appears in the 2026-09-02 drain report so the owner can reverse it. **RE-TRIAGED 2026-09-06:** nothing to verify in the `.py` — the parked state is correct and unchanged. V1/V2/V4/V5-apply are gated on inputs no repo-side change can produce (a Polar Sensor Logger export, a real NTP step, an owner-authorized box deploy); V3 stays retired; V5's drift half stays closed. Zero `- [ ]` items, so there is no open § to stamp — the header IS the state. **Owner:** Vigil box · **Next step:** retire V3's residual, then the PSL capture for V1/V2) · **Created:** 2026-07-16 · **Owner ruling 2026-09-07:** the V1/V2 Polar Logger session is **SCHEDULED** as box day 4. Order: attended ring session (2026-09-08) → RtPpg (`DEVICE-RATE-TRUTH` §5) → perturbed-clock (`KNOWN-CLOCK-ADVERSARIAL-CAPTURE`) → **Polar Logger**, one per night, each on a per-night go. No date beyond 09-08 is set.
...
events, O2Ring SpO₂ → OxyDex meanSpo₂ 96.1 %). ⚠️ **THAT SUMMARY WAS THE 2026-09-02 PARK TEXT AND IT
CONTRADICTED THE CORRECTIONS APPENDED ABOVE IT — rewritten 2026-09-25 (triage).** It read "§2 V1–V5
stay OPEN — all hardware-gated … V3 an OH1", while the same header already recorded V1/V2's gate
DISCHARGED (2026-09-19) and V3 RETIRED as unsuppliable (2026-09-02). Corrections were added at the top
and the original summary below was left asserting the superseded state, so the header disagreed with
itself about three of five items. **VERIFIED STATE 2026-09-25:** V1/V2 are REPO work — the corpus holds
the Verity `_GYRO`/`_MAGN` and H10 `_ACC` exports, so what remains is the byte-diff itself · V3 RETIRED,
unsuppliable by the current vendor app · V4 needs an observed NTP step on the box · **V5's apply path is
NO LONGER waiting on a ruling: `OWNER-DECISION-QUEUE-2026-09-03-BRIEF.md` D2 records
`CAPTURE-HOST-FOLLOWUPS-II` (`deploy/enable-clock-control.sh`) as ✅ AUTHORIZED 2026-09-15**, and that
script is present in the tree — so V5 needs the apply path RUN on the box, which is an execution step
under an authorization already granted, not an owner decision. V5's drift half stays CLOSED. So "no new
desk work available" is false for V1/V2 and "owner-gated" is false for V5.)
