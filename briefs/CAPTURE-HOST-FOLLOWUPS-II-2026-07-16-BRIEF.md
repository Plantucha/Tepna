...
**Status:** PROPOSED (parked 2026-09-02 — §2's V1–V5 remain the only open set and they are hardware-gated in three different ways, which is worth separating: **V1/V2** need a Polar Sensor Logger export of a Verity `_GYRO`/`_MAG` and an H10 `_ACC` to byte-diff against our decoders (phone + hardware, off the box; the decoders themselves have synthetic known-answer coverage at `tests/test_polar_pmd.py:179-252`). **V4** needs a real NTP step on the box. **V5's apply path** needs an owner-authorized box deploy (`sudo bash deploy/enable-clock-control.sh`); V5's drift half is CLOSED — gate-backed by `deploy/check-system-files.sh` and surfaced nightly by `nightqc.system_file_drift`. ⚠ **V3's residual is UNSUPPLIABLE, not merely blocked:** it asks for a byte-diff against a vendor PPI export, and Polar Sensor Logger never produced one. It cannot be closed by any owner action with the current vendor app and should be retired rather than carried. **Owner:** Heron · **Next step:** retire V3's residual, then the PSL capture for V1/V2) · **Created:** 2026-07-16
