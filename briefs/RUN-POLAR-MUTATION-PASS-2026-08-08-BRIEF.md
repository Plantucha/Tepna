...
**Status:** IN-PROGRESS · **Created:** 2026-08-08 · **Follows:** `CAPTURE-HOST-MUTATION-FLEET-2026-08-04-BRIEF.md` §4.1 · **DRAIN 2026-09-02 (Osprey):** section 6's `SKIP anchor` item **CLOSED** with evidence (see it — the string never existed in code; the mechanism's absence is what confirms retirement). **ONE item remains open** and it is real work, not a stamp: *the 45 timeouts re-run un-contended and reclassified*. **Owner: Osprey. Next step:** that re-run is one work-unit under the full gate; the brief flips DONE when it lands. · **RE-TRIAGED 2026-09-13 (Osprey): still ONE open item — the 45 timeouts re-run un-contended — and #2403 sharpened what it is FOR without doing it.** #2403 made the diff-scoped gate treat a timeout as UNDECIDED (required, never killed, never refuting) and REFUSE rather than report green, on the principle that a timeout is UNMEASURED and a tighter bound cannot manufacture a pass. That is the principle this item rests on, now enforced in one tool — but the 45 mutants themselves are still unjudged, and re-running them is a real work-unit on a quiet box, not a stamp. ⚠️ It is heavy: mutation sweeps are what OOM-killed this box on 2026-09-07. Cap it and run it when nothing else is gating. · **DRAIN 2026-09-19 (Osprey): the ONE open item STANDS — the 45 timeouts are still unjudged. Surface (`capture.py`, `alerts.py`, `blind_spots.py`, `writers.py`, + the two named test files): **22 landings since 2026-09-13**, none of them the re-run. ⚠️ **But those 22 change what the re-run MEANS, which is the useful product of this pass:** the 45 timeouts were recorded against a `capture.py` that has since moved 22 commits, so re-running them now is not a re-judgement of the same mutants — the mutant set itself will differ. Re-generate before re-judging, and do not compare the new count to 45 as though it were the same population.** · **TRIAGED 2026-09-25 (Osprey): park stands, but its VERIFICATION is stale against a moved surface — re-read before sizing.** This brief is execution-bound, not decision-bound, so landings on its own surface are decision-relevant, and the surface HAS moved since the last drain (measured 2026-09-25 against `origin/main`: 38 commits on `capture-host/capture.py` since 2026-09-19 alone). Verified this pass: step-1 stale query non-zero from a 925-commit aged base with the positive control firing (RESIDUE 390 / capture.py 105 / CLAUDE.md 12); **0 of 56 pending changesets name it** while 11 name 7 other briefs, so the zero is real; Rule 0 run 2026-09-24. ⚠️ What I did NOT do, stated so nobody reads more into this stamp than it carries: I did not re-verify each Done-when item against the moved surface — that is the next pass's work, and it is now owed rather than optional. Status unchanged.
