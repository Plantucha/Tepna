...
**Status:** DONE — 2026-07-22 (**Part A COMPLETE** — A1·A2·A3·A4; **Part B COMPLETE** — Biome formatter adopted check-only/changed-only via `BIOME-FORMATTER-2026-07-11-BRIEF.md` Phases 0–1: `biome.json` tuned + `@biomejs/biome` devDependency + `format.yml`, provenance untouched; **Part C COMPLETE** — the `node tsc` host materialised and the type gate is fully widened: all 8 `*-dsp.js` + `signal-orchestrate.js` + the core adapters now sit in `tsconfig.include` (`//d2` note "TYPE-GATE COMPLETE") and `npm run typecheck` / `types.yml` is green, meeting Part C's Done-when *"any module newly added to `tsconfig.include` passes `tsc`; `types.yml` green"*. Residue, deliberately OUT of Part C's Done-when: the shared-spine `clock.js`/`metric-registry.js` widening is owner-gated churn tracked under `OWN-THE-BUILD-FOLLOWUPS` D.2; `nsrr-adapter.js` (bare runtime global) + the Tier-3 HTML formatter defer to a `DEV-TOOLCHAIN-FOLLOWUPS`) · **Created:** 2026-06-30 · **Follows:** `OWN-THE-BUILD-2026-06-30-BRIEF.md` (shares the *construction-enforcement over drift-suppression* thesis + the Node-host reality) · **Continues:** `SIGNAL-ADAPTER-AND-FRONTIER-2026-06-23-BRIEF.md` Phase 4 (which shipped 4a/4b — this is "Phase 4, the two missing legs")
