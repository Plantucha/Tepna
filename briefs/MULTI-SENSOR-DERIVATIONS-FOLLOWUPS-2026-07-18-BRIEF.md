...
**Status:** IN-PROGRESS (re-verified 2026-09-25, Kestrel triage, docs only: the §6 wrist leg is still the one remainder — `grep -c stageSleep ppgdex-dsp.js` → 0 on `origin/main`, and `stageSleep` lives only in `ecgdex-dsp.js`, now at `:2445` (the `:2124` cited below has DRIFTED; cite the symbol); it is a full work-unit that moves all six PpgDex fixtures and is parked behind the capture-first phase, not blocked; NO CHANGE. Previously parked 2026-09-05 — drain stamp, Magpie. Re-verified on `origin/main` fc0cf634: §1 closed and gated, §2 at 2 of 3 legs by design, §3 propagated, §5 opportunistic by owner decision. **The ONE remainder is §6's wrist leg** — `stageSleep` lives only in `ecgdex-dsp.js:2124` (`grep -c stageSleep ppgdex-dsp.js` → 0), its veto is still the absolute `m >= 60` (`:2156`), PpgDex publishes no stages, and the scale trap is pinned by `ecgdex-dsp · staging · motion-scale-contract` (`tests/dex-tests.js:51377`). That leg is a full work-unit, not a stamp: an additive threshold argument on `stageSleep`, a PpgDex stager call that moves all six PpgDex fixtures, and the chest-vs-wrist agreement measurement over the trio nights that is its only validation. **Owner:** whoever next opens `ppgdex-dsp.js` for a re-bundle; **next step:** size §6 as its own unit and execute it in one PR — do NOT run the stager on 0..1 motion first. Not PROPOSED: four of five sections are executed work) ⚠️ **DRAIN R2 2026-09-19 — 24 landings, surface 11 files** (`ecgdex-dsp.js`, `ppgdex-dsp.js`, `*-registry.js` resolved). 🔴 **A first pass reported ZERO** — it derived from the subject (`rrMaskOn`/`tvMaskOn` → `cpapdex-dsp.js`, ONE file) rather than from the files this brief names. **Landings recorded, NOT assessed.** · **Created:** 2026-07-18 · **Follows:** `MULTI-SENSOR-DERIVATIONS-2026-07-16-BRIEF.md` (DONE) · **Related:** `MOTIONDEX-BUILD-FOLLOWUPS-2026-07-18-BRIEF.md` · `APNEA-TYPING-FUSION-2026-07-18-BRIEF.md` (DONE) · **Residue:** 2026-09-02-respiration-fusion-no-fixture · **DRAIN 2026-09-11 (Brief runner) — RE-VERIFIED IN THE TREE, UNCHANGED.** The only commit naming this brief since 2026-09-05 is that stamp itself. §6's wrist leg re-measured today rather than re-read: `grep -c stageSleep ppgdex-dsp.js` → **0**, and `ecgdex-dsp.js` carries **3** occurrences — so the leg is still unbuilt and still a full work-unit (threshold argument + a PpgDex stager call that moves all six fixtures + the chest-vs-wrist validation). Status stays IN-PROGRESS.
...
`hrvEpochs.motion` (`integrator-dsp.js` `adaptEnvelopeNode`) and uses it twice:
