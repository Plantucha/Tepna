...
- [x] Recovery is bounded and cannot loop forever. — capped in every path: `min(backoff * 2, 300)` (`capture.py` `run_polar`), `_RECONNECT_BACKOFF_CAP_S` (`capture.py` `_RECONNECT_BACKOFF_CAP_S`), `BACKOFF_MAX` (`cpap_live.py:25`), `backoff_cap` (`settings_schema.py` `power.reconnect_backoff_cap_sec`).
