...
#### ✅ §1.2 — the link-checker exists now, and it found a third residue

`tools/served-link-check.mjs` builds the served set **by parsing `capture-host/deploy/sync-apps.sh`**
rather than keeping its own copy of the list — a checker with a private copy drifts from the deploy
script and then agrees with itself while the box serves something else, which is the shape §C7 already
fixed once. Served set as measured: 9 provenance apps + 14 whitelisted PAGES + 3 ASSETS + 3 ASSET_DIRS
= **115 files**, 45 of them HTML, carrying **253** distinct local refs.

**18 are dead on a deployed box.**

| page | dead | what |
|---|---|---|
| `Science.html` | **15** | analysis harnesses + gate pages — exist in the repo, deliberately not served |
| `index.html` | 2 | `sitemap.xml`, `feed.xml` |
| `papers/timestamp-pathology-analysis.html` | 1 | `<script src="../clock.js">` — **not previously recorded** |

**§1.2's first residue is confirmed exactly: 15, on `Science.html`.**

**Its second is corrected.** The brief says `feed.xml` and `sitemap.xml` "**do not exist in the repo at
all**". They do — `docs/feed.xml` and `docs/sitemap.xml`, generated by `build-docs.mjs`. What is missing
is a **root** copy, and root `index.html` is the page that gets served. So this is a served-tree gap, not
a missing artifact, and the fix is correspondingly smaller (serve the two files, or drop the two `<link>`s
from the served copy) than "generate them".

**And a third residue the brief did not have:** `papers/timestamp-pathology-analysis.html` is served (it
sits under the `papers` ASSET_DIR) and loads `<script src="../clock.js">`, which is **not** in the served
set. That page is broken on the appliance today — a script 404, not a cosmetic dead link. It is exactly
the class §1.2 predicted a checker would catch and the manual pass had missed.

**Four false-positive classes had to be killed before the number meant anything**, and they are recorded
because the next person writing a scanner over these bundles will hit all four:
`data-inline-src="clock.js"` on every plain-inline asset (a bare `/(?:href|src)=/` reported the entire
module graph of every bundle — 295 findings, ~95 % noise); JS assignments like `var src = 'chest-acc'`
inside inlined script; percent-encoded filenames (`OxyDex%20Reference.html` — all seven served guides
read as absent); and duplicate refs per page. The tool now scans **per HTML tag** and percent-decodes,
which kills all four while still catching attributes written inside a JS template string.

**Not wired as a gate**: it would red today on 18 findings, and a gate that cannot go green on the day it
lands is a broken build. It ships with `--strict` so it can become one the moment the served tree is
clean. Fixing the three residues is a `Science.html` / `index.html` / `sync-apps.sh` decision — served-set
policy, not code — and belongs with whoever owns the deploy surface.
