...
- [x] **No correction-history meta-commentary — VERIFIED fleet-wide 2026-08-19, after finding 2 that
      the earlier "dimension 6 clean" claim had missed** (OxyDex ApEn + Ultradian Cycles, both the
      `"vXX fix"` pattern §6 names). Re-swept: 0 across all 7 guides. Internal composites/projections
      honestly labelled: the lower-tier honesty reading above fixed OxyCrash and Positional Shifts and
      deliberately kept MOS, which already labels itself unvalidated.
- [x] **`Dex-Test-Suite.html` all green; `node tests/run-tests.mjs` green — VERIFIED on #1529 (2026-08-19).**
      `browser-gates` PASS covers the first; the six `suite (shard N/6)` checks cover the second. Named run,
      not a remembered one — this brief has already had one box claim a state its own body contradicted.
