...
**Status:** DONE — 2026-09-01 (verified: the charter's required output — the §19 17-part report + ONE named architecture — exists as the superseding REPORT brief, which also says honestly that it was written AFTER the implementation it was meant to gate; the architecture it names is built and running in shadow on the box. The living content is the REPORT's; nothing here remains executable. ⚠️ TRIAGE 2026-09-06: the architecture is BUILT but the harvest is NOT WIRED TO IT — `cpap_supervisor` runs in SHADOW only (`cpap_shadow_runner` writes SESSIONDETECT.csv 'would-have decisions' and drives nothing), while the harvest trigger reads `cpap_live.EndWatch`'s polled debounce; `capture.py`'s own comment says so — *"the debounce is in the supervisor's active/idle state, which this trigger does not read"*. So the device-authoritative stop verdict (`MachineMetrics.LastTherapyUseDateTime` advancing) is computed, journalled and ignored. Wiring it is unit 1b; unit 1a makes the harvest job durable first) · **Created:** 2026-08-24 · **Superseded-by:** `AS11-SESSION-DETECTION-REPORT-2026-08-26-BRIEF.md` · **Follows:** `AS11-AUTO-SESSION-DETECTION-2026-08-24-BRIEF.md`
