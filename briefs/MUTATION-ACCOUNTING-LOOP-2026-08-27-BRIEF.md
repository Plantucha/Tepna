...
-->
**Status:** IN-PROGRESS · **Created:** 2026-08-27 · **Owner-issued charter** ("what should we do next" recon, executed same-day: "write it as brief and implement right after") · **Interlocks:** `QWEN-ENGINEERING-PROGRAM-2026-08-27-BRIEF.md`, `MUTATION-FLEET-EXPANSION-2026-08-25-BRIEF.md`, `MUTATION-PROGRAM-2026-08-09-BRIEF.md` · **DRAIN 2026-09-02 (Osprey):** verified 3 of 6 Done-when boxes ticked, 3 open. Not DONE, not blocked — ordinary unfinished work. **Owner: Osprey. Next step:** the three open boxes are one work-unit; they need the accounting loop run, not more design. · **RE-TRIAGED 2026-09-13 (Osprey): 3 of 6 open, unchanged as work — but the DENOMINATOR this loop accounts over moved under it.** #2323 added `equivalent` to `EXCUSING`, so entries carrying a proof now leave the distinguishable denominator instead of sitting in it and being reported as real gaps; 4 entries were affected (`hrvdex-dsp.js` 755 + 1096, `pulsedex-dsp.js` 448, `ecgdex-dsp.js` 1283). Any §E3 delta measured BEFORE 2026-09-13 is against the old denominator and is not comparable with one measured after — state which side of that line a batch-3 number came from. The three open boxes are otherwise ordinary unfinished work, as the 09-02 drain said. · **DRAIN 2026-09-20 (Osprey): 3 ticked / 3 open — header ACCURATE, unchanged as work. The 8 mutation-specific landings since 2026-09-13 touch the gate's reporting and scoping, not this loop's accounting. Still ordinary unfinished work needing the loop RUN, not more design.** · **TRIAGED 2026-09-25 (Osprey): park stands, but its VERIFICATION is stale against a moved surface — re-read before sizing.** This brief is execution-bound, not decision-bound, so landings on its own surface are decision-relevant, and the surface HAS moved since the last drain (measured 2026-09-25 against `origin/main`: 1 commits on `tools/mutation-crawl.mjs` since 2026-09-19 alone). Verified this pass: step-1 stale query non-zero from a 925-commit aged base with the positive control firing (RESIDUE 390 / capture.py 105 / CLAUDE.md 12); **0 of 56 pending changesets name it** while 11 name 7 other briefs, so the zero is real; Rule 0 run 2026-09-24. ⚠️ What I did NOT do, stated so nobody reads more into this stamp than it carries: I did not re-verify each Done-when item against the moved surface — that is the next pass's work, and it is now owed rather than optional. Status unchanged.
