...
-->
**Status:** PROPOSED (PARKED 2026-08-28 — n=0 clean nights measured, and the sole acquisition route is owner-DECLINED; this experiment is UNRUNNABLE AS DESIGNED, see §6. Deliberately *not* worded "deferred": nobody should retry this.) · **Created:** 2026-08-28 · **Spawned by:** `PAT-ROOT-CAUSE-FORENSICS-2026-08-27-BRIEF.md` (campaign boundary declared 2026-08-28) · **Interlocks:** `PAT-FORENSICS-WINDOW-ORACLE-2026-08-28-BRIEF.md`, `PAT-FORENSICS-WINDOW-REGIMES-2026-08-28-BRIEF.md` · **DRAIN 2026-09-02 (Osprey) — restamped, blocker unchanged and NOT re-litigated.** The sole acquisition route is owner-DECLINED and n=0 clean nights was a measurement, not an estimate; nothing since 2026-08-28 changes either. **Owner: the OWNER** (only a route decision reopens it). **Next step:** none available to the fleet — this stays parked until the owner reopens the route or accepts a different design. · **Residue:** 2026-09-02-pat-applied-works-by-accident · **RE-VERIFIED 2026-09-03 (Osprey): the parked wording is correct and deliberately not "deferred" — leave it.** n=0 clean nights measured and the sole acquisition route is owner-DECLINED, so this is UNRUNNABLE AS DESIGNED rather than queued. Confirmed it holds no table the published-number sweep could attribute to a tool, so nothing here rests on a drifted number. **No stamp change is the correct outcome**; re-verified so the next triager does not re-derive it. · **DRAIN 2026-09-19 (Osprey): PARK STANDS, blocker unchanged and deliberately not re-litigated. `tools/pat-drift-attribution.mjs` — **1 landing since 2026-09-03** (#2246, a citation/gate fix) and it does **not** touch the blocker, which is an OWNER route decision plus a measured n=0 clean nights. **No code landing can clear either.** Owner: the OWNER; the fleet has no next step. Recorded so the next triage does not re-derive it a fourth time.** · **TRIAGED 2026-09-25 (Osprey): FIFTH pass, PARK STANDS, deliberately NOT re-derived.** Both remaining Done-when boxes are struck through ("not achievable as designed"); the blocker is an OWNER route decision plus a measured n=0 clean nights, and prior passes on 2026-09-02, 09-03 and 09-19 each reached this same outcome — the last saying in terms *"recorded so the next triage does not re-derive it a fourth time"*. I read those notes instead of repeating their work. ⚠️ **Process finding, not a brief finding:** a brief parked on an owner decision with no fleet next step has now consumed five triage cycles for zero state change. That is a queue-design cost and belongs to whoever owns the triage procedure, not to this brief. No stamp change.
