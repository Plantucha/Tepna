...
> The same
"pin the whole output, not that it ran" lesson as the JS unexported-metric class. **`writers` DONE (2026-07-18):** night_dir + every per-stream header/row (acc/gyro/mag/ppi/hr) pinned incl. the PPI flag-bit split + HR one-row-per-RR; 6 verified; test_writers 6→11. **`telemetry.push` DONE:** broadcast msg shape + drop-oldest-when-full queue + rate fallback pinned (test_telemetry 9→12; 2 classes verified, the rows/values one is a list/tuple JSON no-op). **§2's named targets (polar_pmd · writers · telemetry) are now all closed; only clockcfg's untestable timedatectl subprocess paths remain, deliberately deprioritized.** **De-prioritize `clockcfg`'s subprocess
