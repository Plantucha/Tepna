...
> The same
"pin the whole output, not that it ran" lesson as the JS unexported-metric class. **`writers` DONE (2026-07-18):** night_dir + every per-stream header/row (acc/gyro/mag/ppi/hr) pinned incl. the PPI flag-bit split + HR one-row-per-RR; 6 mutation classes verified; test_writers 6→11. Remaining: `telemetry.push`. **De-prioritize `clockcfg`'s subprocess
