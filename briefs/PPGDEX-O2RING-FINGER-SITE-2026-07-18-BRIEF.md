...
**Status:** DONE — 2026-07-20 (**§6 round-trip now VERIFIED ON REAL HARDWARE — see `docs/O2RING-FINGER-ROUNDTRIP-2026-07-20.md`.** A real Health-Box session (2026-07-19 ~20:58, finger-worn, with a simultaneous chest-strap ECG) gives all three legs at once: O2Ring→PpgDex median HR **56.3 bpm** vs the ring's own 1 Hz field **57.0** (Δ 0.7) vs the paired H10 ECG gold standard **56.1** (Δ 0.2), over a 96 s overlap; 313 feet detected, PPI foot-to-foot, `ledSingleChannel=true` / `ledAgreementPct=null` (no fabricated agreement). The one nuance: that session predates PR #276, so the file is replicated 3-column and tags `site='wrist'` — but the degenerate-guard collapses it to the exact honest single-channel path #275 built (that is why `ledSingleChannel` is true). A post-#276 capture writes a genuine 1-column file that tags `site='finger'` directly; that path is proven byte-for-byte on the committed twin (48 gated assertions). Re-running the round-trip on a fresh post-#276 session — to see `site='finger'` end-to-end on real data — is the only belt-and-suspenders step left and changes no DSP math; tracked in `PPGDEX-O2RING-FINGER-SITE-FOLLOWUPS-2026-07-20-BRIEF.md`. **Originally §3 · §4 · §5 EXECUTED + gated; §6 executed on the COMMITTED twin.** `parsePPG` resolves 1-vs-3 optical columns and tags `site`; the in-band `156` sentinel is rejected by ISOLATION, not by value, and a rejected sample is a gap that drops its beat rather than being filled; the single-channel path reports `ledAgreementPct: null` and swaps the absent LED vote for an inter-beat cadence axis, surfaced as `beatConfidenceAxis`; finger morphology enters at `experimental` behind site-scoped registry ids with the wrist grades untouched. Committed adversarial twin `uploads/synthetic_ppgdex_o2ring_finger.txt` carries BOTH sentinel classes and gates in CI from bytes — 48 assertions, **mutation-verified in 4 directions** (a value-only rejector, a faked vote, a filled gap, and an inherited grade each red it; the faked-vote mutation initially passed, which is why `beatConfidenceAxis` exists at all). Verity regression proven on the REAL corpus: `parsePPG`+`analyze` byte-identical old-vs-new across 4 nights incl. two full ~7.4 h recordings (4.7 M samples each). PpgDex + both orchestrators re-bundled, GATE A/B clean, suite 3340 green.
**NOT done — the two items that need hardware/data this repo does not have:** (a) the §6 round-trip against a REAL O2Ring capture — no real finger `*_PPG.txt` exists on disk yet (the corpus' 54 PPG files are all 3-LED Verity; the ring's `.dat` files are its stored SpO₂ summary, not the pleth), so HR is validated against the twin's planted 48 bpm rather than the ring's own 1 Hz HR + paired H10 ECG; (b) `DEX_UPLOADS=… node tools/verify-fixtures.mjs` re-stamping of the corpus-backed `PpgDex_2026-06-27_equiv` fixture — its committed input `Polar_Sense_BBBBBBBB_…` is an anonymised file not present in the corpus. §7's capture-host de-replication rides a SEPARATE PR (Python lane).) · **Created:** 2026-07-18
