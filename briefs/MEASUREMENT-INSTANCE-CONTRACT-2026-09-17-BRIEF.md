...
**Status:** DONE — 2026-09-17 (**executed with the scope fence intact: NO node emits the block, so no fixture moved and no `manifestHash` changed — `build.mjs --check` clean, 11 owned bundles, 0 stale.** Landed: `measurement-block.js` as a NEW CORE module (not an edit to `dex-export.js`, which is inlined 8/8), 27 assertions carrying roadmap §9's negative table plus the two this brief added, both anti-vacuity directions, and a published `checked[]` denominator so a leg that cannot run SAYS so instead of passing silently — the identity leg is absent from `checked` without a resolver and present with one. Specified in `docs/EXPORT-SHAPES.md`, `docs/LEXICON.md` §4b and `docs/EVENT-LEXICON.md` §4b. Registered in both lanes' source inventories (ratchet holds at 13) and in the JS mutation `DEFAULT_FLEET` — which, unlike the gattmap case, IS the right fleet here because this one is a `.js` module. `npm run check` all 16 steps, 9514 assertions. ⚠️ Two rows of §9's table are NOT covered and the group SAYS so rather than faking them: "absurd ranges" needs the metric unit, which resolves from the registry and is deliberately not inline on the block, so it belongs with the emitter (§3); "duplicate event ids" belongs to §2's events, not to a per-metric-instance block.) · **Created:** 2026-09-17 · **Executes:** `MEASUREMENT-PROVENANCE-ROADMAP-2026-08-26-BRIEF.md` §1 + §2 (its first Done-when item: *"§1+§2 contract brief spawned, executed, gates green"*) · **Owner:** unassigned — any session may pick this up · **Unblocked by:** owner ruling **2026-09-15** (`OWNER-DECISION-QUEUE-2026-09-03-BRIEF.md` D5 — the schema-bump timing decision the roadmap was parked on) · **Relates:** CLAUDE.md §🔒 Clock Contract, §🎫 evidence ladder, §🔏 provenance gates, §📦 release ledger
