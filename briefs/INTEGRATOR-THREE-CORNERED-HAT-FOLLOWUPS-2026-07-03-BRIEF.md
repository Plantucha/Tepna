...
**Status:** DONE — 2026-07-06 (**§3 DONE — 2026-07-04**: bundle+ledger reconcile at Integrator `manifestHash 5ae5434fe914`, `Dex-Test-Suite.html?full` all-green [1744 passed, 0 boot-skips], `verify-provenance` GATE A/B clean. ~~§1/§2 remain PROPOSED — blocked on upstream OxyDex per-epoch-HR export, see §2.~~ (superseded 2026-07-04 — see the later clause) **2026-07-04 investigation** (no code yet): linchpin + route settled (see the §1/§2/§4 additions below); the current committed Integrator `manifestHash` is **f44363fce611** (§3's `5ae5434fe914` was superseded by later Integrator re-bundles), and the browser owned-build loop (`tools/build-core.js` via run_script) was verified to reproduce committed hashes byte-exactly; route DECIDED = **HR-hat** (ECG+PPG+Oxy). **§1/§2 CODE LANDED — 2026-07-04:** OxyDex per-epoch HR+motion export shipped (`OXYDEX-PER-EPOCH-HR-EXPORT-2026-07-04-BRIEF.md`, DONE) + Integrator `_tchHat` HR-hat (ECG+PPG+Oxy, unions overlapping hr-bearing non-HRV nodes) + `_tchRhoFromMotion` ρ-from-cross-node-motion; `Dex-Test-Suite` headless floor green incl. the new **5e** known-answer group (HR-hat fires, culprit, reconciled HR, ρ≈planted RHO, degrade cases) + `verify-provenance` GATE A/B clean at OxyDex `4d3b2194d942` / Integrator `62da4f43db2a`. CLOSED 2026-07-06: the reference-night golden landed via FU-II §2 (`uploads/integrator_tch_golden.node-export.json` — the FIRST code-gated Integrator fixture, GATE-B code-gated @ `cef329a4fec6`; HR-hat fires, culprit OxyDex, ρ=0.356 from cross-node motion, reconciled HR 58.4 bpm), `tchHR` render shipped (FU-II §3), full `?full` render-coverage confirmed all-green [2102 passed, 0 boot-skips]. **§4 (N-cornered hat, 3→N sensors) DEFERRED** — blocked on a real ≥4-sensor co-recording + EEGDex shipping (a future item, not this brief's scope). real-data ρ validation carried as FU-II §4 (DEFERRED, blocked on a real co-recorded night). Follow-up: `INTEGRATOR-THREE-CORNERED-HAT-FOLLOWUPS-III-2026-07-06-BRIEF.md`) · **Created:** 2026-07-03 · **Executed-residue-of:** `INTEGRATOR-THREE-CORNERED-HAT-2026-07-02-BRIEF.md` (DONE 2026-07-03) · **Extends:** `INTEGRATOR-BUILD-BRIEF.md` §4.4 `fuseHRVConsensus` · **Links:** `PPGDEX-BEAT-DETECTION-PERF-AND-HRV-FIDELITY-2026-07-02-BRIEF.md` (ACC co-motion ρ source) · an OxyDex per-epoch-HR node-export brief (TBD)
