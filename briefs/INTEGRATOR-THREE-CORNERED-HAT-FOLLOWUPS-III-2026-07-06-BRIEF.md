...
**CLOSED 2026-07-13 — the committed-trio distribution landed (docs §11).** With all **24** trio nights now
committed (`TRIO-BATCH-O2RING-DAT`, PR #81),
`node tools/tch-multinight.mjs --dir uploads/trio` produced the owed distribution through the shipped kernel:
**24/24 nights solve**; median σ **ECGDex 0.79 / OxyDex 1.08 / PpgDex 2.20 bpm**, preserving the ranking (the
criterion H10 is the quietest corner) and landing the Verity corner between the two independent anchors that
bracket it (§8's literature ≈1.8, §2's reference-free ≈2.8)
— so the **magnitude check passes on both anchors** and §1's "distribution + reference-anchored magnitude" ask is
met. The rescue reproduced (quiet-order on 11/24 nights; motion-ρ rescued 6, incl. the §6 night at OxyDex σ
0.04 → 1.00 bpm). **The 5 failures are the payload:** ρ_motion (0.04–0.49) fell *short* of the ρ the geometry
requires (0.39–0.71), so `threeCorneredHat` (`integrator-tch.js:275`) fell through the `correlated-external`
...
**SATISFIED 2026-07-13 → DONE.** §1's A/B **landed** on the 24 committed trio nights (docs §11), and §4 is
