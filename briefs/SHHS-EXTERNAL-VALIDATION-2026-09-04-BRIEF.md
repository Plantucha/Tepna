...
**Status:** DONE — 2026-09-12 (EXECUTED by owner assignment. ⚠ **Both gates it named are resolved, and neither the way this brief expected.** (1) The drain condition — NSRR closed until the brief drain completes — was **NOT met** (52 open against ≤20, re-measured 2026-09-12, down from the 73 recorded here) and was **superseded by the owner assigning this work directly** on 2026-09-12, which is exactly the lift this brief said only the owner could give. **Then CANCELLED outright by the owner the same day** — the condition is withdrawn, not merely overridden for this unit, so it gates nothing in future either. (2) §P5's public-benchmark gate was clarified the same day: it covers **PUBLICATION, not measurement** — running the instruments internally is permitted, quoting a rate outside the repo is not. See `STRATEGIC-PRIORITIES-2026-08-26` §P5. ⚠ **THE CENTRAL PREMISE OF THIS BRIEF WAS WRONG.** It records `tools/nsrr-stage-validate.mjs` as "BUILT+proven … what remains is execution, not design". The tool was built and it was **not** proven: it compared a stage OBJECT against a stage STRING, so REM recall was **0 by construction** on every record it had ever seen (#2423). Its `--selftest` refuses to compute recall from synthetic input — correctly, that is the circular oracle §1 bans — and that refusal is exactly why the defect survived: the one computation that would have exposed it was never exercised in the only mode that ever ran. A guard against a false POSITIVE hid a false NEGATIVE. **Do not read "BUILT+proven" in any brief as "produces a correct number".** Results, 99/99 records scored: REM recall 32 % median / precision 26 %; OxyDex finds a median **37 %** of expert-scored SpO₂ desaturations on the same channel. A second §∅ defect was found and fixed on the way — `nsrr-adapter.js` filled SpO₂ dropouts with the last reading, 64 minutes of it on one record, and the test suite **asserted that fabrication as the invariant**.) · **Created:** 2026-09-04 · **Follows:** `PAPERS-ROADMAP-2026-06-24-BRIEF.md` §3.2, `REM-STAGING-FOLLOWUPS-2026-08-02-BRIEF.md` §2b · **Affects:** `nsrr-adapter.js`, `tools/nsrr-stage-validate.mjs`, `tools/nsrr-oxydex-odi.mjs`
