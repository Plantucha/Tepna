...
-->
**Status:** IN-PROGRESS · **TRIAGED 2026-09-01 (Osprey): charter head. **The forensics wave is FIVE briefs, not three** — WINDOW-ORACLE, WINDOW-REGIMES, FIDUCIAL-JITTER, AXIS-LEG-ASYMMETRY, plus this. **20 `tools/pat-*.mjs` exist**, so §0's "genuinely NEW" list is largely TOOLED already (§6 `pat-fiducial-jitter`, §7 `pat-fiducial-compare`, §11–13 `pat-window-oracle`) — what is missing is EXECUTION against the corpus, and the corpus is at **`/srv/data/tepna-corpus/`** — local, 125 GB, 1131 raw `_ECG.txt`. (My first stamp said Heron; wrong — I had searched only the repo tree.) So the `--dir` tools CAN run here and the Done-when items are execution-bound rather than access-bound. ⚠️ **Landmine: `smoketest-captures/2026-08-23` makes trio-batch consume >50 GB from 285 MB raw** (unbounded-growth defect under probe) — exclude that night from any walk.** · **Created:** 2026-08-27 · **Owner-issued charter** (direct, 2026-08-27 evening — condensed capture in the Appendix; original in the coordinator transcript) · **Interlocks:** `PAT-RELATIVE-REFRAME-2026-08-17-BRIEF.md` (DONE), `CLOCK-LEG-SIGN-CONTRADICTION-2026-08-27-BRIEF.md` (DONE), `KNOWN-CLOCK-ADVERSARIAL-CAPTURE-2026-08-14-BRIEF.md`, `PAT-NO-VALID-ANCHOR` lineage, `O2RING-FRAME-SAMPLE-LOCK-2026-08-03-BRIEF.md` · **DRAIN 2026-09-02 (Osprey):** charter head of the five-brief forensics wave (WINDOW-ORACLE, WINDOW-REGIMES, FIDUCIAL-JITTER, AXIS-LEG-ASYMMETRY, plus this). WINDOW-ORACLE is executing in this drain; the other three are runnable-locally with no blocker. **Owner: Osprey. Next step:** none directly — it closes when the four children do. · **RE-VERIFIED 2026-09-03 (Osprey):** charter head, still closes only when its children do — and one child moved. The published-number sweep found **WINDOW-ORACLE's own tables drifted** (§6 withdrawn as circular; drift table 5 of 8 rows, my #2114 picker fix), so this charter cannot close on those numbers as published. FIDUCIAL-JITTER is separately stamped NOT DONE with a named remainder. Owner: Osprey. **Next step:** unchanged — none directly. · **DRAIN 2026-09-19 (Osprey): header STILL TRUE — the open items remain EXECUTION-bound, not stale. Surface derived (no `Affects:` field): the 20 `tools/pat-*.mjs` plus `pat-align.js` and `pat-connection-stability.mjs` = 22 files → **6 landings since 2026-09-03** (#2516 · #2512 · #2507 · #2504 and siblings). ⚠️ **None of them EXECUTES anything against the corpus, so no Done-when box closes — but they CHANGED THE TOOLS, which means any execution result recorded before 2026-09-03 was produced by different code and must not be reused.** That is the actionable product of this pass: re-run, do not re-read. ⚠️ **AND A METHOD CORRECTION ON MYSELF:** my first pass resolved this brief's surface to **1 file** and reported **0 landings** — because the brief names its tools as a GLOB in backticks (`tools/pat-*.mjs`) and my extractor took only literal paths. The negative was an artifact of thin extraction, not evidence. A derived-surface negative is worth exactly what the extraction is worth.** · **TRIAGED 2026-09-25 (Osprey): park stands, but its VERIFICATION is stale against a moved surface — re-read before sizing.** This brief is execution-bound, not decision-bound, so landings on its own surface are decision-relevant, and the surface HAS moved since the last drain (measured 2026-09-25 against `origin/main`: 9 commits on `tools/pat-*.mjs` since 2026-09-19 alone). Verified this pass: step-1 stale query non-zero from a 925-commit aged base with the positive control firing (RESIDUE 390 / capture.py 105 / CLAUDE.md 12); **0 of 56 pending changesets name it** while 11 name 7 other briefs, so the zero is real; Rule 0 run 2026-09-24. ⚠️ What I did NOT do, stated so nobody reads more into this stamp than it carries: I did not re-verify each Done-when item against the moved surface — that is the next pass's work, and it is now owed rather than optional. Status unchanged.
