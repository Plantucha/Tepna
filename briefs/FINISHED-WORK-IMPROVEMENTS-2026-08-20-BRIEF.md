...
-->
**Status:** IN-PROGRESS — re-verified 2026-09-25, Kestrel triage, docs only: no commit since #d8f3a704 (09-21) touches this brief or §C's home briefs; §C stays 2 of 4 with C1/C2 FIELD-GATED on a supervised box evening, which the SOLID-NIGHT capture-first phase now occupies — no rig-side step exists; NO CHANGE. Previously 2026-09-21 (re-verified: the only open box is §C, and it is FIELD-GATED — C3 and C4 recorded (CAPTURE-FILESET-RESUME DONE 09-03; DEVICE-RATE-TRUTH DONE 09-21), C1/C2 need a supervised box evening and cannot be executed from rig. Previously 2026-09-03: **TRIAGED 2026-09-03, and §A's premise is STALE — verified end to end rather than read off the brief.** §A says of the time-capability wiring *"nothing downstream reads it"*. It is wired at BOTH ends today: the producer, `oxydex-app.js:217-220`, carries `timingSource` / `rtcOffsetS` / `rtcVerifiedAtMs` / `rtcResetSuspect` inside the `ganglior.node-export` envelope; the consumer, `integrator-dsp.js:733-742`, reads `json.timingSource` — and its own comment records that the guard was INERT until `quality.timingSource` was added 2026-08-17, i.e. the very gap §A describes was closed three days before this brief was written. ⚠️ Checked by CAPABILITY at both ends, because a producer cannot attest its own consumer. **§B1 landed 2026-08-22** (ticked, unchanged). **§C is the only genuinely open item and it is not code** — four field results needing one box evening, which is Heron's lane and cannot be discharged from this checkout. So the brief is blocked on hardware time, not on software work, and should not be read as an open software queue.) · **Created:** 2026-08-20 · **Follows:** `O2RING-TIME-CAPABILITY-WIRING-2026-08-19-BRIEF.md`, `O2RING-BUZZ-FIDUCIAL-2026-08-19-BRIEF.md`, `VIGIL-OVERNIGHT-FINDINGS-2026-07-24-BRIEF.md`, `CAPTURE-FILESET-RESUME-2026-08-19-BRIEF.md`, `KNOWN-CLOCK-ADVERSARIAL-CAPTURE-FOLLOWUPS-2026-08-14-BRIEF.md`, `DEEP-AUDIT-V-FOLLOWUPS-2026-08-05-BRIEF.md`
...
- [ ] C: all four field results recorded in their home briefs after one box session. **RE-VERIFIED AGAINST THE TREE 2026-09-21 — 1 of 4 recorded, 3 FIELD-GATED:** C3 (fileset-resume night) is discharged — `CAPTURE-FILESET-RESUME` flipped DONE 2026-09-03 with its measured number. C1 (wedge drill): `CONNECT-LOCK-DUTY-CYCLE` still records "none a BlueZ wedge" — no real wedge has been cleared. C2 (buzz prescription): no CLEAN/MARGINAL/SWAMPED verdict recorded anywhere; `pat-buzz-stability.mjs` still reads UNRESOLVED. C4 (ppg2w battery night): **RECORDED 2026-09-21 (`13f073e1`)** — `DEVICE-RATE-TRUTH` closed on it: 58 box nights of `LINK.csv battery_pct`, 8 with ppg2w live at 4.05 %/h against 4.25 %/h without, cost not detectable. (This sentence read "carries no first-night cost" for about an hour: true when written, stale by the next merge.) **So §C stands at 2 of 4.** C1 and C2 need a person at the box with the hardware — a supervised evening — and cannot be ticked from rig.
