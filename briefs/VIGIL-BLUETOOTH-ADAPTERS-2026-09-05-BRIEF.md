...
**Status:** PROPOSED (parked, owner-gated — **TRIAGED 2026-09-25 (Wren, box + tree; rule 0: "vigil bluetooth adapters roster pin sniffer capability")**. §6: **P1 DONE** (#2240, `ble_sniff.py` reads the nRF CRC flag and reports the capture span) · **P5 DONE** (#2245, `adapter_pool.py` now names the Intel AX210 as the CPAP's radio) · **P2 open, owner** (no `/etc/apparmor.d/local/tshark` grant on the box) · **P3 open, Heron** (the F6 duplicate label) · **P4 open, owner** (`gh` absent on vigil) · **P6 open, owner** (31 `config.yaml.*` backups on the box, no `config-archive/`). BOX, measured 2026-09-24 ~19:50 EDT, read-only (a reflash or config edit invalidates it): hci0 CSR `00:01:95:CC:53:02` (BlueZ default, pinned by nothing) · hci1 Intel AX210 `28:0C:50:0C:18:FD` = `cpap.ble_stream.adapter` · hci2 Nordic Zephyr HCI-UART `F4:CE:36:2E:CD:98` = the top-level `adapter:`, i.e. THE WEARABLES RADIO (the CSR until 2026-09-12; the Nordic since at least 2026-09-19 18:34, per the box's dated config backups) · hci3 Nordic Zephyr HCI-UART `F4:CE:36:6B:A4:4E`, unpinned. That day's ADAPTERHCI verdict was PASS: 1083/1083 polls answered on every radio, longest run 0. **No radio carries the sniffer image** (both Nordics reflashed 2026-09-09), so `tepna-sniff.timer` is disabled + inactive, with 0 journal entries since 09-19; the units, `ble_sniff.py` and its tests are in the tree (TREE, verified by Heron 2026-09-24). **That is a RECORDED OWNER DECISION, not a defect: D2, 2026-09-19, leave the sniffer INERT** (no re-flash, no retirement; the timer stays installed and disabled). It is recorded in `BLE-SNIFFER-DUTY-CYCLE-2026-09-07-BRIEF.md`, which stays a separate brief (a dependency, not a supersession). This brief carries the roster facts that decision rests on; reopening it (re-flash a Nordic, most plausibly the unpinned `F4:CE:36:6B:A4:4E`) is the owner's alone. The adapter verdict has no way to express that deliberate INERT state: row `2026-09-25-adapterhci-cannot-express-inert-sniffer`. · EARLIER: parked 2026-09-06 — TRIAGED against the tree and the box; every remaining proposal is owner- or Heron-gated, and the two that were not are closed · ⚠ **RE-VERIFIED 2026-09-19 (Wren, box + tree):** BOX (not checkable from code) — four controllers: hci0 USB `00:01:95:CC:53:02` (the kept winner), hci1 USB `28:0C:50:0C:18:FD` (Intel AX210; the ONLY per-device `adapter:` pin in `config.yaml`, the CPAP's), hci2/hci3 UART `F4:CE:36:2E:CD:98` / `F4:CE:36:6B:A4:4E` (the two nRF dongles, reflashed 09-09 to Zephyr HCI-UART, attached by two running `tepna-btattach@` units, #2408, so their BD addresses survive power loss). `99-tepna-btdongle.rules` + `99-tepna-hidraw.rules` are installed. Residue `2026-09-06-cpap-pin-names-unbonded-adapter` was CORRECTED by `2026-09-10-cpap-unbonded-reason-was-wrong` (remedy right, reason wrong). Landed in the surface I checked since 09-06 (paths `capture-host/systemd`, `adapter_pin.py`, `oxy_power.py`, plus subjects /adapter|dongle|btattach|bond|ladder|hci/): #2408 btattach unit, #2359 radio-clock collector, #2428/#2430 (two Zephyr anchors live on vigil; live capture on Zephyr hci0 held three peripherals), #2556 (an adapter reservation is a LEASE), #2624 (HCI round-trip probe), #2651 (`resolve_hci` reaches its last source on this box), #2623/#2643 (the ladder DID fire; its rungs are btusb-shaped). Whether each remaining proposal is still owner-/Heron-gated I did not re-derive — the header's claim is about people, not the box) · **Created:** 2026-09-05 · **Residue:** 2026-09-06-cpap-pin-names-unbonded-adapter
