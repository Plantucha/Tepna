...
- [x] **DONE — the contract existed; its RATE was retracted (fixed 2026-08-04).** `maxTolerableDriftPpm`
      ...
      **What the correction changes is the engineering answer.** The margin is 2–3×, not 30×, so a
      ...
      not measured one.
- [x] ⛔ **CLOSED 2026-09-21 (Osprey) — RECORDED AS THE BRIEF'S ANSWER: A NEGATIVE, and one that more
      corpus cannot move.** The 2026-09-02 stamp set the choice — record the failed closure as the answer, or
      say why it is not yet conclusive — and the sibling this box spun out already decided it:
      `CLOCK-LEG-SIGN-CONTRADICTION-2026-08-27` §10 (DONE) shows **leg C is the method that fails, by SNR,
      not by defect**: its per-night error is tens of ppm (+26.6 / −13.5 across two fragments of the same
      night, spread 40.1 ppm) against host legs that reproduce to 0.3–0.4 ppm, because its observable — the
      ECG↔PPG-foot offset — wanders by more than the clock difference it is asked to resolve. So the
      "unblocks when" clause below — *nights with ≥2 fragments per device AND consistent legs, and the
      closure holds on them* — cannot be satisfied by adding nights: the cleaner the host legs, the tighter
      the band, and the more certainly leg C fails it. That is the band↔verdict anti-correlation above,
      explained rather than merely observed. `tools/beat-leg-closure.mjs` now REFUSES above
      `BL_MAX_BOUND_PPM = 4` (its own reproducibility floor), so a re-run over the 67 current box nights
      would return mostly REFUSE, which is the correct output and not a closure. **What the brief asked —
      is cross-device drift measured and bounded — is answered YES by the host legs themselves** (each
      device against the capture host, 344 streams / 16 nights, §"DONE 2026-08-17"); the triangle's third
      leg is not an instrument that can check them, and PAT gating must not be built on it. The 37-night
      fragment count and the unmeasured leg-consistency clause (09-02 stamp) are moot under this.
      Original 2026-08-27 text retained below for provenance.
