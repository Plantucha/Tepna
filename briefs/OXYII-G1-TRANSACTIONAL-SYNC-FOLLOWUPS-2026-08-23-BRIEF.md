...
-->
**Status:** PROPOSED (**RE-VERIFIED 2026-09-21 (Heron): still exactly ONE item, still physically gated — and the gate is now proven, not assumed.** The drop test needs a restart DURING a ring transfer. The update timer restarts the daemon ~11.6×/day, so a deploy landing mid-pull would have run this test by accident; measured on vigil's journal 2026-09-13 → 09-21: **150 pulls, 76 clean restarts, 0 restarts inside a pull** — because the `recording_state` interlock this brief's 09-13 stamp added defers deploys during a pull. So the interlock works, and the only path to this test is the owner's deliberate doff window. Nothing else changed. parked 2026-09-02 — **the header was stale and two of the three open items had already shipped.** Re-verified against artifacts today: `pull_session` wiring LANDED 60501cec (#1733) with the T3 instrumentation 954720bc (#1761); the fsync chaos-lane control LANDED 77ecd8c2 (#1731) — `tests/test_chaos_ordering.py` `TestFsyncCallSites`, which reds if either fsync is removed. ONE item remains and it is physically gated: the drop test that would set `resume_strategy`'s `allow_resume` from measurement rather than leaving it False, which needs a physical doff window with a journal watcher. Note the chaos control licenses *the calls are made*, not *the data is durable* — the true power-loss lane needs root + `dmsetup` on the box and is a separate unit. **Owner:** Heron · **Next step:** the doff-window drop test, next time the ring is worn · **RE-VERIFIED 2026-09-13 (Heron):** the three items above are unchanged — 1.3 and 1.4 still landed, and 1.2's drop test is still gated, because what it needs is a restart *during* a transfer and that is an owner-scheduled physical window. **But triaging it found a live defect in the adjacent machinery and this PR fixes it:** `tepna-update.sh`'s `recording_state` had NO interlock for a ring pull. It deferred on `recording` and on a CPAP harvest (`cpap.state == running`) and returned `idle` through `oxy_lifecycle` `pulling` / `paused_for_pull` — so the updater could restart the daemon mid-`.dat`-transfer, which is §1.2's own "pull-before-restart order" hazard arriving in production instead of in a test. The defect predates `tepna-update-pending.timer`; enabling that timer on 2026-09-12 is what made it routine, because the doff ENDS the recording (box turns idle) and TRIGGERS the not-worn auto-pull at the same moment, and the timer then fires ~2 min into a pull that starts ~60-80 s after the doff settle. Bounded, not fatal: `resume_strategy` runs `allow_resume=False`, so an interrupted transfer discards its partial bytes and re-serves from zero — a wasted harvest, never a spliced file, which is exactly the asymmetric bet §1.2 describes. ⚠️ This does NOT retire 1.2: the interlock stops the accident, it does not measure whether mid-file resume is safe, and `allow_resume` stays False until the drop test runs) · **Created:** 2026-08-23 · **TRIAGED 2026-09-25 (Magpie):** Heron's 09-21 reading CONFIRMED, not re-derived — still exactly one open item. `resume_strategy` exists in `capture-host/capture.py` and `oxy_transfer.py`; what is missing is the measurement that sets its flag. **Stop: an attended idle-device window** — `OWNER-DECISION-QUEUE` D1 ("attended box time + a wear schedule", unblocks 13). ⚠ D1 carries NO verdict marker and appears in neither the 09-15 ruled batch nor the "still open: D3 and D7" list; the SOLID-NIGHT programme supplies the worn-night half but not an attended idle-device window.
