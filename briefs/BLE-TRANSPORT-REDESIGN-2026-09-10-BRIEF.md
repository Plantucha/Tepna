...
> 🟢 **RULED 2026-09-17 — BUILD IT.** The owner took this over the deferral proposed the same day.
> Promoted to `GATT-HANDLE-MAP-2026-09-17-BRIEF.md` for execution: it is the only non-incremental item
> here and it changes the capture box's BLE stack, so it needs its own unit and a box session rather
> than a line in this list. The mitigation that closed the SYMPTOM (`_settle_gatt_chars`, #2373) stays —
> it is a settle-and-retry, and this section is the remedy it stands in for.
