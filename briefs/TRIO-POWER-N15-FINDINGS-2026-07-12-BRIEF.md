...
**Status:** PROPOSED (**NOT data-blocked, and never was — `uploads/trio/` already commits 25 post-host-axis nights against this brief's target of 15.** The 2026-08-04 fold turned out to REPRODUCE that committed set, not extend it. The 15 further nights on disk are CONFOUNDED — code version and date are the same variable — so they must be regenerated, not subset. What remains is a judgement: which estimator seeds the sim) · **Created:** 2026-07-12 · **DRAIN 2026-09-02 (Osprey) — re-verified; parked on an OWNER JUDGEMENT, and it is the same one blocking [[TCH-FUSED-ROBUST-HAT]].** The header is right that this was never data-blocked (`uploads/trio/` commits 25 post-host-axis nights against a target of 15; 115 night-dirs on disk today). The single remainder is *which estimator seeds the sim* — which is exactly the planted-sigma decision now sitting with the owner (fused triple 2.87/1.18/0.68, rho* 0.576, computed 2026-09-01). **RE-MEASURED 2026-09-11 (Osprey) — the triple is STABLE, and this is a baseline, NOT an adoption.** Re-ran the identical path (`node tools/tch-fused-corpus.mjs --dir uploads/trio`, unchanged code) on the current corpus, which is post-#2036 AND post-#2059 — #2059 rewrote 21 files under `uploads/trio/` at 20:46 on 09-01, ELEVEN HOURS after the original run (transcript output 2026-09-01T13:53:17Z), so this is a real test rather than a re-run on identical input. Old vs new, median [IQR] over solved nights: **O2Ring 2.87 [2.62-3.08] n=59 -> 2.85 [2.56-3.08] n=62 · H10 1.18 [0.90-1.34] -> 1.18 [0.94-1.36] · Verity 0.68 [0.43-1.01] n=48 -> 0.74 [0.46-1.11] n=51**; solved 59/115 -> 62/116, negative-variance 11/59 -> 11/62. Deltas 0.02 / 0.00 / 0.06 against a ~95 % CI on the median (1.57*IQR/sqrt(n)) of 0.094 / 0.090 / 0.131 — INSIDE on the strict measure, not merely inside the IQR (the IQR is the across-night SPREAD, which the tool itself warns is not the precision of the median). ⚠️ NOT night-identical: n moved 59 -> 62, so the delta could be #2059 or three newly-solved nights; both land inside resolution, so the verdict is unaffected and the attribution was deliberately not isolated. ⚠️ **ADOPTION IS STILL THE OWNER'S AND IS NOT THIS.** The ruling is re-measure AFTER the zero-sidecar whole-corpus refold, **which HAS now run — #2895 (`9b3363d0`, 2026-09-22) re-folded the corpus whole, 116 → 140 nights, one code generation (verified 2026-09-25: 387/387 committed exports carry ONE `kernel` 1.0.0/118ebed5, zero unstamped). Corrected here; the sentence below was true when written and is not now;** this run is the stable-under-#2059 baseline for that comparison. No numbers have been written into any brief body pending it. Also not to be compared with TCH-FUSED-ROBUST-HAT's 1.51/1.56/2.59, which is a PLANTED-RECOVERY result against synthetic ground truth and does not decay under a refold. **Owner: the OWNER. Next step:** the same ruling unblocks both briefs; do not re-derive, do not seed a sim under an unratified sigma. · **Owner ruling 2026-09-07:** re-measure the planted sigma **after** the whole-corpus refold, THEN adopt — **Osprey**. This is the ruling the header above was waiting on: do not seed a sim under the pre-refold sigma. · **TRIAGED 2026-09-25 (Osprey): park re-affirmed, and the blocking condition is OWNER-SHAPED — code movement cannot clear it.** Verified this pass: (a) step-1 stale query from an aged base 925 commits back returns a non-zero, positive-control-backed count for this brief; (b) **0 pending changesets in `changes/` name it** (56 pending, 11 of which DO name 7 other briefs — so the zero is this brief's property, not a dead query); (c) Rule 0 run 2026-09-24 on this brief's subject surfaced no sibling that answers it. Deliberately NOT re-derived beyond that: the blocker is a decision, and no landing on any source file resolves a decision. Status unchanged. · **CORRECTION 2026-09-25 (Osprey, caught by Heron's owner-gate sweep): the 2026-09-07 ruling's PRECONDITION IS MET and its next step is LIVE.** The ruling is *re-measure the planted sigma AFTER the whole-corpus refold, THEN adopt — Osprey*; #2895 ran that refold on 2026-09-22, three days before my own 2026-09-25 stamp above said "park re-affirmed". `tools/tch-fused-corpus.mjs` exists, so the assigned next step (`--dir uploads/trio`, fresh-budget window) is runnable by the fleet TODAY. **The adopt decision remains the owner's** — only the precondition moved. ⚠️ My stamp above reasoned that an owner-shaped brief cannot be invalidated by landings because no commit overrules a decision; that is half true and this is the other half — **a landing cannot overrule a decision but CAN satisfy its precondition**, so the surface check I skipped for owner-shaped briefs was exactly the check that was needed.
