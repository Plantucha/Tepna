...
-->
**Status:** PROPOSED (**RE-VERIFIED 2026-09-21 (Heron), read-only on the box: still two boxes, still on-box, and #2711 closed neither.** Wren's #2711 made the passive scan send the `or_patterns` BlueZ requires — a precondition for §2's matrix, not the matrix. The deployed config at `/opt/tepna/capture-host/config.yaml` carries **0 lines** of `scan_coexistence_verified`, so the flag is at its default OFF and no coexistence result has been recorded; §25's A–O acceptance has no observed-timestamp run either. Both are active RF operations on the production box during capture — owner-authorized, not a read. **core BUILT, remainder BOX-blocked — verified 2026-09-11 (Osprey): 6 of 8 Done-when boxes ticked, and the two open ones are both on-box runs, not code.** Verified in the TREE rather than from this brief's prose: `capture-host/oxy_presence.py` exists and `capture.py` references presence at 75 sites, so the PRESENCE axis, its trigger into the existing transaction, the default-OFF gating and the deadline injection are all present. Remaining: §2's coexistence matrix and §25's A–O acceptance, both requiring the box with observed timestamps — owner-authorized, and Wren/Heron's lane, not a rig session's. Nothing here is pickup-able by a repo coder.) · (parked 2026-09-02 — the module is built, armed and gate-backed (41 assertions, six planted controls). Two items remain and both need the box on the owner's bench: §2's BLE coexistence matrix, which is what earns `scan_coexistence_verified` (the gate exists — `oxy_presence.py:256`, refusal at `capture.py:7355` — only the measurement is missing), and §25's A–O acceptance run with observed timestamps. Confirmed on the box today: no `presence_harvest` or `scan_coexistence` key exists in its config, so presence is default-OFF and the matrix has not been run. **Owner:** owner (box session) · **Next step:** the coexistence matrix, which also unblocks the parent charter's identical item. **DRAIN 2026-09-20 (Wren, box):** keys still absent, observer still off (`presence scan: off — o2ring.presence_harvest.enabled absent` at the 07:06 start today). Two things the next step must know: (1) the observer's passive scan (`capture.py:9796`) shares `_O2_PASSIVE_SCAN` with `_connect_scan`, and that request is refused by vigil's BlueZ without `or_patterns` — the observer's own downgrade line (`:9801`) has never fired only because the observer has never started; when it does, every window will be ACTIVE (residue `2026-09-20-o2ring-passive-scan-needs-or-patterns`); (2) a precision on that residue's count: the 174 downgrade lines since 09-05 are one per DAEMON PROCESS (174 distinct PIDs = the deploy-driven restart count, ~11.6/day, all clean systemd stops), because the flag is per-process and re-learns the refusal on the first connect scan of every process — not 174 scan windows. Harmless per se; it means no process ever remembers the refusal.) · **Created:** 2026-08-26
