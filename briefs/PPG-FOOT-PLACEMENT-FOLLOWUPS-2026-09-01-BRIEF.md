...
**Status:** IN-PROGRESS (re-verified 2026-09-25, Kestrel triage, docs only: still the single §4 box, and still no owner ruling on sdb1 anywhere — searched the fleet's recorded owner decisions and this brief's history; a brief whose sole remainder is an owner decision with no fleet next step should be read as PARKED, not as live work; NO CHANGE. Previously re-verified 2026-09-21: the only open box is §4, an OWNER ruling on sdb1 — no ruling found, and the volume was seen mounting and unmounting on its own within eight hours overnight, which is the §4 finding continuing. Previously: parked 2026-09-05 — drain stamp, Magpie. §1 and §2 are EXECUTED (boxes ticked with their receipts); §3 is discharged by its own text — it is a design constraint for a *future* reopening, and with §1's H_axis failing confirmation that constraint is now the live one, so the box is ticked and the section is the record; **§4 — PUT TO THE OWNER 2026-09-06, who expressed NO PREFERENCE, so it is decided at session level and the actionable half is CLOSED.** Ruling: the incomplete `Ecg-nightly-archive` mirror is **KEPT, not deleted**, and its coverage is now stated where sessions actually look — `docs/CORPUS-LOCATIONS.md` carries a box recording the I/O failure, the 15-of-31 June 10–27 coverage, that it flattered two candidates here, and that the 2026-08-17 byte-identical verdict for the *CPAP* subtree predates the failure and is not evidence about the volume today (two subtrees, one volume — deliberately not merged). Deleting on a no-preference answer is the only irreversible option, and if the disk is unrecoverable those 15 nights may be the only copy; the hazard was misreading, and a coverage statement removes it. **Still owner/hardware, and not blocking this brief:** whether the disk is trustworthy and whether the partial mirror eventually goes. No session remounts it meanwhile. Next step: none schedulable by a session — the owner rules on sdb1, and this brief flips to DONE in the PR that records the ruling. Not PROPOSED: two of four items are executed work) ⚠️ **DRAIN R2 2026-09-19 — 13 landings, surface 2 files** (`ppgdex-dsp.js`, `tools/pat-per-led.mjs`), the files this brief names. ⚠️ **A 2-file surface is thin and the count is stated so a reader can weigh it.** 🔴 **CHECKED FOR A SPECIFIC HAZARD AND DID NOT FIND IT.** A finger-off capture was read as distinguishing two named span kinds — `absence` (value 100) and `in-wear-rail` (0/199) — so any claim here treating a PPG zero as a gap would be refutable. 🔴 **CORRECTION, SAME DAY: THOSE TWO NAMES WERE FALSIFIED WITHIN HOURS AND THIS STAMP CITED THEM AS LIVE.** The evening's pre-registered capture on a WORN finger refuted both in one consistent way (`capture-host/writers.py`, owner ruling D5, #2685): worn + disturbed AC — occlusion, strong light, handling — gives **flat 100 for up to 672 samples**, so 100 does not mean no-finger; and **0/199 never held as a steady rail under any stimulus**, arriving TOGETHER as short full-scale excursions in the same episodes. The sidecar now **emits the measurement and names nothing**. Not renamed — **refuted**. ⚠️ What SURVIVES is the conclusion (this brief makes no zero-is-a-gap claim, checked against its text rather than against the vocabulary); what does NOT survive is the framing, so any future check here reads `writers.py`, not this sentence. **This brief makes no such claim**: its `zero` references are §∅ denominator language and anchor-set arithmetic, not device value semantics. ⚠️ That vocabulary is **NOT on `main`** — pending in **#2675, re-checked 2026-09-19 and still OPEN**; `in-wear-rail` returns nothing from `origin/main`. Recorded as **WATCHED, not cleared**: a hazard cleared against an unmerged measurement is a verification against something that does not exist yet. 🔴 **That caution is why this correction is small** — #2675 was described as landed and was checked twice, both times OPEN; had the hazard been CLEARED, this brief would now assert a cleared hazard on a vocabulary that lasted one afternoon, **and a cleared hazard is not re-checked**. **A claim's half-life is a property of its SUBJECT, not of the writer's care**: a stamp about an actively-moving internal decision goes stale faster than a claim about an external project's release — the opposite of the instinct, and why the date is load-bearing. **Landings recorded, NOT assessed.** · **Created:** 2026-09-01 · **Residue:** 2026-09-02-oracle-split-duplicated, 2026-09-26-pat-ecg-index-axis-is-not-the-h10-sample-clock
...
- [ ] §4: owner has ruled on sdb1 and the stale mirror — **RE-VERIFIED 2026-09-21, no ruling found, and the volume FLAPPED overnight:** `/run/media/michal/data` (sdb1, `ntfs3`) was mounted at ~22:00 on 2026-09-20 — carrying the `Ecg-nightly-archive/CPAP` mirror and an abandoned Tepna checkout whose last commit is 2026-07-22 — and is unmounted at 06:00 on 2026-09-21 with no session having touched it. A disk that attaches and detaches on its own in eight hours is the §4 finding continuing, not a session remount. The item is the owner's by its own text and stays open; nothing here changes the recommendation that no session mount it.
