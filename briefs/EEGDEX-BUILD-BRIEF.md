...
**Status:** PROPOSED (deferred 2026-08-04, **precondition RE-VERIFIED 2026-09-02: still 0 files matching `*muse*`/`*eeg*` across the merged corpus root and `uploads/`** — parked correctly, no code exists and none should. Owner decision: PARK, do not build yet. No EEG corpus exists to build against: a scan of the capture tree, `uploads/`, and the capture host found **zero** Muse/EEG recordings — only `how-to-collect/muse-eeg.md`, which is collection guidance, not data. Building a node whose DSP could be validated on nothing would produce exactly the circular-oracle situation `REM-STAGING-FOLLOWUPS` §1 bans, and would add a ninth bundle, registry, reference guide, fixture set and badge surface to every fleet-wide gate for a signal the suite cannot yet see. Revisit when Muse recordings exist. The brief itself is sound and is kept, not archived — verified NOT built 2026-08-03 — there is no `eegdex-*.js` in the tree and `codegen/manifests/eegdex.manifest.json` declares `"status":"planned"`; `signal-spec.js:95-96` documents `EEGDSP` as a deliberate placeholder. This is the one genuinely open node brief, and it also blocks the EEGDex corner of the N-cornered hat) ⚠️ **DRAIN 2026-09-19 — the CORPUS half of this header is still TRUE; the CODE half is STALE.** Re-verified here: **0** files matching `*muse*`/`*eeg*` across the corpus root and `uploads/`, so the precondition and the PARK decision stand untouched. But *"there is no `eegdex-*.js` in the tree"* was correct when written (absent at both the 2026-08-03 and 2026-09-02 checks — verified by `git ls-tree` at each commit) and is **false since 2026-09-14**: `eegdex-dsp.js` is 417 lines on `main`, landed by **#2505** and extended by **#2513 #2515 #2517 #2583**, alongside `codegen/generated/eegdex-registry.js` and `tools/eegdex-selftest.mjs`. 🔴 **THIS IS STALE-SINCE, NOT FALSE-WHEN-WRITTEN, and the distinction is the point.** The checker was honest: `git ls-tree` at the 2026-08-03 commit AND at the 2026-09-02 commit both show `eegdex-dsp.js` absent, so each verification was correct at the instant it was taken and the world moved afterwards. A false-when-written header indicts the checker; a stale-since header indicts only the clock, and they earn different responses. Labelled in those terms deliberately — a first reading here showed the file present since the initial commit, which would have made this an accusation about whoever verified it. **Landings recorded, NOT assessed against this brief's done-whens** — whether staging-engine work satisfies a build brief is the owner's reading of coverage, not a drain's. The manifest/shipped-code contradiction is a SEPARATE defect and is logged as residue `2026-09-19-eegdex-manifest-says-planned` rather than carried here. · **Created:** (undated — pre-2026-07-03, grandfathered)
