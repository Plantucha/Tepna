...
| --- | --- | --- |
| **1.1** apnea typing (effort × desat) | ⚠️ **EXECUTED 2026-07-18, then WITHDRAWN 2026-07-31.** `APNEA-TYPING-FUSION-2026-07-18-BRIEF.md` (DONE) shipped MotionDex effort series × OxyDex desats → `apnea_obstructive`/`apnea_central`. `INTEGRATOR-APNEA-TYPING-REVIEW-2026-07-22-BRIEF.md` (DONE) then measured the feature against 26 nights of device-scored AASM events and **withdrew the type**: central-apnea effort is 0.99× its own night baseline, so an absolute floor typed 83.5–95.4% of centrals as OBSTRUCTIVE. The **effort series (Phase 1) stands** and is unchanged; only the typing rule is gone. `typeApneaByEffort` now reports `effortCovered` and asserts no type. | withdrawn — reopening needs a 2nd effort sensor or a 2nd modality, not a new constant |
