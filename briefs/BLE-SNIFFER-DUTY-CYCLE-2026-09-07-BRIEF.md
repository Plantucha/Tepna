...
**Status:** PROPOSED (**sniffer BUILT, this brief is duty-cycle POLICY over it, and its remainder is BOX work — verified 2026-09-11 (Osprey) in the tree.** `capture-host/ble_sniff.py` exists and already carries the CRC/span handling (#2240), so nothing here is "write a sniffer". 1 of 4 Done-when boxes is ticked (the mechanism named, 2026-09-05). Of the three open: ⚠️ **the "`tepna-sniff.service`/`.timer` are NOT in the tree — confirmed absent" clause was WRONG WHEN WRITTEN and is corrected 2026-09-19 (Heron).** Both landed **2026-09-06 in #2273** at `capture-host/systemd/`, five days BEFORE the 09-11 verification — most likely checked at the flat `capture-host/` path rather than `capture-host/systemd/`. They are also **INSTALLED on vigil** (`/etc/systemd/system/tepna-sniff.{service,timer}`, systemd knows both). The timer is **`disabled` and `inactive`** (vendor preset `enabled`). ⚠️ **AND THE OBVIOUS REMEDY IS WRONG — corrected within the same PR after Wren measured the RADIO independently.** My first reading was *"installed but disabled, so the action is an enable, minutes not a build"*. It is not: **there is no sniffer radio.** Both dongles now report `Zephyr HCI UART anchor np` (re-measured 2026-09-19 from `/sys/bus/usb/devices/*/product`; the RADIO-CLOCK-SIDECAR flash of 2026-09-07 took them), and the newest sniffer capture is `nightly-20260909-0701.pcap`, **2026-09-09 03:11** — sniffing stopped when the firmware changed. Enabling the timer would schedule a unit against **no hardware**, which is worse than leaving it disabled: it would run, produce nothing, and read as a working capability. ⚠️ So the open item is an **OWNER DECISION, not a task**: re-flash one dongle back to sniffer firmware (losing an anchor), or retire the duty-cycle capability. Either way it is owner-authorized — a daemon-state and firmware change on the box — and no peer relay changes that boundary. ⚠️ **Two-source note:** the tree said *installed*; only the BOX said *and inert*. The units and the radio are independent surfaces and checking one inverted the remedy derived from the other. Details corrected against my own reading of the box: units installed **2026-09-12 14:37** with no `timers.target.wants` link, and the last pcap is 09-09 03:11 (an intermediate relay said 09-05 03:57); the chunk-log verdicts reaching a human and §4's air-census bound are both small repo units against live code. Sized as policy + one pointer line, not as a build.) · **Created:** 2026-09-07
