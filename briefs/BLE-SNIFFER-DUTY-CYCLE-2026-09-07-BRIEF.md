...
**Status:** PROPOSED (**sniffer BUILT, this brief is duty-cycle POLICY over it, and its remainder is BOX work — verified 2026-09-11 (Osprey) in the tree.** `capture-host/ble_sniff.py` exists and already carries the CRC/span handling (#2240), so nothing here is "write a sniffer". 1 of 4 Done-when boxes is ticked (the mechanism named, 2026-09-05). Of the three open: ~~`tepna-sniff.service`/`.timer` are NOT in the tree — confirmed absent, so that box is genuinely open and is a BOX install, owner-authorized;~~ (superseded 2026-09-19 — see the later clause) the chunk-log verdicts reaching a human and §4's air-census bound are both small repo units against live code. Sized as policy + one pointer line, not as a build. · ⚠ **RE-VERIFIED 2026-09-19 (Wren, box + tree):** the 09-11 absence was wrong WHEN WRITTEN — `capture-host/systemd/tepna-sniff.service` + `.timer` + `capture-host/tepna-sniff.sh` landed 2026-09-06 (#2273, #2276), under `capture-host/systemd/`, not root. BOX (not checkable from code): the units are installed (`/etc/systemd/system/tepna-sniff.{service,timer}`, 2026-09-12 14:37) and the timer is DISABLED + inactive (no `timers.target.wants` link — who/when is not in the journal); last sniffer artifact **`nightly-20260909-0701.pcap` (2026-09-09 03:11, 1.74 MB)** — ⚠️ CORRECTED 2026-09-19 (Heron re-read the box, third read by Kestrel over ssh concurring): an earlier relay of this header gave `resmed-overnight-20260904-2032.pcap` (2026-09-05 03:57), which is the newest file of ONE NAMING FAMILY (and the largest, 67 MB) rather than the newest file. **The correction changes the causal story, not just a date**: nightly captures ran through 09-08 and 09-09, i.e. right up to the reflash, so the reflash CAUSED the stoppage instead of post-dating it by four days; both nRF dongles were reflashed to the HCI-UART image 2026-09-09 (Wren, 2026-09-18), so the box has NO sniffer radio regardless of the timer. This is duty-cycle policy over a capability the box no longer has — the remainder is an owner decision (re-flash one dongle back to the sniffer image, or retire the timer), not a build. Surface checked since 09-11 (subjects /sniff|nrf|dongle|air-census|chunk-log/): #2408 btattach unit, #2393 radio-clock — neither touches this brief's boxes; **CORROBORATED INDEPENDENTLY 2026-09-19 (Heron, #2664)** from the tree alone — same false absence, found without sight of the box. That pass sized the remainder as *"the remaining action is an **enable**, not a write-and-install … sized in minutes rather than as a build"*. ⚠️ **THAT SIZING IS SUPERSEDED by the box facts above**: with both dongles on the HCI-UART image, enabling the timer yields a unit with no sniffer radio. Two independent surfaces agreed on the FINDING and disagreed on the CONSEQUENCE, and only the box could say which — which is why the remainder is an owner decision (re-flash one dongle, or retire), not an enable. · **PARKED ON AN OWNER DECISION — D2, 2026-09-19: leave the sniffer INERT.** No re-flash, no retirement; the timer stays installed and disabled and the dongles keep the HCI-UART image. Nothing in this brief is schedulable until the owner reopens it; a triage that finds the timer disabled has found the decision, not a defect) · **Created:** 2026-09-07
