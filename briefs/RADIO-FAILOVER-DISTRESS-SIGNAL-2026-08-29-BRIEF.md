...
**Status:** DONE — 2026-09-07 (was PROPOSED, parked 2026-09-02 — part (a) is BUILT and CONFIRMED RUNNING IN PRODUCTION today: `/api/state` on the box carries `radio_distress` rating a live link (`nights: 14`, `state: ok`, `1 rated link(s) all within band`) and `radio_distress_adapter` folding it per adapter. Part (b), the arming, is the only open item and it is the OWNER's against §6's pre-stated criterion — verified on the box today that it is NOT armed: `radio_switches` is absent from `/api/state`, i.e. the signal half runs and the switching half does not. That is a one-key change to the box's gitignored `config.yaml` (`watchdog.distress_failover: true`), so no repo change can close this and no session should try. **Owner:** owner (arming only) · **Next step:** owner arms it, or records a decision not to) · **Created:** 2026-08-29 · **Owner ruling 2026-09-07:** part (b) — **arm only when the pre-stated §6 criterion fires**; the fleet reports the first DISTRESSED night. That rule DISPOSES of the last open item, so the brief CLOSES on it. The other five Done-when boxes were re-verified 2026-09-07 rather than read off the ticks (`tests/test_failover_planted_wedge.py` + the three `*distress*` test files: **51 passed, exit 0**). ⚠ **Closing does NOT mean armed** — arming remains a one-key change to the box's gitignored `config.yaml` that no repo change can make. Last verified NOT armed 2026-09-02 (`radio_switches` absent from `/api/state`); NOT re-checked on 2026-09-07, the box being unreachable from that session.
...
- [x] `down%` and `frames_dropped` are recorded as report-only, with the reason they are not
      triggers. *(§3.1 above)*
- [x] **Part (b), the arming** — RESOLVED BY RULING, **not by arming** (owner 2026-09-07): arm only
      when §6's pre-stated criterion fires, and the fleet reports the first DISTRESSED night. The box
      is deliberately still unarmed; that is the ruling's intent, not an outstanding task.
