...
**Status:** IN-PROGRESS — 2026-09-25 (**TRIAGED 2026-09-25 (Wren, box + tree; rule 0: "solid night verdict bands continuity completeness validity clocks timebase")**, Done-when item by item. **(1) MET:** bands stated 2026-09-24 before scoring, and the deployed verdict makes UNKNOWN `not settled` precedence rule 1 (the running daemon 2cd12712). **(2) PART:** `SOLID-VERDICT.json` is on the box for 26 of 31 nights since 2026-08-25; absent for 2026-08-25 → 08-28 (each has a LOSS-AUDIT, so it is a backfill gap) and for 2026-09-24 (not settled, which is correct). It is NOT drawn on the monitor: `monitor.html` never references it, and there is no verdicts API; this is the "monitor line" named next below. Every verdict to date is UNKNOWN. **(3) OPEN:** §5's work order. Wren's §4 unit is DONE for the last 7 settled nights (§4.1, 2026-09-25): every unattributed H10 gap is either the daemon's own pull, missed by an attribution window that looks only backward (39 gaps, 11.0 min), or a DELAY, not a loss (16 gaps; the device-clock test is FIXED in the §4.1 unit). The operator's time sync is a third, separate cause (row). Refuting the bands against real nights waits on those two fixes, because both inflate the unattributed leg. **(4) NOT REACHABLE YET, by construction:** the timebase term (measured since 5a1ff8fa) has no PASS path until the A5 step tripwire is built, so no night can be SOLID and the 14-night run cannot start. **Tonight (2026-09-24) is the first night scored end to end by the deployed verdict with wear ends, per-gap times and a measured timebase.** It stays UNKNOWN `not settled` until 2026-09-25's first data write; a pre-registered per-band prediction is on the box (vigil wren-notes, prediction-2026-09-24.md). · EARLIER: IN-PROGRESS — 2026-09-23 (owner-ratified programme; assignments issued the same evening; the first ranking below is measured from the box's own loss audits, the verdict is Magpie's unit; **§3's bands STATED 2026-09-24 (Magpie) before any night is scored** — the composition core landed (#3004, #3008); **the NIGHTLY runner is built (verified 2026-09-24, Magpie, §3.7 A6): `capture.loss_poller` writes `SOLID-VERDICT.json` on the loss audit's own trigger, with the run in `result.run`, and every night reads UNKNOWN on the timebase term until the residual pass lands**; the residual pass and the monitor line are next, and Wren refutes the judgement bands against the re-audited 28 nights first)) · **Created:** 2026-09-23 · **Relates:** `STRATEGIC-PRIORITIES-2026-08-26-BRIEF.md` (P1–P4 deferred by this programme; P5's "two error-free weeks" is this programme's exit) · `OPERATIONAL-MATURITY-ROADMAP-2026-08-27-BRIEF.md` · `ABSENCE-SURVEY-2026-09-22-BRIEF.md` (the one processing lane that continues) · `SAMPLE-VALIDITY-ENVELOPE-2026-09-17-BRIEF.md` · CLAUDE.md §∅ and §🔒 §7
...
### 4.1 · Wren's unit — every H10 gap on the last 7 settled nights, attributed (measured 2026-09-25, read-only)

Nights 2026-09-17 → 09-23 (09-24 was running). Gaps come from the deployed `loss_audit.stream_gaps` over EVERY H10
`_ECG.txt` of each night, plus the gaps BETWEEN files. Each file's worn part comes from the deployed `wear_ends`.
Attribution is the deployed `attribute_gaps` over `read_journal((name, address))`. Every gap it left
`unattributed` inside the worn part was then witnessed three ways:
- the ECG file's own device clock (`sensor timestamp [ns]`: the device-time step across the gap against the
  host-time step);
- `PMDARRIVAL.csv` batches, `LINK.csv` epochs and `STARTS.csv`;
- the raw daemon, bluetoothd and kernel journals for the window.

Scripts and records: vigil wren-notes `h10_attrib.{py,json}`, `h10_devclock.{py,json}`, `h10_window.py`.

| class | gaps | size | nights | cause, with its evidence | the instrument that sees it |
|---|---|---|---|---|---|
| attributed by the audit | 280 + 7 + 1 | 452.5 min not-worn drop · 2.6 min pull · 1.4 min re-negotiate | 09-20, 09-21 | `daemon:not-worn drop` ×280, `daemon:pull paused live` ×7, `daemon:stream stall re-negotiate` ×1 (journal) | journal, as the audit reads it |
| **REAL LOSS, unattributed** | **39** | **11.0 min** (device step 10–55 s, typ. 12 s) | 09-20 (17), 09-21 (22) | **every one is `daemon:pull paused live`**: the daemon's offline-recording pull pauses live capture. Its journal line (`Polar 24:AC:AC:02:84:96: offline-recording op — live capture paused`) is logged 1–3 s AFTER the gap's last delivered row (33 at +1 s, 3 at +2 s, 3 at +3 s), and none is logged before | the journal already carries it; the audit looks only BACKWARD from the gap start (`attribute_gaps`: the last event at or before `t0`), so a cause logged after the last row is never found. Row `2026-09-25-loss-audit-attribution-looks-only-backward` |
| **NOT LOSS — DELAY** | **16** | 0.83 host-min, **0 device samples** (host step 2.3–5.7 s, device step 0.008 s = one sample at 130 Hz) | 09-17 (3), 09-19 (3), 09-20 (3), 09-21 (1), 09-22 (6) | late delivery. PMD ECG batches DID arrive inside every such "gap" (1–2 each). Only **5 of 16** have an `event loop stalled` warning within ±5 s, so a stall is corroboration where it exists, not the test | **the device clock in the same file**, which is now the test: **FIXED in this unit.** `loss_audit.stream_gaps_split` calls a host gap a DELAY when the device step stays under 1.5 sample periods; `audit_night` publishes it in `delays` / `delayed_min`, never in `gaps` / `lost_min`. On 2026-09-24 this turns 21 stall gaps (2.2 min, all devices at the QC poll's cadence, e.g. 23:40:16: 8 s host, 0.008 s / 0.018 s device) into delays, and keeps the operator time-sync losses (Verity 44 s + 13 s, H10 21 s + 11 s) as losses |
| not worn | 1 | 41.2 min (between files) | 09-19 | the H10 went to its idle power-off after an offline pull (`… powered off — idle timer (expected until re-wear or charger …)`, +34 s); the first file was too short for `wear_ends` to judge (`under 60 epochs`), so the gap was scoped as worn by default | the journal (after the gap start, like the pull rows) |

So the "unattributed H10 losses" are two populations with opposite remedies. 39 are real, fully explained, and
