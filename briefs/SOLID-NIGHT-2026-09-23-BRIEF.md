...
| **NOT LOSS** | **16** | 0.83 host-min, **0 device samples** (host step 2.3–5.7 s, device step 0.008 s = one sample at 130 Hz) | 09-17 (3), 09-19 (3), 09-20 (3), 09-21 (1), 09-22 (6) | BLE delivery latency: the samples arrived late, and PMD ECG batches DID arrive inside every such "gap" (1–2 each). No journal line, because nothing happened on the link | the device clock in the same file. The audit measures host stamps against a 2.0 s cadence cut, so late delivery reads as loss. Row `2026-09-25-loss-audit-counts-delivery-latency-as-loss` |
...
Nothing needs a new instrument: the journal and the device clock were both already on disk.
