...
-->
**Status:** DONE — 2026-08-23 · **Created:** 2026-08-23 · **Follows:** `OXYII-ACQUISITION-CHARTER-2026-08-23-BRIEF.md` (G1, spec §8–§19) · **Builds on:** `oxy_inventory.py` (G2, #1681), `oxy_restart.py` (G3, #1689) · **Affects:** `capture-host/pull_session.py`, a new transfer module · **The invariants' TESTS live in `capture-host/tests/test_chaos_ordering.py`** (`.part` staging, fsync file-before-directory, `a_part_is_never_adopted_however_complete_it_looks`, `a_failed_rename_leaves_the_source_never_neither`), with `test_cpap_spool.py` C1/C3 (`kill_mid_round_leaves_part_and_repull_promotes_byte_identical`, `crash_between_promote_and_ledger_is_idempotent_on_repull`) and `test_pull_session.py`'s `a_truncated_transfer_is_recorded_PARTIAL_and_re_pulled_next_run`. ⚠️ **VERIFIED 2026-09-12** after an external review graded these invariants UNTESTED: they are tested per device, but named for `.part` discipline and chaos ordering rather than for the word *transactional*, so a vocabulary search finds nothing. The FOLLOWUPS brief already carried this pointer; the parent did not, which is why the review landed here and found none. ⚠️ **Polar is DELIBERATELY outside the `.part` model** — a live stream is not a two-phase transfer, so its analogues are the header-only discard at teardown and the per-stream sidecar's `rows` / `flush_failures` / `rows_lost`, not a staged rename. ⚠️ **And `test_chaos_ordering.py` is NOT a durability test and cannot be made one** — its own header says so: `fsync` defends against MACHINE failure, while a process kill loses nothing from page cache, so a `kill -9` harness added here would pass for the wrong reason.
