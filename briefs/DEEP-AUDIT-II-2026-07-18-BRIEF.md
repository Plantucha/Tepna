...
| 15 | ⏳ **PARTIAL 2026-07-21** — **CPAPDex `pressureChangePoints`**: the **before/after self-contradiction** is FIXED — before/after are now the two adjacent segment medians (recomputed from the final boundaries, not each recursion's `[lo,hi]`), so `cp[i].after == cp[i+1].before`, no `before` that was never a setting, and correct `delta`; gated by a **3-regime known-answer** test (RED-first: old code gave `cp0.after 7.5 ≠ cp1.before 6`, delta −4.5 not −6). **STILL OPEN (corpus-gated):** the **append-invariance + `PEN_K` re-tune** half — the current penalty barely fires on synthetics (a clean 12→6→9 with 12 nights/regime finds 0 CPs; appending nights flips detection), but re-tuning `PEN_K` must preserve the corpus behavior the finding pins (epap95 10.7→6.8 caught, ZERO false positives on `pressureEnvIqr`), so it needs the gitignored CPAP corpus (owner) + the append-invariance gate | 6.1 | 1 bundle · 3-regime + append-invariance gates, verified RED first |
