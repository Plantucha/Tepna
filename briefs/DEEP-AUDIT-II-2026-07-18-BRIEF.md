...
-->
**Status:** DONE — 2026-07-21 (**All punch-list findings CLOSED + gated — re-verified against the tree 2026-07-21.** The §7 MotionDex cluster is fully merged (#329–#333: §7.1–7.4, §7.6, §7.9, §7.5; §7.7/§7.8 were already closed). The Tier B/C rows the historical line below still lists as "STILL OPEN" — #36 · #10 · #15 · #18 · #5 · #6 · #38 · #26 · #37 · #16 — were each re-verified **fixed with a both-direction gate** (that line lagged the code, exactly as its own ⚠️ warns). **SOLE RESIDUE: §5 MotionDex/PMD corpus rescale — DEFERRED** as an owner corpus operation: there is no in-tree rescale tool and the real captures are out-of-git, so it is neither verifiable nor executable from the tracked tree (already-committed captures may still be 16.4×/655× off; a corpus re-run closes it, not code). The historical IN-PROGRESS execution log — itself a faithful record that its status line lagged the code — follows unchanged. · **~22 punch-list items EXECUTED + gated.** ⚠️ **This header was ~15 items behind the tree until 2026-07-19 and is now reconciled AGAINST THE CODE, commit by commit** — the previous summary still read "#11 MEASURED BUT NOT EXECUTED" and "#12 §9.2 + §9.3 still open" after all three had landed, which is exactly the drift `f517bdc` already fixed once and the failure this brief's own §"Done when" calls its highest-value follow-up. **Verify against the tree, not against this line.**
**Now closed:** #1+#2+#27 OxyDex desSev/SBII/gapPct (PR #229, + `tools/regen-oxydex-goldens.mjs`) · #3 ECGDex QT window-edge (PR #227) · #4 HRVDex absent column ⇒ NaN (`4370099`) · #7 Integrator multi-record events (PR #238) · #8 GlucoDex drift KPI (PR #232) · #9 Integrator long-gap interpolation is not coverage (`e262778`) · **#11 Lomb–Scargle Parseval — EXECUTED, no longer awaiting an owner decision** (`52a9aec` → PR #260): `totalFull` accumulates over the full support (`ppgdex-dsp.js:1032/1057/1070`), mirrored in `pulsedex-dsp.js`/`ecgdex-dsp.js`, and `audits/DEX-DSP-AUDIT-FREQ-HRV.md:54` now carries an explicit retraction of the form it had prescribed; registry cites re-stamped. The fixtures did move (real overnight RR ~2 %, events-fixture HF 382→1) — that WAS the decision. · **#12 FULLY closed** — §9.1 (PR #239) · §9.2 weighted personal baseline (`55680a1`, PR #243, `oxydex-cross.js:209`) · §9.3 direction from τ not OLS (`cba0752`, PR #262, `oxydex-cross.js:245` + `dirDisagree`) · #16 CPAPDex ODI over analyzable time (`2dcb116`) · #17 surge corroborates ONE apnea (`b6667c6`) · #19 ECGDex parked companion (`2ce2995`) · #20+#35 24 h norm scoping / last fail-open badge (`668e923`) · #21 PulseDex artifactClean median (`bf75ed3`) · #23 GlucoDex truncated grid (`ec819cc`) · #24 clampFloor per-event (`0601677`) · #25 roc() tied scores (`03fa765`) · #28 orchestrators boot their hosts (`1b41b66`) · #31 Integrator footer span (`1d8ec81`) · #31b PpgDex replicated channel (`ca80af4`) · #32 GATE B fails closed on a missing outputHash (`9cb9901`) · #40 parseNutrition row/date handling (`16132e2`). Each shipped its own both-direction mutation-verified gate, because this audit's structural finding is that six defects survived behind gates asserting the wrong thing.
**Still open — verified open in the tree 2026-07-19:** #14 MotionDex effort epochs (§7.1) — its stated blocker has PARTLY dissolved: `PMD-DECODE-SCALE-AND-RATE` §2/§3 ARE landed in the host (`polar_pmd.py:204/357`, `capture.py:462/512/647`, `writers.py:158`) so new captures are correctly scaled, and the precondition it named ("write `tools/regen-motiondex-goldens.mjs` FIRST — none exists") is met. What is NOT verifiable from the tree is §5's corpus rescale — no rescale tool exists in `tools/`, the corpus is out-of-git, and every §4 Done-when box at `:171-185` is still unchecked, so ALREADY-COMMITTED captures may still be 16.4×/655× off. · **Tier A — ALL CLOSED:** #33 (FALSE POSITIVE for five nodes — see §12.1 — but **OVERTURNED for OxyDex by DEEP-AUDIT-VI F14, 2026-09-01**: `oxydex-dsp.js` reaches `UP.age`/`upVO2category` inside `compute()`, so `oxydex-profile.js` is now inside the closure) · **#43 CLOSED 2026-07-20** (SpO₂ elevation norm-shift disclosed at the Oxygen KPI; render-only, `computeHash` stable) · **#42 CLOSED 2026-07-21** (`reconstructEventTMs` order-independent anchor documented) · **#39 CLOSED 2026-07-21** (`hrvStability` per-window epoch count `n` surfaced + gated). · **Tier B — 5 of 8 CLOSED** (reconciled 2026-07-21, tree drift): ✅ #13 (`006a8ac`) · ✅ #22 (PR #287) · ✅ #29 (`ab4b40d`) · ✅ #30 (PR #281) · ✅ #34 (PR #285) — each fixed + gated. **STILL OPEN:** #36 (PulseDex `hrv.time` scale-mix) · #10 (oxydex-fusion coverage trilogy) · #15 (CPAPDex `pressureChangePoints` — NB `PEN_K` absent from the tree + the fn now lives in render/cross, so re-scope against current code before executing). · **Tier C:** #18 (partial — §10.5 fixed, the guardless selector remains) · #5+#6 · #38 · #26 · #37. Rows carry ✅/⚠️ markers inline — trust the tree over both those and this line.) · **Created:** 2026-07-18 · **Executes:** `AUDIT-PROMPT.md` (the deep-audit charter) · **Follows:** `DEEP-AUDIT-2026-07-11-BRIEF.md` (DONE 2026-07-12, 21 findings) · **Sibling-axis:** `TEST-AUDIT-FINDINGS-2026-07-18-BRIEF.md` (gates that stay green) · `DEEP-SCOUT-HOLLOW-GATES-2026-07-18-BRIEF.md`
...
| 5 | ✅ **DONE 2026-07-21** — **ECGDex `parseECG` single-delta fs** → fs now the MEAN non-gap interval (stamp-span cross-check) across all THREE mirrored sites (`parseECGText` · WORKER_SRC · inline). Corpus-validated: 3-h recording gives 130.00; single-delta locked 125/128/133 per part. Fixture byte-identical (clean float ms). | 4.3 | 1 bundle · landed with #6 |
| 6 | ✅ **DONE 2026-07-21** — **ECGDex discards parsed sample gaps** → `analyze()` folds each raw gap's excess dead-time into later beats, so gap-aware coverage sees it (folded 30 s gap: coverage 100→98.3 %, rMSSD unchanged). Inert on clean recordings. Both-direction gate added. | 4.2 | rode #5's bundle |
...
| # | Item | § | Size |
|---|---|---|---|
| 11 | ✅ **DONE 2026-07-19 (`52a9aec` → PR #260)** — full-support Parseval denominator in ppgdex/pulsedex/ecgdex; `audits/DEX-DSP-AUDIT-FREQ-HRV.md:54` carries the retraction; fixtures moved as predicted (real overnight RR ~2 %, events HF 382→1). **Lomb–Scargle Parseval calibration** — **the single largest blast radius in this report**; correct `audits/DEX-DSP-AUDIT-FREQ-HRV.md` too | 3.1 | **3 bundles, one unit** · **ALL fixtures for 3 nodes MOVE** · commit an adversarial twin · loud CHANGELOG |
| 12 | ✅ **DONE 2026-07-19 — §9.1 (PR #239) · §9.2 (`55680a1`, PR #243) · §9.3 (`cba0752`, PR #262)** — **Cross-night estimator trilogy** — all five clones together; two are documented spec breaches | 9.1–9.3 | **5 bundles** · **mandatory new non-uniform-weight gate — the feature is untested** |
| 13 | ✅ **DONE** (`006a8ac` — an index slope is never a per-day slope; `slopeBasis` surfaced, gated across all 5 cross clones both directions, `tests/dex-tests.js:4093`) — **`slopePerDay` becomes slope-per-recording** when any item is undated | 9.4 | rides #12 |
...
| 15 | ✅ **DONE 2026-07-21** — **CPAPDex `pressureChangePoints`**: (1) the **before/after self-contradiction** was fixed in PR #317 — before/after are the two adjacent segment medians (`cp[i].after == cp[i+1].before`, correct `delta`), gated by a 3-regime known-answer test. (2) The **append-invariance + PEN_K half** is now fixed: the root cause was NOT `PEN_K`'s value but the SCALE ESTIMATOR — the penalty used the GLOBAL MAD, inflated by the very steps it detects, so a clean 12→6→9 (12 nights/regime) found 0 CPs and appending a regime ERASED an earlier CP. Replaced with the MAD of successive differences (`med(\|Δ\|)/√2`, level-shift-immune; `/√2` ⇒ equals the old global MAD on white noise, so the zero-FP behaviour on noise-dominated `pressureEnvIqr` is preserved BY CONSTRUCTION, `PEN_K` unchanged at 4.0). Verified on the real module: clean → 2 CPs, append-invariant (`[10]→[10,20]`), 0/300+ FP on noise. New both-direction gate. ✅ **CORPUS-VALIDATED 2026-07-21** on the full **180-night** SD card (`Ecg nightly/CPAP`, driven through the real CPAPDex via `tools/cpap-corpus.mjs`): epap95 → exactly **1 change point at 2026-06-12, 10.74→6.52** (the pinned step), pressureEnvIqr → **0** change points (the pinned empty). NEW == OLD on this corpus (one step over a long baseline barely contaminates the global MAD, so pre-fix coincidentally worked here) ⇒ **zero regression** on real data; the fix additionally repairs the multi-step / short-regime cases (12→6→9, append) the single-step corpus never exercises. | 6.1 | 1 bundle · 3-regime + step-immune/append-invariance gates + 180-night corpus |
...
| 18 | ✅ **DONE 2026-07-21** — **Companion pairing, three parts.** §10.5 (fnameStampMs Polar-anchor) landed earlier (ENGINE-VERIFICATION §1.1). §10.2 + §10.3 now: BOTH `DexIngest.pickNearestByStamp` (the app-side ECG/PPG pick) and the orchestrator `pairCompanions` stop scoring a null/unparseable stamp as epoch 0 (`es == null ⇒ skip`, not `|| 0`) and reject a candidate >1 day from the primary (`MAX_COMPANION_GAP_MS`/`PICK_MAX_GAP_MS = 24 h`), so a 5-day-old ACC can't pair. The locked "single-candidate/epoch-0" test is updated to the corrected contract; a §10.2/§10.3 known-answer gate covers both loci. Fixture-inert (committed clips are single-file, no companions → export byte-identical). ✅ **CORPUS-VALIDATED 2026-07-21** on the real H10 set (`Ecg nightly/`, 250 companion files / 33 nights, device serial `02849638`): the OLD unanchored `fnameStampMs` collapsed the WHOLE corpus to **2** stamps (`0292-01-07`, the serial parsed as a date) — the nearest-stamp tiebreak was totally inoperative — while the shipped anchored parser resolves **155** distinct per-session stamps; and the real `pairCompanions('ecg', …)` paired a true same-session ACC (Δ 4 s) and REJECTED a cross-night ACC (Δ 12 days) by the 24 h guard. | 10.2, 10.3, 10.5 | **2 bundles** (ECGDex + PpgDex) + `signal-orchestrate.js` |
...
| 22 | ✅ **DONE** (PR #287 — cadence sub-harmonic refractory; gated `tests/dex-tests.js:3704`) — **PpgDex ACF sub-harmonic — HR halved** | 3.2 | 1 bundle |
...
| 26 | ✅ **DONE 2026-07-21** — **Profile layer** (`dex-profile.js`, shared spine, all 8 apps re-bundled). **§13.4** `_clampAge` floor 6→**18** (its NHANES/ACSM/Tanaka norms are all adult; children fell back to the population default). **§13.2** `vo2Norm` plateaus at the age-65 table row → the panel now discloses "norm held at 65+" past that (`vo2AgedOut`). **§13.1** `vo2Category`/`vo2Percentile` are the suite's own ratio-to-50th-pct heuristics, NOT the ACSM/FRIEND percentile distribution — relabelled `heuristic` / "vs 50th-pct norm", never `validated`/"ACSM" (LITERATURE-USE-POLICY: no fabricated authority). Render-only, `computeHash` stable; goldens carry `Unknown` headless so all reproduce byte-identical. No locked test needed updating (the computation is unchanged; only the tier/citation). **Follow-up (structural, not a fabrication):** the deeper "single-source onto OxyDex's ACSM table" — reconcile this heuristic with `oxydex-profile.js upVO2category` (which DOES use the real ACSM thresholds) so one page shows ONE VO₂ category — remains; it is a two-file refactor, not a false claim | 13.1, 13.2, 13.4 | **shared spine — all 8 bundles, SERIALIZE** · `computeHash` is **blind here** → verify-fixtures by hand |
...
| 29 | ✅ **DONE** (`ab4b40d` — the stream fallback resets every accumulator before re-reading; gated `tests/dex-tests.js:4065`, both directions — reset-before-reread AND streaming-path-must-not-reset. NB the gate is a *structural* source-pin, not an execution of `WORKER_SRC`'s body; an async execution gate remains a nice-to-have) — **ECGDex worker duplicates the parsed prefix** | 4.4 | `-app.js` only → provably export-inert |
| 30 | ✅ **DONE** (PR #281 — `persistHRVRows` storage-failure warning no longer overwritten; gated `tests/dex-tests.js:3929`) — **HRVDex storage-failure warning is dead code** | 1.11 | 1 bundle |
...
| 34 | ✅ **DONE** (PR #285 — DMY preference threaded through HRVDex/PulseDex/Integrator without flipping `preferDMY`; gated `tests/dex-tests.js:3882`) — **DMY file-lock never threaded** | 1.10 | 1–3 bundles · outputs hold (corpus inputs are ISO) |
...
| 36 | ✅ **DONE 2026-07-21** — **PulseDex export mixes analysis scales in `hrv.time`**: added a `windowNote` (+ `units`) disclosing that `rmssd/sdnn/pnn50/hr/meanRR` are whole-record while `lnRMSSD` is the representative 5-min epoch median (`cRm = longRec ? dispRm : rm`), mirroring the ECGDex precedent. Additive/MINOR; synthetic golden regenerated (2 fields moved, no numeric drift) so the `env.equiv.pulsedex` leg now pins the note; real corpus fixtures gain it at release | 3.5 | 1 bundle · **fixtures MOVE** · MINOR |
| 37 | ✅ **DONE 2026-07-21** — **`clock.js` out-of-range component rolling**: `Date.UTC` silently rolled `2026-13-45 25:99` onto a plausible WRONG instant. New `_ckMk` builds tMs only if the date round-trips (real month + calendar day, rejects Feb 30 / Apr 31) and the time is 0–23:0–59:0–59.0–999 — else honest null (Clock §2.6); the ONE legitimate overflow, ISO end-of-day `24:00:00`, normalizes to next-day 00:00 (NO bare `h>23` guard, as the finding warned). All 7 `parseTimestamp` paths routed through it. **Contract AMENDED** in `CLAUDE.md` §🔒 (new step 7) + a §12.3 known-answer gate (12 assertions). Shared spine → all bundles re-stamped; valid stamps parse identically so every fixture reproduces byte-identical (verify-manifest 0 drift) | 12.3 | **shared spine — all 10 bundles, SERIALIZE** |
| 38 | ✅ **DONE 2026-07-21** — **Cross-night bootstrap LCG overflows 2^53**: `Math.imul(seed, 1103515245)` replaces the bare `*` (which reached ~2.3e18 ≫ 2^53, rounding away the low bits the `& 0x7fffffff` keeps) in all five `*-cross.js` `bootstrapDeltaCI` clones (PR #319) **and now `event-coupling.js:447`** — the deferred locus. Its `selfTest` broke on the corrected sequence because `Aall` could draw into the final-window zone (span-end overrun → `excluded=1`); hardened sequence-agnostic by drawing within `[0, SPAN − window]` so coverage is the sole exclusion under test (selfTest 35/0). §9.5 source-scan now PINS all six sources incl. event-coupling.js (13/13). LCG is `selfTest`-only → production `coupling()` byte-identical; Integrator/OverDex re-bundled (they inline it), Integrator fixture `verifiedUnder` re-verified vs corpus. The five cross clones' real n≥7 crossNight fixtures were re-verified vs the corpus in PR #322. | 9.5 | 6 sources · selfTest-only, no export moved |
| 39 | ✅ **DONE 2026-07-21** — **`hrvStability` short trailing group**: each `hrvStability.series` point now carries its epoch count `n`, so a 3-of-6-epoch trailing window is visible instead of silently equal-weighted (slope fit unchanged — real-corpus effect immaterial). `ECGDSP.hrvStability` exposed + a `[6,6,6,3]` known-answer gate. Both committed ECGDex fixtures are short (`hrvStab=null`) so they reproduce byte-identical; real nocturnal nights gain `n` (owner corpus re-verify at release) | 4.5 | ride another ECGDex re-bundle |
...
| 43 | ✅ **DONE 2026-07-20** — **OxyDex profile elevation copy + dead `pd_elev` disclosure**: `upSpo2Adj()` silently lowered every SpO₂ verdict threshold while the only disclosure wrote to a non-existent DOM id. Surfaced the norm-shift at the Oxygen KPI via new `spo2ElevNote()` (shown only when the adjustment is non-zero); removed the dead `pd_elev` write. Render-only, `computeHash` stable (export-inert — no fixture movement), OxyDex re-bundled `471bd50fe5bb → 1fa2cb5dd5eb`. The remaining dead `pd_*` panel writes are §13's profile-panel findings, untouched here. **Regression hotfix 2026-07-21:** the `pd_elev` removal also dropped the shared `_elv`/`_adj` locals still used by the `profileElevSub` sub 3 sections down → `profileDerivedUpdate` threw a ReferenceError (swallowed as an optional-module load warning, so CI stayed green); locals re-declared at the surviving use site | 13.3 | render-only |
