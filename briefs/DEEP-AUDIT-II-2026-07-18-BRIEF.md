...
| 38 | ⏳ **PARTIAL 2026-07-21** — **Cross-night bootstrap LCG overflows 2^53**: FIXED in all five `*-cross.js` `bootstrapDeltaCI` clones — `Math.imul(seed, 1103515245)` replaces the bare `*` (which reached ~2.3e18 ≫ 2^53, rounding away the low bits the `& 0x7fffffff` keeps). Gated by a source scan across every clone (`tests/dex-tests.js §9.5`). Committed synthetic goldens have <7 nights so they are byte-identical; real n≥7 crossNight fixtures shift (owner re-verify at release). **STILL OPEN:** `event-coupling.js:447` carries the same LCG, but its co-loaded `selfTest` builds demo streams *from* that sequence, so the `Math.imul` fix perturbs the data enough to break the brittle `excluded === 0` coverage assertion (a collision in the new sequence) — deferred until the selfTest is hardened to be sequence-agnostic | 9.5 | 5+ bundles · **every crossNight fixture with n ≥ 7 MOVES** |
