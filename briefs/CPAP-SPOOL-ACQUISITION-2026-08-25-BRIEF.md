...
**Status:** PROPOSED (re-verified 2026-09-25, Kestrel triage, docs only: unchanged since Heron's 09-19 pass (#2677); the four open done-whens — a real spool pulled end-to-end with its fragment log, the assembler witnessed on real fragments, the §11 three-way convergence on ≥1 night, the follow-up note — are all ATTENDED BOX work and none has a repo-side prerequisite; the symbol citations Heron switched to (`_maybe_start_cpap_spool_pull`, `cpap_spool_caller.py:64`) still resolve on `origin/main`; NO CHANGE. Parked 2026-09-02 — re-verified today and the blocker still holds, but narrower than the header claimed. The caller IS shipped behind the default-OFF gate and that box is now closed; note the brief's cited `capture.py:6474` has DRIFTED — the wiring is `_maybe_start_cpap_spool_pull` at `capture.py:7277`, import at `:6956`, arming at `cpap_spool_caller.py:64`. ⚠️ **RE-VERIFIED 2026-09-19 (Heron) and TWO OF THOSE THREE HAVE DRIFTED AGAIN**, in 17 days: `_maybe_start_cpap_spool_pull` is now `capture.py:9445` (call at `:10399`), the import is `:9060`; only `cpap_spool_caller.py:64` held. That is the second drift of the same reference — **stop re-stamping the number and cite the SYMBOL**, which has not changed once. `capture.py` is the hottest file in the lane and a line citation into it is stale before the next triage reads it. 🔴 **AND THE SPOOL IS NO LONGER EMPTY-HANDED — the backlog was RETRIEVED on 2026-09-19 10:00.** The ledger went 2 rows → **4**: `from=2026-09-01T16:00 → cursor 2026-09-17T16:00` (3434 B, MORE_DATA_PENDING) and `from=2026-09-17T16:00` (424 B, NO_MORE_DATA). That range begins at exactly the address the loop had been discarding on 22 consecutive pulls, and the deployed `/opt/tepna/capture-host/cpap_spool.py` carries the #2647 fix (`next_from != cursor`, mtime 2026-09-18 21:49, i.e. BEFORE the advance) — verified by reading the deployed file, not inferred from the behaviour. **So ~17 days of Summary-spool data now exists on the box.** The blocker below is UNCHANGED in kind — no fragment log is committed to the repo and the pull was not attended — but item 2 ('the assembler fed those real fragments') now has **real fragments to be fed**, which it did not on 2026-09-02. Everything else needs one attended box session, in order: a witnessed Summary-spool pull with the fragment log committed, then the assembler fed those real fragments, then §11's three-way convergence on ≥1 night. Bands pre-stated 2026-08-25, half discharged. No follow-up brief is owed until the pull happens. **Owner:** owner (box, attended; remote at present) · **Next step:** the witnessed pull — items 2 and 4 are downstream of it) · **Created:** 2026-08-25
