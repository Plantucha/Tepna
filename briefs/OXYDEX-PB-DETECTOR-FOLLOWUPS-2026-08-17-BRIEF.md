...
**Status:** PROPOSED (deferred 2026-09-05 — §1·§2·§3·§5·§6 CLOSED; §4 alone is open and it is DATA-blocked on the DEVICE-POSITIVE BASE RATE, which §4b measured: adding 26 CPAP nights added 12 paired nights and **essentially no device-positives**, because the CPAP scores PB on ~5 % of nights (4/56, 1/17), so widening either corpus does not fix it. ⚠️ §4's own Done-when — "extending the O2Ring side widens the intersection without any new code" — is therefore a NULL THAT HAS ALREADY BEEN RUN; §4b was written after §4 and refutes it, so do not schedule it. **Owner:** whoever brings a subject-period with a higher device-scored PB rate, or a PSG-scored reference. **Next step:** none schedulable. Do not re-report κ = +0.149 as validation. This stamp makes NO claim about how many subjects the 61 pairing nights cover — an earlier draft of it (mine) reasoned from "extending is available" and a peer's from "one subject, so more nights are not independent"; both were unmeasured and neither is needed, the base rate is sufficient and measured. §3b's fired trigger is ENGINEERING, tracked as residue `2026-09-05-timingsource-one-name-two-meanings`, not a reason to defer) · **Prior stamp:** IN-PROGRESS — 2026-08-17 (**§2 and §6 CLOSED by measurement the same day**: ρ measured at median 0.9804 with the CV threshold re-validated across the corpus's real redness including the reddest night, and the gap-spanning hazard shown to be fully caught by the existing guard. §1 CLOSED (mitigated at the declaration; NOT on a reclassification — see the note there). §3 CLOSED 2026-08-17 — the third observer is MEASURED (17/18 nights) and does not raise corroboration; the blocker was a wrong belief about the corpus, and vigil holds a CPAP tree running to 2026-08-16. §4b ADDED — extending the corpus did not help κ, which identifies the real constraint. §3b ADDED — OxyDex publishing axis provenance would silently upgrade an Integrator guard. §5 TARGET CORRECTED — it named the demo generator, not the one on the compute path. **§5 CLOSED 2026-08-18** — a committed 110 s input now reaches the upper band through the real parser, wired into BOTH runners; only §4 remains open, and it is blocked on data rather than effort. **§4's blocker RE-VERIFIED 2026-09-02 and it HOLDS, exactly**: the brief claims 61 pairing O2Ring nights; measured on the merged corpus root, 106 O2Ring nights >50 kB × 183 CPAP nights → intersection **61**, its own number reproduced. κ stays fragile; do not re-report it as validation) · **Created:** 2026-08-17 · **Follows:** `OXYDEX-PB-DETECTOR-2026-08-09-BRIEF.md` (DONE — 2026-08-17) · **Affects:** `oxydex-dsp.js computePatternScores`, `briefs/SYNTHETIC-CORPUS-BRIEF.md`, the CPAP/ECGDex corpora · 🔴 **§3b's TRIGGER HAS FIRED — RE-MEASURED 2026-09-05 (Brief runner), residue `2026-09-05-timingsource-one-name-two-meanings`:** §3b.1 concluded the self-upgrading condition was UNREACHABLE from the CSV path. That is no longer the state. **#1643 (2026-08-23)** made OxyDex publish `recording.timingSource = 'device+host-verified'` and, in the same commit, added `json.recording.timingSource` to the Integrator TCH guard's resolution chain — so the field now flows OxyDex → guard on the axis §3b.1 measured as DRAWN. §3b's obligation (*"re-run the Integrator's TCH path"*) was therefore triggered by that commit. **The verdict is still correct — `pseudo` stays true — but only by STRING INEQUALITY** at `integrator-dsp.js:2916`, and the two fields ask different questions: OxyDex's is an RTC-ANCHOR verdict, the guard's is CLAUDE.md §7 per-sample provenance (`device+host`·`host`·`none`, computed from delta concentration), a vocabulary `'device+host-verified'` is not in. Normalising the string or relaxing the check to a prefix silently spends a drawn axis as a timed corner. **Pinned, not fixed** (guard change deliberately out of unit): `a VERIFIED ANCHOR ('device+host-verified') does NOT make the corner timed` in the `Integrator HRV consensus — TCH wiring (§3)` group. §3b.1's *"the CSV path cannot declare it"* stands; what changed is that OxyDex declares something ELSE into the same field.
