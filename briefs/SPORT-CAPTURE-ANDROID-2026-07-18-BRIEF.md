...
**Status:** PROPOSED — 2026-09-20 (LANE OPENED by owner ruling 2026-09-20: "yes — scope it first". §14 greenlight given; the next unit is a SCOPING brief — what it captures, how it reaches the host, what it adds to the privacy statement — before any code. No session assigned yet) · **Created:** 2026-07-18 · **Extends:** `POLAR-SDK-CAPTURE-2026-07-07-BRIEF.md` (Track B1) · **Related:** `CAPTURE-HOST-2026-06-29-BRIEF.md` (the bedside sibling — owns the *how*), `MOTIONDEX-BUILD-2026-07-17-BRIEF.md` (a downstream consumer), `AMBULATORY-MODE-BRIEF.md` (why "a walk isn't a sleep") · **TRIAGED 2026-09-25 (Magpie):** RATIFIED as a programme (`OWNER-DECISION-QUEUE` D5, ✅ 2026-09-15) and the lane was opened 2026-09-20 — but **no code exists**: the only `sport` paths in the tree are this brief and a mention in `BLE-TRANSPORT-REDESIGN`. Genuinely PROPOSED. **Stop, stated by D5 itself: "no fleet session owns an Android toolchain"** — an acquisition question, not an engineering one. The reference implementation is the vendor SDK's own Android sensor-data-collector example (top doc-search hit, above this brief).
