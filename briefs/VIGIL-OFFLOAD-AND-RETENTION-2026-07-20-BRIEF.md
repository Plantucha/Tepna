...
-->
**Status:** PROPOSED (core BUILT, remainder owner-gated — **TRIAGED 2026-09-25 (Wren, box + tree; rule 0: "vigil offload retention archive NAS mirror keep nights")**. Done-when, item by item: **(2) MET.** A nightly mirror runs: the #2720 fix (20bf94e0, a config-authored rsync target never armed the poller) is on main and in the RUNNING daemon (/api/version 2cd12712, 2026-09-24); the journal shows `archive: ARMED — rsync://…vigil-archive/captures` after each restart and 28 archive lines on 2026-09-24; 61 of 62 night dirs carry `.archived`; parity by this brief's own instrument (`find -type f` + `du -sb`), box vs NAS: 2026-09-22 975,757,885 B both sides, 2026-09-23 794,587,485 B both sides (the NAS holds +1 file, the marker; plain `du` shows 199 MB on the NAS because of ZFS compression, not truncation). **(1) and (4) OPEN, owner:** `storage.keep_nights: 0`, so there is no local prune and no full round-trip box → target → prune; the 2026-09-07 ruling below stands and is not re-raised. **(3) OPEN:** the disk is 143 GB free of 233 GB (2026-09-24) and cannot trend flat while nothing is pruned (~0.9 GB/night). ⚠️ A night now exists in THREE places (box, NAS, rig pulls), so parity claims must name which pair. · EARLIER: IN-PROGRESS — 2026-09-20 (MOVED TODAY: TrueNAS NFS enabled and mounted on rig via fstab (3 exports, 7.5 TB), the 162 GB canonical corpus migrated to it, and the archive push configured on vigil. Software half was already built (`nightarchive.py` + `archive_poller`, FINISHED-WORK-IMPROVEMENTS B2). REMAINING, and none of it is a decision: #2720 (the poller never armed off a config-authored target) must deploy; then one real night must round-trip box → NAS; then `storage.keep_nights` moves off 0 — that last one IS an owner call and is deliberately left at 0 until the round-trip is proven)) · **Created:** 2026-07-20 · **Owner ruling 2026-09-07:** **NOT approved** — brief unchanged. Recorded so the decision is not re-raised as though it were still open. ⚠️ **2026-09-20 (Heron) — THE RULING STANDS AND IS NOT RE-RAISED HERE. What changed is beneath it: the brief's own next step would have been DESTRUCTIVE until 2026-09-20.** This plan ends in "Heron (config + first pruned round-trip)" over a TRANSFER target. Verified at the source across #2702: `storage_poller` computed `archive_enabled = bool(acfg.get("enabled")) and bool(acfg.get("dest"))`, and a transfer (rsync-over-SSH) config is `target`-only with **no `dest`** — so the gate was False, `plan_prune` (age-based by design) ran with **nothing protecting it**, and nights flattened into one directory. Both fixed in #2702, which also records why an `.archived` marker alone would not have sufficed: the marker says a copy was once MADE, not that it still EXISTS, and on this box **6 of 10 nights carried it while the backup volume was absent**. ⚠️ **Read this as a precondition repaired, NOT as a reason to re-open the decision.** It changes the risk of the step the brief already describes; it does not bear on whether the owner wants the offload. If the ruling is ever revisited, `keep_nights` off 0 over a transfer target is now gated — and it was not when the ruling was made.
