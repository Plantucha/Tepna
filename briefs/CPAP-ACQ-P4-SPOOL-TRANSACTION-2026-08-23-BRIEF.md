...
-->
**Status:** DONE — 2026-09-21 — the fifth and last done-when box closed by production evidence, §6 prediction confirmed (Heron). Formerly PROPOSED (**core BUILT and SHIPPED, one analysis remainder — verified 2026-09-11 (Osprey): 4 of 5 Done-when boxes ticked.** Verified in the TREE: `capture-host/cpap_spool.py` exists and drives `as11_pull.pull_spool_round`, exactly as box 1 claims; the C1–C5 planted controls shipped in #1711 at 100 % stmt+branch, and `check.sh` has been green on every capture-host PR since. The single open item is §6's between-rounds capture — a PREDICTION to confirm or a localized guard to add, i.e. one measurement, not the transaction machinery this brief is about.) · (parked 2026-09-02 — four of five done-when boxes are closed (#1711 module + C1–C5 planted controls, #1838 daemon wiring). The one open item is §6's between-rounds capture, and the distinction that matters is that the SEAM is built while the EVIDENCE is not: the injectable `revalidate=` hook exists (`cpap_spool.py:216`, exercised by `test_revalidate_hook_sees_every_cursor_the_loop_trusts`), but no real AirSense-11 drop has been captured between a round-1 cursor commit and a round-2 StartSpool, so the predicted CLEAN cursor re-serve is a prediction and nothing more. **Owner:** owner (attended box run) · **Next step:** one deliberate mid-round drop with the journal watched — confirm the prediction or add the localized guard) · **Created:** 2026-08-23
...
- [x] The between-rounds capture (§6) resolved — **PREDICTION CONFIRMED 2026-09-21 (Heron), from the production ledger, read-only; no guard needed and the `revalidate` seam stays absent.** `cpap_spool_ledger.jsonl` on vigil: row 4 committed cursor `2026-09-17T16:00:00.000Z` at mono 134807 (09-19 10:00, PID 302045, `NO_MORE_DATA`); row 5 resumed from that exact cursor at mono 221198 — **86,391 s = 24.0 h later, in a different process** (09-20 10:00, PID 556837; the daemon had been cleanly restarted in between, so the link was necessarily released) — and the device served a clean round from it (637 bytes vs 424 the day before, new sha, `NO_MORE_DATA`). Four further connections in between (09-19 10:06 and 11:31, 09-20 10:06 and 11:02) resumed from the same cursor and stopped honestly `no-new-data`, 0 rounds each. A 24 h idle across a process exit is the HARDER form of §6's mid-pass drop — any idle-timeout on the device's `nextSpoolAddress` would have fired — so the committed cursor is honoured on a fresh connection and §3's cursor-commit rule needs no re-validation step.
