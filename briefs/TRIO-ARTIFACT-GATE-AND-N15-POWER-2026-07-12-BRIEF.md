...
**Status:** DONE — 2026-08-14 (**all five Done-when boxes are closed**: two DONE/REFUTED on measurement, two RETIRED-in-place as *do not build* with their substance re-homed, one MEASURED — and the `ms;hr;c` ownership that was the last reason to keep it open was discharged 2026-08-09, see the ⊕ note below. **DONE here means every box is resolved — it does NOT mean superseded, dead, or safe to ignore**; that distinction is the whole point of the ⚠️ note below, and §1/§2's findings and the N=15 power tables remain live reading that `SENSOR-TRIO-NIGHTS-PAPER` still depends on. **What is NOT closed and is NOT owed by anyone:** §6.2/§6.3's corner attribution and correlated-error reading stay **open questions parked on corpus size** — §6.4 puts the bar at ~125 trio nights against the 40 available — and they are deliberately *parked, not delegated*, because `TCH-FUSED-ROBUST-HAT` is explicitly the path that does not require answering them. Recording that here so nobody later reads a routed item into a brief that never accepted one. — *previously:* PROPOSED (still LIVE — **do NOT retire this as superseded**; re-checked 2026-07-19; **Done-when reconciled with the ⚠️ DISPROVEN-§3 banner + degeneracy measured 2026-08-01, see §6**. Two Done-when boxes asked for exactly the cross-corner consensus gate the banner above declares dead — the banner landed 2026-07-18 and the list was never reconciled with it, so a reader working the list top-to-bottom would have built the forbidden thing; both are now RETIRED in place with the substance re-homed, not dropped. The residual-degeneracy box is ANSWERED: **8 of 39 nights (21 %)**, not "several" — but its two attributions do **not** survive measurement (the boundary corner is not attributable at n=8, p=0.088; and degenerate nights carry *less* co-motion ρ, 0.26 vs 0.41, which inverts the correlated-error reading). §1/§2's findings and the N=15 power material are untouched)) · **Created:** 2026-07-12 · **⚠️ NOT superseded by [`TCH-FUSED-ROBUST-HAT-2026-07-14-BRIEF.md`](TCH-FUSED-ROBUST-HAT-2026-07-14-BRIEF.md), despite its header saying it "executes its intent with a better estimator"** — that line describes the ESTIMATOR only (§3's cross-corner consensus gate is indeed replaced by the fused weighted-variance hat), and reading it as a full supersession is a trap this note exists to close: an open-brief review on 2026-07-19 did exactly that and nearly retired a brief that is on the critical path. ~~TCH-FUSED's own "Done when" **routes work BACK here** — its second box is still open on the power tool's REAL overlay, which needs a **confidence-carrying (`ms;hr;c`) corpus re-derivation** that THIS brief owns, and the same re-derivation is what [`TRIO-POWER-N15-FINDINGS-2026-07-12-BRIEF.md`](TRIO-POWER-N15-FINDINGS-2026-07-12-BRIEF.md) is blocked on. Retiring this brief would orphan that leg and leave both the paper chain and the fused hat stuck with no owner.~~ (superseded 2026-08-09 — see the later clause) **⊕ 2026-08-09 — that leg is now BUILT and this brief's ownership of it is discharged:** the `ms;hr;c` contract shipped in #1014 (`OxyDex timeseries.hr` 1 Hz + `ECGDex rr.conf` + `PpgDex ppi.conf`), and the re-fit has been RUN at N = 17 (`tools/tch-fused-corpus.mjs`). ⚠️ The header's framing was also too generous to itself: the leg was not merely un-owned, it was **structurally impossible** — 0 of 40 committed OxyDex exports carried any HR timeseries, so the O2Ring corner was absent from the file and the per-second hat could not run at ANY N. What remains is `TCH-FUSED-ROBUST-HAT`'s `loadReal` box (small, and deliberately not built until an input exists that exercises it) and a window-length decision — **not** a corpus derivation, and no longer a reason to keep this brief open on its own. · **Follows:** `PPGDEX-OPTICAL-DETECTOR-AND-SIGMA-REDERIVE-2026-07-11-BRIEF.md` §2 · **Feeds:** `SENSOR-TRIO-NIGHTS-PAPER-BRIEF.md` · `SIGMA-PAPER-REWRITE-2026-07-06-BRIEF.md` · `INTEGRATOR-THREE-CORNERED-HAT-FOLLOWUPS-III-2026-07-06-BRIEF.md`
