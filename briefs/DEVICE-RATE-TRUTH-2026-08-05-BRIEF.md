...
**Status:** DONE — 2026-09-21 (Osprey: the last box — the `RtPpg` battery night — measured on the box 2026-09-21, cost NOT DETECTABLE at ≤ 0.3 %/h against a +0.9 %/h unrelated August drift; every other box was ticked 2026-08-06. Earlier header text follows. **§6.3 DONE** — `effFs` measures the device clock, the k/(k−1) bias that made 130 Hz read 146.6 is gone, and an unmeasurable rate is `None` not `0.0`; known-answer test written first and observed failing. **§6.2 AUDITED** — four of five were already landed unstamped, and the fifth **cannot be done as written**: PPI back-timing contradicts a test-backed PulseDex contract, and PPI has no device-clock column to fix instead (§9.2). **§6.4's clobber bug** was already fixed. **§3's corrections landed in the source.** Still open: §5's configuration and the `RtPpg` night are **FIELD** actions; §6.5 Phase 4 is unblocked but wants a quiet window — see §9.4.) · **Created:** 2026-08-05 · **Extends:** `H10-ECG-RATE-CORPUS-CHECK-2026-08-04-BRIEF.md` · **DRAIN 2026-09-02 (Osprey) — re-verified; the remainder is FIELD work on the box, not code.** Section 5's configuration and the `RtPpg` night are physical capture actions, and section 6.5 Phase 4 is unblocked but wants a quiet recording window. None of it is actionable from a dev checkout. **Owner: Heron** (capture-host + box ops lane; deploys to `vigil` remain owner-authorized). **Next step:** Heron schedules the config change and the RtPpg night in the next quiet window; this brief should not be picked up by an analysis session. · **DRAIN 2026-09-06 (Magpie) — re-verified; status UNCHANGED at IN-PROGRESS.** Boxes on today's tree: **6 `[x]` · 1 `[~]` · 1 `[ ]`**. §6.3's artifacts confirmed in source: `effFs` is live (`capture-host/capture_status.py` gates STREAMING on *a real* `effFs`), and the k/(k−1) frame-count bias that made 130 Hz read 146.6 is recorded as corrected in `capture-host/telemetry.py` (search `146.6`). The remainder is unchanged FIELD work in Heron's lane, not code — PARKED on a physical blocker, so DONE and PROPOSED would both be false. · **Owner ruling 2026-09-07:** §5's RtPpg night is **SCHEDULED** as box day 2. Order: attended ring session (2026-09-08) → **RtPpg** → perturbed-clock (`KNOWN-CLOCK-ADVERSARIAL-CAPTURE`) → Polar Logger (`CAPTURE-HOST-FOLLOWUPS-II` V1/V2), one per night, each on a per-night go. No date beyond 09-08 is set. **DRAIN 2026-09-20 (Wren, measured on the box against §5's table — the "field" remainder is mostly DONE, and two rows were decided the other way):** the `RtPpg` night happened on the brief's own day and every night since — `ppg2w` (cmd 0x05) files exist for 2026-08-05 → 09-19 (530 files); last night's ran **194.96 Hz over 7.20 h** on host stamps (5,051,018 samples; its `sensor timestamp` column is EMPTY, so no device-clock `effFs` exists for that stream — host-stamped only). Verity MAG + GYRO are OFF (last MAG/GYRO files 2026-09-03; `streams: [acc, ppg]`) ✓. Verity ACC 52, `sdk_mode: false` ✓. H10 ECG 130 ✓. **Two rows the box does NOT follow, by the owner's later config, and the table should not be read as the current decision:** Verity PPG runs **55 Hz** (`rates.ppg: 55`, measured 55.14 Hz over 7.21 h), not the "176 — keep" row; H10 ACC runs **200 Hz** (`rates.acc: 200`, measured 200.5 Hz host / 203.2 Hz device-ns over the first 98 s), not the "25 → 50" row. ⚠️ The H10 recorded **67 min** last night (19:20:57 → 20:28:13) and then died at 10 % battery — a night that needs the H10 needs it charged, which is a field precondition no row here names. §6.5 Phase 4 still wants its quiet window; nothing else in the remainder is field work any more.
...
- [x] O2Ring `RtPpg` enabled behind a flag; first night's battery cost measured and recorded here.
      **DONE 2026-09-21 (Osprey) — measured on the box, read-only, from `Tepna_*_LINK.csv battery_pct` as
      §5 prescribed, over every night in `/srv/tepna/captures`.** The stream has been live behind
      `streams: ["ppg2w"]` since 2026-09-07 (89 `PPG2WRUNS` sidecars; on 2026-09-20 the `PPG2W.txt` spans
      22:39→04:51, 4.40 M rows ≈ 200 Hz, so it ran through the whole measured drain window). Per night: the
      longest connected, non-increasing `battery_pct` segment ≥ 3 h; docked-at-100 % sessions excluded.

      | window | n | median drain | IQR |
      |---|---|---|---|
      | Jul 26 – Aug 22, no ppg2w | 24 | 3.33 %/h | 3.20–3.52 |
      | Aug 25 – Sep 06, no ppg2w | 12 | **4.25 %/h** | 3.89–4.34 |
      | **Sep 07 – Sep 20, WITH ppg2w** | 8 | **4.05 %/h** | 3.88–4.18 |
      | Sep 14 / 21, no ppg2w | 2 | 3.90 %/h | — |

      **The `RtPpg` cost is NOT DETECTABLE: −0.19 %/h against the preceding fortnight, inside the
      night-to-night IQR; bounded above at ≈ +0.3 %/h, i.e. ≤ 2.5 % of battery per 8-h night.** What IS
      in the data is a **+0.9 %/h drift across August with no configuration change** (3.33 → 4.25), which
      dwarfs any stream term and is unattributed here (battery ageing, wear, or the 08-2x capture changes —
      not measured, not claimed). The 1 % counter over a 4–8 h segment quantises each night at ±0.15 %/h.
      §5's "true saving above 30 %" estimate for the trimmed configuration was never separable from that
      drift either, and is left as the estimate it was.
...
- **Whether the ring exposes an accelerometer.** `AUTO_RT_ACC` (`cmd 0x14`) is decoded by the vendor SDK
  ...
  → **It was wrong, and the tree already says so:** the box writes `*_ACCRAW.txt` + `*_ACCRAWRUNS.txt`
  per O2Ring session (9.2 MB on 2026-09-20). Noted 2026-09-21 in passing; the raw-ACC briefs own it.
