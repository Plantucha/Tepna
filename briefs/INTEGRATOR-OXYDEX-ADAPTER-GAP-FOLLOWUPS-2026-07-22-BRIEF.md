...
**Status:** DONE — 2026-09-05 (drain stamp, Magpie: the 2026-09-01 triage below re-verified on `origin/main` fc0cf634 — `provenance/Integrator.json` now carries THREE code-gated fixtures (`integrator_tch_golden`, `integrator_apnea_null_twins`, `integrator_respiration_fusion_twins`, all `verifiedUnder a302e25f8199`) and the two `integrator_fusion_2026-06-1{1,3}.json` are still `historical: true` with no `inputHashes`; the code-gated three exercise the TCH / apnea-null / respiration fusers, so `runFusion` (`integrator-dsp.js:6528`) and `buildFusionExport` (`:6954`) still have no known-answer leg over a committed multi-node night. That is §5's residue, and it is a corpus/privacy decision, not an edit — ledgered as the residue key below rather than left in prose. Residue 1 (a gitignored working file) is not a defect and gets no row. Nothing else is open: every section is closed and verified, so the brief is DONE) · **Created:** 2026-07-22 · Supersedes: none · Follows: INTEGRATOR-OXYDEX-ADAPTER-GAP-2026-07-21-BRIEF.md · **§1 CLOSED + §2 ANSWERED 2026-08-01** · **TRIAGED 2026-09-01 — every section is closed; the two residues are NOT code** · **Residue:** 2026-09-05-integrator-fusion-no-code-gated-fixture
