...
**Phase 4 — CVHR. ✅ EXECUTED 2026-07-20** (on the recorded §3.1 (b) basis). PpgDex computes a
whole-record finger-PPI **CVHR** via `cvhrFromNN` — a faithful port of the audited ECGDex `detectCVHR`
(shared apnea-band method so the two are comparable) — and exports `apnea.cvhrIndex` (rich-only; the light
export + committed golden stay byte-identical). The Integrator `fuseCvhrCorroboration` publishes that
finger CVHR **named**, tier `emerging`, when a finger PpgDex + an OxyDex (the O2Ring night) are present,
and **corroborates** it against ECGDex cardiac CVHR (per-hour gap, agree/diverge, never averaged). Per
§3.1 (b) it emits **no AHI** (`ahiPublished:false`) — OxyDex `ahiEst` remains the ONLY published AHI.
`cvhrIndex` 0 (none detected) is a real reading, accepted. Read-only; attach-only-when-present keeps
Integrator fixtures byte-identical, and the PB-consensus `_pbObserver` is untouched (distinct
`cvhrIndexWave` field). Tier `emerging` — `validated` owed a real-corpus PSG comparison (follow-up).
