...
-->
**Status:** PROPOSED (parked 2026-09-02 — drain triage, Kestrel: the whole build is gated on §6 Q1 — does an idle H10 accept a recording-start over the same link the box holds — which is the experiment `POLAR-ONBOARD-BACKUP-FOLLOWUPS` §4 now has an endpoint for (#2042). ⚠️ **The endpoint is `POST /api/polar/recording` — `webmon.py` `polar_recording`, over `polar_psftp.recording_control` — re-verified present 2026-09-19 (Heron).** Naming it because I searched for it as `rr_accept`/`rr-accept` first, got zero hits, and nearly recorded it as GONE: the identifiers never contained 'rr_accept'. A guessed token returning 0 is not an absence. No code is owed until Q1 answers; a NO closes this brief as infeasible, a YES makes it one work-unit. Owner: Heron for the probe, Kestrel for the build if YES) · **Created:** 2026-08-01 · **Related:** `POLAR-OFFLINE-DOWNLOAD-2026-07-17-BRIEF.md` (the pull path this builds on — and one finding it corrects), `VIGIL-O2RING-AUTOPULL-2026-07-21-BRIEF.md` (the O2Ring precedent), `CAPTURE-HOST-2026-06-29-BRIEF.md` (the daemon), `POLAR-SDK-CAPTURE-2026-07-07-BRIEF.md` (SDK capability survey) · **TRIAGED 2026-09-25 (Magpie):** onboard-backup surfaces exist in `capture-host/capture.py`, `config.example.yaml` and `webmon.py`. Gated as its header says. **Stop: `OWNER-DECISION-QUEUE` D1, the same strapped-idle-H10 window as `POLAR-OFFLINE-DOWNLOAD`** — not merged with it, because the two carry different remaining work behind one shared stop.
