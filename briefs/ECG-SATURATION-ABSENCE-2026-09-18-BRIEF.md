...
- [ ] `GAP_S`: a threshold decision across TWO nodes — **OWNER RULED 2026-09-21: MEASURE BEFORE RULING.** A session sweeps `GAP_S` over the corpus and reports where the CVHR index moves on both nodes (ECGDex `GAP_S`, PpgDex `PPG_CVHR_GAP_S`, coupled by design); the owner rules on the numbers. Not a decision yet — a measurement unit, unassigned, JS lane
