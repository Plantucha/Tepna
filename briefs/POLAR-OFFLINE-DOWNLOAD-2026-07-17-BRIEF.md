...
-->
**Status:** IN-PROGRESS (parked 2026-09-02 — drain triage, Kestrel: both open done-when items are HARDWARE-gated, not code-gated — the `.BPB` decoder has no input until an onboard exercise session is pulled off a real H10, and the on-box web-pull demonstration needs an idle strapped H10 on vigil, which is the same experiment as `POLAR-ONBOARD-BACKUP-FOLLOWUPS` §4 / `POLAR-ONBOARD-BACKUP` §6 Q1 and now runs through the #2042 probe endpoint. Owner: Heron, in the owner-scheduled Phase B window; next step: one recorded pull over the web path, then the `.BPB` bytes exist to decode. Previously: pull core + charger/doff triggers SHIPPED and gate-tested; verified 2026-09-01 against the done-when: the `.BPB` decoder is still UNLANDED — the only `BPB` mentions in the tree are `polar_mirror.py`'s PII/bonding-table notes, no decoder module — and the on-box web-pull demonstration is still unrecorded, so this stays open on both counts. Related but not closing it: the H10 RR-acceptance probe endpoint (`POLAR-ONBOARD-BACKUP-FOLLOWUPS` §4) is in flight 2026-09-01) · **Created:** 2026-07-17 · **DRAIN 2026-09-11 (Brief runner) — RE-VERIFIED, UNCHANGED.** No commit since the 2026-09-02 stamp. Both open done-when items remain HARDWARE-gated: the `.BPB` decoder has no input until an onboard exercise session is pulled off a real H10. Not schedulable from a dev checkout. Status stays IN-PROGRESS. · ⚠ **RE-VERIFIED 2026-09-21 (Wren, ON the box, read-only) — still hardware-gated, and three box facts the header lacked:** (1) the pull path HAS written on this box — `captures/stored/Polar_Offline_0C301E3F_20260802224700/` holds `ACC.REC` (1763 B, session `/U/0/20260802/R/224700/`, `recording.meta.json`), a Verity offline session pulled 2026-08-02 21:12; whether the CLI or the web endpoint triggered it is UNRECOVERABLE — the box journal now begins 2026-09-05, so no `/api/polar/pull` line survives and the web-triggered box stays open on evidence, not on doubt. (2) The only `.BPB` files on the box are the system set (`DEVICE`/`SYSLOG`/`ERRORLOG`/`SYNCINFO`/`U/UDB.BPB`) under `captures/device-mirror*/` from `polar_mirror.py`; no exercise session (`/U/0/<date>/E/…`) exists anywhere, so the decoder has no input — unchanged. (3) The daemon walks PS-FTP on the H10 routinely (`PS-FTP 24:AC:AC:02:84:96: walk complete — 1 entries`, several per night, 2026-09-21 04:17–04:31), so the link and the listing work in production; what is missing is one commanded pull on an idle strapped H10 outside a capture window. That is a live BLE operation on the production box → owner-authorized, run from Wren's session when the owner says so; a pull mid-night contends with the capture link (§Known caveat) and is refused by this stamp. · **TRIAGED 2026-09-25 (Magpie):** code side present and unchanged — `capture-host/polar_psftp.py`, `webmon.py`, `/api/polar/pull` served and covered by `tests/test_webmon_api.py`. Both open items remain HARDWARE-gated, not code-gated. **Stop: `OWNER-DECISION-QUEUE` D1** — the three Polar items are "one strapped-idle-H10 window", and D1 carries no verdict marker (see the OXYII-G1 note). · **Residue:** 2026-09-25-owner-queue-d1-has-no-verdict · verified 2026-09-25 (cloud): 1 parked item unblocked, see §Validation status
...
- [ ] **Web-triggered pull demonstrated green** — unblocked 2026-09-25: the BLE trusted-auto-reconnect race's untrust-after-pair mitigation landed as `830f8d15` — not executed
