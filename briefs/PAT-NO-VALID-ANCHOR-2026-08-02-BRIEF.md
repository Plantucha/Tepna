...
-->
**Status:** PROPOSED (parked 2026-09-06 — the remaining item's BLOCKER IS MISSTATED: a beat-free anchor exists and is proven; what is missing is a verdict computed against it. See §9a) · 🟢 **§9a's UNIT (1) EXECUTED 2026-09-14 (Kestrel) — and it does not yield a verdict, for a reason worth more than a pass.** Ran `tools/pat-buzz-stability.mjs` on the real capture. Three corrections to §9a first, all from the evidence: it says *"the 5 pairwise NIGHTS"* and *"Five such nights exist now"* — there is **ONE night** (2026-08-19); "5/5" in the source is five of five BUZZ EVENTS, not nights. The night carries **THREE** runs of five commands (15 total, recovered from the journal to the ms), not the two the evidence memory records: 22:35:21 (ring→H10 pod, gaps 4·2·6·3), 22:39:04 (ring→Verity pod, same pattern) and **22:42:28 (gaps 3·5·2·7) which no brief or memory mentions**. Measured, per run, against the contacted device: H10 offsets 85/97/82/83/138 ms, residual SD **21.3 ms**; Verity run 2 430/1085/264/367/361, SD **358.7 ms**; Verity run 3 475/216/260/353/164, SD **107.2 ms**. ⚠️ All three read SWAMPED against the 15 ms dip budget and THAT MUST NOT BE READ AS MEASURED DRIFT: von Neumann is 1.52 / 2.53 / 1.87, i.e. ≈2 = white noise with no trend, so the slope is UNRESOLVED and the tool charges it at |slope|+2SE. The cause is capture geometry — every run packed its five fires into ~15 s (0.3 min). The tool computes the fix per device: spread the fires over **~4.4 min (H10)**, ~22 min (Verity at run-3 noise), ~74 min (at run-2 noise). **So unit (1) is now CLOSED as executed-and-negative, and unit (2) gains a specification it did not have: the buzz sequence must span MINUTES, not seconds, and the H10 leg needs 17× less spreading than the Verity leg.** ⚠️ Two traps for the next runner, both hit here: the pre-22:30 Verity ACC files that night are 2–3 SECOND fragments from failed connections — only `..._223006_ACC.txt` (22:30:10 → 00:54:25) is the session, and picking by pattern gets a fragment that reports `noLag 5` and reads exactly like a structural negative; and the cross-device (`--a`/`--b`) leg cannot work on these runs at all, because the two contacts are SEQUENTIAL four minutes apart rather than one shared simultaneous event, so any H10↔Verity anchor from this night is TRANSITIVE through the ring and inherits the ring's own weak leg (the memory records the ring's motion channel detecting only 2/5 while pressed). · **Created:** 2026-08-02 · **Follows:** `WEARABLE-HOST-AXIS-FOLLOWUPS-2026-08-02-BRIEF.md` §F3-ter, `PAT-UNDER-PERBLOCK-ALIGNMENT-2026-08-02-BRIEF.md` §5 · **Affects:** no code yet — a capture decision and one measurement · **DRAIN 2026-09-02 (Osprey):** verified 2 open Done-when boxes across 11 sections. **Owner: Osprey. Next step:** confirm whether the two remainders survived the host-axis work (#2044/#2052/#2082 changed the anchor and refusal paths under this brief) before executing — they may already be answered. · **TRIAGED 2026-09-03 (Osprey): 5 of 6 Done-when items verify; the sixth is ticked as NOT ACHIEVED in the brief's own text and that is the honest state.** Route 2 executed (Δ_H10 −865 ms, Δ_Verity −4363 ms, both stable to 0 ms across 7 h; ACC anchor corrected to −199 ms), and §10's corpus-wide run CORRECTS §7 rather than extending it — a brief that publishes its own retraction. What remains is a PAT verdict surviving an anchor NOT derived from beats, which §5 shows the current corpus cannot supply: the two requirements are mutually exclusive on the nights we hold. ~~**So this is corpus-bound, not effort-bound** — distinct from its siblings, which need a run rather than a recording. Owner: Osprey. **Next step:** none executable here; it unblocks only on a capture that satisfies both requirements at once.~~ (superseded 2026-09-06 — see the later clause) · **TRIAGED 2026-09-25 (Osprey): ONE genuinely open item, verified by reading the boxes — 9 ticked, 2 unticked of which one is STRUCK THROUGH.** The live item is §10's *"a PAT verdict that survives an anchor NOT derived from beats"*; the per-fragment-Δ item at line 424 is struck and carries its original text for provenance only, so it is a recorded answer and not work. Park stands and the blocker is unchanged from the 2026-09-06 restatement (a beat-free anchor EXISTS and is proven; what is missing is a verdict computed against it, and §9a's unit (1) executed 2026-09-14 without yielding one). No stamp change is the correct outcome.
