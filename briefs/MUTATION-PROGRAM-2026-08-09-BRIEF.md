...
**Status:** IN-PROGRESS · **Created:** 2026-08-09 · **Folds:** `MUTATION-EQUIVALENCE-2026-08-04-BRIEF.md`, `CAPTURE-HOST-MUTATION-FLEET-2026-08-04-BRIEF.md`, `JS-DSP-MUTATION-FLEET-2026-08-08-BRIEF.md`, `PPGDEX-TESTABLE-SURFACE-2026-08-08-BRIEF.md` · **Affects:** `tools/mutate.mjs`, `tools/mutate-equivalence.json`, `capture-host/tools/mutate_diff.py`, `tests/dex-tests.js` · **DRAIN 2026-09-02 (Osprey):** charter head — folds four briefs, 5 of 12 Done-when boxes ticked. Kept as the family's index; it closes when its children do, not on its own. **Owner: Osprey. Next step:** none directly — drive the children (COVERAGE-SELECTION is nearest at 9/10). · **RE-TRIAGED 2026-09-13 (Osprey): 7 of 12 boxes still open; ONE advanced by a landing, and §7.4 explicitly NOT.** §7.1's payload advanced by #2323: `tools/mutate-equivalence.json` now carries **422 entries across 4 classes** (`no-distinguishing-input` 414 · `equivalent` 4 · `real-gap` 3 · `untestable-by-design` 1), and `equivalent` is documented in the ledger's own `_README` vocabulary AND excuses in `EXCUSING` — before #2323 the STRONGEST class was the one that did not leave the denominator, and such an entry was reported as a real gap. ⚠️ **§7.4 (the 13 loop-condition timeouts) is NOT closed by #2403**, and I checked rather than assumed: #2403 fixed `tools/mutate_diff.py`, the DIFF-SCOPED gate, which silently counted a timeout as killed. The full sweep (`tools/mutate.py`) does not classify at all — it stores `mutmut results` raw and prints it for a human (`:462-473`, `:530`), so the 13 were read off a listing where timeouts were already visible. #7.4 still needs the judgement it asks for. The other five open boxes are untouched by anything that landed. · **DRAIN 2026-09-20 (Osprey): 5 ticked / 7 open — count UNCHANGED and the header is accurate. Used the brief's own `Affects:` field. ⚠️ Its four paths report **50+ landings since 2026-09-13, which is noise**: `tests/dex-tests.js` is in the list and nearly every PR touches it. **Narrowed to the three mutation-specific paths (`tools/mutate.mjs`, `tools/mutate-equivalence.json`, `capture-host/tools/mutate_diff.py`): 8 landings, 6 of them mine (#2628 · #2632 · #2635 · #2639 · #2644 · #2673) — and NONE closes a box.** They changed how the gate REPORTS, not what this brief asks to be recorded. ⚠️ **One datum for §7.4 that did not exist at the last triage:** four UNDECIDED refusals measured 2026-09-19 were **100 % `not checked`, 0 % `timeout`**, so the 13 loop-condition timeouts are a HISTORICAL population — re-deriving them needs a re-run, not a re-read of the current gate's output.** · **TRIAGED 2026-09-25 (Osprey): park stands, but its VERIFICATION is stale against a moved surface — re-read before sizing.** This brief is execution-bound, not decision-bound, so landings on its own surface are decision-relevant, and the surface HAS moved since the last drain (measured 2026-09-25 against `origin/main`: 1 commits on `tools/mutate.mjs` since 2026-09-19 alone). Verified this pass: step-1 stale query non-zero from a 925-commit aged base with the positive control firing (RESIDUE 390 / capture.py 105 / CLAUDE.md 12); **0 of 56 pending changesets name it** while 11 name 7 other briefs, so the zero is real; Rule 0 run 2026-09-24. ⚠️ What I did NOT do, stated so nobody reads more into this stamp than it carries: I did not re-verify each Done-when item against the moved surface — that is the next pass's work, and it is now owed rather than optional. Status unchanged.
