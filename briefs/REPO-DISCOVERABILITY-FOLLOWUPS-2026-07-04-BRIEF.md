...
- **§5.3 / §5.6 per-page meta on the non-index served pages — DONE 2026-07-14 (generated, not hand-maintained).** The 7 reference guides + 4 content pages (Science · Architecture · How to Collect Data · Why This Exists) now carry a roster-derived `<head>` block: `<meta description>`, `<link rel=canonical>` (the `%20`-encoded `tepna.net` URL — resolves the docs/-mirror duplicate-content question), `theme-color`, and OG/Twitter text tags (`og:type=article`, title/description/url + the shared `tepna-og.png`). **Single-sourced from `suite.manifest.json`** — new `content[].description` fields + the existing node `summary`/`signal`/`device` — and injected by a new **Phase 0** in `tools/build-docs.mjs` that UPSERTS a marked `<!-- meta:auto … -->` region into each page's head, in place at root, then Phase 1 syncs it to `docs/` (a `preserve`d page like Science.html is injected in both root and its docs twin, so its editorial de-link body is untouched). Idempotent + `--check`-guarded, so the guide/content meta can never drift from the roster. The front door (`index.html`) keeps its bespoke hand-authored head. Titles left as-is (already node/keyword-front-loaded). Gates green: full suite 2322✓ (cohesion-badges 60, discoverability-cohesion 6, docs-ledger 16, CSP/no-network), `build-docs.mjs --check` clean for the 22 meta pages.
  - **Adjacent finding — FIXED 2026-07-14 in a dedicated deploy-refresh PR.** The 8 served `docs/*.html` **app bundles** were STALE vs their root twins on `main` (earlier re-bundles moved the root builds without re-running `build-docs`, and `build-docs --check` is not a CI gate so nothing flagged it — the live site was serving pre-1.10.0 app builds), and `docs/llms-full.txt` still read `Suite version: 1.9.0`. A one-shot `node tools/build-docs.mjs` re-synced all 8 bundles (de-linked form == `delink(current root)`, `--check` clean) + refreshed `llms-full.txt` to 1.10.0. Kept as a separate single-concern PR from the §5.3/§5.6 SEO pass.
  - **Root cause of the `llms-full.txt` half — FOUND + FIXED 2026-07-14 (`tools/build-docs.mjs` was not idempotent in one pass).** The finding above attributed the stale `llms-full.txt` to build-docs never having been *run*. That was only half of it: **the tool could not settle the tree in a single pass.** `llms-full.txt` (Phase 2) CONCATENATES `README.md`, and `README.md` is a Phase-3 **stamp target** — so Phase 2 read the version the *previous* run had left on disk, and the artifact came out **exactly one release behind**. (That is precisely the fingerprint above: `llms-full.txt` read `Suite version: 1.9.0` while the suite was at 1.10.0.) It reproduced deterministically at v1.10.1: one `node tools/build-docs.mjs` stamped README to 1.10.1 but emitted `llms-full.txt` still reading 1.10.0, and `--check` then reported `STALE (1): llms-full.txt` — the writer's own output failing the writer's own gate. Two runs were needed to converge, which no doc ever said (release.mjs + `CONTROLLED-RELEASES` step 7 both prescribe ONE run) and which the tool's own header comment contradicted ("Phase 2 — regenerated after the sync so they see the current page set"). **Fix:** the version stamp is now a pure text transform (`applyStamp`) declared above Phase 2, and Phase 2 derives its text through it instead of re-reading the stamped file — so every artifact is a pure function of (source text, canonical version) and is independent of phase order. Phase NUMBERS are unchanged (briefs cite "build-docs Phase 3"); only the declaration moved. Verified by bumping the manifest to a dummy version: old tool → `STALE` after one pass; fixed tool → `--check` clean after one pass. **The durable fix — DONE 2026-07-14: `build-docs --check` is now a CI gate.** Added to `tests.yml`'s `static` job beside the `build.mjs` / `build-analysis.mjs` drift guards (pure Node, no `uploads/`, no network), so the served tree can no longer drift from its root twins unnoticed — the hole through which BOTH the stale app bundles and the stale `llms-full.txt` reached `main` and the live site.
    - **Why it could not be wired before — `build-docs.mjs` was NON-DETERMINISTIC (found while wiring it; fixed in the same PR).** It stamped `new Date()` into `sitemap.xml` `<lastmod>`, `feed.xml` `<updated>` and the `llms-full.txt` header, so it rewrote three committed artifacts **every time the calendar rolled over**, with no content change. A gate on that would have gone red on every PR from the day *after* it landed — a daily false alarm, which is worse than no gate (it trains everyone to ignore the one guard standing between the repo and the live site). Verified: under a simulated future clock the old code reports `STALE (3): sitemap.xml, feed.xml, llms-full.txt` on a perfectly in-sync tree. The stamp now comes from the newest `RELEASE-MANIFEST.json` record — **honest** (`<lastmod>` = when the served tree actually last shipped, not when someone happened to run the build), **stable** between releases, and byte-identical on the day of the change. Same doctrine as the Clock Contract's "never fabricate `now()`" and `tools/build.mjs`'s determinism.
