...
-->
**Status:** DONE — 2026-09-06 (the ONE open item, PPGDex two-channel ingest, is BUILT and gated — see the closing note on this line. Previously: parked 2026-09-02 — five of the six §5 follow-ups are discharged: IR-vs-RED settled, the rate measured with zero hardware (`tools/ppg2w-rate.mjs`, a8e85415), the OxyDex SpO₂ route closed as dead 2026-08-17, the upstream contribution made (`README.md:462`), and the config-opcode question answered by `ring_config.py` (0cb3ab34) rather than the hoped-for JSON route. The ONE open item is **PPGDex two-channel ingest** — `ppgdex-dsp.js:559` still branches `nCh === 1 ? [ch0] : [ch0, ch1, ch2]` with no `nCh === 2` path. It is not hardware-blocked: it is gated on a decision to open the heavy JS release gate (three build systems re-bundled, GATE A/B, `computeHash` re-verification against the real corpus). **Owner:** owner/lead (gate decision) · **Next step:** none — built 2026-09-06.) · **Created:** 2026-08-05 · **EXECUTED 2026-09-06 (Magpie).** The gate decision was taken and the ingest built. ⚠️ THREE THINGS THE BRIEF AND ITS SIZING HAD WRONG, each found by running rather than reading: (1) the refusal was NOT `:559`'s ternary — `ppgColsFromHeader` returns null for two channels DELIBERATELY, so a 2-channel file never reached it; measured, 60 of 60 real files were cleanly REFUSED with an honest error, so there was no latent wrong answer to fix. (2) The actual refusal was `minFields = 6`, the Verity layout's field count, rejecting every 5-field row. (3) 'A 2-LED file is the 3-LED path minus a channel' is false: the row carries `motion` and NO ambient, and `writers.py` says a two-wavelength row is neither layout. The data was never missing — 1075 files sit in the corpus (1066 non-empty, ONE distinct header, 5-field rows universally); an earlier report of ZERO was a case-sensitive search against UPPERCASE `_PPG2W.txt`. Accepted on POSITIVE identification (`motion` present AND no ambient) so the deliberate 2-column refusal still holds for a shifted/truncated 3-LED file — asserted both ways. Site comes from PROVENANCE: two real wavelengths are not replicated, so the replication scan would have said 'wrist', skipped the 156-sentinel pass and stamped a wrist tier on a fingertip pleth. Channel identity remains OPEN — ch0/ch1, no RED/IR claim, no SpO₂. Gates: 11 bundles clean, verify:docs + verify:analysis OK, verify-fixtures green with one PpgDex fixture re-stamped.
