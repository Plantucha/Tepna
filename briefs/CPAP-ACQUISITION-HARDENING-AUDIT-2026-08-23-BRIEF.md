...
-->
**Status:** PROPOSED (re-verified 2026-09-25, Kestrel triage, docs only — Heron's four pending §8 rows are now RECONCILED one at a time against `capture-host/cpap_record.py` and `tests/test_cpap_record.py` on `origin/main`: INV3 met (device start + samples written VERBATIM, `test_device_start_and_samples_are_verbatim`); INV5 met for the RECORD half (`device_interval_ms` is the OBSERVED interval, `None` when the device omitted it, `test_observed_interval_is_recorded_and_none_when_absent`; the "prefer observed" consumer half is P3's); INV9 met (the JSONL raw record is a sidecar beside the bus, wired at `capture.py` `RawRecordSink(...)` and read by `acq_evidence_cpap.py`); INV1 met BY CONSTRUCTION (one `RawRecordSink` per host-authored `session_id`, the id in the header) with no test asserting cross-session uniqueness — the rows in §8 carry these verdicts. Master checklist still open on P3/P5 and the box; NO STATUS CHANGE. Parked 2026-09-02 — the living CHARTER for the P1–P5 programme; it outlives its phases and cannot be closed by them. P1 (#1679-era `cpap_record.py`) and P2 (`cpap_acq.py`) are DONE, **P4 is DONE 2026-09-21 (#2738)** — its last box, the between-rounds capture, was answered by the production ledger, and INV6 below moves DESIGNED → SHIPPED with it. ⚠️ **Re-verified 2026-09-21 (Heron): four §8 rows — INV1, INV3, INV5, INV9 — still read `pending P1` while this header says P1 is DONE.** `capture-host/cpap_record.py` exists in the tree; the rows were not updated when it shipped. Reconciling them is four verifications against that module and its tests, each its own claim, deliberately NOT done here by reading the header — the next pickup should do them one at a time in the tree. Two things hold the master checklist open: **P3 now HAS its brief** — `CPAP-ACQ-P3-GAP-ACCOUNTING-2026-09-18-BRIEF.md`, created 2026-09-18, which re-measured §8 and found the INV7 row stale: the gap-accounting module is NOT "held" but partly wired, with a dead classifier twin and three counters that no code can increment; and **INV8 and INV11 have no artifact** — there is no `continuity_status` field anywhere in `capture-host/`, and no owner/lock for INV11's one-acquisition-owner-at-a-time rule, which §8 itself marks *pending wiring*. Ten of twelve invariants are code-backed and cited. **Owner:** lead · **Next step:** execute `CPAP-ACQ-P3-GAP-ACCOUNTING-2026-09-18-BRIEF.md` W1–W4 — INV7's remainder, INV8 and INV11 all hang off it) · **Created:** 2026-08-23
...
|---|---|---|---|
| INV1 | Every stored sample belongs to exactly one acquisition session. | P1 record + P2 session id | **met** — verified 2026-09-25 by construction: one `RawRecordSink` per host-authored `session_id`, header carries it (no cross-session uniqueness test) |
| INV2 | Every acquisition session has an explicit lifecycle. | P2 (`cpap_acq` state machine) | **SHIPPED #1679** |
| INV3 | Raw samples are never silently replaced by derived values. | P1 (raw preserved; no interpretation in acquisition) | **met** — verified 2026-09-25: `on_batch` writes device start + samples VERBATIM (`test_device_start_and_samples_are_verbatim`) |
...
| INV5 | Observed sample interval is preferred over requested interval. | P1/P3 (record device-reported interval; prefer observed) | **met (record half)** — verified 2026-09-25: `device_interval_ms` is OBSERVED, `None` when absent (`test_observed_interval_is_recorded_and_none_when_absent`); the prefer-observed consumer half is P3 |
| INV6 | Partial spool rounds cannot advance the committed cursor. | P4 (brief §3 cursor-commit rule) | **SHIPPED and test-backed** — #1711 C1 (a round killed mid-transfer leaves `.part` and NO ledger line, so the cursor cannot advance), wired #1838; and **production-confirmed 2026-09-21** (#2738): a committed cursor was resumed 24 h later in a different process and served a clean round, so P4 §6's prediction held and its `revalidate` seam stays absent |
| INV7 | A transport gap is represented explicitly. | P3 (gap accounting) | **PARTLY LIVE — re-measured 2026-09-18** (`CPAP-ACQ-P3-GAP-ACCOUNTING-2026-09-18-BRIEF.md` §0). NOT "held": `GapCounters`/`FrameKind` are wired (`cpap_stream.py` `stream_to_bus` → `as11_pull.py` `stream` → logged `:210` → `acq_evidence_cpap.py`), so OK/FOREIGN/MALFORMED frames are counted. But `classify_frame` is a DEAD TWIN of that live logic, and `overflow`/`post_drop_tail`/`stalls` have **no writer** — so `total_lost` ≡ `malformed` and `transport_gaps` is structurally 0. Transport loss is still unrepresented |
| INV8 | Recovery does not imply continuity until continuity is verified. | P3/P5 (continuity-status field) | **SHIPPED 2026-09-18** — `capture-host/cpap_continuity.py`, wired by `_build_cpap_controller`, four states (`continuous` · `resumed-unverified` · `verified-continuous` · `verified-gap`), verified on the device's own `startTime` across the drop; published on the acq-evidence envelope, the controller's start result and the gap-accounting log. W3 of `CPAP-ACQ-P3-GAP-ACCOUNTING-2026-09-18-BRIEF.md`. |
| INV9 | The live bus is not the sole authoritative copy. | P1 (raw sidecar beside the bus — the centerpiece) | **met** — verified 2026-09-25: JSONL raw record beside the bus, `RawRecordSink` wired in `capture.py`, read by `acq_evidence_cpap.py` |
