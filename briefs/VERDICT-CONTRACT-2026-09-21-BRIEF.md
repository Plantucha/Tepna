...
**Status:** DONE — 2026-09-25 (**TRIAGED 2026-09-25 (Wren, tree; rule 0: "tepna.verdict/1 machine readable verdict contract adoption set gate")**, §5 item by item. (1) `verdict.js` + schema + gate group: done 2026-09-22, #2797. (2) Wave 1 emits valid objects: `oracle-ecg-firmware-rr`, `measurement-walk` and `verify-seals` are all `adopted` in `tools/verdict-adoption.json`; `n1-cohort-track` is LOCAL (§3b row 6, 9/9 valid) and correctly absent from a manifest that enumerates repo producers only. (3) The adoption gate holds the set as an equality: done. (4) CLAUDE.md §🧾 states the rule, and the DOCS-INDEX row exists. **Adopter count, MEASURED 2026-09-25 by `node tools/verdict-adoption.mjs` on origin/main 60755013:** 203 producers enumerated, all 203 binned — decides 65 · already-json 8 · word-only 77 · test 53; **69 adopted (69 validated) · 4 pending · 0 de-adopted**. The 4 pending are all BAND-LESS and wait on the owner's held-out decision (`HELD-OUT-BAND-CAMPAIGN-2026-09-22-BRIEF.md`), each already an OPEN residue row: `2026-09-22-beat-correspondence-has-no-band`, `2026-09-22-deep-desat-falsifier-states-no-alpha`, `2026-09-22-finger-validate-batch-has-no-aggregate-bar`, `2026-09-22-stmt-delete-has-no-ratchet`. The two remaining debts are RATCHETS the gate enforces, not open items: 77 word-only (`WO_CLAIM_RATCHET`) and 4 ungated `decides` (`UNGATED_RATCHET`), both may only shrink. · EARLIER: IN-PROGRESS — 2026-09-22 (§3b steps 1–7 landed and wave 2 mostly drained in one day — 24 rows adopted, 23 pending (12 of them UNKNOWN-by-design awaiting the owner's held-out decision, 2 runners awaiting the wave-3 design, 2 decide-nothing tools with residue rows); §3c records the rules the adopters found. Earlier: steps 1 and 2 of §3b landed: `verdict.js` #2797, the adoption gate + manifest with 152 producers binned; 38 adoptions pending across the waves)) · **Created:** 2026-09-21 · **Interlocks:** `MEASUREMENT-INSTANCE-CONTRACT-2026-09-17-BRIEF.md` (the sibling contract for a *number*; this one is for a *verdict*) · `CAPTURE-NIGHT-SEAL-2026-09-21-BRIEF.md` (phase A's `verify-seals` is a first adopter; phase C gives the suite an independent reader) · `PARTIAL-ADOPTION-DETECTION-2026-09-20-BRIEF.md` (adoption is counted, not assumed) · CLAUDE.md §🧾
...
- [x] Wave 1 adopters emit valid objects (oracle · measurement-walk · verify-seals · n1-cohort-track). ✅ verified 2026-09-25 (Wren): the three repo tools `adopted`; n1-cohort-track local, 9/9 valid.
...
- [x] CLAUDE.md §🧾 states the rule (landed with this brief); DOCS-INDEX row; header → DONE with the
