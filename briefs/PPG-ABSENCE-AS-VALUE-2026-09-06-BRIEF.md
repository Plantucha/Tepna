...
**Status:** IN-PROGRESS (live writer BUILT — `stuck` + hold classification; `pinned` back-check is WU-6/WU-7, not this unit · **D1 (owner ruling 2026-09-19, BUILT the same day, Wren):** the sidecar names two SPAN KINDS — `absence` (the O2Ring reads exactly 100 with no finger: finger-off capture 2026-09-19, six independent stretches, 100.0 %; `docs/O2RING-FINGER-OFF-2026-09-19.md`) and `in-wear-rail` (0 / 199 occur ONLY while worn — zero is NOT idle; a statement about observation, not mechanism). Ninth column `kind` + a `kinds=` header token (`writers.RUN_KIND_BY_STREAM`, per stream, `unknown` wherever unmeasured — the Verity rail, the ring's 99/124). Emission is UNCHANGED: kind labels rows the run-length rule already chose, so `min_run` remains a rails-only question. NOT done here, deliberately: no refold, no consumer reads the kind yet (`ppgdex-dsp.js` parses the 9-column row and ignores it — asserted), and the sidecars on vigil keep their 8-column shape until the next deploy writes new ones · **D5 (owner re-ruling 2026-09-19 evening, BUILT the same night, Wren) SUPERSEDES D1: EMIT THE MEASUREMENT, NAME NOTHING.** D1's `absence` / `in-wear-rail` were falsified by the evening's pre-registered capture on a WORN finger (`docs/O2RING-FINGER-OFF-2026-09-19.md`, evening section): worn + disturbed AC ⇒ flat 100 up to 672 samples, and 0/199 never a steady rail — paired transient excursions. Off-finger ⇒ 100 was measured; 100 ⇒ off-finger was written; a converse error. The corpus bracketing pass split the 164 ring 100-runs ≥ T_STUCK into 3 bracketed (worn both sides) · 22 one-sided · 139 resolved by FILE POSITION — the value is necessary, not sufficient. The ninth column is now `bracket=<before>/<after>`, each `varied` · `flat` · `unavailable` (observations; `unavailable` = no window to examine, kept distinct from `flat` = examined and quiet, §∅ at the class level); rows are HELD until their after-window arrives and flushed at close() as after=unavailable; the rule line carries `bracket_window=375` (3 s, ≥ 2 beats at any rate ≥ 40 bpm) and `bracket_varied_min=20`. `RUN_KIND_BY_STREAM`, `run_kind` and `kinds=` are gone. AND A SECOND WITNESS, the device's own (owner: "fix it"): the ring declares contact out-of-band every second — RtParam byte 5 `sensorState`, already persisted as OXYFRAME's `contact` column and already the daemon's not-worn verdict; the sidecar was the one reader that could not see it. Measured against the labelled captures (host-time join, ±2 s): settled off-finger contact 0 ×423 / 1 ×0; worn ON 1 ×1,123 / 0 ×20 (those 20 s ARE the 2,492-plateau — the device called it); removal debounce ~5 s; evening occlusion/handling/shake contact=1 throughout, flashlight 0 on 8 frames exactly at the 672-run; `run_status` does NOT discriminate; `alarm_raw&3` empty; of the 25 ambiguous corpus runs the device calls 16 lead-off and 9 worn (all ≤ 419 samples). A tenth column `contact` carries the majority byte over frames within ±2 s of the span VERBATIM (0·1·2·3) or `none` when no frame overlapped — distinct from 0; `writers.ContactLedger` is fed at the OXYFRAME write in `run_oxyii` and handed to the ppg1 writer; Verity/ACC rows read `none` with `contact_source=none` on the rule line — bracketing is their only witness. Both witnesses on every row, neither standing in for the other. Emission is unchanged (plant-tested). Naming absence is the CONSUMER's, which now has the device's word, the neighbourhood, and the file boundary) · **Created:** 2026-09-06 · **Residue:** 2026-09-06-warmup-verdict-goes-stale, 2026-09-06-t-stuck-validated-on-ring-only
...
```
# stream=<s> rule=stuck min_run=<n> t_stuck=<n> held_warmup=<n> held_top2_share=<f> unit=unknown bracket_window=<n> bracket_varied_min=<n>
