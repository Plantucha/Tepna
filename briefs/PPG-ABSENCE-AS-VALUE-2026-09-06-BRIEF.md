...
**Status:** DONE — 2026-09-21 (the last item, the byte-level parity fixture, is RE-SCOPED and closed: the two witnesses key on different predicates by design — §4b — so a parity fixture between them cannot exist; what replaces it, `crossCheckPinned`'s stated non-comparability plus `coincidentSpans`/`sidecarBlind`, is gated. Measured on the 09-19 night: 3 stuck spans vs 0 rail-pinned, coincident 0. Previously IN-PROGRESS 2026-09-20 — NOT owner-blocked: D1 ruled 2026-09-19; the item was described as waiting on WU-6's plant, which would have been a plant of the other predicate) · **Created:** 2026-09-06 · **Residue:** 2026-09-06-warmup-verdict-goes-stale, 2026-09-06-t-stuck-validated-on-ring-only
...
- [x] Byte-level parity fixture against the JS recompute path — **RE-SCOPED AND CLOSED 2026-09-21: the fixture as conceived cannot exist, and §4b is why.** A parity fixture asserts two computations of ONE quantity agree byte for byte. The live writer keys on CONSTANCY (`stuck`, any value, min run 200) and the JS recompute keys on RAIL MEMBERSHIP (`pinnedSpans`, rail-pinned) — `crossCheckPinned` declares this in its own contract (`fileRule: 'stuck-any-value'`, `derivedRule: 'rail-pinned'`, `rulesComparable: false — permanently false, and stated rather than implied`). Measured 2026-09-21 on the 09-19 O2Ring night (2,597,071 samples): sidecar comparable, min run 200, **3** stuck spans; JS rail-pinned derivation at the same min run **0** spans; coincident 0; sidecarBlind 0. A fixture pinning 3 ≡ 0 is red by construction; one made to pass would have forced a single predicate onto both sides, the category error §4b names. What stands in its place is already gated: `crossCheckPinned`'s stated non-comparability, `coincidentSpans` (geometric only, named so it cannot read as corroboration) and `sidecarBlind` (the one real threshold question), in the `sidecar populations` group. WU-6's plant is not the blocker and never was — it would have been a plant of the OTHER predicate. Nothing remains for this brief; the precedence question is answered by §4b as not arising.
