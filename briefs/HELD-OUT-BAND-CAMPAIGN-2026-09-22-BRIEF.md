...
**Status:** PROPOSED (**TRIAGED 2026-09-25 (Wren, tree; rule 0: "held-out band campaign derive thresholds then freeze and read held-out nights")**: still UNANSWERED. §5's box is open, line 13 says so, and no commit has touched this brief since it was created. Two facts for whoever puts it to the owner: (a) it now GATES four named producers. The 4 `pending` rows of `tools/verdict-adoption.json` (beat-correspondence, deep-desat-falsifier, o2ring-finger-validate-batch, stmt-delete) are all BAND-LESS and wait on this answer (see `VERDICT-CONTRACT-2026-09-21-BRIEF.md`, stamped DONE the same day with those four as OPEN residue rows). (b) It is NOT listed in `OWNER-DECISION-QUEUE-2026-09-03-BRIEF.md`, the page that surfaces owner calls (grep "held-out": 0 hits), so as things stand nothing puts it in front of the owner. · EARLIER: an OWNER QUESTION with its cost measured — not a plan; nothing here runs until the owner answers, and the bands in §4 are drafts to ratify or strike BEFORE any run) · **Created:** 2026-09-22 · **Relates:** `VERDICT-CONTRACT-2026-09-21-BRIEF.md` §1 (a threshold derived from the data it judges is UNKNOWN, not PASS) · residues `2026-09-22-nsrr-pool-scorers-have-no-prestated-band`, `2026-09-22-bandless-analysis-tools-emit-unknown`
