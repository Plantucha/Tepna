...
function _clampAge(v){ var n = parseInt(v, 10); return (isFinite(n) && n >= 6 && n <= 100) ? n : null; }
...
var _memStore = (function(){ var m={}; return {
...
function _store(){
  ...
  try { if(global.localStorage){ global.localStorage.getItem(STORAGE_KEY); return (_storeRef=global.localStorage); } } catch(e){}
  return (_storeRef=_memStore);
}
function _setStore(obj){ _storeRef = obj || _memStore; _rec = null; _runtimeDetected = {}; }
...
// ════════════════════════════════════════════════════════════════════════════
var NHANES = {
  M:[[18,79.8,177.4],[30,85.6,177.2],[40,89.4,176.7],[50,91.4,175.8],[60,87.9,174.6],[70,82.1,172.3]],
  F:[[18,68.2,163.5],[30,73.4,163.2],[40,76.8,162.7],[50,78.3,161.9],[60,76.1,160.5],[70,71.4,158.2]]
};
var VO2_NORM = {
  M:[[25,45],[35,43],[45,39],[55,35],[65,31]],
  F:[[25,36],[35,33],[45,29],[55,26],[65,22]]
};
var NORMS = {
...
function _interp2(tbl, age){
  ...
  for(var i=0;i<tbl.length-1;i++){ if(age>=tbl[i][0]&&age<tbl[i+1][0]){
    var f=(age-tbl[i][0])/(tbl[i+1][0]-tbl[i][0]);
    return [ +(tbl[i][1]+f*(tbl[i+1][1]-tbl[i][1])).toFixed(1),
             +(tbl[i][2]+f*(tbl[i+1][2]-tbl[i][2])).toFixed(1) ]; } }
...
function _interp1(tbl, age){
  ...
  for(var i=0;i<tbl.length-1;i++){ if(age>=tbl[i][0]&&age<tbl[i+1][0]){
    var f=(age-tbl[i][0])/(tbl[i+1][0]-tbl[i][0]); return Math.round(tbl[i][1]+f*(tbl[i+1][1]-tbl[i][1])); } }
  return tbl[0][1];
}
function vo2Norm(age, sex){ return _interp1(VO2_NORM[sex==='F'?'F':'M'], age); }

// Population defaults keyed on the record's age + sex (cascade tier 1 — the floor)
function popDefaults(rec){
  var age = rec.age || 42, sex = rec.sex === 'F' ? 'F' : 'M';
  ...
  return {
    ...
    sbp:    120, dbp: 80,                    // ACC/AHA guideline-normal (type-only override)
    vo2:    vo2Norm(age, sex),
    elevation: 0,
    activity: 'light', betablk: 'no', afib: 'no', cpap: 'no'
  };
}
...
// ════════════════════════════════════════════════════════════════════════════
function _blank(){ return { schema:SCHEMA, units:'metric', age:DEFAULT_AGE, ageSet:false, sex:'M', manual:{}, detected:{}, _pristine:true }; }
...
function load(){
  if(_rec) return _rec;
  var r = null;
  try { r = JSON.parse(_store().getItem(STORAGE_KEY) || 'null'); } catch(e){}
  if(!r){ migrate(); try{ r = JSON.parse(_store().getItem(STORAGE_KEY)||'null'); }catch(e){} }
  _rec = r || _blank();
  ...
  { var _a = _clampAge(_rec.age), _chg = false;
    if(_rec.ageSet===undefined){ _rec.ageSet = (_a!=null && _a!==DEFAULT_AGE); _chg = true; }
    if(_a==null){ if(_rec.age!==DEFAULT_AGE){ _rec.age = DEFAULT_AGE; _chg = true; } if(_rec.ageSet){ _rec.ageSet = false; _chg = true; } }
    else if(_a!==_rec.age){ _rec.age = _a; _chg = true; }
    if(_chg){ try{ _store().setItem(STORAGE_KEY, JSON.stringify(_rec)); }catch(e){} }
  }
  if(!_rec.detected) _rec.detected = {};
  if(_rec.sex!=='M' && _rec.sex!=='F') _rec.sex = (String(_rec.sex).toLowerCase().charAt(0)==='f'?'F':'M');
  return _rec;
}
function getRecord(){ return load(); }
function isPristine(){ return !!load()._pristine; }
function save(rec){ if(rec) _rec = rec; load(); try{ _store().setItem(STORAGE_KEY, JSON.stringify(_rec)); }catch(e){} return _rec; }
...
function _sex(v){ if(v==null) return null; var c=String(v).toLowerCase().charAt(0); return c==='f'?'F':(c==='m'?'M':null); }
function _yn(v){ if(v==null) return null; return (v===true||v==='yes'||v==='true')?'yes':'no'; }
function _readJSON(k){ try{ return JSON.parse(_store().getItem(k)||'null'); }catch(e){ return null; } }

function migrate(){
  if(_store().getItem(STORAGE_KEY)) return false;            // already unified
  var rec = _blank(), m = rec.manual, touched = false;
  function setId(v){ var a=_clampAge(v); if(a!=null){ rec.age = a; rec.ageSet = true; touched = true; } }
  function setSx(v){ if(v!=null){ rec.sex = v; touched = true; } }
  function put(k,v){ if(v!=null && v!=='' && !(k in m)){ m[k]=v; touched=true; } }

  // OxyDex — oxydex_profile (JSON, canonical superset shape)
  var ox = _readJSON('oxydex_profile') || _readJSON('o2ring_profile');
  if(ox){
    setId(_num(ox.age)); setSx(_sex(ox.sex));
    put('weight',_num(ox.weight)); put('height',_num(ox.height));
    put('sbp',_num(ox.sbp)); put('dbp',_num(ox.dbp));
    ...
    put('elevation',_num(ox.elevation)); put('cpap',_yn(ox.cpap));
  }
  // ECGDex / PpgDex — keyed by DOM id (ecg*/ppg*)
  [['ecgdex_profile','ecg'],['ppgdex_profile','ppg']].forEach(function(pair){
    var o=_readJSON(pair[0]), p=pair[1]; if(!o) return;
    setId(_num(o[p+'Age'])); setSx(_sex(o[p+'Sex']));
    put('weight',_num(o[p+'Weight'])); put('height',_num(o[p+'Height']));
    ...
    put('elevation',_num(o[p+'Elev'])); put('cpap',_yn(o[p+'CPAP']));
  });
  // PulseDex — pulsedex_profile (keyed by DOM id, mixed)
  var px = _readJSON('pulsedex_profile');
  if(px){
    setId(_num(px.pxAge||px.age)); setSx(_sex(px.pxSex||px.sex));
    put('weight',_num(px.pxWeight||px.weight)); put('height',_num(px.pxHeight||px.height));
    ...
    put('elevation',_num(px.pxElev)); put('cpap',_yn(px.pxCPAP));
  }
  // HRVDex — 10× per-key prof_*
  var st=_store();
  if(st.getItem('prof_age')!=null || st.getItem('prof_sex')!=null){
    setId(_num(st.getItem('prof_age'))); setSx(_sex(st.getItem('prof_sex')));
    put('weight',_num(st.getItem('prof_weight'))); put('height',_num(st.getItem('prof_height')));
    put('sbp',_num(st.getItem('prof_sbp'))); put('dbp',_num(st.getItem('prof_dbp')));
  ...
  if(gl){
    setId(_num(gl.gluAge)); setSx(_sex(gl.gluSex));
    put('diabetes', gl.gluDiab && gl.gluDiab!=='none' ? gl.gluDiab : null);
    put('dxTherapy', gl.gluTherapy && gl.gluTherapy!=='none' ? gl.gluTherapy : null);
    put('glucoseTargetLo',_num(gl.gluTgtLo)); put('glucoseTargetHi',_num(gl.gluTgtHi));
    if(_num(gl.gluA1c)>0) put('a1c',_num(gl.gluA1c));
  }

  if(touched){ delete rec._pristine; try{ _store().setItem(STORAGE_KEY, JSON.stringify(rec)); }catch(e){} _rec = rec; return true; }
  return false;
}
...
// ════════════════════════════════════════════════════════════════════════════
function setDetected(map){ _runtimeDetected = map || {}; }
function getDetected(){ return _runtimeDetected; }
function prefillFrom(detected){
  load(); detected = detected || _runtimeDetected || {};
...
function resolve(field){
  var rec = load();
  if(field==='age'){ var _ca=_clampAge(rec.age); return { v:(_ca!=null?_ca:DEFAULT_AGE), origin:(rec.ageSet?'you':'pop') }; }
...
function get(){
  var rec = load(), pd = popDefaults(rec);
  var p = { age:resolve('age').v, sex:rec.sex, units:rec.units||'metric' };
  ['weight','height','bodyfat','waist','hrRest','hrMax','betablk','afib','sbp','dbp',
   'vo2','activity','elevation','cpap','diabetes','dxTherapy','glucoseTargetLo','glucoseTargetHi','a1c']
   .forEach(function(k){ p[k] = resolve(k).v; });
  p._origins = {};
  Object.keys(p).forEach(function(k){ if(k.charAt(0)!=='_') p._origins[k] = resolve(k).origin; });
  return p;
}
...
function setManual(field, vMetric){
  ...
  if(field==='age'){ var _sa=_clampAge(vMetric); if(_sa!=null){ _rec.age=_sa; _rec.ageSet=true; } else { _rec.age=DEFAULT_AGE; _rec.ageSet=false; } }
  else if(field==='sex'){ _rec.sex = _sex(vMetric)||_rec.sex; }
  else if(field==='units'){ _rec.units = (vMetric==='imperial')?'imperial':'metric'; }
...
function vo2Category(v, age, sex){ var n=vo2Norm(age,sex), r=v/n;
  return r>=1.25?'Superior':r>=1.10?'Excellent':r>=0.95?'Good':r>=0.80?'Fair':'Poor'; }
function vo2Percentile(v, age, sex){ var n=vo2Norm(age,sex); return Math.max(1,Math.min(99,Math.round(50+(v/n-1)*120))); }

function derive(profile, signalCtx){
  var p = profile || get();
  var age=p.age, sex=p.sex==='F'?'F':'M';
  var W=+p.weight, H=+p.height, sbp=+p.sbp, dbp=+p.dbp;
  var bb = p.betablk==='yes', af = p.afib==='yes';
  var hM = H/100;
  ...
  var rmr, rmrFormula;
  var bf = +p.bodyfat;
  if(bf>3 && bf<60){ var lean=W*(1-bf/100); rmr=Math.round(370+21.6*lean); rmrFormula='Katch-McArdle 370 + 21.6·LBM ('+lean.toFixed(1)+' kg)'; }
  else { rmr = Math.round(10*W + 6.25*H - 5*age + (sex==='M'?5:-161)); rmrFormula='Mifflin-St Jeor'; }
  ...
  var whtr=null, whtrRisk=null;
  var waist=+p.waist;
  if(waist>40 && waist<200){ whtr=+(waist/H).toFixed(2); whtrRisk = whtr>=0.5?'elevated':'ok'; }
  ...
  function gt(field, label, refV, refSrc){
    ...
    var entered=+r.v, delta=entered-refV, pct = refV? Math.round(delta/refV*100):0;
    gtc.push({ field:field, label:label, entered:entered, reference:+(+refV).toFixed(1),
               refSource:refSrc, deltaPct:pct, status:Math.abs(pct)<=15?'match':'diverge' });
  }
  ...
  return {
    bmi:bmi, bmiCat:bmiLabel(bmi), bsa:bsa, ibw:ibw, map:map, pp:pp,
    rmr:rmr, rmrFormula:rmrFormula, hrMax:hrMax, vo2:vo2, vo2Cat:vo2Cat, vo2Pct:vo2Pct,
    whtr:whtr, whtrRisk:whtrRisk,
    flags:flags, groundTruthChecks:gtc
  };
}
...
var GROUP_DEFS = {
  identity:{ label:'Identity', fields:[
    ...
    {k:'sex', label:'Sex', identity:true, select:[['M','Male'],['F','Female']]} ] },
  body:{ label:'Body', fields:[
    {k:'weight', label:'Weight'}, {k:'height', label:'Height'},
    {k:'bodyfat', label:'Body fat %', opt:true}, {k:'waist', label:'Waist', opt:true} ] },
  cardio:{ label:'Cardio / autonomic', fields:[
    {k:'hrRest', label:'Resting HR (awake)', unit:'bpm', popLabel:'typical'},
    {k:'hrMax', label:'HRmax', unit:'bpm'},
    {k:'betablk', label:'Beta-blocker / HR meds', select:[['no','No'],['yes','Yes']], popLabel:'default'},
    {k:'afib', label:'Irregular rhythm / AF', select:[['no','No'],['yes','Yes']], popLabel:'default'} ] },
  hemo:{ label:'Hemodynamic', fields:[
    {k:'sbp', label:'Systolic BP', unit:'mmHg', popLabel:'default'},
    {k:'dbp', label:'Diastolic BP', unit:'mmHg', popLabel:'default'} ] },
  fitness:{ label:'Fitness', fields:[
    {k:'vo2', label:'VO₂max', unit:'mL/kg/min'},
    {k:'activity', label:'Activity level', popLabel:'default',
      select:[['sed','Sedentary'],['light','Light'],['mod','Moderate'],['active','Active'],['vig','Very active']]} ] },
  env:{ label:'Environment', fields:[ {k:'elevation', label:'Elevation'} ] },
  therapy:{ label:'Therapy', fields:[ {k:'cpap', label:'CPAP / BiPAP therapy', select:[['no','No'],['yes','Yes']]} ] },
  // Combined display group (mockup parity) — 4 fields = a filled 2×2, no empty cells.
  fitnessEnvTherapy:{ label:'Fitness, environment & therapy', fields:[
    {k:'vo2', label:'VO₂max', unit:'mL/kg/min'},
    {k:'activity', label:'Activity level', popLabel:'default',
      select:[['sed','Sedentary'],['light','Light'],['mod','Moderate'],['active','Active'],['vig','Very active']]},
    {k:'elevation', label:'Elevation'},
    {k:'cpap', label:'CPAP / BiPAP therapy', select:[['no','No'],['yes','Yes']]} ] },
  metabolic:{ label:'Metabolic', fields:[
    {k:'diabetes', label:'Diabetes status', popLabel:'default', select:[['none','None'],['predm','Pre-diabetes'],['t1','Type 1'],['t2','Type 2']]},
    {k:'dxTherapy', label:'Therapy', popLabel:'default', select:[['none','None / lifestyle'],['orals','Oral agents'],['basal','Basal insulin'],['mdi','MDI'],['pump','Pump']]},
...
function unitLabel(k){ var u=UNITS[k]; if(!u) return ''; return resolve('units').v==='imperial'?u.i:u.m; }
function toDisp(k,vMetric){ var u=UNITS[k]; if(!u||resolve('units').v!=='imperial'||vMetric==null) return vMetric; return +(vMetric*u.f).toFixed(u.dp); }
function toMetric(k,vDisp){ var u=UNITS[k]; if(!u||resolve('units').v!=='imperial'||isNaN(vDisp)) return vDisp; return +(vDisp/u.f).toFixed(2); }
function _badge(t){ return (global.MetricRegistry && global.MetricRegistry.badge) ? global.MetricRegistry.badge(t) : '<span class="ev ev-'+t+'"></span>'; }
...
function _injectCSS(){
  ...
  var s=global.document.createElement('style'); s.id='dex-profile-css'; s.textContent=css;
...
function _sub(f, r){
  ...
  switch(f.k){
    case 'age': return r.origin==='you' ? ['ok','✓ your value — drives every age norm below'] : ['est','default '+DEFAULT_AGE+' · set your age — drives every age norm below'];
    case 'sex': return ['','— drives BMR · IBW · VO₂ & body norms'];
    case 'weight': return [r.origin==='you'?'ok':'est', r.origin==='you'?'✓ your value':'~ pop avg '+toDisp('weight',pd.weight)+' '+unitLabel('weight')];
    case 'height': return [r.origin==='you'?'ok':'est', r.origin==='you'?'✓ your value':'~ pop avg '+toDisp('height',pd.height)+' '+unitLabel('height')+' (age '+load().age+')'];
    case 'hrRest':
      if(r.origin==='you') return ['ok','✓ your value (awake)'];
      if(r.origin==='detected'){ var d=getDetected(); return d._floor?['est','~ sleeping floor '+d._floor+' + 8 = '+r.v+' bpm awake']:['est','~ detected '+r.v+' bpm']; }
      return ['est','~ typical '+pd.hrRest+' bpm · wide spread — load a recording or type yours'];
    case 'hrMax': return r.origin==='you'?['ok','✓ your value']:['est','~ Tanaka 208 − 0.7×'+load().age+' = '+pd.hrMax];
    case 'sbp': return [r.origin==='you'?'ok':'', r.origin==='you'?'✓ your value':'default 120 · type a cuff reading'];
    case 'dbp': return [r.origin==='you'?'ok':'', r.origin==='you'?'✓ your value':'default 80 · age/sex-independent'];
    case 'vo2':
    ...
    case 'elevation': return ['', (r.v>1500)?('⛰ '+toDisp('elevation',r.v)+' '+unitLabel('elevation')+' · adjusts VO₂ & SpO₂ norms'):'~ sea level · adjusts above '+toDisp('elevation',1500)+' '+unitLabel('elevation')];
    case 'bodyfat': return r.origin==='you'?['ok','✓ '+r.v+'% → Katch-McArdle (lean-mass) RMR']:['','optional · enables Katch-McArdle RMR'];
    case 'waist': { var h=resolve('height').v; return r.origin==='you'?['ok','✓ WHtR '+(r.v/h).toFixed(2)+' · visceral / OSA risk']:['','optional · waist÷height (visceral / OSA risk)']; }
    case 'betablk': return (r.v==='yes')?['est','⚠ blunts HRmax & resting HR — HR-based VO₂ / zones flagged']:['','— none · HR-based calcs valid'];
    case 'afib': return (r.v==='yes')?['est','⚠ irregular rhythm — HRV (rMSSD / SDNN) unreliable']:['','— regular rhythm · HRV metrics valid'];
    case 'activity': return ['','— activity level · energy-budget input'];
    case 'cpap': return ['','reframes apnea read as residual on therapy'];
    case 'diabetes': return ['', (r.v&&r.v!=='none')?'— shifts which thresholds matter & how hypo reads':'— general-population ranges'];
    case 'glucoseTargetLo': return ['', r.origin==='you'?'✓ custom lower bound':'consensus default 70 mg/dL'];
    case 'glucoseTargetHi': return ['', r.origin==='you'?'✓ custom upper bound':'consensus default 180 mg/dL'];
    case 'dxTherapy': return (['basal','mdi','pump'].indexOf(r.v)>=0)?['est','⚠ on insulin — hypo (TBR) goal tightens to <4%']:['','reframes hypo risk vs therapy'];
    case 'a1c': return r.origin==='you'?['ok','✓ lab HbA1c — anchors eAG comparison']:['','optional · last lab HbA1c'];
  }
  return ['',''];
}
...
function renderPanel(opts){
  ...
  if(!mount){ mount = global.document.getElementById('pbody') || global.document.getElementById('profileBody'); }
  if(!mount) return;
  ...
  function _legendHTML(){
    var tiers = [['measured','Measured'],['validated','Validated'],['emerging','Emerging'],['experimental','Experimental'],['heuristic','Heuristic']];
    return '<div class="dxp-legend"><span class="lg-t">Evidence</span>' +
      tiers.map(function(t){ return '<span class="lg-i">'+_badge(t[0])+'<b>'+t[1]+'</b></span>'; }).join('') +
      '<span class="lg-n">disc fill = trust</span></div>';
  }
  function _cascadeHTML(){
    return '<div class="dxp-card"><h4>How each value is chosen</h4>'
  ...
  function _normsHTML(){
    ...
    ages.forEach(function(a){
      var m = _interp2(NHANES.M, a), f = _interp2(NHANES.F, a);
    ...
    return '<div class="dxp-card"><h4>Unified norms &amp; sources <span class="h4n">every default is cited</span></h4>'
      ...
      + '<table class="dxp-matrix"><thead><tr><th>Age</th><th>Wt M</th><th>Ht M</th><th>Wt F</th><th>Ht F</th><th>RHR</th><th>HRmax</th><th>BP</th></tr></thead><tbody>'+body+'</tbody></table>'
      + '<div class="dxp-srcs">'+src.map(function(s){ return '<div class="s"><b>'+s[0]+'</b> — '+s[1]+'<span>'+s[2]+'</span></div>'; }).join('')+'</div></div>';
  }
  function build(){
    ...
    groups.forEach(function(gk){
      var g = GROUP_DEFS[gk]; if(!g) return;
      var right;
      if(gk==='identity'){
        right = '<span class="units-toggle">'+[['metric','Metric'],['imperial','Imperial']].map(function(u){
          return '<button class="ut'+(rec.units!=='imperial'&&u[0]==='metric'||rec.units==='imperial'&&u[0]==='imperial'?' on':'')+'" data-u="'+u[0]+'">'+u[1]+'</button>'; }).join('')+'</span>';
      } else { right = '<span class="src shared">↔ shared identity</span>'; }
      ...
      g.fields.forEach(function(f){
        ...
        if(f.select){
          ...
          f.select.forEach(function(o){ html += '<option value="'+o[0]+'"'+(String(r.v)===o[0]?' selected':'')+'>'+o[1]+'</option>'; });
          html += '</select></div>';
        } else {
    ...
    wire();
    if(_fk){ var _el = mount.querySelector('[data-k="'+_fk+'"]'); if(_el){ try{ _el.focus(); if(_caret!=null && _el.setSelectionRange) _el.setSelectionRange(_caret, _caret); }catch(e){} } }
  }
  ...
  function wire(){
    mount.querySelectorAll('.prof-input').forEach(function(el){
      ...
      el.addEventListener(evt, function(){
        ...
        if(evt==='change'){ onChange(get()); }
        else { clearTimeout(_ocTimer); _ocTimer = setTimeout(function(){ onChange(get()); }, 400); }
      });
    });
    mount.querySelectorAll('.dxp-revert').forEach(function(b){
      b.addEventListener('click', function(){ setManual(b.getAttribute('data-rev'), null); rec=load(); build(); onChange(get()); });
    });
    mount.querySelectorAll('.units-toggle .ut').forEach(function(b){
      b.addEventListener('click', function(){ setManual('units', b.getAttribute('data-u')); rec=load(); build(); onChange(get()); });
    });
  }

  function _derivedHTML(){
    var p = get(), d = derive(p);
    ...
    function di(l,v,f,t){ return '<div class="prof-derived-item">'+_badge(t)+' <b>'+l+'</b> '+v+(f?'<span class="pdi-formula">'+f+'</span>':'')+'</div>'; }
    var items = '';
    if(hasBody){
    ...
    if(hasCardio) items += di('HRmax', d.hrMax+' bpm'+(d.flags.betaBlocker?' ⚠':''), d.flags.betaBlocker?'β-blocker: Tanaka unreliable':'Tanaka 208−0.7·age', d.flags.betaBlocker?'experimental':'validated');
    if(hasHemo){
...
// ════════════════════════════════════════════════════════════════════════════
global.DexProfile = {
  VERSION:'1.0.0', STORAGE_KEY:STORAGE_KEY, SCHEMA:SCHEMA,
  NORMS:NORMS, MANIFESTS:MANIFESTS, GROUP_DEFS:GROUP_DEFS, UNITS:UNITS,
  get:get, getRecord:getRecord, load:load, save:save, migrate:migrate,
  resolve:resolve, setManual:setManual, derive:derive, _setStore:_setStore, isPristine:isPristine,
  setDetected:setDetected, getDetected:getDetected, prefillFrom:prefillFrom,
  popDefaults:popDefaults, vo2Norm:vo2Norm, vo2Category:vo2Category, vo2Percentile:vo2Percentile,
  renderPanel:renderPanel,
  toDisp:toDisp, toMetric:toMetric, unitLabel:unitLabel
};
