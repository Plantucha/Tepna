...
function loadProfile(){
  if(DP()){
    ...
    set('ppgAge',p.age); const sx=$('ppgSex'); if(sx) sx.value=p.sex==='F'?'F':'M';
    set('ppgWeight',p.weight); set('ppgHeight',p.height);
    set('ppgHRmax', man.hrMax!=null?man.hrMax:''); set('ppgRHR', man.hrRest!=null?man.hrRest:'');
    set('ppgVO2', man.vo2!=null?man.vo2:''); set('ppgElev', p.elevation!=null?p.elevation:0);
    const cp=$('ppgCPAP'); if(cp && man.cpap!=null) cp.value=man.cpap;
    return;
  }
  let saved=null; try{ saved=JSON.parse(localStorage.getItem('ppgdex_profile')||'null'); }catch(e){}
  if(saved) KEYS.forEach(k=>{ const el=$(k); if(!el) return; if(saved[k]!=null&&saved[k]!=='') el.value=saved[k]; });
}
function saveProfile(){
  if(DP()){
    const g=id=>{ const e=$(id); return e?e.value:''; };
    const num=v=>{ const n=parseFloat(v); return isFinite(n)?n:null; };
    DP().setManual('age', num(g('ppgAge'))); DP().setManual('sex', g('ppgSex'));
    DP().setManual('weight', num(g('ppgWeight'))); DP().setManual('height', num(g('ppgHeight')));
    ...
    DP().setManual('elevation', num(g('ppgElev'))); DP().setManual('cpap', g('ppgCPAP'));
    return;
  }
  const o={}; KEYS.forEach(k=>{ const el=$(k); if(el) o[k]=el.value; }); try{ localStorage.setItem('ppgdex_profile',JSON.stringify(o)); }catch(e){}
}
...
function vo2Base(rhr,hrmax){ return 15.3*hrmax/rhr; }
...
function ansAge(rmssd,sdnn,hr){
  ...
  let cs=0,ws=0; if(c1!=null){cs+=0.25*c1;ws+=0.25;} if(c2!=null){cs+=0.40*c2;ws+=0.40;} if(c3!=null){cs+=0.35*c3;ws+=0.35;}
...
function perfusionBand(pi){ if(pi==null) return {sev:'neutral',txt:'—'}; if(pi>=4) return {sev:'ok',txt:'strong'}; if(pi>=1) return {sev:'ok',txt:'adequate'}; if(pi>=0.4) return {sev:'warn',txt:'low'}; return {sev:'bad',txt:'poor contact'}; }
...
function personalize(r){
  ...
  Object.assign(r,{ profile:p, tanaka, autoRHR, rhrEff, hrmaxEff, hrmaxRejected:(p.hrmax>0&&!hrmaxValid),
    altFactor:altF, vo2base:vo2b, vo2adj:vo2a, vo2gt:p.vo2gt>0?p.vo2gt:null,
    cpapInUse:p.cpap,
    ansAge:aa, hrvScore:hrvScore(r.dispRm), expRmssd:expectedRmssd(p.age), expRHR:expectedRHR(p.age) });
...
function renderHero(r){
  ...
  const ratio=r.dispRm/r.expRmssd; let ageNote;
  ...
  wrap.innerHTML=`<div class="readiness-hero" style="--readiness-color:${css}">`
    ...
    +(chips?`<div class="readiness-zones">${chips}</div>`:'')+`</div>`;
...
function renderHrvBench(r){
  ...
  if(!host){ host=document.createElement('div'); host.id='heroBenchPpg'; host.className='proj-grid'; top.appendChild(host); }
  const rm=r.dispRm, sd=r.dispSd, sd1=r.sd1, sd2=r.sd2, pn=r.dispPn, pi=r.perfusionIndex;
  ...
  const st=(lbl,val,unit,cls)=>`<div class="proj-stat ps-${cls}"><span class="ps-label">${lbl}${eb(lbl)}</span><span class="ps-val">${val!=null&&!isNaN(val)?val:'—'}<span class="ps-unit">${unit}</span></span></div>`;
...
function renderProfileDerived(){
  ...
  if(DP()){
    ...
    rmrF='Mifflin'; bmiCat=bmi<18.5?'Underweight':bmi<25?'Normal':bmi<30?'Overweight':'Obese';
  ...
  d.innerHTML=grp('Body composition',
      di('BMI',bmi.toFixed(1)+' ('+bmiCat+')','kg ÷ m²')+
      di('IBW',ibw.toFixed(1)+' kg','Devine')+
      di('BSA',bsa.toFixed(2)+' m²','DuBois')+
...
function computeHints(r){
  ...
  if(r){ const aa=r.ansAge||ansAge(r.dispRm,r.dispSd,r.dispHr);
  ...
  if(r){ const hm=r.hrmaxEff||Math.round(208-0.7*p.age), rh=Math.round(r.rhrEff||r.dispHr);
    ...
    else { const hrIn=Number($('ppgHRmax').value)||0; set('lbl_ppgHRmax', hrIn>0?'✓ your value '+hm+' bpm':'~ Tanaka: 208 − 0.7 × age = '+r.tanaka); }
    ...
    if(rhrIn>0) set('lbl_ppgRHR','✓ your value'); else set('lbl_ppgRHR','~ measured '+Math.round(r.dispHr)+' bpm',true);
  }
}

function render(r){ $('heroTop').style.display='grid'; $('sec-profile').style.display='block'; $('profilePanel').style.display='block';
  personalize(r); computeHints(r); renderHero(r); renderHrvBench(r); if (_dexPanel) _dexPanel.refresh(); else renderProfileDerived(r); }
function hide(){ $('heroTop').style.display='none'; $('sec-profile').style.display='none'; $('profilePanel').style.display='none';
...
function applyNorms(ideal){
  ...
  const hEl=$('ppgHeight'), wEl=$('ppgWeight'); if(!hEl||!wEl) return;
  const p=getProfile(); const n=popNorms(p.sex);
  hEl.value=n.h; wEl.value= ideal?+(22.5*(n.h/100)**2).toFixed(1):n.w;
  saveProfile(); if(reRenderFn) reRenderFn();
}
function init(reRender){ reRenderFn=reRender; loadProfile();
  if (DP() && DP().renderPanel) { _dexPanel = DP().renderPanel({ node:'ppgdex', mount:'dexProfilePanel', onChange:function(){ if(reRenderFn) reRenderFn(); } }); }
  else { renderProfileDerived(); } }
