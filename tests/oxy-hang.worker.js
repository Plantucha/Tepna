...
self.onmessage = function (e) {
  ...
  if (!READY) { self.postMessage({ type: 'error', error: ERR || 'not ready' }); return; }
  var nPatients = m.nPatients || 24;
  var targetNights = m.targetNights || 15;
  var nNights = 0, maxMs = 0, total0 = performance.now(), worstSeed = -1;
  try {
    for (var seed = 0; seed < nPatients * 8 && nNights < targetNights; seed++) {
      var pf;
      try { pf = CohortGen.patient(seed, { only: ['oxy'] }); } catch (_) { continue; }
      ...
      for (var k = 0; k < pf.nights.length; k++) {
        ...
        if (dt > maxMs) { maxMs = dt; worstSeed = seed; }
        nNights++;
