...
(function (global) {
  ...
  function renderOxy(tl) {
    ...
    const N = tl.durSec;
    const baseSpo2 = 96;
    /* Physiological rate limits, %/s. See the glide below for the measurement these come from.
       Deliberately NOT imported from oxydex-dsp's SELFGATE: the generator must not track the gate
       it is judged by, or the fixture passes by construction (SYNTH-GEN-DESAT-KINETICS §5). */
    const SPO2_FALL_MAX_PCT_S = 1.0;
    const SPO2_RISE_MAX_PCT_S = 1.5;
    let spo2f = baseSpo2,
      prf = 58;
    ...
    for (let s = 0; s < N; s++) {
      ...
      /* Glide toward the target, then RATE-LIMIT the step (SYNTH-GEN-DESAT-KINETICS).
         The glide alone is a first-order lag, so its INITIAL rate is k × depth: at k = 0.28 a 10 %
         event fell 2.8 %/s and a 15 % one 4.2 %/s. `SELFGATE.FALL_RATE_MAX` is 1.5 %/s because a
         real systemic desaturation is limited by circulation and lung oxygen stores, so OxyDex
         correctly classified those edges as probe artifact and threw them away — 232 of 242 events
         on night 2, giving ODI-4 1.4/h against a planted AHI of 38. The rejection scaled with
         severity purely because severe nights plant deeper events, which on its own reproduced the
         "severity-dependent ODI-4 deficit" that papers/odi4-ahi-bias.html reports.

         A lag is the wrong model: saturation is rate-limited, not exponentially-relaxing, so the
         limit is imposed directly rather than by shrinking k (which would also flatten shallow
         events that were never the problem).

         The caps are MEASURED, not inherited from the brief that asked for them — 36 real nights of
         `timeseries.spo2` plus 342 events carrying `onsetTMs`:
             1 s falls exceeding 1.5 %/s : 0.20 % of 21 609   (synthetic nights 1-2: 16.3 %, 32.0 %)
             onset → nadir               : p50 7 s · p90 25 s · p99 71 s
             nadir → end                 : p50 14 s · p90 38 s
             event depth                 : p50 5 % · p90 8 %
             implied mean fall rate      : 0.714 %/s
         1.0 %/s sits above that median and still leaves 0.5 %/s of headroom under the gate; the
         rise cap is looser because resaturation is the slower limb (14 s vs 7 s) and clamping it
         hard would merge events that should separate. Depth is untouched: a 7 % desaturation is
         still 7 % deep, it just takes 7 s to get there. */
      const k = target < spo2f ? 0.28 : 0.16;
      const step = (target - spo2f) * k;
      spo2f += Math.max(-SPO2_FALL_MAX_PCT_S, Math.min(SPO2_RISE_MAX_PCT_S, step));
  ...
  /* ═══ RAW µV ECG — the RR→PQRST renderer, lifted here from cohort-full.js ═════════════════════
     GENERATOR-FOLLOWUPS-II §3 step 1. This engine emits RR intervals, and every other node's
     waveform renderer already lives here — ECG's did not. It sat in `cohort-full.js`, a
     FULL-lane-worker file ECGDex itself cannot load, which is the whole reason ECGDex stayed
     single-recording while every other node moved onto the shared `DexPatientGen` axis.

     ⚠ BYTE-IDENTICAL LIFT, and it has to be: `cohort-full.js`'s output is snapshotted by the
     waveform-fidelity gate and consumed by `qrs-yield` and `qrs-equiv`. So this is a MOVE, not a
     rewrite — same template constants, same beat-nearest phase anchoring, same wander/noise terms,
     and the same `mulberry32`/`gaussFrom` (character-identical to the pair it replaces, `var` vs
     `let` aside — checked before lifting, because a different RNG here would silently move three
     published analyses). `cohort-full.js` now DELEGATES, keeping its 3-arg signature for its callers.

     Clock Contract: `t0Ms` is floating wall-clock ms off the timeline; sample times derive from it,
     never from a local `Date`. */

  // PQRST morphology: sum of skew-tuned Gaussians (µV), phase 0..1 within a beat. Amplitudes/widths
  // fit the Lead-II look ECGDSP.genSynthetic targets: R ~1.1 mV, sharp QRS, broad T, small P. Width
  // scales mildly with RR (rate-dependent QT).
  function pqrst(ph, rrSec) {
    function g(c, amp, w) {
      const d = ph - c;
      return amp * Math.exp(-(d * d) / (2 * w * w));
    }
    const qtScale = Math.min(1.25, Math.max(0.8, Math.sqrt(rrSec / 0.8))); // Bazett-ish T stretch
    return (
      g(0.18, 90, 0.025) + // P
      g(0.385, -180, 0.012) + // Q
  ...
  /* Render a window of the night's RR beats into an Int16 µV ECG @130 Hz + ground-truth deviceRR.
     tl = masterTimeline(...) ; win = { startRel, lenSec } (pickWindow, or a whole-night span).
     Beats come from the SAME buildRR(tl) series every other node consumes, so the ECG stays
     event-coherent with Oxy/PPG/Pulse (shared apnea clusters) and ECGDSP's own Pan–Tompkins must
     re-derive them from morphology — a round-trip test of the detector, not a replay of truth RR. */
  function renderECGInt16(tl, win) {
    const fs = 130;
    const t0Ms = tl.t0Ms + win.startRel * 1000;
    const N = Math.round(win.lenSec * fs);
    const rng = mulberry32((tl.seed ^ 0x5eed1ce) >>> 0);

    // beats overlapping the window (+2 s guard each side), from the SAME RR series
    const all = buildRR(tl);
    const beats = all.filter(function (b) {
    ...
    for (let i = 0; i < N; i++) {
      ...
      const bcur = beats[bi],
        bnext = beats[Math.min(bi + 1, beats.length - 1)];
      /* R-CENTER each beat on its TRUE time. The old form measured phase from the beat's ONSET
         (ph = (ms − bcur.tMs)/span) with the R lobe at template phase 0.41, so each R landed at
         bcur.tMs + 0.41·RR and the rendered R-to-R interval became RR + 0.41·ΔRR — a low-pass of the
         true tachogram that attenuated reconstructed beat-to-beat rMSSD ≈26% (a renderer artifact,
         NOT a detector property; surfaced by qrs-yield-analysis.html). Picking the NEAREST beat and
         offsetting phase so template 0.41 sits exactly on nb.tMs places every R at its true instant
         → detected R-to-R == true RR (modulo 130 Hz quantization). */
      const nb = Math.abs(ms - bcur.tMs) <= Math.abs(bnext.tMs - ms) ? bcur : bnext;
      const nbi = nb === bcur ? bi : Math.min(bi + 1, beats.length - 1);
      const prevB = beats[Math.max(0, nbi - 1)];
      const span = Math.max(300, nb.tMs - prevB.tMs) || 900; // local RR (QT/width scaling)
      const ph = Math.max(0, Math.min(1, 0.41 + (ms - nb.tMs) / span)); // template R (0.41) at nb.tMs
      const amp = 1.0;
      const wander = 70 * Math.sin((2 * Math.PI * rel) / 11) + 35 * Math.sin((2 * Math.PI * rel) / 3.7); // resp + drift
      ecg[i] = pqrst(ph, span / 1000) * amp + wander + gaussFrom(rng) * 14;
    }
    const int16 = new Int16Array(N);
    for (let k = 0; k < N; k++) {
      let v = Math.round(ecg[k]);
      if (v > 32767) v = 32767;
      if (v < -32768) v = -32768;
      int16[k] = v;
    }
    ...
    const devRR = [];
    for (let j = 1; j < beats.length; j++) devRR.push({ tsMs: beats[j].tMs, rr: Math.round(beats[j].tMs - beats[j - 1].tMs) });
