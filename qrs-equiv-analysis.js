...
(function () {
  ...
  function bootPool(n) {
    ...
    for (var i = 0; i < n; i++) {
      (function () {
        var w = new Worker('qrs-equiv-worker.js');
        var rec = { w: w, ready: false, err: null };
        readies.push(
  ...
  function bootPdx() {
    return new Promise(function (resolve) {
      ...
      f.src = 'cohort-harness.html?node=pulsedex';
      pdx = { iframe: f, ready: false };
      document.body.appendChild(f);
      var done = false;
      window.addEventListener('message', function (ev) {
        var m = ev.data || {};
        if (m.node !== 'pulsedex') return;
        if (m.type === 'ready') {
          pdx.ready = !m.error;
          pdx.bootErr = m.error || null;
          if (!done) {
            done = true;
            resolve();
          }
  ...
  function ba(aArr, bArr) {
    ...
    for (var i = 0; i < aArr.length; i++) {
      if (aArr[i] != null && bArr[i] != null && isFinite(aArr[i]) && isFinite(bArr[i])) {
        d.push(aArr[i] - bArr[i]);
        m.push((aArr[i] + bArr[i]) / 2);
        a.push(aArr[i]);
        b.push(bArr[i]);
      }
    }
    ...
    var bias = mean(d),
      s = sd(d),
      mb = mean(b);
    return { n: d.length, bias: bias, sd: s, loLoA: bias - 1.96 * s, hiLoA: bias + 1.96 * s, biasPct: mb ? (100 * bias) / mb : null, r: pearson(a, b), d: d, m: m };
  }
  ...
  async function run() {
    ...
    setStatus('booting FULL-lane + PulseDex realms…', 'run');
    setProg(0);
    ...
    if (
      ...
      setStatus('realm boot failed' + (pdx.bootErr ? ' (pulsedex: ' + pdx.bootErr + ')' : ''), 'idle');
      $('run').disabled = false;
      return;
    }
    ...
    var raw = await runWorkers(seeds, function (f) {
      setProg(f * 0.6);
    });
    ...
    for (var i = 0; i < raw.length; i++) {
      ...
      if (i % 4 === 0) {
        setStatus('PulseDex ' + (i + 1) + '/' + raw.length, 'run');
  ...
  function analyze() {
    ...
    /* §∅ — THREE CASES WORE ONE NUMBER. The old `: 0` was taken when the ECG−Pulse pair was absent,
       when the PPG−Pulse pair was absent, or when the optical SD genuinely did not exceed the
       electrical floor. Only the third is a measurement. The first two are absences, and publishing
       0 for them negates this paper's central claim by reporting "the optical arm carries no PAT
       jitter" (row 2026-09-16-patsd-zero-is-a-fallback). Observed, not hypothesised: a run at
       nSubj 240 emitted patJitterSdMs 0 with only ONE of the three pairs present, because both
       inputs need the `pulse` arm. Absent input ⇒ null; a measured non-excess is still 0. */
    var patSD = !baEP || !baPP ? null : baPP.sd > baEP.sd ? Math.sqrt(baPP.sd * baPP.sd - baEP.sd * baEP.sd) : 0;
    ...
    cards += hcard(
      COL.ecg,
      ms(baEP && baEP.bias) + '<span class="hu"> ms</span>',
      'ECGDex − PulseDex bias',
      'LoA ' + (baEP ? baEP.loLoA.toFixed(1) + '…' + baEP.hiLoA.toFixed(1) + ' ms' : '—') + ' · sampling + QRS detect'
    );
    cards += hcard(
      COL.ppg,
      ms(baPP && baPP.bias) + '<span class="hu"> ms</span>',
      'PpgDex − PulseDex bias',
      'LoA ' + (baPP ? baPP.loLoA.toFixed(1) + '…' + baPP.hiLoA.toFixed(1) + ' ms' : '—') + ' · + PAT jitter'
    );
    cards += hcard(COL.line, patSD == null ? '—' : '±' + patSD.toFixed(1) + '<span class="hu"> ms</span>', 'PAT-jitter dispersion', 'optical-only SD = √(SD²ₚₚ − SD²ₑₚ)');
    $('headline').innerHTML = cards;

    // table
    var rows = [
      ['ECGDex − PulseDex', baEP],
      ['PpgDex − PulseDex', baPP],
      ['PpgDex − ECGDex', baPE]
    ];
    var tb = '';
    rows.forEach(function (r) {
      var b = r[1];
      if (!b) {
        tb += '<tr><td>' + r[0] + '</td><td colspan="6" class="muted">—</td></tr>';
        return;
      }
      tb +=
        '<tr><td>' +
        ...
        '</td>' +
        '<td class="num">' +
        (b.biasPct != null ? (b.biasPct >= 0 ? '+' : '') + b.biasPct.toFixed(1) + '%' : '—') +
        '</td>' +
        '<td class="num">' +
        b.sd.toFixed(2) +
        '</td>' +
    ...
    RESULT = {
      ...
      nWindows: WIN.length,
      framing:
        'Three-way rMSSD equivalence on co-generated beats. One ~9-min apnea-cluster window per patient, scored by the REAL PulseDex (RR text reference), ECGDex (raw int16 ECG @130 Hz, R rendered at true beat times), and PpgDex (optical pulse @176 Hz) on the SAME beats. Pairwise Bland–Altman: ECG−Pulse = sampling + QRS detection; PPG−Pulse = + PAT jitter (+ optical detection); PAT-jitter SD = quadrature excess of PPG−Pulse over ECG−Pulse. SYNTHETIC GROUND TRUTH — certifies detector agreement and quantifies intrinsic dispersion, not a real-patient equivalence study.',
      pairs: {}
    };
    [
      ['ecg_minus_pulse', baEP],
      ['ppg_minus_pulse', baPP],
      ['ppg_minus_ecg', baPE]
    ].forEach(function (p) {
      /* A PAIR THAT WAS NOT COMPUTED IS PRESENT-AND-NULL, NOT ABSENT. Omitting it made a partial
         run read as a complete one: a consumer cannot tell "this pair was not computed" from "this
         tool never emits that pair", which is how a re-cut looked ready with two of three headline
         comparisons missing (same row, sibling defect). */
      if (!p[1]) {
        RESULT.pairs[p[0]] = null;
        return;
      }
      RESULT.pairs[p[0]] = {
        n: p[1].n,
        biasMs: +p[1].bias.toFixed(3),
        biasPct: p[1].biasPct != null ? +p[1].biasPct.toFixed(2) : null,
        sdMs: +p[1].sd.toFixed(3),
        loaLoMs: +p[1].loLoA.toFixed(2),
        loaHiMs: +p[1].hiLoA.toFixed(2),
        pearson: p[1].r != null ? +p[1].r.toFixed(4) : null
      };
    });
    RESULT.patJitterSdMs = patSD == null ? null : +patSD.toFixed(3);
    try {
      window.QRS_EQUIV = RESULT;
    } catch (e) {}
  ...
  function hcard(c, hl, k, s) {
    return '<div class="hcard"><div class="hl" style="color:' + c + '">' + hl + '</div><div class="hk">' + k + '</div><div class="hs">' + s + '</div></div>';
  }
  ...
  function axes(ctx, P, w, h, xmin, xmax, ymin, ymax, xt, yt, xlab, ylab, xfmt, yfmt) {
    ...
    ctx.font = '11px ui-monospace,monospace';
    ctx.fillText(xlab, w / 2 - 60, h - 5);
    ctx.save();
    ctx.translate(13, h / 2 + 50);
  ...
  function ptStyle() {
    var n = WIN.length;
    if (n <= 3000) return { r: 3.0, a: 0.6 };
    if (n <= 15000) return { r: 1.6, a: 0.34 };
  ...
  function drawScatter(pulse, ecg, ppg) {
    ...
    for (var i = 0; i < pulse.length; i++) {
      [pulse[i], ecg[i], ppg[i]].forEach(function (v) {
    ...
    ctx.fillText('identity', X(hi) - 70, Y(hi) + 14);
    var st = ptStyle();
    for (var k = 0; k < pulse.length; k++) {
      if (pulse[k] == null) continue;
      if (ecg[k] != null && isFinite(ecg[k])) {
        ctx.fillStyle = hexA(COL.ecg, st.a);
        dot(ctx, X(pulse[k]), Y(ecg[k]), st);
      }
      if (ppg[k] != null && isFinite(ppg[k])) {
        ctx.fillStyle = hexA(COL.ppg, st.a);
        dot(ctx, X(pulse[k]), Y(ppg[k]), st);
      }
    }
    $('scatterLeg').innerHTML =
      '<span class="chip" style="background:' +
      COL.ecg +
      '"></span>ECGDex &nbsp; <span class="chip" style="background:' +
      COL.ppg +
      '"></span>PpgDex &nbsp;·&nbsp; vs PulseDex reference (x). On-identity = agreement.';
  }
  ...
  function drawBA(id, b, title, color) {
    ...
    var X = function (v) {
      return P + (w - P - 14) * Math.max(0, Math.min(1, v / xhi));
    };
    var Y = function (v) {
      return h - P - (h - P - 12) * Math.max(0, Math.min(1, (v - ylo) / (yhi - ylo)));
    };
    // zero, bias, LoA lines
    function hline(val, style, dash, label) {
      ctx.strokeStyle = style;
      ctx.setLineDash(dash || []);
      ctx.beginPath();
      ctx.moveTo(P, Y(val));
      ctx.lineTo(w - 14, Y(val));
      ctx.stroke();
      ctx.setLineDash([]);
      if (label) {
        ctx.fillStyle = style;
        ctx.font = '10px ui-monospace';
        ctx.fillText(label, w - 150, Y(val) - 4);
      }
    }
    ...
    for (var i = 0; i < b.d.length; i++) {
      ctx.fillStyle = hexA(color, stB.a - 0.05 > 0.08 ? stB.a - 0.05 : 0.08);
      dot(ctx, X(b.m[i]), Y(b.d[i]), stB);
    }
  }

  // ── exports ──
  function dl(name, text, mime) {
    var bl = new Blob([text], { type: mime || 'text/plain' });
