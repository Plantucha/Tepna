...
function evBadge(label, fallback){
  try { return (window.EcgRegistry && window.EcgRegistry.badgeForLabel(label, fallback!==false)) || ''; }
  catch(e){ return ''; }
}
...
class ECGScope {
  ...
  setData(rec){
    ...
    this.pyr = {};
    for (const f of factors){ this.pyr[f] = f === 1 ? null : buildEnvelope(rec.int16, f); }
    ...
    let lo = 32767, hi = -32768, step = Math.max(1, Math.floor(this.N/40000));
    for (let i = 0; i < this.N; i += step){ const v = rec.int16[i]; if (v<lo) lo=v; if (v>hi) hi=v; }
    const pad = (hi-lo)*0.08 || 100; this.yLo = lo-pad; this.yHi = hi+pad;
  ...
  _pickLevel(samplesPerPx){
    ...
    let best = 1;
    for (const f of this.factors){ if (f <= samplesPerPx) best = f; }
    return best;
  }
  ...
  draw(){
    ...
    const padL = 4*dpr, padR = 4*dpr, padT = 6*dpr, padB = 18*dpr;
    ...
    const secStart = start/this.fs, secSpan = span/this.fs;
    let tick = secSpan/8 > 1 ? Math.ceil(secSpan/8) : (secSpan/8 > 0.2 ? 0.5 : 0.2);
    if (secSpan > 120) tick = Math.ceil(secSpan/8/60)*60;
    ctx.fillStyle = C.dim; ctx.font = (10*dpr)+'px IBM Plex Mono, monospace'; ctx.textAlign='center';
    ...
    ctx.strokeStyle = C.teal; ctx.lineWidth = 1*dpr;
    ctx.beginPath();
    if (f === 1){
      // direct polyline
      ...
      const b0 = Math.max(0, Math.floor(start/f)), b1 = Math.min(env.mins.length-1, Math.ceil((start+span)/f));
      for (let b = b0; b <= b1; b++){
    ...
    if (span < 40*this.fs && this.peakSamp.length){
      for (let k = 0; k < this.peakSamp.length; k++){
        ...
        const good = !this.sqi.length || this.sqi[k] >= 0.30;
        const typ = this.types ? this.types[k] : 'N';
        let col = good ? C.amber : C.red;
        if (typ === 'V') col = C.purple;            // PVC
        else if (typ === 'S') col = C.blue;         // PAC
        const yTop = sy(this.data.int16[Math.round(ps)]||0)-6*dpr;
        ctx.fillStyle = col;
        ctx.beginPath(); ctx.arc(x, yTop, (typ==='N'?2.4:3.2)*dpr, 0, 2*Math.PI); ctx.fill();
        if (typ !== 'N'){                            // label ectopic beats
          ctx.fillStyle = col; ctx.font = (9*dpr)+'px IBM Plex Mono, monospace'; ctx.textAlign='center';
  ...
  drawMini(){
    ...
    ctx.strokeStyle = 'rgba(61,224,208,.45)'; ctx.lineWidth = 1;
    ctx.beginPath();
  ...
  zoom(factor, centerFrac){
    ...
    this.draw(); this.drawMini(); this._emit();
  }
  pan(dSamples){
    let start = this.view.start + dSamples;
    start = Math.max(0, Math.min(this.N-this.view.span, start));
    this.view.start = start; this.draw(); this.drawMini(); this._emit();
  }
  ...
  _emit(){ if (this.onView) this.onView(this.view, this.fs, this.N); }
  ...
  _bindEvents(){
    ...
    if (this.mini){
      const jump = e => { const r = this.mini.getBoundingClientRect(); const frac = (e.clientX-r.left)/r.width;
        let start = Math.max(0, Math.min(this.N-this.view.span, frac*this.N - this.view.span/2));
        this.view.start = start; this.draw(); this.drawMini(); this._emit(); };
      let md = false;
      this.mini.addEventListener('pointerdown', e => { md = true; jump(e); });
      this.mini.addEventListener('pointermove', e => { if (md) jump(e); });
      window.addEventListener('pointerup', () => md = false);
    }
    window.addEventListener('resize', () => { if (this.data){ this.resize(); this.draw(); this.drawMini(); } });
  }
}
...
function lineChart(pts, color, opts){
  opts = opts||{};
  const W=opts.W||680, H=opts.H||150, P={l:46,r:14,t:14,b:24}, n=pts.length;
  if(!n) return '';
...
function poincare(nn, sd1v, sd2v){
  const W=320,H=300,P=34; const n=nn.length;
  if(n<3) return '';
  const xs=[],ys=[]; for(let i=1;i<n;i++){ xs.push(nn[i-1]); ys.push(nn[i]); }
  let mn=Math.min(arrMinL(xs),arrMinL(ys)), mx=Math.max(arrMaxL(xs),arrMaxL(ys));
  ...
  let dots=''; for(let i=0;i<xs.length;i+=stepP){ dots+=`<circle cx="${sc(xs[i]).toFixed(1)}" cy="${scY(ys[i]).toFixed(1)}" r="1.1" fill="${C.teal}" opacity=".5"/>`; }
  // identity line + ellipse (rotated 45°)
  const cx=sc(m), cy=scY(m);
  const ex=sd2v/(mx-mn)*(W-2*P), ey=sd1v/(mx-mn)*(H-2*P);
...
function hypnogram(stages){
  const W=680,H=160,P={l:54,r:14,t:14,b:24}; const n=stages.length;
  if(!n) return '';
  const lvls=['Deep','Light','REM','Wake']; const yOf={Deep:0,Light:1,REM:2,Wake:3};
  const colOf={Deep:C.blue,Light:C.teal,REM:C.purple,Wake:C.amber};
  const xmx=stages[n-1].tMin+5, xmn=0;
  const sx=x=>P.l+(x-xmn)/(xmx-xmn)*(W-P.l-P.r);
  const sy=l=>H-P.b-(l/3)*(H-P.t-P.b);
  let path='', segs='';
  for(let i=0;i<n;i++){
    const x0=sx(stages[i].tMin), x1=sx(i<n-1?stages[i+1].tMin:xmx), y=sy(yOf[stages[i].stage]);
  ...
  return `<svg viewBox="0 0 ${W} ${H}" role="img" style="width:100%;height:auto">
    ...
    ${segs}
    ${[0,0.25,0.5,0.75,1].map(f=>{const x=xmn+f*(xmx-xmn);return `<text x="${sx(x).toFixed(1)}" y="${H-7}" fill="${C.dim}" font-size="9" text-anchor="middle" font-family="IBM Plex Mono,monospace">${(x/60).toFixed(1)}h</text>`;}).join('')}
  </svg>`;
}

// small array helpers (avoid spread on huge arrays)
function arrMinL(a){let m=Infinity;for(let i=0;i<a.length;i++)if(a[i]<m)m=a[i];return m;}
function arrMaxL(a){let m=-Infinity;for(let i=0;i<a.length;i++)if(a[i]>m)m=a[i];return m;}
function meanL(a){let s=0;for(let i=0;i<a.length;i++)s+=a[i];return s/a.length;}

// median-beat with delineation markers (P-QRS-T intervals)
function medianBeatChart(mb, del){
  const W=680,H=260,P={l:48,r:16,t:18,b:30};
  const beat=mb.beat, L=beat.length, fs=mb.fs;
  let ymn=Infinity,ymx=-Infinity; for(const v of beat){ if(v<ymn)ymn=v; if(v>ymx)ymx=v; }
  const pad=(ymx-ymn)*0.10||50; ymn-=pad; ymx+=pad;
  ...
  const m=del.marks, B=m.B;
  const vline=(i,col,lbl,dash)=>`<line x1="${sx(i).toFixed(1)}" y1="${P.t}" x2="${sx(i).toFixed(1)}" y2="${H-P.b}" stroke="${col}" stroke-width="1" ${dash?'stroke-dasharray="3 3"':''} opacity=".6"/><text x="${sx(i).toFixed(1)}" y="${P.t-5}" fill="${col}" font-size="9" text-anchor="middle" font-family="IBM Plex Mono,monospace">${lbl}</text>`;
  ...
  const span=(i0,i1,col,lbl,yoff)=>{ const y=H-P.b+yoff; return `<line x1="${sx(i0).toFixed(1)}" y1="${y}" x2="${sx(i1).toFixed(1)}" y2="${y}" stroke="${col}" stroke-width="2"/><line x1="${sx(i0).toFixed(1)}" y1="${y-3}" x2="${sx(i0).toFixed(1)}" y2="${y+3}" stroke="${col}"/><line x1="${sx(i1).toFixed(1)}" y1="${y-3}" x2="${sx(i1).toFixed(1)}" y2="${y+3}" stroke="${col}"/><text x="${((sx(i0)+sx(i1))/2).toFixed(1)}" y="${y-4}" fill="${col}" font-size="8.5" text-anchor="middle" font-family="IBM Plex Mono,monospace">${lbl}</text>`; };
  return `<svg viewBox="0 0 ${W} ${H}" role="img" style="width:100%;height:auto">
