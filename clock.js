  ...
  function hostAxis(anchors, opts) {
    ...
    return {
      ...
      drawnReason: deviceDrawn
        ? 'device axis is DRAWN — ' + (drawnShare * 100).toFixed(2) + ' % of inter-sample deltas share one value (≥ ' + (CK_AXIS_DRAWN_SHARE * 100).toFixed(0) + ' %); this column is a synthesised counter, not a clock, so there is no second timebase to discipline against'
        : null,
