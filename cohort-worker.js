...
var KIND = null, READY = false;

function kern() { return (self.DexKernel ? { hash: DexKernel.HASH, version: DexKernel.VERSION } : null); }
...
function runOxy(p) {
  ...
  p.nights.forEach(function (nt) {
    ...
    score.nights.push({
      ...
      estAHI: firstFinite(night.ahiEst), minSpo2: st.minSpo2, durationMin: st.durationMin,
...
function runPulse(nt) {
  ...
  var rm = rmssd(clean), sd = std(clean);
...
function winHypoFromCSV(csv, cadenceMin) {
  ...
  for (var i = 1; i < lines.length; i++) {            // row 0 is the header
    ...
    var c = ln.indexOf(','); if (c < 0) continue;
    var v = parseFloat(ln.slice(c + 1));              // Measurement(mg/dL) is the 2nd column
    if (isFinite(v) && v < 70) { run++; if (run === minCells) episodes++; }
    else run = 0;
  }
  return episodes;
}
function runGluco(cgmCSV) {
  ...
  return {
    envelope: {
      ...
      kernel: kern(), ganglior_events: r.events || [],
    },
...
function parseHRVrows(csv) {
  ...
  var iR = hdr.indexOf('rMSSD'), iS = hdr.indexOf('SDNN'), out = [];
  for (var i = 1; i < lines.length; i++) { var c = lines[i].split(','); if (c.length < 5) continue; out.push({ rmssd: parseFloat(c[iR]), sdnn: parseFloat(c[iS]) }); }
  return out;
}
...
function runCpap(night) {
  ...
  return {
    envelope: env,
    score: { n: night.n, residualAHI: m.residualAHI, ahiTruth: night.cfg.ahi, therapyHours: nightObj.therapyHours,
      medianLeak: m.medianLeak, largeLeakPct: m.largeLeakPct, centralIndex: m.centralIndex,
      medianPressure: m.medianPressure, t0Ms: nightObj.t0Ms, nEvents: (env.ganglior_events || []).length },
  };
}
...
function splitCgmByDay(csv) {
  ...
  Object.keys(byDay).sort().forEach(function (day) {
    var rows = byDay[day]; if (rows.length < 200) return;   // need ~16h of 5-min readings for a stable daily CV
...
function runPPGfull(tl) {
  ...
  return {
    envelope: {
      schema: { name: 'ganglior.node-export', node: 'PpgDex' }, bus: 'ganglior',
    ...
    score: { rmssd: r.rmssd, sdnn: r.sdnn, nPulses: r.nPulses, analyzablePct: r.analyzablePct,
      meanSQI: r.meanSQI, motionRejectedPct: r.motionRejectedPct, t0Ms: r.t0Ms, winStartRel: win.startRel },
  };
}
function runECGfull(tl) {
  ...
  var rrVal = null; try { rrVal = ECGDSP.validateRR(r.nn, rec.deviceRR); } catch (e) {}
  return {
    envelope: {
    ...
    score: { rmssd: r.rmssd, sdnn: r.sdnn, lfhf: r.lfhf, nBeats: r.nBeats, meanSQI: r.meanSQI,
      analyzablePct: r.analyzablePct, cvhrIndex: r.cvhr ? r.cvhr.index : null,
      nBeatsTrue: rec.deviceRR.length, rrMismatchPct: rrVal ? rrVal.dRMSSD : null,
      detVsTrue: rec.deviceRR.length ? +(r.nBeats / rec.deviceRR.length).toFixed(3) : null,
      t0Ms: r.t0Ms, winStartRel: win.startRel },
  };
}
...
function doJob(seed) {
  ...
  if (KIND === 'full') {
    ...
    if (ecgNight) { var te = performance.now(); try { ecg = runECGfull(ecgNight.tl); } catch (e) { errs.ECGDex = String(e && e.message || e); } t.ecg = +(performance.now() - te).toFixed(2); }
    if (ppgNight) { var tpp = performance.now(); try { ppg = runPPGfull(ppgNight.tl); } catch (e) { errs.PpgDex = String(e && e.message || e); } t.ppg = +(performance.now() - tpp).toFixed(2); }
  ...
  if (KIND === 'iccpg') {
    ...
    t.gen = +(performance.now() - g0).toFixed(2);
    var pulseI = [], tpi = performance.now();
    pi.nights.forEach(function (nt) {
      if (!(nt.present.PulseDex && nt.files.rrText)) return;
      var r = runPulse(nt); if (r) pulseI.push(r);
    });
    t.pulse = +(performance.now() - tpi).toFixed(2);
    var perDay = [], tgi = performance.now();
    if (pi.files.glucoCSV) {
      splitCgmByDay(pi.files.glucoCSV).forEach(function (d) {
        try { var rg = runGluco(d.csv); if (rg && rg.score && rg.score.cv != null && isFinite(rg.score.cv)) perDay.push({ day: d.day, cv: rg.score.cv, mean: rg.score.mean }); } catch (e) {}
      });
    }
  ...
  if (KIND === 'cgmcouple') {
    ...
    t.gen = +(performance.now() - g0).toFixed(2);
    var gtByN = {}; ((pc.groundTruth && pc.groundTruth.nights) || []).forEach(function (gn) { gtByN[gn.n] = gn; });
    var pulseC = [], nocturnal = [], tcz = performance.now();
    pc.nights.forEach(function (nt) {
      if (!(nt.present.PulseDex && nt.files.rrText)) return;
      var rp = runPulse(nt); if (!rp) return;
      var gn = gtByN[nt.n]; if (!gn || gn.t0Ms == null) { pulseC.push(rp); return; }
  ...
  if (KIND === 'oxy') {
    ...
    t.gen = +(performance.now() - g0).toFixed(2);
    var pulse = [], tp = performance.now();
    p2.nights.forEach(function (nt) {
      if (!(nt.present.PulseDex && nt.files.rrText)) return;
      var r = runPulse(nt); if (r) pulse.push(r);
    });
    t.pulse = +(performance.now() - tp).toFixed(2);
    var gluco = null;
    if (p2.files.glucoCSV) { var tg = performance.now(); try { gluco = runGluco(p2.files.glucoCSV); } catch (e) { gluco = { error: String(e && e.message || e) }; } t.gluco = +(performance.now() - tg).toFixed(2); }
    var hrv = [];
    if (p2.files.hrvCSV) {
      ...
      p2.nights.forEach(function (nt, i) {
        ...
        hrv.push({
          n: nt.n, rmssd: row.rmssd, sdnn: row.sdnn, target: nt.cfg.rmssd, t0Ms: nt.groundTruth.t0Ms, durMin: +(nt.cfg.durSec / 60).toFixed(1),
        });
      });
    }
    // CPAPDex: only nights the patient is actually on therapy (cfg.cpap)
    var cpap = [], tc = performance.now();
    p2.nights.forEach(function (nt) {
      if (!nt.cfg.cpap) return;
      nt._profile = p2.profile; nt._seed = seed;
      try { var rc = runCpap(nt); if (rc) cpap.push(rc); } catch (e) { cpap.push({ error: String(e && e.message || e), n: nt.n }); }
    });
...
self.onmessage = function (e) {
  ...
  if (m.type === 'init') {
    KIND = m.kind;
    try { importScripts.apply(self, SCRIPTS[KIND]); READY = true; self.postMessage({ type: 'ready', kind: KIND }); }
    catch (err) { self.postMessage({ type: 'ready', kind: KIND, err: String(err && err.message || err) }); }
    return;
  }
  if (m.type === 'job') {
    if (!READY) { self.postMessage({ type: 'done', reqId: m.reqId, kind: KIND, error: 'not ready' }); return; }
    var t0 = performance.now();
    try { var res = doJob(m.seed >>> 0); self.postMessage({ type: 'done', reqId: m.reqId, kind: KIND, result: res, wallMs: +(performance.now() - t0).toFixed(2) }); }
    catch (err) { self.postMessage({ type: 'done', reqId: m.reqId, kind: KIND, error: String(err && err.message || err), wallMs: +(performance.now() - t0).toFixed(2) }); }
  }
};
