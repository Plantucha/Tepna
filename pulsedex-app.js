...
// ─── MOBILE NAV DRAWER ─────────────────────────────────────────────────────────
(function(){
  const tgl=document.getElementById('navToggle'), sb=document.querySelector('.app-shell > .sidebar'), bd=document.getElementById('navBackdrop');
  if(!tgl||!sb) return;
  const setOpen=(open)=>{
...
}, { rootMargin: '-18% 0px -72% 0px', threshold: [0, .25, .5, 1] });
...
// drag & drop on zone
rawZone.addEventListener('dragover', e => { e.preventDefault(); rawZone.classList.add('drag'); });
rawZone.addEventListener('dragleave', () => rawZone.classList.remove('drag'));
rawZone.addEventListener('drop', e => {
  e.preventDefault(); rawZone.classList.remove('drag');
...
let activeKey = null, _recSeq = 0, _curFname = null;
wtInput.addEventListener('change', (e) => {
  ...
  fr.onload = ev => {
    ...
    if (lastResult) { renderWTTable(lastResult); reRender(); }   // refresh KPI deltas, Recov Index, SDNN-Z, hero trend chips
  };
  fr.readAsText(f);
});
...
(function(){
  ...
  cmpInput.addEventListener('change', (e) => {
    ...
    f.text().then(txt => {
      ...
      if (!p.vals || p.vals.length < 5) { st.textContent = '⚠ Could not read ≥5 intervals from that file.'; return; }
      const kind = _pdKind(p.sourceFormat, f.name);
      cmpData = { vals:p.vals, tsMs:p.tsMs, t0Ms:p.t0Ms, kind, label:f.name };
      st.textContent = '✅ ' + p.vals.length.toLocaleString() + ' ' + (kind==='ppi'?'PPI':'RR') + ' intervals · ' + f.name
        + (lastResult ? '' : ' — load/calculate a recording to compare');
...
function _cmpTile(label, val, unit, sub, col){
  return '<div style="background:var(--surface2);border:1px solid rgba(255,255,255,.06);border-radius:12px;padding:13px 14px">'
    + '<div class="cmp-tile-l" style="font-size:11px;letter-spacing:.04em;text-transform:uppercase;color:var(--text3);margin-bottom:6px">'+(typeof evBadge==='function'?evBadge(label):'')+label+'</div>'
    + '<div style="font-family:\'IBM Plex Mono\',monospace;font-size:24px;font-weight:700;color:'+(col||'var(--text)')+';line-height:1">'+val
    + (unit?'<span style="font-size:12px;color:var(--text3);font-weight:500"> '+unit+'</span>':'')+'</div>'
...
function _pdBASVG(pairs, bias, loLo, loHi){
  ...
  const W=680,H=210,P={l:54,r:80,t:14,b:30};
  let xmn=Infinity,xmx=-Infinity,ymn=Infinity,ymx=-Infinity;
  for(const pr of pairs){ const x=pr[0],y=pr[1]; if(x<xmn)xmn=x;if(x>xmx)xmx=x;if(y<ymn)ymn=y;if(y>ymx)ymx=y; }
  ymn=Math.min(ymn,loLo); ymx=Math.max(ymx,loHi);
  if(xmx===xmn)xmx=xmn+1; if(ymx===ymn)ymx=ymn+1;
  const padY=(ymx-ymn)*0.14; ymn-=padY; ymx+=padY;
  ...
  const hl=(v,col,dash,lab)=>'<line x1="'+P.l+'" y1="'+sy(v).toFixed(1)+'" x2="'+(W-P.r)+'" y2="'+sy(v).toFixed(1)+'" stroke="'+col+'" stroke-dasharray="'+dash+'" opacity=".75"/>'
    +'<text x="'+(W-P.r+5)+'" y="'+(sy(v)+3).toFixed(1)+'" fill="'+col+'" font-size="9" font-family="IBM Plex Mono,monospace">'+lab+'</text>';
  return '<svg viewBox="0 0 '+W+' '+H+'" preserveAspectRatio="none" style="width:100%;height:auto" role="img">'
    ...
    +'<text x="'+(P.l-6)+'" y="'+(sy(ymx)+9).toFixed(1)+'" fill="#6F8096" font-size="9" text-anchor="end" font-family="IBM Plex Mono,monospace">'+ymx.toFixed(0)+'</text>'
    +'<text x="'+(P.l-6)+'" y="'+(sy(ymn)).toFixed(1)+'" fill="#6F8096" font-size="9" text-anchor="end" font-family="IBM Plex Mono,monospace">'+ymn.toFixed(0)+'</text>'
    +'<text x="'+((P.l+W-P.r)/2).toFixed(0)+'" y="'+(H-6)+'" fill="#6F8096" font-size="9" text-anchor="middle" font-family="IBM Plex Mono,monospace">mean of the two paired intervals (ms)</text>'
    +'<text x="14" y="'+((P.t+H-P.b)/2).toFixed(0)+'" fill="#6F8096" font-size="9" text-anchor="middle" font-family="IBM Plex Mono,monospace" transform="rotate(-90 14 '+((P.t+H-P.b)/2).toFixed(0)+')">ref − primary (ms)</text>'
    +'</svg>';
}
function renderComparison(){
  const card = document.getElementById('cmpCard'), title = document.getElementById('slCmp');
  ...
  if (res.error){ card.innerHTML = '<div style="background:var(--surface);border:1px solid rgba(255,255,255,.06);border-radius:16px;padding:16px;color:var(--text2)">'+res.error+'</div>'; return; }
  ...
  if (res.weak){
    card.innerHTML = '<div style="background:var(--surface);border:1px solid rgba(255,255,255,.06);border-radius:16px;padding:16px;color:var(--text2);line-height:1.6">'
      + '<b>'+res.matched+'</b> beats lined up ('+res.matchRatePct+'%). '+(res.note||'')
      + '</div>';
    return;
  }

  const A=res.statsA, B=res.statsB, ag=res.agreement, dc=res.discrepancy;
  const rows=[['Mean interval','meanRR','ms'],['Rate','hr','bpm'],['SDNN','sdnn','ms'],['rMSSD','rmssd','ms'],['pNN50','pnn50','%']];
  const tbl = '<table style="width:100%;border-collapse:collapse;font-size:13px">'
    ...
    + '<th style="padding:8px 10px;font-weight:600;max-width:170px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap">'+priLab+'</th>'
    + '<th style="padding:8px 10px;font-weight:600;max-width:170px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap">'+refLab+'</th>'
    + '<th style="padding:8px 10px;font-weight:600">Δ (ref−pri)</th></tr></thead><tbody>'
    + rows.map(r=>{
        const a=A[r[1]], b=B[r[1]], d=+(b-a).toFixed(1);
        const dcol = Math.abs(d) < (r[1]==='meanRR'?12:r[1]==='hr'?2:r[1]==='pnn50'?5:6) ? 'var(--green)' : 'var(--amber)';
        return '<tr style="border-top:1px solid rgba(255,255,255,.05);font-family:\'IBM Plex Mono\',monospace;text-align:right">'
          + '<td style="text-align:left;padding:8px 10px;font-family:inherit;color:var(--text2)">'+r[0]+'<span style="opacity:.5"> '+r[2]+'</span></td>'
          + '<td style="padding:8px 10px;color:var(--text)">'+a+'</td>'
          + '<td style="padding:8px 10px;color:var(--text)">'+b+'</td>'
          + '<td style="padding:8px 10px;color:'+dcol+'">'+(d>0?'+':'')+d+'</td></tr>';
      }).join('')
    + '</tbody></table>';

  const agTiles = '<div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(150px,1fr));gap:10px;margin-top:14px">'
  ...
  const discTiles = '<div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(150px,1fr));gap:10px;margin-top:10px">'
  ...
  card.innerHTML =
    ...
    + '<div style="font-size:12px;color:var(--text2);margin-bottom:12px;line-height:1.5"><b>'+res.matched.toLocaleString()+'</b> beats matched ('+res.matchRatePct+'%) · '+clk+'.</div>'
    + tbl
    ...
    + agTiles
    + '<div style="margin-top:14px">'+_pdBASVG(res.blandAltman, ag.biasMs, ag.loaLoMs, ag.loaHiMs)+'</div>'
    ...
    + discTiles
    + '<div style="font-size:12px;color:var(--text2);margin-top:12px;line-height:1.55">'+dc.note+'</div>'
    + '</div>';
}
...
(function(){ var _b=document.getElementById('btnSumCSV'); if(_b) _b.addEventListener('click', exportSummaryCSV); })();
...
function genSyntheticPatient(){
  ...
  if(!r) return;
  try{ showChip(DexPatientGen.chip(r)); }catch(_){}
...
function calculate() {
  ...
  if (parsed.t0Ms==null) {
    ...
    if (fnT0!=null) { parsed.t0Ms = fnT0; _t0Source = 'filename'; }
    else { _t0Source = 'none'; console.warn('PulseDex: no timestamp in RR content or filename — recording stays undated (excluded from fusion).'); }
  }
  ...
  const a = clean.clean;
  const N = a.length;
  if (N < 10) { showErr('Fewer than 10 usable RR values found.'); return; }
  ...
  if (parsed.tsMs && isFinite(parsed.tsMs[0]) && isFinite(parsed.tsMs[N-1])) {
    const wall=(parsed.tsMs[N-1]-parsed.tsMs[0])/1000; let rrSum=0;
  ...
  const meanRR=mean(a), sdnn=std(a), rm=rmssd(a), pn=pnn50(a), nn50=nn50c(a);
  const hr=+(60000/meanRR).toFixed(1), cv=+(sdnn/meanRR*100).toFixed(2);
  const mx=+mxdmn(a).toFixed(1), mo=modeV(a), amo=+amo50(a,mo).toFixed(1);

  // ── windowed (long recordings) ──
  let win=null, sdann=null, sdnnIdx=null, repSeg=a, winSpec=null;
  let dispRm=rm, dispSd=sdnn, dispHr=hr, dispPn=pn, dispMeanRR=meanRR;
  if (longRec) {
    progress(50,'Windowing '+(durSec/3600).toFixed(1)+' h into 5-min epochs…');
    win = windowAnalysis(a, times, 300);
    if (win.wins.length>=3) {
      const rmA=win.wins.map(w=>w.rmssd), sdA=win.wins.map(w=>w.sdnn),
            hrA=win.wins.map(w=>w.hr), pnA=win.wins.map(w=>w.pnn), rrA=win.wins.map(w=>w.meanRR);
      dispRm=+medianOf(rmA).toFixed(2); dispSd=+medianOf(sdA).toFixed(2);
      dispHr=+medianOf(hrA).toFixed(1); dispPn=+medianOf(pnA).toFixed(1);
      ...
      let bi=0,bd=Infinity;           // representative window = rmssd closest to night median
      for(let i=0;i<rmA.length;i++){const d=Math.abs(rmA[i]-dispRm);if(d<bd){bd=d;bi=i;}}
      repSeg=win.segs[bi];
      // robust spectrum = MEDIAN of per-window Lomb–Scargle (detrended), not one window
      progress(60,'Per-window spectra…');
      const sh=[],sl=[],sv=[],stp=[],srr=[];
      for(const seg of win.segs){ const w=lombScargle(seg, 256); sh.push(w.hf); sl.push(w.lf); sv.push(w.vlf); stp.push(w.tp); if(w.respRate>0)srr.push(w.respRate); }
      winSpec={hf:Math.round(medianOf(sh)), lf:Math.round(medianOf(sl)), vlf:Math.round(medianOf(sv)),
               tp:Math.round(medianOf(stp)), respRate:srr.length?+medianOf(srr).toFixed(1):0};
    }
  }
  ...
  const sp = winSpec ? {tp:winSpec.tp, hf:winSpec.hf, lf:winSpec.lf, vlf:winSpec.vlf}
                     : {tp:ls.tp, hf:ls.hf, lf:ls.lf, vlf:ls.vlf};
  ...
  const cSd=longRec?dispSd:sdnn, cRm=longRec?dispRm:rm, cPn=longRec?dispPn:pn, cHr=longRec?dispHr:hr;
  // representative geometric values for the comparable export line + the (short-term) Baevsky SI
  const gSeg = longRec ? repSeg : a;
  const cMx = +mxdmn(gSeg).toFixed(1), cMo = modeV(gSeg), cAmo = +amo50(gSeg,cMo).toFixed(1);
  const cCv = +(cSd/cMeanRR*100).toFixed(2);
  const expSd1 = +sd1(cRm).toFixed(2), expSd2 = +sd2(cSd,cRm).toFixed(2);
  const stress=stressEst(cSd,cRm), hrv=hrvEst(cSd,cRm,cPn), energy=energyEst(cSd,cRm),
        focus=focusEst(cSd,cRm), coh=cohEst(cRm,cSd);
  const satP=ans.psns>=100, satE=energy>=100;       // ceiling-saturation flags
  const sd1v=+sd1(rm).toFixed(2), sd2v=+sd2(sdnn,rm).toFixed(2);
  const lnrm=+lnR(cRm).toFixed(3);
  const lfhfv=winSpec ? +(winSpec.lf/(winSpec.hf||1)).toFixed(3) : ls.lfhf;
  const hfnu=+nu(sp.hf,sp.hf+sp.lf).toFixed(1), lfnu=+nu(sp.lf,sp.hf+sp.lf).toFixed(1);
  ...
  let hrFloor=null, autoRHR=Math.round(cHr);
  ...
  const sfg=stress-focus, fe=+(focus/(ans.sns+1)).toFixed(3);
  ...
  const dc=prsaCapacity(a,+1), ac=prsaCapacity(a,-1), triIdx=triangularIndex(a);
  ...
  lastResult = {
    t0Ms: parsed.t0Ms, offsetMin: parsed.offsetMin,
    ...
    mode, modeLabel:MODE_LABEL[mode]||mode, modeWhy:cls.why, modeConf:Math.round(cls.conf*100),
    overridden, longRec, durMin:+(durSec/60).toFixed(1),
    coverage, artifactPct:clean.pct, nArtifact:clean.nArt, health,
    nWindows: win?win.wins.length:0, windows: win?win.wins:null, sdann, sdnnIdx,
    N, meanRR:+meanRR.toFixed(1), hr, sdnn:+sdnn.toFixed(2),
    rmssd:+rm.toFixed(2), pnn50:+pn.toFixed(1), nn50,
    dispRm:+dispRm.toFixed(2), dispSd:+dispSd.toFixed(2), dispHr:+dispHr.toFixed(1),
    dispPn:+dispPn.toFixed(1), dispMeanRR:+dispMeanRR.toFixed(1),
    cv, mx, mode_ms:mo, amo50:amo,
    expCv:cCv, expMx:cMx, expMo:cMo, expAmo:cAmo, expSd1, expSd2,
    tp:sp.tp, hf:sp.hf, lf:sp.lf, vlf:sp.vlf,
    stress, hrv, energy, focus, coherence:coh,
    sns:ans.sns, psns:ans.psns, snsBal:ans.snsBal, psnsBal:ans.psnsBal, satP, satE,
    sd1:sd1v, sd2:sd2v, sd1sd2:+(sd1v/(sd2v||1)).toFixed(3),
    ellArea:ell, lnrmssd:lnrm, lfhf:lfhfv, hfnu, lfnu, si,
    vo2base:vo2b, vo2adj:vo2a, vo2gt, hrmaxEff, rhrEff, autoRHR, hrFloor, tanaka, hrmaxRejected,
    elev, altFactor:+altFactor.toFixed(3), pb,
    efc, crs, abs:absV, sfg:+sfg.toFixed(1), fe,
    pnse, otr, rsa,
    lsTP:ls.tp, lsVLF:ls.vlf, lsLF:ls.lf, lsHF:ls.hf, lsLFHF:ls.lfhf, respRate,
    dfa1, sampen, dc, ac, triIdx,
    pip:frag.pip, ials:frag.ials, pss:frag.pss, pas:frag.pas,
    min:arrMin(a), max:arrMax(a),
    q25:quant(a,.25), median:quant(a,.5), q75:quant(a,.75)
  };
  if(_curFname) lastResult.fname=_curFname;
  // keep the cleaned interval series + stamps so a second signal can be beat-matched
  lastResult._series = { vals:a.slice(), tsMs:(parsed.tsMs?parsed.tsMs.slice():null), t0Ms:parsed.t0Ms,
    kind:_pdKind(parsed.sourceFormat, _curFname), label:(_curFname||lastResult.modeLabel||'Loaded signal') };
  ...
  lastResult.contentId = (typeof SignalFrame!=='undefined' && SignalFrame && SignalFrame.computeContentId)
  ...
  progress(100,'Done');
  setTimeout(()=>{document.getElementById('prog').classList.remove('show');document.getElementById('proc').textContent='';},700);
}


// ─── MULTI-DAY: switcher + cross-recording trends (additive; gated at ≥2) ─────
function recordingsSortedPx(){ return Object.values(allRecordings).sort((a,b)=>(a.t0Ms||0)-(b.t0Ms||0)); }
function _pxFmtDT(ms){ if(ms==null) return 'undated'; const d=new Date(ms),p=n=>(n<10?'0':'')+n;
  return d.getUTCFullYear()+'-'+p(d.getUTCMonth()+1)+'-'+p(d.getUTCDate())+' '+p(d.getUTCHours())+':'+p(d.getUTCMinutes()); }
function renderRecSwitcherPx(){
  const wrap=document.getElementById('recSwitcherPx'); if(!wrap) return;
  ...
  wrap.innerHTML='<div class="sec-label" style="margin-top:10px">Recordings · '+list.length+'</div>'+list.map(s=>{
    const k=s._key, active=k===activeKey, q=s.coverage>=90?'ok':s.coverage>=70?'warn':'bad';
    return '<button class="rec-item-px '+(active?'active':'')+'" data-key="'+k+'">'
      +'<div class="ri-top-px"><span class="ri-date-px">'+_pxFmtDT(s.t0Ms)+'</span><span class="ri-q-px '+q+'">'+(s.coverage!=null?s.coverage+'%':s.modeLabel||'')+'</span></div>'
      +'<div class="ri-sub-px">'+(s.modeLabel||'')+' · '+s.dispHr+' bpm · rMSSD '+s.dispRm+'</div></button>';
  }).join('');
...
function selectRecordingPx(key){
  ...
  activeKey=key; lastResult=r;
  renderContext(r); renderOverviewPx(r); renderANS(r); renderGraphs(r); renderTable(r);
  if (welltoryData && welltoryData.rows.length) renderWTTable(r);
  renderRecSwitcherPx(); renderCrossNightPx();
  if(cmpData) renderComparison();
  document.getElementById('sidebarDataInfo').innerHTML =
...
function _pxSpark(vals, color){
  const W=180,H=42,P=4,n=vals.length; if(n<2) return '';
  let mn=Math.min(...vals),mx=Math.max(...vals); if(mx===mn)mx=mn+1;
  const sx=i=>P+i/(n-1)*(W-2*P), sy=v=>H-P-(v-mn)/(mx-mn)*(H-2*P);
  const d=vals.map((v,i)=>(i?'L':'M')+sx(i).toFixed(1)+' '+sy(v).toFixed(1)).join(' ');
  const mean=vals.reduce((s,v)=>s+v,0)/n;
  return '<svg viewBox="0 0 '+W+' '+H+'" preserveAspectRatio="none" style="width:100%;height:42px">'
    +'<line x1="'+P+'" y1="'+sy(mean).toFixed(1)+'" x2="'+(W-P)+'" y2="'+sy(mean).toFixed(1)+'" stroke="'+color+'" stroke-dasharray="3 3" opacity=".4"/>'
    +'<path d="'+d+'" fill="none" stroke="'+color+'" stroke-width="1.6" stroke-linejoin="round"/></svg>';
}
function renderCrossNightPx(){
  const sec=document.getElementById('crossNightSectionPx'); if(!sec) return;
  const list=recordingsSortedPx();
  if(list.length<2 || !window.PulseCross){ sec.style.display='none'; return; }
  ...
  metrics.forEach(m=>{
    ...
    rows+='<tr><td class="cn-metric-px">'+(typeof evBadge==='function'?evBadge(m.label):'')+m.label+'<span style="opacity:.5"> '+m.unit+'</span></td>'
      +'<td style="width:188px">'+_pxSpark(ser.map(p=>p.v),col)+'</td>'
      ...
      +'<td><span class="cn-trend-px '+tCol(st.trendLabel)+'">'+st.trendLabel+'</span></td>'
      +'<td class="mono"><span class="cn-z-px '+zCol+'">'+(st.zLatest==null?'—':(st.zLatest>0?'+':'')+st.zLatest+'σ')+'</span></td></tr>';
  ...
  document.getElementById('crossNightTablePx').innerHTML='<table class="cn-table-px"><thead><tr>'
    +'<th>Metric</th><th>Trend</th><th>Mean</th><th>CV</th><th>Slope</th><th>Mann–Kendall</th><th>Direction</th><th>Latest z</th>'
    +'</tr></thead><tbody>'+rows+'</tbody></table>';
  document.getElementById('crossNightHeadlinePx').innerHTML='<div class="cn-head-label-px">Newest reading vs baseline</div>'
    +(headline.length?headline.slice(0,3).map(h=>'<div class="cn-head-item-px">'+h+'</div>').join('')
      :'<div class="cn-head-item-px dim">No metric is beyond ±1σ of its personal baseline — a consistent stretch.</div>');
  document.getElementById('crossNightNotePx').innerHTML=list.length+' recordings · OLS slope vs date + non-parametric <b>Mann–Kendall</b> (τ, p) · personal-baseline <b>z-scores</b> · coverage-weighted. Same <code>crossNight()</code> engine as PpgDex/ECGDex/OxyDex.';
}
...
function exportCSV(){
  ...
  if(hasBase){
    for(const row of welltoryData.rows){
      lines.push(outHdr.map(h=>{const i=baseHdr.indexOf(h);return csvCell(i>=0&&row[i]!==undefined?row[i]:'');}).join(','));
    }
  }
  dl('\uFEFF'+lines.join('\r\n')+'\r\n',
     hasBase?'welltory_log_updated.csv':'pulsedex_welltory_'+(lastResult.datetime||'unknown').slice(0,10)+'.csv',
     'text/csv');
  showOK(hasBase
...
function exportJSON() {
  ...
  const payload = {
    ...
    schema:{ name:'ganglior.node-export', version:'2.0', node:'PulseDex', nodeVersion:'1.0', multiRecording:true,
...
function _pdClockS(ms){ const d=new Date(ms), p=n=>(n<10?'0':'')+n; return p(d.getUTCHours())+':'+p(d.getUTCMinutes())+':'+p(d.getUTCSeconds()); }
...
function _summaryCSV(){
  ...
  const tier=label=>{ try{ const R=window.PulseRegistry; if(!R||!R.idForLabel) return ''; const id=R.idForLabel(label); return (id&&R.REGISTRY[id]&&R.REGISTRY[id].evidence)||''; }catch(e){ return ''; } };
...
function exportSummaryCSV(){ if(!lastResult) return; dl(_summaryCSV(),exportName({node:'PulseDex',t0Ms:lastResult.t0Ms,kind:'summary',ext:'csv',contentId:lastResult.contentId}),'text/csv'); showOK('Summary CSV exported (tidy: metric · value · unit · range · status · notes).'); }
function copyTable() {
...
function resetAll() {
  ...
  lastResult=null;
  allRecordings={}; activeKey=null; _recSeq=0; _curFname=null;
  cmpData=null;
  { const cs=document.getElementById('cmpStatus'); if(cs) cs.textContent='No reference loaded — load the loaded recording first, then drop the other sensor (e.g. ECGDex RR vs PpgDex PPI)'; }
  { const ci=document.getElementById('cmpInput'); if(ci) ci.value=''; }
  { const ct=document.getElementById('slCmp'); if(ct) ct.style.display='none'; }
  { const cc=document.getElementById('cmpCard'); if(cc){ cc.style.display='none'; cc.innerHTML=''; } }
  { const w=document.getElementById('recSwitcherPx'); if(w){ w.style.display='none'; w.innerHTML=''; } }
  { const c=document.getElementById('crossNightSectionPx'); if(c) c.style.display='none'; }
}
