...
## And a survivor from #1412, surfaced by touching the same function

The diff-scoped mutation gate mutates the FUNCTION a PR touches, so adding one line to
`arrival_quality` re-examined the whole row — and found one live gap in the lattice guard shipped a few
hours earlier:

    bool(stamp_frozen)  →  bool(None)        SURVIVED

`device_axis_is_clock` is `not (quantised or frozen)`, and **the `quantised` half alone carries the
ring** (`_DURATION_S`). Every existing test used the ring, so the frozen term was never the deciding
one and could be deleted without a single assertion noticing. The stream it was actually written for —
Verity `ppi`, frozen **without** being name-quantised — had a test asserting `device_stamp_constant`
and the offset refusal, but never the lattice refusal.

So the guard would have kept working for the ring and silently stopped working for `ppi`. Now asserted,
and the mutant dies.
