...
## The classifier is NOT touched — and there is a second copy of it

`classify`, `classify_mdev`, `_NOISE`, `_NOISE_MDEV`, `_DRIFT`, `_PHASE_ADEV_NAME` and
`_PHASE_MDEV_NAMES` are **byte-identical to main**; zero lines of this diff touch a boundary or a
comparison. The only deletion is `_clean`'s comprehension replaced by the identical `_finite(v)`.

Stated explicitly because that rule has a **second implementation in another lane**:
`ppgdex-dsp.js:2994 classifyAllan` applies the same `sl < threshold` walk over its own `ALLAN_NOISE`
table (`ppgdex-dsp.js:2955`), and `tests/dex-tests.js:921-930` exercises the JS side alone — **nothing
asserts the two tables or their comparison semantics agree.** A tightening of the Python boundary would
silently split the lanes' noise-type names for one slope, and no gate here could see it.

⚠️ **FOLLOW-UP, deliberately not in this PR:** a JS↔Python parity gate over the Allan noise tables, in
the mould of `registry-defs-parity` — same hazard (two sources of one truth), same shape of fix. Both
sides are Node-lane readable, so it needs no bundle. Named here rather than fixed because this is a
`capture-host/`-only work-unit, and because naming it makes a later divergence attributable rather than
mysterious. (Raised by Papers; the JS twin and the absence of a gate were verified here.)

This is the same species as the bug this PR fixes, one layer up: a rule implemented twice, tested once,
with no assertion that the two agree.
