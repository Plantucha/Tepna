...
(function () {
  ...
  function bootPools(K) {
    return Promise.all([bootKind(oxyPool, 'oxy', K), bootKind(pulsePool, 'pulse', K)]);
  }
  ...
  function idbOpen() {
    return new Promise(function (res, rej) {
      var r = indexedDB.open('txresp_ckpt', 1);
  ...
  function lockFresh() {
    try {
      var l = JSON.parse(localStorage.getItem('txresp_lock') || 'null');
  ...
  function lockBeat() {
    try {
      localStorage.setItem('txresp_lock', JSON.stringify({ id: RUN_ID, ts: Date.now() }));
    } catch (e) {}
  }
  function lockRelease() {
    try {
      var l = lockFresh();
      if (l && l.id === RUN_ID) localStorage.removeItem('txresp_lock');
    } catch (e) {}
  }
  function fmtETA(sec) {
  ...
  function std(a) {
    return a.length > 1 ? Math.sqrt(sse(a) / (a.length - 1)) : 0;
  }
  function zscore(a) {
    var m = mean(a),
      s = std(a) || 1;
  ...
  function measureFromResults(seed, oxyRes, pulseRes) {
    ...
    for (var i = 0; i < N; i++) {
      var sc = on[i];
      if (!sc || sc.odi == null || !isFinite(sc.odi)) return null;
      odi.push(sc.odi);
    }
    ...
    var pairs = pn
      .map(function (pp) {
        var ps = pp.score || pp;
        return { n: pp.n != null ? pp.n : ps.n, v: ps.rmssd };
      })
      .filter(function (o) {
        return o.v != null && isFinite(o.v);
      })
      .sort(function (a, b) {
        return a.n - b.n;
      });
    if (pairs.length !== N) return null;
    var rmssd = pairs.map(function (o) {
      return o.v;
    });
    return { seed: seed, nNights: N, arc: prof.arc, cp: prof.interventionNight, baseAHI: prof.baseAHI, age: prof.age, odi: odi, rmssd: rmssd };
  }
  ...
  async function run(resumeCk) {
    ...
    setStatus('booting ' + K + '× OxyDex + ' + K + '× PulseDex realms…', 'run');
    setProg(0);
    if (!oxyPool.length || !pulsePool.length) await bootPools(K);
    var oxyRdy = oxyPool.filter(function (r) {
        return r.ready;
      }),
      pulseRdy = pulsePool.filter(function (r) {
        return r.ready;
      });
    if (!oxyRdy.length || !pulseRdy.length) {
    ...
    while ((txSeeds.length < targetSubj || flatSeeds.length < targetSubj) && seed < CAP) {
      ...
      if (pf) {
        if (pf.arc === 'intervention' && pf.nNights >= minNights && pf.interventionNight >= 2 && pf.interventionNight <= pf.nNights - 2 && txSeeds.length < targetSubj) txSeeds.push(seed);
    ...
    var all = [],
      _mx = Math.max(txSeeds.length, flatSeeds.length);
    ...
    var K2 = Math.min(oxyRdy.length, pulseRdy.length),
      lanes = [];
    ...
    else ckptSave({ ver: ver, targetSubj: targetSubj, minNights: minNights, cursor: cursor, TX: TX, FLAT: FLAT, savedAt: Date.now() });
    try {
      localStorage.setItem('txresp_ptPerSec', String(pdone / Math.max(0.001, elapsed)));
    } catch (e) {}
    ...
    ['dlCsv', 'dlStats', 'dlFig'].forEach(function (id) {
      $(id).disabled = !TX.length && !FLAT.length;
    });
  }
  ...
  function analyze() {
    ...
    DET.forEach(function (d) {
      ...
      loc[d] = {
        n: n,
        exactPct: n ? (100 * exact) / n : null,
        within1Pct: n ? (100 * w1) / n : null,
        medAbsErr: errs.length ? (errs.length % 2 ? errs[errs.length >> 1] : (errs[errs.length / 2 - 1] + errs[errs.length / 2]) / 2) : null
      };
      var posR2 = TX.map(function (p) {
        return p.det[d] ? p.det[d].r2 : null;
      }).filter(function (v) {
        return v != null;
      });
      var negR2 = FLAT.map(function (p) {
        return p.det[d] ? p.det[d].r2 : null;
      }).filter(function (v) {
        return v != null;
      });
    ...
    cards += hcard(
      DMETA.fused.color,
      loc.fused.within1Pct != null ? loc.fused.within1Pct.toFixed(0) + '%' : '—',
      'Fused · CPAP-night localized within ±1 night',
      'exact ' + (loc.fused.exactPct != null ? loc.fused.exactPct.toFixed(0) + '%' : '—')
    );
    cards += hcard(
      DMETA.fused.color,
      det.fused.auc != null ? det.fused.auc.toFixed(2) : '—',
      'Fused · detection AUC (intervention vs flat)',
      'step-R² ' + fmt(det.fused.txMedR2) + ' vs ' + fmt(det.fused.flatMedR2)
    );
    ...
    DET.forEach(function (d) {
      tb +=
        ...
        '</td>' +
        '<td class="num">' +
        pct(loc[d].within1Pct) +
        '</td>' +
        '<td class="num">' +
        (loc[d].medAbsErr != null ? loc[d].medAbsErr.toFixed(1) : '—') +
        '</td>' +
        '<td class="num">' +
        (det[d].auc != null ? det[d].auc.toFixed(2) : '—') +
        '</td>' +
        '<td class="num">' +
        fmt(det[d].txMedR2) +
        '</td>' +
    ...
    RESULT = {
      ...
      minNights: minNights,
      nIntervention: TX.length,
      nFlatControl: FLAT.length,
      framing:
        'Intervention-arc synthetic patients (planted CPAP-start night = first treated night, 0-based index; restricted to ≥2 pre- and ≥2 post-treatment nights). Per-night ODI-4 (OxyDex) and rMSSD (PulseDex) measured by the REAL detectors. Single change-point by minimum within-segment SSE; FUSED = z-scored ODI-4 (sign-flipped) + rMSSD averaged. Localization scored vs the planted night; detection = step-R² AUC vs flat-arc controls. Synthetic ground truth — certifies pipeline + change-point recovery, not real-world CPAP-response detection.',
      localization: loc,
      detection: det
    };
    try {
      window.TREATMENT_RESPONSE = RESULT;
    } catch (e) {}
  ...
  function drawExample() {
    ...
    var p =
      TX.find(function (q) {
        return q.det.fused && Math.abs(q.det.fused.k - q.cp) <= 1 && q.nNights >= 7;
      }) || TX[0];
    if (!p) {
    ...
    var N = p.nNights;
    var odiMax = Math.max.apply(null, p.odi) * 1.15 + 1,
      rmMin = Math.min.apply(null, p.rmssd) - 3,
      rmMax = Math.max.apply(null, p.rmssd) + 3;
    ...
    ctx.stroke();
    var X = function (i) {
      return P + (w - 2 * P) * (N <= 1 ? 0.5 : i / (N - 1));
    };
    var Yo = function (v) {
      return h - P - (h - P - 14) * (v / odiMax);
    };
    var Yr = function (v) {
      return h - P - (h - P - 14) * ((v - rmMin) / (rmMax - rmMin));
    };
    // planted CP band (between cp-1 and cp)
    var cx = (X(p.cp - 1) + X(p.cp)) / 2;
    ctx.fillStyle = 'rgba(57,217,138,.10)';
    ctx.fillRect(cx, 14, w - P - cx, h - P - 14);
    ctx.strokeStyle = 'rgba(57,217,138,.7)';
    ctx.setLineDash([5, 4]);
    ctx.beginPath();
    ctx.moveTo(cx, 14);
    ctx.lineTo(cx, h - P);
    ...
    ctx.fillText('planted CPAP start', cx + 6, 26);
    // detected fused CP
    if (p.det.fused) {
      var dx = (X(p.det.fused.k - 1) + X(p.det.fused.k)) / 2;
      ctx.strokeStyle = 'rgba(61,224,208,.9)';
      ctx.setLineDash([2, 3]);
      ctx.beginPath();
      ctx.moveTo(dx, 14);
      ctx.lineTo(dx, h - P);
      ...
      ctx.fillText('detected', dx + 6, h - P - 8);
    }
    // series
    ...
    line(p.odi, Yo, '#58A6FF');
    line(p.rmssd, Yr, '#FFB84D');
    ctx.fillStyle = '#9fb0c4';
    ctx.font = '11px ui-monospace';
    ctx.fillText('night →', w / 2 - 20, h - P + 20);
    ctx.fillStyle = '#58A6FF';
    ctx.fillText('● ODI-4 (events/h)', P + 4, h - 8);
    ctx.fillStyle = '#FFB84D';
    ctx.fillText('● rMSSD (ms)', P + 150, h - 8);
    $('exampleLeg').innerHTML =
      'Patient ' + p.seed + ' · ' + N + ' nights · planted CPAP-start night ' + p.cp + (p.det.fused ? ' · fused detected night ' + p.det.fused.k + ' (err ' + (p.det.fused.k - p.cp) + ')' : '');
  }
  ...
  function drawAccuracy(loc) {
    ...
    for (var t = 0; t <= 5; t++) {
      ...
      ctx.beginPath();
      ctx.moveTo(gx, top - 6);
      ctx.lineTo(gx, top + rowH * DET.length);
      ctx.stroke();
      ctx.fillStyle = '#6f8096';
      ctx.font = '10px ui-monospace';
      ctx.fillText(t * 20 + '%', gx - 8, top + rowH * DET.length + 30);
    }
    DET.forEach(function (d, i) {
      ...
      ctx.fillText(DMETA[d].label, 12, y + 14);
      // within ±1 (light) then exact (solid) overlaid
      ctx.fillStyle = 'rgba(255,255,255,.07)';
      rrect(ctx, x0, y, x1 - x0, bh, 4);
      ctx.fill();
      if (L.within1Pct != null) {
        ctx.fillStyle = col + '66';
        rrect(ctx, x0, y, ((x1 - x0) * L.within1Pct) / 100, bh, 4);
        ctx.fill();
      }
      if (L.exactPct != null) {
        ctx.fillStyle = col;
        rrect(ctx, x0, y, ((x1 - x0) * L.exactPct) / 100, bh, 4);
        ctx.fill();
      }
      ctx.fillStyle = '#0b0e13';
      ctx.font = 'bold 11px ui-monospace';
      if (L.exactPct != null) ctx.fillText(L.exactPct.toFixed(0) + '%', x0 + 6, y + 13);
      ctx.fillStyle = '#aab8cc';
      ctx.font = '10px ui-monospace';
      ctx.fillText('±1: ' + pct(L.within1Pct), x0 + 4, y + 32);
    });
    ctx.fillStyle = '#9fb0c4';
    ctx.font = '11px ui-monospace';
    ctx.fillText('CPAP-night localization — solid = exact, light = within ±1 night', 12, top + rowH * DET.length + 14);
  }
  ...
  function drawRoc(det) {
    ...
    ctx.setLineDash([]);
    DET.forEach(function (d) {
      var pos = TX.map(function (p) {
        return p.det[d] ? p.det[d].r2 : null;
      }).filter(function (v) {
        return v != null;
      });
      var neg = FLAT.map(function (p) {
        return p.det[d] ? p.det[d].r2 : null;
      }).filter(function (v) {
        return v != null;
      });
      ...
      grid.forEach(function (th) {
        var tp =
          pos.filter(function (v) {
            return v >= th;
          }).length / pos.length;
        var fp =
          neg.filter(function (v) {
            return v >= th;
          }).length / neg.length;
        pts.push([fp, tp]);
      });
      ctx.strokeStyle = DETA(d);
      ctx.lineWidth = 2;
      ctx.beginPath();
      pts.forEach(function (pt, i) {
        (i ? ctx.lineTo : ctx.moveTo).call(ctx, X(pt[0]), Y(pt[1]));
      });
      ctx.stroke();
    });
    ctx.fillStyle = '#9fb0c4';
    ctx.font = '11px ui-monospace';
    ctx.fillText('false-positive rate (flat controls) →', w / 2 - 90, h - 6);
    ctx.save();
    ctx.translate(13, h / 2 + 60);
    ctx.rotate(-Math.PI / 2);
    ctx.fillText('sensitivity (intervention)', 0, 0);
    ctx.restore();
    $('rocLeg').innerHTML = DET.map(function (d) {
      return '<span class="chip" style="background:' + DETA(d) + '"></span>' + DMETA[d].label + (det[d].auc != null ? ' (AUC ' + det[d].auc.toFixed(2) + ')' : '');
    }).join(' &nbsp; ');
  }
  ...
  $('dlCsv').onclick = function () {
    ...
    TX.concat(FLAT).forEach(function (p) {
      rows.push([
        ...
        p.cp,
        p.baseAHI,
        p.det.odi ? p.det.odi.k : '',
        p.det.rmssd ? p.det.rmssd.k : '',
        p.det.fused ? p.det.fused.k : '',
        p.det.odi ? p.det.odi.r2.toFixed(3) : '',
        p.det.rmssd ? p.det.rmssd.r2.toFixed(3) : '',
        p.det.fused ? p.det.fused.r2.toFixed(3) : ''
      ]);
    });
    dl(
      'treatment-response-results.csv',
      rows
        .map(function (r) {
          return r.join(',');
        })
  ...
  $('dlStats').onclick = function () {
    dl('treatment-response-stats.json', JSON.stringify(RESULT, null, 2), 'application/json');
  };
  $('dlFig').onclick = function () {
    var a = $('example'),
      ...
      c = $('roc'),
      gap = 16;
    var W = Math.max(a.width, b.width + c.width + gap) + gap * 2,
      H = a.height + Math.max(b.height, c.height) + gap * 3;
    ...
    ctx.drawImage(c, gap * 2 + b.width, gap * 2 + a.height);
    out.toBlob(function (bl) {
      var u = URL.createObjectURL(bl);
      var an = document.createElement('a');
      an.href = u;
      an.download = 'treatment-response-figures.png';
      an.click();
    }, 'image/png');
  };
  var CANCEL = false;
  ...
  function updTot() {
    var e = $('totEcho');
    if (!e) return;
    var n = Math.max(15, Math.min(100000, +$('nSubj').value || 45));
    e.textContent = '= ' + n * 2 + ' total (' + n + ' tx + ' + n + ' flat)';
  }
  if ($('nSubj'))
    $('nSubj').addEventListener('input', function () {
      updTot();
      updEta();
    });
  updTot();
  ...
  (function () {
    async function tryResume() {
      try {
        ...
        if (lockHeldByOther()) {
          setStatus('another instance running ' + ck.cursor + '/~' + ck.targetSubj * 2 + '… (watching)', 'run');
