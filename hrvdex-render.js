...
function evBadge(label, fallback){
  try { return (window.HrvRegistry && window.HrvRegistry.badgeForLabel(label, fallback!==false)) || ''; }
  catch(e){ return ''; }
}
...
function setMode(mode, btn){
  ...
  if(window.MetricRegistry){ window.MetricRegistry.setTier(mode); }
  else {
    document.body.dataset.mode = mode;
    document.querySelectorAll('.mode-btn, .nav-mode-btn, .mnav-mode-btn').forEach(function(b){
      b.classList.toggle('active', b.dataset.mode === mode);
    });
    try { localStorage.setItem('hrvdex_dashMode', mode); } catch(e){}
  }
...
function applyChartTiers(){
  document.querySelectorAll('.tab-content .chart-grid').forEach(function(grid){
    ...
    cards.forEach(function(card, i){
      ...
      if(tier === 'core'){ card.removeAttribute('data-tier'); }
      else { card.setAttribute('data-tier', tier); }
...
(function(){
  var saved='core';
  if(window.MetricRegistry){ saved = window.MetricRegistry.getTier(); window.MetricRegistry.applyTier(saved); }
  else { try { saved = localStorage.getItem('hrvdex_dashMode') || 'core'; } catch(e){} document.body.dataset.mode = saved; }
  function syncBtns(){
    document.querySelectorAll('.mode-btn, .nav-mode-btn, .mnav-mode-btn').forEach(function(b){
...
function rerender(){
  ...
  if (typeof renderCardiovascularResearch === 'function') {
    try { renderCardiovascularResearch(rows); } catch(e) { console.warn('Cardiovascular research render failed:', e); }
  }
  renderTable(rows);
}
...
function renderHero(r, prev){
  ...
  let color, tier;
  if(score == null)      { color = 'bad';  tier = 'Upload data to analyze'; }
  else if(score >= 55)   { color = 'good'; tier = 'Primed · Peak readiness'; }
  else if(score >= 45)   { color = 'good'; tier = 'Ready · Train as planned'; }
  else if(score >= 33)   { color = 'warn'; tier = 'Moderate · Keep it easy'; }
  else                   { color = 'bad';  tier = 'Strained · Prioritize rest'; }
...
function renderHrvBench(r){
  ...
  if(!host){ host = document.createElement('div'); host.id = 'heroBenchHrv'; host.className = 'proj-grid'; top.appendChild(host); }
  const num = v => (v != null && !isNaN(v)) ? v : null;
  const rm = num(r._rmssd), sd = num(r._sdnn), hf = num(r.d_hfnu), cai = num(r.d_cai), hr = num(r._hr), si = num(r.d_si);
  const exp = 30;
  ...
  const f = (lbl,sub,val,unit,cls)=>`<div class="proj-factor"><span>${eb(lbl)}${lbl} <span style="opacity:.55">${sub}</span></span><span class="pf-val cv-${cls}">${val!=null?val+' '+unit:'—'}</span></div>`;
...
function renderKPIs(rows){
  ...
  const kpis = [
    { label:'HRV Score', val:(r._hrv != null ? r._hrv.toFixed(1) : undefined), sub:'Today', color: r._hrv > 45 ? 'good' : r._hrv > 30 ? 'warn' : 'bad',
      delta: prev ? (r._hrv - prev._hrv).toFixed(1) : null, unit:'' },
    { label:'SDNN', val:(r._sdnn != null ? r._sdnn.toFixed(1) : undefined), sub:'ms — today', color: r._sdnn > 60 ? 'good' : r._sdnn > 40 ? 'warn' : 'bad',
      delta: prev ? (r._sdnn - prev._sdnn).toFixed(1) : null, unit:'ms' },
    { label:'rMSSD', val:(r._rmssd != null ? r._rmssd.toFixed(1) : undefined), sub:'ms — parasympathetic', color: r._rmssd > 35 ? 'good' : r._rmssd > 20 ? 'warn' : 'bad',
      delta: prev ? (r._rmssd - prev._rmssd).toFixed(1) : null, unit:'ms' },
    { label:'Stress', val:(r._stress != null ? r._stress.toFixed(0) : undefined), sub:'HRV score', color: r._stress < 45 ? 'good' : r._stress < 65 ? 'warn' : 'bad',
      delta: prev ? (r._stress - prev._stress).toFixed(0) : null, unit:'', inverse:true },
    { label:'Energy', val:(r._energy != null ? r._energy.toFixed(0) : undefined), sub:'HRV score', color: r._energy > 55 ? 'good' : r._energy > 35 ? 'warn' : 'bad',
      delta: prev ? (r._energy - prev._energy).toFixed(0) : null, unit:'' },
    ...
    { label:'ANS Load', val:(r.d_ans_load != null ? r.d_ans_load.toFixed(2) : undefined), sub:'composite burden', color: r.d_ans_load < 1 ? 'good' : r.d_ans_load < 2 ? 'warn' : 'bad', delta:null, unit:'' },
    { label:'Recov Index', val:(r.d_ari != null ? r.d_ari.toFixed(2) : undefined), sub:'>1.05 = above baseline', color: r.d_ari > 1.0 ? 'good' : r.d_ari > 0.85 ? 'warn' : 'bad', delta:null, unit:'' },
    { label:'Coherence', val:(r._coherence != null ? r._coherence.toFixed(0) : undefined), sub:'HRV coherence', color: r._coherence > 60 ? 'good' : r._coherence > 30 ? 'warn' : 'bad', delta:null, unit:'' },
    { label:'SDNN Z-score', val:(r.d_sdnn_z != null ? r.d_sdnn_z.toFixed(2) : undefined), sub:'vs 7d baseline', color: r.d_sdnn_z > -0.5 ? 'good' : r.d_sdnn_z > -1.5 ? 'warn' : 'bad', delta:null, unit:'' },
    { label:'SD1/SD2', val:(r.d_sd1_sd2 != null ? r.d_sd1_sd2.toFixed(3) : undefined), sub:'0.25–0.35 typical', color: (r.d_sd1_sd2>0.2&&r.d_sd1_sd2<0.5)?'good':'warn', delta:null, unit:'' },
    ...
    { label:'VO2max Est', val:(r.d_vo2_hrv != null ? r.d_vo2_hrv.toFixed(1) : undefined), sub:'ml/kg/min HRV-adj',
      color: r.d_vo2_hrv >= 45 ? 'good' : r.d_vo2_hrv >= 41 ? 'warn' : 'bad', delta:null, unit:'' },
    { label:'VO2 7d Avg', val:(r.d_vo2_roll7 != null ? r.d_vo2_roll7.toFixed(1) : undefined), sub:'rolling baseline',
      color: r.d_vo2_roll7 >= 43 ? 'good' : r.d_vo2_roll7 >= 40 ? 'warn' : 'bad', delta:null, unit:'' },
    ...
    { label:'Restoration Index', val:(r.d_welfare != null ? r.d_welfare.toFixed(1) : undefined), sub:'E×Coh/Stress', color: r.d_welfare > 40 ? 'good' : r.d_welfare > 20 ? 'warn' : 'bad', delta:null, unit:'' },
    { label:'HRV Momentum', val:(r.d_hrv_momentum != null && !isNaN(r.d_hrv_momentum) ? (r.d_hrv_momentum > 0 ? '+' : '') + r.d_hrv_momentum.toFixed(4) : undefined), sub:'14d ln(rMSSD) slope', color: r.d_hrv_momentum > 0 ? 'good' : r.d_hrv_momentum > -0.01 ? 'warn' : 'bad', delta:null, unit:'' },
    { label:'Recov Debt', val:(r.d_recovery_debt != null ? r.d_recovery_debt.toString() : undefined), sub:'ARI<0.9 days/14d', color: r.d_recovery_debt < 3 ? 'good' : r.d_recovery_debt < 7 ? 'warn' : 'bad', delta:null, unit:'d' },
    { label:'DFA α1', val:(r.d_dfa_proxy != null && !isNaN(r.d_dfa_proxy) ? r.d_dfa_proxy.toFixed(2) : undefined), sub:'fractal scaling proxy', color: r.d_dfa_proxy > 0.8 ? 'good' : r.d_dfa_proxy > 0.7 ? 'warn' : 'bad', delta:null, unit:'' },
    { label:'Spectral Ent', val:(r.d_spectral_ent != null && !isNaN(r.d_spectral_ent) ? r.d_spectral_ent.toFixed(2) : undefined), sub:'regulatory complexity', color: r.d_spectral_ent > 0.7 ? 'good' : r.d_spectral_ent > 0.5 ? 'warn' : 'bad', delta:null, unit:'' },
    { label:'HF n.u.', val:(r.d_hfnu != null && !isNaN(r.d_hfnu) ? r.d_hfnu.toFixed(0) : undefined), sub:'parasympathetic %', color: r.d_hfnu > 35 ? 'good' : r.d_hfnu > 20 ? 'warn' : 'bad', delta:null, unit:'' },
    { label:'PTI', val:(r.d_pti != null && !isNaN(r.d_pti) ? r.d_pti.toFixed(1) : undefined), sub:'PSNS × rMSSD tone', color: r.d_pti > 18 ? 'good' : r.d_pti > 10 ? 'warn' : 'bad', delta:null, unit:'' },
    { label:'Overtrain Risk', val:(r.d_otr_sat ? '500+' : (r.d_otr != null && !isNaN(r.d_otr) ? r.d_otr.toFixed(0) : undefined)), sub:'SNS/PSNS load', color: r.d_otr < 50 ? 'good' : r.d_otr < 150 ? 'warn' : 'bad', delta:null, unit:'', inverse:true },
  ];
...
function renderAlerts(rows){
  ...
  if(r.d_si_flagged) alerts.push({type:'yellow', icon:'📐', msg:`<strong>Baevsky SI surfaced, not trusted</strong> — Mode/MxDMn fell outside the plausible RR range after unit normalization. The Stress Index is shown but may be mis-scaled by the source file.`});
  else if(r.d_si_ms) alerts.push({type:'blue', icon:'📐', msg:`<strong>Baevsky inputs normalized</strong> — Mode/MxDMn arrived in milliseconds and were converted to seconds before the Stress Index (unit-safe; SI/CSI agree with a seconds-unit file).`});
  if(r.d_vlf_pct > 50) alerts.push({type:'yellow', icon:'🌊', msg:`<strong>VLF ${(r.d_vlf_pct != null ? r.d_vlf_pct.toFixed(0) : undefined)}% of total power</strong> — Dominant VLF may indicate slow regulatory stress or measurement artifact.`});
  if(r.d_ari < 0.85) alerts.push({type:'yellow', icon:'📉', msg:`<strong>Recovery Index ${(r.d_ari != null ? r.d_ari.toFixed(2) : undefined)}</strong> — rMSSD significantly below 7-day baseline. Rest day indicated.`});
  if(r.d_sdnn_z < -1.5) alerts.push({type:'yellow', icon:'📊', msg:`<strong>SDNN Z-score ${(r.d_sdnn_z != null ? r.d_sdnn_z.toFixed(2) : undefined)}</strong> — SDNN more than 1.5 SD below personal rolling baseline.`});
  if(r._coherence > 70 && r._energy > 55) alerts.push({type:'green', icon:'✅', msg:`<strong>Optimal Coherence + Energy</strong> — Coherence ${r._coherence}, Energy ${r._energy}. Excellent HRV regulation today.`});
  if(r.d_ari > 1.1) alerts.push({type:'green', icon:'🔋', msg:`<strong>Recovery above baseline</strong> — rMSSD ${(r.d_ari*100-100).toFixed(0)}% above 7-day mean. Good day for higher intensity training.`});
...
/* ===== CHART HELPERS ===== */
const COLORS = {
  blue:'rgba(61,158,255,', green:'rgba(0,232,122,', red:'rgba(255,79,94,',
  yellow:'rgba(245,166,35,', purple:'rgba(167,139,250,', orange:'rgba(251,146,60,',
  teal:'rgba(0,201,177,', cyan:'rgba(34,211,238,'
};
function rgba(name, a){ return (COLORS[name]||'rgba(88,166,255,')+a+')'; }
...
function barChart(id, labels, datasets, opts={}){
  opts.type = 'bar'; mkChart(id, labels, datasets, opts);
}
...
function renderAllCharts(rows, sm){
  ...
  const S = k => smooth(rows.map(r=>r[k]), sm);
  ...
  mkChart('ch_ov_si', labs, [
    { label:'Baevsky SI', data:Sf(r=>r.d_si), borderColor:rgba('orange',1), backgroundColor:rgba('orange',0.15), fill:true }
  ]);
  ...
  mkChart('ch_cv', labs, [
    { label:'CV %', data:Sf(r=>r.d_cv_calc), borderColor:rgba('cyan',1), fill:false }
  ]);
  ...
  mkChart('ch_mxdmn_meanrr', labs, [
    { label:'MxDMn/MeanRR', data:Sf(r=>r.d_mxdmn_meanrr), borderColor:rgba('blue',1), fill:false }
  ]);
  mkChart('ch_se_div', labs, [
    { label:'|Stress−Energy|', data:Sf(r=>r.d_se_div), borderColor:rgba('yellow',1), backgroundColor:rgba('yellow',0.1), fill:true }
  ]);
  mkChart('ch_lnrmssd', labs, [
    { label:'ln(rMSSD)', data:Sf(r=>r.d_lnrmssd), borderColor:rgba('teal',1), backgroundColor:rgba('teal',0.1), fill:true }
  ]);
  ...
  mkChart('ch_spectral_ent', labs, [
    { label:'Spectral Entropy', data:Sf(r=>r.d_spectral_ent), borderColor:rgba('cyan',1), fill:false }
  ]);
  mkChart('ch_lfhf_total', labs, [
    { label:'(LF+HF)/Total', data:Sf(r=>r.d_lfhf_totpow), borderColor:rgba('teal',1), fill:false }
  ]);
  ...
  mkChart('ch_coh_energy', labs, [
    { label:'Coherence×Energy', data:Sf(r=>r.d_coh_energy), borderColor:rgba('green',1), fill:false }
  ]);
  mkChart('ch_pti', labs, [
    { label:'Parasympathetic Tone Index', data:Sf(r=>r.d_pti), borderColor:rgba('teal',1), fill:false }
  ]);
  mkChart('ch_incoherent_stress', labs, [
    { label:'Stress×(1/Coherence)', data:Sf(r=>r.d_incoherent_stress), borderColor:rgba('orange',1), fill:false }
  ]);
  mkChart('ch_vei', labs, [
    { label:'Vagal Efficiency (rMSSD/HR)', data:Sf(r=>r.d_vei), borderColor:rgba('blue',1), fill:false },
    { label:'Optimal 0.45', data:rows.map(()=>0.45), borderColor:rgba('green',0.4), borderDash:[4,2], pointRadius:0 }
  ...
  mkChart('ch_stress_auc', labs, [
    { label:'7-day Stress AUC', data:Sf(r=>r.d_stress_auc), borderColor:rgba('orange',1), backgroundColor:rgba('orange',0.15), fill:true }
  ]);
  mkChart('ch_rmssd_cv', labs, [
    { label:'7d rMSSD CV%', data:Sf(r=>r.d_rmssd_cv7), borderColor:rgba('purple',1), fill:false }
  ]);
  ...
  charts['ch_cvi_csi_scatter'] = new Chart(document.getElementById('ch_cvi_csi_scatter'), {
    type:'scatter',
    data:{ datasets:[{ label:'CVI vs CSI', data:rows.map(r=>({x:r.d_csi,y:r.d_cvi})),
      backgroundColor:rows.map(r=>r.d_abs>0?rgba('green',0.7):rgba('red',0.7)), pointRadius:4 }] },
  ...
  mkChart('ch_crs', labs, [
    { label:'Cardiac Resilience Score', data:Sf(r=>r.d_crs), borderColor:rgba('teal',1), fill:false }
  ]);
  mkChart('ch_abs', labs, [
  ...
  mkChart('ch_sfd', labs, [
    { label:'Stress-Focus Dissociation', data:Sf(r=>r.d_sfd), borderColor:rgba('yellow',1), backgroundColor:rows.map(r=>r.d_sfd>15?rgba('red',0.2):rgba('yellow',0.1)), fill:true },
    { label:'Zero', data:rows.map(()=>0), borderColor:rgba('yellow',0.3), borderDash:[4,2], pointRadius:0 }
  ]);
  mkChart('ch_focus_eff', labs, [
    { label:'Focus Efficiency', data:Sf(r=>r.d_focus_eff), borderColor:rgba('cyan',1), fill:false }
  ]);
  mkChart('ch_pns_eff', labs, [
    { label:'PNS Efficiency', data:Sf(r=>r.d_pns_eff), borderColor:rgba('purple',1), fill:false }
  ]);
  mkChart('ch_otr', labs, [
    { label:'OTR Index (capped 500)', data:Sf(r=>r.d_otr), borderColor:rgba('orange',1), backgroundColor:rows.map(r=>r.d_otr>15?rgba('red',0.2):rgba('orange',0.1)), fill:true,
  ...
  mkChart('ch_vo2_delta', labs, [
    { label:'Delta from ground truth', data:S('d_vo2_delta'),
  ...
  charts['ch_vo2_hr_scatter'] = new Chart(document.getElementById('ch_vo2_hr_scatter'), {
    ...
    options:{ responsive:true, maintainAspectRatio:true,
      plugins:{ legend:{labels:{color:'#6e85a8',font:{size:10}}},
        tooltip:{ callbacks:{ label:ctx=>{ const d=ctx.raw; return (d.label||'')+' HR:'+d.x+' VO2:'+d.y.toFixed(1); }}} },
      scales:{
  ...
  mkChart('ch_vo2_rmssd_mod', labs, [
    { label:'rMSSD modifier (%)', data:rows.map(r=>!isNaN(r.d_vo2_hrv)&&!isNaN(r.d_vo2_base)&&r.d_vo2_base>0?(r.d_vo2_hrv/r.d_vo2_base-1)*100:NaN),
      borderColor:rgba('purple',1),
      backgroundColor:rows.map(r=>{ const pct=!isNaN(r.d_vo2_hrv)&&!isNaN(r.d_vo2_base)&&r.d_vo2_base>0?(r.d_vo2_hrv/r.d_vo2_base-1)*100:0; return pct>=0?rgba('green',0.2):rgba('red',0.2); }),
      fill:true },
  ...
  mkChart('ch_ortho', labs, [
    { label:'Orthostatic Load (HR/SDNN)', data:Sf(r=>r.d_ortho), borderColor:rgba('red',1), fill:false }
  ]);
  mkChart('ch_pnn50_slope', labs, [
    { label:'pNN50 7d slope', data:Sf(r=>r.d_pnn50_slope), borderColor:rgba('cyan',1), backgroundColor:rows.map(r=>r.d_pnn50_slope<0?rgba('red',0.2):rgba('green',0.15)), fill:true },
    { label:'Zero', data:rows.map(()=>0), borderColor:rgba('yellow',0.4), borderDash:[4,2], pointRadius:0 }
  ]);
  // New v2.5 rolling charts
  mkChart('ch_hrv_momentum', labs, [
    { label:'HRV Momentum (14d slope)', data:Sf(r=>r.d_hrv_momentum), borderColor:rgba('teal',1),
      backgroundColor:rows.map(r=>(!isNaN(r.d_hrv_momentum)&&r.d_hrv_momentum>0)?rgba('green',0.15):rgba('red',0.15)), fill:true },
    { label:'Zero', data:rows.map(()=>0), borderColor:rgba('yellow',0.4), borderDash:[4,2], pointRadius:0 }
  ]);
  ...
  barChart('ch_recovery_debt', labs, [
    { label:'Recovery Debt (days)', data:rows.map(r=>r.d_recovery_debt||0),
      backgroundColor:rows.map(r=>(r.d_recovery_debt||0)<3?rgba('green',0.6):(r.d_recovery_debt||0)<7?rgba('yellow',0.6):rgba('red',0.6)),
      borderColor:rows.map(r=>(r.d_recovery_debt||0)<3?rgba('green',1):(r.d_recovery_debt||0)<7?rgba('yellow',1):rgba('red',1)), borderWidth:1 }
  ], {type:'bar'});
  mkChart('ch_rmssd_reactivity', labs, [
    { label:'rMSSD Δ% (daily)', data:Sf(r=>r.d_rmssd_delta_pct),
      borderColor:rgba('orange',1),
      backgroundColor:rows.map(r=>(!isNaN(r.d_rmssd_delta_pct)&&r.d_rmssd_delta_pct>0)?rgba('green',0.15):rgba('red',0.15)), fill:true },
    { label:'Zero', data:rows.map(()=>0), borderColor:rgba('yellow',0.4), borderDash:[4,2], pointRadius:0 }
  ]);
  ...
  barChart('ch_hile', Object.keys(monthly), [
    { label:'High-Stress/Low-Energy Days', data:Object.values(monthly), backgroundColor:rgba('red',0.6), borderColor:rgba('red',1), borderWidth:1 }
  ], { type:'bar' });

  // DISTRIBUTIONS & PATTERNS tab (histograms, weekday rhythm, correlation explorer + matrix)
  if (typeof renderPatterns === 'function') {
    try { renderPatterns(); } catch(e){ console.warn('Patterns render failed:', e); }
  }
}
...
function _patMean(a){ return a.length ? a.reduce((s,v)=>s+v,0)/a.length : NaN; }
function _patMedian(a){ if(!a.length) return NaN; const s=a.slice().sort((x,y)=>x-y); const m=Math.floor(s.length/2); return s.length%2 ? s[m] : (s[m-1]+s[m])/2; }
function _patStd(a){ if(a.length<2) return NaN; const m=_patMean(a); return Math.sqrt(a.reduce((s,v)=>s+(v-m)*(v-m),0)/(a.length-1)); }
function _patVals(rows, key){ return rows.map(r=>r[key]).filter(v=>typeof v==='number' && isFinite(v)); }
function _patLabel(key){ const m=PATTERN_METRICS.find(x=>x.key===key); return m?m.label:key; }
...
function _patPearson(xs, ys){
  ...
  let num=0, dx=0, dy=0;
  for(let i=0;i<n;i++){ const a=px[i]-mx, b=py[i]-my; num+=a*b; dx+=a*a; dy+=b*b; }
...
function renderScatterExplorer(){
  ...
  const cv = document.getElementById('ch_corr'); if(!cv) return;
...
function renderWeekday(){
  ...
  if(sub){
    if(finite.length>=2){
      let hi=-Infinity,lo=Infinity,hiD='',loD='';
      avgs.forEach((v,i)=>{ if(isFinite(v)){ if(v>hi){hi=v;hiD=PATTERN_WEEKDAYS[i];} if(v<lo){lo=v;loD=PATTERN_WEEKDAYS[i];} } });
      sub.innerHTML = 'highest <strong style="color:var(--status-ok)">'+hiD+'</strong> ('+hi.toFixed(1)+') · lowest <strong style="color:var(--status-concern)">'+loD+'</strong> ('+lo.toFixed(1)+')';
    } else sub.textContent = 'not enough data';
  }
  barChart('ch_weekday', PATTERN_WEEKDAYS, [
    { label:'Avg '+_patLabel(key),
      ...
      borderColor:PATTERN_WEEKDAYS.map((_,i)=> i>=5 ? rgba('purple',1) : rgba('blue',1)), borderWidth:1 }
  ], { yAxis:{} });
}
...
function _patHeatColor(r){
  ...
  return r>=0 ? 'rgba(0,201,177,'+(0.10+0.80*a).toFixed(3)+')' : 'rgba(255,79,94,'+(0.10+0.80*a).toFixed(3)+')';
}
function renderHeatmap(){
  ...
  for(let i=0;i<n;i++){
    ...
    for(let j=0;j<n;j++){
      ...
      html += '<td title="'+_patShort(keys[i])+' ↔ '+_patShort(keys[j])+': r='+(isFinite(r)?r.toFixed(2):'n/a')+'" style="background:'+_patHeatColor(r)+';color:'+tc+'">'+txt+'</td>';
    }
    html += '</tr>';
  }
  html += '</tbody></table>';
...
const TABLE_COLS = [
  ...
  { key:'d_bp_risk',  label:'BP Risk', fmt:v=>{ if(!v||v==='—') return '—'; const c=v==='Normal'?'green':v==='Borderline'?'yellow':'red'; return '<span class="pill pill-'+c+'">'+v+'</span>'; } },
...
function renderTable(rows){
  ...
  tbody.innerHTML = displayRows.map(r=>'<tr>'+TABLE_COLS.map(c=>{
    const raw = r[c.key];
...
function renderCardiovascularResearch(rows) {
  ...
  var dates = rows.map(function(r){ return r._date; });

  // ch_htn_pattern (HRV→hypertension-risk) REMOVED 2026-06-23 (DEX-METRIC-REMOVAL-AUDIT 🔴).

  var camqScores = rows.map(computeCAMQ).filter(function(v){return v!=null;});
  var camqDates  = rows.filter(function(r){return computeCAMQ(r)!=null;}).map(function(r){return r._date;});
  drawSimpleLine('ch_camq', camqDates, camqScores, {
    label: 'CAMQ',
    yMin: 0, yMax: 100,
    bands: [{from:60,to:100,color:'rgba(57,217,138,.06)'},
