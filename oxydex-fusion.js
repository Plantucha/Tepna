...
function oxyEcgForNight(n) {
  ...
  if (nw) {
    ...
    var best = null,
      bestScore = -Infinity,
      MAXGAP = 4 * 3600000; // tolerate ≤4 h device-start offset
...
function _oxyStageAt(ecg, minFromStart) {
  ...
  var bin = Math.floor(minFromStart / 5) * 5,
    exact = null,
    nearest = null,
    na = Infinity;
...
function oxyHeroBenchCard(n, ecg) {
  ...
  var _ev = function (l) {
    return typeof evBadge === 'function' ? evBadge(l) : '';
  };
  var row = function (lbl, sub, v, unit) {
    return '<div class="opc-row"><span>' + _ev(lbl) + lbl + ' <span style="opacity:.55">' + sub + '</span></span><span class="v">' + (v != null ? '→ ' + v + ' ' + unit : '—') + '</span></div>';
  };
  var rows = '';
  ...
  var st = n.stats || {};
  var kpi = function (lbl, val, cls) {
    return '<div class="opc-kpi opc-' + cls + '"><div class="opc-kpi-v">' + val + '</div><div class="opc-kpi-l">' + _ev(lbl) + lbl + '</div></div>';
  };
  var kpis = '';
  ...
  return (
    ...
    '<div class="opc-main"><div class="opc-val" style="color:' +
    vc +
    '">' +
    headVal +
    '</div><div class="opc-unit">' +
    headUnit +
    ' · ' +
    _ev(headLbl) +
    headLbl +
...
function oxyComputeFusion(n, ecg) {
  ...
  var s = n.stats || {},
    hrv = n.hrv || {};
  var rec = ecg.recording || {},
    t = (ecg.hrv && ecg.hrv.time) || {},
    ap = ecg.apnea || null;
  ...
  var _term = Math.exp(-_lambda),
    _cum = 0;
  for (var _k = 0; _k < confirmed; _k++) {
    _cum += _term;
    _term *= _lambda / (_k + 1);
  }
  ...
  var medLat = null,
    ...
    latType = null;
  if (latencies.length) {
    var ls = latencies.slice().sort(function (a, b) {
      return a - b;
    });
    medLat = ls[Math.floor(ls.length / 2)];
    if (medLat >= 5) {
      apneaType = 'obstructive-type';
      latType = 'bad';
    } else if (medLat <= -5) {
      apneaType = 'central-type';
      latType = 'warn';
    } else {
      apneaType = 'mixed / co-incident';
      latType = 'warn';
    }
  }
  ...
  var stageProxyCtx = sp ? { remProxyPct: sp.remProxyPct, nremDeepProxyPct: sp.nremDeepPct, remProxyMin: sp.remProxyMin, nremDeepMin: sp.nremDeepMin } : null;

  // ── chip 8: cardiorespiratory phase-locking drop during surges ──
  var cr = ecg.cardiorespiratory || {};
  var plvB = cr.plvBaseline != null ? cr.plvBaseline : null,
    plvS = cr.plvDuringSurges != null ? cr.plvDuringSurges : null;

  // ── HR cross-check ──
  var oxyMeanHr = s.meanHr,
    ecgMeanHr = t.hr != null ? t.hr : null;
  var hrDelta = oxyMeanHr != null && ecgMeanHr != null ? +(oxyMeanHr - ecgMeanHr).toFixed(1) : null;
  var oxyFloor = hrv.hrFloor,
    ecgFloor = (ecg.personalization && (ecg.personalization.restingHRNocturnalFloor || ecg.personalization.restingHR)) || null;

  // ── resp-rate instability (metric 6) ──
  var rstat = (ecg.hrv && ecg.hrv.frequency && ecg.hrv.frequency.respRateEpochStats) || (ecg.hrv && ecg.hrv.respRateEpochStats) || null;
  ...
  return {
    ...
    pSpurious: +pSpurious.toFixed(3),
    expectedConfirmed: +_lambda.toFixed(2),
...
function oxyEcgFusionSection(n, ecg) {
  ...
  var belowChance = F.belowChance,
    pSpurious = F.pSpurious,
    expectedConfirmed = F.expectedConfirmed;
  ...
  var stageAgg = F.stageAgg,
    stageProxyCtx = F.stageProxyCtx,
    plvB = F.plvB,
    plvS = F.plvS;
  var oxyMeanHr = F.oxyMeanHr,
    ecgMeanHr = F.ecgMeanHr,
    hrDelta = F.hrDelta,
    oxyFloor = F.oxyFloor,
    ecgFloor = F.ecgFloor;
  var rstat = F.rstat;
  ...
  var tile = function (val, cls, lbl, sub) {
    return (
      '<div class="efz-tile"><div class="efz-val ' + (cls || '') + '">' + val + '</div>' + '<div class="efz-lbl">' + lbl + '</div>' + (sub ? '<div class="efz-sub">' + sub + '</div>' : '') + '</div>'
    );
  };

  var html =
    '<div class="sec-label sec-label-lg" id="sec-ecgfusion">⚡ ECG Cross-Confirmation ' +
    '<span style="color:var(--text3);font-weight:var(--fw-regular);font-size:13px">· paired ECGDex export (' +
    (_oxyEcgDate(rec.startEpochMs) || 'ECG') +
    ')</span></div>';
  html += '<div class="ecg-fusion">';
  // chip 8 — CRC-PLV drop during surges (Cardiorespiratory coupling breakdown)
  if (plvB != null && plvS != null) {
    var plvDrop = +(plvB - plvS).toFixed(3),
      plvOk = plvS < plvB;
    html +=
      '<div class="efz-chiprow"><span class="efz-chip ' +
      (plvOk ? 'ok' : '') +
      '">' +
      (plvOk ? '✓' : '•') +
      ' CRC-PLV ' +
      ...
      plvS.toFixed(3) +
      '</b>' +
      (plvOk ? ' (−' + plvDrop.toFixed(3) + ')' : '') +
      '</span><span class="efz-chip-note">' +
      (plvOk ? 'cardiorespiratory coupling weakens during autonomic surges — consistent with arousal-driven events' : 'coupling held through surges — surges look non-respiratory') +
      (cr.crcPLV != null ? ' · whole-night CRC-PLV ' + cr.crcPLV : '') +
      '</span></div>';
  }
  ...
  if (hrDelta != null) {
    ...
    html += tile(
      (hrDelta > 0 ? '+' : '') + hrDelta + '<span style="font-size:13px;color:var(--text3)"> bpm</span>',
      hCls,
      'Pulse vs ECG HR',
      'OxyDex ' + oxyMeanHr + ' · ECG ' + ecgMeanHr + ' bpm mean'
    );
  }
  // real HRV vs proxy
  if (t.rmssd != null) {
    html += tile(t.rmssd + '<span style="font-size:13px;color:var(--text3)"> ms</span>', 'blue', 'Real rMSSD (ECG)', 'OxyDex carries only a pulse-rate proxy');
  }
  // metric 1 — desat→arousal latency (apnea typing)
  if (medLat != null) {
    html += tile(
      (medLat > 0 ? '+' : '') + Math.round(medLat) + '<span style="font-size:13px;color:var(--text3)"> s</span>',
      latType,
      'Desat→arousal latency',
      apneaType + ' · ' + latencies.length + ' paired event' + (latencies.length > 1 ? 's' : '') + ' (median surge−nadir)'
    );
  } else if (desN) {
  ...
  if (stageKeys.length) {
    ...
    stageKeys.sort(function (a, b) {
      var ia = STAGE_ORDER.indexOf(a),
        ib = STAGE_ORDER.indexOf(b);
    ...
    stageKeys.forEach(function (k) {
      ...
      bars +=
        '<div class="efz-stagerow' +
        un +
        '"><span class="efz-stagename">' +
        nm +
        '</span>' +
    ...
    if (stageProxyCtx && stageProxyCtx.remProxyPct != null) {
      ctx =
        '<div class="efz-stagectx">OxyDex stage proxy (oximetry-derived, whole-night): REM ≈ ' +
        stageProxyCtx.remProxyPct +
        '% · deep ≈ ' +
        (stageProxyCtx.nremDeepProxyPct != null ? stageProxyCtx.nremDeepProxyPct : '—') +
    ...
    html +=
      '<div class="efz-stagewrap"><div class="efz-stagehd">Sleep-stage context</div>' +
  ...
  // narrative
  var story = 'The fusion layer (Ganglior) correlates ECGDex <code>autonomic_surge</code> events against ' + 'OxyDex desaturations on the shared timeline — ';
      ...
      confirmed > 0
        ? belowChance
          ? '<b style="color:var(--text)">' +
            ...
            '</b> desaturation' +
            (confirmed > 1 ? 's' : '') +
            ' fell within the directional window of an ECG surge, ' +
            'but that is at or below what chance alone would produce here (expected \u2248' +
            expectedConfirmed +
            '), so it is <b>not asserted</b> as confirmed apnea.'
          : '<b style="color:var(--text)">' +
            confirmed +
            ' of ' +
            desN +
            '</b> desaturations line up with a directionally-consistent ECG surge (\u2212' +
            GATE.leadMaxSec +
            's\u2026+' +
            GATE.trailMaxSec +
            's), ' +
  ...
  if (apneaType)
    story += ' The desat→arousal latency (<b style="color:var(--text)">' + (medLat > 0 ? '+' : '') + Math.round(medLat) + ' s</b>) reads <b style="color:var(--text)">' + apneaType + '</b>.';
  ...
  if (rstat && rstat.sd != null) {
    ...
    html +=
      '<div class="efz-note" style="margin-top:8px"><b style="color:var(--text)">Resp-rate instability (ECG-derived):</b> ' +
      'SD ' +
      rstat.sd +
      ' br/min' +
      (rstat.median != null ? ' around ' + rstat.median + ' br/min' : '') +
      ' — ' +
      (unstable ? '<b style="color:var(--amber)">unstable</b>, consistent with periodic breathing' : 'stable') +
      (n.osc && n.osc.episodeCount != null
        ? '. OxyDex flagged ' +
          n.osc.episodeCount +
          ' oscillation window' +
          (n.osc.episodeCount === 1 ? '' : 's') +
          ' — ' +
  ...
  // real-HRV strip
  html +=
    '<div class="efz-note" style="margin-top:8px"><b style="color:var(--text)">Real HRV (from ECG):</b> ' +
    'rMSSD ' +
    (t.rmssd != null ? t.rmssd : '—') +
    ' ms · SDNN ' +
    (t.sdnn != null ? t.sdnn : '—') +
    ' ms · ' +
    'mean HR ' +
    ...
    '. OxyDex\u2019s own HRV (SD ' +
    (hrv.hrSdnn != null ? hrv.hrSdnn : '—') +
    ' bpm) is a 1 Hz pulse-rate proxy — use the ECG values for clinical HRV.</div>';
...
var OXY_RESEARCH_GROUPS = [
  ...
  ['Derived Indices & Recording QA', ['mos', 'ahiEst', 'extras', 'dataGaps', 'sbii', 'pred3p', 'desSev', 'ctPrec', 'tIdx', 'period', 'cross', 'spo2Adv', 'hrAdv', 'comp']]
];
...
function buildFullMetricsTable(n) {
  ...
  var s = n.stats || {},
    hrv = n.hrv || {},
    ecg = typeof oxyEcgForNight === 'function' ? oxyEcgForNight(n) : null;
  var rows = []; // [section] or [metric, value, unit, normal, status, notes]
  var sec = function (t) {
    rows.push({ sec: t });
  };
  var r = function (m, v, u, nm, st, note) {
    rows.push({ m: m, v: v, u: u || '', nm: nm || '—', st: st || '', note: note || '' });
  };
  ...
  if (n.odi4) {
    r('ODI-4', _oxyFmt(n.odi4.rate, 1), '/h', '<5', _oxySev(5, 15, n.odi4.rate, true), n.odi4.count + ' events ≥4% drop');
  }
  if (n.odi3) {
    r('ODI-3', _oxyFmt(n.odi3.rate, 1), '/h', '<5', _oxySev(5, 15, n.odi3.rate, true), n.odi3.count + ' events ≥3% drop');
  }
  if (n.hb) {
    r('Hypoxic burden', _oxyFmt(n.hb.total, 1), '%·min', '—', '', 'Area under 90% (Azarbarzin)');
  ...
  if (n.motion) {
    r('Arousal index', _oxyFmt(n.motion.arousalIndex), '/h', '<10', _oxySev(5, 10, n.motion.arousalIndex, true));
  }
  if (n.motSleep) {
    r('Sleep efficiency', _oxyFmt(n.motSleep.sleepEff, 1), '%', '≥85', _oxySev(85, 75, n.motSleep.sleepEff), 'Motion-derived');
  }
  if (n.stab) {
    r('Sleep stability', _oxyFmt(n.stab.score), '/100', '≥80', _oxySev(80, 60, n.stab.score), n.stab.grade || '');
  }
  ...
  // ── ECG-paired (real) ──
  if (ecg) {
    var t = (ecg.hrv && ecg.hrv.time) || {},
      aa = (ecg.personalization && ecg.personalization.ansAge) || {},
      ap = ecg.apnea || {};
  ...
  OXY_RESEARCH_GROUPS.forEach(function (g) {
    ...
    g[1].forEach(function (key) {
      ...
      Object.keys(obj).forEach(function (f) {
        ...
        if (typeof v === 'object') {
          try {
            v = Array.isArray(v) ? v.slice(0, 12).join(' / ') : JSON.stringify(v);
          } catch (e) {
            return;
          }
  ...
  rows.forEach(function (row) {
    if (row.sec) {
      ...
      body +=
        ...
        '</td>' +
        '<td>' +
        escHTML(String(row.nm)) +
        '</td>' +
        '<td>' +
        pill(row.st) +
        '</td>' +
        '<td class="fmt-note">' +
        escHTML(String(row.note)) +
        '</td>' +
        '</tr>';
    }
  });
  return (
    '<div class="table-wrap oxy-fulltable"><table><thead><tr>' +
...
function oxyFrontFullTable(n) {
  ...
  var tbl;
  try {
    tbl = buildFullMetricsTable(n);
  } catch (e) {
    return '';
  }
  if (!tbl) return '';
  return (
    '<div class="sec-section" id="sec-fulltable">' +
...
function handleEcgJson(file) {
  ...
  rd.onload = function () {
    var obj;
    try {
      obj = JSON.parse(rd.result);
    } catch (e) {
      setS('⚠ not valid JSON', false);
      return;
    }
    var isEcg = (obj && obj.schema && /ecgdex/i.test(obj.schema.node || '')) || (obj && obj.personalization && obj.personalization.ansAge);
    if (!isEcg) {
      setS('⚠ not an ECGDex export', false);
      return;
    }
    ...
    if (chip) {
      ...
      chip.textContent = '⚡ ECGDex ' + date + ' linked';
    }
    // re-render so the hero card + fusion section pick up the paired ECG
    try {
      if (typeof renderAll === 'function' && typeof allNights !== 'undefined' && Object.keys(allNights).length) renderAll();
    } catch (e) {}
  };
  rd.onerror = function () {
    setS('⚠ could not read file', false);
  };
  rd.readAsText(file);
}
  ...
  function wire() {
    ...
    if (inp && !inp._wired) {
      ...
      inp.addEventListener('change', function () {
        if (this.files && this.files[0]) handleEcgJson(this.files[0]);
        this.value = '';
      });
    }
