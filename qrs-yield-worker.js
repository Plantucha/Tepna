...
var READY = false;
var TOL = 0.12; // beat-match tolerance (s) after PAT lag correction
...
function rmssdFromTimes(timesSec) {
  ...
  var t = timesSec.slice().sort(function (a, b) {
    return a - b;
  });
  var rr = [];
  for (var i = 1; i < t.length; i++) {
    var d = (t[i] - t[i - 1]) * 1000;
    if (d > 200 && d < 3000) rr.push(d);
  }
...
function matchWindow(truth, det, winStartRel, tl) {
  ...
  for (var i = 0; i < truth.t.length; i++) {
    var ti = truth.t[i],
      hit = false;
    for (var k = 0; k < det.t.length; k++) {
      if (Math.abs(det.t[k] - ti - lag) <= TOL) {
        hit = true;
        break;
      }
    }
    var b = truth.apnea[i] ? rec.apnea : rec.clean;
    b.tot++;
    if (hit) b.hit++;
  }
  ...
  var sqiApnea = { sum: 0, n: 0 },
    sqiClean = { sum: 0, n: 0 };
  for (var k = 0; k < det.t.length; k++) {
    var dk = det.t[k],
      m = false;
    for (var i = 0; i < truth.t.length; i++) {
      if (Math.abs(dk - truth.t[i] - lag) <= TOL) {
        m = true;
        break;
      }
    }
...
function ecgWindow(tl) {
  ...
  for (var i = 0; i < rec.deviceRR.length; i++) {
    ...
    tT.push(relWin);
    tA.push(apneaAt(tl, win.startRel + relWin));
    rrTrue.push(b.rr);
  }
  ...
  return {
    ...
    recall: mw.recall,
    prec: mw.prec,
    sqiApnea: mw.sqiApnea,
    sqiClean: mw.sqiClean,
    nTrue: tT.length,
    nDet: det.t.length,
    lagMs: mw.lagMs,
    rmssdTrue: rmssdFromTimes(tT),
    rmssdDet: rmssdFromTimes(det.t),
...
function ppgWindow(tl) {
  ...
  for (var i = 0; i < all.length; i++) {
    ...
    tT.push(relWin);
    tA.push(apneaAt(tl, win.startRel + relWin));
    if (prevMs != null) rrTrue.push(b.tMs - prevMs);
    prevMs = b.tMs;
  }
  var truth = { t: tT, apnea: tA };
  var det = {
    t: (r.beatTimes || []).filter(function (v) {
      return v != null;
    }),
    sqi: (r.sqi || []).slice()
  };
  ...
  var dt = [],
    ds = [];
  for (var k = 0; k < (r.beatTimes || []).length; k++) {
    if (r.beatTimes[k] != null && isFinite(r.beatTimes[k])) {
      dt.push(r.beatTimes[k]);
      ds.push(r.sqi ? r.sqi[k] : null);
    }
  }
  det = { t: dt, sqi: ds };
  var mw = matchWindow(truth, det, win.startRel, tl);
...
function doJob(seed) {
  ...
  return {
    seed: seed,
    age: pf.profile.age,
    sev: pf.profile.osaSeverity,
    baseAHI: pf.profile.baseAHI,
    ecg: ecg,
    ppg: ppg,
    errors: errs
  };
