...
def _iso(s):
    return time.strftime("%H:%M:%S", time.gmtime(s)) + f".{int((s % 1) * 1000):03d}"
...
def write_ppg2w_file(path, samples):
    with open(path, "w") as fh:
        ...
        for host_s, a, b, mo in samples:
            stamp = time.strftime("%Y-%m-%dT%H:%M:%S", time.gmtime(host_s)) + f".{int((host_s % 1) * 1000):03d}"
...
def selftest():
    ...
    def ok(nm, c, d=""):
        ...
        if c:
            ...
            f += 1; print(f"  FAIL {nm}{' — ' + d if d else ''}")
    # back_time: N records land within span_s before arrival, last at arrival
    bt = back_time(_fake_recs(5, 0), 100.0, 1.0)
    ok("back_time spreads N records across the span", len(bt) == 5 and abs(bt[-1][0] - 100.0) < 1e-9)
    ok("back_time earliest is span before arrival", abs(bt[0][0] - 99.0) < 1e-9, str(bt[0][0]))
    ok("back_time empty buffer → []", back_time([], 100.0) == [])
    # locate_artifact: a motion spike after the buzz is detected
    samples = [(t, 2000, 2000, 1) for t in [98.0, 98.5, 99.0, 99.5]] + \
              [(t, 2000, 2000, 40) for t in [100.0, 100.3, 100.6, 100.9]]
    r = locate_artifact(samples, 100.0, 1.0)
    ok("a motion spike after the buzz is detected", r["detected"] is True, str(r))
    ok("ratio reflects the lift", r["motion_ratio"] >= 2, str(r.get("motion_ratio")))
    # CONTROL: flat motion (no artifact) is NOT detected — the detector discriminates
    flat = [(t, 2000, 2000, 1) for t in [98.0, 98.5, 99.0, 99.5, 100.0, 100.5, 100.9]]
    ok("flat motion is NOT called an artifact", locate_artifact(flat, 100.0, 1.0)["detected"] is False)
    # no buzz / empty window → inconclusive, never a false positive
    ok("no buzz → inconclusive (not False)", locate_artifact(samples, None)["detected"] is None)
    ok("empty window → inconclusive", locate_artifact(samples, 200.0, 1.0)["detected"] is None)
    print(f"\n{'%d FAILURE(S)' % f if f else '%d assertions — all green' % p}")
