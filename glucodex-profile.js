...
function getCalib(){ try{ return localStorage.getItem('glucodex_calib')==='true'; }catch(e){ return false; } }
function getProfile(){
...
function loadProfile(){
  if(DP()){
    ...
    set('gluAge',p.age); const sx=$('gluSex'); if(sx) sx.value=p.sex==='F'?'F':'M';
    const dd=$('gluDiab'); if(dd&&man.diabetes) dd.value=man.diabetes;
    const th=$('gluTherapy'); if(th&&man.dxTherapy) th.value=man.dxTherapy;
    set('gluTgtLo',man.glucoseTargetLo); set('gluTgtHi',man.glucoseTargetHi); set('gluA1c',man.a1c);
    const cb=$('gluCalib'); if(cb) cb.checked=getCalib();
    return;
  }
  let s=null; try{ s=JSON.parse(localStorage.getItem('glucodex_profile')||'null'); }catch(e){}
  if(s) KEYS.forEach(k=>{ const el=$(k); if(!el||s[k]==null||s[k]==='') return; if(el.type==='checkbox') el.checked=(s[k]===true||s[k]==='true'); else el.value=s[k]; }); }
function saveProfile(){
  if(DP()){
    ...
    const cb=$('gluCalib'); try{ localStorage.setItem('glucodex_calib', cb&&cb.checked?'true':'false'); }catch(e){}
    return;
  }
  const o={}; KEYS.forEach(k=>{ const el=$(k); if(el) o[k]= el.type==='checkbox'?el.checked:el.value; }); try{ localStorage.setItem('glucodex_profile',JSON.stringify(o)); }catch(e){} }
...
function expectedGMI(diab){ return diab==='none'?5.4:diab==='predm'?5.9:6.8; }
...
function personalize(r){
  ...
  const tgtLo=clamp(p.tgtLo||70, 50, 110), tgtHi=clamp(p.tgtHi||180, 130, 260);
  ...
  let gmiCheck=null;
  if(p.a1c>0){ const d=+(r.gmi-p.a1c).toFixed(1); gmiCheck={ lab:p.a1c, gmi:r.gmi, delta:d, agree:Math.abs(d)<=0.5 }; }
  ...
  if(p.a1c>0){
    ...
    calib = { labEAG:Math.round(labEAG), sensorMean:Math.round(sensorMean), bias,
      applied: !!r.biasOffset, appliedOffset: Math.round(r.biasOffset||0),
  ...
  Object.assign(r, {
    profile:p, tgtLo, tgtHi, onInsulin, hypoGoal,
    stabilityScore: stabilityScore(r),
    gmiCheck, expGMI: expectedGMI(p.diab),
    dataQualityConf, dqLabel, calib
  });
  return r;
}
...
function renderHero(r){
  ...
  const score=r.stabilityScore, p=r.profile;
  let color,tier;
  if(score>=80){color='good';tier='Stable · tight, in-range control';}
  else if(score>=65){color='good';tier='Steady · largely in-range';}
  else if(score>=50){color='warn';tier='Variable · room to smooth excursions';}
  else {color='bad';tier='Unstable · high variability / hypo burden';}
  ...
  if(r.onInsulin) note += `. On ${({basal:'basal insulin',mdi:'MDI',pump:'a pump'}[p.therapy])} — time-below-range <b>${hypo}%</b> ${hypo<=r.hypoGoal?`is within the &lt;${r.hypoGoal}% goal`:`<b style="color:var(--status-concern)">exceeds the &lt;${r.hypoGoal}% goal — review hypo timing with your clinician</b>`}`;
  ...
  if(r.dqLabel!=='high') note += ` <b>Score confidence ${r.dqLabel}</b> (×${r.dataQualityConf}) given ${r.pctActive}% coverage${r.compMin>0?' + compression flagging':''} — read it as indicative.`;
  ...
  if(r.calib&&r.calib.applied) chips+=`<div class="readiness-zone-chip ok" title="trace shifted to match lab-A1c-implied average glucose">🧪 calibrated ${r.calib.appliedOffset>0?'+':''}${window.GluDisp.delta(r.calib.appliedOffset)} ${window.GluDisp.label()}</div>`;
  else if(r.calib&&r.calib.magnitude!=='small') chips+=`<div class="readiness-zone-chip warn" title="enable calibration in your profile to apply">sensor bias ${r.calib.bias>0?'+':''}${window.GluDisp.delta(r.calib.bias)} ${window.GluDisp.label()} vs lab</div>`;
...
function computeHints(r){
  ...
  if(calibRow){ calibRow.style.display = p.a1c>0 ? 'block' : 'none';
    const cs=$('calibState');
    if(cs){ if(r&&r.calib){ cs.textContent = r.calib.applied ? `· applied ${r.calib.appliedOffset>0?'+':''}${window.GluDisp.delta(r.calib.appliedOffset)} ${window.GluDisp.label()} (sensor read ${r.calib.bias>0?'low':'high'})` : `· detected ${r.calib.bias>0?'+':''}${window.GluDisp.delta(r.calib.bias)} ${window.GluDisp.label()} (mean ${window.GluDisp.val(r.calib.sensorMean)} vs lab-implied ${window.GluDisp.val(r.calib.labEAG)})`; }
      else cs.textContent=''; } }
  ...
  if(r){
    ...
    if(r.gmiCheck) set('lbl_gluA1c', r.gmiCheck.agree?'✓ GMI '+r.gmi+'% ≈ lab '+p.a1c+'% (Δ'+r.gmiCheck.delta+')':'⚠ GMI '+r.gmi+'% vs lab '+p.a1c+'% — Δ'+r.gmiCheck.delta+'% (they measure differently)', !r.gmiCheck.agree);
...
function onInput(lblId){ saveProfile(); if(reRenderFn) reRenderFn(); }
...
function init(reRender){
  ...
  if (DP() && DP().renderPanel) {
    _dexPanel = DP().renderPanel({ node:'glucodex', mount:'dexProfilePanel', onChange:function(){
