...
function evBadge(label, fallback) {
  try {
    return (window.PpgRegistry && window.PpgRegistry.badgeForLabel(label, fallback !== false)) || '';
  } catch (e) {
    return '';
  }
}
...
(function (global) {
  ...
  class PPGScope {
    constructor(canvas, mini) {
      ...
      this.data = null;
      this.view = { start: 0, span: 0 };
      this._bindEvents();
    }
    setData(r) {
      this.fs = r.fs;
      this.N = r.disp.length;
      ...
      this.pyr = {};
      for (const f of factors) this.pyr[f] = f === 1 ? null : buildEnvelope(int16, f);
    ...
    _secAt(samp) {
      const i = Math.max(0, Math.min(this.N - 1, Math.round(samp)));
      return this.relSec ? this.relSec[i] : i / this.fs;
    }
    _pickLevel(spp) {
      let best = 1;
    ...
    draw() {
      ...
      const secStart = this._secAt(start),
        secEnd = this._secAt(start + span),
        secSpan = secEnd - secStart || span / this.fs;
      // gridlines
      ...
      if (this.motion) {
        ...
        const dt = this.motion.dt,
          grid = this.motion.grid;
        for (let g = 0; g < grid.length; g++) {
          if (grid[g] > 0.5) {
            const t0 = g * dt,
              t1 = t0 + dt;
            const x0 = padL + ((t0 - secStart) / secSpan) * plotW,
              x1 = padL + ((t1 - secStart) / secSpan) * plotW;
      ...
      const spp = (span / plotW) * dpr,
        f = this._pickLevel(spp);
      ...
      if (span < 45 * this.fs && this.peakSamp.length) {
        for (let k = 0; k < this.peakSamp.length; k++) {
          ...
          ctx.beginPath();
          ctx.arc(x, yTop, 2.6 * dpr, 0, 2 * Math.PI);
          ctx.fill();
          // foot tick
          const fsmp = this.footSamp[k];
          if (fsmp != null && fsmp >= start && fsmp <= start + span) {
            const fx = padL + ((fsmp - start) / span) * plotW;
            ctx.strokeStyle = 'rgba(88,166,255,.6)';
            ctx.lineWidth = 1 * dpr;
            ctx.beginPath();
            ctx.moveTo(fx, sy(this.int16[Math.round(fsmp)] || 0));
            ctx.lineTo(fx, sy(this.int16[Math.round(fsmp)] || 0) + 8 * dpr);
            ctx.stroke();
          }
        }
      }
    ...
    zoom(factor, cf) {
      const c = this.view.start + this.view.span * (cf == null ? 0.5 : cf);
      let span = this.view.span * factor;
      span = Math.max(this.fs * 0.5, Math.min(this.N, span));
      let start = c - span * (cf == null ? 0.5 : cf);
    ...
    _emit() {
      if (this.onView) this.onView(this.view, this.fs, this.N, this._secAt.bind(this));
    }
    _bindEvents() {
  ...
  function poincare(nn, sd1v, sd2v) {
    ...
    let mn = Math.min(arrMin(xs), arrMin(ys)),
      mx = Math.max(arrMax(xs), arrMax(ys));
    const pad = (mx - mn) * 0.06 || 30;
    mn -= pad;
    mx += pad;
    const sc = (v) => P + ((v - mn) / (mx - mn)) * (W - 2 * P),
      scY = (v) => H - P - ((v - mn) / (mx - mn)) * (H - 2 * P);
    const m = meanA(nn);
    const stepP = Math.max(1, Math.floor(xs.length / 2600));
    let dots = '';
    for (let i = 0; i < xs.length; i += stepP) {
      dots += `<circle cx="${sc(xs[i]).toFixed(1)}" cy="${scY(ys[i]).toFixed(1)}" r="1.3" fill="${C.teal}" opacity=".5"/>`;
    }
    const cx = sc(m),
      cy = scY(m);
  ...
  function medianPulseChart(mb, del) {
    const W = 680,
      H = 260,
      P = { l: 30, r: 16, t: 24, b: 34 };
    const beat = del.beat || mb.beat,
      L = beat.length,
      fs = mb.fs;
    let ymn = Infinity,
      ymx = -Infinity;
    ...
    const dot = (i, col, lbl) =>
      i >= 0
        ? `<circle cx="${sx(i).toFixed(1)}" cy="${sy(beat[Math.round(i)]).toFixed(1)}" r="3.4" fill="${col}"/><text x="${sx(i).toFixed(1)}" y="${(sy(beat[Math.round(i)]) - 9).toFixed(1)}" fill="${col}" font-size="9.5" text-anchor="middle" font-family="ui-monospace,monospace">${lbl}</text>`
        : '';
    const vline = (i, col) =>
      i >= 0 ? `<line x1="${sx(i).toFixed(1)}" y1="${P.t}" x2="${sx(i).toFixed(1)}" y2="${H - P.b}" stroke="${col}" stroke-width="1" stroke-dasharray="3 3" opacity=".45"/>` : '';
    const gid = 'mp' + Math.random().toString(36).slice(2, 7);
    // rise-time span bar foot→systolic
    let riseBar = '';
    if (m.footI != null && m.sysI != null) {
      const y = H - P.b + 14;
  ...
  function ledRibbon(series, opts) {
    ...
    const W = opts.W || 680,
      H = opts.H || 120,
      P = { l: 38, r: 14, t: 12, b: 24 };
    const plotW = W - P.l - P.r,
      plotH = H - P.t - P.b;
    let xmn = Infinity,
      xmx = -Infinity;
    for (const s of series) {
      if (s.tMin < xmn) xmn = s.tMin;
      if (s.tMin > xmx) xmx = s.tMin;
    }
    ...
    const bars = series
      .map((s) => {
        ...
        const out = [];
        const seg = (frac, col) => {
          const hh = (frac || 0) * plotH;
          if (hh <= 0.2) return;
          y -= hh;
          out.push(`<rect x="${x0.toFixed(1)}" y="${y.toFixed(1)}" width="${bw.toFixed(1)}" height="${hh.toFixed(1)}" fill="${col}"/>`);
        };
        seg(s.f3, C.green);
        seg(s.f2, C.amber);
        seg(s.f1, C.red); // stack bottom→top: 3/3, 2/3, 1/3
        return out.join('');
      })
      .join('');
    const xt = [];
    for (let i = 0; i <= 4; i++) xt.push(xmn + (i * span) / 4);
