...
// Tiers mirror the registry exactly: rMSSD/SDNN/ln rMSSD validated · Pulse HR/Perfusion/Motion-rej
// measured · Aug. index emerging.
var PPG_DEFS = {
  rmssd:{ good:'up', label:'rMSSD', unit:'ms', evidence:'validated', get:function(s){ return s.rmssd; } },
  sdnn:{ good:'up', label:'SDNN', unit:'ms', evidence:'validated', get:function(s){ return s.sdnn; } },
  lnRMSSD:{ good:'up', label:'ln rMSSD', unit:'', evidence:'validated', get:function(s){ return s.lnRMSSD; } },
  hr:{ good:'down', label:'Pulse HR', unit:'bpm', evidence:'measured', get:function(s){ return s.dispHr; } },
  pi:{ good:'up', label:'Perfusion', unit:'%', evidence:'measured', get:function(s){ return s.perfusionIndex; } },
  ai:{ good:'down', label:'Aug. index', unit:'%', evidence:'emerging', get:function(s){ return s.morph?s.morph.augmentationIndexPct:null; } },
  motionRejected:{ good:'down', label:'Motion-rej', unit:'%', evidence:'measured', get:function(s){ return s.motionRejectedPct; } }
};
function ppgSessionT0(s){ return (s&&s.t0Ms!=null)?s.t0Ms:null; }
function ppgSessionWeight(s){ return Math.max(0.05,((s&&s.analyzablePct)||0)/100); }
function crossNightBlock(list){
  if(global.CrossNightEnvelope){
    return global.CrossNightEnvelope.build({
      node:'PpgDex', nodeVersion:'1.0', unit:'session',
      items:list,
      t0Of: ppgSessionT0,
      ...
      metrics: Object.keys(PPG_DEFS).map(function(id){ var d=PPG_DEFS[id];
  ...
  out.nights=sorted.map(s=>({ t0Ms:s.t0Ms, date:window.PPGDSP?window.PPGDSP.fmtDate(s.t0Ms):null, analyzablePct:s.analyzablePct,
    rmssd:s.rmssd, sdnn:s.sdnn, hr:s.dispHr, pi:s.perfusionIndex }));
  return out;
}
