...
`;
  ...
  function emptyState(ico, title, sub) {
    return '<div class="iv-empty"><div class="iv-ico">' + ico + '</div>' + '<div class="iv-title">' + esc(title) + '</div>' + (sub ? '<div class="iv-sub">' + esc(sub) + '</div>' : '') + '</div>';
  }
  ...
  function provStamp(r) {
    ...
    if (h == null) {
    ...
    } else if (expected != null && h === expected) {
      cls = 'ok';
      lbl = 'kernel ✓';
      ...
      lbl = 'kernel ⚠';
      tip =
        'Built against a DIFFERENT physiology kernel (' +
        (r.kernelVersion || String(h).slice(0, 8)) +
        (expected ? ' vs ' + String(expected).slice(0, 8) : '') +
        ") — thresholds may differ; fusion flags this so drift can't masquerade as agreement.";
    }
  ...
  function mrBadge(evidence, cite) {
    var R = window.MetricRegistry;
    return R && R.badge ? R.badge(evidence, cite) : '';
  }
  ...
  function renderChips(recs, onRemove, onClear) {
    ...
    if (!box) return;
    if (!recs.length) {
      box.innerHTML = '<div class="chips-empty">No nodes loaded yet.</div>';
      return;
    }
    box.innerHTML =
      recs
        .map(function (r, i) {
          ...
          return (
            '<div class="node-chip" data-i="' +
            i +
            '">' +
            dot +
            '<div class="chip-main"><div class="chip-node">' +
            esc(r.node) +
            (r.dateUnknown ? ' <span class="chip-warn">· date unknown</span>' : '') +
            '</div>' +
            '<div class="chip-meta">' +
            esc(r.dateStr || '—') +
            ' · ' +
    ...
    box.querySelectorAll('[data-rm]').forEach(function (b) {
      b.addEventListener('click', function () {
        onRemove(+b.dataset.rm);
      });
    });
    var clr = box.querySelector('.chip-clear');
  ...
  function renderWarnings(warns) {
    ...
    box.innerHTML = warns
      .map(function (w) {
        return '<div class="warn-row">⚠ ' + esc(w) + '</div>';
      })
      .join('');
  }
  ...
  function renderHero(fusion) {
    ...
    if (!fusion || !fusion.nodes.length) {
      ...
      box.innerHTML =
        ...
        '</div>' +
        '<div class="hero-nodes"><span class="hero-nodes-label">Reads</span>' +
        chips +
        '</div>' +
        '<div class="hero-flow">' +
    ...
    var dated = fusion.nodes.filter(function (n) {
      return !n.dateUnknown;
    });
    if (fusion.anyOverlap) {
      ...
      var geom =
        w.intersectionMin > 0 &&
        fusion.nodes.filter(function (n) {
          return !n.dateUnknown;
        }).length > 2
          ? ' · ' + w.intersectionMin.toFixed(0) + ' min all-node'
          : '';
      var excl =
        w.nodesExcluded && w.nodesExcluded.length
      ...
      box.innerHTML =
        ...
        excl +
        '<span class="hero-ev">' +
        mrBadge('measured', "Direct overlap of the recordings' time windows on the shared clock — a counted quantity, not a model.") +
        '</span>' +
        '</div>';
    } else {
      // build the explicit "no overlap" message from the two widest-apart nodes
      var lines = dated.map(function (n) {
        return esc(n.node) + ' (' + (n.window ? D.fmtDayShort(n.window.startMs) + ', ' + D.fmtClock(n.window.startMs) + '–' + D.fmtClock(n.window.endMs) : '?') + ')';
      });
      box.innerHTML =
        ...
        '<div class="hero-sub">' +
        lines.join(' and ') +
        " do not overlap on the shared clock. Showing each node's own events on a common axis — load overlapping nights to enable fusion. No correlations are asserted.</div>" +
        '</div>';
    }
  }
  ...
  function renderKPIs(fusion) {
    ...
    if (a) {
      ...
      var ahiSub = below ? 'below chance · expected ' + (a.nullModel ? a.nullModel.expectedConfirmed : '?') + '/night' : 'events/h · desat⟷surge';
      ...
      if (a.apneaAuthority) {
        ...
        var aaSub = aa.confirmedIndex != null ? 'reference · confirmed ' + aa.confirmedIndex + '/h, +' + aa.residualGap + ' central/non-desat' : aa.node + ' firmware-scored';
        k.push(kpi('Device-scored AHI', aa.ahi, aaSub, aa.ahi > 15 ? 'warn' : aa.ahi > 5 ? 'warn' : 'good', 'device_ahi'));
      }
      var cSub = below ? 'p(spurious)=' + (a.findings[0] ? a.findings[0].pSpurious : '?') + ' · ' + a.overlapHours + ' h' : a.overlapHours + ' h overlap';
    ...
    if (fusion.positional && fusion.positional.available) {
      ...
      k.push(
        kpi(
          'Supine event rate',
          p.supineRate != null ? Math.round(p.supineRate * 100) + '%' : '—',
          p.supine + ' supine · ' + p.nonsupine + ' non-supine',
          p.positional ? 'warn' : 'good',
          'positional_apnea'
        )
      );
    }
  ...
  function kpi(label, val, sub, cls, evKey) {
    ...
    return (
      ...
      (cls || '') +
      '"><div class="kpi-label">' +
      (badge ? '<span class="kpi-ev">' + badge + '</span>' : '') +
      esc(label) +
      '</div>' +
      '<div class="kpi-val">' +
      esc(val) +
  ...
  function renderTimeline(recs, fusion) {
    var box = $('timeline');
    if (!box) return;
    var allDated = recs.filter(function (r) {
      return !r.dateUnknown;
    });
    var undated = recs.filter(function (r) {
      return r.dateUnknown;
    });
    if (!allDated.length) {
      box.innerHTML = emptyState('▦', 'No timeline yet', 'Load 2 or more dated -Dex exports and they line up here on one shared wall-clock — one swimlane per node.');
      return;
    }
    ...
    (fusion.pairs || []).forEach(function (p) {
      if (p.overlap) {
        overlapUids[p.an + '|' + p.aWin.startMs] = 1;
      }
    });
    var dated = allDated;
    var capped = 0;
    if (allDated.length > MAX_LANES) {
      var inOverlap = allDated.filter(function (r) {
        var w = D.recWindow(r);
        return (
          w &&
          (fusion.pairs || []).some(function (p) {
            return p.overlap && ((p.aWin && p.aWin.startMs === w.startMs) || (p.bWin && p.bWin.startMs === w.startMs));
          })
        );
      });
      var rest = allDated
        .filter(function (r) {
          return inOverlap.indexOf(r) < 0;
        })
        .sort(function (a, b) {
          return b.nEvents - a.nEvents;
        });
      dated = inOverlap.concat(rest).slice(0, MAX_LANES);
      capped = allDated.length - dated.length;
      dated.sort(function (a, b) {
        var wa = D.recWindow(a),
          wb = D.recWindow(b);
        return (wa ? wa.startMs : 0) - (wb ? wb.startMs : 0);
      });
    }
    ...
    dated.forEach(function (r) {
      var w = D.recWindow(r);
      if (!w) return;
      if (t0 == null || w.startMs < t0) t0 = w.startMs;
      if (t1 == null || w.endMs > t1) t1 = w.endMs;
    });
    dated.forEach(function (r) {
      r.events.forEach(function (e) {
        if (e.tMs < t0) t0 = e.tMs;
        if (e.tMs > t1) t1 = e.tMs;
      });
    });
    ...
    var g0 = Math.ceil(t0 / gridStep) * gridStep,
      _gi = 0;
    ...
    (fusion.pairs || []).forEach(function (p) {
      if (!p.overlap) return;
      var ox = X(p.overlap.startMs),
        ow = X(p.overlap.endMs) - ox;
    ...
    dated.forEach(function (r, i) {
      laneY[r.uid] = padT + i * (laneH + laneGap);
    });
    (fusion.findings || []).forEach(function (f) {
      ...
      svg +=
        ...
        '" y2="' +
        (H - axisH) +
        '" stroke="rgba(255,107,122,.55)" stroke-width="2" class="tl-band tl-band-vis" data-find="' +
        f.tMs +
        '"/>';
      svg +=
        '<line x1="' +
        ...
        '" y2="' +
        (H - axisH) +
        '" stroke="transparent" stroke-width="14" class="tl-band tl-band-hit" data-find="' +
        f.tMs +
    ...
    dated.forEach(function (r, i) {
      ...
      var w = D.recWindow(r);
      if (w) {
        svg += '<line x1="' + X(w.startMs).toFixed(1) + '" y1="' + cy + '" x2="' + X(w.endMs).toFixed(1) + '" y2="' + cy + '" stroke="' + col + '" stroke-opacity=".18" stroke-width="2"/>';
      }
      // event ticks
      r.events.forEach(function (e) {
        var ex = X(e.tMs);
        ...
        svg +=
          ...
          '" class="tl-tick" ' +
          'data-tip="' +
          esc(D.fmtClock(e.tMs) + ' · ' + e.impulse + ' · conf ' + (e.conf != null ? e.conf.toFixed(2) : '—') + ' · ' + r.node) +
          '"/>';
      });
    });
    ...
    var legend =
      dated
        .map(function (r) {
          return '<span class="tl-leg"><span class="tl-leg-dot" style="background:' + D.nodeColor(r.node) + '"></span>' + esc(r.node) + '</span>';
        })
        .join('') +
      (fusion.findings &&
      fusion.findings.some(function (f) {
        return f.type === 'confirmed_apnea_event';
      })
        ? '<span class="tl-leg"><span class="tl-leg-bar"></span>confirmed apnea</span>'
        : '') +
    ...
    var toolbar =
      ...
      '<span class="tl-zoom-val">' +
      _tlZoom.toFixed(1) +
      '\u00d7</span>' +
      '<button class="tl-zoom-btn" type="button" data-z="in" aria-label="Zoom in">+</button>' +
      '<span class="tl-zoom-hint">' +
      (hasBands ? 'hover to scrub the clock · click a red apnea marker for detail · scroll when zoomed' : 'hover to scrub the clock · scroll horizontally when zoomed') +
      '</span>' +
      '</div>';
    box.innerHTML = toolbar + '<div class="tl-legend">' + legend + '</div><div class="tl-scroll">' + svg + '</div>' + '<div id="tlDetail" class="tl-detail" style="display:none"></div>' + notes;
    var meta = { t0: t0, span: span, padL: padL, padR: padR, innerW: innerW, W: W, fusion: fusion };
    var svgEl = box.querySelector('svg');
    if (svgEl) svgEl.style.width = _tlZoom * 100 + '%';
  ...
  function bindTimeline(box, meta) {
    ...
    box.querySelectorAll('.tl-zoom-btn').forEach(function (b) {
      b.addEventListener('click', function () {
        ...
        var v = box.querySelector('.tl-zoom-val');
        if (v) v.textContent = _tlZoom.toFixed(1) + '\u00d7';
      });
    });
  }
  function showFindingDetail(box, fusion, tMs) {
    var det = box.querySelector('#tlDetail');
    if (!det) return;
    box.querySelectorAll('.tl-band-vis.active').forEach(function (b) {
      b.classList.remove('active');
    });
    var vis = box.querySelector('.tl-band-vis[data-find="' + tMs + '"]');
    if (vis) vis.classList.add('active');
    var f = (fusion.findings || []).filter(function (x) {
      return x.tMs === tMs;
    })[0];
    if (!f) {
    ...
    var tags = (f.nodes || [])
      .map(function (n) {
        var base = n.split(' ')[0];
        return '<span class="fc-tag" style="--c:' + D.nodeColor(base) + '">' + esc(n) + '</span>';
      })
      .join('');
    det.innerHTML =
      '<div class="tld-head">' +
  ...
  function renderFindings(fusion) {
    ...
    if (!fusion || !fusion.anyOverlap) {
      box.innerHTML = emptyState(
        '◈',
        'No fused findings yet',
        "Fusion needs overlapping recordings. Once two or more nodes share a time window, cross-signal findings appear here — each node's own events still show on the timeline."
      );
      return;
    }
    var cards = [];
    ...
    if (a) {
      cards.push(
        findingCard(
          'Confirmed apnea events',
          '#FF6B7A',
          '<div class="fc-big">' + a.findings.length + '<span class="fc-unit"> events · index ' + (a.confirmedAHI != null ? a.confirmedAHI : '—') + '/h</span></div>',
          'Each is an OxyDex O₂ desaturation matched to an ECGDex/PpgDex autonomic surge inside a directional window (surge −' +
            fusion.matchWindow.leadMaxSec +
            's…+' +
            fusion.matchWindow.trailMaxSec +
            's vs the nadir). ' +
            'Matched ' +
            a.matched.desat +
            '/' +
            ...
            ' surges. ' +
            'Unmatched desats (' +
            a.unmatched.desat.length +
            ') may be central / low-arousal; unmatched surges (' +
            a.unmatched.surge.length +
            ') may be non-respiratory arousals.',
          ['OxyDex', 'ECGDex'],
          'confirmed_apnea'
        )
      );
      if (a.apneaAuthority) {
        ...
        var recon =
          aa.confirmedIndex != null
            ? 'The confirmed index (' +
              aa.confirmedIndex +
              '/h, desaturating obstructive events) sits below the device AHI by ' +
              aa.residualGap +
              '/h — ' +
        ...
        cards.push(
          findingCard(
            ...
            'Firmware-scored on the PAP device itself — the strongest single apnea truth on the bus, used here as the clinical reference.' +
              (compBits.length ? ' Components: ' + compBits.join(' · ') + '/h.' : '') +
              ' ' +
              recon,
            [aa.node],
            'device_ahi'
    ...
    if (p) {
      cards.push(
        findingCard(
          'Positional apnea',
          p.positional ? '#FFB84D' : '#3DE0D0',
          p.available
            ? '<div class="fc-big">' + (p.supineRate != null ? Math.round(p.supineRate * 100) + '%' + '<span class="fc-unit"> supine</span>' : '—') + '</div>'
    ...
    if (ag) {
      var wp = (ag.pairs || []).filter(function (p) {
        return p.windowed;
      });
      var wnote = '';
      if (wp.length) {
        ...
        wnote =
          " Glucose variability is computed on each night's EXACT overlap window" +
    ...
    if (hv) {
      hv.blocks.forEach(function (b) {
        cards.push(
          findingCard(
            'HRV consensus · ' + b.window,
            b.qc === 'divergent' ? '#FF6B7A' : '#39D98A',
            '<div class="fc-big">' + b.divergencePct + '%<span class="fc-unit"> max divergence</span></div>',
            b.note + ' Sources: ' + b.nodes.join(', ') + '.',
            b.nodes,
            'hrv_consensus'
          )
        );
    ...
    if (!cards.length) {
      box.innerHTML = emptyState('◈', 'No findings on these recordings', "The fusion rules ran across the overlap but nothing crossed threshold — a clean night, or signals that don't corroborate.");
      return;
    }
  ...
  function findingCard(title, color, big, note, nodes, evKey) {
    ...
    var badge = evKey ? evBadge(evKey) : '';
    return (
      '<div class="finding-card chart-card" style="--card-accent:' +
      color +
      '">' +
      '<div class="fc-head"><h4>' +
      esc(title) +
      (badge ? '<span class="fc-ev">' + badge + '</span>' : '') +
      '</h4><div class="fc-tags">' +
      tags +
      '</div></div>' +
      big +
      '<p class="fc-note">' +
  ...
  function tchBars(b, which) {
    var isHR = which === 'hr',
      t = isHR ? b.tchHR : b.tch;
    if (!t || !t.sigma2) return '';
    var keys = Object.keys(t.sigma2),
      sig = {},
      max = 0;
    keys.forEach(function (k) {
      var s = t.sigma && t.sigma[k] != null ? t.sigma[k] : Math.sqrt(t.sigma2[k]);
      sig[k] = s;
      if (s > max) max = s;
    });
    ...
    var rows = keys
      .map(function (k) {
        ...
        return (
          ...
          (isC ? ' tch-culprit' : '') +
          '">' +
          '<span class="tch-node" style="--c:' +
          D.nodeColor(base) +
          '">' +
          esc(k) +
          (isC ? ' \u25B2' : '') +
          '</span>' +
          '<span class="tch-bar"><i style="width:' +
          pct +
          '%;background:' +
          D.nodeColor(base) +
          '"></i></span>' +
          '<span class="tch-val mono">\u03C3\u2009' +
          sig[k].toFixed(1) +
          '</span></div>'
        );
      })
    ...
    var wm = isHR
      ? b.hrReconciled != null
        ? '<div class="tch-recon">Reconciled HR <b class="mono">' + b.hrReconciled + ' bpm</b> <span class="tch-recon-sub">inverse-variance weighted</span></div>'
        : ''
      : b.rmssd && b.rmssd.weightedMean != null
        ? '<div class="tch-recon">Reconciled RMSSD <b class="mono">' + b.rmssd.weightedMean + ' ms</b> <span class="tch-recon-sub">inverse-variance weighted</span></div>'
        : '';
  ...
  function tchTauSpark(b, which) {
    ...
    var a = _t && _t.allan;
    if (!a || !a.adev) return '';
    var nodes = Object.keys(a.adev),
      taus = a.tausMin || a.taus || [];
    if (!taus.length) return '';
    var maxV = 0,
      any = false;
    nodes.forEach(function (k) {
      (a.adev[k] || []).forEach(function (v) {
    ...
    var yAt = function (v) {
      return padT + (1 - v / maxV) * ih;
    };
    var lines = nodes
      .map(function (k) {
        var base = k.split(' ')[0],
          col = D.nodeColor(base),
          segs = [],
          cur = [];
        (a.adev[k] || []).forEach(function (v, i) {
          if (v != null && isFinite(v)) {
        ...
        return segs
          .map(function (pts) {
            ...
            var xy = pts[0].split(',');
            return '<circle cx="' + xy[0] + '" cy="' + xy[1] + '" r="1.8" fill="' + col + '"/>';
          })
          .join('');
      })
      .join('');
    var xlabs = taus
      .map(function (t, i) {
        return '<text x="' + xAt(i).toFixed(1) + '" y="' + (H - 4) + '" text-anchor="middle" class="tch-tau-lab">' + t + 'm</text>';
      })
      .join('');
    return (
      '<div class="tch-tau"><div class="tch-tau-cap">Allan deviation vs averaging time \u2014 lower = steadier at that timescale</div>' +
      '<svg viewBox="0 0 ' +
      W +
      ' ' +
      H +
      '" class="tch-tau-svg" role="img" aria-label="Per-sensor Allan deviation across averaging times">' +
      lines +
      xlabs +
      '</svg></div>'
    );
  ...
  function tchNote(b, which) {
    ...
    if (!t) return '';
    var u = isHR ? 'bpm' : 'ms';
    var s2 = Math.round(t.sigma2[t.culprit]),
      nn = Object.keys(t.sigma2).length;
    return (
      t.culprit +
      ' carries the largest error variance (\u03C3\u00B2\u2248' +
      s2 +
      ' ' +
      u +
      '\u00B2) across ' +
      nn +
      ' co-recorded sites over ' +
      t.n +
      ' epochs' +
      ...
      (t.quietOrderUncertain
        ? ' Trust the culprit and its \u03C3\u00B2; the two quieter sensors (' +
          (t.quietSensors || []).join(', ') +
          ') are both low and their relative order is uncertain (reference-free TCH determines quiet sensors poorly).'
        : '')
    );
  }
  ...
  function renderTable(fusion) {
    ...
    if (!rows.length) {
      box.innerHTML = emptyState('≣', 'Nothing to tabulate yet', 'Confirmed findings are listed here with their wall-clock time, type, confidence and source events.');
      return;
    }
    var body = rows
      .map(function (f) {
        return (
          ...
          '</td>' +
          '<td>' +
          evBadge(TYPE_EV[f.type] || '') +
          '<span class="tbl-type" style="--c:' +
          (f.type === 'confirmed_apnea_event' ? '#FF6B7A' : '#FFB84D') +
          '">' +
          esc(f.type) +
          '</span></td>' +
          '<td class="mono">' +
  ...
  function renderPairs(fusion) {
    ...
    if (!ps.length) {
      box.innerHTML = emptyState('⇄', 'Nothing to compare yet', 'Load 2 or more dated nodes to see how their recording windows overlap, pair by pair.');
      return;
    }
    box.innerHTML = ps
      .map(function (p) {
        ...
        return (
          '<div class="pair-card chart-card" style="--card-accent:' +
          (ov ? '#3DE0D0' : '#FFB84D') +
          '">' +
          '<div class="pair-head"><span class="pair-dot" style="background:' +
          D.nodeColor(p.an) +
          '"></span>' +
          esc(p.a) +
          '<span class="pair-vs">vs</span><span class="pair-dot" style="background:' +
          D.nodeColor(p.bn) +
          '"></span>' +
          esc(p.b) +
          '</div>' +
          '<div class="pair-rows"><div><span>' +
          esc(p.an) +
          '</span><b class="mono">' +
          aw +
          '</b></div>' +
          ...
          (ov ? 'ok' : 'no') +
          '"><span>' +
          mrBadge('measured', "Overlap of the two recordings' windows on the shared clock — a counted quantity.") +
          'Overlap</span><b class="mono">' +
          (ov ? ov.overlapMin.toFixed(0) + ' min' + (ov.basis === 'utc-instant' ? ' · UTC-aligned' : '') : 'none') +
          '</b></div></div>' +
          '</div>'
        );
      })
  ...
  function renderAll(recs, fusion, warns) {
    ...
    renderHero(fusion);
    renderKPIs(fusion);
    renderTimeline(recs, fusion);
    renderFindings(fusion);
    renderTable(fusion);
    renderPairs(fusion);
    renderWarnings(warns || []);
    var exb = $('exportBar');
    if (exb) exb.classList.toggle('show', recs.length > 0);
  }
