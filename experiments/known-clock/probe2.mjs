...
const o2 = readFileSync('/tmp/kc/Wellue_O2Ring-S_S8AW2100_20260813231713_PMDARRIVAL.csv','utf8')
  ...
  .map(c=>({hostMs:hostMsOf(c[0]), devMs:Number(c[3])/1e6})).filter(x=>x.hostMs!==null&&isFinite(x.devMs));

// find the reset and take the LONGEST monotonic run
let runs=[], cur=[o2[0]];
for (let i=1;i<o2.length;i++){ if (o2[i].devMs >= o2[i-1].devMs) cur.push(o2[i]); else { runs.push(cur); cur=[o2[i]]; } }
runs.push(cur);
runs.sort((a,b)=>b.length-a.length);
const seg = runs[0];
console.log(`monotonic runs: ${runs.length}  longest=${seg.length} of ${o2.length} packets`);
const hS = seg[seg.length-1].hostMs - seg[0].hostMs, dS = seg[seg.length-1].devMs - seg[0].devMs;
...
console.log('  rate-normalised  :', r.ok
  ? `ok ppm=${r.ppm.toFixed(2)}  spreadMs=${r.spreadMs.toFixed(1)}  independent=${r.independent}`
  : `REFUSED ${r.reason}`);
if (r.ok) console.log(`  >>> VERDICT: ${r.independent
  ? 'FALSE POSITIVE — a 1 s-quantised DRAWN counter is reported as an independent second clock.'
  : 'correctly reported independent:false'}`);

// drawn-share of this segment, the quantity ppgdex-dsp gates at >=99%
const dd=[]; for(let i=1;i<seg.length;i++) dd.push(seg[i].devMs-seg[i-1].devMs);
const t=new Map(); dd.forEach(v=>t.set(v,(t.get(v)||0)+1));
