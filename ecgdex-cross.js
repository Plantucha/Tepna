...
function crossNightBlock(list){
  ...
  if(global.CrossNightEnvelope){
    return global.CrossNightEnvelope.build({
      node:'ECGDex', nodeVersion:'1.1', unit:'recording',
      items:list,
      t0Of: s=>s.t0Ms,
  ...
  out.nights=sorted.map(s=>({ t0Ms:s.t0Ms, date:fmtDateUTC(s.t0Ms), analyzablePct:s.analyzablePct,
    rmssd:s.dispRm, sdnn:s.dispSd, hr:s.dispHr, dfaAlpha1:s.dfa1, qtc:qtcOf(s) }));
  return out;
}
