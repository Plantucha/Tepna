...
| **§3 D** the walk-through | `docs/MEASUREMENT-WALKTHROUGH-OXYDEX-2026-09-21.md` + `tools/measurement-walk.mjs` | re-derives every hop from disk (value, window, channel, code vs bundle AND ledger, `contentId` recomputed by the real parser, envelope reason, ledger vs raw bytes) — green on 3/3 fixtures; 13 plants, each hop catching its own corruption; an absent input prints `∘`, never ✓. **Corrected #PRNUM (2026-09-22):** on those three CSV fixtures the `envelope` hop was never WALKED — it was null-with-reason, and printing ✓ on it was the fabricated-pass shape the walk exists to catch. It now prints `—` (not applicable, EXCLUDED from the verdict's population); the other hops were and are re-derived. The walk now also carries the ECGDex leg (four fixtures, value re-derived from a recompute) |
...
| **§10 performance is measured on one dimension only** | ~~not executed (except size)~~ **EXECUTED 2026-09-22 — all three dimensions, bands pre-stated, verdict `PASS` as a `tepna.verdict/1` record: `audits/MEASUREMENT-EMITTER-PERF-2026-09-22.json`** | Measured with the ECGDex emitter (the second) against `origin/main`, 3 runs per tree per input, medians: real H10 clip — compute() light 227.8 → 207.3 ms, rich 225.3 → 204.3 ms (the delta is negative, i.e. noise), peak RSS 107.8 → 107.4 MB, export +2 334 B (light and rich alike — three blocks); synthetic night — 43.0 → 41.6 ms, 41.2 → 40.6 ms, 68.7 → 69.4 MB, +2 336 B. Bands (written before the runs): runtime ≤ +5 % or +50 ms, peak RSS ≤ +50 MB, export ≤ +4 KB. The Integrator adapter's consume step is ~1 ms either way. | Re-measure only if a per-window emission lands (the shared-parts row); the whole-night emitter is inside every band by an order of magnitude. |
