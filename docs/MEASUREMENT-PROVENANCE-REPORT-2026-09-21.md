...
| **The envelope hop is unexercised** | found (by the fixtures' shape) | Every committed fixture is a CSV, so `evidence.envelopeRef` is `null` + reason on all three, and no gate has ever walked a block to an acquisition envelope. | A committed `.dat` night with its `acq/*.json` envelope. The join (`session_id`) exists (Phase C); the fixture does not. Follow-up brief. |
