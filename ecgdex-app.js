...
const DSP = window.ECGDSP, UI = window.ECGUI;
const ING = window.DexIngest;   // §3 (ECG-INGEST-FOLLOWUPS): shared, gate-backed file-ingest classification (dex-ingest.js)
let RESULT = null, SCOPE = null, DEVICE_RR = null, DEVICE_HR = null, DEVICE_ACC = null, ACC_FS = null;
...
let activeKey = null, _recSeq = 0, _loadQueue = [], _replaceMode = false;
...
function loadECGFile(files){
  ...
  if(big){
    ...
    fr.onload = e => {
      ...
      const lines = txt.split(/\r?\n/); const arr = []; let t0Ms=null, prevMs=null, msStep=null;
      for(const line of lines){ const t=line.trim(); if(!t) continue;
        const p = t.split(/[;\t,]/); const v = parseFloat(p[p.length-1]); if(!isFinite(v)) continue;
        arr.push(Math.max(-32768,Math.min(32767,Math.round(v))));
        if(t0Ms===null){ const ms=parseTSfloat(p[0]); if(ms!=null) t0Ms=ms; }
...
function genSynthetic(){
  ...
  const map = { spot:{durSec:330,scenario:'spot'}, hour:{durSec:3600,scenario:'hour'},
                overnight:{durSec:3*3600,scenario:'overnight'}, full:{durSec:7*3600,scenario:'overnight'},
  ...
  setTimeout(()=>{
    ...
    DEVICE_HR = rec.deviceHR; DEVICE_ACC = rec.deviceACC; ACC_FS = rec.accFs;
...
function setLoad(which, msg){
  const st = $(which+'Status'), card = $(which+'Load');
...
function loadDeviceRR(file){
  ...
  fr.onload = e => {
    ...
    if(RESULT){ RESULT.deviceRR = DEVICE_RR; renderValidation(RESULT); }
  };
  fr.readAsText(file);
}
function loadDeviceHR(file){
  ...
  fr.onload = e => {
    ...
    if(RESULT){ RESULT.deviceHR = DEVICE_HR; renderHRValidation(RESULT); }
  };
  fr.readAsText(file);
}
  ...
  fr.onload = e => {
    ...
    if(RESULT){ RESULT.deviceACC = DEVICE_ACC; RESULT.accFs = ACC_FS;
      ...
      if(DSP.stampEpochPositions && RESULT.epochs){
        if(DEVICE_ACC._relBase && RESULT.t0Ms!=null){ DEVICE_ACC.forEach(o=>{ o.tsMs+=RESULT.t0Ms; }); DEVICE_ACC._relBase=false; }
        const ep = DSP.stampEpochPositions(RESULT.epochs, DEVICE_ACC, ACC_FS, RESULT.t0Ms, RESULT.durSec);
        const posAt = (sec)=>{ const m=sec/60; for(const p of ep){ if(m>=p.tMin && m<p.tMin+5) return p.position; } return ep.length?ep[ep.length-1].position:null; };
        (RESULT.events||[]).forEach(ev=>{ if(ev.impulse==='autonomic_surge' && ev.meta && ev._sec!=null) ev.meta.position = posAt(ev._sec); });
      }
...
function runPipeline(rec, primaryName){
  try {
    ...
    if(DEVICE_ACC && !rec.deviceACC){
      ...
      if(DEVICE_ACC._relBase && rec.t0Ms!=null){ DEVICE_ACC.forEach(o=>{ o.tsMs+=rec.t0Ms; }); DEVICE_ACC._relBase=false; }
      rec.deviceACC = DEVICE_ACC; rec.accFs = ACC_FS;
    }
    clearAlerts();
    setTimeout(()=>{
      let r;
      try { r = DSP.analyze(rec, progress); }
      catch(err){ showErr(err.message||String(err)); progress(0,''); $('prog').classList.remove('show'); return; }
      RESULT = r;
      // store into the multi-recording map (key by floating t0Ms; dedupe identical)
      const key = (r.t0Ms!=null ? String(r.t0Ms) : ('rec_'+(++_recSeq)));
      r._key = key; allRecordings[key] = r; activeKey = key;
      ...
      showOK('Analyzed '+r.nBeats+' beats from '+(r.durSec>=3600?(r.durSec/3600).toFixed(1)+' h':(r.durSec/60).toFixed(0)+' min')+' of ECG · '+r.analyzablePct+'% analyzable');
      setTimeout(()=>{ $('prog').classList.remove('show'); $('proc').textContent=''; }, 700);
      // advance the multi-file load queue (if any)
      if(_loadQueue.length){ _loadQueue.shift(); if(_loadQueue.length) setTimeout(_processQueue, 40); }
    }, 20);
...
function renderAll(r){
  ...
  if(!SCOPE){ SCOPE = new UI.ECGScope($('ecgCanvas'), $('ecgMini')); SCOPE.onView = updateScopeReadout; }
  SCOPE.setData(r);
  ...
  $('sidebarDataInfo').innerHTML = (r.source==='synthetic'?'Synthetic ECG':'Raw ECG file')+'<br>'+
    r.nBeats.toLocaleString()+' beats · '+r.hr+' bpm · '+(r.durSec>=3600?(r.durSec/3600).toFixed(1)+' h':(r.durSec/60).toFixed(0)+' min')+
    '<br>'+r.analyzablePct+'% analyzable';
}

function updateScopeReadout(view, fs, N){
  const s0 = view.start/fs, s1 = (view.start+view.span)/fs, span = view.span/fs;
  const fmt = s => { const m=Math.floor(s/60), sec=(s%60); return (m>0? m+'m ':'')+sec.toFixed(span<8?2:0)+'s'; };
  $('scopeReadout').innerHTML = `<span>window <b>${fmt(s0)} – ${fmt(s1)}</b></span><span>span ${span<60?span.toFixed(span<8?2:0)+' s':(span/60).toFixed(1)+' min'}</span><span>${(N/fs/3600).toFixed(2)} h total</span>`;
}
...
function renderContext(r){
  ...
  let notes = '';
  if(r.ambulatory) notes='<div class="ctx-note">🚶 <b>Ambulatory / awake-active</b> — '+r.modeWhy.replace('ambulatory: ','').replace(' — overnight veto','')+'. The duration heuristic is overridden by activity: <b>sleep staging and the CVHR apnea screen are suppressed</b> (invalid under exercise). Heart-rate, HRV-under-activity and gait still compute.</div>';
  else if(r.tier==='ultra-short') notes='<div class="ctx-note">⚠ Ultra-short recording — SDNN, LF, VLF and LF:HF need ≥5 min and are withheld/flagged. rMSSD, pNN50, SD1 and HF remain valid.</div>';
  else if(r.tier==='overnight') notes='<div class="ctx-note">Per-epoch medians are the representative value; whole-night spread feeds SDANN / SDNN-index. CVHR & sleep staging unlocked at this length.</div>';
  else if(r.tier==='short') notes='<div class="ctx-note">5-min standard window — full short-term suite valid (Task Force 1996). Overnight metrics (VLF, DFA α1, CVHR, staging) need ≥90 min.</div>';
...
function renderKPI(r){
  ...
  const personalized = [
    ...
    {l:'Rest HR', v:r.rhrEff?r.rhrEff+'bpm':'—', sub:r.expRHR?('age norm ~'+r.expRHR):'awake est', s:!r.rhrEff?'neutral':r.rhrEff<=(r.expRHR||62)?'ok':r.rhrEff<=(r.expRHR||62)+8?'warn':'bad'},
    {l:'VO₂max Est', v:r.vo2adj!=null?r.vo2adj:'—', sub:'ml/kg/min · HR-ratio', s:r.vo2adj==null?'neutral':r.vo2adj>=45?'ok':r.vo2adj>=38?'warn':'bad'},
  ];
  if(r.longRec && !r.ambulatory && r.apneaRisk) personalized.push(
    {l:'Apnea Risk', v:r.apneaRisk.cat, sub:r.apneaRisk.note, s:r.apneaRisk.sev});
  if(r.estAHI) personalized.push(
    {l:'Est. AHI', v:'≈'+r.estAHI.value, sub:'/h · '+r.estAHI.band+' · ECG-only', s:r.apneaRisk?r.apneaRisk.sev:'neutral'});
  if(r.morph){
    personalized.push({l:'Ectopy', v:(r.morph.nPVC+r.morph.nPAC), sub:r.morph.ectopyBurden+'% · '+r.morph.nPVC+'V/'+r.morph.nPAC+'S', s:r.morph.ectopyBurden<0.5?'ok':r.morph.ectopyBurden<3?'warn':'bad'});
    ...
    personalized.push({l:'AF Screen', v:{'no-af':'Clear','occasional-irregular':'Watch','possible-af':'Flag','insufficient':'—'}[af], sub:r.morph.af.suspiciousPct+'% irregular', s:{'no-af':'ok','occasional-irregular':'warn','possible-af':'bad','insufficient':'neutral'}[af]});
  }
  if(r.hrvStab){
    const hs=r.hrvStab;
    personalized.push({l:'HRV Stability', v:(hs.sigma_lnRMSSD_slope>0?'+':'')+hs.sigma_lnRMSSD_slope, sub:hs.severity==='good'?'stabilizing/h':hs.severity==='bad'?'rising/h':'flat/h', s:hs.severity});
  }
  const items = personalized.concat([
    {l:'Mean HR', v:r.dispHr, sub:r.longRec?'bpm · median':'bpm', s:hrStat(r.dispHr)},
    {l:'rMSSD'+(r.longRec?' (med)':''), v:r.dispRm+'ms', sub:r.expRmssd?('age norm ~'+r.expRmssd):'≥30 good', s:r.expRmssd?(r.dispRm>=r.expRmssd?'ok':r.dispRm>r.expRmssd*0.7?'warn':'bad'):sc(r.dispRm,30,20)},
    ...
    {l:'% Analyzable', v:r.analyzablePct+'%', sub:r.coveragePct!=null&&r.coveragePct<99?(r.coveragePct+'% covered · '+r.cleanBeatPct+'% clean'):'night usable', s:r.analyzablePct>=90?'ok':r.analyzablePct>=75?'warn':'bad'},
    ...
    {l:'CVHR Index', v:(r.longRec&&!r.ambulatory)?r.cvhr.index:'—', sub:r.ambulatory?'suppressed · ambulatory':(r.longRec&&r.profile&&r.profile.cpap?'surges/h · on CPAP':'surges/h (apnea)'), s:(!r.longRec||r.ambulatory)?'neutral':r.cvhr.index<5?'ok':r.cvhr.index<15?'warn':'bad'},
  ...
  $('kpiGrid').innerHTML = items.map(k=>`<div class="kpi ${k.s}">
    <div class="kpi-label">${evBadge(k.l)}${k.l}</div><div class="kpi-val ${k.s}">${k.v}</div><div class="kpi-sub">${k.sub}</div></div>`).join('');
...
function _chartCardBadge(bodyId, label){
  ...
  const card=b.closest('.card,.section-card'); if(!card) return;
  const h=card.querySelector('.card-h,.section-card-title'); if(!h||h.querySelector('.ev')) return;
  const disc=evBadge(label); if(disc) h.insertAdjacentHTML('afterbegin', disc+' ');
}
...
function renderCharts(r){
  ...
  $('poincareStats').innerHTML = `SD1 <b>${r.sd1}</b> ms · SD2 <b>${r.sd2}</b> ms · SD1/SD2 <b>${r.sd1sd2}</b> · area <b>${r.ellArea.toLocaleString()}</b> ms²`
  ...
  if(r.longRec && !r.ambulatory && r.cvhr.hrSeries && r.cvhr.hrSeries.length>30){
    const hs = r.cvhr.hrSeries; const stepC = Math.max(1, Math.floor(hs.length/1400));
    const pts = []; for(let i=0;i<hs.length;i+=stepC) pts.push({ x:i/60, y:hs[i] });
    ...
    $('cvhrBody').innerHTML =
      ...
      (r.estAHI?`<div class="gang-summary" style="margin-top:10px">
...
function renderCRC(r){
  const c = r.crc;
  if(!c){ $('crcCard').style.display='none'; return; }
  const NB = c.nbins || 16;
  // phase-averaged HR across the respiratory cycle (RSA shape) — plot one-and-a-bit cycles
  const curve = c.phaseCurve || [];
  const pts = [];
  for(let rep=0; rep<2; rep++){
    for(let i=0;i<curve.length;i++){ if(curve[i]==null) continue; pts.push({ x:(rep*NB+i)/NB*360, y:curve[i] }); }
  }
  ...
  $('crcBody').innerHTML =
    ...
    + (pts.length>4 ? UI.lineChart(pts, ratioCol, { W:680,H:150, xfmt:x=>x.toFixed(0)+'°' })
...
function renderMorph(r){
  const m = r.morph;
  if(!m){ $('morphSection').style.display='none'; return; }
  const d = m.delin, af = m.af;
  ...
  if(delinOK && m.medianBeat && m.medianBeat.valid){
    ...
    $('medianBeatStats').innerHTML = `median of <b>${m.medianBeat.nUsed}</b> normal beats · R <b>${d.Ramp}</b>µV · T <b>${d.Tamp}</b>µV${d.pPresent?` · P <b>${d.Pamp}</b>µV`:' · P not resolved'}`;
  } else {
    $('medianBeatBody').innerHTML = `<div class="q-note" style="padding:24px 8px;text-align:center">⚠ Median beat could not be built — too few clean, well-aligned normal beats in this recording. Interval delineation (QRS/QT/QTc/PR/ST) is withheld rather than reported as unreliable values.</div>`;
  ...
  // intervals + AF
  const cell=(l,v,u,nr,sev,note)=>`<tr><td style="color:var(--text2);font-weight:600;font-family:Inter,sans-serif">${typeof evBadge==='function'?evBadge(l):''}${l}</td><td class="${sev}">${v==null?'—':v}</td><td style="color:var(--text3)">${u}</td><td style="color:var(--text3)">${nr}</td><td style="color:var(--text3);font-family:Inter,sans-serif;font-size:11px">${note}</td></tr>`;
  ...
  $('morphIntervals').innerHTML = `
    ...
    ${(()=>{ const qt=m.qtcTrend, tw=m.twa;
      const twaPill = tw?`<div class="gang-pill" style="border-color:var(--${tw.abnormal?'red':'green'});color:var(--${tw.abnormal?'red':'green'})"><b>T-wave alternans:</b> ${tw.uv} µV${tw.abnormal?' · abnormal (≥47)':' · normal'}</div>`:'';
      let chart='';
      if(qt && qt.length>=2 && UI && UI.lineChart){
        const pts=qt.map(p=>({ x:p.tMin, y:p.qtc }));
        const lo=Math.min(...qt.map(p=>p.qtc)), hi=Math.max(...qt.map(p=>p.qtc));
        chart=`<div class="mini-h" style="margin-top:14px">Nocturnal QTc trend <span class="mini-sub">${qt.length} × 15-min windowed median beats · ${lo}–${hi} ms · the GlucoDex cross-node feed</span></div>`
...
function renderSleep(r){
  if(r.longRec && r.ambulatory){
    $('hypnoBody').innerHTML = '<div class="q-note" style="margin:6px 0"><span class="pill pill-gray">Suppressed</span> Sleep staging withheld — '+r.modeWhy+'. A hypnogram is not published for an ambulatory recording (a walk is not a sleep study).</div>';
    ...
    const sm = r.stageMin;
    $('sleepStats').innerHTML = ['Deep','Light','REM','Wake'].map(st=>{
...
function renderValidation(r){
  ...
  if(!v){ $('valCard').style.display='none'; return; }
...
function dualLineSVG(pts, opts){
  opts=opts||{}; const W=opts.W||680,H=opts.H||160,P={l:46,r:14,t:14,b:24};
  const ca=opts.ca||UI.COLORS.teal, cb=opts.cb||UI.COLORS.amber;
  let ymn=Infinity,ymx=-Infinity,xmn=Infinity,xmx=-Infinity;
  for(const p of pts){ for(const v of [p.a,p.b]){ if(v!=null&&isFinite(v)){ if(v<ymn)ymn=v; if(v>ymx)ymx=v; } } if(p.x<xmn)xmn=p.x; if(p.x>xmx)xmx=p.x; }
  if(!isFinite(ymn)) return '';
  if(ymx===ymn)ymx=ymn+1; if(xmx===xmn)xmx=xmn+1;
  const pad=(ymx-ymn)*0.08; ymn-=pad; ymx+=pad;
  const sx=x=>P.l+(x-xmn)/(xmx-xmn)*(W-P.l-P.r);
  const sy=y=>H-P.b-(y-ymn)/(ymx-ymn)*(H-P.t-P.b);
  const path=key=>{ let d='',pen=false; for(const p of pts){ const v=p[key]; if(v==null||!isFinite(v)){pen=false;continue;} d+=(pen?'L':'M')+sx(p.x).toFixed(1)+' '+sy(v).toFixed(1)+' '; pen=true; } return d; };
  const xt=[]; for(let i=0;i<=5;i++) xt.push(xmn+i*(xmx-xmn)/5);
...
function renderHRValidation(r){
  ...
  if(!v){ $('hrCard').style.display='none'; return; }
  ...
  const rPill = !v.rMeaningful ? '<span style="display:inline-block;padding:2px 8px;border-radius:999px;font-size:11px;font-weight:600;background:rgba(255,255,255,.07);color:var(--text3)">flat · n/a</span>'
    : v.r>=0.95 ? '<span class="pill pill-green">excellent</span>'
    : v.r>=0.85 ? '<span class="pill pill-yellow">good</span>'
...
function renderACCComparison(r){
  ...
  if(!a){ $('accCard').style.display='none'; return; }
  ...
  const respPill = dResp==null?'' : dResp<2?'<span class="pill pill-green">agree</span>':dResp<4?'<span class="pill pill-yellow">close</span>':'<span class="pill pill-red">diverge</span>';
  // posture
  ...
  const posturePills = longPosture
    ? a.postureBreakdown.map(p=>`<div class="gang-pill">${p.label} <b>${p.pct}%</b></div>`).join('')
    : '';
  // motion tie: how many high-motion epochs fall in Wake/REM (vs deep/light)
  const stageAt = {}; (r.stages||[]).forEach(s=>stageAt[s.tMin.toFixed(1)]=s.stage);
  ...
  if(a.motionSeries && a.motionSeries.length>=3){
    const span = a.durMin;
    motionChart = `<div class="mini-h" style="margin-top:14px">Motion / movement trace <span class="mini-sub">de-gravitated accel · ${a.nSamples.toLocaleString()} samples @ ${a.accFs} Hz${surgeMin.length?' · red ticks = CVHR clusters':''}</span></div>`
      + UI.lineChart(a.motionSeries, UI.COLORS.purple, { W:680,H:140, ymn:0, med:a.motionMedian,
          marks: surgeMin.map(m=>m), xfmt:x=> span>90 ? (x/60).toFixed(1)+'h' : x.toFixed(0)+'m' });
  } else {
  ...
  $('accBody').innerHTML = `
    ...
    <div class="q-note" style="margin-top:8px"><b>Three ties to the ECG, no extra interpretation needed.</b>
      ...
      <span style="opacity:.7">${r.source==='synthetic'?'ACC here is synthetic ground truth. ':''}Posture labelling depends on sensor mounting; tilt angle is mount-independent. Informational.</span></div>` + accExtraCards(r);
...
function accExtraCards(r){
  ...
  window.accRRacc = ex.rracc; window.accSteps = ex.gait;
  window.accAgreement = ex.agreement; window.accConsensus = ex.consensus;
  return _accCardRR(ex) + _accCardAgreement(ex, r) + _accCardConsensus(ex, r) + _accCardGait(ex, r);
}
function _fmtAxisMin(m, durMin){ return durMin>90 ? (m/60).toFixed(1)+'h' : m.toFixed(0)+'m'; }
function _fmtEpochClock(r, tMin){
  if(r && r.t0Ms!=null){ const d=new Date(r.t0Ms+tMin*60*1000), p=x=>String(x).padStart(2,'0');
    return p(d.getUTCHours())+':'+p(d.getUTCMinutes()); }
  return tMin.toFixed(0)+'m';
}
function _zoneColor(c){ return { gray:'#6F8096', blue:UI.COLORS.blue, green:UI.COLORS.green, amber:UI.COLORS.amber, red:UI.COLORS.red }[c]||'#6F8096'; }
...
function _accCardRR(ex){
  ...
  const summary = s ? `<div class="gang-summary" style="margin-top:10px">
...
function _accRRChart(ex){
  const C=UI.COLORS, W=600,H=120,P={l:42,r:12,t:12,b:22}, pts=ex.rracc;
  const xs=pts.map(p=>p.tStartMin), xmn=Math.min.apply(null,xs), xmx=Math.max.apply(null,xs)||xmn+1, ymn=6, ymx=30;
  ...
  const dots=pts.map(p=>`<circle cx="${sx(p.tStartMin).toFixed(1)}" cy="${sy(p.rr).toFixed(1)}" r="${p.conf==='high'?1.7:1.4}" fill="${p.conf==='high'?C.teal:C.dim}" opacity="${p.conf==='high'?.9:.45}"/>`).join('');
...
function _accCardAgreement(ex){
  const ag=ex.agreement; let body;
  if(!ag){
    ...
    const pill = strong?'<span class="pill pill-green">Strong agreement</span>':moderate?'<span class="pill pill-yellow">Moderate</span>':'<span class="pill pill-red">Poor — check sensor contact</span>';
    const narrowNote = (ag.r<0.65 && ag.mae<=2.0)
    ...
    const evg = lbl => { const b = evBadge(lbl); return b ? '<span style="margin-right:5px">'+b+'</span>' : ''; };
...
function _blandAltman(ag){
  const C=UI.COLORS, W=420,H=190,P={l:44,r:14,t:14,b:28};
  const ms=ag.ba.map(p=>p.mean), ds=ag.ba.map(p=>p.diff);
  const xmn=Math.min.apply(null,ms)-1, xmx=Math.max.apply(null,ms)+1;
  ...
  const loaL=ag.loa.map(l=>`<line x1="${P.l}" y1="${sy(l).toFixed(1)}" x2="${W-P.r}" y2="${sy(l).toFixed(1)}" stroke="${C.dim}" stroke-dasharray="3 4" opacity=".6"/><text x="${W-P.r}" y="${(sy(l)-3).toFixed(1)}" fill="${C.dim}" font-size="8" text-anchor="end" font-family="IBM Plex Mono,monospace">${l>0?'+':''}${l}</text>`).join('');
...
function _accCardConsensus(ex, r){
  const co=ex.consensus; let body;
  if(!co){
    ...
    const pill=co.rate>=85?'<span class="pill pill-green">Strong consensus</span>':co.rate>=70?'<span class="pill pill-yellow">Moderate</span>':'<span class="pill pill-red">Weak — review staging</span>';
...
function _accCardGait(ex, r){
  ...
  const zoneBar=`<div class="acc-zonebar">${g.zonePct.filter(z=>z.pct>0).map(z=>`<div class="acc-zone-seg" style="width:${z.pct}%;background:${_zoneColor(z.col)}" title="${z.zone} ${z.pct}%">${z.pct>=10?z.pct+'%':''}</div>`).join('')}</div>
    <div class="acc-zone-legend">${g.zonePct.map(z=>`<span><i style="background:${_zoneColor(z.col)}"></i>${z.zone} ${z.pct}%</span>`).join('')}</div>`;
  const cad = g.cadEpochs.length>=3
    ? `<div class="mini-h" style="margin-top:14px">Cadence <span class="mini-sub">steps / min · per 60-s epoch</span></div>${UI.lineChart(g.cadEpochs.map(c=>({x:c.tMin,y:c.cadence})), UI.COLORS.amber, { W:600,H:120, ymn:0, xfmt:x=> ex.durMin>90?(x/60).toFixed(1)+'h':x.toFixed(0)+'m' })}` : '';
...
function renderGanglior(r){
  ...
  $('gangBody').innerHTML = `
    ...
    <div class="gang-stream">${ev.slice(0,160).map(e=>{
...
function renderTable(r){
  ...
  const rows = [
    ...
    ['HRV Score', r.hrvScore==null?'—':r.hrvScore, '0–100','≥45', r.hrvScore==null?'neutral':r.hrvScore>=45?'ok':r.hrvScore>=33?'warn':'bad','Autonomic readiness (rMSSD-calibrated)'],
    /* ANS Age table row REMOVED 2026-06-23 (BADGE-COVERAGE-AUDIT-FOLLOWUPS R9, WP-A) — see renderKPI note. */
    ['Resting HR', r.rhrEff||'—','bpm', r.expRHR?('~'+r.expRHR):'—', !r.rhrEff?'neutral':r.rhrEff<=(r.expRHR||62)?'ok':r.rhrEff<=(r.expRHR||62)+8?'warn':'bad', r.longRec?'Nocturnal floor + 8 (awake est)':'Measured'],
    ...
    ...(r.estAHI?[['Est. AHI', '≈'+r.estAHI.value,'/h','<5', r.apneaRisk?r.apneaRisk.sev:'neutral', r.estAHI.band+' · CVHR/CPC proxy from ECG alone ('+r.estAHI.lo+'–'+r.estAHI.hi+') · screen-only']]:[]),
    ...(r.morph?[
      ...
      ...(D.valid?[
        ...
        ['QTc (Bazett)', D.qtcBazett==null?'—':D.qtcBazett,'ms','<450M/460F', D.qtcBazett==null?'neutral':D.qtcBazett>470?'bad':D.qtcBazett>450?'warn':'ok','Rate-corrected QT'],
        ['QTc (Fridericia)', D.qtcFrid==null?'—':D.qtcFrid,'ms','<450', D.qtcFrid==null?'neutral':D.qtcFrid>460?'warn':'ok','Less rate-biased'],
        ['PR interval', D.pr==null?'—':D.pr,'ms','120–200', D.pr==null?'neutral':(D.pr>220||D.pr<120?'warn':'ok'), D.pPresent?'P-onset → QRS-onset':'P not resolved @130 Hz'],
      ...
      ['AF screen', {'no-af':'Clear','occasional-irregular':'Watch','possible-af':'Flag','insufficient':'—'}[r.morph.af.verdict],'—','Clear', {'no-af':'ok','occasional-irregular':'warn','possible-af':'bad','insufficient':'neutral'}[r.morph.af.verdict], r.morph.af.suspiciousPct+'% windows irregular · ΔRR entropy '+r.morph.af.shannon+' · screen only'],
    ]:[]),
    ...
    ...(r.longRec?[['SDANN', r.sdann==null?'—':r.sdann,'ms','≥50',r.sdann==null?'neutral':sc(r.sdann,50,30),'SD of 5-min mean-RR'],
    ...
    ['DFA α1', r.dfa1==null?'—':r.dfa1,'—','0.9–1.2', r.dfa1==null?'neutral':(r.dfa1>=0.9&&r.dfa1<=1.2?'ok':(r.dfa1<0.75||r.dfa1>1.5?'bad':'warn')),'Short-term fractal scaling'],
    ...
    ...(r.crc?[
      ...
      ['RSA efficiency', r.crc.rsaEfficiencyRatio,'ratio','≥1.05', r.crc.rsaEfficiencyRatio>=1.08?'ok':r.crc.rsaEfficiencyRatio>=1.04?'warn':'neutral','Insp:exp HR ratio — higher = stronger RSA (Border 2025; 1.5 = theoretical optimum)'],
      ...
      ['Coupling strength', r.crc.couplingStrength,'0–1','≥0.5', r.crc.couplingStrength>=0.5?'ok':r.crc.couplingStrength>=0.3?'warn':'bad','CSI-style sync index (arXiv:2605.18802)'],
      ['EDR resp rate', r.crc.respFromEDR,'br/min','12–20','neutral','Respiration from R-peak amplitude modulation'],
      ...(r.crc.plvDuringSurges!=null&&r.crc.plvBaseline!=null?[['PLV surge vs base', r.crc.plvDuringSurges+' / '+r.crc.plvBaseline,'0–1','drop','neutral','Coupling drop during CVHR clusters → surge confidence']]:[]),
    ]:[]),
  ];
  window.__summaryRows = rows;   // structured source for the tidy CSV export (not a DOM scrape)
  $('tblBody').innerHTML = rows.map(([m,v,u,nr,s,n])=>`<tr>
    ...
    <td style="color:var(--text3);font-family:Inter,sans-serif;font-size:10px">${n}</td></tr>`).join('');
  $('tblWrap').classList.add('show'); $('slTbl').style.display='flex';
}
...
function csvClean(t){ const s=String(t==null?'':t).replace(/[\u2191\u2193\u2192\u2197\u2198\u2B06\u2B07]/g,'').replace(/\s+/g,' ').trim(); return (s===''||s==='—'||s==='–'||/^n\/?a$/i.test(s))?'':s; }
// Unified summary CSV: serialize the rendered Full Metrics Table as clean data (no glyphs, no over-quoting).
function _tblCSV(){ const rows=document.querySelectorAll('#tblWrap table tr'); return csvDoc([...rows].map(r=>[...r.cells].map(c=>csvClean(c.textContent)))); }
...
function _summaryCSV(){
  ...
  const tier=label=>{ try{ const R=window.EcgRegistry; if(!R||!R.idForLabel) return ''; const id=R.idForLabel(label); return (id&&R.REGISTRY[id]&&R.REGISTRY[id].evidence)||''; }catch(e){ return ''; } };
  const out=[['Metric','Value','Unit','Normal Range','Status','Evidence','Notes']];
  rows.forEach(row=>{
    const metric=String(row[0]==null?'':row[0]).replace(/^[\s—–]+|[\s—–]+$/g,'').trim();
    const isSection=(row[1]===''||row[1]==null)&&(row[2]===''||row[2]==null);
    out.push(isSection
      ? [metric,'','','','(section)','','']
      : [metric, dash(row[1]), dash(row[2]), dash(row[3]), (STAT[row[4]]!==undefined?STAT[row[4]]:dash(row[4])), tier(row[0]), dash(row[5])]);
  });
  return csvDoc(out);
}
function exportCSV(){ if(!RESULT) return; dl(_summaryCSV(), exportName({node:'ECGDex',t0Ms:RESULT.t0Ms,kind:'summary',ext:'csv'}), 'text/csv'); showOK('Summary CSV exported (tidy: metric · value · unit · range · status · notes).'); }
...
function _baevskyGeom(nn){
  ...
  let mn=Infinity, mx=-Infinity; const bins={};
  for(const v of nn){ if(!isFinite(v)) continue; if(v<mn)mn=v; if(v>mx)mx=v;
    const b=Math.round(v/50)*50; bins[b]=(bins[b]||0)+1; }
  let modeBin=0, modeCnt=0; for(const b in bins){ if(bins[b]>modeCnt){ modeCnt=bins[b]; modeBin=+b; } }
  return {
...
function exportWelltoryCSV(){
  ...
  showOK(multi
    ? ('HRVDex / Welltory-style CSV exported — '+recs.length+' session rows → load in HRVDex (they accumulate).')
    : 'HRVDex / Welltory-style CSV exported (1 session row → load in HRVDex).');
}
...
function sniffEcgKind(file){
  try {
    return file.slice(0, 600).text().then(head=>{
      const k = ING.sniffFirstLine(String(head).split(/\r?\n/)[0]);
      return (k===null || k==='ecg') ? 'ecg' : k;
    }).catch(()=>'ecg');
  } catch(_){ return Promise.resolve('ecg'); }
}
const _FOREIGN_LABEL = {
  ppg:'optical PPG', magn:'magnetometer', gyro:'gyroscope', acc:'accelerometer',
  spo2:'SpO₂ pulse-ox (→ OxyDex)', cgm:'glucose CGM (→ GlucoDex)',
  otherdevice:'other-device sensor', duplicate:'duplicate ECG', skip:'non-ECG stream'
};
...
async function loadFiles(fileList){
  ...
  [['rr',loadDeviceRR],['hr',loadDeviceHR],['acc',loadDeviceACC]].forEach(([kind,loader])=>{
    plan.companionLanes[kind].forEach(parts=>{
      if(parts.length===1) loader(parts[0]);        // unchanged single-file path
  ...
  if(_replaceMode && activeKey){ delete allRecordings[activeKey]; activeKey=null; _replaceMode=false; }
...
function reportSkipped(foreign, nEcg){
  ...
  if(RESULT && RESULT.deviceKey && foreign.length && foreign.every(f=>f.kind==='otherdevice')){
    showErr('Set aside '+summary+' — '+(foreign.length>1?'they don’t':'it doesn’t')+' match the loaded '+_deviceName(RESULT.deviceKey)+' recording. Drop the matching '+_deviceName(RESULT.deviceKey)+' sidecar, or load that device’s ECG first.');
    return;
  }
  if(foreign.some(f=>f.kind==='ppg')){
    showErr('No chest-strap ECG in this drop — these look like Polar Verity Sense optical files ('+summary+'). ECGDex needs a Polar H10 “…_ECG.txt” (last column “ecg [uV]”). For optical PPG / HRV, use PulseDex.');
  } else if(foreign.some(f=>f.kind==='spo2')){
    showErr('No raw ECG in this drop — set aside '+summary+'. That looks like a pulse-oximeter SpO₂ file → load it in OxyDex. ECGDex reads a Polar “…_ECG.txt” waveform (last column “ecg [uV]”).');
  } else {
...
function mergeFileParts(parts){
  return Promise.all(parts.map(f=>f.text())).then(texts=>{
    let text = texts[0];
    for(let i=1;i<texts.length;i++){ const lines=texts[i].split(/\r?\n/); lines.shift(); text += (text.endsWith('\n')?'':'\n')+lines.join('\n'); }
    const pk = DSP.partKey && DSP.partKey(parts[0].name);
    return new File([text], pk?pk.base:parts[0].name, { type:'text/plain' });
  });
}
function _processQueue(){ if(_loadQueue.length) loadECGFile(_loadQueue[0]); }
function recordingsSorted(){ return Object.values(allRecordings).sort((a,b)=>(a.t0Ms||0)-(b.t0Ms||0)); }

function renderRecSwitcher(){
  const list = recordingsSorted(), wrap = $('recSwitcher');
  if(!wrap) return;
  if(list.length <= 1){ wrap.style.display='none'; wrap.innerHTML=''; return; }
...
function selectRecording(key){
  const r = allRecordings[key]; if(!r) return;
  activeKey = key; RESULT = r;
  // restore this recording's own device-lane attachments (per-recording, not global)
  DEVICE_RR = r.deviceRR||null; DEVICE_HR = r.deviceHR||null; DEVICE_ACC = r.deviceACC||null; ACC_FS = r.accFs||null;
  renderAll(r);
  renderRecSwitcher();
  renderCrossNight();
}
...
function renderCrossNight(){
  const sec = $('crossNightSection'); if(!sec) return;
  const list = recordingsSorted();
  if(list.length < 2 || !window.ECGCross){ sec.style.display='none'; return; }
  sec.style.display='block';
  const CN = window.ECGCross, UI2 = window.ECGUI;
  ...
  metrics.forEach(m=>{
    ...
    const spark = UI2.lineChart(series.map(p=>({x:p.x,y:p.v})), col, { W:200,H:52, med:st.mean });
    ...
    rows += '<tr><td class="cn-metric fmt-m">'+evBadge(m.label)+m.label+'<span style="opacity:.5"> '+m.unit+'</span></td>'
      +'<td class="cn-spark">'+spark+'</td>'
      +'<td class="mono">'+st.mean+'<span style="opacity:.5"> ±'+st.sd+'</span></td>'
      +'<td class="mono">'+st.cv+'%</td>'
      +'<td class="mono">'+(st.slopePerDay==null?'—':(st.slopePerDay>0?'+':'')+st.slopePerDay)+'<span style="opacity:.5">/d</span></td>'
      +'<td class="mono">'+(st.tau==null?'—':st.tau)+' <span style="opacity:.5">p'+(st.p==null?'—':st.p)+'</span></td>'
      +'<td><span class="cn-trend '+tCol(st.trendLabel)+'">'+st.trendLabel+'</span></td>'
      +'<td class="mono"><span class="cn-z '+zCol+'">'+(st.zLatest==null?'—':(st.zLatest>0?'+':'')+st.zLatest+'σ')+'</span></td></tr>';
  ...
  $('crossNightTable').innerHTML = '<table class="cn-table" style="width:100%;border-collapse:collapse"><thead><tr>'
  ...
  $('crossNightHeadline').innerHTML = '<div class="cn-head-label">Newest recording vs baseline</div>'
    + (headline.length ? headline.slice(0,3).map(h=>'<div class="cn-head-item">'+h+'</div>').join('')
       : '<div class="cn-head-item dim">No metric is beyond ±1σ of its personal baseline — a consistent night.</div>');
  $('crossNightNote').innerHTML = list.length+' recordings · OLS slope vs date + non-parametric <b>Mann–Kendall</b> (τ, p) for short noisy series · personal-baseline <b>z-scores</b> · coverage-weighted by each night\'s analyzable %. The same <code>crossNight()</code> engine powers PpgDex &amp; OxyDex trends.';
}
...
function buildV2(r){
  ...
  const accEx = r._accEx || ((r.deviceACC && r.deviceACC.length && DSP.accExtras)
    ? DSP.accExtras(r.deviceACC, r.accFs, r.t0Ms, r.durSec, r.epochs, r.stages) : null);
  // per-5-min RRacc (median of high-confidence 30-s epochs) for timeseries alignment
  const accRespAt = {};
  if(accEx && accEx.rracc){
    r.epochs.forEach(e=>{ const w=accEx.rracc.filter(x=>x.conf==='high' && x.tStartMin>=e.tMin && x.tStartMin<e.tMin+5).map(x=>x.rr);
      if(w.length){ w.sort((a,b)=>a-b); accRespAt[e.tMin.toFixed(1)]=+w[w.length>>1].toFixed(1); } });
  }
  ...
  const out = {
    ...
    schema: {
      name:'ganglior.node-export', version:'2.0', node:'ECGDex', nodeVersion:'1.1',
      ...
      units:{ rr:'ms', hr:'bpm', power:'ms^2', sd:'ms', time:'s', tMin:'min', amplitude:'uV',
              interval:'ms', vo2:'ml/kg/min', age:'yr', ahi:'events/h', cvhr:'events/h',
              slope:'ln-units/h', burden:'%', conf:'0..1' }
    },
    recording:{ source:r.source, sampleRateHz:r.fs, durationSec:round(r.durSec,1), durationMin:r.durMin,
      tier:r.tier, tierNote:r.tierMsg, mode:r.mode, modeLabel:r.modeLabel, modeWhy:r.modeWhy, ambulatory:!!r.ambulatory, activityScore:r.activityScore,
      startEpochMs:r.t0Ms||null, beats:r.nBeats, epochs5min:r.epochs.length, longRecording:r.longRec },
    quality:{ analyzablePct:r.analyzablePct, cleanBeatPct:r.cleanBeatPct, coveragePct:r.coveragePct,
      activeMin:r.activeMin, spanMin:r.spanMin, gapMin:r.gapMin, gaps:r.nGaps, lowCoverage:!!r.lowCoverage,
      correctionRatePct:r.correctionRate, beatsCorrected:r.nCorrected, ectopicBeatsCorrected:r.nEctopyCorrected,
      correctionNote:'beats replaced by local median: low-SQI + out-of-range + ectopic (>20% from local median, Malik/Kubios). ectopicBeatsCorrected isolates the ectopy-only fixes — these clean rMSSD/pNN50 of PAC/PVC spikes so the HRV agrees with PulseDex/Kubios.',
      meanSQI:r.meanSQI, sqiNote:'kSQI · bSQI(±50ms) · RR-plausibility · flatline/rail · amplitude. Per-surge SQI rides on each Ganglior event as `sqi` (separate from `conf`, which now scales to CVHR surge magnitude).',
      meanSQICaveat: (r.meanSQI!=null && r.meanSQI<0.6) ? ('Mean SQI '+r.meanSQI+' is borderline (<0.6) — derived HRV/morphology are usable but lower-confidence; treat intervals & frequency metrics as directional and weight cross-node consensus accordingly.') : null,
    ...
    hrv:{
      time:{ hr:r.dispHr, meanRR:r.meanRR, medianRR:r.medianRR, sdnn:r.dispSd, rmssd:r.dispRm, pnn50:r.dispPn,
        nn50:r.nn50, cv:r.cv, minRR:r.minRR, maxRR:r.maxRR, q25RR:r.q25, q75RR:r.q75,
        sdann:r.sdann, sdnnIndex:r.sdnnIdx, lnRMSSD:r.lnrmssd, triangularIndex:r.triIdx,
        wholeRecordHR:r.hr, wholeRecordSDNN:r.sdnn, wholeRecordRMSSD:r.rmssd,
        amo50:_geom.amo50, mode:_geom.mode, mxDMn:_geom.mxDMn,
        units:'ms',
        geometricNote:'Baevsky-SI inputs (Brennan/Welltory convention): amo50 = amplitude of the modal RR (%), mode = modal RR (ms), mxDMn = RR variation range (SECONDS). Same values as the ⬇ HRVDex CSV columns; null when the NN series is empty.',
        windowNote:'sdnn/rmssd/pnn50 here are DISPLAY values = representative 5-min epoch median on overnight recordings (short recs: whole-record). For CROSS-NODE comparison use wholeRecordSDNN/wholeRecordRMSSD (whole-record) or sdnnIndex (mean of per-5-min SDNN).' },
      poincare:{ sd1:r.sd1, sd2:r.sd2, sd1sd2:r.sd1sd2, ellipseArea:r.ellArea,
        representativeWindow:r.poincareRep, representativeEpochIndex:r.poincareRepIdx??null,
      ...
      frequency:{ totalPower:r.tp, vlf:r.vlf, lf:r.lf, hf:r.hf, lfhf:r.lfhf, lfnu:r.lfnu, hfnu:r.hfnu,
        respRate:r.respRate, respRateEpochStats:r.respStats||null, method:'Lomb–Scargle',
      ...
      nonlinear:{ dfaAlpha1:r.dfa1, sampleEntropy:r.sampen, decelCapacity:r.dc, accelCapacity:r.ac,
        fragmentationPIP:r.pip, fragmentationIALS:r.ials, fragmentationPSS:r.pss,
        thresholds:{ fragmentationPIPflagPct:55, decelCapacityLowMs:4.5, decelCapacityNote:'DC<4.5 ms = elevated all-cause mortality marker (PRSA, Bauer 2006)',
    ...
    personalization:{
      ...
      hrMax:r.hrmaxEff??null, hrMaxSource:r.hrmaxRejected?'tanaka(entry-rejected)':(p.hrmax>0?'user':'tanaka'),
      vo2maxBase:r.vo2base??null, vo2maxAdjusted:r.vo2adj??null, vo2maxGroundTruth:r.vo2gt??null, altitudeFactor:r.altFactor??null,
      ...
      expectedRMSSDForAge:r.expRmssd??null, expectedRestHRForAge:r.expRHR??null
    },
    apnea: r.ambulatory ? {
        reportable:false,
        suppressedReason:(r.apneaSuppressed&&r.apneaSuppressed.suppressedReason)||'ambulatory — CVHR invalid under exercise',
        cvhrIndex:null, estimatedAHI:null, riskCategory:null, onCPAP:!!p.cpap,
        method:'CVHR/cardiopulmonary-coupling proxy (Hilmisson 2019) — WITHHELD: recording is ambulatory/awake-active, exercise HR dynamics read as cardiogenic oscillation. Mirrors the R5 null-model pattern (index withheld with a reason, never fabricated).'
      } : r.longRec?{
      cvhrIndex:r.cvhr.index, cvhrEvents:r.cvhr.events.length,
      estimatedAHI:r.estAHI?{ value:r.estAHI.value, range:[r.estAHI.lo,r.estAHI.hi], band:r.estAHI.band }:null,
      riskCategory:r.apneaRisk?r.apneaRisk.cat:null, onCPAP:!!p.cpap,
    ...
    hrvStability: r.hrvStab?{
      sigma_lnRMSSD_slope:r.hrvStab.sigma_lnRMSSD_slope, var_lnRMSSD_slope:r.hrvStab.var_lnRMSSD_slope,
      mean_lnRMSSD_slope:r.hrvStab.mean_lnRMSSD_slope, classification:r.hrvStab.classification,
      windows:r.hrvStab.nWindows, ref:'Li & Kiyono 2026 Sensors 26(4):1118 [CC BY 4.0]',
    ...
    cardiorespiratory: r.crc?{
      ...
      rsaEfficiencyRatio:r.crc.rsaEfficiencyRatio, rsaAmplitudeBpm:r.crc.rsaAmplitudeBpm,
      crcPLV:r.crc.crcPLV, couplingStrength:r.crc.couplingStrength,
      plvDuringSurges:r.crc.plvDuringSurges, plvBaseline:r.crc.plvBaseline,
      perEpochPLV:r.crc.epochCRC,
      method:'EDR (R-peak amplitude modulation) ⟷ RR coupling — ECG-only, no airflow/PPG',
      refs:{ rsaEfficiency:'Border et al. 2025 arXiv:2507.00597 (RSA minimises cardiac power; ~1.5× insp HR optimum)',
      ...
      interpretation:'rsaEfficiencyRatio = insp:exp HR ratio (higher = stronger RSA). crcPLV & couplingStrength ∈ [0,1]; both drop during CVHR/apnea clusters (plvDuringSurges < plvBaseline) → a CVHR confidence channel. All derived from existing pipeline outputs — zero new sensors.'
    }:null,
    morphology: r.morph?{
      ectopy:{ pvc:r.morph.nPVC, pac:r.morph.nPAC, pvcBurdenPct:r.morph.pvcBurden, pacBurdenPct:r.morph.pacBurden,
        couplets:r.morph.couplets, ventricularRunsGE3:r.morph.runsGE3, bigeminyCycles:r.morph.bigeminyCycles, medianQRSWidthMs:r.morph.medW },
      intervals: r.morph.delin.valid ? (function(){
        var dq=r.morph.delin.qrsDur, mw=r.morph.medW, sat=!!r.morph.delin.qrsSaturated;
        var qt=(r.morph.qtcTrend||[]); var nSat=qt.filter(function(w){return w.qrsSaturated;}).length;
        ...
        return { qrsDur:(mw!=null?mw:dq), qrsDurDelineated:dq, qrsDurSaturated:sat,
          ...
          qt:r.morph.delin.qt, qtcBazett:r.morph.delin.qtcBazett,
          qtcFridericia:r.morph.delin.qtcFrid, pr:r.morph.delin.pr, stLevel:r.morph.delin.st,
          rAmplitude:r.morph.delin.Ramp, tAmplitude:r.morph.delin.Tamp, pWaveResolved:r.morph.delin.pPresent,
          sampleGridMs:sampleMs,
          precisionNote:'Intervals reported in ms but resolved on a '+sampleMs+' ms sample grid ('+(r.fs||130)+' Hz) → ±1 sample (~'+sampleMs+' ms) uncertainty. Clinical QT analysis wants ≥250 Hz; treat as a within-subject directional trend, single lead ≠ 12-lead.' };
      })()
      ...
      tWaveAlternans: r.morph.twa ? { uv:r.morph.twa.uv, abnormal:r.morph.twa.abnormal, nBeats:r.morph.twa.nBeats, method:r.morph.twa.method,
        thresholdUv:47, note:'MTWA ≥47 µV flags repolarisation instability; co-travels with hypoglycemic QTc stress. Single-lead MMA screen, not the spectral clinical test.' } : null,
      afScreen:{ verdict:r.morph.af.verdict, flaggedWindowsPct:r.morph.af.suspiciousPct, deltaRREntropy:r.morph.af.shannon,
        note:'RR-irregularity screen only — P-wave weak at 130 Hz' }
    }:null,
    sleep: r.ambulatory ? { suppressed:true,
        suppressedReason:(r.sleepSuppressed&&r.sleepSuppressed.suppressedReason)||'high-activity / ambulatory',
        stages:null,
        method:'Cardiorespiratory staging WITHHELD — ambulatory/awake-active recording (a walk is not a sleep study). Field kept present + explicitly suppressed so consumers never hit a missing key.' }
      : r.longRec?(function(){
      var sm=r.stageMin||{}, tot=r.totSleep||0;
      var remF=tot>0&&sm.REM!=null?sm.REM/tot:null, deepF=tot>0&&sm.Deep!=null?sm.Deep/tot:null;
      ...
      var flags=[];
      if(remF!=null && (remF<0.10||remF>0.35)) flags.push('REM '+(remF*100).toFixed(0)+'% outside plausible 13–25%');
      if(deepF!=null && (deepF<0.05||deepF>0.30)) flags.push('Deep '+(deepF*100).toFixed(0)+'% outside plausible 13–23%');
      return { totalSleepMin:tot, stageMinutes:r.stageMin,
        ...
        confidence:'low', confidenceNote:'Single autonomic channel cannot resolve true sleep stages reliably; minute counts are directional. Reconcile against another node (see Integrator staging_disagreement) or PSG.',
        plausibility: flags.length? { ok:false, issues:flags } : { ok:true } };
    })():null,
    acc: accEx ? {
      sampleRateHz: accEx.accFs, durationMin: accEx.durMin,
      respiratoryRate: accEx.rraccSummary ? {
        mean: accEx.rraccSummary.mean, sd: accEx.rraccSummary.sd,
        highConfidencePct: accEx.rraccSummary.highPct, nEpochs30s: accEx.rraccSummary.nEpochs,
      ...
      edrAgreement: accEx.agreement ? {
        pairedEpochs: accEx.agreement.n, pearsonR: accEx.agreement.r, mae: accEx.agreement.mae,
        meanDelta: accEx.agreement.meanDelta, sdDelta: accEx.agreement.sdDelta,
        limitsOfAgreement: accEx.agreement.loa, disagreementRatePct: accEx.agreement.disagreeRate,
        verdict: (accEx.agreement.r>=0.85||(accEx.agreement.mae<=1.5&&Math.abs(accEx.agreement.meanDelta)<=1.5))?'strong'
               : (accEx.agreement.r>=0.65||accEx.agreement.mae<=3.0)?'moderate':'poor',
      ...
      sleepStageConsensus: accEx.consensus ? {
        consensusRatePct: accEx.consensus.rate, epochs: accEx.consensus.n, conflicts: accEx.consensus.nConflict,
      ...
      gait: accEx.gait ? (accEx.gait.walking ? {
        totalSteps: accEx.gait.totalSteps, bouts: accEx.gait.bouts.length,
        boutDetail: accEx.gait.bouts, activityZones: accEx.gait.zonePct, cadenceEpochs: accEx.gait.cadEpochs,
      ...
      } : { walking:false, totalSteps: accEx.gait.totalSteps||0,
            reason: accEx.gait.reason==='lowfs' ? ('ACC sample rate '+accEx.gait.accFs+' Hz < 7 Hz needed to resolve the 0.5–3.5 Hz step band') : 'fewer than 50 steps detected (sleep / still recording)' }) : null,
    ...
    timeseries:{
      ...
      epochs: r.epochs.map(e=>{ const o={ tMin:e.tMin, beats:e.n, hr:e.hr, meanRR:e.meanRR, rmssd:e.rmssd, sdnn:e.sdnn, pnn50:e.pnn, lf:e.lf, hf:e.hf, lfhf:e.lfhf, respRate:e.resp, position:e.position||'unknown' };
        if(r.morph && r.morph.qtcTrend){ let best=null,bd=1e9; for(const q of r.morph.qtcTrend){ const dd=Math.abs(q.tMin-e.tMin); if(dd<bd&&dd<=15){bd=dd;best=q;} } if(best) o.qtc=best.qtc; }
        const ar=accRespAt[e.tMin.toFixed(1)]; if(ar!=null) o.accResp=ar;
        return o; }),
      sleepStages: (r.longRec && !r.ambulatory)? r.stages.map(s=>({ tMin:s.tMin, stage:s.stage })) : null
    },
    ganglior_events:r.events.map(ev=>{ const {_sec, ...clean}=ev; return clean; }),
    reserved:{ doc:'Fields awaiting other fleet nodes/models; null until available.',
      osaTransformerLabels:null, osaTransformerRef:'Almarshad 2026 Front.AI 9:1727091 (Phase 2 model)',
      deltaSBP:null, deltaSBPSource:'BioZDex', glucoseCorrelation:null, glucoseSource:'GlucoDex' }
  };
  return out;
}
function exportJSON(){
  const list = recordingsSorted();
  if(list.length <= 1){
    const r = list[0] || RESULT; if(!r) return;
    ...
    const payload = {
      kernel:(window.DexKernel?{version:DexKernel.VERSION,hash:DexKernel.HASH}:null),
      schema:{ name:'ganglior.node-export', version:'2.0', node:'ECGDex', nodeVersion:'1.1', multiRecording:true,
        generated:new Date().toISOString(), provenance:(window.GangliorProvenance?GangliorProvenance.stamp():null),
        doc:'Array of per-recording v2.0 envelopes (each element is Integrator-readable unchanged) + a crossNight aggregate header.' },
      generated:new Date().toISOString(), recordingCount:list.length,
...
function exportGanglior(){
  ...
  const out = build ? build(r, {
    kernel:(window.DexKernel?{version:DexKernel.VERSION,hash:DexKernel.HASH}:null),
    provenance:(window.GangliorProvenance?GangliorProvenance.stamp():null),
    offsetMin:(r.offsetMin??null)
  }) : null;
  if(!out){ showErr('Ganglior export unavailable — ECGDSP.buildNodeExport not loaded.'); return; }
...
function resetAll(){
  ...
  RESULT=null; DEVICE_RR=null; DEVICE_HR=null; DEVICE_ACC=null; ACC_FS=null;
  allRecordings={}; activeKey=null; _loadQueue=[]; _replaceMode=false;
  { const w=$('recSwitcher'); if(w){ w.style.display='none'; w.innerHTML=''; } }
  { const c=$('crossNightSection'); if(c) c.style.display='none'; }
  ['scopeSection','qualitySection','chartsSection','morphSection','hypnoCard','stabilityCard','cvhrCard','crcCard','trendCard','valCard','hrCard','accCard','gangSection'].forEach(id=>{ const e=$(id); if(e) e.style.display='none'; });
  ['slKPI','slTbl'].forEach(id=>$(id).style.display='none');
  $('kpiGrid').classList.remove('show'); $('kpiGrid').innerHTML='';
  $('tblWrap').classList.remove('show'); $('tblBody').innerHTML='';
  ...
  ['rr','hr','acc'].forEach(w=>{ const st=$(w+'Status'), card=$(w+'Load');
    if(st){ st.textContent='No '+w.toUpperCase()+' file'; st.classList.remove('ok'); }
...
// ─── alerts / progress ───────────────────────────────────────────────────────
function progress(pct,msg){ $('prog').classList.add('show'); $('progBar').style.width=pct+'%'; $('proc').textContent=msg||''; }
function clearAlerts(){ ['aInfo','aOK','aErr'].forEach(id=>$(id).classList.remove('show')); }
function clearAlertsExceptInfo(){ ['aOK','aErr'].forEach(id=>$(id).classList.remove('show')); }
function showOK(m){ $('aOKmsg').textContent=m; $('aOK').classList.add('show'); setTimeout(()=>$('aOK').classList.remove('show'),6500); }
function showErr(m){ $('aErrmsg').textContent=m; $('aErr').classList.add('show'); }
...
function init(){
  ...
  // theme
  const tb = $('themeBtn');
  tb.addEventListener('click', ()=>{ document.body.classList.toggle('light'); tb.textContent = document.body.classList.contains('light')?'🌙 Dark':'☀️ Light'; if(SCOPE&&RESULT){ SCOPE.draw(); SCOPE.drawMini(); } });
  ...
  zone.addEventListener('drop', e=>{ e.preventDefault(); zone.classList.remove('drag'); const fs=e.dataTransfer.files; if(fs&&fs.length) loadFiles(fs); });
  $('genBtn').addEventListener('click', genSynthetic);
  $('replaceBtn').addEventListener('click', ()=>{ _replaceMode=true; input.click(); });
  { const ab=$('addRecBtn'); if(ab) ab.addEventListener('click', ()=>{ _replaceMode=false; input.click(); }); }

  // device cross-check loaders
  $('rrInput').addEventListener('change', e=>{ const f=e.target.files[0]; if(f) loadDeviceRR(f); });
  $('hrInput').addEventListener('change', e=>{ const f=e.target.files[0]; if(f) loadDeviceHR(f); });
  $('accInput').addEventListener('change', e=>{ const f=e.target.files[0]; if(f) loadDeviceACC(f); });
  ...
  $('spanSel').addEventListener('change', e=>{ if(SCOPE) SCOPE.setSpanSec(parseFloat(e.target.value)); });
  ...
  if(window.MetricRegistry){
    let stored=null; try{ stored=localStorage.getItem('dex_depth_tier'); }catch(e){}
  ...
  function setActive(id){ navItems.forEach(a=>a.classList.toggle('active', a.getAttribute('href')==='#'+id)); }
  navItems.forEach(a=>a.addEventListener('click',()=>setActive(a.getAttribute('href').slice(1))));
  const spy=new IntersectionObserver(es=>{ const vis=es.filter(e=>e.isIntersecting).sort((a,b)=>b.intersectionRatio-a.intersectionRatio); if(vis[0]) setActive(vis[0].target.id); }, { rootMargin:'-18% 0px -72% 0px', threshold:[0,.25,.5,1] });
