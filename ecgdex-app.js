...
`;
  let workerURL = null;
  // CLOCK-UNIFY: ONE parser. This used to be a hand-rolled mirror of the worker's `;
_ckPF`, and both
  // built `;
tMs` with a bare `;
Date.UTC(...)`. Date.UTC SILENTLY ROLLS out-of-range components onto a
  // plausible WRONG instant, which `
clock.js;
:_ckMk` exists to refuse (Clock Contract §2.7): a stamp
    ...
    morphology: r.morph
      ? {
          ...
          qtcTrendNote: r.morph.qtcTrend
            ? 'Per-15-min windowed median-beat QTc (Bazett). The cross-node feed GlucoDex lines up against overnight glucose — rising QTc as glucose falls is the beat-level hypoglycemia⟷repolarisation link. Within-subject trend, single lead ≠ 12-lead. A window whose T-end could not be delineated inside the median-beat window (`qtSaturated` — tangent extrapolating past the window edge, no repolarisation downslope, or a T peak on the truncated search boundary) is WITHHELD entirely rather than reported at the window edge, so a shorter trend means those windows were dropped, not that the night was quiet. Of the windows that remain, `unstable:true` deviates >60 ms from their median — a residual delineation artifact (lead-vector or T-end flip), NOT physiology; exclude them before reading a QTc trend or correlating against glucose.'
...
function resetAll() {
  ...
  ['scopeSection', 'qualitySection', 'chartsSection', 'morphSection', 'hypnoCard', 'stabilityCard', 'cvhrCard', 'crcCard', 'trendCard', 'valCard', 'hrCard', 'accCard', 'gangSection'].forEach((id) => {
    const e = $(id);
