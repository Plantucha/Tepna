...
const DSP=window.GLUDSP, UI=window.GLUUI;
let RESULT=null, SCOPE=null, ECGJSON=null, FUSION=null, LASTPARSED=null, NUTRITION=null;
let CORR={ levelSessions:false, deDrift:false };
let MEALS=[];
try{ MEALS=JSON.parse(localStorage.getItem('glucodex_meals')||'[]'); }catch(e){ MEALS=[]; }
function saveMeals(){ try{ localStorage.setItem('glucodex_meals',JSON.stringify(MEALS)); }catch(e){} }
...
function analyzeWithProfile(parsed, prog){
  ...
  if(p && p.calib && p.a1c>0){
    ...
    if(Math.abs(bias)>=1){ r=DSP.analyze(parsed, null, Object.assign({}, baseOpts, { biasOffset:bias })); }
  }
  return r;
}
...
function loadCSV(file){
  ...
  fr.onload=e=>{
    setTimeout(()=>{
      ...
      try{ if(typeof glucoClearReview==='function') glucoClearReview(); }catch(_gc){}
      var _j=null; try{ _j=JSON.parse(_txt); }catch(_pe){}
      if(_j && _j.schema && _j.schema.name==='ganglior.node-export'){
        ...
        if(_res && _res.ok){ if(typeof glucoRenderReview==='function') glucoRenderReview(_res); showOK('Loaded GlucoDex export \u2014 review mode (not recomputed).'); progress(0,''); $('prog').classList.remove('show'); return; }
        if(_res && _res.reason==='foreign-node'){ showErr(_res.message); progress(0,''); $('prog').classList.remove('show'); return; }
        // not a GlucoDex envelope kind \u2014 fall through to the CSV path (which reports if unreadable).
      }
      let parsed;
      try{ parsed=DSP.parseCSV(_txt); parsed.source='file'; }
...
function genSynthetic(){
  if(!window.DexPatientGen || !window.SYNTH || typeof SYNTH.renderGlucoAll!=='function'){
    showErr('Synthetic generator unavailable'); return;
  }
  const sel=$('genScenario').value;
  const daysEl=$('genDays'); const days=daysEl?+daysEl.value:7;
  ...
  if(!r){ showErr('Synthetic generator unavailable'); return; }
  ...
  setTimeout(()=>{
    try{
      ...
      const parsed=DSP.parseCSV(csv); parsed.source='synthetic';
      runPipeline(parsed);
    }catch(err){ showErr(err.message||String(err)); progress(0,''); $('prog').classList.remove('show'); }
  },30);
}
...
function runPipeline(parsed){
  ...
  setTimeout(()=>{
    let r;
    try{ r=analyzeWithProfile(parsed, progress); }
    catch(err){ showErr(err.message||String(err)); progress(0,''); $('prog').classList.remove('show'); return; }
    RESULT=r;
    // if a fusion JSON was already loaded, recompute against this recording
    if(ECGJSON){ try{ FUSION=computeFusion(r, ECGJSON); r.fusion=FUSION; }catch(e){ FUSION=null; } }
    ...
    showOK('Analyzed '+r.nReadings.toLocaleString()+' readings · '+(r.durDays>=1?r.durDays.toFixed(1)+' days':Math.round(r.activeMin/60)+' h')+' · '+r.pctActive+'% sensor active · GMI '+r.gmi+'%');
...
function _glucoFmtGen(g){ if(!g) return ''; try{ return String(g).replace('T',' ').replace(/\..*$/,'').replace(/Z$/,' UTC'); }catch(e){ return String(g); } }
function _glucoInjectReviewCSS(){
  ...
  var st=document.createElement('style'); st.id='gluco-selfingest-css'; st.textContent=css;
...
function glucoReviewTimeline(events){
  ...
  var nm=function(e){ return e.impulse==='dawn_surge'?'Dawn surge':(e.impulse==='nocturnal_hypo'?'Nocturnal hypo':(e.impulse==='glucose_excursion'?'Glucose excursion':(e.impulse||'event'))); };
  var h='<div class="grv-tl">'+shown.map(function(e){
    var when=e.t || '\u2014';
    return '<div class="grv-tlrow"><span class="tl-t">'+_gesc(when)+'</span><span>'+_gesc(nm(e))+'</span><span class="tl-conf">conf '+(e.conf!=null?e.conf:'\u2014')+'</span></div>';
  }).join('')+'</div>';
  if(evs.length>CAP) h+='<div class="grv-none">+ '+(evs.length-CAP)+' more events</div>';
  return h;
}
function glucoReviewView(review){
  var g=review.glucose||{}, rec=review.recording||{}, tir=g.tir||{};
  var prov=review.provenance||{}, bh=prov.buildHash||(review.derivedFrom&&review.derivedFrom.buildHash)||null, gen=_glucoFmtGen(prov.generated||review.generated);
  ...
  var h='<div class="grv-banner" role="status">'
...
function glucoRenderReview(review){
  ...
  if(!host){ host=document.createElement('section'); host.id='glucoReviewCard';
    var anchor=document.getElementById('scopeSection');
    if(anchor&&anchor.parentNode){ anchor.parentNode.insertBefore(host,anchor); }
    else { var m=document.querySelector('main')||document.body; m.insertBefore(host,m.firstChild); } }
  host.innerHTML=glucoReviewView(review); host.style.display='';
  try{ document.body.classList.add('has-data'); }catch(e){}
}
function glucoClearReview(){ var h=document.getElementById('glucoReviewCard'); if(h){ h.innerHTML=''; h.style.display='none'; } }
...
try{ if(typeof window!=='undefined' && window.GlucoDex){ window.GlucoDex.reviewView=glucoReviewView; window.GlucoDex.renderReview=glucoRenderReview; } }catch(_rvx){}
...
function renderAll(r){
  ...
  if(!SCOPE){ SCOPE=new UI.GlucoScope($('gluCanvas'),$('gluMini')); SCOPE.onView=updateScopeReadout; }
  ...
  $('sidebarDataInfo').innerHTML=(r.source==='synthetic'?'Synthetic CGM':'CGM file')+'<br>'+
    r.nReadings.toLocaleString()+' readings · '+r.cadence+'-min · GMI '+r.gmi+'%'+'<br>'+r.pctActive+'% sensor active';
}

function updateScopeReadout(view, cad, N, gT){
  const t0=gT[Math.max(0,Math.floor(view.start))], t1=gT[Math.min(N-1,Math.floor(view.start+view.span))];
  const spanH=view.span*cad/60;
  const fmt=ms=>{ const d=new Date(ms); return (d.getUTCMonth()+1)+'/'+d.getUTCDate()+' '+String(d.getUTCHours()).padStart(2,'0')+':'+String(d.getUTCMinutes()).padStart(2,'0'); };
  $('scopeReadout').innerHTML=`<span>window <b>${fmt(t0)} – ${fmt(t1)}</b></span><span>span ${spanH<24?spanH.toFixed(1)+' h':(spanH/24).toFixed(1)+' d'}</span><span>${(N*cad/1440).toFixed(1)} d total</span>`;
}
...
function renderContext(r){
  ...
  let notes='';
  if(r.tier==='partial') notes='<div class="ctx-note">⚠ Under 24 h — mean glucose, GMI, SD, CV and basic TIR are valid; MODD, dawn-trend and AGP percentile bands need more days and are withheld or flagged.</div>';
  else if(r.tier==='multi-day') notes='<div class="ctx-note">Full TIR/TAR/TBR, MAGE, CONGA, GVP and dawn-phenomenon are valid at this length. AGP percentile bands shown but firm up at ≥14 days (international consensus).</div>';
  ...
  let calibNote='';
  if(r.calib){
    if(r.calib.applied) calibNote=`<div class="ctx-note" style="color:var(--green)">🧪 <b>Lab-A1c calibration applied:</b> every reading shifted <b>${r.calib.appliedOffset>0?'+':''}${window.GluDisp.delta(r.calib.appliedOffset)} ${window.GluDisp.label()}</b> so the sensor mean matches the ${window.GluDisp.val(r.calib.labEAG)} ${window.GluDisp.label()} average your ${r.profile.a1c}% lab A1c implies (ADAG eAG = 28.7·A1c − 46.7). All metrics below reflect the corrected trace.</div>`;
    else if(r.calib.magnitude!=='small') calibNote=`<div class="ctx-note" style="color:var(--amber)">⚠ <b>Sensor bias detected:</b> your sensor mean (${window.GluDisp.val(r.calib.sensorMean)} ${window.GluDisp.label()}) sits <b>${window.GluDisp.spread(Math.abs(r.calib.bias))} ${window.GluDisp.label()} ${r.calib.bias>0?'below':'above'}</b> the ${window.GluDisp.val(r.calib.labEAG)} ${window.GluDisp.label()} your ${r.profile.a1c}% lab A1c implies — so GMI/eA1c may read ${r.calib.bias>0?'optimistically':'pessimistically'}. Tick <i>Calibrate trace to lab A1c</i> in your profile to correct it.</div>`;
  }
...
function renderKPI(r){
  ...
  if(r.gmiCheck) personalized.push({l:'GMI vs Lab', v:(r.gmiCheck.delta>0?'+':'')+r.gmiCheck.delta+'%', sub:'GMI '+r.gmi+' vs lab '+r.gmiCheck.lab, s:r.gmiCheck.agree?'ok':'warn'});
  if(r.calib){ if(r.calib.applied) personalized.push({l:'Calibrated', v:(r.calib.appliedOffset>0?'+':'')+window.GluDisp.delta(r.calib.appliedOffset), sub:window.GluDisp.label()+' · lab-A1c shift', s:'ok'}); else if(r.calib.magnitude!=='small') personalized.push({l:'Sensor bias', v:(r.calib.bias>0?'+':'')+window.GluDisp.delta(r.calib.bias), sub:window.GluDisp.label()+' vs lab A1c', s:'warn'}); }
  if(r.fusion) personalized.push({l:'IR-risk band', v:r.fusion.irBand, sub:'autonomic+glycemic · directional', s:r.fusion.irSev});
  if(r.fusion&&r.fusion.morph&&r.fusion.morph.qtc!=null) personalized.push({l:'QTc (ECGDex)', v:r.fusion.morph.qtc, sub:r.fusion.morph.hypoQtcRisk==='elevated'?'⚠ hypo⟷QTc risk':'ms · '+r.fusion.morph.qtcMethod, s:r.fusion.morph.hypoQtcRisk==='elevated'?'bad':r.fusion.morph.qtcProlonged?'warn':'ok'});
  const items=personalized.concat([
    ...
    {l:'Time Above', v:(r.tir.tar1+r.tir.tar2).toFixed(1)+'%', sub:window.GluDisp.cmp('>',180)+' · goal <25', s:(r.tir.tar1+r.tir.tar2)<25?'ok':(r.tir.tar1+r.tir.tar2)<40?'warn':'bad'},
    ...
    {l:'MAGE', v:r.mage==null?'—':window.GluDisp.spread(r.mage), sub:window.GluDisp.label()+' · excursions', s:r.mage==null?'neutral':r.mage<60?'ok':r.mage<100?'warn':'bad'},
    ...
    {l:'% Sensor active', v:r.pctActive+'%', sub:r.nGaps?(r.nGaps+' gap'+(r.nGaps===1?'':'s')+' · '+r.gapMin+' min'):'continuous', s:r.pctActive>=70?'ok':r.pctActive>=50?'warn':'bad'},
  ]);
  $('kpiGrid').innerHTML=items.map(k=>`<div class="kpi ${k.s}"><div class="kpi-label">${evBadge(k.l)}${k.l}</div><div class="kpi-val ${k.s}">${k.v}</div><div class="kpi-sub">${k.sub}</div></div>`).join('');
  $('kpiGrid').classList.add('show'); $('slKPI').style.display='flex';
}

function renderSessions(r){
  const card=$('sessionCard'); if(!card) return;
  const ss=r.sessions||[];
  if(ss.length<2){ card.style.display='none'; return; }   // only interesting across multiple wears
  ...
  const tl=$('togLevel'), td=$('togDrift');
  if(tl) tl.addEventListener('change',()=>{ CORR.levelSessions=tl.checked; reanalyzeMeals(true); });
  if(td) td.addEventListener('change',()=>{ CORR.deDrift=td.checked; reanalyzeMeals(true); });
}

function renderNutrition(r){
  const sec=$('nutSection'); if(!sec) return;
  const n=r.nutrition;
  if(!n){ sec.style.display='none'; return; }
  sec.style.display='block';
  if(n.matchedDays<3){
    $('nutBody').innerHTML=`<div class="q-note">${n.note||'Not enough overlapping days between the CGM and nutrition logs.'} <b>${n.matchedDays}</b> shared day${n.matchedDays===1?'':'s'} found — load logs that cover the same dates.</div>`;
    return;
  }
  ...
  const rows=n.matched.slice().sort((a,b)=>a.ms-b.ms).map(m=>`<tr>
    ...
    <td class="mono">${m.mean==null?'—':window.GluDisp.val(m.mean)}</td><td class="mono ${m.tir>=70?'ok':m.tir>=50?'warn':'bad'}">${m.tir==null?'—':m.tir+'%'}</td><td class="mono">${m.cv==null?'—':m.cv+'%'}</td></tr>`).join('');
  $('nutBody').innerHTML=`
...
function renderAGP(r){
  $('tirBody').innerHTML=UI.tirBar(r.tir)
...
function renderDaypart(r){
  const d=r.daypart; if(!d) return '';
  const parts=[['overnight','Overnight','00–06h'],['morning','Morning','06–12h'],['afternoon','Afternoon','12–18h'],['evening','Evening','18–24h']];
...
function renderPatterns(r){
  ...
  if(r.dawn.days&&r.dawn.days.length){
    ...
    dawnHtml=`<div class="mini-h" style="margin-top:4px">${evBadge('Dawn phenomenon')}Dawn rise per day <span class="mini-sub">nadir(03–06h) → pre-breakfast(06–08h) · median +${window.GluDisp.val(r.dawn.medianDelta)} ${window.GluDisp.label()}${r.dawn.present?' · flagged ≥'+window.GluDisp.tick(20):''}</span></div>`
      + UI.lineChart(pts, r.dawn.present?UI.COLORS.amber:UI.COLORS.teal, { W:680,H:130, ymn:0, med:20, xfmt:x=>'d'+(x+1) });
  }
  // nocturnal hypos
  const noct=r.nocturnalHypo.map(h=>`<div class="gang-ev bad"><span class="ge-t">${DSP.hhmm(h.startMs).slice(0,5)}</span><span class="ge-imp">nocturnal_hypo</span><span class="ge-meta">min ${window.GluDisp.val(h.min)} · ${h.durMin}m</span><span class="ge-conf">&lt;70</span></div>`).join('');
  // excursions summary
  const exc=r.excursions;
  const nAnn=exc.filter(e=>e.annotated).length;
  const excStream = exc.length? exc.slice(0,40).map(e=>{
    const cat=e.mealCat; const catCls=cat==='heavy'?'bad':cat==='medium'?'surge':'';
...
function renderDaily(r){
  ...
  const cad=r.cadence; const days=new Map();
  for(let i=0;i<r.series.N;i++){
    if(r.series.gF[i]===r.series.FLAG.WARMUP) continue;
    const d=new Date(r.series.gT[i]); const k=d.getUTCFullYear()+'-'+d.getUTCMonth()+'-'+d.getUTCDate();
...
function loadNutrition(file){
  const fr=new FileReader();
  fr.onload=e=>{
    let nut; try{ nut=DSP.parseNutrition(e.target.result); }catch(err){ showErr('Could not parse nutrition CSV: '+(err.message||err)); return; }
    NUTRITION=nut;
    const st=$('nutStatus');
    st.textContent='✅ '+nut.nDays+'-day '+(nut.shape==='servings'?('servings → '+nut.mealMarkers.length+' meal markers'):'daily summary');
    st.classList.add('ok'); $('nutLoad').classList.add('loaded');
    if(nut.shape==='servings' && nut.mealMarkers && nut.mealMarkers.length){
      ...
      saveMeals(); renderMealList();
    }
    if(LASTPARSED){ reanalyzeMeals(true);
      const n=RESULT&&RESULT.nutrition;
      showOK(nut.shape==='servings'?('Loaded '+nut.mealMarkers.length+' meal markers from Cronometer servings — excursions re-annotated.'):('Nutrition log matched '+(n?n.matchedDays:0)+' shared days — carbs ⟷ glycemia correlation below.'));
...
function loadECGJSON(file){
  const fr=new FileReader();
  fr.onload=e=>{
    let json; try{ json=JSON.parse(e.target.result); }catch(err){ showErr('Could not parse ECGDex JSON.'); return; }
    if(json.node!=='ECGDex' && !(json.schema&&json.schema.node==='ECGDex')){
      // tolerate but warn
    }
    ECGJSON=json;
    $('ecgJsonStatus').textContent='✅ '+(json.schema?('schema '+json.schema.version):(json.version?('v'+json.version):'loaded'))+(json.recording&&json.recording.startEpochMs?' · '+new Date(json.recording.startEpochMs).toLocaleDateString(undefined,{timeZone:'UTC'}):'');
    $('ecgJsonStatus').classList.add('ok'); $('ecgJsonLoad').classList.add('loaded');
    if(RESULT){ try{ FUSION=computeFusion(RESULT, json); RESULT.fusion=FUSION;
      RESULT.events=RESULT.events.filter(ev=>ev.node==='GlucoDex').concat(FUSION.events||[]);
      renderKPI(RESULT); renderFusion(RESULT); renderGanglior(RESULT); renderTable(RESULT);
      showOK('Fusion computed — autonomic-risk vector vs glycemic variability cross-validated. IR-risk band: '+FUSION.irBand+'.');
    }catch(err){ showErr('Fusion failed: '+(err.message||err)); } }
...
function regressLnRmssd(epochs){
  const xs=[],ys=[];
  for(const e of epochs){ const rm=e.rmssd; if(rm>0&&isFinite(rm)&&isFinite(e.tMin)){ xs.push(e.tMin); ys.push(Math.log(rm)); } }
...
function computeFusion(r, json){
  ...
  if(slope==null){ const s=regressLnRmssd(epochs); if(s!=null){ slope=s; slopeRecomputed=true; } }
  ...
  const prInt = mInt ? mInt.pr : null, qrsDur = mInt ? mInt.qrsDur : null, stLevel = mInt ? mInt.stLevel : null;
  const sex = (r.profile && r.profile.sex) || 'M';
  // QTc prolongation thresholds (consensus): >450 M / >460 F borderline, >470/>480 prolonged.
  const qtcBorderline = sex==='F'?460:450, qtcLong = sex==='F'?480:470;
  ...
  let hypoQtcRisk='none', hypoQtcNote=null;
  if(qtc!=null){
    if(nNocturnalHypo>0 && qtcProlonged){ hypoQtcRisk='elevated';
      hypoQtcNote='Nocturnal hypoglycemia ('+nNocturnalHypo+' episode'+(nNocturnalHypo>1?'s':'')+', nadir '+minNightGlucose+' mg/dL) coincides with a prolonged QTc ('+qtc+' ms). Hypoglycemia is a recognised QTc-prolonging trigger — this same-night coincidence is the arrhythmic-risk pattern behind "dead-in-bed". Directional, single-lead — confirm with a clinician/12-lead.'; }
    else if(qtcProlonged){ hypoQtcRisk='qtc-only';
      hypoQtcNote='QTc '+qtc+' ms is '+(qtc>qtcLong?'prolonged':'borderline')+' (threshold '+qtcBorderline+' ms for '+(sex==='F'?'F':'M')+') but no nocturnal hypos were measured this period — repolarisation watch, not a glucose-linked flag.'; }
    else if(nNocturnalHypo>0){ hypoQtcRisk='hypo-only';
      hypoQtcNote='Nocturnal hypos present but QTc ('+qtc+' ms) is within range — no repolarisation amplification seen on this (possibly different) night. Keep aligned-night data to watch the pair.'; }
    else { hypoQtcRisk='clear';
      hypoQtcNote='QTc '+qtc+' ms within range and no nocturnal hypos — the hypo⟷QTc arrhythmic pattern is absent.'; }
  }
  const morph = mInt ? { qtc, qtcMethod, qtcBorderline, qtcLong, qtcProlonged, pr:prInt, qrsDur, stLevel,
    pPresent: json.morphology&&json.morphology.intervals?json.morphology.intervals.pWaveResolved:null,
    hypoQtcRisk, hypoQtcNote, nNocturnalHypo, minNightGlucose } : null;
  ...
  if(morph){
    ...
    const gluAt=(ms)=>{ let best=null,bd=1e9; const cad=r.series.cadence*60000;
      const idx=Math.round((ms-r.series.gT[0])/cad); for(let k=Math.max(0,idx-2);k<=Math.min(r.series.N-1,idx+2);k++){ if(r.series.gF[k]===r.series.FLAG.WARMUP)continue; const d=Math.abs(r.series.gT[k]-ms); if(d<bd){bd=d;best=r.series.gV[k];} } return bd<=cad*3?best:null; };
    if(trend && trend.length>=4 && ecgStartMs){
      const qs=[],gs=[],pairs=[];
      for(const pt of trend){ const ms=ecgStartMs + (pt.tMin||0)*60000; const g=gluAt(ms); if(g!=null && pt.qtc!=null){ qs.push(pt.qtc); gs.push(g); pairs.push({ms,qtc:pt.qtc,glu:Math.round(g)}); } }
      if(qs.length>=4){ morph.qtcTrend={ n:qs.length, r:DSP.pearson(gs,qs), pairs, qtcRange:[Math.min(...qs),Math.max(...qs)],
  ...
  const parts=[]; const w=[];
  let slopeRisk=null;
  if(slope!=null){ slopeRisk=clamp(0.5 + slope*4, 0, 1); parts.push(slopeRisk); w.push(0.60); }    // rising slope = instability
  let surgeRisk=null;
  if(surgeEsc!=null){ surgeRisk=clamp(surgeEsc/60, 0, 1); parts.push(surgeRisk); w.push(0.20); }
  let couplingRisk=null;
  if(coupling!=null){ couplingRisk=clamp(1-coupling, 0, 1); parts.push(couplingRisk); w.push(0.20); } // low coupling = dysfunction
  ...
  const cvR=clamp((r.cv-25)/25,0,1), mageR=clamp(((r.mage||50)-40)/80,0,1), dawnR=clamp((r.dawn.present?r.dawn.medianDelta:0)/60,0,1);
  const glyVar=+(0.45*cvR + 0.35*mageR + 0.20*dawnR).toFixed(3);

  // ── directional IR-risk (recalibrated bands so a normal trace doesn't flag) ──
  let irScore, irBand, irSev;
  if(autoRisk!=null){ irScore=+(0.45*autoRisk + 0.55*glyVar).toFixed(3); }
  else irScore=+glyVar.toFixed(3);
  if(irScore<0.34){ irBand='Lower'; irSev='ok'; }
  else if(irScore<0.60){ irBand='Moderate'; irSev='warn'; }
  else { irBand='Higher'; irSev='bad'; }

  // ── concordance: do autonomic instability & glycemic variability point the same way? ──
  const autoHigh = autoRisk!=null && autoRisk>=0.55;
  const glyHigh = glyVar>=0.40;
  ...
  let nightOverlap=false, alignNote;
  if(ecgStartMs){
    const ecgDay=new Date(ecgStartMs); ecgDay.setHours(0,0,0,0);
  ...
  return {
    slope, slopeRecomputed, slopeRisk, surgeEsc, surgeRisk, coupling, couplingRisk, cvhrIdx,
    autoRisk, glyVar, irScore, irBand, irSev, concordance, nightOverlap, alignNote, nightCV,
    nocturnalGlucoseRisk, glucoseAutonomicCorrelation, ecgStartMs, morph,
...
function renderFusion(r){
  const f=r.fusion;
  if(!f){ $('fusionResult').style.display='none'; $('fusionEmpty').style.display='block'; return; }
  $('fusionEmpty').style.display='none'; $('fusionResult').style.display='block';
  const col={ok:UI.COLORS.green,warn:UI.COLORS.amber,bad:UI.COLORS.red}[f.irSev];
  // autonomic vector pills
  const vec=[];
  vec.push(`<div class="gang-pill"${f.slope!=null?` style="border-color:${f.slopeRisk>0.55?UI.COLORS.amber:UI.COLORS.dim}"`:''}><b>bσ(lnRMSSD) slope</b> ${f.slope!=null?(f.slope>0?'+':'')+f.slope+'/h':'—'}${f.slopeRecomputed?' · recomputed':''}</div>`);
  ...
  const scatter=UI.fusionScatter([{ x:f.autoRisk!=null?f.autoRisk:0.5, y:f.glyVar, label:'this night', col }], { xmn:0,xmx:1,ymn:0,ymx:1, xlab:'autonomic instability →', ylab:'glycemic variability →' });

  const concTxt={ 'concordant-elevated':'<b style="color:var(--status-concern)">Concordant — both elevated.</b> ECGDex\'s autonomic instability and GlucoDex\'s glucose variability agree, the stronger cross-node signal neither app sees alone.',
    ...
    'autonomic-input-missing':'Autonomic input unavailable in this export — showing the glycemic side only.' }[f.concordance];
  ...
  $('fusionBody').innerHTML=`
    ...
    ${f.morph?(()=>{ const m=f.morph; const rk={elevated:'bad','qtc-only':'warn','hypo-only':'warn',clear:'ok',none:'neutral'}[m.hypoQtcRisk];
      ...
      ${(()=>{ const t=m.qtcTrend, tw=m.twa;
        const twaPill = tw?`<div class="gang-pill" style="border-color:${tw.abnormal?UI.COLORS.red:UI.COLORS.green};color:${tw.abnormal?UI.COLORS.red:UI.COLORS.green}"><b>TWA ${tw.uv} µV</b>${tw.abnormal?' · abnormal':''}</div>`:'';
        if(t){ const pts=t.pairs.map((p,i)=>({x:i,y:p.qtc})); const gpts=t.pairs.map((p,i)=>({x:i,y:p.glu}));
          const rstr=t.r==null?'—':t.r; const rsev=t.r==null?'neutral':t.r<=-0.4?'bad':t.r<=-0.2?'warn':'ok';
...
function renderTable(r){
  ...
  const rows=[
    ...
    ['MAGE', r.mage==null?'—':window.GluDisp.spread(r.mage),window.GluDisp.label(),'—', r.mage==null?'neutral':r.mage<60?'ok':r.mage<100?'warn':'bad','Mean amplitude of excursions >1 SD'],
    ...
    ...(r.daypart?[
      ['CV · overnight', r.daypart.overnight.cv==null?'—':r.daypart.overnight.cv,'%','<36', r.daypart.overnight.cv==null?'neutral':r.daypart.overnight.cv<36?'ok':'warn','00–06h'],
      ...
      ['CV · afternoon', r.daypart.afternoon.cv==null?'—':r.daypart.afternoon.cv,'%','<36', r.daypart.afternoon.cv==null?'neutral':r.daypart.afternoon.cv<36?'ok':'warn','12–18h'],
    ...
    ['MAG', r.magRate==null?'—':window.GluDisp.spread(r.magRate),window.GluDisp.label()+'/h','—', r.magRate==null?'neutral':r.magRate<30?'ok':r.magRate<55?'warn':'bad','Mean absolute glucose rate of change'],
    ...
    ['Dawn phenomenon', r.dawn.present?'Present':(r.dawn.days&&r.dawn.days.length?'Not flagged':'—'), r.dawn.medianDelta!=null?'+'+window.GluDisp.val(r.dawn.medianDelta)+' '+window.GluDisp.label():'—','≥20 flags', r.dawn.present?'warn':'ok','Nadir(03–06h)→pre-breakfast rise'],
    ...
    ...(r.gmiCheck?[['GMI vs lab A1c', (r.gmiCheck.delta>0?'+':'')+r.gmiCheck.delta,'%','±0.5', r.gmiCheck.agree?'ok':'warn','GMI '+r.gmi+' vs your lab '+r.gmiCheck.lab+' — they measure differently']]:[]),
    ...(r.calib?[['Sensor bias vs lab A1c', (r.calib.bias>0?'+':'')+window.GluDisp.delta(r.calib.bias),window.GluDisp.label(),'±'+window.GluDisp.spread(8), r.calib.magnitude==='small'?'ok':'warn', 'sensor mean '+r.calib.sensorMean+' vs lab-implied eAG '+r.calib.labEAG+(r.calib.applied?' · CALIBRATION APPLIED ('+(r.calib.appliedOffset>0?'+':'')+r.calib.appliedOffset+')':'')]]:[]),
    ...(r.postprandial?[['— Postprandial —','','','','neutral','per meal marker · '+r.postprandial.length+' meals'],
      ...r.postprandial.map(m=>['PPGR · '+m.label, '+'+window.GluDisp.val(m.peakDelta), window.GluDisp.label()+' peak', '—', m.peakDelta<40?'ok':m.peakDelta<70?'warn':'bad', 'peak @'+m.timeToPeakMin+'min · +2h Δ'+(m.delta2h>0?'+':'')+m.delta2h+' · returned '+m.returnedPct+'% of '+m.nDays+'d'])
    ]:[]),
    ...(f?[
      ['— Fusion (ECGDex) —','','','','neutral', f.alignNote],
      ['Autonomic risk', f.autoRisk==null?'—':f.autoRisk,'0–1','—', f.autoRisk==null?'neutral':f.autoRisk<0.45?'ok':f.autoRisk<0.6?'warn':'bad','slope'+(f.slopeRecomputed?'(recomputed)':'')+' + surge + coupling'],
      ...
      ...(f.morph&&f.morph.qtc!=null?[
        ...
        ['Hypo⟷QTc risk', ({elevated:'Elevated','qtc-only':'QTc watch','hypo-only':'Hypo only',clear:'Clear',none:'—'})[f.morph.hypoQtcRisk],'—','Clear', f.morph.hypoQtcRisk==='elevated'?'bad':f.morph.hypoQtcRisk==='clear'?'ok':'warn','Nocturnal hypo + prolonged QTc = dead-in-bed pattern'],
      ]:[]),
...
//  EXPORTS
// ════════════════════════════════════════════════════════════════════════
function dl(content,name,type){ const a=document.createElement('a'); a.href=URL.createObjectURL(new Blob([content],{type:type+';charset=utf-8;'})); a.download=name; document.body.appendChild(a); a.click(); document.body.removeChild(a); URL.revokeObjectURL(a.href); }
...
function exportCleanCSV(){
  if(!RESULT) return; const r=RESULT, s=r.series;
  ...
  for(let i=0;i<s.N;i++){ const g=s.gV[i]; rows.push([ new Date(s.gT[i]).toISOString(), (g==null||!isFinite(g))?'':Math.round(g), (fnames[s.gF[i]]||'ok') ]); }
...
function exportJSON(){
  if(!RESULT) return; const r=RESULT, p=r.profile||{}, f=r.fusion;
  const out={
    ...
    recording:{ source:r.source, unit:'mg/dL (normalised internally)', sourceUnit:r.unit, cadenceMin:r.cadence,
      startEpochMs:r.t0Ms, durationDaysActive:r.durDays, spanMin:r.spanMin, activeMin:r.activeMin,
      tier:r.tier, tierNote:r.tierMsg, readings:r.nReadings, longRecording:r.longRec },
    quality:{ sensorActivePct:r.pctActive, gaps:r.nGaps, gapMin:r.gapMin, warmupMinSuppressed:r.warmupMin, compressionLowMin:r.compMin,
      dataQualityConfidence:r.dataQualityConf, dataQualityLabel:r.dqLabel,
      ...
      sessionCorrections:{ leveled:r.sessionCorr&&r.sessionCorr.leveled, deDrifted:r.sessionCorr&&r.sessionCorr.deDrifted, offsetsMgdl:r.sessionCorr?r.sessionCorr.offsets:[], globalMedian:r.sessionCorr?r.sessionCorr.globalMedian:null,
    ...
    glycemic:{ mean:r.mean, gmi:r.gmi, estA1c:r.ea1c, sd:r.sd, cv:r.cv,
      median:r.median, p10:r.p10, p25:r.p25, p75:r.p75, p90:r.p90, min:r.min, max:r.max,
      ...
      thresholds:{ veryLow:54, low:70, high:180, veryHigh:250, cvStablePct:36, tirGoalPct:70, tbrGoalPct:r.hypoGoal, note:'mg/dL · consensus 2019. tbrGoal tightens to 4% on insulin therapy.' },
      labels:{ gmi:'GMI is a lab-A1c PROXY, NOT your A1c. estA1c (ADAG) and GMI use different formulas and will differ.' } },
    variability:{ mage:r.mage, conga1h:r.conga1, conga2h:r.conga2, conga4h:r.conga4, modd:r.modd, gvp:r.gvp, jIndex:r.jIndex, lbgi:r.lbgi, hbgi:r.hbgi,
      mag:r.magRate, grade: r.grade?{ score:r.grade.score, hypoPct:r.grade.hypoPct, euPct:r.grade.euPct, hyperPct:r.grade.hyperPct }:null, adrr:r.adrr,
      byDaypart: r.daypart?{ totalCV:r.daypart.total, overnightCV:r.daypart.overnight.cv, morningCV:r.daypart.morning.cv, afternoonCV:r.daypart.afternoon.cv, eveningCV:r.daypart.evening.cv, windows:r.daypart.windows, note:'CV localised to time-of-day — where the swings live' }:null,
      formulas:{ jIndex:'0.001·(mean+SD)²', lbgi:'mean(10·f² for f<0), f=1.509·(ln(BG)^1.084 − 5.381)', hbgi:'mean(10·f² for f>0)', gvp:'(traceLength/flatLength − 1)·100', mag:'Σ|ΔBG| / hours', grade:'mean of 425·(log10(log10(BG_mmol))+0.16)², capped 50', adrr:'mean over days of (LRmax+HRmax) in Kovatchev risk space' },
      note:'MODD/ADRR null if <2 days. All on the full cleaned series — decimation is render-only.' },
    patterns:{ dawnPhenomenon:{ present:r.dawn.present, medianRiseMgdl:r.dawn.medianDelta??null, nDays:r.dawn.nDays??0, perDay:r.dawn.days||[], flagThresholdMgdl:20, note:'nadir(03–06h)→pre-breakfast(06–08h)' },
      nocturnalHypos:r.nocturnalHypo, excursions:r.excursions,
      mealMarkers: (r.mealMarkers&&r.mealMarkers.length)? r.mealMarkers.map(m=>({ timeOfDay:('0'+Math.floor(m.minOfDay/60)).slice(-2)+':'+('0'+m.minOfDay%60).slice(-2), category:m.category, label:m.label })) : null,
    ...
    personalization:{ profile:{ age:p.age??null, sex:p.sex??null, diabetesStatus:p.diab??null, therapy:p.therapy??null,
      ...
      gmiVsLabA1c: r.gmiCheck?{ gmi:r.gmiCheck.gmi, labA1c:r.gmiCheck.lab, deltaPct:r.gmiCheck.delta, agree:r.gmiCheck.agree, note:'validates the GMI proxy against the user lab value' }:null,
      sensorBiasCalibration: r.calib?{ labImpliedEAG:r.calib.labEAG, sensorMean:r.calib.sensorMean, biasMgdl:r.calib.bias, applied:r.calib.applied, appliedOffsetMgdl:r.calib.appliedOffset, magnitude:r.calib.magnitude,
        formula:'eAG = 28.7·labA1c − 46.7 (ADAG); bias = eAG − sensorMean; when applied, every reading is shifted by bias',
        note:'optional. Corrects a systematically low/high sensor against lab ground truth. +bias ⇒ sensor reads low. Informational; all metrics here reflect the corrected trace when applied.' }:null },
    fusion: f?{
      ecgInputs:{ usedSlopeScalar:f.inputs.usedSlope&&!f.slopeRecomputed, slopeRecomputedFromEpochs:f.slopeRecomputed,
        sigma_lnRMSSD_slope:f.slope, surgeEscalationPct:f.surgeEsc, couplingStrength:f.coupling, cvhrIndex:f.cvhrIdx, nEpochsRegressed:f.inputs.nEpochs },
      autonomicRiskVector:{ score:f.autoRisk, slopeRisk:f.slopeRisk, surgeRisk:f.surgeRisk, couplingRisk:f.couplingRisk,
        weights:{ slope:0.60, surgeEscalation:0.20, couplingInverse:0.20 },
      ...
      morphology: f.morph?{ qtcMs:f.morph.qtc, qtcMethod:f.morph.qtcMethod, qtcProlonged:f.morph.qtcProlonged, qtcThresholds:{ borderline:f.morph.qtcBorderline, prolonged:f.morph.qtcLong, note:'sex-specific consensus; single-lead median-beat estimate, directional' },
        prMs:f.morph.pr, qrsDurMs:f.morph.qrsDur, stLevelUv:f.morph.stLevel,
        hypoQtcRisk:f.morph.hypoQtcRisk, nocturnalHypos:f.morph.nNocturnalHypo, nadirMgdl:f.morph.minNightGlucose,
        ...
        trendAvailable: !!f.morph.trendAvailable, twaAvailable: !!f.morph.twaAvailable,
        ...
        note:'Consumes ECGDex 1.1 morphology.intervals. hypoQtcRisk=elevated when nocturnal hypoglycemia coincides with prolonged QTc (the dead-in-bed arrhythmic pattern). Directional, single-lead ≠ 12-lead.' }:null,
      irRisk:{ band:f.irBand, score:f.irScore, formula:'0.45·autonomicRisk + 0.55·glycemicVariability', note:'directional insulin-resistance-risk band, recalibrated bands — NOT a diagnosis, no dosing advice' },
      concordance:f.concordance, nightOverlap:f.nightOverlap, alignment:f.alignNote, nightCV:f.nightCV,
      producerSide:{ nocturnalGlucoseRisk:f.nocturnalGlucoseRisk, glucoseAutonomicCorrelation:f.glucoseAutonomicCorrelation,
    ...
    }:{ available:false, note:'No ECGDex export present at analysis time — single-node CGM run. Cross-signal autonomic⟷glycemic fusion (IR-risk, hypo-QTc) is computed by the Integrator when an overlapping ECGDex night is also loaded; this is not missing computation.' },
    timeseries:{ doc:'Per-hour AGP percentiles + per-day rollups + per-cell trace. cells[] makes this continuous node SLICEABLE: a consumer (the Integrator) recomputes glycemic metrics on any sub-window (e.g. a same-night ECG/Oxy overlap) without GlucoDex ever pre-segmenting into nights. One signal, one codex — windowing lives at the fusion layer.',
      cadenceMin:r.cadence, t0Ms:r.t0Ms, unit:'mg/dL',
      cells:(function(){ const s=r.series; if(!s||!s.N) return []; const W=s.FLAG?s.FLAG.WARMUP:1; const out=[];
        for(let i=0;i<s.N;i++){ if(s.gF[i]===W) continue;          // skip warm-up garbage; keep OK/GAP/COMPRESSION (consumer holds COMPRESSION out of stats)
          out.push({ tMs:s.gT[i], v:Math.round(s.gV[i]), f:s.gF[i] }); }   // absolute floating tMs straight from r.series.gT[i]
        return out; })(),
      cellsNote:'ONE entry per cell in time order; WARMUP cells dropped. tMs = absolute floating wall-clock ms (NOT reconstructed from a string); v = mg/dL; f = clean flag (0 OK · 2 gap-interp · 3 compression-low). Native cadence — no downsampling.',
      agpHourly:r.hourly, perDay:r.daily },
    nutrition: r.nutrition?{ matchedDays:r.nutrition.matchedDays, carbsField:r.nutrition.carbsKey,
      correlations:r.nutrition.corr||null,
      perDay:r.nutrition.matched?r.nutrition.matched.map(m=>({ date:m.date, netCarbs:m.netCarbs, carbs:m.carbs, sugars:m.sugars, fiber:m.fiber, energy:m.energy, glucoseMean:m.mean, tir:m.tir, cv:m.cv })):null,
      source:'Cronometer (date-matched)', note:r.nutrition.note||'Pearson r across shared days; directional, n small, day-totals miss meal timing. Confounded by activity/sleep/stress.' }:null,
    ganglior_events:r.events,
    reserved:{ doc:'Fields awaiting other fleet nodes; null until available.',
      autonomicInstabilitySlope: f?f.slope:null, autonomicSource:'ECGDex',
      desatCorrelation:null, desatSource:'OxyDex',
      mealMarkers:null, mealSource:'Integrator (v1 has no food log)' }
  };
...
function _summaryCSV(){
  ...
  const tier=label=>{ try{ const R=window.GlucoRegistry; if(!R||!R.idForLabel) return ''; const id=R.idForLabel(label); return (id&&R.REGISTRY[id]&&R.REGISTRY[id].evidence)||''; }catch(e){ return ''; } };
...
function exportSummaryCSV(){ if(!RESULT) return; dl(_summaryCSV(),exportName({node:'GlucoDex',t0Ms:RESULT.t0Ms,kind:'summary',ext:'csv'}),'text/csv'); showOK('Summary CSV exported (tidy: metric · value · unit · range · status · notes).'); }
function exportGanglior(){ if(!RESULT) return; const r=RESULT;
  ...
  dl(JSON.stringify(out,null,2),exportName({node:'GlucoDex',t0Ms:r.t0Ms,kind:'ganglior',ext:'json'}),'application/json;charset=utf-8'); showOK('Ganglior event stream exported ('+(r.events?r.events.length:0)+' events).'); }
function copyTable(){ const rows=document.querySelectorAll('#tblWrap table tr'); const txt=[...rows].map(r=>[...r.cells].map(c=>c.textContent.trim()).join('\t')).join('\n'); navigator.clipboard.writeText(txt).then(()=>showOK('Metrics table copied.')); }

function resetAll(){
  document.body.classList.remove('has-data');
  RESULT=null; ECGJSON=null; FUSION=null;
  ['scopeSection','qualitySection','agpSection','variSection','patternsSection','ppgrSection','dailySection','gangSection'].forEach(id=>{ const e=$(id); if(e) e.style.display='none'; });
  $('fusionResult').style.display='none'; $('fusionEmpty').style.display='block';
  ...
  $('ctxBanner').style.display='none'; $('exportBar').classList.remove('show');
  $('sidebarDataCard').style.display='none'; $('gluChip').classList.remove('show');
  $('mealCard').style.display='none'; LASTPARSED=null;
  NUTRITION=null; CORR={ levelSessions:false, deDrift:false };
  { const ns=$('nutStatus'); if(ns){ ns.textContent='No nutrition log'; ns.classList.remove('ok'); } $('nutLoad').classList.remove('loaded'); }
  { const sec=$('nutSection'); if(sec) sec.style.display='none'; }
  { const sc=$('sessionCard'); if(sc) sc.style.display='none'; }
  const st=$('ecgJsonStatus'); if(st){ st.textContent='No ECGDex JSON'; st.classList.remove('ok'); } $('ecgJsonLoad').classList.remove('loaded');
  if(window.GLUProfile) window.GLUProfile.hide();
  $('aInfo').classList.add('show'); clearAlertsExceptInfo();
}
function reRenderProfile(){ if(!RESULT) return; if(LASTPARSED){ reanalyzeMeals(true); return; } if(window.GLUProfile) window.GLUProfile.render(RESULT); renderKPI(RESULT); if(RESULT.fusion) renderFusion(RESULT); renderTable(RESULT); }

// ── meal markers ───────────────────────────────────────────────────────────────
function minOfDay(hhmm){ const [h,m]=hhmm.split(':').map(Number); return h*60+m; }
function fmtMin(mod){ return String(Math.floor(mod/60)).padStart(2,'0')+':'+String(mod%60).padStart(2,'0'); }
...
function addMeal(){
  const t=$('mealTime').value||'07:30', cat=$('mealCat').value, lbl=$('mealLabel').value.trim();
  MEALS.push({ minOfDay:minOfDay(t), category:cat, label:lbl||({light:'Snack',medium:'Meal',heavy:'Large meal'}[cat]) });
  $('mealLabel').value=''; saveMeals(); renderMealList(); reanalyzeMeals();
}
function seedMeals(){
  MEALS=[{minOfDay:7*60+30,category:'medium',label:'Breakfast'},{minOfDay:12*60+45,category:'medium',label:'Lunch'},{minOfDay:19*60+15,category:'heavy',label:'Dinner'}];
  saveMeals(); renderMealList(); reanalyzeMeals();
}
function reanalyzeMeals(silent){
  if(!LASTPARSED) return;
  let r; try{ r=analyzeWithProfile(LASTPARSED, null); }catch(e){ return; }
  RESULT=r;
  if(ECGJSON){ try{ FUSION=computeFusion(r, ECGJSON); r.fusion=FUSION; }catch(e){} }
  r.events=r.events.concat(FUSION&&FUSION.events?FUSION.events:[]);
  if(window.GLUProfile) window.GLUProfile.render(r);
  renderKPI(r); renderPatterns(r); renderVariability(r); renderPPGR(r); renderNutrition(r); renderSessions(r); renderGanglior(r); renderTable(r);
  if(SCOPE) SCOPE.setData(r);
  if(silent===true){
    if(r.calib&&r.calib.applied) showOK('Calibrated to lab A1c — trace shifted '+(r.calib.appliedOffset>0?'+':'')+r.calib.appliedOffset+' mg/dL to match your lab-implied average glucose.');
    return;
  }
...
function init(){
  ...
  (function(){
    ...
    if(!document.getElementById('gluUnitToggleCss')){
      const st=document.createElement('style'); st.id='gluUnitToggleCss';
      st.textContent='.unit-toggle{display:inline-flex;border:1px solid var(--border2,rgba(255,255,255,.14));border-radius:8px;overflow:hidden}'
    ...
    const wrap=document.createElement('div'); wrap.className='unit-toggle'; wrap.id='gluUnitToggle';
    wrap.setAttribute('role','group'); wrap.setAttribute('aria-label','Glucose display unit');
    wrap.title='Display unit — glucose is stored & computed in mg/dL either way';
    [['mgdl','mg/dL'],['mmol','mmol/L']].forEach(([u,txt])=>{
      const b=document.createElement('button'); b.type='button'; b.textContent=txt; b.dataset.u=u;
      b.className='unit-btn'+(window.GluDisp.unit===u?' on':''); wrap.appendChild(b);
    });
    wrap.addEventListener('click',e=>{ const b=e.target.closest('.unit-btn'); if(!b) return;
  ...
  zone.addEventListener('drop',e=>{ e.preventDefault(); zone.classList.remove('drag'); const f=e.dataTransfer.files[0]; if(f) loadCSV(f); });
  $('genBtn').addEventListener('click',genSynthetic);
  $('replaceBtn').addEventListener('click',()=>input.click());
  $('ecgJsonInput').addEventListener('change',e=>{ const f=e.target.files[0]; if(f) loadECGJSON(f); });
  $('nutInput').addEventListener('change',e=>{ const f=e.target.files[0]; if(f) loadNutrition(f); });
  $('mealAdd').addEventListener('click',addMeal);
  $('mealSeed').addEventListener('click',seedMeals);
  $('mealLabel').addEventListener('keydown',e=>{ if(e.key==='Enter') addMeal(); });
  renderMealList();
  ...
  $('spanSel').addEventListener('change',e=>{ if(SCOPE) SCOPE.setSpanMin(parseFloat(e.target.value)); });
  ...
  ['sec-input','heroTop','scopeSection','sec-profile','slKPI','qualitySection','agpSection','variSection','patternsSection','dailySection','fusionCard','gangSection','slTbl'].forEach(id=>{ const e=$(id); if(e) spy.observe(e); });
