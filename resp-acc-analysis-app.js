...
(function () {
  ...
  /* ── THE TWO FIGURES `papers/cpap-flow-reference.html` OWES (residue
     2026-09-20-sibling-papers-have-no-figure-source) ─────────────────────────────────────────────
     That paper carried ZERO images while its own status callout said it stays a DRAFT "until the
     figures are emitted into papers/figures/". The row correctly refused to call the item done for
     three papers when the tool drew for one — and named the gap as a missing CHART LAYER rather than
     a regeneration left undone. It is narrower than that: this page ALREADY COMPUTES both of the
     paper's plottable claims (§3.1 the reference's self-noise per night, §3.2 the clock offset and
     its drift model) and rendered them as TABLES ONLY, which is the same defect the page's own §1
     comment records about itself before 2026-08-06. So the figures are drawn from the data already
     on screen; nothing here re-derives a number the tables do not show.
     ⚠ The sibling `papers/effort-typing-null.html` gets NO figure and that is deliberate — see the
     note in this file's header table and the paper's own §Status: it is a negative result with two
     tables and no figure reference anywhere in its text, and its DRAFT condition is the corpus
     regeneration, not a missing image. Drawing one to fill a slot would be the fabricated close the
     row warned about. */
  function drawRefFloor(rows) {
    var cv = $('figRefFloor');
    if (!cv) return;
    var ctx = figBase(
      cv,
      'Reference self-noise — the floor no algorithm may claim to beat',
      rows.length + ' night(s): two estimators on the SAME flow signal, 60/median(period) vs a plain breath count'
    );
    if (!rows.length) return;
    var box = { l: 54, r: cv.width - 34, t: 52, b: cv.height - 52 };
    var d = rows.slice().sort(function (a, b) {
    ...
    box.xticks = [];
    for (var xi = 0; xi < d.length; xi++) box.xticks.push(xi);
    var sc = figAxes(ctx, box, -0.5, d.length - 0.5, 0, ymax, 'night (sorted by self-noise)', 'MAE med-vs-count (br/min)', function (v) {
      var rec = d[Math.round(v)];
      if (!rec) return '';
      var m = /(\d{4})(\d{2})(\d{2})/.exec(String(rec.name || ''));
      return m ? m[2] + '-' + m[3] : String(Math.round(v) + 1);
    ...
    /* THE MEDIAN IS THE FLOOR THE PAPER QUOTES — drawn as the rule, so a reader sees the claim and
       the spread it was taken from in one object instead of a single number in prose. */
    var my = Math.round(sc.Y(med)) + 0.5;
    ctx.strokeStyle = FIG.warn;
    ctx.setLineDash([5, 4]);
    ctx.beginPath();
    ctx.moveTo(box.l, my);
    ctx.lineTo(box.r, my);
    ...
    ctx.textAlign = 'right';
    ctx.textBaseline = 'bottom';
    ctx.fillText('median floor ' + med.toFixed(2) + ' br/min', box.r, my - 4);
    for (var i = 0; i < d.length; i++) figDot(ctx, sc.X(i), sc.Y(d[i].mae), 4.5, FIG.series);
    ctx.fillStyle = FIG.ink;
    ctx.textBaseline = 'bottom';
    ctx.textAlign = 'left';
  ...
  /* §3.2 — the recovered offsets against the fitted drift model. The TABLE says which nights are
     on-model; the figure says what "on-model" looks like, which is the whole validity argument
     (consistency with a LINEAR drift, never correlation magnitude). Nights with no lock are absent
     rather than plotted at zero — a fabricated point here would read as a perfect alignment. */
  function drawDrift(done, drift) {
    var cv = $('figDrift');
    if (!cv) return;
    var pts = [];
    for (var i = 0; i < done.length; i++) {
      var d = done[i];
      if (!d.lock || !isFinite(d.dayNum)) continue;
      var pred = drift ? drift.predict(d.dayNum) : Number.NaN;
      pts.push({ day: d.dayNum, off: d.lock.off, pred: pred, onModel: isFinite(pred) ? Math.abs(d.lock.off - pred) < 5 : false, name: d.name });
    }
    var ctx = figBase(
      cv,
      ...
      pts.length +
        ' locked night(s)' +
        (drift ? ' · fitted ' + drift.slopePerDay.toFixed(3) + ' s/day, residual SD ' + (isFinite(drift.residSD) ? drift.residSD.toFixed(2) + ' s' : '—') : ' · no drift fit')
    );
    if (!pts.length) return;
    var box = { l: 62, r: cv.width - 34, t: 52, b: cv.height - 52 };
    var xs = pts.map(function (p) {
      return p.day;
    });
    var on = pts.filter(function (p) {
      return p.onModel;
    });
    /* SCALE TO THE ON-MODEL NIGHTS, and say so: an off-model argmax can sit 80 minutes away (the
       comment in the clock table records −5163 … +4804 s on the real corpus), and letting one set the
       y-range collapses the 9-second spread the figure exists to show into a flat line. Off-model
       points outside the range are drawn clamped at the edge, in the warn colour, so they are still
       visible as "off the scale" rather than silently dropped. */
    var base = (on.length ? on : pts).map(function (p) {
      return p.off;
    });
    var lo = Math.min.apply(null, base);
    var hi = Math.max.apply(null, base);
    var pad = Math.max(2, (hi - lo) * 0.35);
    var xmin = Math.min.apply(null, xs) - 0.5;
    var xmax = Math.max.apply(null, xs) + 0.5;
    var sc = figAxes(ctx, box, xmin, xmax, lo - pad, hi + pad, 'day number', 'recovered offset (s)', null, function (v) {
      return v.toFixed(0);
    });
    if (drift) {
      ctx.strokeStyle = FIG.warn;
      ctx.lineWidth = 1.5;
      ctx.beginPath();
      ctx.moveTo(sc.X(xmin), sc.Y(drift.predict(xmin)));
      ctx.lineTo(sc.X(xmax), sc.Y(drift.predict(xmax)));
      ctx.stroke();
      ctx.lineWidth = 1;
    }
    var clampY = function (v) {
      var y = sc.Y(v);
      return Math.max(box.t, Math.min(box.b, y));
    };
    for (var k = 0; k < pts.length; k++) figDot(ctx, sc.X(pts[k].day), clampY(pts[k].off), 4.5, pts[k].onModel ? FIG.series : FIG.warn);
    ctx.fillStyle = FIG.ink2;
    ctx.font = FIG.mono;
    ctx.textAlign = 'left';
    ctx.textBaseline = 'top';
    ctx.fillText(on.length + ' on-model (|Δ| < 5 s) · ' + (pts.length - on.length) + ' off-model, clamped to the axis', box.l, box.t - 16);
  }
  ...
  async function run(files) {
    ...
    /* The figure is drawn from the SAME rows the table just rendered (residue
       2026-09-20-sibling-papers-have-no-figure-source) — one dataset, two renderings. */
    drawRefFloor(
      done
        .filter(function (nn) {
          return nn.selfAg;
        ...
        .map(function (nn) {
          return { name: nn.name, mae: nn.selfAg.mae };
        })
    );
    ...
    tc.innerHTML = '';
    var okLocks = 0;
    /* ONE LABELLING FOR ONE COLUMN. The no-lock branch used `d.name.slice(-22)` while the locked
       branch below stripped the prefix and suffix by pattern — two spellings of the same identity in
       the same table, and the magic 22 was off by one: `_ACC.txt` is 8 characters and the stamp 15,
       so it needs 23 and ate the leading digit of the year. Every no-lock night rendered as
       `0260611_173042_ACC.txt`. Measured 2026-09-05 on a 49-night run: 17 rows, a third of the table,
       could not be joined back to the file that produced them — which is exactly what the cohort
       manifest (`2026-09-02-papers-cohort-never-recorded`) exists to make possible. */
    var nightLabel = function (name) {
      return name.replace(/Polar_H10_\d+_/, '').replace('_ACC.txt', '');
    };
    for (i = 0; i < done.length; i++) {
      var d = done[i];
      if (!d.lock) {
        row(tc, [nightLabel(d.name), '—', '—', '—', '—', '—', '<span style="color:var(--red)">no lock</span>']);
        continue;
      }
  ...
  [
    ...
    /* The two `papers/cpap-flow-reference.html` embeds — named for the paper that uses them, the same
       convention `papers/figures/` already follows for the ACC paper's three. */
    ['dlRefFloor', 'figRefFloor', 'cpap-reference-self-noise.png'],
    ['dlDrift', 'figDrift', 'cpap-clock-drift.png']
  ].forEach(function (t) {
    var el = $(t[0]);
    if (el)
