...
(function (root) {
  ...
  function asciiTrim(buf, start, len) {
    return ascii(buf, start, len)
      .replace(/\u0000+$/, '')
      .trim();
  }
  function asciiInt(buf, start, len) {
    var s = asciiTrim(buf, start, len);
    var n = parseInt(s, 10);
    return isNaN(n) ? null : n;
  }
  function asciiNum(buf, start, len) {
    var s = asciiTrim(buf, start, len);
    var n = parseFloat(s);
    return isNaN(n) ? null : n;
  }
  ...
  function parseEdfClock(startdate, starttime) {
    ...
    // Clock Contract §2.7 (DEEP-AUDIT-II §12.3) — the `dd<=31` guard catches month 13 / hour 25 but LEAKS
    // calendar-invalid days: Date.UTC SILENTLY ROLLS Feb 30 → Mar 2, Apr 31 → May 1 onto a WRONG instant.
    // Reject any component out of range, then require the date to ROUND-TRIP (rejects Feb 30 / Apr 31…).
    if (mo < 1 || mo > 12 || dd < 1 || dd > 31 || hh > 23 || mi > 59 || ss > 59) return null;
    var dateAnchorMs = Date.UTC(year, mo - 1, dd);
    var _dd = new Date(dateAnchorMs);
    if (_dd.getUTCFullYear() !== year || _dd.getUTCMonth() !== mo - 1 || _dd.getUTCDate() !== dd) return null; // date rolled ⇒ invalid
  ...
  function readEDF(arrayBuffer) {
    ...
    function block(width) {
      var arr = [];
      for (var s = 0; s < ns; s++) arr.push(asciiTrim(buf, o + s * width, width));
      o += ns * width;
      return arr;
    }
    function blockNum(width) {
      var arr = [];
      for (var s = 0; s < ns; s++) arr.push(asciiNum(buf, o + s * width, width));
      o += ns * width;
      return arr;
    }
    function blockInt(width) {
      var arr = [];
      for (var s = 0; s < ns; s++) arr.push(asciiInt(buf, o + s * width, width));
      o += ns * width;
      return arr;
    }
    labels = block(16);
  ...
  function selfTest() {
    ...
    function ok(name, cond, extra) {
      cond ? pass++ : fail++;
      log.push((cond ? '✓ ' : '✗ ') + name + (extra && !cond ? ' — ' + extra : ''));
    }

    // 1) clock — floating, viewer-tz-independent
    var c = parseEdfClock('13.06.26', '23.14.33');
    ok('clock parses', !!c);
    ok('t0Ms is floating (getUTC* readback == as-written)', c && new Date(c.t0Ms).getUTCHours() === 23 && new Date(c.t0Ms).getUTCFullYear() === 2026 && new Date(c.t0Ms).getUTCDate() === 13);
    ...
    var ev = rec.annotations.filter(function (a) {
      return a.class !== 'Unclassified';
    });
  ...
  /* ── exports + CLI ──────────────────────────────────────────────────────── */
  /* ── THE SESSION-GROUPING RULE (CPAP-REAL-CORPUS §F4) — single-sourced here ──────────────────
     Lives in the shared EDF reader because BOTH surfaces that group ResMed files load it, and until
     now each had its OWN rule (EXPORT-PATH-UNREACHABLE-FOLLOWUPS-III):

       adapters/resmed-edf.js : ±60 s of the anchor AND a repeated type opens a NEW set  ← correct
       cpapdex-app.js         : a >15 MINUTE gap opens a new cluster, `files[type] = e`   ← lossy
                                with the comment "last-wins on duplicate type"

     The app's `last-wins` IS silent data loss, and the 15-minute window makes it fire far more often
     than the naive ±60 s cluster the §F4 note was written about. Measured over the real SD card —
     199 night folders, 1008 EDF files — the app rule silently DISCARDS 76 files across 16 nights
     (8.0 %), and the two rules disagree on session count on those same 16. Worst night 2026-04-23:
     the app sees 2 sessions where the rule sees 6, dropping 14 files.

     The `!byType[type]` clause is the load-bearing one: a second EVE/CSL inside the same minute is a
     SEPARATE therapy session, and overwriting it makes its scored apneas vanish with no error.
     Pure: names in, clusters out — no filesystem, no I/O, so it is unit-testable anywhere. */
  var _F4_RE = /^(\d{8})_(\d{6})_([A-Z0-9]+)\.edf$/i;
  function sessionStampOf(name) {
    var base =
      String(name || '')
        .split('/')
        .pop() || '';
    var m = _F4_RE.exec(base);
    if (!m) return null;
    var d = m[1],
      t = m[2];
  ...
  var api = {
    ...
    sessionStampOf: sessionStampOf,
    parseEdfClock: parseEdfClock,
    sampleTMs: sampleTMs,
    parseTAL: parseTAL,
