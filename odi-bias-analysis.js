...
(function () {
  ...
  function maxOf(arr, f) {
    ...
    for (var i = 0; i < arr.length; i++) {
      var v = f ? f(arr[i]) : arr[i];
      if (v > mv) mv = v;
    }
    return mv === -Infinity ? 0 : mv;
  }
  ...
  function pressFit(xs, ys) {
    ...
    return { n: n, sx: sx, sy: sy, mx: mx, my: my, sxx: sxx, sxy: sxy, slope: sxy / sxx, intercept: my - (sxy / sxx) * mx };
  }
  function looRMSE(xs, ys, kind) {
    ...
    if (!g) return null;
    var inSub = new Int32Array(n);
    for (i = 0; i < n; i++) inSub[i] = -1;
    ...
    for (k = 0; k < n; k++) {
      ...
      if (j >= 0) {
        ...
        var dxj = lx[j] - g.mx,
          hj = 1 / M + (dxj * dxj) / g.sxx;
        if (!(hj < 1)) continue;
        var mx2 = (g.sx - lx[j]) / (M - 1),
          my2 = (g.sy - ly[j]) / (M - 1);
  ...
  async function runSubjectA() {
    ...
    for (var i = 0; i < NIGHTS.length; i++) {
      ...
      try {
        ...
        console.warn('SubjectA night', cfg.n, e);
      }
    }
  ...
  function loadSynthetic() {
    ...
    rq.onerror = function () {
      setStatus('no cohort DB found', 'idle');
    };
    rq.onsuccess = function () {
      var db = rq.result;
      if (!db.objectStoreNames.contains('results')) {
        setStatus('cohort DB empty', 'idle');
        db.close();
        return;
      }
      var tx = db.transaction('results', 'readonly'),
        st = tx.objectStore('results'),
        added = 0;
        ...
        if (cur) {
          ...
          if (r && r.oxy && Array.isArray(r.oxy.calib)) {
            ...
            r.oxy.nights.forEach(function (nt, idx) {
              ...
              if (nt && nt.odi != null && truth != null) {
                if (addPoint('synthetic', r.pid + '·n' + (idx + 1), nt.odi, truth, nt.minSpo2, nt.durationMin, 'synthetic')) added++;
              }
            });
          }
          if (added >= SYNTH_CAP) {
            setStatus('synthetic: ' + added + ' nights (capped)', 'done');
            db.close();
            render();
            return;
          }
          ...
          setStatus('synthetic: ' + added + ' nights', 'done');
          db.close();
          render();
        }
      };
  ...
  async function onNsrrFiles(files) {
    for (var i = 0; i < files.length; i++) {
      ...
      if (/\.edf$/i.test(f.name)) NSRR_FILES[b].edf = f;
      else if (/\.xml$/i.test(f.name)) NSRR_FILES[b].xml = f;
    }
    var keys = Object.keys(NSRR_FILES).filter(function (k) {
      return NSRR_FILES[k].edf;
    });
    ...
    for (var k = 0; k < keys.length; k++) {
      ...
      try {
        ...
        if (rec.xml) opts.xmlText = await readTxt(rec.xml);
          ...
          var csvAhi = lookupAhi(keys[k]);
          if (csvAhi != null) {
            opts.scoredAHI = csvAhi.ahi;
            opts.ahiVar = csvAhi.via;
          }
        }
        var res = window.NSRR.analyzeRecord(opts);
        if (res.err) {
          console.warn('NSRR', keys[k], res.err);
          continue;
        }
        if (res.scoredAHI == null) {
          console.warn('NSRR', keys[k], 'no reference AHI (need XML or CSV)');
          continue;
        }
        if (addPoint('nsrr', keys[k], res.odi4, res.scoredAHI, res.minSpo2, res.durMin, res.ahiSource)) added++;
      } catch (e) {
        console.warn('NSRR', keys[k], e);
      }
    }
  ...
  function onAhiCsv(text) {
    ...
    if (!lines.length) {
      setStatus('empty CSV', 'idle');
      return;
    }
    var delim = lines[0].indexOf('\t') >= 0 ? '\t' : ',';
    var head = lines[0].split(delim).map(function (h) {
      return h.trim().replace(/^"|"$/g, '').toLowerCase();
    });
    var headerLooksReal = head.some(function (h) {
    ...
    if (headerLooksReal) {
      ...
      var idCol = -1;
      for (var a = 0; a < NSRR_ID_VARS.length && idCol < 0; a++) idCol = head.indexOf(NSRR_ID_VARS[a]);
      if (idCol < 0) idCol = 0;
      var ahiCol = -1,
        ahiVar = null;
      for (var v = 0; v < NSRR_AHI_VARS.length && ahiCol < 0; v++) {
        var ix = head.indexOf(NSRR_AHI_VARS[v]);
        if (ix >= 0) {
          ahiCol = ix;
          ahiVar = NSRR_AHI_VARS[v];
        }
      }
      if (ahiCol < 0) {
        setStatus('no known AHI column (looked for ahi_a0h4, poohi4, rdi4p…)', 'idle');
        return;
      }
      var n = 0;
      for (var r = 1; r < lines.length; r++) {
        var c = lines[r].split(delim);
        if (c.length <= Math.max(idCol, ahiCol)) continue;
        var id = c[idCol].trim().replace(/^"|"$/g, '');
        var ahi = parseFloat(c[ahiCol]);
        if (id && isFinite(ahi)) {
          AHI_CSV[id] = ahi;
          n++;
        }
    ...
    lines.forEach(function (ln) {
      ...
      var id = c[0].trim(),
        ahi = parseFloat(c[1]);
      if (id && isFinite(ahi)) {
        AHI_CSV[id.replace(/\.(edf|xml)$/i, '')] = ahi;
        cnt++;
      }
    });
  ...
  function axes(ctx, P, w, h, xmax, ymax, xlab, ylab) {
    ...
    for (var i = 0; i <= ticks; i++) {
      var xv = (xmax * i) / ticks,
        yv = (ymax * i) / ticks;
      var px = P + ((w - P - 12) * i) / ticks,
        py = h - P - ((h - P - 10) * i) / ticks;
  ...
  function render() {
    ...
    var biases = POINTS.map(function (p) {
      return p.odi4 - p.ahi;
    });
    var meanBias = mean(biases),
      sdBias = sd(biases);
    var severe = POINTS.filter(function (p) {
      return p.sev === 'severe';
    });
    var severeUnder = severe.length
      ? mean(
          severe.map(function (p) {
            return p.ahi - p.odi4;
          })
        )
      : null;
    ...
    drawScatter(fitScatter);
    drawBland(meanBias, sdBias);
    drawBySev();
    drawCorr(xs, ys);
    drawTable();
    drawSources();
  ...
  function drawScatter(fit) {
    ...
    var maxV = Math.max(
      10,
      Math.ceil(
        maxOf(POINTS, function (p) {
          return Math.max(p.odi4, p.ahi);
        }) / 10
      ) * 10
    );
    axes(ctx, P, w, h, maxV, maxV, 'reference AHI (events/h)', 'OxyDex ODI-4 (events/h)');
    var X = function (v) {
        return P + ((w - P - 12) * v) / maxV;
      },
      Y = function (v) {
        return h - P - ((h - P - 10) * v) / maxV;
      };
    // identity
    ...
    ctx.beginPath();
    ctx.moveTo(X(0), Y(0));
    ctx.lineTo(X(maxV), Y(maxV));
    ctx.stroke();
    ctx.setLineDash([]);
    // OxyDex ×1.1 surrogate (odi = ahi/1.1 → as ahi varies)
    ctx.strokeStyle = 'rgba(255,184,77,.5)';
    ctx.setLineDash([2, 3]);
    ctx.beginPath();
    ctx.moveTo(X(0), Y(0));
    ctx.lineTo(X(maxV), Y(maxV / 1.1));
    ctx.stroke();
    ctx.setLineDash([]);
    ...
    if (fit) {
      ...
      ctx.lineWidth = 1.8;
      ctx.beginPath();
      ctx.moveTo(X(0), Y(fit.intercept));
      ctx.lineTo(X(maxV), Y(fit.slope * maxV + fit.intercept));
      ctx.stroke();
    }
    ...
    dotsBatched(
      ...
      function (p) {
        return SEV_COLOR[p.sev] || '#888';
      },
      function (p) {
        return p.src === 'synthetic' ? 0.4 : 0.95;
      },
      function (p) {
        return p.src === 'synthetic' ? 2.2 : 4;
      }
    );
    ...
    (function () {
      ...
      POINTS.forEach(function (p) {
        if (p.src !== 'synthetic') (bySrc[p.src] || (bySrc[p.src] = [])).push(p);
      });
      ctx.globalAlpha = 1;
      ctx.lineWidth = 1;
    ...
    $('scatterLegend').innerHTML =
      'Dashed white = identity (perfect). Dotted amber = OxyDex shipped ×1.1 surrogate. Solid teal = OLS. Below-identity points = ODI-4 under-counts. ' +
  ...
  function drawBland(meanBias, sdBias) {
    var cv = $('bland'),
      ...
      P = 46;
    clearCanvas(ctx, w, h);
    var means = POINTS.map(function (p) {
      return (p.odi4 + p.ahi) / 2;
    });
    var diffs = POINTS.map(function (p) {
      return p.odi4 - p.ahi;
    });
    ...
    var pad = (dmax - dmin) * 0.1 || 2;
    dmin -= pad;
    dmax += pad;
    var X = function (v) {
      return P + ((w - P - 12) * v) / xmax;
    };
    var Y = function (v) {
      return h - P - ((h - P - 10) * (v - dmin)) / (dmax - dmin);
    };
    ctx.strokeStyle = 'rgba(255,255,255,.14)';
    ctx.beginPath();
    ctx.moveTo(P, 10);
    ...
    [0, 0.5, 1].forEach(function (f) {
      var xv = xmax * f;
      ctx.fillText(Math.round(xv), X(xv) - 6, h - P + 16);
    });
    // zero line
    ...
    if (meanBias != null) {
      ...
      ctx.lineWidth = 1.6;
      ctx.beginPath();
      ctx.moveTo(P, Y(meanBias));
      ctx.lineTo(w - 12, Y(meanBias));
      ctx.stroke();
      if (sdBias != null) {
        ctx.strokeStyle = 'rgba(255,107,122,.6)';
        ctx.setLineDash([5, 4]);
        [meanBias + 1.96 * sdBias, meanBias - 1.96 * sdBias].forEach(function (L) {
          ctx.beginPath();
          ctx.moveTo(P, Y(L));
          ctx.lineTo(w - 12, Y(L));
          ctx.stroke();
    ...
    dotsBatched(
      ...
      POINTS.map(function (p, i) {
        return { p: p, mx: means[i], dy: diffs[i] };
      }),
      function (q) {
        return X(q.mx);
      },
      ...
      function (q) {
        return SEV_COLOR[q.p.sev];
      },
      function (q) {
        return q.p.src === 'synthetic' ? 0.35 : 0.9;
      },
      function (q) {
        return q.p.src === 'synthetic' ? 2 : 3.6;
      }
    );
    ctx.globalAlpha = 1;
    ctx.fillStyle = '#aab8cc';
    ctx.fillText('mean(ODI-4, AHI)', w / 2 - 50, h - 6);
    ctx.save();
    ctx.translate(12, h / 2 + 30);
    ctx.rotate(-Math.PI / 2);
    ctx.fillText('ODI-4 − AHI', 0, 0);
    ctx.restore();
    $('blandLegend').innerHTML =
      'Mean bias <b style="color:#FFB84D">' +
      (meanBias != null ? meanBias.toFixed(1) : '—') +
      '/h</b>, 95% LoA ±' +
      (sdBias != null ? (1.96 * sdBias).toFixed(1) : '—') +
      '. Points trend more negative at higher mean → proportional under-count.';
  }
  ...
  function drawBySev() {
    ...
    var ratios = order.map(function (s) {
      var g = POINTS.filter(function (p) {
        return p.sev === s && p.ahi > 0;
      }).map(function (p) {
        return p.odi4 / p.ahi;
      });
      return { s: s, r: median(g), n: g.length };
    });
    ...
    ctx.stroke();
    var Y = function (v) {
      return h - P - ((h - P - 10) * Math.min(v, 1.2)) / 1.2;
    };
    ...
    ctx.fillStyle = '#6f8096';
    ctx.font = '11px ui-monospace,monospace';
    ctx.fillText('1.0 (perfect)', w - 100, Y(1) - 5);
    ratios.forEach(function (d, i) {
      var x = P + 16 + i * bw;
      if (d.r != null) {
        ctx.fillStyle = SEV_COLOR[d.s];
        ctx.fillRect(x, Y(d.r), bw - 20, h - P - Y(d.r));
        ctx.fillStyle = '#e6edf6';
        ctx.font = '12px ui-monospace';
        ctx.fillText(d.r.toFixed(2), x + 4, Y(d.r) - 6);
      }
      ctx.fillStyle = '#aab8cc';
      ctx.font = '11px ui-monospace';
      ctx.fillText(d.s + ' (' + d.n + ')', x, h - P + 16);
    });
  }
  ...
  function drawCorr(xs, ys) {
    ...
    var X = function (v) {
        return P + ((w - P - 12) * v) / maxX;
      },
      Y = function (v) {
        return h - P - ((h - P - 10) * v) / maxY;
      };
    dotsBatched(
      ctx,
      POINTS,
      ...
      function (p) {
        return SEV_COLOR[p.sev];
      },
      function (p) {
        return p.src === 'synthetic' ? 0.3 : 0.9;
      },
      function (p) {
        return p.src === 'synthetic' ? 2 : 3.6;
      }
    );
    ctx.globalAlpha = 1;
    // naive ×1.1
    ctx.strokeStyle = 'rgba(255,184,77,.7)';
    ctx.setLineDash([2, 3]);
    ctx.beginPath();
    ctx.moveTo(X(0), Y(0));
    ctx.lineTo(X(maxX), Y(maxX * 1.1));
    ctx.stroke();
    ctx.setLineDash([]);
    ...
    if (lin) {
      ...
      ctx.lineWidth = 1.8;
      ctx.beginPath();
      ctx.moveTo(X(0), Y(lin.intercept));
      ctx.lineTo(X(maxX), Y(lin.slope * maxX + lin.intercept));
      ctx.stroke();
    }
    // power correction
    ...
    if (pw) {
      ...
      ctx.lineWidth = 1.6;
      ctx.beginPath();
      for (var v = 0.5; v <= maxX; v += maxX / 80) {
        var py = pw.a * Math.pow(v, pw.b);
        if (v === 0.5) ctx.moveTo(X(v), Y(py));
        else ctx.lineTo(X(v), Y(py));
      }
      ctx.stroke();
    }
    var rNaive = looRMSE(xs, ys, 'naive'),
      rLin = looRMSE(xs, ys, 'linear'),
      rPw = looRMSE(xs, ys, 'power');
    $('corrTag').textContent = pw ? 'AHI≈' + pw.a.toFixed(2) + '·ODI^' + pw.b.toFixed(2) : 'model —';
    $('corrLegend').innerHTML =
      'LOO-RMSE (events/h): <b style="color:#FFB84D">naive ×1.1 ' +
      (rNaive != null ? rNaive.toFixed(1) : '—') +
      '</b> · ' +
      '<b style="color:#3DE0D0">linear ' +
      (rLin != null ? rLin.toFixed(1) : '—') +
      '</b> · <b style="color:#B98AFF">power ' +
      (rPw != null ? rPw.toFixed(1) : '—') +
      '</b>. Lower = better recalibration than the shipped surrogate.';
  }
  ...
  function drawTable() {
    ...
    POINTS.filter(function (p) {
      ...
      .forEach(function (p) {
        ...
        tr.innerHTML =
          '<td><span class="src-chip" style="background:' +
          SRC_COLOR[p.src] +
          '"></span>' +
          p.src +
          '</td><td class="muted">' +
          ...
          (p.odi4 - p.ahi < 0 ? '#FF6B7A' : '#39D98A') +
          '">' +
          (p.odi4 - p.ahi).toFixed(1) +
          '</td>' +
          '<td class="num">' +
          ratio +
          '</td><td style="color:' +
          ...
          '</td>' +
          '<td class="num">' +
          (p.minSpo2 != null ? p.minSpo2 : '—') +
          '</td><td class="num">' +
          (p.durMin != null ? Math.round(p.durMin) : '—') +
  ...
  function drawSources() {
    var counts = {};
    POINTS.forEach(function (p) {
      counts[p.src] = (counts[p.src] || 0) + 1;
    });
    $('sources').innerHTML =
      Object.keys(counts)
        .map(function (s) {
          var g = POINTS.filter(function (p) {
            return p.src === s && p.ahi > 0;
          });
          var f = ols(
            g.map(function (p) {
              return p.ahi;
          ...
          return (
            ...
            '<div style="display:flex;justify-content:space-between;font-size:12px"><span><span class="src-chip" style="background:' +
            SRC_COLOR[s] +
            '"></span>' +
            s +
            ...
            counts[s] +
            ' nights</span></div>' +
            '<div class="mono" style="font-size:10px;color:var(--text3);margin-top:2px">' +
            stat +
            '</div></div>'
          );
        })
        .join('') || '<div class="muted mono" style="font-size:11px">none yet</div>';
    var hasReal = POINTS.some(function (p) {
      return p.src === 'nsrr';
    });
    if (!hasReal && POINTS.length)
      $('sources').innerHTML +=
        '<div class="mono" style="font-size:10px;color:var(--amber);margin-top:8px;line-height:1.4">⚠ no NSRR points — synthetic/SubjectA prove the apparatus but are NOT a clinical claim. Add real PSG to publish.</div>';
  }
  ...
  function exportCsv() {
    ...
    POINTS.forEach(function (p) {
      rows.push([p.src, p.id, p.odi4, p.ahi, (p.odi4 - p.ahi).toFixed(2), p.ahi > 0 ? (p.odi4 / p.ahi).toFixed(3) : '', p.sev, p.minSpo2, p.durMin, p.ahiSrc]);
    });
    download(
      'odi-bias-results.csv',
      rows
        .map(function (r) {
          return r.join(',');
        })
  ...
  function exportStats() {
    ...
    var fit = ols(xs, ys),
      biases = POINTS.map(function (p) {
        return p.odi4 - p.ahi;
      });
    var bySev = {};
    ['none', 'mild', 'mod', 'severe'].forEach(function (s) {
      var g = POINTS.filter(function (p) {
        return p.sev === s && p.ahi > 0;
      });
      bySev[s] = {
    ...
    Object.keys(srcCounts).forEach(function (s) {
      ...
      perSource[s] = {
        ...
        severeMeanUnder: (function () {
          var sv = g.filter(function (p) {
            return p.sev === 'severe';
          });
          return sv.length
            ? mean(
  ...
  function exportFig() {
    var ids = ['scatter', 'bland', 'bysev', 'corr'],
      pad = 16,
      cols = 2;
    var c0 = $('scatter');
    var cw = c0.width,
    ...
    out.toBlob(function (b) {
      var a = document.createElement('a');
      a.href = URL.createObjectURL(b);
      a.download = 'odi-bias-figures.png';
      a.click();
    }, 'image/png');
  }
  ...
  function relCI(r2, n) {
    return n > 2 ? (1.96 * Math.sqrt((1 - r2) / (n - 2))) / Math.sqrt(r2) : Infinity;
  }
  function nForRel(r2, rel) {
    var r = Math.sqrt(r2);
    return Math.ceil(2 + Math.pow(1.96 / (rel * r), 2) * (1 - r2));
  }
  ...
  function setPowProg(f) {
    $('powBar').style.width = (f * 100).toFixed(1) + '%';
  }
  var POOL = [];
  ...
  function renderAnalytic() {
    ...
    $('sevVal').textContent = (pSev * 100).toFixed(0) + '%';
    var n10 = nForRel(r2, 0.1),
      n15 = nForRel(r2, 0.15),
      nFloor = nForRel(0.93, 0.1),
      nSevere = Math.ceil(35 / pSev);
    var target = Math.max(n10, nSevere);
    drawAnalytic(r2, n10, nFloor);
    $('powReport').innerHTML =
      '<b style="color:#3DE0D0;font-size:14px">Need ≈ ' +
      target +
      ' paired PSG nights</b><br>' +
      ...
      '% → <b>' +
      nSevere +
      '</b> total for ≥35 severe nights<br><br>' +
      '<span class="muted">~' +
      target +
      ' nights. Sleep-clinic cohorts (more severe) need fewer; community more. SHHS n≈5,800 is far past sufficient.</span>';
  ...
  function drawAnalytic(r2, knee, floorN) {
    ...
    var maxN = 400,
      ...
      lnMax = Math.log(maxN);
    var X = function (n) {
      return P + ((w - P - 14) * (Math.log(n) - lnMin)) / (lnMax - lnMin);
    };
    var Y = function (v) {
      return h - P - ((h - P - 12) * Math.min(v, maxY)) / maxY;
    };
    ctx.strokeStyle = 'rgba(255,255,255,.14)';
    ctx.beginPath();
    ctx.moveTo(P, 10);
    ...
    ctx.fillStyle = '#6f8096';
    ctx.font = '11px ui-monospace,monospace';
    [10, 20, 50, 100, 200, 400].forEach(function (n) {
      ctx.fillText(n, X(n) - 7, h - P + 16);
    });
    for (var i = 0; i <= 5; i++) {
      var yv = (maxY * i) / 5;
      ctx.fillText((yv * 100).toFixed(0) + '%', 6, Y(yv) + 4);
      ctx.strokeStyle = 'rgba(255,255,255,.05)';
      ctx.beginPath();
      ctx.moveTo(P, Y(yv));
      ctx.lineTo(w - 14, Y(yv));
      ctx.stroke();
    }
    ...
    ctx.beginPath();
    ctx.moveTo(P, Y(0.1));
    ctx.lineTo(w - 14, Y(0.1));
    ctx.stroke();
    ctx.setLineDash([]);
    ctx.fillStyle = '#888';
    ctx.fillText('±10% CI target', w - 132, Y(0.1) - 5);
    function curveLine(R, color, lw) {
      ctx.strokeStyle = color;
      ctx.lineWidth = lw;
      ctx.beginPath();
      for (var n = 8; n <= maxN; n += 2) {
        var v = relCI(R, n);
        if (n === 8) ctx.moveTo(X(n), Y(v));
        else ctx.lineTo(X(n), Y(v));
      }
      ctx.stroke();
    }
    ...
    ctx.setLineDash([4, 4]);
    ctx.beginPath();
    ctx.moveTo(X(knee), 10);
    ctx.lineTo(X(knee), h - P);
    ctx.stroke();
    ctx.setLineDash([]);
    ctx.fillStyle = '#FFB84D';
    ctx.font = '12px ui-monospace';
    ctx.fillText('n≈' + knee, X(knee) + 5, 24);
    ctx.fillStyle = '#B98AFF';
    ctx.font = '11px ui-monospace';
    ctx.fillText('R²0.93 floor (n≈' + floorN + ')', X(38), Y(relCI(0.93, 38)) - 8);
    ctx.fillStyle = '#3DE0D0';
    ctx.fillText('assumed R²' + r2.toFixed(2), X(150), Y(relCI(r2, 150)) - 8);
    ctx.fillStyle = '#aab8cc';
    ctx.font = '11px ui-monospace';
    ctx.fillText('paired nights n (log)', w / 2 - 50, h - 4);
    ctx.save();
    ctx.translate(13, h / 2 + 70);
    ctx.rotate(-Math.PI / 2);
    ctx.fillText('slope 95%-CI half-width (relative)', 0, 0);
    ctx.restore();
  }

  async function buildPool(target) {
    if (!window.SYNTH || !window.CohortGen) {
      $('powStat').textContent = 'synth not loaded';
      return false;
    }
    POOL = [];
    var seed = 0,
    ...
    while (POOL.length < target && guard < target * 6) {
      ...
      for (var k = 0; k < pat.nights.length && POOL.length < target; k++) {
        ...
        try {
          var clean = nt.files.oxyCSV
            .split('\n')
            .filter(function (ln, i) {
              return i === 0 || ln.indexOf(',--') < 0;
            })
            .join('\n');
      ...
      if (POOL.length % 10 < 2) {
        setPowProg(POOL.length / target);
        $('powStat').textContent = 'pool ' + POOL.length + '/' + target;
  ...
  function slopeOf(idx) {
    ...
    var mx = sx / n,
      ...
      sxy = 0;
    for (var j = 0; j < n; j++) {
      var dx = POOL[idx[j]].ahi - mx;
      sxx += dx * dx;
      sxy += dx * (POOL[idx[j]].odi4 - my);
    }
    return sxx ? sxy / sxx : null;
  }
  function sampleNoReplace(N) {
    var idx = [],
      used = {};
    while (idx.length < N) {
  ...
  async function runBootstrap() {
    $('runPow').disabled = true;
    $('powStat').textContent = 'building pool…';
    var ok = await buildPool(130);
    if (!ok) {
      $('powStat').textContent = 'pool too small';
      $('runPow').disabled = false;
      return;
    }
    var fullSlope = slopeOf(
      POOL.map(function (_, i) {
        return i;
      })
    );
    var maxN = Math.floor(POOL.length * 0.6);
    var Ns = [10, 15, 20, 30, 40, 50, 60, 75].filter(function (n) {
      return n <= maxN;
    });
    var B = 200,
      out = [];
    for (var s = 0; s < Ns.length; s++) {
      var N = Ns[s],
        slopes = [];
      for (var b = 0; b < B; b++) {
        var sl = slopeOf(sampleNoReplace(N));
        if (sl != null) slopes.push(sl);
      }
      slopes.sort(function (a, c) {
        return a - c;
      });
      var half = (quantile(slopes, 0.975) - quantile(slopes, 0.025)) / 2,
        med = quantile(slopes, 0.5);
      out.push({ N: N, rel: med ? half / med : null });
      $('powStat').textContent = 'bootstrap N=' + N;
      setPowProg(s / Ns.length);
      await new Promise(function (r) {
        setTimeout(r, 0);
      });
    }
    setPowProg(1);
    var knee = out.find(function (c) {
      return c.rel != null && c.rel < 0.1;
    });
    var cv = $('powCanvas'),
      ctx = cv.getContext('2d'),
      w = cv.width,
      ...
      maxY = 0.3;
    var X = function (n) {
      return P + ((w - P - 14) * (Math.log(n) - Math.log(8))) / (Math.log(maxN2) - Math.log(8));
    };
    var Y = function (v) {
      return h - P - ((h - P - 12) * Math.min(v, maxY)) / maxY;
    };
    out.forEach(function (c) {
      if (c.rel != null) {
        ctx.fillStyle = '#fff';
        ctx.beginPath();
        ctx.arc(X(c.N), Y(c.rel), 3, 0, 7);
        ctx.fill();
      }
    });
    ctx.fillStyle = '#fff';
    ctx.font = '11px ui-monospace';
    ctx.fillText('● bootstrap (real OxyDex, n=' + POOL.length + ')', X(55), Y(0.27));
    $('powStat').textContent = 'bootstrap knee≈' + (knee ? knee.N : '>' + maxN) + ' (vs R²0.93 analytic ' + nForRel(0.93, 0.1) + ')';
    $('runPow').disabled = false;
  ...
  $('nsrrFiles').onchange = function (e) {
    onNsrrFiles(e.target.files);
  };
  $('ahiCsv').onchange = function (e) {
    var f = e.target.files[0];
    if (f) readTxt(f).then(onAhiCsv);
  };
  $('dlCsv').onclick = exportCsv;
  $('dlStats').onclick = exportStats;
  $('dlFig').onclick = exportFig;
})();
