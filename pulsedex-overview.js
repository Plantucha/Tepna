...
function pxProfile() {
  ...
  if (DP) {
    ...
    return {
      age: n(p.age, 40),
      sex: p.sex === 'F' ? 'F' : 'M',
      weight: n(p.weight, 78),
      height: n(p.height, 176),
      sbp: n(p.sbp, 120),
      dbp: n(p.dbp, 78),
      vo2gt: detOr0('vo2'),
  ...
  const rows = welltoryData.rows
    .map((r) => {
      ...
      return {
        ...
        rmssd: parseFloat(r[iRm]),
        sdnn: parseFloat(r[iSd]),
        hrv: parseFloat(r[iHrv]),
        stress: parseFloat(r[iSt]),
        energy: parseFloat(r[iEn])
      };
    })
    .filter((r) => isFinite(r.rmssd))
    .sort((a, b) => (a.tMs || 0) - (b.tMs || 0)); // oldest→newest
...
const pxStd = (a) => {
  const m = pxMean(a);
  return Math.sqrt(pxMean(a.map((v) => (v - m) ** 2)));
};
const pxMedian = (a) => {
...
function renderHeroPx(r) {
  ...
  let ari = null;
  if (hist) {
    const last7 = hist.rows.slice(-7).map((x) => x.rmssd);
    if (last7.length >= 3) ari = +(r.dispRm / pxMean(last7)).toFixed(2);
  }
  ...
  if (hist && hist.rows.length >= 4) {
    const ln = hist.rows
      .slice(-14)
      .map((x) => Math.log(x.rmssd))
      .filter(isFinite);
    if (ln.length >= 4) {
      const xs = ln.map((_, i) => i);
      const mx = pxMean(xs),
        my = pxMean(ln);
      let nu = 0,
        de = 0;
  ...
  const srbScore = document.getElementById('srbScore'),
    srbNote = document.getElementById('srbNote'),
...
function renderHrvBenchPx(r) {
  ...
  const f = (lbl, sub, val, unit, cls) =>
    `<div class="proj-factor"><span>${evBadge(lbl)}${lbl} <span style="opacity:.55">${sub}</span></span><span class="pf-val cv-${cls}">${val != null && !isNaN(val) ? val + ' ' + unit : '—'}</span></div>`;
  const sdC = sd > 50 ? 'good' : sd > 35 ? 'warn' : 'bad';
  const pnC = pn >= 15 ? 'good' : pn >= 3 ? 'warn' : 'bad';
  const st = (lbl, val, unit, cls) =>
    `<div class="proj-stat ps-${cls}"><span class="ps-label">${evBadge(lbl)}${lbl}</span><span class="ps-val">${val != null && !isNaN(val) ? val : '—'}<span class="ps-unit">${unit}</span></span></div>`;
  host.innerHTML =
...
function renderProfileDerivedPx() {
  if (window._pxPanel) {
    window._pxPanel.refresh();
    return;
  } // unified panel owns derived now
  ...
  function vo2Perc(vo2, age, sex) {
    ...
    for (let j = 0; j < pts.length - 1; j++) {
      if (vo2 >= pts[j][0] && vo2 < pts[j + 1][0]) {
        const fr = (vo2 - pts[j][0]) / (pts[j + 1][0] - pts[j][0]);
        return Math.round(pts[j][1] + fr * (pts[j + 1][1] - pts[j][1]));
      }
    }
...
function renderKpiGridPx(r) {
  ...
  if (!g || !r) return;
  const hist = pxHistory();
  const prev = hist ? hist.prev : null;
  let ari = null,
    ariSub = '>1.05 = above base',
    sdz = null,
    sdzSub = 'vs 7d baseline';
  if (hist) {
    const r7 = hist.rows.slice(-7);
    if (r7.length >= 3) {
      ari = +(r.dispRm / pxMean(r7.map((x) => x.rmssd))).toFixed(2);
      const sd7 = r7.map((x) => x.sdnn);
      const sg = pxStd(sd7);
      sdz = sg ? +((r.dispSd - pxMean(sd7)) / sg).toFixed(2) : null;
    }
  }
  ...
  if (ari == null && r.windows && r.windows.length >= 6) {
    ...
    const early = pxMedian(w.slice(0, t).map((x) => x.rmssd)),
      late = pxMedian(w.slice(-t).map((x) => x.rmssd));
    if (early > 0) {
      ari = +(late / early).toFixed(2);
      ariSub = 'late/early rMSSD';
    }
  }
  if (sdz == null) {
    // population z-score: adult SDNN norm ≈ 50 ms, SD ≈ 16 (Task Force / Nunan 2010)
    sdz = +((r.dispSd - 50) / 16).toFixed(2);
    sdzSub = 'vs population norm';
  }
  ...
  g.innerHTML = K.map((k) => {
    ...
    if (k.delta) {
      const v = k.delta.d;
      const good = k.delta.inv ? v < 0 : v > 0;
      const cls = Math.abs(v) < 0.05 ? 'neutral' : good ? 'up' : 'down';
      const ar = Math.abs(v) < 0.05 ? '→' : v > 0 ? '↑' : '↓';
      dH = `<div class="kpi-delta ${cls}">${ar} ${Math.abs(v).toFixed(1)}${k.u || ''} vs prev</div>`;
    }
...
function loadProfile() {
  ...
  if (window.DexProfile) {
    try {
      ...
      if (window.DexProfile.renderPanel && !window._pxPanel) {
        window._pxPanel = window.DexProfile.renderPanel({
          node: 'pulsedex',
      ...
      if (!DexProfile.isPristine()) {
        ...
        set('profWeight', p.weight);
        set('profHeight', p.height);
        set('profSBP', p.sbp);
        set('profDBP', p.dbp);
        set('profVO2', man.vo2 != null ? man.vo2 : '');
        set('profHRmax', man.hrMax != null ? man.hrMax : '');
        set('profRHR', man.hrRest != null ? man.hrRest : '');
        set('profElev', p.elevation != null ? p.elevation : '');
      }
    } catch (e) {}
    renderProfileDerivedPx();
    return;
  ...
  let saved = null;
  try {
    saved = JSON.parse(localStorage.getItem('pulsedex_profile') || 'null');
  } catch (e) {}
  if (saved) {
    PX_PROFILE_KEYS.forEach((k) => {
...
function saveProfile() {
  ...
  try {
    localStorage.setItem('pulsedex_profile', JSON.stringify(o));
  } catch (e) {}
}
...
function toggleProfilePanel() {
  const body = document.getElementById('profileBody'),
    btn = document.getElementById('profileToggleBtn');
