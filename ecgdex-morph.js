...
(function (global) {
  ...
  function localMedRR(rr, k, half) {
    const seg = [];
    for (let j = Math.max(1, k - half); j <= Math.min(rr.length - 1, k + half); j++) {
      if (j !== k && rr[j] >= 300 && rr[j] <= 2000) seg.push(rr[j]);
    }
  ...
  function classifyBeats(int16, bp, fs, refIdx, rr, sqi) {
    ...
    const peakI = new Int32Array(n);
    for (let k = 0; k < n; k++) peakI[k] = Math.round(refIdx[k]);
    ...
    for (let k = 0; k < n; k++) {
      const i = peakI[k],
        s = Math.max(0, i - halfQ),
        e = Math.min(bp.length - 1, i + halfQ);
      let pk = 0;
      for (let j = s; j <= e; j++) {
        const a = Math.abs(bp[j]);
        if (a > pk) pk = a;
      }
      const thr = 0.25 * pk;
      let lo = i,
        hi = i;
      for (let j = i; j >= s; j--) {
        if (Math.abs(bp[j]) >= thr) lo = j;
        else break;
      }
      for (let j = i; j <= e; j++) {
        if (Math.abs(bp[j]) >= thr) hi = j;
        else break;
      }
    ...
    const tHalf = Math.round(0.07 * fs),
      tLen = 2 * tHalf + 1;
    const acc = new Float64Array(tLen);
    let nAcc = 0;
    ...
    for (let k = 2; k < n - 2; k++) {
      ...
      const i = peakI[k];
      if (i - tHalf < 0 || i + tHalf >= int16.length) continue;
      const b = int16[i - tHalf];
      for (let j = 0; j < tLen; j++) acc[j] += int16[i - tHalf + j] - b;
      nAcc++;
      if (nAcc >= 400) break;
    }
    const tmpl = new Float64Array(tLen);
    for (let j = 0; j < tLen; j++) tmpl[j] = acc[j] / (nAcc || 1);
    // normalize template
    let tm = mean(Array.from(tmpl)),
      ts = std(Array.from(tmpl)) || 1;
    ...
    for (let k = 0; k < n; k++) {
      const i = peakI[k];
      if (i - tHalf < 0 || i + tHalf >= int16.length) {
        corr[k] = 1;
        continue;
      }
      let s = 0,
        ss = 0;
      const win = new Float64Array(tLen);
      for (let j = 0; j < tLen; j++) {
        const v = int16[i - tHalf + j];
        win[j] = v;
        s += v;
        ss += v * v;
      }
      const m = s / tLen,
        sd = Math.sqrt(Math.max(1e-6, ss / tLen - m * m));
      let c = 0;
      for (let j = 0; j < tLen; j++) c += ((win[j] - m) / sd) * tN[j];
      corr[k] = c / tLen;
    }
    ...
    for (let k = 1; k < n - 1; k++) {
      ...
      if (prem < 0.84) {
        // clearly premature beat
        if (wide || lowCorr)
          types[k] = 'V'; // wide OR bizarre morphology → PVC
    ...
    return {
      ...
      longestRun,
      runsGE3,
      bigeminyCycles: bigemCycles,
      template: Array.from(tmpl),
      tHalf,
      nTemplate: nAcc
    };
  }
  ...
  function medianBeat(int16, fs, refIdx, rr, sqi, types) {
    const pre = Math.round(0.32 * fs),
      post = Math.round(0.46 * fs),
      L = pre + post + 1;
    const medRR = median(Array.from(rr).filter((v) => v >= 300 && v <= 2000));
    ...
    for (const sqiGate of [0.65, 0.5, 0.35, 0]) {
      ...
      for (let k = 2; k < refIdx.length - 2; k += stride) {
        ...
        let i = Math.round(refIdx[k]);
        if (i - pre < 0 || i + post >= int16.length) continue;
        // snap to the true raw R-peak (|max|) within ±30 ms — refIdx is on the band-passed signal
        let bi = i,
          bv = Math.abs(int16[i]);
        for (let j = Math.max(0, i - rWin); j <= Math.min(int16.length - 1, i + rWin); j++) {
  ...
  function tWaveAlternans(int16, fs, refIdx, rr, sqi, types) {
    ...
    const js = Math.round(0.08 * fs),
      je = Math.round(0.4 * fs),
      L = je - js; // ST-T window after R
    if (L < 4) return null;
    const medRR = median(Array.from(rr).filter((v) => v >= 300 && v <= 2000));
    let A = null,
      B = null,
  ...
  function delineate(mb) {
    ...
    var Qon = Math.max(1, R - qWin),
      qHit = false;
    {
      var run = 0;
      for (var i = R - 1; i >= Math.max(1, R - qWin); i--) {
        if (Math.abs(beat[i] - B) <= thr) {
          if (++run >= SUS) {
            Qon = Math.min(R, i + SUS);
            qHit = true;
            break;
          }
        } else run = 0;
    ...
    var Joff = Math.min(beat.length - 2, R + qWin),
      jHit = false;
    {
      var run2 = 0;
      for (var i2 = R + 1; i2 <= Math.min(beat.length - 2, R + qWin); i2++) {
        if (Math.abs(beat[i2] - B) <= thr) {
          if (++run2 >= SUS) {
            Joff = Math.max(R, i2 - SUS);
            jHit = true;
            break;
          }
        } else run2 = 0;
    ...
    if (medW && (qrsSaturated || Math.abs(ms(Joff - Qon) - medW) > 30)) {
      Qon = Math.max(1, R - Math.round(((medW * 0.4) / 1000) * fs));
      Joff = Math.min(beat.length - 2, R + Math.round(((medW * 0.6) / 1000) * fs));
    }
    const qrsDur = ms(Joff - Qon);
    if (qrsDur < 20 || qrsDur > 220) {
      // degenerate (flat) or implausible → withhold all intervals
      return {
        valid: false,
    ...
    const ts = Math.min(beat.length - 2, Joff + Math.round(0.04 * fs)),
      te = Math.min(beat.length - 2, Joff + Math.round(0.4 * fs));
    let Tpk = ts,
      Tdev = 0;
    for (let i = ts; i <= te; i++) {
      const d = Math.abs(beat[i] - B);
      if (d > Tdev) {
        Tdev = d;
        Tpk = i;
      }
    ...
    let Tend = Tpk,
      steep = 0,
      steepAt = Tpk;
    for (let i = Tpk; i < Math.min(beat.length - 2, Tpk + Math.round(0.2 * fs)); i++) {
      const sl = beat[i + 1] - beat[i];
      if (Math.sign(sl) !== Math.sign(Tamp) && Math.abs(sl) > Math.abs(steep)) {
        steep = sl;
        steepAt = i;
      }
    }
    ...
    const ps = Math.max(1, R - Math.round(0.28 * fs)),
      pe = Math.max(2, R - Math.round(0.07 * fs));
    let Ppk = ps,
      Pdev = 0;
    for (let i = ps; i <= pe; i++) {
      const d = Math.abs(beat[i] - B);
    ...
    let Pon = Ppk;
    for (let i = Ppk; i > ps; i--) {
      if (Math.abs(beat[i] - B) < 0.25 * Math.abs(Pamp)) {
        Pon = i;
        break;
      }
      Pon = i;
  ...
  function afScreen(rr, sqi, types) {
    ...
    for (let s = 0; s + W <= n; s += W) {
      const seg = [],
        ect = [];
      for (let j = s; j < s + W; j++) {
        if (sqi[j] >= 0.4 && rr[j] >= 300 && rr[j] <= 2000) {
          seg.push(rr[j]);
          ect.push(types[j]);
        }
      }
      ...
      for (let i = 1; i < seg.length; i++) {
        const b = Math.round((seg[i] - seg[i - 1]) / 25);
        bins[b] = (bins[b] || 0) + 1;
      }
      const tot = seg.length - 1;
      let H = 0;
  ...
  function analyze(int16, bp, fs, refIdx, rr, sqi) {
    ...
    let qtcTr = null,
      twa = null;
    try {
      qtcTr = qtcTrend(int16, fs, refIdx, rrArr, sqi, cls.types, 15, cls.medW);
    } catch (e) {
      qtcTr = null;
    }
    try {
      twa = tWaveAlternans(int16, fs, refIdx, rrArr, sqi, cls.types);
    } catch (e) {
      twa = null;
    }
