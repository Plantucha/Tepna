...
var ECG_REGISTRY = {
  ...
  rmssd:        { label:'rMSSD',          unit:'ms',     goodDirection:'up',   depth:'advanced', evidence:'validated',
                  cite:'RMSSD — short-term parasympathetic HRV (Task Force 1996)' },
  sdnn:         { label:'SDNN',           unit:'ms',     goodDirection:'up',   depth:'advanced', evidence:'validated',
                  cite:'SDNN — overall HRV over the analysis window (Task Force 1996)' },
  lnRMSSD:      { label:'ln rMSSD',       unit:'',       goodDirection:'up',   depth:'advanced', evidence:'validated',
                  cite:'Log-RMSSD — readiness-friendly parasympathetic HRV scale' },
  qtc:          { label:'QTc',            unit:'ms',     goodDirection:'down', depth:'advanced', evidence:'validated',
                  cite:'Rate-corrected QT (Bazett) — ventricular repolarisation marker' },
  sd1:          { label:'SD1',            unit:'ms',     goodDirection:'up',   depth:'research', evidence:'validated',
                  cite:'Poincaré SD1 — short-term HRV (≈ RMSSD/√2)' },
  sd2:          { label:'SD2',            unit:'ms',     goodDirection:'up',   depth:'research', evidence:'validated',
                  cite:'Poincaré SD2 — long-term HRV dispersion' },

  /* ── MEASURED — direct sensor reading / raw statistic of the signal ─────── */
  hr:           { label:'Mean HR',        unit:'bpm',    goodDirection:'down', depth:'basic',    evidence:'measured',
                  cite:'Mean heart rate — direct from detected R-peaks' },
  steps:        { label:'Total steps',    unit:'',       goodDirection:'up',   depth:'basic',    evidence:'measured',
                  cite:'Accelerometer step count — direct' },
  analyzable:   { label:'% Analyzable',   unit:'%',      goodDirection:'up',   depth:'basic',    evidence:'measured',
                  cite:'Fraction of the recording that is analyzable — direct coverage' },
  coverage:     { label:'Coverage',       unit:'%',      goodDirection:'up',   depth:'advanced', evidence:'measured',
                  cite:'On-body recording coverage — direct' },
  correction:   { label:'Correction',     unit:'%',      goodDirection:'down', depth:'advanced', evidence:'measured',
                  cite:'Beats corrected during cleaning — direct quality statistic' },
  meanSqi:      { label:'Mean SQI',       unit:'',       goodDirection:'up',   depth:'advanced', evidence:'measured',
                  cite:'Mean signal-quality index — direct per-beat quality' },
  ectopy:       { label:'Ectopy',         unit:'',       goodDirection:'down', depth:'advanced', evidence:'measured',
                  cite:'Detected ectopic-beat count (PVC + PAC) — direct classification' },

  /* ── EMERGING — published, less standardized / device-dependent ─────────── */
  dfaAlpha1:    { label:'DFA α1',         unit:'',       goodDirection:'up',   depth:'research', evidence:'emerging',
                  cite:'Detrended-fluctuation short-term scaling exponent — device-dependent' },
  cvhrIndex:    { label:'CVHR index',     unit:'/h',     goodDirection:'down', depth:'advanced', evidence:'emerging',
                  cite:'Cyclical-variation-of-HR index (Hayano) — oximetry/ECG apnea surrogate' },
  decelCapacity:{ label:'Decel. capacity',unit:'ms',     goodDirection:'up',   depth:'research', evidence:'emerging',
                  cite:'Heart-rate deceleration capacity (Bauer 2006) — vagal/mortality marker' },
  respRate:     { label:'Resp Rate',      unit:'br/min', goodDirection:'down', depth:'advanced', evidence:'emerging',
                  cite:'ECG-derived respiration (EDR) — surrogate, not a flow sensor' },
  sd1sd2:       { label:'SD1/SD2',        unit:'',       goodDirection:'up',   depth:'research', evidence:'emerging',
                  cite:'Poincaré SD1/SD2 ratio — nonlinear short/long balance' },
  ellArea:      { label:'Ellipse area',   unit:'ms²',    goodDirection:'up',   depth:'research', evidence:'emerging',
  ...
  crCoupling:   { label:'CR Coupling',    unit:'',       goodDirection:'up',   depth:'research', evidence:'emerging',
                  cite:'Cardiorespiratory phase-locking value (PLV) — coupling strength' },
  lfhf:         { label:'LF/HF',          unit:'',       goodDirection:'up',   depth:'research', evidence:'emerging',
                  cite:'LF:HF power ratio — sympatho-vagal balance proxy' },

  /* ── EMERGING — companion-accelerometer cross-checks (ACC sub-cards) ─────── */
  rraccRate:    { label:'ACC Resp Rate',  unit:'br/min', goodDirection:'down', depth:'advanced', evidence:'emerging',
                  cite:'Respiration from chest-axis accelerometer FFT (0.15–0.45 Hz) — device-dependent surrogate' },
  edrAgreement: { label:'RRacc–EDR Agreement', unit:'', goodDirection:'up',   depth:'advanced', evidence:'emerging',
  ...
  edrDisagree:  { label:'EDR Disagreement', unit:'%', goodDirection:'down', depth:'advanced', evidence:'heuristic',
  ...
  stageConsensus:{ label:'Stage Consensus', unit:'%',    goodDirection:'up',   depth:'advanced', evidence:'heuristic',
  ...
  afScreen:     { label:'AF Screen',      unit:'',       goodDirection:'down', depth:'advanced', evidence:'experimental',
                  cite:'Atrial-fibrillation irregularity screen — directional only, not diagnostic' },
  hrvStability: { label:'HRV Stability',  unit:'',       goodDirection:'up',   depth:'research', evidence:'experimental',
                  cite:'Overnight ln-RMSSD stability slope — ECGDex composite' },
  rsaEfficiency:{ label:'RSA Efficiency', unit:'',       goodDirection:'up',   depth:'research', evidence:'experimental',
                  cite:'Inspiratory:expiratory HR ratio — cardiorespiratory composite' },
  ...
  sampen:     { label:'SampEn',          unit:'',      goodDirection:'up',   depth:'research', evidence:'experimental', cite:'Sample entropy (m=2, r=0.2·SD) — ECGDex-tuned application, no published normal band at these settings' },
...
var ECG_LABEL_ALIAS = {
  'rmssd':'rmssd', 'rmssd (med)':'rmssd', 'rmssd (median)':'rmssd',
  'sdnn':'sdnn', 'sdnn (med)':'sdnn', 'sdnn (median)':'sdnn',
  ...
  'resp rate':'respRate',
  'sd1':'sd1', 'sd2':'sd2', 'sd1/sd2':'sd1sd2', 'ellipse area':'ellArea',
  'cr coupling':'crCoupling', 'rsa efficiency':'rsaEfficiency', 'lf/hf':'lfhf',
  'beats (nn)':'beatsNN', 'beats':'beatsNN',
  'mean rr':'meanRR', 'median rr':'medianRR', 'min rr':'minRR', 'max rr':'maxRR',
  'nn50':'nn50', 'cv':'cv',
  'qrs duration':'qrs', 'qrs':'qrs', 'qt':'qt', 'pr interval':'pr', 'st level':'stLevel',
  'r amplitude':'rAmp', 't amplitude':'tAmp',
  'pvcs (v)':'pvc', 'pacs (s)':'pac', 'couplets':'couplets', 'ventr. runs ≥3':'ventRuns', 'bigeminy':'bigeminy',
  'cvhr events':'cvhrEvents',
  'pnn50':'pnn50', 'qtc (fridericia)':'qtcFrid',
  'hf power':'hf', 'lf power':'lf', 'vlf power':'vlf', 'hf nu':'hfnu', 'lf nu':'lfnu', 'total power':'totalPower',
  'sdann':'sdann', 'sdnn index':'sdnnIdx', 'tri index':'triIdx',
  'sampen':'sampen', 'accel cap':'accelCapacity', 'pip':'pip',
  'rsa amplitude':'rsaAmplitude', 'crc plv':'crcPLV', 'coupling strength':'couplingStrength', 'edr resp rate':'edrResp',
  'est. ahi':'estAHI', 'apnea risk':'apneaRisk',
  'bσ(ln rmssd)':'sigmaLnRmssd', 'bs²(ln rmssd)':'varLnRmssd', 'surge escalation':'surgeEsc',
  'hrv score':'hrvScore', 'resting hr':'restingHR', 'rest hr':'restingHR', 'expected rmssd':'expRmssd',
  'vo₂max base':'vo2base', 'vo₂max adj':'vo2adj', 'vo₂max est':'vo2adj', 'total sleep':'totSleep', 'deep':'deepMin', 'rem':'remMin',
  'total steps':'steps', 'steps':'steps',
  '% analyzable':'analyzable', 'analyzable':'analyzable',
  'coverage':'coverage', 'beat coverage':'coverage', 'correction':'correction', 'correction rate':'correction',
  'mean sqi':'meanSqi', 'ectopy':'ectopy',
  // ACC companion cross-checks — both the section-card titles and their dense sub-stats
  'acc respiratory rate (rracc)':'rraccRate', 'acc resp rate':'rraccRate', 'rracc':'rraccRate',
  'rracc vs edr agreement':'edrAgreement', 'rracc–edr agreement':'edrAgreement',
  'pearson r':'edrAgreement', 'mae br/min':'edrAgreement', 'disagreement':'edrDisagree', 'mean δ (bias)':'edrAgreement',
  'sleep-stage consensus (acc motion vote)':'stageConsensus', 'stage consensus':'stageConsensus', 'staging consensus':'stageConsensus',
  'af screen':'afScreen', 'hrv stability':'hrvStability'
};
...
var _META_DENY = { 'date':1, 'start':1, 'end':1, 'source':1, 'sample rate':1, 'recording':1,
  'active flags':1, 'tier':1, 'duration':1, 'scenario':1, 'metric':1, 'vo₂max gt':1, 'vo2max gt':1 };
...
global.EcgRegistry = {
  REGISTRY: ECG_REGISTRY, ALIAS: ECG_LABEL_ALIAS,
