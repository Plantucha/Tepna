...
(function () {
  ...
  function idbOpen() {
    return new Promise(function (res, rej) {
      var r = indexedDB.open('cgmcpl_ckpt', 1);
  ...
  async function ckptLoad() {
    try {
      var db = await idbOpen();
      var o = await new Promise(function (res, rej) {
        var tx = db.transaction('s', 'readonly');
        var rq = tx.objectStore('s').get('run');
  ...
  function genVer() {
    return (window.CohortGen && CohortGen.VERSION) || '?';
  }
  var RUN_ID = Math.random().toString(36).slice(2);
  function lockFresh() {
    try {
      var l = JSON.parse(localStorage.getItem('cgmcpl_lock') || 'null');
  ...
  function lockBeat() {
    try {
      localStorage.setItem('cgmcpl_lock', JSON.stringify({ id: RUN_ID, ts: Date.now() }));
    } catch (e) {}
  }
  function lockRelease() {
    try {
      var l = lockFresh();
      if (l && l.id === RUN_ID) localStorage.removeItem('cgmcpl_lock');
    } catch (e) {}
  }
  function fmtETA(sec) {
  ...
  async function run(resumeCk) {
    if (lockHeldByOther()) {
      setStatus('another tab/instance is running this cohort — not duplicating', 'idle');
      $('run').disabled = false;
      return;
    }
    lockBeat();
    var _hb = setInterval(lockBeat, 2000);
    $('run').disabled = true;
    CANCEL = false;
    if ($('cancel')) {
    ...
    setStatus('booting ' + K + '× PulseDex+GlucoDex worker realms…', 'run');
    setProg(0);
    if (!pool.length) await bootPool(K);
    var rdy = pool.filter(function (r) {
      return r.ready;
    });
    if (!rdy.length) {
      setStatus('worker boot failed', 'idle');
      $('run').disabled = false;
      clearInterval(_hb);
      lockRelease();
      return;
    ...
    if (
      resumeCk &&
      resumeCk.ver === ver &&
      resumeCk.targetSubj === targetSubj &&
      resumeCk.minNights === minNights &&
      resumeCk.cursor < total &&
      Array.isArray(resumeCk.ROWS) &&
      Array.isArray(resumeCk.SUBJ)
    ) {
      ROWS = resumeCk.ROWS;
      SUBJ = resumeCk.SUBJ;
      startCursor = resumeCk.cursor;
    ...
    async function lane(rec) {
      while (!CANCEL) {
        ...
        if (m && m.result && m.result.meta && m.result.meta.profile) {
          ...
          var gtN = {};
          ((res.meta.groundTruth && res.meta.groundTruth.nights) || []).forEach(function (gn) {
            gtN[gn.n] = gn;
          });
          var nocB = {};
          (res.nocturnal || []).forEach(function (x) {
            nocB[x.n] = x.score;
          });
          var subjNights = [];
          (res.pulse || []).forEach(function (pp) {
            ...
            var g = nocB[ps.n];
            if (!g || g.mean == null || !isFinite(g.mean)) return;
            var gn = gtN[ps.n] || {};
            var rec2 = {
              seed: sd,
              age: prof.age,
              ...
              gCV: g.cv,
              nHypo: g.nHypo || 0,
              nHypoWin: g.winHypo || 0,
              dawn: g.dawnSurge
            };
            ROWS.push(rec2);
            subjNights.push(rec2);
          });
    ...
    else ckptSave({ ver: ver, targetSubj: targetSubj, minNights: minNights, cursor: cursor, ROWS: ROWS, SUBJ: SUBJ, savedAt: Date.now() });
    try {
      localStorage.setItem('cgmcpl_ptPerSec', String(pdone / Math.max(0.001, elapsed)));
    } catch (e) {}
  ...
  function analyze() {
    ...
    var rmAll = ROWS.map(function (r) {
      return r.rmssd;
    });
    var gAll = ROWS.map(function (r) {
      return r[yvar];
    });
    var aAll = ROWS.map(function (r) {
      return r.ahi;
    });
    var pooled = pearson(gAll, rmAll);
    var rgA = pearson(aAll, gAll),
      rmA = pearson(aAll, rmAll);
    ...
    SUBJ.forEach(function (su) {
      ...
      ns.forEach(function (r) {
        winG.push(r[yvar] - mg);
        winM.push(r.rmssd - mm);
        winA.push(r.ahi - ma);
      });
    });
    var within = pearson(winG, winM),
      between = pearson(btwG, btwM);
    // partial within-patient r(gluc, rmssd | AHI), all on within residuals
    var rwGA = pearson(winG, winA),
      rwMA = pearson(winG.length ? winM : [], winA);
    ...
    var dRm = [],
      ...
      hypoRecallN = 0;
    SUBJ.forEach(function (su) {
      var hyp = su.nights.filter(function (r) {
        return r.gluc === 'hypo';
      });
      var flat = su.nights.filter(function (r) {
        return r.gluc === 'flat';
      });
      hyp.forEach(function (r) {
        hypoRecallN++;
        if (r.nHypoWin > 0) hypoRecallHit++;
        if (r.nHypo > 0) hypoFlagHit++;
      });
    ...
    cards += hcard(
      '#3DE0D0',
      pooled ? fmtR(within ? within.r : pooled.r) : '—',
      'within-patient rMSSD ↔ ' + (yvar === 'gCV' ? 'CGM-CV' : 'nocturnal glucose'),
      within ? 'night-level, ' + within.n + ' nights · pooled ' + fmtR(pooled.r) : '—'
    );
    cards += hcard('#58A6FF', partWin != null ? fmtR(partWin) : '—', 'same coupling, partialling out apnea burden', 'shared-driver test — collapse ⇒ AHI-mediated');
    cards += hcard('#FFB84D', rgA && rmA ? fmtR(rgA.r) + ' / ' + fmtR(rmA.r) : '—', 'apnea burden drives both: glucose ↑ / rMSSD ↓', 'the shared latent (AHI), ' + (rgA ? rgA.n : '—') + ' nights');
    ...
    RESULT = {
      ...
      glucoseVar: yvar,
      minNights: minNights,
      nPatients: SUBJ.length,
      nNights: ROWS.length,
      framing:
        'Cross-node coupling on co-generated synthetic patients (FAST cohort). rMSSD (REAL pulsedex-dsp) and nocturnal glucose (REAL glucodex-dsp on the [t0,t0+dur] sleep-window slice) measured per night by two INDEPENDENT detectors on their own file formats. Coupling decomposed POOLED / WITHIN-patient / BETWEEN-patient; partial correlation controls for the planted apnea burden (AHI, the shared latent driver). Discrete nocturnal-hypo coupling scored as a paired within-patient hypo-vs-flat contrast + GlucoDex hypo-flag recall. Synthetic ground truth — certifies harness cross-node coherence + two-detector recovery, not a real-world CGM↔HRV effect size.',
      coupling: {
        pooled: r2obj(pooled),
        within: r2obj(within),
        between: r2obj(between),
        within_partial_AHI: partWin != null ? +partWin.toFixed(3) : null,
        glucose_vs_AHI: r2obj(rgA),
        rmssd_vs_AHI: r2obj(rmA)
      },
      hypo: {
        recall: hypoRecall != null ? +hypoRecall.toFixed(3) : null,
        recallMethod: 'window-local ≥15-min run <70 mg/dL on the slice',
        flagRecall: hypoFlagRecall != null ? +hypoFlagRecall.toFixed(3) : null,
        flagRecallMethod: 'legacy glucodex analyze() nocturnalHypo flag (full-context, slice-truncated)',
        enriched: !!($('hypoEnrich') && $('hypoEnrich').checked),
        nHypoNights: hypoRecallN,
        pairedPatients: dRm.length,
        medianDeltaRmssd: median(dRm),
        medianDeltaCV: median(dCv)
      }
    };
    try {
      window.CGM_HRV_COUPLING = RESULT;
    } catch (e) {}
  ...
  function fmtR(r) {
    return r == null ? '—' : (r >= 0 ? '+' : '−') + Math.abs(r).toFixed(2);
  }
  function fmtMs(v) {
    return v == null ? '—' : (v >= 0 ? '+' : '−') + Math.abs(v).toFixed(1) + ' ms';
  }
  function r2obj(p) {
    return p ? { r: +p.r.toFixed(3), n: p.n, ci: [+p.lo.toFixed(3), +p.hi.toFixed(3)] } : null;
  }
  function hcard(col, big, k, sub) {
    return '<div class="hcard"><div class="hl" style="color:' + col + '">' + big + '</div><div class="hk">' + k + '</div><div class="hs">' + sub + '</div></div>';
  }
  function crow(pair, level, p, hi) {
    var r = p ? p.r : null,
      n = p ? p.n : null,
      ci = p && p.lo != null ? fmtR(p.lo) + ', ' + fmtR(p.hi) : '—';
    return (
      '<tr' +
      (hi ? ' style="background:rgba(61,224,208,.06)"' : '') +
      '>' +
      '<td>' +
      pair +
      '</td><td class="dim">' +
      ...
      (r == null ? 'var(--text3)' : r < 0 ? 'var(--teal)' : 'var(--amber)') +
      '">' +
      fmtR(r) +
      '</td>' +
      '<td class="num">' +
  ...
  function ptStyle(n) {
    if (n <= 3000) return { r: 2.6, a: 0.55 };
    if (n <= 15000) return { r: 1.5, a: 0.3 };
    if (n <= 40000) return { r: 0.9, a: 0.18 };
    return { r: 0.6, a: 0.11 };
  }
  ...
  function drawScatter(yvar, glabel, within) {
    ...
    SUBJ.forEach(function (su) {
      ...
      ns.forEach(function (r) {
        pts.push({ x: r[yvar] - mg, y: r.rmssd - mm, c: GCOL[r.glyc] || '#9fb0c4' });
      });
    });
    if (!pts.length) {
    ...
    var X = function (v) {
      return P + ((w - P - 16) * (v + xmax)) / (2 * xmax);
    };
    var Y = function (v) {
      return h - P - ((h - P - 14) * (v + ymax)) / (2 * ymax);
    };
    // axes through origin
    ...
    ctx.fillText('+', X(xmax) - 16, Y(0) - 6);
    ctx.fillText('Δ ' + (yvar === 'gCV' ? 'CV' : 'glucose') + ' (night − patient mean) →', P, h - P + 22);
    ctx.save();
    ctx.translate(15, h / 2 + 64);
    ctx.rotate(-Math.PI / 2);
    ctx.fillText('Δ rMSSD (night − patient mean, ms) →', 0, 0);
    ctx.restore();
    // points
    var stS = ptStyle(pts.length);
    ctx.globalAlpha = stS.a;
    pts.forEach(function (p) {
      ctx.fillStyle = p.c;
      dot(ctx, X(p.x), Y(p.y), stS);
    });
    ctx.globalAlpha = 1;
    ...
    if (within) {
      ...
      ctx.lineWidth = 2;
      ctx.beginPath();
      ctx.moveTo(X(-xmax), Y(-xmax * sl));
      ctx.lineTo(X(xmax), Y(xmax * sl));
      ctx.stroke();
      ctx.fillStyle = '#e6edf6';
      ctx.font = 'bold 12px ui-monospace';
      ctx.fillText('within r = ' + fmtR(within.r), w - 168, 26);
    }
    $('scatterLeg').innerHTML =
      'Each point = one patient-night, centered on that patient\u2019s own mean (removes between-patient glycemic class). ' +
      '<span class="chip" style="background:' +
      GCOL.normal +
      '"></span>normal &nbsp;<span class="chip" style="background:' +
      GCOL.preDM +
      '"></span>pre-DM &nbsp;<span class="chip" style="background:' +
      GCOL.T2D +
      '"></span>T2D. ' +
  ...
  function drawDriver(within, partWin, rgA, rmA, yvar) {
    ...
    for (var t = 0; t <= 5; t++) {
      ...
      ctx.beginPath();
      ctx.moveTo(gx, top - 6);
      ctx.lineTo(gx, top + rowH * bars.length - 12);
      ctx.stroke();
      ctx.fillStyle = '#6f8096';
      ctx.font = '10px ui-monospace';
      ctx.fillText((t / 5).toFixed(1), gx - 7, top + rowH * bars.length + 2);
    }
    bars.forEach(function (b, i) {
      ...
      ctx.textAlign = 'right';
      ctx.fillText(b.lab, P - 8, y + 15);
      ctx.textAlign = 'left';
      ctx.fillStyle = 'rgba(255,255,255,.07)';
      rrect(ctx, x0, y, span, bh, 5);
      ctx.fill();
      if (b.r != null) {
        var mag = Math.min(1, Math.abs(b.r));
        ctx.fillStyle = b.col;
        rrect(ctx, x0, y, span * mag, bh, 5);
        ctx.fill();
        ctx.fillStyle = '#0b0e13';
        ctx.font = 'bold 11px ui-monospace';
        ctx.fillText(fmtR(b.r), x0 + 7, y + 15);
      } else {
        ctx.fillStyle = '#6f8096';
        ctx.font = '11px ui-monospace';
        ctx.fillText('—', x0 + 7, y + 15);
      }
    });
    ctx.fillStyle = '#9fb0c4';
    ctx.font = '11px ui-monospace';
    ctx.fillText('| r |  ·  the coupling (top) collapses once apnea burden is partialled out', 10, top + rowH * bars.length + 2);
  }
  ...
  function drawDrivers(yvar) {
    ...
    panel(
      ...
      h,
      ROWS.map(function (r) {
        return { x: r.ahi, y: r[yvar], c: GCOL[r.glyc] || '#9fb0c4' };
      }),
      'apnea burden (AHI, events/h) →',
      glab,
      '#FFB84D'
    );
    panel(
      ...
      h,
      ROWS.map(function (r) {
        return { x: r.ahi, y: r.rmssd, c: GCOL[r.glyc] || '#9fb0c4' };
      }),
      'apnea burden (AHI, events/h) →',
      'rMSSD (ms)',
      '#B98AFF'
    ...
    $('hypoLeg').innerHTML =
      'The two legs of the shared driver, each a real-detector readout over all ' +
      ROWS.length +
      ' nights. ' +
      ...
      '<span class="chip" style="background:' +
      GCOL.T2D +
      '"></span>T2D), which shifts glucose level but not the AHI slope.';
  }
  function panel(ctx, ox, pw, h, pts, xlab, ylab, lineCol) {
    ...
    if (!pts.length) {
      ctx.fillStyle = '#6f8096';
      ctx.font = '12px ui-monospace';
      ctx.fillText('run to populate', ox + 20, 40);
      return;
    }
    var xs = pts.map(function (p) {
        return p.x;
    ...
    var ylo = ymin - (ymax - ymin) * 0.08,
      yhi = ymax + (ymax - ymin) * 0.08;
    var X = function (v) {
      return ox + P + ((pw - P - 16) * v) / (xmax || 1);
    };
    var Y = function (v) {
      return h - P - ((h - P - top - 4) * (v - ylo)) / (yhi - ylo || 1);
    };
    ...
    ctx.moveTo(ox + P, top);
    ctx.lineTo(ox + P, h - P);
    ctx.lineTo(ox + pw - 16, h - P);
    ctx.stroke();
    ctx.fillStyle = '#6f8096';
    ctx.font = '10px ui-monospace';
    for (var g = 0; g <= 4; g++) {
      var yv = ylo + ((yhi - ylo) * g) / 4;
      ctx.fillText(Math.round(yv), ox + 18, Y(yv) + 4);
      ctx.strokeStyle = 'rgba(255,255,255,.05)';
      ctx.beginPath();
      ctx.moveTo(ox + P, Y(yv));
      ctx.lineTo(ox + pw - 16, Y(yv));
      ctx.stroke();
    }
    ctx.fillStyle = '#9fb0c4';
    ctx.font = '11px ui-monospace';
    ctx.fillText(xlab, ox + P, h - P + 22);
    ctx.save();
    ctx.translate(ox + 14, h / 2 + ylab.length * 3);
    ctx.rotate(-Math.PI / 2);
    ctx.fillText(ylab, 0, 0);
    ctx.restore();
    var stP = ptStyle(pts.length);
    ctx.globalAlpha = stP.a;
    pts.forEach(function (p) {
      ctx.fillStyle = p.c;
      dot(ctx, X(p.x), Y(p.y), stP);
    });
    ctx.globalAlpha = 1;
    // OLS line
    ...
    if (pr) {
      ...
      ctx.beginPath();
      ctx.moveTo(X(0), Y(a));
      ctx.lineTo(X(xmax), Y(a + b * xmax));
      ctx.stroke();
      ctx.fillStyle = '#e6edf6';
      ctx.font = 'bold 12px ui-monospace';
      ctx.fillText('r = ' + fmtR(pr.r), ox + pw - 96, top + 12);
    }
  }
  ...
  $('dlCsv').onclick = function () {
    ...
    ROWS.forEach(function (r) {
      rows.push([r.seed, r.age, r.glyc, r.arc, r.night, r.ahi, r.gluc, r.cpap, r.rmssd, r.gMean, r.gCV, r.nHypo, r.nHypoWin, r.dawn == null ? '' : r.dawn]);
    });
    dl(
      'cgm-hrv-coupling-results.csv',
      rows
        .map(function (r) {
          return r.join(',');
        })
  ...
  $('dlStats').onclick = function () {
    dl('cgm-hrv-coupling-stats.json', JSON.stringify(RESULT, null, 2), 'application/json');
  };
  $('dlFig').onclick = function () {
    var a = $('scatter'),
      ...
      c = $('hypo'),
      gap = 16;
    var W = Math.max(a.width + b.width + gap, c.width) + gap * 2,
      H = Math.max(a.height, b.height) + c.height + gap * 3;
    ...
    ctx.drawImage(b, gap * 2 + a.width, gap);
    ctx.drawImage(c, gap, gap * 2 + Math.max(a.height, b.height));
    out.toBlob(function (bl) {
      var u = URL.createObjectURL(bl);
      var an = document.createElement('a');
      an.href = u;
      an.download = 'cgm-hrv-coupling-figures.png';
      an.click();
    }, 'image/png');
  };
  $('gvar').onchange = function () {
    if (RESULT) analyze();
  };
  var CANCEL = false;
  ...
  (function () {
    async function tryResume() {
      try {
        ...
        if (lockHeldByOther()) {
          setStatus('another instance running ' + ck.cursor + '/' + ck.targetSubj + '… (watching)', 'run');
          setTimeout(tryResume, 5000 + Math.random() * 2000);
          return;
        }
