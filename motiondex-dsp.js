...
(function (global) {
  ...
  /* ── THE ACC VALIDITY SIDECAR (SAMPLE-VALIDITY-ENVELOPE §3.2, "acc" row) ─────────────────────
     `…_ACCRUNS.txt`, written beside every Polar `_ACC.txt`. Spans are keyed by the writer's own
     `Phone timestamp` + `dur_ms`, NOT by `first_index`: this parser drops implausible rows
     (`_implausibleDropped`), so our array index is not the writer's sample index and an
     index-keyed intersection would silently slide. Time is the frame both sides agree on.

     ⚠️ THE REFUSAL KEYS ON THE SPAN AND THE FILE'S OWN `min_run`, NEVER ON `class=`. Measured
     2026-09-25 over the real corpus: the same Polar ACC channel reads `class=held` in a 2026-09-08
     sidecar (`held_top2_share=0.95`) and `class=variable` in a 2026-09-23 one (0.85 + exit/confirm).
     The older verdict is the inverted misclassification `HELD_MIN_RUN = 2` exists to prevent — a
     stream where nothing repeats has every run at length 1 and scores a perfect concentration. So
     `class` is not comparable across writer generations, and a refusal keyed on it would inherit a
     retired bug from every old file. The zero-order hold that genuinely repeats 6-7x by design is
     the O2Ring's `accraw`, a DIFFERENT stream from this one. */
  function parseAccRuns(text) {
    if (typeof text !== 'string' || !text) return null;
    var out = /** @type {any} */ ({ params: null, minRun: null, spans: [], rows: [], malformed: false, emitted: 0, examined: null, sawHeader: false, comparable: false });
    ...
    for (var i = 0; i < lines.length; i++) {
      ...
      var stamp = parseTimestamp((c[0] || '').trim());
      var dur = +c[5];
      var n = +c[4];
      if (!stamp || !isFinite(dur) || dur < 0 || !isFinite(n) || n <= 0) {
        out.malformed = true;
        continue;
      }
      out.rows.push({ channel: (c[1] || '').trim(), value: +c[2], n: n });
      out.spans.push({ t0Ms: stamp.tMs, t1Ms: stamp.tMs + dur, channel: (c[1] || '').trim() });
      out.emitted++;
    }
    /* No rule line ⇒ nothing to compare against. An EMPTY file that DOES carry its rule is a real
       observation at that rule — the writer looked and emitted nothing — and one 2026-09-24 Verity
       ACCRUNS is 0 bytes, which is neither. Three states, kept apart. */
    out.comparable = out.minRun != null;
    return out;
  }

  function parseSensorXYZ(text, opts) {
    var lines = String(text || '').split(/\r?\n/);
    var out = [];
    var ns0 = null;
    var prevNsB = null,
    ...
    /* Same idiom as the seam list: hung on the row array, so no consumer signature changes and a
       stream without a sidecar keeps today's bytes exactly. */
    var _runs = parseAccRuns(opts && opts.runsText);
    if (_runs) {
      out._runsSidecar = _runs;
      if (_runs.spans.length) out._blankSpans = _runs.spans;
    }
  ...
  function actigraphy(accRows, t0Ms, durSec, unit) {
    ...
    /* ── §∅ A BLANKING RUN FABRICATES STILLNESS, AND `seen` CANNOT SEE IT ────────────────────────
       The coverage guard above exists because "an uncovered epoch scores counts=0 ⇒ moving=false ⇒
       counted as IMMOBILE, i.e. a recording gap fabricates stillness". A recorded blanking run does
       the SAME THING through a door that guard cannot watch: the rows are present, so `seen[idx]`
       counts them, but they hold one constant, so their dynamic-g is ~0 and the epoch scores
       `moving:false` — IMMOBILE, from samples nobody measured. The error runs toward "still", which
       for a sleep/staging consumer is the direction that matters.

       So a blanked epoch leaves the covered set exactly as an unrecorded one does — refuse at the
       EPOCH (the window that contains absent input), annotate at the NIGHT (the fraction carries its
       reduced denominator). That is §∅'s refuse-vs-annotate line applied at the level where the
       window actually sits, and it reuses the tri-state already here rather than inventing a second
       mechanism. Spans are absolute ms from the sidecar's own stamps (see parseAccRuns). */
    var blanked = new Uint8Array(nE);
    var nBlankEpochs = 0;
    var _spans = accRows && accRows._blankSpans;
    if (_spans && _spans.length && t0Ms != null) {
      for (var bi = 0; bi < _spans.length; bi++) {
        var be0 = Math.floor((_spans[bi].t0Ms - t0Ms) / 1000 / epoch);
        var be1 = Math.floor((_spans[bi].t1Ms - t0Ms) / 1000 / epoch);
        if (!isFinite(be0) || !isFinite(be1)) continue;
        for (var ee = Math.max(0, be0); ee <= Math.min(nE - 1, be1); ee++) blanked[ee] = 1;
      }
    }
    var epochs = [],
      immobile = 0,
    ...
    for (var e = 0; e < nE; e++) {
      ...
      if (blanked[e]) {
        // RECORDED, but the box says the signal was absent here — never "immobile" either.
        nBlankEpochs++;
        epochs.push({ tStartMs: tEp, count: null, moving: null, reason: 'blanking-run' });
        continue;
      }
      covered++;
    ...
    return {
      ...
      /* Present only when a sidecar reported one, so every stream without one keeps today's bytes.
         The COUNT is published rather than folded away: a denominator that shrank silently is the
         shape §∅ objects to, and a reader can see how much of the night the box called absent. */
      ...(nBlankEpochs ? { blankedEpochs: nBlankEpochs } : {}),
  ...
  // ════════════════════════════════════════════════════════════════════════
  function asRows(v, runsText) {
    if (!v) return null;
    // `runsText` is the matching `…_ACCRUNS.txt` when the drop carried one. LAST and optional: absent
    // it, this is the exact single-argument call as before and no stream's bytes move.
    if (typeof v === 'string') return runsText ? parseSensorXYZ(v, { runsText: runsText }) : parseSensorXYZ(v);
  ...
  function compute(input) {
    input = input || {};
    var acc = asRows(input.acc, input.accRuns),
      gyro = asRows(input.gyro),
      mag = asRows(input.mag),
      chest = asRows(input.chestAcc, input.chestAccRuns);
