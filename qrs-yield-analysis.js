...
(function () {
  ...
  function bootPool(n) {
    ...
    for (var i = 0; i < n; i++) {
      (function () {
        ...
        readies.push(new Promise(function (res) {
          w.onmessage = function (ev) {
            var m = ev.data || {};
            if (m.type === 'ready') { rec.ready = !m.err; rec.err = m.err || null; res(); }
          };
        }));
        workers.push(rec);
  ...
  function runSeeds(seeds, onProg) {
    return new Promise(function (resolve) {
      var idx = 0, done = 0, total = seeds.length;
      function pump(rec) {
        if (CANCEL || idx >= total) { if (!workers.some(function (r) { return r.busy; })) resolve(); return; }
        var seed = seeds[idx++];
        rec.busy = true;
        rec.w.onmessage = function (ev) {
          ...
          done++; onProg(done / total);
          rec.busy = false;
          if (CANCEL) { if (!workers.some(function (r) { return r.busy; })) resolve(); return; }
      ...
      if (!workers.some(function (r) { return r.ready; })) resolve();
    });
  }
  ...
  function poolArm(arr) {
    ...
    var sqiA = { sum: 0, n: 0 }, sqiC = { sum: 0, n: 0 };
    var biasAbs = [], biasPct = [], trueBeats = 0;
    arr.forEach(function (w) {
      rec.apnea.tot += w.recall.apnea.tot; rec.apnea.hit += w.recall.apnea.hit;
      rec.clean.tot += w.recall.clean.tot; rec.clean.hit += w.recall.clean.hit;
      prec.tot += w.prec.tot; prec.hit += w.prec.hit;
      sqiA.sum += w.sqiApnea.sum; sqiA.n += w.sqiApnea.n;
      sqiC.sum += w.sqiClean.sum; sqiC.n += w.sqiClean.n;
      trueBeats += w.nTrue;
    ...
    return {
      nWindows: arr.length, trueBeats: trueBeats,
      ...
      apneaBeats: rec.apnea.tot, cleanBeats: rec.clean.tot,
      precision: prec.tot ? prec.hit / prec.tot : null,
      sqiApnea: sqiA.n ? sqiA.sum / sqiA.n : null,
      sqiClean: sqiC.n ? sqiC.sum / sqiC.n : null,
      biasMedPct: median(biasPct), biasMedAbs: median(biasAbs), nBias: biasAbs.length,
    };
  }
  ...
  var targetN;
  async function run() {
    $('run').disabled = true; WIN = { ECG: [], PPG: [] }; CANCEL = false;
    if ($('cancel')) { $('cancel').style.display = ''; $('cancel').disabled = false; }
    targetN = Math.max(10, Math.min(20000, +$('nSubj').value || 60));
    setStatus('booting FULL-lane realms…', 'run'); setProg(0);
    var cores = Math.max(1, Math.min(4, (navigator.hardwareConcurrency || 4)));
    await bootPool(cores);
    var bad = workers.filter(function (w) { return !w.ready; });
    if (bad.length === workers.length) { setStatus('worker boot failed: ' + (bad[0] && bad[0].err || 'unknown'), 'idle'); $('run').disabled = false; return; }
    ...
    if (nWin) analyze();
    $('run').disabled = false;
    ['dlCsv', 'dlStats', 'dlFig'].forEach(function (id) { $(id).disabled = !nWin; });
  }
  ...
  function analyze() {
    ...
    cards += hcard(META.ECG.color, pct(agg.ECG.recallApnea) + '<span class="hu">%</span>',
      'ECG QRS recall in apnea', 'vs ' + pct(agg.ECG.recallClean) + '% clean — yield-robust');
    cards += hcard(META.PPG.color, pct(agg.PPG.recallApnea) + '<span class="hu">%</span>',
      'PPG pulse recall in apnea', 'vs ' + pct(agg.PPG.recallClean) + '% clean — the perfusion gap');
    cards += hcard('#39D98A', (agg.PPG.sqiApnea != null ? agg.PPG.sqiApnea.toFixed(2) : '—'),
      'PPG SQI in apnea', 'quality stays green while yield falls');
    var bias = agg.PPG.biasMedPct;
    cards += hcard('#FF6B7A', (bias != null ? (bias >= 0 ? '+' : '') + bias.toFixed(1) : '—') + '<span class="hu">%</span>',
      'PPG rMSSD bias', 'median detected−true (missed-beat inflation)');
    $('headline').innerHTML = cards;

    // table
    var tb = '';
    ARMS.forEach(function (a) {
      var g = agg[a], mm = META[a];
      tb += '<tr>'
        + '<td><span class="chip" style="background:' + mm.color + '"></span>' + mm.label + ' <span class="muted">' + mm.sub + '</span></td>'
        + '<td class="num">' + g.nWindows + '</td>'
        + '<td class="num">' + g.trueBeats + '</td>'
        + '<td class="num">' + p1(g.recallAll) + '</td>'
        + '<td class="num">' + p1(g.recallClean) + '</td>'
        + '<td class="num">' + p1(g.recallApnea) + '</td>'
        + '<td class="num">' + p1(g.precision) + '</td>'
        + '<td class="num">' + (g.sqiClean != null ? g.sqiClean.toFixed(2) : '—') + '</td>'
        + '<td class="num">' + (g.sqiApnea != null ? g.sqiApnea.toFixed(2) : '—') + '</td>'
        + '<td class="num">' + (g.biasMedPct != null ? (g.biasMedPct >= 0 ? '+' : '') + g.biasMedPct.toFixed(1) + '%' : '—') + '</td>'
        + '</tr>';
    });
    $('tblBody').innerHTML = tb;

    RESULT = {
      generated: new Date().toISOString(), nPatients: targetN,
    ...
    ARMS.forEach(function (a) {
      var g = agg[a];
      RESULT.arms[a] = {
        node: META[a].node, windows: g.nWindows, trueBeats: g.trueBeats,
        recallAll: r4(g.recallAll), recallClean: r4(g.recallClean), recallApnea: r4(g.recallApnea),
        apneaBeats: g.apneaBeats, cleanBeats: g.cleanBeats, precision: r4(g.precision),
        sqiClean: r4(g.sqiClean), sqiApnea: r4(g.sqiApnea),
        rmssdBiasMedianPct: r2(g.biasMedPct), rmssdBiasMedianMs: r2(g.biasMedAbs), nBiasPairs: g.nBias,
        rmssdNote: a === 'ECG' ? 'faithful (R-peaks rendered at true beat times; reference arm)' : 'genuine (PPG truth + pulse-foot detection)',
      };
    });
    try { window.QRS_YIELD = RESULT; } catch (e) {}
  ...
  function hcard(color, hl, k, s) {
    return '<div class="hcard"><div class="hl" style="color:' + color + '">' + hl + '</div>'
      + '<div class="hk">' + k + '</div><div class="hs">' + s + '</div></div>';
  }
  function p1(v) { return v == null ? '—' : (100 * v).toFixed(1) + '%'; }
  function r4(v) { return v == null ? null : +v.toFixed(4); }
  ...
  function axes(ctx, P, w, h, xmin, xmax, ymin, ymax, xt, yt, xlab, ylab, xfmt, yfmt) {
    ...
    for (i = 0; i <= xt; i++) {
      ...
      ctx.fillStyle = '#6f8096'; ctx.fillText(xfmt ? xfmt(xv) : Math.round(xv), px - 10, h - P + 16);
    }
    for (i = 0; i <= yt; i++) {
    ...
    ctx.save(); ctx.translate(13, h / 2 + 40); ctx.rotate(-Math.PI / 2); ctx.fillText(ylab, 0, 0); ctx.restore();
  }
  function rrect(ctx, x, y, w, h, r) { r = Math.min(r, w / 2, Math.abs(h) / 2); if (h < 0) { y += h; h = -h; } ctx.beginPath(); ctx.moveTo(x + r, y); ctx.arcTo(x + w, y, x + w, y + h, r); ctx.arcTo(x + w, y + h, x, y + h, r); ctx.arcTo(x, y + h, x, y, r); ctx.arcTo(x, y, x + w, y, r); ctx.closePath(); }

  // Fig 1 — grouped recall bars: ECG{clean,apnea} · PPG{clean,apnea}
  function drawRecall(agg) {
    var cv = $('recall'), ctx = cv.getContext('2d'), w = cv.width, h = cv.height, P = 46; clearC(ctx, w, h);
    axes(ctx, P, w, h, 0, 1, 0, 1.0, 1, 5, '', 'beat recall (detected / true)', function () { return ''; }, function (v) { return v.toFixed(1); });
    var groups = [
      { arm: 'ECG', clean: agg.ECG.recallClean, apnea: agg.ECG.recallApnea },
      { arm: 'PPG', clean: agg.PPG.recallClean, apnea: agg.PPG.recallApnea },
    ];
    var x0 = P + 18, x1 = w - 24, gw = (x1 - x0) / groups.length;
    var Y = function (v) { return (h - P) - (h - P - 12) * Math.max(0, Math.min(1, v)); };
    // 1.0 reference
    ctx.strokeStyle = 'rgba(255,255,255,.25)'; ctx.setLineDash([5, 4]); ctx.beginPath(); ctx.moveTo(P, Y(1)); ctx.lineTo(w - 14, Y(1)); ctx.stroke(); ctx.setLineDash([]);
    ctx.fillStyle = '#9fb0c4'; ctx.font = '10px ui-monospace'; ctx.fillText('1.00 (all beats)', w - 120, Y(1) - 5);
    groups.forEach(function (g, gi) {
      var col = META[g.arm].color, cx = x0 + gi * gw, bw = Math.min(64, gw * 0.3);
      var bx1 = cx + gw * 0.5 - bw - 8, bx2 = cx + gw * 0.5 + 8;
      // clean (solid), apnea (hatched/translucent)
      ctx.fillStyle = col; rrect(ctx, bx1, Y(g.clean), bw, (h - P) - Y(g.clean) > 0 ? -((h - P) - Y(g.clean)) : -1, 4); ctx.fill();
      ctx.fillStyle = hexA(col, 0.42); rrect(ctx, bx2, Y(g.apnea), bw, -((h - P) - Y(g.apnea)), 4); ctx.fill();
      ctx.strokeStyle = col; ctx.lineWidth = 1.4; rrect(ctx, bx2, Y(g.apnea), bw, -((h - P) - Y(g.apnea)), 4); ctx.stroke();
      // labels
      ctx.fillStyle = '#e6edf6'; ctx.font = 'bold 11px ui-monospace'; ctx.textAlign = 'center';
      ctx.fillText((100 * g.clean).toFixed(0) + '%', bx1 + bw / 2, Y(g.clean) - 6);
      ctx.fillText((100 * g.apnea).toFixed(0) + '%', bx2 + bw / 2, Y(g.apnea) - 6);
      ctx.fillStyle = '#cdd9e6'; ctx.font = 'bold 12px ui-sans-serif,system-ui'; ctx.fillText(META[g.arm].label, cx + gw * 0.5, h - P + 30);
  ...
  function drawDissoc() {
    var cv = $('dissoc'), ctx = cv.getContext('2d'), w = cv.width, h = cv.height, P = 46; clearC(ctx, w, h);
    var x0 = 0.70, x1 = 1.0, y0 = 0.70, y1 = 1.0;
    axes(ctx, P, w, h, x0, x1, y0, y1, 6, 6, 'beat recall in apnea segments', 'detector SQI in apnea segments',
      function (v) { return v.toFixed(2); }, function (v) { return v.toFixed(2); });
    var X = function (v) { return P + (w - P - 14) * Math.max(0, Math.min(1, (v - x0) / (x1 - x0))); };
    ...
    $('dissocLeg').innerHTML = ARMS.map(function (a) { return '<span class="chip" style="background:' + META[a].color + '"></span>' + META[a].label; }).join(' &nbsp; ')
      + ' &nbsp;·&nbsp; each point = one patient window (apnea-segment beats only)';
  }
  ...
  function drawBias() {
    var cv = $('bias'), ctx = cv.getContext('2d'), w = cv.width, h = cv.height, P = 46; clearC(ctx, w, h);
    var pts = WIN.PPG.filter(function (win) { return win.rmssdTrue != null && win.rmssdDet != null && isFinite(win.rmssdTrue) && isFinite(win.rmssdDet); });
    if (!pts.length) { ctx.fillStyle = '#6f8096'; ctx.font = '12px ui-monospace'; ctx.fillText('run to populate', 20, 40); return; }
    var vals = []; pts.forEach(function (p) { vals.push(p.rmssdTrue, p.rmssdDet); });
    var hi = Math.min(160, Math.ceil(Math.max.apply(null, vals) / 10) * 10 + 5);
    axes(ctx, P, w, h, 0, hi, 0, hi, 5, 5, 'true rMSSD (ms)', 'PPG detected rMSSD (ms)', function (v) { return Math.round(v); }, function (v) { return Math.round(v); });
  ...
  $('dlCsv').onclick = function () {
    ...
    ARMS.forEach(function (a) {
      WIN[a].forEach(function (w) {
        rows.push([a, w.ahi, w.cpap ? 1 : 0, w.nTrue, w.nDet, w.lagMs,
          frac(w.recall.clean), frac(w.recall.apnea), frac(w.prec),
          w.sqiClean.n ? (w.sqiClean.sum / w.sqiClean.n).toFixed(3) : '',
          w.sqiApnea.n ? (w.sqiApnea.sum / w.sqiApnea.n).toFixed(3) : '',
          w.rmssdTrue != null ? w.rmssdTrue.toFixed(1) : '', w.rmssdDet != null ? w.rmssdDet.toFixed(1) : '',
          w.rmssdNode != null ? w.rmssdNode.toFixed(1) : '']);
      });
    });
    dl('qrs-yield-results.csv', rows.map(function (r) { return r.join(','); }).join('\n'), 'text/csv');
  };
  function frac(o) { return o.tot ? (o.hit / o.tot).toFixed(4) : ''; }
  $('dlStats').onclick = function () { dl('qrs-yield-stats.json', JSON.stringify(RESULT, null, 2), 'application/json'); };
  $('dlFig').onclick = function () {
    var a = $('recall'), b = $('dissoc'), c = $('bias'), gap = 16;
    ...
    out.toBlob(function (bl) { var u = URL.createObjectURL(bl); var an = document.createElement('a'); an.href = u; an.download = 'qrs-yield-figures.png'; an.click(); }, 'image/png');
  };
  var CANCEL = false;
  $('run').onclick = run;
