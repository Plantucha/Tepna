...
(function (root) {
        ...
        setTimeout(function () {
          ...
          try {
            ...
            _wk.forEach(function (stem) {
              ...
              if (!rows2) return;
              /* THE CALLER IS THE HALF THAT WAS MISSING. `parsePPG2W` is where the sidecar is read, but
                 a parser that accepts an optional companion no production caller ever passes is inert —
                 the exact state `_PPGRUNS` was in for four months (#3060: "it had a reader and no
                 caller"). An absent sidecar leaves `runsText` undefined, `_parsePinnedRuns` returns
                 null, and every other field of the result is unchanged. */
              var res = spo2WaveformTrend(parsePPG2W(_w2[stem], { runsText: _w2r[stem] }), rows2);
              ...
              if (exact || res.usable) {
                ...
                delete _w2[stem];
                /* Consumed with its waveform. It is keyed by the SAME stem, so leaving it behind would
                   let this night's validity spans refuse a LATER night's bins — a stale sidecar is
                   worse than none, because its refusals look exactly like real ones. */
                delete _w2r[stem];
              }
            });
          } catch (_ew2) {
    ...
    return new Promise(function (resolve) {
      ...
      reader.onload = function (e) {
        try {
          ...
          /* Its validity sidecar (SAMPLE-VALIDITY-ENVELOPE §3.2 row `ppg2w`). Tested FIRST and by its
             own suffix: the writer derives the name by APPENDING to the stream suffix
             (`writers.py` — `…_PPG2W.txt` → `…_PPG2WRUNS.txt`), so the two names share a prefix and
             only the anchored `$` keeps them apart. Explicit order rather than relying on that anchor,
             because the anchor is the whole of what distinguishes them and a later edit could lose it.
             PAIRED EXACTLY, not by nearest stamp — stripping each suffix yields the SAME stem by
  ...
  function computeSmartSummary(n) {
    ...
    // ── Autonomic / HR ──
    /* §∅ — `null < 20` is TRUE, so an NSI nobody could compute would have scored the BEST bucket. */
    if (n.comp && n.comp.nsi != null) push('nsi', 'NSI', n.comp.nsi, n.comp.nsi < 20 ? 0 : n.comp.nsi < 40 ? 2 : n.comp.nsi < 60 ? 5 : n.comp.nsi < 80 ? 7 : 10, n.comp.nsi);
  ...
  function buildImpression(n, top5, all) {
    ...
    var leads = {
      ...
      nsi: function () {
        return n.comp && n.comp.nsi != null ? 'NSI ' + n.comp.nsi : 'NSI unavailable';
      },
      waso: function () {
  ...
  function processNight(rows, fname) {
    ...
    var night_obj = {
      ...
      /* ∅ `sparse` and `discontinuous` are different claims, and the shared builder can only make
         the first. `DexExport.coverageFromSegments` labels every multi-segment record `kind:'sparse'`
         — correct for a dropout, wrong for a night whose CLOCK stepped, which is the distinction the
         2026-09-17 ruling turns on (reduced coverage annotates; a discontinuity is not one stretch of
         signal). `clockNonMonotonic` is the fact that separates them and it is an OxyDex stat, not
         something the spine module knows, so the relabel is applied HERE rather than in
         `dex-export.js` — which is inlined into all 8 bundles and would serialise a fleet-wide
         re-verification for a label change. Measured: nothing reads `coverage.kind` today, so this
         buys honesty in the export for a human reader, not behaviour. */
      coverage: _oxyLabelCoverage(oxyCoverage(rows, t0Ms), stats),
      fname: fname,
      stats: stats,
      spikes: spikes,
      period: period,
  ...
  function computeComposite(rows, spikes, desat, cross, motSleep, durationHr) {
    ...
    /* §∅ — a fragmentation RATE with no duration to divide by is undefined, not zero. `sfi` is a
       per-hour rate and 0/hr is the healthiest possible reading. */
    var sfi = durationHr > 0 ? +((wasoWin + spikes.length) / durationHr).toFixed(1) : null;

    // Nocturnal Stress Index (0-100): normalized composite
    // Components: dip3Rate (flag >5), hbRate (flag >5), t95pct (flag >15), AAI (flag >5)
    /* §∅ — NSI AVERAGED FOUR TERMS AND SILENTLY SUBSTITUTED 0 FOR EACH ONE IT COULD NOT COMPUTE.
       `nsi` is goodDirection:'down', so every absent component pulled the score toward the BEST
       value: a night with none of them measured scored 0 — "no nocturnal stress" — and rendered
       green at four sites. `couplingScore` fifteen lines above already refuses for exactly this
       reason and says so ("UNDEFINED, not 0 %… the renderer's own guard was already written
       expecting null"); these four are the siblings that pass never reached.
       ⚠️ The `aai` line carried a comment naming the trap — "`Math.min(null / 5, 1)` is 0, not
       absent" — and then wrote `: 0` anyway, which is the same fabrication one step later. Catching
       a coercion is not the same as refusing.
       The t95 term also counted an ABSENT second as desaturated: `null < 95` is true and `n` was the
       row count, the defect #2943 fixed in computeTIndex and computeHypoxicBurden and did not reach
       here. It now measures over the measured seconds.
       Reduced coverage ANNOTATES (§∅ 2026-09-17): NSI is the mean of the components that EXIST, and
       `nsiComponents` publishes how many that was — a mean of two terms is a different statement
       from a mean of four. No component ⇒ null, never 0. */
    var dip3 = desat && desat.dip3Rate != null ? Math.min(desat.dip3Rate / 5, 1) : null;
    var hbR = desat && desat.auc90Rate != null ? Math.min(desat.auc90Rate / 2, 1) : null; // AUC-90 flag >2
    var _measured = [];
    for (var _s9 = 0; _s9 < spo2.length; _s9++) {
      if (spo2[_s9] != null && isFinite(spo2[_s9])) _measured.push(spo2[_s9]);
    }
    var below95 = _measured.filter(function (v) {
      return v < 95;
    }).length;
    var t95 = _measured.length ? Math.min(((below95 / _measured.length) * 100) / 15, 1) : null;
    var aai = cross && cross.autoArousalIdx != null ? Math.min(cross.autoArousalIdx / 5, 1) : null;
    var _terms = [];
    var _cand = [dip3, hbR, t95, aai];
    for (var _t9 = 0; _t9 < _cand.length; _t9++) {
      var _tv = _cand[_t9];
      if (_tv != null && isFinite(_tv)) _terms.push(_tv);
    }
    var _tsum = 0;
    for (var _u9 = 0; _u9 < _terms.length; _u9++) _tsum += _terms[_u9];
    var nsi = _terms.length ? +((_tsum / _terms.length) * 100).toFixed(0) : null;

    return { couplingScore: couplingScore, sfi: sfi, nsi: nsi, nsiComponents: _terms.length };
  }
  ...
  /* recording.coverage for an oximetry night — INTEGRATOR-GAP-AWARE-OVERLAP part 2.
     ...
     byte-identical. */
  /* Relabels a coverage block whose holes came from a CLOCK STEP rather than from a dropout. Returns
     the same object shape (additively relabelled) so every consumer keeps working, and returns the
     input untouched when there is nothing to relabel. */
  function _oxyLabelCoverage(cov, stats) {
    if (!cov || !stats || stats.clockNonMonotonic !== true) return cov;
    cov.kind = 'discontinuous';
    return cov;
  }
  ...
  // The `_PPG2WRUNS.txt` validity sidecar reader (SAMPLE-VALIDITY-ENVELOPE §3.2, row `ppg2w`).
  // Exposed so the suite can assert BEHAVIOURAL PARITY against `PPGDSP.parsePinnedRuns` on one text —
  // two copies of a parser are only defensible while a gate holds them to the same answer.
  OxyDex._parsePinnedRuns = _parsePinnedRuns;
  ...
  /* ── §∅ / SAMPLE-VALIDITY-ENVELOPE §3.2 row `ppg2w`: READ `…_PPG2WRUNS.txt` ───────────────────
     The capture host has written this sidecar since `writers.py RUN_MIN_BY_STREAM` gained `"ppg2w":
     T_STUCK`, and nothing read it — the same shape #3060 closed for the Verity's `_PPGRUNS`. A
     recorded run is a span the LIVE WRITER saw the signal stuck at one value; §∅ classifies that as
     absent input, and the 2026-09-17 owner ruling says absent input REFUSES rather than annotates,
     because a ratio over a frozen window is a number computable from nothing.

     ⚠️ THIS IS A COPY of `ppgdex-dsp.js parsePinnedRuns`, deliberately, and the alternative was
     measured before choosing. The natural shared home is `signal-frame.js` — and it is inlined by
     **all 10** bundles, so moving the function there moves `manifestHash` and `computeHash` for every
     node, re-verifies every fixture in the suite, and collides in all 11 `provenance/*.json`
     fragments plus both orchestrators with the ECG row of this same brief, which a peer holds in a
     parallel PR. CLAUDE.md §👥.3 is explicit that a shared-spine change lands BEFORE node-local work
     rather than riding on it. So: copy now, held to parity by a gate
     (`OxyDex._parsePinnedRuns` ≡ `PPGDSP.parsePinnedRuns` on one text), consolidation filed as
     residue. A copy with no parity gate would be the defect; a copy with one is a scheduling choice.

     The field names are the PpgDex ones on purpose — the parity assertion is a deep compare, so a
     rename here would make the gate vacuous. */
  function _parsePinnedRuns(text) {
    if (typeof text !== 'string' || !text) return null;
    /* The `any` cast is PpgDex's too, and for the same reason: every field starts null and is filled
       from the file, so inference pins each one to `null` and rejects the assignment that is the point. */
    var out = /** @type {any} */ ({
      params: null,
      minRun: null,
      mergeGapMax: null,
    ...
    for (var li = 0; li < lines.length; li++) {
      ...
      var ch = (c[1] || '').trim();
      var row = { stream: ch, value: +c[2], first: first, n: n };
      out.rows.push(row);
  ...
  /* SIDECAR SPANS ARE SAMPLE INDICES; THE TREND WORKS IN TIME — and the two index DIFFERENT
     populations, which is why this mapping is a function and not an array lookup. The writer's
     `first_index` counts the data rows it WROTE; `parsePPG2W`'s array index counts the rows it KEPT,
     and it drops a row with an unparseable stamp or a non-finite channel. Using one as the other is
     the same-name-two-populations error, silently off by however many rows were dropped.
     So the span is resolved against the SOURCE data-row index recorded on each kept row, and a span
     that lands entirely on dropped rows has NO time — it is COUNTED (`spansUnmapped`), never
     discarded, because a span we cannot place is exactly the absence §∅ forbids rounding to zero. */
  function _ppg2wBlankSpans(rows, runs) {
    if (!runs || !runs.rows.length) return { spans: [], unmapped: 0 };
    var bySrc = /** @type {any} */ ({});
    for (var i = 0; i < rows.length; i++) bySrc[rows[i].src] = i;
    var lastSrc = rows.length ? rows[rows.length - 1].src : -1;
    var spans = /** @type {any[]} */ ([]),
      unmapped = 0;
    for (var r = 0; r < runs.rows.length; r++) {
      var row = runs.rows[r];
      var lo = null,
        hi = null;
      /* Walk OUTWARD from the span's own bounds to the nearest kept row on each side, so a span whose
         first or last sample was dropped still bounds the right stretch of time. */
      for (var k = row.first; k <= row.first + row.n - 1 && k <= lastSrc; k++) {
        if (bySrc[k] != null) {
          if (lo === null) lo = bySrc[k];
          hi = bySrc[k];
        }
      }
      if (lo === null) {
      ...
      spans.push({ stream: row.stream, value: row.value, n: row.n, first: row.first, tMs0: rows[lo].tMs, tMs1: rows[hi].tMs });
    }
    return { spans: spans, unmapped: unmapped };
  }
   ...
   *  Returns { t0Ms, rows:[{tMs, ch0, ch1, motion, src}], runs, blankSpans, spansUnmapped } or
   *  { rows: [] } when nothing parses.
   *
   *  `opts.runsText` (LAST and OPTIONAL — an absent sidecar leaves every other field byte-identical)
   *  is the `…_PPG2WRUNS.txt` companion. Its spans are mapped to time here, where the source
   *  data-row index is still known, and `spo2WaveformTrend` REFUSES any bin whose window meets one. */
  function parsePPG2W(text, opts) {
    var rows = [];
    var lines = String(text || '').split(/\r?\n/);
    /* `src` counts DATA rows as the writer numbered them — every non-header line, whether or not this
       parser keeps it. It is the join key to the sidecar's `first_index`, so it must NOT skip a row
       the parser rejects; incrementing it only on kept rows is precisely the off-by-N the span mapper
    ...
    for (var i = 0; i < lines.length; i++) {
      ...
      rows.push({ tMs: ts.tMs, ch0: a, ch1: b, motion: isFinite(mo) ? mo : 0, src: src });
    }
    var runs = _parsePinnedRuns(opts && opts.runsText);
    var mapped = _ppg2wBlankSpans(rows, runs);
    return {
      t0Ms: rows.length ? rows[0].tMs : null,
      rows: rows,
      runs: runs,
      blankSpans: mapped.spans,
      spansUnmapped: mapped.unmapped
    };
  }
  ...
  function spo2WaveformTrend(rec, spo2Rows, opts) {
    ...
    /** @type {{usable:boolean, reason:(string|null), bins:any[], calib:(any|null), summary:(any|null), trend?:any[], oneHz?:any[], compare?:(any|null), refusedBins?:any[], spansUnmapped?:number}} */
    ...
    /* ── §∅ · A RECORDED BLANKING RUN REFUSES THE BIN IT MEETS ──────────────────────────────────
       The owner ruling of 2026-09-17: a DISCONTINUITY refuses, reduced COVERAGE annotates. A
       `_PPG2WRUNS` row is a stretch the live writer saw frozen at one value, so a bin whose window
       intersects it does not describe one continuous stretch of signal — and R = (AC/DC)₀/(AC/DC)₁
       over a frozen window has AC ≈ 0 on both channels, which is a ratio of two nothings. It is
       computable, it is finite, and it carries no information: the exact shape §∅ records for
       `ppiConf` reading [1,1,1,…] across an 86-second clock seam.
       Refused bins are EXCLUDED from the fit (a meaningless R must not calibrate the rest of the
       night) and REPORTED, never dropped silently. Without a sidecar `blankSpans` is empty and this
       loop is a no-op, which is what keeps the no-sidecar export byte-identical. */
    var _blank = (rec && rec.blankSpans) || [];
    /* 🔴 THE REFUSED SET IS DERIVED FROM THE SPANS, NOT FROM THE BINS THAT HAPPEN TO EXIST — and the
       first draft of this had it the other way round, which measured as a silent hole.
       A bin inside a frozen stretch contributes NO points at all: `flush` rejects a buffer with
       `A.ac <= 0`, and a constant window has exactly that, so `bins[kk]` is never created. Iterating
       the bins therefore refuses only the bin that STRADDLES the edge of the run and reports nothing
       for the fully-frozen middle — the absence stays invisible, which is the one outcome §∅ forbids.
       Measured on a planted 60 s run: 1 bin refused where 2 windows were affected.
       So: walk the spans, mark every bin window each one touches, and report all of them. A marked bin
       with no points is still named — with its `binStartMs`, which is a fact, and `tMs: null`, because
       the median of no samples does not exist and must not be interpolated from the bin bounds. */
    var _refusedBy = Object.create(null);
    for (var _bi = 0; _bi < _blank.length; _bi++) {
      var _k0 = Math.floor(_blank[_bi].tMs0 / (binS * 1000));
      var _k1 = Math.floor(_blank[_bi].tMs1 / (binS * 1000));
      /* Every bin from the span's first to its last, inclusive — intersecting, not containing. A 0.6 s
         run inside one 60 s bin refuses that bin; a 60 s run straddling a boundary refuses two. A
         containment test would let every short run through, which is every run §∅ actually recorded. */
      for (var _kx = _k0; _kx <= _k1; _kx++) if (!_refusedBy[_kx]) _refusedBy[_kx] = _blank[_bi];
    }
    var B2 = [],
      refused = [];
    for (kk in bins) {
      if (_refusedBy[kk]) continue; // reported below, from the span side, points or not
      if (bins[kk].R.length < 8) continue;
      B2.push({ tMs: med(bins[kk].t), R: avg(bins[kk].R), s: avg(bins[kk].s) });
    }
    for (var _rk in _refusedBy) {
      var _hit = _refusedBy[_rk];
      refused.push({
        binStartMs: +_rk * binS * 1000,
        tMs: bins[_rk] ? med(bins[_rk].t) : null,
        points: bins[_rk] ? bins[_rk].R.length : 0,
        spo2w: null,
        reason: 'blanking-run',
        value: _hit.value,
        n: _hit.n,
    ...
    refused.sort(function (a, b) {
      return a.binStartMs - b.binStartMs; // `tMs` is null for a bin with no points, so it cannot be the key
    });
    out.refusedBins = refused;
    out.spansUnmapped = (rec && rec.spansUnmapped) || 0;
    if (B2.length < 40) {
      /* NAME THE REAL STATE. "too little paired data" is true of the count and wrong about the CAUSE
         once blanking is what removed the bins — §∅'s own rule against a borrowed reason that happens
         to fire. A reader told the night was short will look for a short recording. */
      out.reason = refused.length
        ? 'too little paired data outside recorded blanking (' + B2.length + ' usable bins, need ≥40; ' + refused.length + ' refused as blanking-run)'
        : 'too little paired data (' + B2.length + ' bins, need ≥40)';
      return out;
    }
    ...
    out.summary = {
      ...
      bins: n,
      /* The DENOMINATOR of the refusal, beside the value — a trend drawn over 41 of 480 bins is a
         different claim from one drawn over 480, and only these two numbers distinguish them. Both
         are 0 when no sidecar was ingested. */
      binsRefusedBlanking: refused.length,
      spansUnmapped: out.spansUnmapped
    };
