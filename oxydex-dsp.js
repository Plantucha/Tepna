...
function oxyBuildEpochSeries(rows, t0Ms){
  ...
  var EP=300000, bins={};   // 5 min in ms
  for(var i=0;i<rows.length;i++){ var r=rows[i]; if(!r || r.tMs==null) continue;
    var k=Math.floor((r.tMs - t0Ms)/EP); if(k<0) continue;
    var b=bins[k] || (bins[k]={hr:[],mo:[]});
    if(typeof r.hr==='number' && isFinite(r.hr)) b.hr.push(r.hr);
    if(typeof r.motion==='number' && isFinite(r.motion)) b.mo.push(r.motion);
  }
  function _median(a){ if(!a.length) return null; var s=a.slice().sort(function(x,y){return x-y;}); var m=s.length>>1; return s.length%2 ? s[m] : (s[m-1]+s[m])/2; }
  function _mean(a){ if(!a.length) return null; var t=0; for(var j=0;j<a.length;j++) t+=a[j]; return t/a.length; }
  var out=[];
  Object.keys(bins).map(Number).sort(function(a,b){return a-b;}).forEach(function(k){
    var b=bins[k]; if(b.hr.length<60) return;   // ≥1 min HR coverage in the 5-min window
    var mh=_median(b.hr), mm=_mean(b.mo);
    out.push({ tMin:k*5, hr:(mh!=null?+mh.toFixed(1):null), motionIndex:(mm!=null?+mm.toFixed(3):null) });
  });
  return out;
}
...
function oxyBuildTimeseriesBlock(nightsChrono){
  ...
  return { doc:'5-min epochs — cross-node feed for three-cornered-hat: hr = median pulse-HR (bpm), motionIndex = mean O2Ring motion count. HR-hat 3rd corner (ECG+PPG+Oxy) + a 2nd per-epoch motion series for the correlated-TCH ρ.', epochs:n.tchEpochs };
}
