...
// ±2 min of a clock hour during overnight hours (00–05h). Catches gradual-ramp
// artifacts that the single-sample HR cleaner cannot see.
...
// Default OFF — exportJSON only calls this when the user opts into "scrub for sharing", so the normal
// export keeps full provenance for the user's own archive + Integrator fusion (and stays byte-identical).
function oxyScrubExport(envelope){
  if(!envelope || typeof envelope !== 'object') return envelope;
  var out = JSON.parse(JSON.stringify(envelope));   // deep clone — never mutate the caller's object
  ...
  sc.scrubbed = true;
  // recording.contentId (identity-free) is KEPT; strip any device serial / model string a node may carry.
