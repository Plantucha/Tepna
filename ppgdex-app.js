...
(function () {
      ...
      .then(async (parsed) => {
        ...
        for (const _p of parsed) {
          ...
          if (_j && _j.schema && _j.schema.name === 'ganglior.node-export' && ((_j.schema.node || '') + '').trim() === 'PpgDex') {
            ...
            if (_res && _res.ok) {
              if (typeof ppgRenderReview === 'function') ppgRenderReview(_res);
              if (typeof showOK === 'function') showOK('Loaded PpgDex export \u2014 review mode (not recomputed).');
              progress(0, '');
              return;
            }
          }
        ...
        parsed.forEach((p) => {
          if (p.kind === 'ppg') {
            const fk = ING.sniffFirstLine((p.text || '').split(/\r?\n/)[0]);
        ...
        if (plan.hr.length)
          showOK('Ignored ' + plan.hr.length + ' Polar `*_HR.txt` device-HR file' + (plan.hr.length > 1 ? 's' : '') + ' — PpgDex analyzes the raw PPG waveform; drop the matching `*_PPG.txt`.');
        if (!ppgFiles.length) {
          reportNoPPG(skipped);
          progress(0, '');
          return;
        ...
        for (let pi = 0; pi < ppgFiles.length; pi++) {
          ...
          const accF = nearest('acc'),
            gyroF = nearest('gyro'),
            magF = nearest('magn'),
            ppiF = nearest('ppi'),
            markF = nearest('marker');
          ...
          if (markF) {
            ...
            for (const line of rows) {
              ...
              const ts = DSP.parseTimestamp(p[0]);
              if (!ts) continue;
              const type = (p[1] || '').trim();
        ...
        if (added) {
          ...
          try {
            localStorage.setItem('ppgdex_active', activeKey);
          } catch (e) {}
          let msg = 'Analyzed ' + added + ' session' + (added > 1 ? 's' : '') + ' · ' + Object.keys(allSessions).length + ' loaded total.';
          if (skipped.length) {
            const dups = skipped.filter((s) => s.kind === 'duplicate').length,
              fors = skipped.length - dups,
              bits = [];
            if (fors) bits.push(fors + ' foreign/non-PPG');
            if (dups) bits.push(dups + ' duplicate' + (dups > 1 ? 's' : ''));
  ...
  function reportNoPPG(skipped) {
    ...
    const counts = {};
    skipped.forEach((s) => {
      const l = _PPG_FOREIGN_LABEL[s.kind] || 'non-PPG';
      counts[l] = (counts[l] || 0) + 1;
    });
    const summary = Object.keys(counts)
      .map((l) => counts[l] + '× ' + l)
      .join(', ');
    if (skipped.some((s) => s.kind === 'ecg'))
      showErr('No optical PPG in this drop — these look like raw ECG files (' + summary + '). PpgDex needs a Polar Verity Sense “…_PPG.txt”. For chest-strap ECG, use ECGDex.');
  ...
  function renderSession(r) {
    ...
    $('sidebarDataInfo').innerHTML =
      ...
      r.fs +
      ' Hz<br>' +
      r.nPulses.toLocaleString() +
      ' pulses · ' +
      r.dispHr +
      ' bpm · ' +
      (r.durSec >= 3600 ? (r.durSec / 3600).toFixed(1) + ' h' : (r.durSec / 60).toFixed(1) + ' min') +
      '<br>' +
      r.analyzablePct +
      '% analyzable';
  }
  ...
  function updateScopeReadout(view, fs, N, secAt) {
    const s0 = secAt(view.start),
      s1 = secAt(view.start + view.span),
      span = s1 - s0;
    const fmt = (s) => {
    ...
    $('scopeReadout').innerHTML =
      `<span>window <b>${fmt(s0)} – ${fmt(s1)}</b></span><span>span ${span < 60 ? span.toFixed(span < 8 ? 2 : 0) + ' s' : (span / 60).toFixed(1) + ' min'}</span><span>${(N / fs / 60).toFixed(1)} min total</span>`;
  }
  ...
  function renderContext(r) {
    ...
    let notes = '';
    if (r.tier === 'ultra-short')
      notes =
        '<div class="ctx-note">⚠ Ultra-short / spot recording — rMSSD, pNN50, SD1 &amp; pulse morphology are valid; SDNN, LF, VLF need ≥5 min and are flagged. Pulse-wave indices (rise time, dicrotic notch, AI, perfusion) are robust at this length.</div>';
      ...
      notes = '<div class="ctx-note">Overnight — per-epoch medians representative; CVHR apnea screen from the PPI bradycardia-rebound signature is unlocked at this length.</div>';
  ...
  function renderKPI(r) {
    ...
    const items = [
      ...
      { l: 'VO₂max Est', v: r.vo2adj != null ? r.vo2adj : '—', sub: 'ml/kg/min · HR-ratio', s: r.vo2adj == null ? 'neutral' : r.vo2adj >= 45 ? 'ok' : r.vo2adj >= 38 ? 'warn' : 'bad' },
      {
        ...
        s: r.expRmssd ? (r.rmssd >= r.expRmssd ? 'ok' : r.rmssd > r.expRmssd * 0.7 ? 'warn' : 'bad') : sc(r.rmssd, 30, 20)
      },
      {
        l: 'SDNN',
        v: (r.sdnnRobust != null ? r.sdnnRobust : r.sdnn) + 'ms',
        sub: r.tier === 'ultra-short' ? 'needs ≥5min' : r.sdnnRobust != null ? 'robust · per-5-min median' : '≥50 good',
        s: r.tier === 'ultra-short' ? 'neutral' : sc(r.sdnnRobust != null ? r.sdnnRobust : r.sdnn, 50, 30)
      },
      { l: 'Perfusion Idx', v: r.perfusionIndex == null ? '—' : r.perfusionIndex + '%', sub: 'AC/DC · contact', s: pb },
      {
        l: 'Motion-rejected',
        v: r.motionRejectedPct + '%',
        sub: r.motion && r.motion.hasData ? 'ACC+GYRO gate' : 'no motion data',
        s: r.motionRejectedPct < 10 ? 'ok' : r.motionRejectedPct < 30 ? 'warn' : 'bad'
      }
    ];
    if (r.morph) {
      ...
      items.push({
        l: 'Dicrotic notch',
        v: r.morph.dicroticNotchPresent ? 'Present' : '—',
        sub: r.morph.augmentationIndexPct != null ? 'AI ' + r.morph.augmentationIndexPct + '%' : 'reflection',
        s: r.morph.dicroticNotchPresent ? 'ok' : 'neutral'
      });
    }
    items.push(
    ...
    if (r.ledAgreementPct != null)
      items.push({
        l: '3-LED agree',
        v: r.ledAgreementPct + '%',
        sub: r.ledAgree3of3Pct != null ? r.ledAgree3of3Pct + '% all-3 LEDs' : 'optical bSQI',
        s: r.ledAgreementPct >= 90 ? 'ok' : r.ledAgreementPct >= 75 ? 'warn' : 'bad'
      });
    $('kpiGrid').innerHTML = items
      .map((k) => `<div class="kpi ${k.s}"><div class="kpi-label">${k.l}${evBadge(k.l)}</div><div class="kpi-val ${k.s}">${k.v}</div><div class="kpi-sub">${k.sub}</div></div>`)
      .join('');
  ...
  function renderCharts(r) {
    ...
    $('poincareStats').innerHTML =
      `${evBadge('SD1')}SD1 <b>${r.sd1}</b> ms · ${evBadge('SD2')}SD2 <b>${r.sd2}</b> ms · ${evBadge('SD1/SD2')}SD1/SD2 <b>${r.sd1sd2}</b> · area <b>${(r.ellArea || 0).toLocaleString()}</b> ms²`;
    if (r.epochs.length >= 3) {
      const rmPts = r.epochs.map((e) => ({ x: e.tMin, y: e.rmssd })),
        hrPts = r.epochs.map((e) => ({ x: e.tMin, y: e.hr }));
      $('trendBody').innerHTML =
    ...
    (function () {
      ...
      const b = document.getElementById('tachoBody');
      if (!b) return;
  ...
  function renderMorph(r) {
    ...
    $('morphMetrics').innerHTML = rows
      .map(
        ([l, v, u, d2]) =>
          `<div class="morph-row"><div class="mr-l">${typeof evBadge === 'function' ? evBadge(l) : ''}${l}</div><div class="mr-v">${v == null ? '<span class="dim">—</span>' : v + (u ? ' ' + u : '')}</div><div class="mr-d">${d2}</div></div>`
      )
      .join('');
    ...
    if (m.perWindow && m.perWindow.length >= 3) {
      ...
      if (aiPts.length >= 3)
        html +=
          `<div class="mini-h">${evBadge('Augmentation index')}Augmentation index drift <span class="mini-sub">vascular tone</span></div>` +
          UI.lineChart(aiPts, UI.COLORS.amber, { W: 680, H: 130, xfmt: (x) => x.toFixed(0) + 'm' });
      if (piPts.length >= 3)
        html +=
          `<div class="mini-h" style="margin-top:10px">${evBadge('Perfusion index')}Perfusion index drift <span class="mini-sub">AC/DC %</span></div>` +
          UI.lineChart(piPts, UI.COLORS.green, { W: 680, H: 130, xfmt: (x) => x.toFixed(0) + 'm' });
  ...
  function renderMotion(r) {
    ...
    if (!m || !m.hasData) {
      $('motionCard').innerHTML =
        `<div class="q-note" style="padding:20px 8px">No ACC/GYRO companion files loaded. Drop the matching <code>*_ACC.txt</code> and <code>*_GYRO.txt</code> for this session to enable the motion gate — PpgDex's signature quality channel.</div>`;
    ...
    const ribbon =
      m.series && m.series.length > 2
        ? UI.lineChart(
            m.series.map((p) => ({ x: p.x, y: p.y })),
            UI.COLORS.purple,
            { W: 680, H: 140, ymn: 0, ymx: 1, xfmt: (x) => x.toFixed(0) + 'm', med: 0.5 }
          )
        : '<div class="q-note" style="text-align:center;padding:16px">motion index unavailable</div>';
    $('motionCard').innerHTML = `
    <div class="q-grid">
    ...
    ${(() => {
      ...
      (r.epochs || []).forEach((e) => {
        if (e.position) {
          pc[e.position] = (pc[e.position] || 0) + 1;
        }
      });
      const tot = (r.epochs || []).length || 1;
      const chip = (p, pct, interf) =>
        `<span style="display:inline-flex;gap:6px;align-items:center;padding:3px 10px;border:1px solid var(--line,#22303a);border-radius:999px;font-size:12px;margin:4px 6px 0 0;${interf ? 'opacity:.6' : ''}">${p}<b style="color:var(--teal,#3DE0D0)">${pct}%</b></span>`;
      const rows =
        Object.entries(pc)
          .sort((a, b) => b[1] - a[1])
          .map(([p, n]) => chip(p, Math.round((n / tot) * 100)))
          .join('') || '<span class="dim">insufficient coverage (&lt;30 s/epoch)</span>';
      const mag = m.hasMag
  ...
  function renderGanglior(r) {
    ...
    const counts = {};
    ev.forEach((e) => {
      counts[e.impulse] = (counts[e.impulse] || 0) + 1;
    });
    const chip = (k, col) => (counts[k] ? `<span class="g-chip ${col}">${counts[k]} ${k}</span>` : '');
    $('gangliorSummary').innerHTML = ev.length
      ? chip('hrv_drop', 'bad') +
        chip('autonomic_surge', 'warn') +
        chip('cvhr_cluster', 'bad') +
        chip('perfusion_drop', 'warn') +
        chip('motion_artifact_segment', 'neutral') +
        chip('pulse_morphology_shift', 'warn') +
        chip('contact_loss', 'bad')
      : '<span class="g-chip ok">no events — clean window</span>';
    const rows = ev
      .slice(0, 60)
      .map(
        (e) =>
          `<tr><td class="mono">${e.t}</td><td>${e.impulse}</td><td class="mono">${e.conf}</td><td class="mono dim">${
            e.meta
  ...
  function renderTable(r) {
    ...
    const pill = (st) =>
      st === 'ok'
        ...
        : st === 'warn'
          ...
          : st === 'bad'
            ? '<span class="pill pill-red">FLAG</span>'
            : '<span class="dim">—</span>';
    // rows: [metric, value, unit, note, status?] — status optional, '' ⇒ no pill.
    const sec = (title, rows) =>
      `<tr class="tbl-sec"><td colspan="5">${title}</td></tr>` +
      rows
        .map(
          ([m, v, u, n, st]) =>
            `<tr><td class="fmt-m">${evBadge(m)}${m}</td><td class="mono ${st || ''}">${fr(v)}</td><td class="dim">${u || ''}</td><td>${pill(st || '')}</td><td class="dim">${n || ''}</td></tr>`
        )
        .join('');
    const fq = r.freq || {};
    ...
    html += sec('Time-domain HRV (PPI-derived)', [
      ...
      [
        'SDNN',
        r.sdnnRobust != null ? r.sdnnRobust : r.sdnn,
        'ms',
        us ? 'needs ≥5 min' : r.sdnnRobust != null ? 'robust · per-5-min median' : '≥50 good',
        us ? '' : sc(r.sdnnRobust != null ? r.sdnnRobust : r.sdnn, 50, 30)
      ],
    ...
    if (r.morph)
      html += sec('Pulse-wave morphology', [
        ...
        ['Perfusion index', r.morph.perfusionIndexPct, '%', 'AC/DC', r.morph.perfusionIndexPct == null ? '' : r.morph.perfusionIndexPct >= 1 ? 'ok' : r.morph.perfusionIndexPct >= 0.4 ? 'warn' : 'bad']
      ]);
    html += sec('Quality', [
      ...
      ['Motion-rejected', r.motionRejectedPct, '%', r.motion && r.motion.hasData ? 'ACC+GYRO' : 'n/a', r.motionRejectedPct < 10 ? 'ok' : r.motionRejectedPct < 30 ? 'warn' : 'bad'],
  ...
  function sessionsSorted() {
    return Object.values(allSessions).sort((a, b) => (a.t0Ms || 0) - (b.t0Ms || 0));
  }
  function renderSwitcher() {
    ...
    wrap.innerHTML =
      `<div class="sec-label">Sessions · ${list.length}</div>` +
      list
        .map((s) => {
      ...
      <div class="si-sub">${s.durMin} min · ${s.dispHr} bpm · rMSSD ${s.rmssd}</div></button>`;
        ...
        .join('');
    wrap.querySelectorAll('.sess-item').forEach((btn) =>
  ...
  function renderCrossNight() {
    ...
    metrics.forEach((m) => {
      ...
      const spark = UI.lineChart(
        series.map((p) => ({ x: p.x, y: p.v })),
        trendColor(st.trendLabel, m.good),
        { W: 220, H: 54, med: st.mean }
      );
    ...
    $('crossNightTable').innerHTML =
      `<table class="data-table cn-table"><thead><tr><th>Metric</th><th>Trend</th><th>Mean</th><th>CV</th><th>Slope</th><th>Mann–Kendall</th><th>Direction</th><th>Latest z</th></tr></thead><tbody>${rows}</tbody></table>`;
    $('crossNightHeadline').innerHTML = headline.length
      ? `<div class="cn-head-label">Newest night vs baseline</div>` +
        headline
          .slice(0, 3)
    ...
    $('crossNightNote').innerHTML =
      `${list.length} sessions · OLS slope vs date + non-parametric <b>Mann–Kendall</b> (τ, p) for short noisy series · personal-baseline <b>z-scores</b> · coverage-weighted by each night's analyzable %. The same <code>crossNight()</code> engine powers ECGDex &amp; OxyDex trends.`;
  }
  function trendColor(label, good) {
    if (label === 'improving') return UI.COLORS.green;
    if (label === 'declining') return UI.COLORS.red;
  ...
  function _round(v, d = 2) {
    return v == null || typeof v !== 'number' || !isFinite(v) ? (v == null ? null : v) : +v.toFixed(d);
  }
  function buildV2(r) {
    const fq = r.freq || {};
    const m = r.morph || {};
    const epochs = r.epochs.map((e) => ({
      tMin: e.tMin,
      beats: e.beats,
      hr: _round(e.hr, 1),
      meanRR: _round(e.meanRR, 1),
      rmssd: _round(e.rmssd, 2),
      sdnn: _round(e.sdnn, 2),
      pnn50: _round(e.pnn50, 1),
      lf: _round(e.lf, 1),
      hf: _round(e.hf, 1),
      lfhf: _round(e.lfhf, 3),
      respRate: _round(e.respRate, 1),
      pi: _round(e.pi, 1),
      motionIndex: _round(e.motionIndex, 3),
      ledAgreementPct: _round(e.ledAgreementPct, 0),
    ...
    return {
      ...
      quality: {
        ...
        meanSQI: r.meanSQI,
        motionRejectedPct: r.motionRejectedPct,
        correctionRatePct: r.correctionRate,
        ledAgreementPct: r.ledAgreementPct != null ? r.ledAgreementPct : null,
        ledAgree3of3Pct: r.ledAgree3of3Pct != null ? r.ledAgree3of3Pct : null,
        droppedBeats: r.nDroppedBeats != null ? r.nDroppedBeats : null,
      ...
      hrv: {
        time: {
          ...
          lnRMSSD: r.lnRMSSD,
          triIdx: r.triIdx,
          sdnnIndex: r.sdnnIndex != null ? r.sdnnIndex : null,
          sdnnRobust: r.sdnnRobust != null ? r.sdnnRobust : null,
          sd2Robust: r.sd2Robust != null ? r.sd2Robust : null,
          window: 'wholeRecord',
          units: 'ms',
        ...
        frequency: {
          ...
          lfnu: fq.lfnu,
          hfnu: fq.hfnu,
          totalPower: fq.totalPower,
          method: 'Lomb-Scargle',
          lowConfidence: !!r.hrvLowConfidence,
          lfRobust: r.lfRobust != null ? r.lfRobust : null,
          hfRobust: r.hfRobust != null ? r.hfRobust : null,
          lfhfRobust: r.lfhfRobust != null ? r.lfhfRobust : null,
          hfRobustLowMotion: r.hfRobustLowMotion != null ? r.hfRobustLowMotion : null
        },
      ...
      personalization: {
        profile: (function () {
          ...
          return {
            age: ent('age', pr.age),
            sex: ent('sex', pr.sex),
            weight: ent('weight', pr.weight),
            height: ent('height', pr.height),
            elev: ent('elevation', pr.elev),
            cpap: ent('cpap', pr.cpap),
            hrmax: pr.hrmax > 0 ? pr.hrmax : null,
            rhr: pr.rhr > 0 ? pr.rhr : null,
            vo2gt: pr.vo2gt > 0 ? pr.vo2gt : null,
        ...
        assumedDefaults: (function () {
          ...
          return {
            age: asm('age', pr.age),
            sex: asm('sex', pr.sex),
            weight: asm('weight', pr.weight),
            height: asm('height', pr.height),
            elev: asm('elevation', pr.elev),
            source: 'population norm (NHANES age×sex) — the value the analysis ran on',
            note: 'NOT a measurement of this person.'
          };
        })(),
        ansReadinessScore: r.hrvScore,
        ansAge: null /* ANS Age REMOVED 2026-06-21 (external-review WP-A); null for node-export back-compat. */,
        restingHR: r.rhrEff,
        vo2maxEst: r.vo2adj,
        expectedRmssd: r.expRmssd
      },
      apnea: r.longRec ? { screen: 'CVHR (PPI-derived bradycardia-rebound)', note: 'screen not diagnosis', index: null } : null,
      morphology: {
        riseTimeMs: m.riseTimeMs,
        crestTimeMs: m.crestTimeMs,
        dicroticNotchPresent: !!m.dicroticNotchPresent,
        augmentationIndexPct: m.augmentationIndexPct,
        reflectionIndex: m.reflectionIndex,
        pulseWidthMs: m.pulseWidthMs,
        notchTimeMs: m.notchTimeMs,
        sdppgBA: m.sdppgBA,
        sdppgAGI: m.sdppgAGI,
      ...
      validation:
        r.validation && r.validation.usable
          ? {
              deviceAgreementPct: r.validation.deviceAgreementPct,
      ...
      motion:
        r.motion && r.motion.hasData
          ? {
              meanMotionIndex: r.motion.meanMotionIndex,
              motionRejectedPct: r.motionRejectedPct,
              source: 'ACC+GYRO',
              accFs: r.motion.accFs,
              gyroFs: r.motion.gyroFs,
              perEpochMotion: r.epochs.map((e) => e.motionIndex),
              position: r.motion.postureAtSec
                ? {
                    source: r.motion.hasMag ? 'limb-acc+mag' : 'limb-acc',
                    reliability: 'low',
                    ...
                    magnetometer: r.motion.hasMag
                      ? {
                          present: true,
                          headingRef: 'relative',
                          magFs: r.motion.magFs,
                          magBaseG: r.motion.magBaseG,
                          refHeadingDeg: r.motion.refHeadingDeg,
                          magInterferencePct: r.magInterferencePct,
                          note: 'Earth-field-scale only (~0.15 µT LSB). Splits merged lateral into lateral_L/lateral_R via tilt-compensated heading offset from a supine/upright datum — L/R labels are RELATIVE (may be mirrored without a calibration gesture). magInterference is an informational artifact flag; it does NOT modify beat SQI/conf here. NOT biomagnetic HR (cardiac field ~3000× below one LSB).'
                        }
                      : { present: false },
                    breakdown: (() => {
                      const t = {};
                      r.epochs.forEach((e) => {
                        if (e.position) {
                          t[e.position] = (t[e.position] || 0) + 1;
                        }
                      });
                      const tot = r.epochs.length || 1;
                      return Object.entries(t)
                        .map(([p, n]) => ({ position: p, pct: Math.round((n / tot) * 100) }))
                        .sort((a, b) => b.pct - a.pct);
                    })()
                  }
                : { source: 'none', note: 'no ACC — position unavailable' }
            }
          : { source: 'none', note: 'no ACC/GYRO loaded' },
      ...
      reserved: {
        doc: 'hooks for the Integrator 2-site fusion',
        ptt: null,
        pttSource: 'Integrator',
        deltaSBP: null,
        deltaSBPSource: 'Integrator (wrist⟷fingertip PTT)',
        spo2Correlation: null,
        spo2Source: 'OxyDex'
      }
    };
  }
  function exportJSON() {
    ...
    download(
      ...
      exportName({ node: 'PpgDex', t0Ms: _aT0, kind: list.length > 1 ? 'series' : 'summary', ext: 'json', spanDays: _aSpan }),
  ...
  function ppgReviewTimeline(events) {
    ...
    var nm = function (e) {
      ...
      if (i === 'hrv_drop') return 'HRV drop';
      if (i === 'motion_artifact_segment') return 'Motion artifact';
      return i;
    };
    var h =
      '<div class="pgrv-tl">' +
      evs
        ...
        .map(function (e) {
          return (
            '<div class="pgrv-tlrow"><span class="tl-t">' +
            _pgesc(_pgD(e.tMs) + ' ' + (e.t || '')) +
            '</span><span>' +
            _pgesc(nm(e)) +
  ...
  function ppgReviewView(review) {
    ...
    var prov = review.provenance || {},
      ...
      gen = _pgFmtGen(prov.generated || review.generated);
    var nv = function (v, d) {
      return v == null || Number.isNaN(v) ? d || '\u2014' : v;
    };
    var h =
      '<div class="pgrv-banner" role="status">' +
      ...
      '</span>' +
      '<span class="pgrv-meta">' +
      (bh ? 'built <code>' + _pgesc(bh) + '</code>' : 'build unknown') +
      (gen ? ' on <code>' + _pgesc(gen) + '</code>' : '') +
      '</span>' +
    ...
    h +=
      '<div class="pgrv-head"><span class="pgrv-title">PpgDex \u2014 wrist-PPG review</span>' +
      '<span class="pgrv-sub">' +
      _pgesc(rec.source || 'ppg') +
      (rec.durationMin != null ? ' \u00b7 ' + Math.round(rec.durationMin) + ' min' : '') +
      (rec.beats != null ? ' \u00b7 ' + rec.beats + ' beats' : '') +
      '</span></div>';
    h += '<div class="pgrv-sec">Impression</div>';
    h +=
      '<div class="pgrv-imp">Mean HR ' +
      nv(t.hr) +
      ' bpm \u00b7 rMSSD ' +
      nv(t.rmssd) +
      ' ms \u00b7 SDNN ' +
      ...
      (t.lowConfidence ? ' \u00b7 ⚠ low-confidence HRV' : '') +
    ...
    var kpis = [
      ...
      ['LED agreement', nv(q.ledAgreementPct), '%'],
      ['Beats', nv(rec.beats), 'count']
    ];
    h +=
      '<div class="pgrv-sec">Key metrics</div><div class="pgrv-kpis">' +
      kpis
        .map(function (k) {
          return (
            '<div class="pgrv-kpi"><div class="k-lab">' +
    ...
    h +=
      '<div class="pgrv-sec">Raw signal</div>' +
  ...
  function ppgRenderReview(review) {
    ...
    if (!host) {
      ...
      host.id = 'ppgReviewCard';
      var m = document.querySelector('main') || document.body;
      m.insertBefore(host, m.firstChild);
    }
    host.innerHTML = ppgReviewView(review);
    host.style.display = '';
    try {
      window.scrollTo(0, 0);
    } catch (e) {}
  }
  function ppgClearReview() {
    var h = document.getElementById('ppgReviewCard');
    if (h) {
      h.innerHTML = '';
  ...
  try {
    if (typeof window !== 'undefined' && window.PpgDex) {
      window.PpgDex.reviewView = ppgReviewView;
      window.PpgDex.renderReview = ppgRenderReview;
    }
  } catch (_rvx) {}
  function exportGanglior() {
    ...
    if (list.length === 1) {
      ...
      const events = [];
      list.forEach((s) => {
        s.events.forEach((e) => events.push({ t: e.t, tMs: e.tMs, impulse: e.impulse, node: e.node, conf: e.conf, meta: e.meta, session: s.t0Ms }));
      });
  ...
  function clearAll() {
    ...
    try {
      localStorage.removeItem('ppgdex_active');
    } catch (e) {}
    document.body.classList.remove('has-data');
    ['scopeSection', 'qualitySection', 'chartsSection', 'morphSection', 'motionSection', 'validationSection', 'gangliorSection', 'tableSection', 'crossNightSection', 'ctxBanner'].forEach((id) => {
      const el = $(id);
      if (el) el.style.display = 'none';
    });
    $('kpiGrid').classList.remove('show');
    $('sessionSwitcher').style.display = 'none';
  ...
  function showErr(msg) {
    const a = $('alert');
    a.className = 'alert err show';
    a.textContent = '⚠ ' + msg;
    clearTimeout(a._t);
    a._t = setTimeout(() => a.classList.remove('show'), 9000);
  }
  function showOK(msg) {
    const a = $('alert');
    ...
    a.textContent = '✓ ' + msg;
    clearTimeout(a._t);
    a._t = setTimeout(() => a.classList.remove('show'), 5000);
  }

  // ── scope controls ────────────────────────────────────────────────────────
  function zoomIn() {
    if (SCOPE) SCOPE.zoom(0.6);
  }
  function zoomOut() {
    if (SCOPE) SCOPE.zoom(1.6);
  }
  function fitAll() {
    if (SCOPE) SCOPE.fitAll();
  }
  function span(sec) {
    if (SCOPE) SCOPE.setSpanSec(sec);
  }
  ...
  function genSyntheticPatient() {
    ...
    r.tls.forEach((tl) => {
      ...
      if (typeof SYNTH.buildRR === 'function') {
        const t0Win = tl.t0Ms + win.startRel * 1000,
  ...
  function init() {
    if (window.PPGProfile)
      window.PPGProfile.init(() => {
        if (activeKey && allSessions[activeKey]) renderSession(allSessions[activeKey]);
      });
    ...
    if (drop) {
      ...
      ['dragleave', 'drop'].forEach((ev) =>
        drop.addEventListener(ev, (e) => {
          e.preventDefault();
          if (ev === 'drop' || e.target === drop) drop.classList.remove('drag');
        })
      );
    ...
    window.addEventListener('drop', (e) => {
      if (e.dataTransfer && e.dataTransfer.files && e.dataTransfer.files.length && !e.target.closest('#dropZone')) {
    ...
    if (window.MetricRegistry) {
      ...
      if (ge && !ge.querySelector('.ev')) {
        const note = ge.querySelector('.st-note');
        const b = MetricRegistry.badge('measured', 'Events detected directly off the measured signal · conf = SQI × motion gate');
        if (note) note.insertAdjacentHTML('beforebegin', b + ' ');
        else ge.insertAdjacentHTML('beforeend', ' ' + b);
      }
    }
  }
