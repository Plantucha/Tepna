...
(function () {
  ...
  function genWindow(regime, rho) {
    ...
    for (const k of DKEYS) {
      ...
      const sInd = d.sigma0 * Math.sqrt(1 - corr),
        sCor = d.sigma0 * Math.sqrt(corr);
      for (let i = 0; i < n; i++) {
  ...
  function regimeTargets(regime) {
    return {
      o2: regime === 'dynamic' ? DEV.o2.sigmaDyn : DEV.o2.sigmaRest,
  ...
  function minN(devRes, target) {
    for (const N of N_GRID) {
      const r = devRes[N];
      if (r && r.half != null && r.half <= target) return N;
    }
  ...
  async function sweepRho(onProg) {
    ...
    for (let ri = 0; ri < RHO_GRID.length; ri++) {
      ...
      for (let t = 0; t < M; t++) {
        seed(trialSeed(3 + ri, 0, t));
        if (windowSigma(genWindow('resting', rho)).neg) neg++;
      }
      const p = neg / M;
      grid[rho] = {};
      for (const N of N_GRID) grid[rho][N] = 1 - Math.pow(1 - p, N);
      done++;
      if (onProg) await onProg(done / RHO_GRID.length);
    }
    return grid;
  }
  ...
  function bpmMap(text, sep, col) {
    ...
    for (let i = 1; i < L.length; i++) {
      if (!L[i]) continue;
      const cc = L[i].split(sep);
      const t = parseTimestamp(cc[0]);
      if (t == null) continue;
      const hr = +cc[col];
  ...
  async function loadReal() {
    ...
    for (const e of TRIOS) {
      let H, O, V;
      try {
        H = derivedMap(await fetchText(e.h10));
        O = bpmMap(await fetchText(e.o2), ',', 2);
        V = derivedMap(await fetchText(e.verity));
      } catch (err) {
        windows.push({ skip: true, label: e.label, reason: err.message });
        continue;
      }
      const ks = [...H.keys()].filter((k) => O.has(k) && V.has(k)).sort((a, b) => a - b);
      if (ks.length < MIN_WIN_S) {
        windows.push({ skip: true, label: e.label, reason: ks.length + ' s < ' + MIN_WIN_S });
        continue;
      }
      const hh = [],
        vv = [],
  ...
  function lockFresh() {
    try {
      const l = JSON.parse(localStorage.getItem('striopwr_lock') || 'null');
      return l && Date.now() - l.ts < 6000 ? l : null;
    } catch (e) {
      return null;
    }
  }
  function lockHeldByOther() {
    const l = lockFresh();
    return !!(l && l.id !== RUN_ID);
  }
  function lockBeat() {
    try {
      localStorage.setItem('striopwr_lock', JSON.stringify({ id: RUN_ID, ts: Date.now() }));
    } catch (e) {}
  }
  function lockRelease() {
    try {
      const l = lockFresh();
      if (l && l.id === RUN_ID) localStorage.removeItem('striopwr_lock');
    } catch (e) {}
  }
  ...
  function idbOpen() {
    return new Promise((res, rej) => {
      const r = indexedDB.open('striopwr_ckpt', 1);
      r.onupgradeneeded = () => r.result.createObjectStore('s');
      r.onsuccess = () => res(r.result);
      r.onerror = () => rej(r.error);
    });
  }
  async function ckptSave(o) {
    try {
      const db = await idbOpen();
      await new Promise((res) => {
        const tx = db.transaction('s', 'readwrite');
        tx.objectStore('s').put(o, 'run');
        tx.oncomplete = res;
        tx.onerror = res;
      });
    } catch (e) {}
  }
  async function ckptLoad() {
    try {
      const db = await idbOpen();
      return await new Promise((res) => {
        const tx = db.transaction('s', 'readonly');
        const rq = tx.objectStore('s').get('run');
  ...
  async function ckptClear() {
    try {
      const db = await idbOpen();
      await new Promise((res) => {
        const tx = db.transaction('s', 'readwrite');
        tx.objectStore('s').delete('run');
        tx.oncomplete = res;
        tx.onerror = res;
      });
    } catch (e) {}
  ...
  function runJob(rec, job) {
    return new Promise((resolve) => {
      const id = seq++;
      pend.set(id, resolve);
      rec.w.postMessage(Object.assign({ type: 'job', reqId: id, ar1: CFG.ar1, winSec: CFG.winSec }, job));
      setTimeout(() => {
        if (pend.has(id)) {
          pend.delete(id);
  ...
  function finalizeCell(a, tgt) {
    ...
    for (const k of DKEYS) {
      ...
      res[k] = {
        ...
        rmse: rmse,
        negRate: a.negTot[k] ? a.negCount[k] / a.negTot[k] : 0
      };
    }
    return res;
  }
  ...
  function persistRate(secs) {
    try {
      localStorage.setItem('striopwr_secPer500', String((secs * 500) / Math.max(1, CFG.trials)));
    } catch (e) {}
  }
  function updEta() {
    const e = $('eta');
  ...
  async function run(resumeCk) {
    if (lockHeldByOther()) {
      setStatus('idle', 'another tab/instance is running this sweep — not duplicating');
      $('runBtn').disabled = false;
      return;
    }
    lockBeat();
    const _hb = setInterval(lockBeat, 2000);
    ...
    if (resumeCk && resumeCk.sig === sig && resumeCk.acc) {
      acc = resumeCk.acc;
      rhoAcc = resumeCk.rhoAcc;
      durAcc = resumeCk.durAcc;
      done = resumeCk.done || {};
    } else {
      acc = { dynamic: {}, resting: {} };
    ...
    for (const regime of ['dynamic', 'resting'])
      for (const N of N_GRID)
        for (let s = 0; s < CFG.trials; s += BLK) {
          const key = 'c|' + regime + '|' + N + '|' + s;
          if (!done[key]) {
            const cnt = Math.min(BLK, CFG.trials - s);
            jobs.push({ kind: 'cell', regime: regime, N: N, rho: 0, t0: s, count: cnt, key: key, work: cnt * N });
          }
        }
    RHO_GRID.forEach((rho, ri) => {
      for (let s = 0; s < M; s += BLK) {
        const key = 'r|' + ri + '|' + s;
        if (!done[key]) {
          const cnt = Math.min(BLK, M - s);
          jobs.push({ kind: 'rho', rho: rho, ri: ri, t0: s, count: cnt, key: key, work: cnt });
        }
      }
    });
    for (const regime of ['dynamic', 'resting'])
      for (let di = 0; di < DUR_GRID.length; di++)
        for (let s = 0; s < CFG.trials; s += BLK) {
          const key = 'd|' + regime + '|' + di + '|' + s;
          if (!done[key]) {
            const cnt = Math.min(BLK, CFG.trials - s);
            jobs.push({ kind: 'dur', regime: regime, N: 1, rho: 0, durIdx: di, winSec: DUR_GRID[di], seedStream: (regime === 'dynamic' ? 100 : 200) + di, t0: s, count: cnt, key: key, work: cnt });
          }
        }
    ...
    if (GPU_OK) {
      ...
      for (let ri = 0; ri < RHO_GRID.length && !CANCEL; ri++) {
        // ρ-injection sweep
        const rho = RHO_GRID[ri];
        const r = await TrioGPU.runRho(rho, ri, 0, M, CFG.ar1, CFG.winSec);
        rhoAcc[rho].neg += r.neg;
        rhoAcc[rho].M += r.count;
        bump(M);
        await yieldUI();
      }
      for (const regime of ['dynamic', 'resting']) {
      ...
      if (!rdy.length) {
        ...
        const dyn = await sweepRegime('dynamic', 0, (f) => {
          setStatus('run', 'serial · dynamic ' + Math.round(f * 100) + '%');
          return new Promise((r) => setTimeout(r, 0));
        });
        const rest = await sweepRegime('resting', 0, (f) => {
          setStatus('run', 'serial · resting ' + Math.round(f * 100) + '%');
          return new Promise((r) => setTimeout(r, 0));
        });
        ...
        assembleResult(dyn, rest, rhoS, real);
        render();
        clearInterval(_hb);
        lockRelease();
        ...
        ['dlFig1', 'dlFig2', 'dlFig3', 'dlStats', 'dlCsv'].forEach((id) => {
          const e = $(id);
      ...
      async function lane(rec) {
        while (!CANCEL) {
          ...
          if (j.kind === 'cell' || j.kind === 'dur') {
            ...
            rhoAcc[j.rho].neg += r.neg;
            rhoAcc[j.rho].M += r.count;
          }
          done[j.key] = 1;
          doneWork += j.work;
          const el = (performance.now() - t0) / 1000,
          ...
          setStatus(
            'run',
            Math.round((100 * doneWork) / totalWork) + '% · ' + rdy.length + '× workers · ' + (rate / 1000).toFixed(0) + 'k win/s · ETA ' + fmtETA((totalWork - doneWork) / (rate || 1))
          );
          if (doneWork - lastCk > totalWork / 30) {
            lastCk = doneWork;
            ckptSave({ sig: sig, trials: CFG.trials, winSec: CFG.winSec, acc: acc, rhoAcc: rhoAcc, durAcc: durAcc, done: done, savedAt: Date.now() });
    ...
    lockRelease();
    if (CANCEL) {
      await ckptSave({ sig: sig, trials: CFG.trials, winSec: CFG.winSec, acc: acc, rhoAcc: rhoAcc, durAcc: durAcc, done: done, savedAt: Date.now() });
      setStatus('idle', 'cancelled — partial checkpoint saved (re-run resumes)');
      $('runBtn').disabled = false;
      if ($('cancel')) $('cancel').style.display = 'none';
      return;
    }
    ...
    const tgtD = regimeTargets('dynamic'),
      tgtR = regimeTargets('resting');
    const dyn = { regime: 'dynamic', rho: 0, target: tgtD, dev: {} },
      rest = { regime: 'resting', rho: 0, target: tgtR, dev: {} };
    for (const k of DKEYS) {
      dyn.dev[k] = {};
      rest.dev[k] = {};
    }
    for (const N of N_GRID) {
      const fD = finalizeCell(acc.dynamic[N], tgtD),
        fR = finalizeCell(acc.resting[N], tgtR);
      for (const k of DKEYS) {
        dyn.dev[k][N] = fD[k];
        rest.dev[k][N] = fR[k];
      }
    ...
    for (const rho of RHO_GRID) {
      const a = rhoAcc[rho];
      const p = a.M ? a.neg / a.M : 0;
      rhoS[rho] = {};
      for (const N of N_GRID) rhoS[rho][N] = 1 - Math.pow(1 - p, N);
    }
    ...
    for (const reg of ['dynamic', 'resting']) {
      const tg = reg === 'dynamic' ? tgtD : tgtR;
      for (const k of DKEYS) durSweep[reg].dev[k] = {};
      for (let di = 0; di < DUR_GRID.length; di++) {
        const f = finalizeCell(durAcc[reg][di], tg);
        for (const k of DKEYS) durSweep[reg].dev[k][di] = f[k];
      }
    }
    ...
    for (const reg of ['dynamic', 'resting'])
      for (const k of DKEYS) {
        ...
        for (const tg of TARGETS) {
          let mm = null;
          for (let di = 0; di < DUR_GRID.length; di++) {
            const r = durSweep[reg].dev[k][di];
            if (r && r.half != null && r.half <= tg) {
              mm = DUR_GRID[di] / 60;
              break;
            }
          }
    ...
    try {
      ...
      console.warn('real arm:', e.message);
    }
    assembleResult(dyn, rest, rhoS, real);
    RESULT.duration = durSweep;
    RESULT.minMinutes = minMin;
    render();
    const secs = (performance.now() - t0) / 1000;
    persistRate(secs);
  ...
  window.__trioTryResume = async function () {
    try {
      ...
      if (lockHeldByOther()) {
        setStatus('run', 'another instance running… (watching)');
        setTimeout(window.__trioTryResume, 5000 + Math.random() * 2000);
        return;
      }
      if ($('trials')) $('trials').value = ck.trials;
      if ($('winSec')) $('winSec').value = ck.winSec;
      setStatus('run', 'resuming previous run…');
      run(ck);
    } catch (e) {}
  };
  function minNTable(sweep) {
    const o = {};
    for (const k of DKEYS) {
      o[k] = {};
      for (const tg of TARGETS) o[k][tg] = minN(sweep.dev[k], tg);
    }
    return o;
  }
  ...
  window.__trioResult = function () {
    return RESULT && RESULT.dynamic ? RESULT : null;
  };
  window.__trioLane = function () {
    return typeof TrioGPU !== 'undefined' && TrioGPU.ready ? 'webgpu' : 'cpu-pool';
  };

  window.__trioRunSync = function (trials, winSec, ar) {
    CFG.trials = trials || 500;
    CFG.winSec = winSec || 3600;
    if (ar) CFG.ar1 = ar;
    seed(0xc0ffee);
  ...
  function worstMinN(tg) {
    let w = 0;
    for (const k of DKEYS) {
      const v = RESULT.minN.dynamic[k][tg];
  ...
  function drawCurves(c, sweep, metric) {
    const P = { l: 56, r: 16, t: 24, b: 40 },
      { x, w, h } = axes(c, P);
    let top = 0;
    for (const k of DKEYS)
      for (const N of N_GRID) {
        const r = sweep.dev[k][N];
        if (r && r[metric] != null) top = Math.max(top, r[metric]);
      }
    const y1 = Math.max(0.5, Math.ceil(top * 1.05 * 2) / 2),
      y0 = 0;
    // grid + y labels
    for (let i = 0; i <= 5; i++) {
      ...
      x.stroke();
      x.fillStyle = MUT;
      x.fillText(v.toFixed(1), 14, yy + 4);
    }
    // x labels (log)
    for (const N of N_GRID) {
      const px = sxLog(N, P, w);
      x.fillStyle = MUT;
      x.textAlign = 'center';
      x.fillText(N, px, h - P.b + 16);
      x.textAlign = 'left';
    }
    // target lines
    for (const tg of TARGETS) {
      const yy = sy(tg, y0, y1, P, h);
      if (yy > P.t && yy < h - P.b) {
        x.strokeStyle = 'rgba(255,107,122,.35)';
        x.setLineDash([4, 4]);
        x.beginPath();
        x.moveTo(P.l, yy);
        x.lineTo(w - P.r, yy);
    ...
    for (const k of DKEYS) {
      ...
      for (const N of N_GRID) {
        ...
        x.fillStyle = d.col;
        x.beginPath();
        x.arc(sxLog(N, P, w), sy(r[metric], y0, y1, P, h), 3, 0, 7);
        x.fill();
      }
    }
    // legend + title
    x.fillStyle = MUT2;
    x.fillText('95% CI half-width of σ̂ (bpm)  vs  N windows — dynamic regime', P.l, P.t - 8);
    // stacked legend (top-right)
    let ly = P.t + 6;
    for (const k of DKEYS) {
      x.fillStyle = DEV[k].col;
      x.fillRect(w - P.r - 92, ly - 7, 12, 4);
      x.fillStyle = MUT2;
      x.fillText(DEV[k].name, w - P.r - 76, ly - 3);
      ly += 16;
    }
    x.fillStyle = MUT;
    x.save();
    ...
    x.fillStyle = MUT;
    x.textAlign = 'center';
    x.fillText('N co-recorded windows (log)', (P.l + w - P.r) / 2, h - 6);
    x.textAlign = 'left';
  }
  ...
  function drawRegime(c, dyn, rest) {
    const P = { l: 56, r: 16, t: 26, b: 52 },
      { x, w, h } = axes(c, P);
    const N = REGIME_N;
    let lo = 0,
      hi = 0;
    for (const k of DKEYS)
      for (const sw of [dyn, rest]) {
        const r = sw.dev[k][N];
        if (!r) continue;
        lo = Math.min(lo, r.ciLo - sw.target[k]);
        hi = Math.max(hi, r.ciHi - sw.target[k]);
      }
    const y0 = Math.floor((lo - 0.08) * 10) / 10,
      y1 = Math.ceil((hi + 0.08) * 10) / 10;
    for (let i = 0; i <= 6; i++) {
      const v = y0 + ((y1 - y0) * i) / 6,
        yy = sy(v, y0, y1, P, h);
      x.strokeStyle = v === 0 ? AXIS : GRID;
      x.beginPath();
      x.moveTo(P.l, yy);
      x.lineTo(w - P.r, yy);
      x.stroke();
    ...
    x.strokeStyle = 'rgba(57,217,138,.6)';
    x.lineWidth = 1.4;
    x.beginPath();
    x.moveTo(P.l, yZero);
    x.lineTo(w - P.r, yZero);
    x.stroke();
    const groups = DKEYS.length,
      gw = (w - P.l - P.r) / groups;
    DKEYS.forEach((k, gi) => {
      const cx = P.l + gw * gi + gw / 2;
      const items = [
        [dyn.dev[k][N], cx - gw * 0.2, true],
        [rest.dev[k][N], cx + gw * 0.2, false]
      ];
      for (const [r, bx, fill] of items) {
        if (r.bias == null) continue;
        const yb = sy(r.bias, y0, y1, P, h),
          bw = gw * 0.28;
        x.fillStyle = DEV[k].col;
        x.strokeStyle = DEV[k].col;
        x.lineWidth = 1.6;
        if (fill) x.fillRect(bx - bw / 2, Math.min(yZero, yb), bw, Math.abs(yb - yZero));
        else {
          x.globalAlpha = 0.2;
          x.fillRect(bx - bw / 2, Math.min(yZero, yb), bw, Math.abs(yb - yZero));
          x.globalAlpha = 1;
          x.strokeRect(bx - bw / 2, Math.min(yZero, yb), bw, Math.abs(yb - yZero));
        }
        const ehi = sy(r.bias + r.half, y0, y1, P, h),
          elo = sy(r.bias - r.half, y0, y1, P, h);
        ...
        x.textAlign = 'center';
        x.fillText((r.bias >= 0 ? '+' : '') + r.bias.toFixed(2), bx, r.bias >= 0 ? ehi - 5 : elo + 12);
        x.textAlign = 'left';
        x.font = '11px ui-monospace,monospace';
      }
      x.fillStyle = MUT2;
      x.textAlign = 'center';
      x.fillText(DEV[k].name.split(' ')[0], cx, h - P.b + 18);
      x.fillStyle = MUT;
      x.fillText('dyn  ·  rest', cx, h - P.b + 32);
      x.textAlign = 'left';
    });
    x.fillStyle = MUT2;
    x.fillText(`Recovery bias  σ̂ − true σ  by regime (N=${N}) — bpm · 0 = exact recovery`, P.l, P.t - 10);
    let lx = w - P.r - 96,
      ly = P.t + 4;
    x.fillStyle = MUT2;
    x.fillRect(lx, ly - 7, 11, 9);
    x.fillText('dynamic', lx + 15, ly);
    x.strokeStyle = MUT2;
    x.strokeRect(lx, ly + 8, 11, 9);
    x.fillText('resting', lx + 15, ly + 15);
  }
  ...
  function drawReal(c, sweep, real) {
    ...
    let top = 9;
    for (const k of DKEYS) {
      const r = sweep.dev[k][1];
      if (r && r.ciHi) top = Math.max(top, r.ciHi);
    }
    for (const wn of valid) for (const k of DKEYS) if (wn.ci && wn.ci[k]) top = Math.max(top, wn.ci[k].hi);
    const y1 = Math.ceil(top + 1),
      y0 = 0;
    for (let i = 0; i <= 5; i++) {
      const v = y0 + ((y1 - y0) * i) / 5,
        yy = sy(v, y0, y1, P, h);
      ...
      x.stroke();
      x.fillStyle = MUT;
      x.fillText(v.toFixed(0), 18, yy + 4);
    }
    for (const N of N_GRID) {
      const px = sxLog(N, P, w);
      x.fillStyle = MUT;
    ...
    for (const k of DKEYS) {
      ...
      for (const N of N_GRID) {
        const r = sweep.dev[k][N];
        if (!r || r.sigma == null) continue;
        ptsHi.push([sxLog(N, P, w), sy(r.ciHi, y0, y1, P, h)]);
        ptsLo.push([sxLog(N, P, w), sy(r.ciLo, y0, y1, P, h)]);
      }
      ptsHi.forEach((p, i) => (i ? x.lineTo(p[0], p[1]) : x.moveTo(p[0], p[1])));
      for (let i = ptsLo.length - 1; i >= 0; i--) x.lineTo(ptsLo[i][0], ptsLo[i][1]);
      x.closePath();
      x.fill();
      x.globalAlpha = 1;
      // central sim σ line
    ...
    valid.forEach((wn, idx) => {
      const N = idx + 1,
        px = sxLog(N, P, w);
      for (const k of DKEYS) {
        if (wn.sigma[k] == null) continue;
        const py = sy(wn.sigma[k], y0, y1, P, h);
        const ci = wn.ci && wn.ci[k];
        if (ci) {
          x.strokeStyle = '#e6edf6';
          x.lineWidth = 1.4;
          const yhi = sy(ci.hi, y0, y1, P, h),
            ylo = sy(ci.lo, y0, y1, P, h);
          x.beginPath();
          x.moveTo(px, yhi);
          x.lineTo(px, ylo);
          x.moveTo(px - 5, yhi);
        ...
        x.arc(px, py, 4.5, 0, 7);
        x.fill();
        x.strokeStyle = '#0f141b';
        x.lineWidth = 1.5;
        x.stroke();
      }
    });
    x.fillStyle = MUT2;
    x.fillText('Real trio σ̂ ± CI (points) vs sim predicted band (shaded) — vs cumulative N', P.l, P.t - 8);
    let ly = P.t + 6;
    for (const k of DKEYS) {
      x.fillStyle = DEV[k].col;
      x.fillRect(w - P.r - 92, ly - 7, 12, 4);
    ...
    if (!valid.length) {
      x.fillStyle = MUT;
      x.fillText('no real trio window committed', P.l + 12, h / 2);
    }
    x.fillStyle = MUT;
    x.textAlign = 'center';
    x.fillText('cumulative N real windows (log)', (P.l + w - P.r) / 2, h - 6);
    x.textAlign = 'left';
  }

  // assumption panel — negative-variance rate vs N at several injected ρ
  function drawNeg(c, grid) {
    const P = { l: 50, r: 16, t: 24, b: 40 },
      { x, w, h } = axes(c, P);
    const y0 = 0,
      y1 = 1;
    for (let i = 0; i <= 5; i++) {
      ...
      x.stroke();
      x.fillStyle = MUT;
      x.fillText((v * 100).toFixed(0) + '%', 14, yy + 4);
    }
    for (const N of N_GRID) {
      const px = sxLog(N, P, w);
      x.fillStyle = MUT;
    ...
    RHO_GRID.forEach((rho, ri) => {
      ...
      for (const N of N_GRID) {
        ...
        x.fillStyle = col;
        x.beginPath();
        x.arc(sxLog(N, P, w), sy(v, y0, y1, P, h), 2.5, 0, 7);
        x.fill();
      }
    });
    x.fillStyle = MUT2;
    x.fillText('P(≥1 negative TCH variance) vs N — by injected H10·Verity error ρ (resting)', P.l, P.t - 8);
    let ly = P.t + 6;
    RHO_GRID.forEach((rho, ri) => {
      x.fillStyle = cols[ri % cols.length];
      x.fillRect(w - P.r - 70, ly - 7, 12, 4);
      x.fillStyle = MUT2;
      x.fillText('ρ=' + rho, w - P.r - 54, ly - 3);
      ly += 15;
    });
    x.fillStyle = MUT;
    x.textAlign = 'center';
  ...
  function drawDuration(c, sweep) {
    ...
    for (const k of DKEYS)
      for (let di = 0; di < DUR_GRID.length; di++) {
        const r = sweep.dev[k][di];
        if (r && r.half != null) top = Math.max(top, r.half);
      }
    const y1 = Math.max(0.5, Math.ceil(top * 1.05 * 2) / 2),
      y0 = 0;
    for (let i = 0; i <= 5; i++) {
    ...
    for (let di = 0; di < DUR_GRID.length; di++) {
      const px = sxL(mins[di]);
      x.fillStyle = MUT;
      x.textAlign = 'center';
      x.fillText(mins[di] + 'm', px, h - P.b + 16);
      x.textAlign = 'left';
    }
    for (const tg of TARGETS) {
      const yy = sy(tg, y0, y1, P, h);
    ...
    for (const k of DKEYS) {
      ...
      for (let di = 0; di < DUR_GRID.length; di++) {
        const r = sweep.dev[k][di];
        if (!r || r.half == null) continue;
        const px = sxL(mins[di]),
          py = sy(r.half, y0, y1, P, h);
        started ? x.lineTo(px, py) : x.moveTo(px, py);
        started = true;
      }
      x.stroke();
      for (let di = 0; di < DUR_GRID.length; di++) {
        ...
        x.fillStyle = d.col;
        x.beginPath();
        x.arc(sxL(mins[di]), sy(r.half, y0, y1, P, h), 3, 0, 7);
        x.fill();
      }
    }
    x.fillStyle = MUT2;
    x.fillText('95% CI half-width of \u03c3\u0302 (bpm)  vs  window length (min) \u2014 dynamic, N=1', P.l, P.t - 8);
    let ly = P.t + 6;
    for (const k of DKEYS) {
      x.fillStyle = DEV[k].col;
      x.fillRect(w - P.r - 92, ly - 7, 12, 4);
    ...
    x.fillStyle = MUT;
    x.textAlign = 'center';
    x.fillText('window length (minutes, log)', (P.l + w - P.r) / 2, h - 6);
    x.textAlign = 'left';
  }

  function fillDurTbl() {
    const tb = $('durTbl') && $('durTbl').querySelector('tbody');
    if (!tb) return;
    tb.innerHTML = '';
  ...
  function fillMinTbl() {
    const tb = $('minTbl').querySelector('tbody');
    tb.innerHTML = '';
    for (const k of DKEYS) {
      const d = DEV[k],
        dyn = RESULT.minN.dynamic[k];
      const tr = document.createElement('tr');
      const cell = (v) => (v == null ? '<span style="color:#FF6B7A">&gt;20</span>' : v);
      tr.innerHTML =
        `<td><span style="color:${d.col}">●</span> ${d.name}</td>` +
  ...
  function fillRealTbl() {
    const tb = $('realTbl').querySelector('tbody');
    tb.innerHTML = '';
    for (const wn of RESULT.real) {
      const tr = document.createElement('tr');
      if (wn.skip) {
        tr.innerHTML = `<td>${wn.label}</td><td class="num" colspan="5" style="color:#FF6B7A">skipped — ${wn.reason}</td>`;
        tb.appendChild(tr);
        continue;
      }
      tr.innerHTML =
        `<td>${wn.label}</td><td class="num">${wn.n.toLocaleString()}</td>` +
  ...
  function hiRes(srcId, scale) {
    ...
    cc.width = src.width * scale;
    cc.height = src.height * scale;
  ...
  window.__figClear = function () {
    const e = document.getElementById('__figOverlay');
    if (e) e.remove();
    return true;
  };
  window.__figDataURL = function (which, W, H) {
    window.__figShow(which, W, H);
    const e = document.getElementById('__figOverlay');
    const u = e.toDataURL('image/png');
    e.remove();
    return u;
  };
  function exportCsv() {
    ...
    for (const reg of ['dynamic', 'resting']) {
      ...
      for (const k of DKEYS)
        for (const N of N_GRID) {
          const r = sw.dev[k][N];
          rows.push([reg, k, N, r.sigma, r.bias, r.ciLo, r.ciHi, r.half, r.rmse, r.negRate, sw.target[k]]);
        }
    }
    rows.push([]);
    rows.push(['min_N table (smallest N with CI half ≤ target)']);
    rows.push(['device', 'sigma_rest', 'sigma_dyn', 'dyn_to_0.5', 'dyn_to_1.0', 'rest_to_0.5', 'rest_to_1.0']);
    for (const k of DKEYS)
      rows.push([k, DEV[k].sigmaRest, DEV[k].sigmaDyn.toFixed(2), RESULT.minN.dynamic[k][0.5], RESULT.minN.dynamic[k][1.0], RESULT.minN.resting[k][0.5], RESULT.minN.resting[k][1.0]]);
    rows.push([]);
    rows.push(['real_window', 'n_s', 'sigma_o2', 'sigma_h10', 'sigma_verity', 'r_HV', 'r_HO', 'r_VO']);
    RESULT.real.forEach((wn) => {
      if (!wn.skip) rows.push([wn.label, wn.n, wn.sigma.o2, wn.sigma.h10, wn.sigma.verity, wn.rHV, wn.rHO, wn.rVO]);
    });
  ...
  // ════════════════════════════════════════════════════════════════════════
  const NIGHTS = {};
  /* TWO CAPTURE LAYOUTS, ONE RULE EACH. The phone app (Polar Sensor Logger / the O2Ring app) writes
     `Polar_H10_<id>_YYYYMMDD_HHMMSS_<KIND>.txt`, `Polar_Sense_…`, `O2Ring…_<14 digits>.csv`; the capture
     host writes the SAME bytes as `Polar_H10_<id>_YYYYMMDDHHMMSS_<KIND>.txt` (one 14-digit run, no
     separator), `Polar_VeritySense_…`, and `Wellue_O2Ring-S_<serial>_YYYYMMDDHHMMSS_SPO2.csv`. Until
     2026-09-20 only the first set matched, so a box night — every input on disk — indexed as "0 nights
     from 26 files" (RESIDUE 2026-09-20-tool-classifiers-phone-names). The stamp is `(\d{8})_?(\d{6})` so
     each role has ONE pattern rather than a phone copy and a box copy that drift; the O2Ring's box
     `_SPO2.csv` IS the 'o2' role. Gated by capture-host `test_the_tool_classifiers_accept_box_filenames`,
     which reads these literals and runs the box's real filenames through them. */
  ...
  function renderNightTable() {
    const tb = $('nightTbl') && $('nightTbl').querySelector('tbody');
    if (!tb) return;
    tb.innerHTML = '';
    ...
    keys.forEach((k) => {
      ...
      const tr = document.createElement('tr');
      tr.innerHTML =
        `<td><input type="checkbox" class="nchk" data-k="${k}"${ok ? ' checked' : ' disabled'}></td>` +
        `<td class="mono">${k}</td>` +
  ...
  async function processSelected() {
    ...
    const setBar = (frac, txt) => {
      const fl = $('realFill');
      if (fl) fl.style.width = Math.round(frac * 100) + '%';
      const et = $('realEta');
      if (et) et.textContent = txt;
    };
    setBar(0, 'estimating…');
    _progHook = (mm) => {
      const rc = $('nres-' + mm.label);
      if (rc) {
        rc.textContent = mm.phase + '…';
        rc.style.color = '#58A6FF';
      }
      const ph = $('realPhase');
      if (ph) ph.textContent = mm.label + ' · ' + mm.phase;
    };
    const K = Math.max(1, Math.min(8, navigator.hardwareConcurrency || 4));
    if (!pool.length) await bootPool(K);
    const rdy = pool.filter((r) => r.ready);
    if (!rdy.length) {
      setRealStatus('no worker realms available');
    ...
    const bytesTotal = sel.reduce((s, nt) => s + weight(nt), 0);
    let bytesDone = 0;
    async function lane(rec) {
      while (true) {
        const nt = sel[qi++];
        ...
        {
          const el = (performance.now() - t0) / 1000;
          const frac = bytesTotal > 0 ? bytesDone / bytesTotal : doneN / total;
          const eta = fmtETA(bytesDone > 0 ? (el * (bytesTotal - bytesDone)) / bytesDone : NaN);
          setBar(frac, doneN < total ? '~' + eta + ' left · ' + doneN + '/' + total + ' nights' : 'finishing…');
        }
        const res = (r && r.real) || { skip: true, reason: (r && r.error) || 'no result' };
        res.label = nt.key;
        results.push(res);
        if (rc) {
          if (res.skip) {
    ...
    try {
      drawReal($('realCanvas'), band, RESULT.real);
    } catch (e) {}
    try {
      fillRealTbl();
    } catch (e) {}
        ...
        if (entry.isFile) {
          ...
          (function batch() {
            rd.readEntries(
              (ents) => {
                if (!ents.length) {
                  Promise.all(all.map(walk)).then(() => res());
                  return;
                }
                all = all.concat(ents);
                batch();
  ...
  window.addEventListener('DOMContentLoaded', () => {
    ...
    if ($('procBtn'))
      $('procBtn').addEventListener('click', () =>
        processSelected().catch((err) => {
          console.error(err);
          setRealStatus('error: ' + err.message);
          $('procBtn').disabled = false;
        })
      );
    const drawPlaceholder = (id, msg) => {
      const c = $(id);
      if (!c) return;
      const x = c.getContext('2d');
      x.fillStyle = '#0f141b';
