...
(function () {
  ...
  async function loadReal() {
    ...
    for (const e of TRIOS) {
      ...
      windows.push({ label: e.label, n: ks.length, sigma: { o2: s.o2, h10: s.h10, verity: s.verity }, ci, neg: s.neg, h10Unreliable: h10Cls !== 'ok', h10Fault: h10Cls !== 'ok' ? h10Cls : null, rHV, rHO, rVO });
    }
    return windows;
  }
  ...
  function classify(file) {
    ...
    if ((mo = n.match(/^Polar_Sense_[0-9A-Za-zx]+_(\d{8})_(\d{6})_([A-Z]+)\.txt$/i))) {
  ...
  async function processSelected() {
      ...
      while (true) {
        ...
        if (rc) {
          if (res.skip) {
            ...
            rc.textContent = 'σ ' + f2(res.sigma.o2) + ' / ' + f2(res.sigma.h10) + ' / ' + f2(res.sigma.verity) + (res.neg ? ' ⚠neg' : '') + (res.h10Unreliable ? ' ⚠H10-lead' : '') + ' · ' + res.source;
    ...
    RESULT.real = results.map((r) =>
      r.skip ? { skip: true, label: r.label, reason: r.reason } : { label: r.label, n: r.n, sigma: r.sigma, ci: r.ci, neg: r.neg, h10Unreliable: !!r.h10Unreliable, h10Fault: r.h10Fault || null, rHV: r.rHV, rHO: r.rHO, rVO: r.rVO, source: r.source }
    );
