...
(function () {
  var D = window.IntegratorDSP,
    R = window.IntegratorRender,
    L = window.IntegratorLong;
  var $ = function (id) {
  ...
  function ingestJSON(json, filename) {
    ...
    if (L) {
      try {
        ...
        if (li && li.count) {
          WARN.push('✓ Longitudinal: absorbed ' + li.rows + ' day-summary row' + (li.rows === 1 ? '' : 's') + ' from ' + li.nodes.join(', ') + ' (persisted).');
          _longDirty = true;
        }
      } catch (e) {
        /* never let longitudinal break fusion ingest */
      }
    }
  }
  ...
  function readFiles(fileList) {
    ...
    files.forEach(function (file) {
      ...
      reader.onerror = function () {
        WARN.push('"' + file.name + '" — could not be read');
        if (--pending === 0) recompute();
      };
  ...
  function bindLoad() {
    ...
    if (dz) {
      ...
      dz.addEventListener('click', function (e) {
        if (e.target.closest('button,a,label,select,input,.synth-line')) return;
    ...
    ['dragover', 'drop'].forEach(function (ev) {
      window.addEventListener(ev, function (e) {
        ...
        e.preventDefault();
        if (ev === 'drop' && e.dataTransfer && e.dataTransfer.files && e.dataTransfer.files.length) readFiles(e.dataTransfer.files);
      });
    });
  }
  ...
  function exportFusion() {
    ...
    var _ws = FUSION.window.startMs,
      _we = FUSION.window.endMs;
  ...
  function exportCSV() {
    ...
    var a = document.createElement('a');
    a.href = url;
    a.download = exportName({ node: 'Integrator', t0Ms: _ws != null ? _ws : null, kind: 'series', ext: 'csv', spanDays: _span });
    document.body.appendChild(a);
    a.click();
    setTimeout(function () {
      document.body.removeChild(a);
  ...
  function buildExportBar() {
    ...
    if (viewRow) {
      ...
      $('pdfBtnMain').addEventListener('click', function () {
        window.print();
      });
    ...
    b.addEventListener('click', function () {
      ...
      files.forEach(function (path) {
        fetch(path)
          ...
          .then(function (json) {
            ingestJSON(json, path.split('/').pop());
            got++;
          })
          .catch(function (err) {
            WARN.push('Sample "' + path + '" unavailable (' + err.message + ') — drag files in manually.');
          })
          .finally(function () {
            if (--pending === 0) recompute();
          });
  ...
  function genSynthetic() {
    if (!window.DexPatientGen || !window.SYNTH) {
      WARN.push('Generator unavailable (SYNTH not loaded).');
      recompute();
      return;
    }
    ...
    r.tls.forEach(function (tl) {
      ...
      var sevs = tl.events.map(function (e) {
        return e.severity;
      });
      var desN = tl.events.filter(function (e) {
        return e.severity >= 4;
      }).length;
      ...
      var rmssd = +(+hrv.rmssd).toFixed(1),
        sdnn = +(+hrv.sdnn).toFixed(1);
    ...
    function env(node, metrics, rows) {
      return {
        schema: { name: 'ganglior.crossnight', version: '1.0', engineVersion: '1.0.0', node: node, generated: new Date().toISOString(), synthetic: true },
    ...
    var oxyMetrics = {
      odi4: { label: 'ODI-4', unit: '/h', goodDirection: 'down', evidence: gradeFor('OxyDex', 'odi4') },
      minSpo2: { label: 'Min SpO\u2082', unit: '%', goodDirection: 'up', evidence: gradeFor('OxyDex', 'minSpo2') }
    };
    var hrvDefs = {
      rmssd: { label: 'rMSSD', unit: 'ms', goodDirection: 'up', evidence: gradeFor('ECGDex', 'rmssd') },
    ...
    var desats = last.events
      .filter(function (e) {
        return e.severity >= 4;
      })
      .slice(0, 60);
    // EVENT-LEXICON §1: the synthetic demo emits the canonical `desat_event` (was `spo2_desaturation`).
    var oxyEvents = desats.map(function (e) {
      return { tMs: e.t0Ms, impulse: 'desat_event', node: 'OxyDex', conf: +Math.min(0.95, 0.45 + e.severity / 24).toFixed(2), meta: { depth: e.severity, durSec: e.durSec } };
    });
    var ecgEvents = desats.map(function (e) {
      return { tMs: e.t0Ms + 5000, impulse: 'autonomic_surge', node: 'ECGDex', conf: 0.7, meta: {} };
    });
    ...
    if (lastOdi4 >= 5) {
      var pbSrc = desats
        .filter(function (e, i) {
          return i % Math.max(1, Math.floor(desats.length / 5)) === 0;
        })
        .slice(0, 6);
      oxyPB = pbSrc.map(function (e) {
        return { tMs: e.t0Ms, impulse: 'periodic_breathing', node: 'OxyDex', conf: 0.6, meta: { cycleLen: Math.round(45 + (e.severity || 4)), crossings: 8, windowSec: 120 } };
      });
    ...
    var lastRmssd = ecgRows[ecgRows.length - 1].values.rmssd,
      lastSdnn = ecgRows[ecgRows.length - 1].values.sdnn;
    ...
    function accRnd() {
      accSeed = (accSeed * 1664525 + 1013904223) >>> 0;
      return accSeed / 4294967296;
    }
    var posAcc = [];
    for (var pm = 0; pm < durMin; pm += 5) {
      var u = accRnd();
      posAcc.push({ tMin: pm, position: u < 0.72 ? 'supine' : u < 0.87 ? 'left' : u < 0.98 ? 'right' : 'prone' });
    }
    var scenEl = $('genScenario');
    var scenario = scenEl ? scenEl.value : r.profile || '';
    var oxyExport = { schema: { name: 'ganglior.node-export', node: 'OxyDex' }, recording: rec, ganglior_events: oxyEvents, crossNight: env('OxyDex', oxyMetrics, oxyRows) };
    var ecgExport = {
      schema: { name: 'ganglior.node-export', node: 'ECGDex' },
      recording: rec,
      hrv: { time: { rmssd: lastRmssd, sdnn: lastSdnn } },
      hrvStability: { mean_lnRMSSD_slope: autoSlope },
      acc: posAcc,
      apnea: { cvhrIndex: +Math.min(35, Math.max(0, lastOdi4 * 0.8)).toFixed(1) },
      ganglior_events: ecgEvents,
      crossNight: env('ECGDex', hrvDefs, ecgRows)
    };
    var gluExport = {
      schema: { name: 'ganglior.node-export', node: 'GlucoDex' },
      recording: rec,
      glycemic: { cv: lastCV },
      variability: { cv: lastCV },
      ganglior_events: [],
      crossNight: env('GlucoDex', gluMetrics, gluRows)
    };
    var hrvExport = {
      ...
      ganglior_events: [],
      crossNight: env('HRVDex', hrvDefs, hrvRows)
    };
    var pulseExport = {
      ...
      ganglior_events: [],
      crossNight: env('PulseDex', hrvDefs, pulseRows)
    };
    RECS = [];
    WARN = []; // fresh fusion view for the freshly generated night
    ...
    if (scenario === 'cpap') {
      var residRows = oxyRows.map(function (row) {
        return { t0Ms: row.t0Ms, date: row.date, values: { residualAHI: +Math.max(1.2, row.values.odi4 + 1.6).toFixed(1) } };
      });
      ...
      var cpapPB = (cpapPbPct > 0 ? oxyPB.slice(0, 3) : []).map(function (e) {
        return { tMs: e.tMs + 1000, impulse: 'periodic_breathing', node: 'CPAPDex', conf: 0.8, meta: {} };
      });
      var cpapExport = {
        schema: { name: 'ganglior.node-export', node: 'CPAPDex' },
        ...
        metrics: {
          residualAHI: resid,
          centralIndex: +(resid * 0.35).toFixed(1),
          obstructiveIndex: +(resid * 0.4).toFixed(1),
          hypopneaIndex: +(resid * 0.25).toFixed(1),
          periodicBreathingPct: cpapPbPct,
          largeLeakPct: 4.2,
          medianPressure: 8.6
        },
        ganglior_events: cpapPB,
        crossNight: env('CPAPDex', { residualAHI: { label: 'Residual AHI', unit: '/h', goodDirection: 'down', evidence: gradeFor('CPAPDex', 'residualAHI') } }, residRows)
      };
    ...
    WARN.push(
      '\u2713 Generated ' +
        r.days +
        '-day synthetic patient (' +
        r.label +
        ') \u2014 ' +
        nodesGen.join(' \u00b7 ') +
        '. Last night fused (apnea, positional, glucose\u27f7autonomic, 3-way HRV consensus' +
        (oxyPB.length ? ', periodic breathing' : '') +
        (scenario === 'cpap' ? ', device-scored AHI' : '') +
        ').'
    );
    recompute();
  ...
  function bindGen() {
    var b = $('genBtn');
    if (b) b.addEventListener('click', genSynthetic);
  }
