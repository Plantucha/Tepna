...
var CHART_EV = {
  /* ── Overview ── */
  ch_rmssd:'validated',  ch_hrv_sdnn:'experimental', ch_sns_psns:'experimental',
  ch_ov_lfhf:'validated', ch_ov_si:'validated', ch_ov_vei:'emerging',
  ch_ov_dfa:'emerging',
  /* ch_ov_age (ANS age) + ch_ov_map (HRV→MAP) REMOVED 2026-06-23 (DEX-METRIC-REMOVAL-AUDIT 🔴) */
  /* ── Time domain ── */
  ch_stressEnergy:'experimental', ch_si:'validated', ch_cv:'validated',
  ch_rmssd_sdnn:'emerging', ch_mxdmn_meanrr:'emerging', ch_se_div:'experimental',
  ch_lnrmssd:'validated',
  /* ── Frequency domain ── */
  ch_lfhf:'validated', ch_lfhf_nu:'validated', ch_vlf_pct:'emerging',
  ch_svi:'emerging', ch_spectral_ent:'emerging', ch_lfhf_total:'emerging',
  /* ── Composite scores (HRVDex-derived) ── */
  ch_ans_load:'experimental', ch_coh_energy:'experimental', ch_pti:'experimental',
  ch_incoherent_stress:'experimental', ch_vei:'emerging', ch_sdi:'experimental',
  /* ── Rolling ── */
  ch_ari:'experimental', ch_sdnn_z:'emerging', ch_rmssd_rolling:'validated',
  ch_stress_auc:'experimental', ch_rmssd_cv:'emerging', ch_stress_ac:'experimental',
  ch_hrv_momentum:'emerging', ch_welfare:'experimental', ch_recovery_debt:'experimental',
  ch_rmssd_reactivity:'emerging',
  /* ── Clinical ── */
  ch_bap:'heuristic', ch_ortho:'experimental', ch_pnn50_slope:'emerging',
  ch_hile:'experimental', ch_camq:'experimental',
  /* ch_htn_pattern (HRV→HTN-risk) + ch_map_est (HRV→MAP) REMOVED 2026-06-23 (DEX-METRIC-REMOVAL-AUDIT 🔴) */
  /* ── Poincaré & Toichi ── */
  ch_sd1_sd2:'validated', ch_sd1sd2_ratio:'validated', ch_cvi:'validated',
  ch_csi:'validated', ch_cvi_csi_scatter:'validated', ch_dfa_plaw:'emerging',
  ch_vlf_hf_sai:'emerging', ch_cai:'emerging',  /* ch_auto_age REMOVED 2026-06-23 (DEX-METRIC-REMOVAL-AUDIT 🔴) */
  /* ── Cognitive & resilience ── */
  ch_efc:'experimental', ch_crs:'experimental', ch_abs:'experimental',
  ch_sfd:'experimental', ch_focus_eff:'experimental', ch_pns_eff:'experimental',
  ch_otr:'experimental', ch_rmssd_circ:'emerging', ch_rsa_nn50:'emerging',
  /* ── VO₂max (population projection) ── */
  ch_vo2_trend:'heuristic', ch_vo2_delta:'heuristic', ch_vo2_hr_scatter:'heuristic',
...
function injectCss(){
  ...
  s.textContent =
    '.chart-card > h4 > .ev{position:static;margin-right:7px;vertical-align:middle;opacity:.8;}' +
    '.chart-card:hover > h4 > .ev{opacity:1;}';
  (global.document.head || global.document.documentElement).appendChild(s);
}

function decorate(){
  var MR = global.MetricRegistry, doc = global.document;
  if (!MR || !doc) return;
  injectCss();
...
function run(){
  if (global.document.readyState === 'loading')
    global.document.addEventListener('DOMContentLoaded', decorate, { once:true });
  else decorate();
