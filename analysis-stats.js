...
(function (root) {
  ...
  /* ── Block-bootstrap CI whose ESTIMATOR FOLLOWS THE POINT (DEEP-AUDIT-VI F16) ─────────────────
     ...
     different estimators, and on an artifact-flagged night the rendered σ sits entirely OUTSIDE its
     own rendered 95 % CI (measured: σ_H10 = 1.009 against [7.296, 12.801]). Single-sourced here so
     the page DELEGATES and the known-answer group can execute it: when the per-corner confidence
     arrays are supplied they are resampled in LOCKSTEP with the same block indices and every
     replicate runs tchSigmasFused — the same estimator as the point; without them every replicate
     runs the classic tchSigmas, which is what the committed-TRIOS path pairs with its classic
     point. `rand` is injectable so a test can pin the resampling exactly. */
  function tchBlockBootstrapCI(hh, vv, oo, opts) {
    opts = opts || {};
    var cH = opts.cH || null,
      cV = opts.cV || null,
      cO = opts.cO || null;
    ...
    for (var b = 0; b < B; b++) {
      ...
      var s = fused ? tchSigmasFused(H, V, O, CH, CV, CO) : tchSigmas(H, V, O);
      var keys = ['h10', 'verity', 'o2'];
      for (var ki = 0; ki < 3; ki++) {
        var kk = keys[ki];
        if (s[kk] != null && isFinite(s[kk])) acc[kk].push(s[kk]);
      }
    }
    var out = { estimator: fused ? 'fused' : 'classic' };
    var keys2 = ['h10', 'verity', 'o2'];
    for (var ko = 0; ko < 3; ko++) {
      var kn = keys2[ko];
      var a = acc[kn].sort(function (p1, p2) {
        return p1 - p2;
      });
      out[kn] = a.length >= 20 ? { lo: q(a, 0.025), hi: q(a, 0.975) } : null;
    }
    return out;
  }
