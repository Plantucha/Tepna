...
(function (root) {
  ...
  function iccOneWay(groups) {
    ...
    return {
      ...
      medianOcc: median(g.map(function (a) { return a.length; })),
    };
  }
  ...
  function tchSigmas(hh, vv, oo) {
    ...
    return {
      ...
      dHV: dHV, dHO: dHO, dVO: dVO,
    };
  }
  // Bland–Altman summary of a difference series: bias, SD, 95% LoA half-width, Arms.
  function blandAltman(d) { var b = mean(d), s = sd(d); return { n: d.length, bias: b, sd: s, loa: 1.96 * s, arms: Math.sqrt(b * b + s * s) }; }
  // Bare Pearson r (no guard/clamp) — the pairwise-control leg in the σ tool.
  function pearson(x, y) { var mx = mean(x), my = mean(y), sxy = 0, sx = 0, sy = 0; for (var i = 0; i < x.length; i++) { var dx = x[i] - mx, dy = y[i] - my; sxy += dx * dy; sx += dx * dx; sy += dy * dy; } return sxy / Math.sqrt(sx * sy); }
  ...
  function olsFit(y, Xrows) {
    ...
    var my = mean(y), ssTot = 0, sseR = 0;
    for (i = 0; i < n; i++) { var pred = 0, xj = Xrows[i]; for (a = 0; a < p; a++) pred += beta[a] * xj[a]; var e = y[i] - pred; sseR += e * e; ssTot += (y[i] - my) * (y[i] - my); }
    var df = n - p, sigma2 = sseR / df, se = [], t = [], pv = [], ci = [];
  ...
  var AnalysisStats = {
    // helpers
    mean: mean, variance: variance, sd: sd, median: median, sse: sse,
    // reliability
    iccOneWay: iccOneWay, spearmanBrown: spearmanBrown, minOccForReliability: minOccForReliability,
    // reference-free agreement
    threeCorneredHat: threeCorneredHat, tchSigmas: tchSigmas, blandAltman: blandAltman, pearson: pearson,
    // correlation decomposition
    pearsonCI: pearsonCI, partialCorr: partialCorr,
    // regression
    ols: ols, olsFit: olsFit, invMat: invMat, roc: roc,
    // change-point
    bestSplit: bestSplit, mannWhitneyAUC: mannWhitneyAUC,
  };
