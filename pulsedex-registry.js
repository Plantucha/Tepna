...
(function (global) {
  ...
  var PULSE_REGISTRY = {
    ...
    hf: {
      ...
      depth: 'advanced',
      evidence: 'validated',
      cite: 'HF power — parasympathetic (Task Force 1996) · calibration corrected 2026-07-19 (§3.1)'
    },
    lf: { label: 'LF Power', unit: 'ms²', goodDirection: 'up', depth: 'research', evidence: 'validated', cite: 'LF power (Task Force 1996) · calibration corrected 2026-07-19 (§3.1)' },
    vlf: { label: 'VLF Power', unit: 'ms²', goodDirection: 'up', depth: 'research', evidence: 'validated', cite: 'VLF power (Task Force 1996) · calibration corrected 2026-07-19 (§3.1)' },
  ...
  function idForLabel(label) {
    ...
    if (label != null && PULSE_REGISTRY[label]) return String(label);
    var k = _norm(label);
    if (PULSE_REGISTRY[k]) return k;
    if (PULSE_LABEL_ALIAS[k]) return PULSE_LABEL_ALIAS[k];
    ...
    if (!_labelIdx) {
      _labelIdx = {};
      for (var _lk in PULSE_REGISTRY) {
        var _le = PULSE_REGISTRY[_lk];
        if (_le && _le.label) {
