...
(function (global) {
  ...
  (function injectSynthCSS() {
    try {
      ...
      var css =
        '.long-actions{ margin-left:auto; display:flex; gap:8px; align-items:center; flex-wrap:wrap; }' +
  ...
  // ── IndexedDB store ────────────────────────────────────────────────────────
  var DB_NAME = 'ganglior_integrator',
    DB_VER = 1,
    STORE = 'summaries',
    DEFS = 'metricDefs';
  var _db = null,
    _rows = {},
    _defs = {},
    _meta = {}; // in-memory mirror (id→record)
  ...
  function _txAll(store) {
    return new Promise(function (res, rej) {
      var out = [],
        tx = _db.transaction(store, 'readonly'),
  ...
  function open() {
    return _openDB()
      ...
      .catch(function () {
        _db = null;
        return false;
      }); // degrade gracefully (private mode etc.)
  }
  ...
  function _findEnvelopes(json) {
    ...
    function isEnv(o) {
      return o && o.schema && o.schema.name === 'ganglior.crossnight' && o.metrics && Array.isArray(o.series);
    }
    ...
    ['crossnight', 'cross_night', 'longitudinal'].forEach(function (k) {
      if (json[k] && isEnv(json[k])) out.push(json[k]);
    });
    return out;
  }
    ...
    envs.forEach(function (env) {
      ...
      (env.series || []).forEach(function (it) {
        ...
        var rec = {
          ...
          node: node,
          date: date,
          t0Ms: it.t0Ms != null ? it.t0Ms : null,
          values: it.values || {},
          weight: it.weight != null ? it.weight : 1,
          lowQuality: !!it.lowQuality,
          synthetic: !!(env.schema && env.schema.synthetic),
          engineVersion: eng,
          ingestedAt: Date.now(),
          src: filename || null
        };
  ...
  function clear() {
    ...
    _meta = {};
    if (_db) {
      return Promise.all([_clearStore(STORE), _clearStore(DEFS)]);
    }
  ...
  function clearSynthetic() {
    var ids = Object.keys(_rows).filter(function (id) {
      return !!_rows[id].synthetic;
    });
    ids.forEach(function (id) {
      delete _rows[id];
    }); // prune in-memory mirror (sync)
    if (_db) {
      return _deleteIds(STORE, ids);
    } // metricDefs left as-is (shared, harmless)
    return Promise.resolve();
  }
  function hasData() {
    return Object.keys(_rows).length > 0;
  }
  ...
  function filterSynthetic(rows, includeSynthetic) {
    return includeSynthetic === false
      ? rows.filter(function (r) {
          return !r.synthetic;
        })
      : rows;
  }
  function countSynthetic() {
    var n = 0;
    Object.keys(_rows).forEach(function (id) {
      if (_rows[id].synthetic) n++;
    });
    return n;
  }
  function hasSynthetic() {
    return countSynthetic() > 0;
  }

  function _allRows(includeSynthetic) {
    var rows = Object.keys(_rows).map(function (k) {
      return _rows[k];
    });
    return filterSynthetic(rows, includeSynthetic);
  }
  function nodes(includeSynthetic) {
    var s = {};
    _allRows(includeSynthetic).forEach(function (r) {
      s[r.node] = 1;
    });
    return Object.keys(s).sort();
  }
  function datesSorted(includeSynthetic) {
    var s = {};
    _allRows(includeSynthetic).forEach(function (r) {
      s[r.date] = 1;
    });
  ...
  function metricKeys(includeSynthetic) {
    var s = {};
    _allRows(includeSynthetic).forEach(function (r) {
      Object.keys(r.values || {}).forEach(function (mid) {
        if (r.values[mid] != null) s[r.node + '|' + mid] = 1;
      });
    });
  ...
  function seriesFor(node, mid, includeSynthetic) {
    return _allRows(includeSynthetic)
      .filter(function (r) {
        return r.node === node && r.values && r.values[mid] != null;
      })
      .map(function (r) {
        return { date: r.date, t0Ms: r.t0Ms, v: r.values[mid], w: r.weight, lowQuality: r.lowQuality, synthetic: !!r.synthetic };
      })
      .sort(function (a, b) {
  ...
  function evBadge(def) {
    try {
      return def && def.evidence && global.MetricRegistry ? global.MetricRegistry.badge(def.evidence, def.cite) : '';
    } catch (e) {
      return '';
    }
  }
  ...
  function crossCorrelations(minShared, includeSynthetic) {
    ...
    keys.forEach(function (k) {
      ...
      var map = {};
      seriesFor(node, mid, includeSynthetic).forEach(function (p) {
        map[p.date] = p.v;
      });
      idx[k] = { node: node, mid: mid, map: map };
    });
    var out = [];
    for (var i = 0; i < keys.length; i++) {
      for (var j = i + 1; j < keys.length; j++) {
        ...
        if (r == null) continue;
        var dA = defOf(A.node, A.mid),
          dB = defOf(B.node, B.mid);
        out.push({
          aNode: A.node,
          aMid: A.mid,
          aLabel: dA.label,
          aGood: dA.goodDirection,
          aEvidence: dA.evidence,
          aCite: dA.cite,
          bNode: B.node,
          bMid: B.mid,
          bLabel: dB.label,
          bGood: dB.goodDirection,
          bEvidence: dB.evidence,
          bCite: dB.cite,
          r: +r.toFixed(2),
          n: xs.length,
          dates: dts
  ...
  function state(includeSynthetic) {
    return {
      nodes: nodes(includeSynthetic),
      dates: datesSorted(includeSynthetic),
      nRows: _allRows(includeSynthetic).length,
      metricKeys: metricKeys(includeSynthetic),
      engineVersions: _engineVersions(includeSynthetic)
    };
  }
  function _engineVersions(includeSynthetic) {
    var s = {};
    _allRows(includeSynthetic).forEach(function (r) {
      if (r.engineVersion) s[r.engineVersion] = 1;
    });
    return Object.keys(s);
  }
  ...
  function _synthPrefKey() {
    return 'ganglior_long_includeSynth';
  }
  function _readSynthPref() {
    try {
      return localStorage.getItem(_synthPrefKey()) !== '0';
    } catch (e) {
      return true;
    }
  } // default: show
  function _writeSynthPref(inc) {
    try {
      localStorage.setItem(_synthPrefKey(), inc ? '1' : '0');
    } catch (e) {}
  }

  function render() {
    var host = document.getElementById('longBody');
    if (!host) return;
    var full = state(); // FULL store — drives the store-bar counts
    if (!full.nRows) {
      host.innerHTML =
        '<div class="long-empty">No longitudinal data yet. Load any node export that carries a <span class="mono">crossNight</span> block — a PpgDex / ECGDex / OxyDex / PulseDex multi-session export (≥2 recordings), or a bare <span class="mono">ganglior.crossnight</span> envelope. Summaries persist in this browser and accumulate across sessions.</div>';
      var sb0 = document.getElementById('longStoreBar');
      if (sb0) sb0.innerHTML = '';
      return;
    }
    ...
    if (sb) {
      ...
      var synCtl = nSyn
        ? '<button class="btn long-synth-toggle" id="longSynthToggle" title="Show/hide generated (synthetic) day-summaries in the views below. Real and synthetic rows for the same node|date collide by design (last write wins).">' +
          (inc ? 'Hide synthetic' : 'Show synthetic') +
          '</button>' +
        ...
        : '';
      sb.innerHTML =
        '<span class="long-store-ico">💾</span> <b>' +
        full.nRows +
        '</b> day-summaries persisted across <b>' +
        full.nodes.length +
        '</b> node' +
        (full.nodes.length === 1 ? '' : 's') +
        ' · <b>' +
        full.dates.length +
        '</b> distinct dates · survives reload' +
        synNote +
        engNote +
        '<span class="long-actions">' +
        synCtl +
        '<button class="btn btn-destructive" id="longClearBtn">Clear store</button></span>';
    }
    ...
    html +=
      '<div class="section-block"><h3>Cross-node coverage</h3>' +
    ...
    if (!cors.length) {
      ...
      html +=
        '<p class="long-sub">Pearson <i>r</i> between metrics from <b>different nodes</b>, joined on shared dates. Exploratory — ranked by strength, not multiple-comparison corrected. Direction noted against each metric\'s healthy direction.</p>';
    ...
    st.nodes.forEach(function (node) {
      var mids = {};
      _allRows(inc).forEach(function (r) {
        if (r.node === node)
          Object.keys(r.values || {}).forEach(function (m) {
            if (r.values[m] != null) mids[m] = 1;
          });
      });
      Object.keys(mids).forEach(function (mid) {
        var ser = seriesFor(node, mid, inc);
        if (ser.length < 2) return;
        var def = defOf(node, mid),
          col = nodeColor(node);
        var vals = ser.map(function (p) {
          return p.v;
        });
        var first = vals[0],
          last = vals[vals.length - 1];
        var delta = last - first;
        var dir = Math.abs(delta) < 1e-9 ? '→' : delta > 0 ? '↑' : '↓';
        var synChip = ser.some(function (p) {
          return p.synthetic;
        })
          ? '<span class="ltc-synth" title="includes synthetic (generated) days">synthetic</span>'
          : '';
        html +=
          '<div class="long-trend-cell"><div class="ltc-head"><span class="ltc-node" style="color:' +
          col +
          '">' +
          _esc(node) +
          synChip +
          '</span><span class="ltc-metric">' +
          _esc(def.label) +
          evBadge(def) +
          '</span></div>' +
          _spark(vals, col) +
          '<div class="ltc-foot"><span class="mono">' +
          +last.toFixed(2) +
          ' ' +
          _esc(def.unit) +
          '</span><span class="mono ltc-dir">' +
          dir +
          ' ' +
          (delta > 0 ? '+' : '') +
          +delta.toFixed(2) +
    ...
    if (tg)
      tg.addEventListener('click', function () {
        _writeSynthPref(!_readSynthPref());
        render();
      });
    var sc = document.getElementById('longSynthClear');
    if (sc)
      sc.addEventListener('click', function () {
        clearSynthetic().then(function () {
          render();
        });
      });
  }
  ...
  function _coverageGrid(st, includeSynthetic) {
    ...
    var head =
      '<th class="cg-node-h"></th>' +
      dates
        .map(function (d) {
          return '<th class="cg-date-h"><span>' + _esc(d.slice(5)) + '</span></th>';
        })
        .join('');
    var body = nodesL
      .map(function (node) {
        ...
        var cells = dates
          .map(function (d) {
            ...
            return (
              '<td class="cg-cell cg-on' +
              lq +
              sy +
              '" style="--c:' +
              col +
              '" title="' +
              _esc(node + ' · ' + d + (r.lowQuality ? ' · low quality' : '') + (r.synthetic ? ' · synthetic' : '')) +
              '"></td>'
            );
          })
          .join('');
  ...
  function _corCard(c) {
    var strong = Math.abs(c.r) >= 0.6,
      mod = Math.abs(c.r) >= 0.4;
    ...
    var reading;
    if (c.r >= 0.4) {
      reading = c.aGood === c.bGood ? 'rise and fall together — consistent direction' : 'one improving tracks the other worsening';
    } else if (c.r <= -0.4) {
      reading = c.aGood === c.bGood ? 'one rises as the other falls — divergent' : 'as one improves the other does too (inverse metrics)';
    } else reading = 'weak association';
    return (
      '<div class="cor-card" style="--c:' +
      accent +
      '">' +
      '<div class="cor-pair"><span class="cor-node" style="color:' +
      nodeColor(c.aNode) +
      '">' +
      _esc(c.aNode) +
      '</span> <b>' +
      _esc(c.aLabel) +
      evBadge({ evidence: c.aEvidence, cite: c.aCite }) +
      '</b>' +
      '<span class="cor-link">' +
      (c.r < 0 ? '⟷' : '⟺') +
      '</span>' +
      '<span class="cor-node" style="color:' +
      nodeColor(c.bNode) +
      '">' +
      _esc(c.bNode) +
      '</span> <b>' +
      _esc(c.bLabel) +
      evBadge({ evidence: c.bEvidence, cite: c.bCite }) +
      '</b></div>' +
      '<div class="cor-r"><span class="cor-rval">r = ' +
      (c.r > 0 ? '+' : '') +
      c.r +
      '</span><span class="cor-n">' +
      c.n +
      ' shared days</span></div>' +
      '<div class="cor-note">' +
      (strong ? 'Strong' : mod ? 'Moderate' : 'Weak') +
      ' — metrics ' +
      arrow +
      '. ' +
      reading +
      '.</div>' +
      '<span class="cor-ev">' +
      (global.MetricRegistry ? global.MetricRegistry.badge('experimental', 'Exploratory cross-night Pearson r — ranked by strength, not multiple-comparison corrected.') : '') +
      '</span></div>'
    );
  }
