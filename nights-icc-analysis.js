...
(function () {
  ...
  function bootPools(K) {
    return Promise.all([bootKind(oxyPool, 'oxy', K), bootKind(iccPool, 'iccpg', K)]);
  }
  ...
  function idbOpen() {
    return new Promise(function (res, rej) {
      var r = indexedDB.open('nicc_ckpt', 1);
  ...
  function lockFresh() {
    try {
      var l = JSON.parse(localStorage.getItem('nicc_lock') || 'null');
  ...
  function lockBeat() {
    try {
      localStorage.setItem('nicc_lock', JSON.stringify({ id: RUN_ID, ts: Date.now() }));
    } catch (e) {}
  }
  function lockRelease() {
    try {
      var l = lockFresh();
      if (l && l.id === RUN_ID) localStorage.removeItem('nicc_lock');
    } catch (e) {}
  }
  function fmtETA(sec) {
  ...
  function measureFromResults(seed, oxyRes, iccRes) {
    ...
    var rec = { seed: seed, age: prof.age, baseAHI: prof.baseAHI, sev: prof.osaSeverity, odi: [], rmssd: [], cv: [], odiLatent: [], rmssdLatent: [] };
    var on = oxyRes.result.oxy && oxyRes.result.oxy.score && oxyRes.result.oxy.score.nights;
    (on || []).forEach(function (sc) {
      if (sc && sc.odi != null && isFinite(sc.odi)) rec.odi.push(sc.odi);
    });
    (iccRes.result.pulse || []).forEach(function (pp) {
      var ps = pp.score || pp;
      if (ps && ps.rmssd != null && isFinite(ps.rmssd)) rec.rmssd.push(ps.rmssd);
    });
    (iccRes.result.perDay || []).forEach(function (d) {
      if (d.cv != null && isFinite(d.cv)) rec.cv.push(d.cv);
    });
    gtN.forEach(function (n) {
      if (n.ahiTruth != null) rec.odiLatent.push(n.ahiTruth);
      if (n.rmssdTarget != null) rec.rmssdLatent.push(n.rmssdTarget);
    });
    return rec;
  }
  ...
  async function run(resumeCk) {
    ...
    setStatus('booting ' + K + '× OxyDex + ' + K + '× (PulseDex+GlucoDex) realms…', 'run');
    setProg(0);
    if (!oxyPool.length || !iccPool.length) await bootPools(K);
    var oxyRdy = oxyPool.filter(function (r) {
        return r.ready;
      }),
      iccRdy = iccPool.filter(function (r) {
        return r.ready;
      });
    if (!oxyRdy.length || !iccRdy.length) {
    ...
    var K2 = Math.min(oxyRdy.length, iccRdy.length),
      lanes = [];
    ...
    else ckptSave({ ver: ver, targetSubj: targetSubj, minNights: minNights, cursor: cursor, SUBJ: SUBJ, savedAt: Date.now() });
    try {
      localStorage.setItem('nicc_ptPerSec', String(pdone / Math.max(0.001, elapsed)));
    } catch (e) {}
  ...
  function analyze() {
    ...
    MET.forEach(function (m) {
      var groups = SUBJ.map(function (s) {
        return s[m];
      }).filter(function (a) {
        return a.length >= minNights;
      });
    ...
    MET.forEach(function (m) {
      ...
      var n75 = st ? minOcc(icc, 0.75) : null,
        n80 = st ? minOcc(icc, 0.8) : null,
        n90 = st ? minOcc(icc, 0.9) : null;
    ...
    MET.forEach(function (m) {
      ...
      RESULT.metrics[m] = {
        ...
        occasion: META[m].occ,
        unit: META[m].unit,
        icc1: +st.icc.toFixed(4),
        subjects: st.k,
        occasions: st.N,
        medianOccPerSubject: st.medianOcc,
        varBetween: +st.varB.toFixed(4),
        varWithin: +st.varW.toFixed(4),
        withinSD: +st.withinSD.toFixed(3),
        withinCVpct: st.withinCVpct != null ? +st.withinCVpct.toFixed(1) : null,
        minOcc: { 0.75: minOcc(st.icc, 0.75), '0.80': minOcc(st.icc, 0.8), '0.90': minOcc(st.icc, 0.9), target: minOcc(st.icc, target) }
      };
    });
    ['odi', 'rmssd'].forEach(function (m) {
      if (lat[m]) RESULT.latent[m] = { icc1: +lat[m].icc.toFixed(4), subjects: lat[m].k };
    });
  ...
  function axes(ctx, P, w, h, xmin, xmax, ymin, ymax, xt, yt, xlab, ylab, xfmt, yfmt) {
    ...
    for (i = 0; i <= xt; i++) {
      var xv = xmin + ((xmax - xmin) * i) / xt,
        px = P + ((w - P - 14) * i) / xt;
      ctx.fillStyle = '#6f8096';
      ctx.fillText(xfmt ? xfmt(xv) : Math.round(xv), px - 6, h - P + 16);
    }
    for (i = 0; i <= yt; i++) {
  ...
  function drawCurves(stats, target) {
    ...
    axes(
      ...
      11,
      5,
      'occasions averaged (nights / days)',
      'reliability  ICC(m)',
      function (v) {
        return Math.round(v);
      },
    ...
    ctx.setLineDash([5, 4]);
    ctx.beginPath();
    ctx.moveTo(P, Y(target));
    ctx.lineTo(w - 14, Y(target));
    ctx.stroke();
    ctx.setLineDash([]);
    ctx.fillStyle = '#9fb0c4';
    ctx.font = '10px ui-monospace';
    ctx.fillText('target ' + target.toFixed(2), w - 110, Y(target) - 5);
    MET.forEach(function (m) {
      var st = stats[m];
      if (!st) return;
      var icc = st.icc,
      ...
      ctx.lineWidth = 2.2;
      ctx.beginPath();
      for (var mm = 1; mm <= XMAX; mm++) {
        var r = sb(icc, mm);
        (mm === 1 ? ctx.moveTo : ctx.lineTo).call(ctx, X(mm), Y(r));
      }
      ctx.stroke();
      // single-occasion dot
      ctx.fillStyle = col;
      ctx.beginPath();
      ctx.arc(X(1), Y(icc), 3.2, 0, 7);
      ctx.fill();
      ...
      if (isFinite(k) && k <= XMAX) {
        ...
        ctx.fillStyle = col;
        ctx.beginPath();
        ctx.arc(kx, ky, 4.2, 0, 7);
        ctx.fill();
        ctx.fillStyle = '#e6edf6';
        ctx.font = 'bold 11px ui-monospace';
        ctx.fillText(META[m].label + ' → ' + k, kx + 7, ky - 6);
      }
    });
    $('curvesLeg').innerHTML = MET.map(function (m) {
      var st = stats[m];
      return '<span class="chip" style="background:' + META[m].color + '"></span>' + META[m].label + (st ? ' (ICC₁=' + st.icc.toFixed(2) + ')' : '');
    }).join(' &nbsp; ');
  }
  function drawIcc(stats, target) {
    ...
    for (var t = 0; t <= 5; t++) {
      ...
      ctx.beginPath();
      ctx.moveTo(gx, top - 6);
      ctx.lineTo(gx, top + rowH * MET.length);
      ctx.stroke();
      ctx.fillStyle = '#6f8096';
      ctx.fillText((t / 5).toFixed(1), gx - 7, top + rowH * MET.length + 16);
    }
    MET.forEach(function (m, i) {
      ...
      ctx.fillText(META[m].label, 12, y + bh - 7);
      ctx.fillStyle = '#6f8096';
      ctx.font = '10px ui-monospace';
      ctx.fillText(META[m].node, 12, y + bh + 8);
      if (!st) return;
      // between (filled = ICC) + within fraction
      var icc = st.icc,
        bw = x1 - x0;
      ctx.fillStyle = 'rgba(255,255,255,.07)';
      rrect(ctx, x0, y, bw, bh, 5);
      ctx.fill();
      ctx.fillStyle = col;
      rrect(ctx, x0, y, bw * icc, bh, 5);
      ctx.fill();
      ctx.fillStyle = '#0b0e13';
      ctx.font = 'bold 12px ui-monospace';
      ctx.fillText(icc.toFixed(2), x0 + 8, y + bh - 8);
    ...
    ctx.fillStyle = '#9fb0c4';
    ctx.font = '11px ui-monospace';
    ctx.fillText('single-occasion ICC (filled = reliable share of variance)   ·   target ICC ≥ ' + target.toFixed(2), 12, top + rowH * MET.length + 16);
  }
  ...
  function ptStyle(n) {
    if (n <= 3000) return { r: 2.0, a: 0.55 };
    if (n <= 15000) return { r: 1.2, a: 0.3 };
    if (n <= 40000) return { r: 0.8, a: 0.18 };
    return { r: 0.6, a: 0.11 };
  }
  function pdot(ctx, x, y, st) {
    if (st.r <= 0.8) {
  ...
  function drawRepro(stats) {
    ...
    var cv = $('repro'),
      ...
      P = 46;
    clearC(ctx, w, h);
    var subs = SUBJ.filter(function (s) {
      return s.rmssd.length >= minNights;
    })
      .map(function (s) {
        return { vals: s.rmssd, m: mean(s.rmssd) };
      })
      .sort(function (a, b) {
        return a.m - b.m;
      });
    ...
    var ymin = Math.max(0, Math.floor(Math.min.apply(null, allv) / 5) * 5 - 2),
      ymax = Math.ceil(Math.max.apply(null, allv) / 5) * 5 + 2;
    axes(
      ctx,
      P,
      w,
      ...
      ymin,
      ymax,
      Math.min(subs.length - 1, 6),
      5,
      'subject (sorted by mean rMSSD)',
      'measured rMSSD (ms)',
      function () {
        return '';
      },
    ...
    var X = function (i) {
      return P + (w - P - 14) * (subs.length <= 1 ? 0.5 : i / (subs.length - 1));
    };
    var Y = function (v) {
      return h - P - ((h - P - 12) * (v - ymin)) / (ymax - ymin);
    };
    var stR = ptStyle(subs.length);
    subs.forEach(function (s, i) {
      var x = X(i);
      ctx.strokeStyle = 'rgba(255,184,77,' + (stR.a * 0.6).toFixed(2) + ')';
      ctx.lineWidth = 1;
      ctx.beginPath();
      ctx.moveTo(x, Y(Math.min.apply(null, s.vals)));
      ctx.lineTo(x, Y(Math.max.apply(null, s.vals)));
      ctx.stroke();
      ...
      ctx.beginPath();
      ctx.arc(x, Y(s.m), subs.length > 3000 ? 1.4 : 3, 0, 7);
      ctx.fill();
    });
    var st = stats.rmssd;
    $('reproLeg').innerHTML =
      'Each column = one subject (vertical bar = night-to-night range, large dot = subject mean). ' +
  ...
  $('dlCsv').onclick = function () {
    ...
    SUBJ.forEach(function (s) {
      MET.forEach(function (m) {
        s[m].forEach(function (v, i) {
          rows.push([s.seed, s.age, s.baseAHI, s.sev, m, i, v]);
        });
      });
    });
    dl(
      'nights-icc-results.csv',
      rows
        .map(function (r) {
          return r.join(',');
        })
  ...
  $('dlStats').onclick = function () {
    dl('nights-icc-stats.json', JSON.stringify(RESULT, null, 2), 'application/json');
  };
  $('dlFig').onclick = function () {
    ...
    out.toBlob(function (bl) {
      ...
      an.href = u;
      an.download = 'nights-icc-figures.png';
      an.click();
    }, 'image/png');
  };
