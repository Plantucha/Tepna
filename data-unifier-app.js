...
(function () {
  ...
  function pulseHost() { return ORCH.pulseHost(); }
  function oxyHost() { return ORCH.oxyHost(); }
  function hrvHost() { return ORCH.hrvHost(); }
  ...
  function emitNodeExport(frame) { var e = ORCH.emitNodeExport(frame); if (e && e.schema && e.schema.ingest) e.schema.ingest.via = 'Data Unifier'; return e; }
  ...
  // ── routing one file ────────────────────────────────────────────────────────
  function head(text) { return (text || '').slice(0, 2048); }

  function processOne(file, text, pw, ow, hw, entries) {
    var REG = window.SignalAdapters, SF = window.SignalFrame, ORCH = window.SignalOrchestrate;
  ...
  function _emitNote(exp) {
    return (window.SignalOrchestrate && window.SignalOrchestrate.nodeExportSummary)
      ? window.SignalOrchestrate.nodeExportSummary(exp) : 'computed';
  }
  ...
  function renderResult(res, idx) {
    var f = res.frame, r = res.route;
    var card = document.createElement('div');
    card.className = 'rescard ' + pillClass(res);

    var best = r.best;
    var routeLine = best
      ? '<span class="vendor">' + esc(best.vendor) + '</span> <span class="muted">·</span> ' +
        '<span class="sig">' + esc(best.signalType) + '</span> ' +
        '<span class="conf">detect ' + best.confidence.toFixed(2) + '</span>' +
    ...
    card.innerHTML =
      '<div class="reshead"><span class="dot"></span><span class="fname">' + esc(res.file.name) + '</span>' +
        '<span class="route">' + routeLine + '</span></div>' +
      '<div class="resbody">' + body + '</div>' +
  ...
  function rerender(results) {
    ...
    results.forEach(function (res, i) { resultsEl.appendChild(renderResult(res, i)); });
    _lastExports = results;
  }

  // ── orchestration ───────────────────────────────────────────────────────────
  function setStatus(txt, cls) { var p = document.getElementById('status'); if (p) { p.textContent = txt; p.className = 'pill ' + (cls || 'idle'); } }

  async function handleFiles(fileList) {
    var files = Array.prototype.slice.call(fileList).filter(function (f) { return f && f.size != null; });
    if (!files.length) return;
    setStatus('routing ' + files.length + ' file' + (files.length === 1 ? '' : 's') + '…', 'run');
    var pw = null, ow = null, hw = null;
    try { pw = await pulseHost(); } catch (e) { setStatus('RR parser host failed: ' + e.message, 'bad'); }
    try { ow = await oxyHost(); } catch (e) { /* SpO₂ host optional — only needed if an O2Ring file is dropped */ }
    try { hw = await hrvHost(); } catch (e) { /* HRV host optional — only needed if a Welltory summary CSV is dropped */ }
    // read EVERY file's text up front so ECG/PPG companion sidecars can be paired by stamp (§2(b))
    var entries = [];
    for (var ri = 0; ri < files.length; ri++) {
      try { entries.push({ name: files[ri].name, text: await readText(files[ri]) }); }
      catch (e) { entries.push({ name: files[ri].name, text: '', __err: e.message }); }
    }
    var results = [];
    for (var i = 0; i < files.length; i++) {
      var ent = entries[i];
      if (ent.__err != null) { results.push({ file: files[i], route: { unknown: true, best: null }, frame: null, valid: null, note: 'read error: ' + ent.__err }); continue; }
      try {
    ...
    rerender(results);
    var ok = results.filter(function (r) { return r.frame && r.frame.usable && r.valid.ok; }).length;
  ...
  function wire() {
    ...
    dropEl.addEventListener('drop', function (e) { if (e.dataTransfer && e.dataTransfer.files) handleFiles(e.dataTransfer.files); });
    ...
    if (inv && window.SignalAdapters) {
      inv.innerHTML = window.SignalAdapters.list().map(function (a) {
