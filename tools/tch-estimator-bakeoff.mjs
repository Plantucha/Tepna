...
function nnls(Vab, Vac, Vbc) {
  ...
  const resid = (s) =>
    eqs.reduce((acc, [i, j, v]) => {
      const d = (s[i] || 0) + (s[j] || 0) - v;
      return acc + d * d;
    }, 0);
  ...
  for (const F of subsets) {
    ...
    const clamped = {};
    for (const k of LABELS) clamped[k] = Math.max(s[k] || 0, 0);
...
function solveFree(F, eqs) {
  ...
  for (const [i, j, v] of eqs) {
    ...
    if (fi) {
      A[idx[i]][idx[i]] += 1;
      b[idx[i]] += v;
      if (fj) A[idx[i]][idx[j]] += 1;
    }
    if (fj) {
      A[idx[j]][idx[j]] += 1;
      b[idx[j]] += v;
      if (fi) A[idx[j]][idx[i]] += 1;
    }
  }
  const x = solveLin(A, b);
  if (!x) return null;
  const out = { ECGDex: 0, PpgDex: 0, OxyDex: 0 };
  F.forEach((k, n) => {
    out[k] = x[n];
  });
  return out;
...
function solveLin(A, b) {
  // forward elimination (partial pivot) + back-substitution
  const n = b.length,
    M = A.map((r, i) => [...r, b[i]]);
  for (let c = 0; c < n; c++) {
    let p = c;
...
function sigmaFromS2(s2) {
  ...
  for (const k of LABELS) {
    const v = s2[k] == null ? null : Math.max(s2[k], 0);
    sigma[k] = v == null ? null : Math.sqrt(v);
  }
  let culprit = null,
    cv = -Infinity;
  for (const k of LABELS)
    if (s2[k] != null && s2[k] > cv) {
      cv = s2[k];
      culprit = k;
    }
...
function runNight(night) {
  const A = night.series.ECGDex.map((o, i) => o),
    B = night.series.PpgDex,
    C = night.series.OxyDex;
  // integrator-tch aligns via alignTriplet on {tMin,v}; here series are already index-aligned plain arrays
  const aA = A,
    aB = B,
    aC = C;
  const pAB = TCH.pairDiffVar(aA, aB),
    pAC = TCH.pairDiffVar(aA, aC),
    pBC = TCH.pairDiffVar(aB, aC);
  const Vab = pAB.v,
    Vac = pAC.v,
    Vbc = pBC.v;
...
function main() {
  ...
  for (const r of rows) {
    ...
    for (const e of ESTS) {
      const s = r.est[e].sigma,
        cul = r.est[e].culprit,
        m = r.est[e].method ? ` (${r.est[e].method})` : '';
  ...
  const winner = ['GCOV', 'NNLS'].find((e) => mae(quietErr(e)) < baseMAE - 0.1 && culpritOK(e));
  console.log('\n════════════════════════════════════════════════════════════════');
  if (winner) {
    ...
    console.log(
      `  BASE(min-ρ) quiet-MAE=${baseMAE.toFixed(3)} · GCOV=${mae(quietErr('GCOV')).toFixed(3)} · NNLS=${mae(quietErr('NNLS')).toFixed(3)} · ORACLE(external ρ)=${mae(quietErr('ORACLE')).toFixed(3)}`
    );
