...
function restampLedgers(bundleFile, oldHash, newHash) {
  ...
  /* ── THE SECOND ARTIFACT, WHICH THIS FUNCTION DOES NOT WRITE ────────────────────────────────
     Everything above re-stamps ONE thing: the fixture records inside `provenance/<App>.json`.
     Three of the nine nodes (ECGDex, OxyDex, PpgDex — measured 2026-09-23, and it is all-or-
     nothing per node) ALSO embed `code.manifestHash` inside the exported golden itself, and that
     copy can only be moved by re-running the app (`tools/regen-<node>-goldens.mjs`). A build
     cannot do it: §🔏 is explicit that an output is regenerated by re-running, never re-stamped
     blind. So we COUNT it and say it is owed. Reported, never written.
     Row 2026-09-23-build-restamp-line-counts-not-names: the old line said "N fixture(s)
     re-stamped", a session read that as completion, and shipped a PR whose goldens still carried
     the previous code identity — the gate caught it, one artifact later than it needed to. */
  return { manifest: 1, fixtures: owned.length, embedded: embeddedStampCount(owned) };
}

/* Exported and PURE so the carrier/non-carrier split is a known-answer test over committed bytes
   rather than a claim in a comment: ECGDex/OxyDex/PpgDex goldens embed `code.manifestHash`, the
   other six nodes' do not, and it is all-or-nothing per node (measured 2026-09-23 over the 38
   fixtures named in `provenance/<App>.json`). A golden ABSENT from this checkout is NOT counted and
   NOT claimed - its absence says nothing about whether it embeds a stamp, and guessing either way is
   the fabricated absence the Clock Contract and CLAUDE.md forbid. */
/* A MESSAGE THAT NAMES A FILE HAS TO HAVE LOOKED FOR IT. The regen tools are NOT uniformly
   `regen-<bundle>-goldens.mjs`: CPAPDex's is `regen-cpap-goldens.mjs`, so a name built by string
   transformation is right for eight nodes and wrong for the ninth. It does not fire for CPAPDex
   today (that node embeds no stamp), which is exactly why it would go unnoticed until the day it
   did. So: check, and when the guess is absent say what is owed without inventing a path. */
function regenHintFor(bundleFile) {
  const stem = bundleFile.replace(/\.html$/, '').toLowerCase();
  const guess = 'tools/regen-' + stem + '-goldens.mjs';
  return existsSync(join(ROOT, guess)) ? 'run ' + guess : "re-run the app and re-export (this node's regen-*-goldens tool)";
}

export function embeddedStampCount(fixtureNames, root) {
  let n = 0;
  for (const k of fixtureNames) {
    try {
      if (readFileSync(join(root || ROOT, 'uploads', k), 'utf8').includes('"manifestHash"')) n++;
    } catch {
      /* unreadable here => not counted (see above) */
    }
  }
  return n;
}
      ...
      paint('  \u2713 ', C.green) +
        ...
        paint(
          '  manifestHash ' +
            ...
            r.assetNames.length +
            ' assets' +
            (stamp.fixtures ? ', ' + stamp.fixtures + ' fixture record(s) re-stamped in ' + fragPathFor(bundleFile) : '') +
            (stamp.embedded ? ' \u2014 ' + stamp.embedded + ' golden(s) ALSO carry an embedded code.manifestHash, which a build cannot move: ' + regenHintFor(bundleFile) : '') +
            ')',
          C.dim
        )
    );
