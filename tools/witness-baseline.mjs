...
export const LADDER = [
  ...
  '""', '"x"', '[]', '[1]', '{}', 'true', 'false', '5', '300', '0.5'
];

const RESERVED = new Set(['true', 'false', 'null', 'undefined', 'NaN', 'Infinity', 'typeof', 'instanceof',
  'Math', 'Number', 'String', 'Array', 'Object', 'JSON', 'Date', 'Boolean', 'isFinite', 'parseInt', 'parseFloat', 'in', 'of', 'new']);
...
if (IS_MAIN && !process.argv.includes('--selftest')) {
  if (!existsSync(SWEEPS)) { console.error('✗ no .mutation-sweeps — a missing INPUT, not a finding of zero.'); process.exit(2); }
  const files = [];
  ...
  const t0 = Date.now();
  let hit = 0, miss = 0, declined = 0;
  for (const p of probes) {
    const w = enumerate(p.a, p.b);
    if (w) hit++; else if (freeVars(p.a).length > 4 || !freeVars(p.a).length) declined++; else miss++;
  }
