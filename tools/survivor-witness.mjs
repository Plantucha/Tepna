...
export function condOf(line) {
  ...
  for (let k = i + 3; k < line.length; k++) {
    if (line[k] === '(') d++;
    else if (line[k] === ')') { d--; if (d === 0) return line.slice(i + 4, k); }
  }
  return null;
}
...
export function probesFrom(files) {
  ...
  for (const { file, survivors } of files) {
    for (const r of survivors || []) {
      const a = condOf(r.before), b = condOf(r.after);
...
async function ask(c) {
  ...
  return String(j.response || '').trim().split('\n').filter((l) => l.trim())[0] || '';
}

const IS_MAIN = (() => { try { return process.argv[1] && join(process.argv[1]) === fileURLToPath(import.meta.url); } catch { return false; } })();
...
if (IS_MAIN && process.argv.includes('--selftest')) {
  ...
  const p = probesFrom([{ file: 'f.js', survivors: [
...
if (IS_MAIN && !process.argv.includes('--selftest')) {
  ...
  const led = new ResumeLedger(argv.includes('--resume') ? join(SWEEPS, 'witness-progress.jsonl') : null,
    fingerprint({ tool: 'survivor-witness@1', model: MODEL, n: probes.length })).load();
  ...
  let hit = 0, none = 0, no = 0, threw = 0, n = 0;
  for (const c of todo) {
    if (Date.now() > deadline) { process.stderr.write('\n  ⏱ budget reached — stopping cleanly\n'); break; }
    let raw;
    try { raw = (await ask(c)).replace(/^```\w*|```$/g, '').trim(); } catch (e) { process.stderr.write('  ⚠ model call failed: ' + e.message + '\n'); break; }
    n++;
    let rec;
    if (/^NONE$/i.test(raw)) { none++; rec = { verdict: 'NONE' }; }
    else {
      ...
      } else if (v.err) { threw++; rec = { verdict: 'THREW', err: v.err }; }
      else { no++; rec = { verdict: 'NO-SEPARATION', tried: raw }; }
    }
