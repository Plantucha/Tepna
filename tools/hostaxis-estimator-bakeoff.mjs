...
const probe = spine.hostAxis(
  geom.devMs.map((d, i) => ({ devMs: d, hostMs: d + geom.host[i] * 0 + i })),
  {}
);
...
for (const noiseKind of ['symmetric', 'oneSided']) {
  ...
  const best = rows[0],
    shipped = rows.find((r) => r.k === 'median-21');
  console.log(
    `  → best ${best.k} at ${best.worst.toFixed(1)} ms vs shipped median-21 at ${shipped.worst.toFixed(1)} ms` +
