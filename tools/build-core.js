...
(function (root) {
  ...
  function sha256hex(str) {
    ...
    var w = new Array(64), a, b, c, d, e, f, g, hh, t1, t2, i, j;
  ...
  function isPlainInline(bundleText) {
    return typeof bundleText === 'string' &&
      /\bdata-inline-src="/.test(bundleText) &&
      !/<script type="__bundler\/manifest">/.test(bundleText);
  }
  ...
  function isLocalRef(p) { return !!p && !/^(?:[a-z]+:)?\/\//i.test(p) && !/^data:/i.test(p) && !/^#/.test(p); }

  function scanRefs(srcHtml) {
    var scripts = [], styles = [], m;
    var sre = /<script\b([^>]*?)\bsrc="([^"]+)"([^>]*)>\s*<\/script>/gi;
    while ((m = sre.exec(srcHtml)) !== null) { if (isLocalRef(m[2])) scripts.push(m[2]); }
    var lre = /<link\b([^>]*)>/gi, tag;
  ...
  function build(opts) {
    opts = opts || {};
    var src = opts.srcHtml, assets = opts.assets || {};
    if (typeof src !== 'string') throw new Error('build: srcHtml must be a string');
    var out = '', i = 0, si = 0, ci = 0, assetNames = [];
    var tagRe = /<(script|style|link)\b/gi, m;
    while ((m = tagRe.exec(src)) !== null) {
      var start = m.index, kind = m[1].toLowerCase();
      ...
      if (kind === 'link') {                                       // void element
        ...
        } else { out += openTag; }                                 // icon / preload / non-local
        i = gt + 1; tagRe.lastIndex = i; continue;
      }

      // script | style: find the matching close from just past the open tag
      var closeRe = new RegExp('</' + kind + '\\s*>', 'i');
      var tail = src.slice(gt + 1), cm = tail.match(closeRe);
      if (!cm) throw new Error('build: unterminated <' + kind + '> element at ' + start);
      var body = tail.slice(0, cm.index), closeEnd = gt + 1 + cm.index + cm[0].length;
      ...
      if (kind === 'script') {
        ...
        if (/\bdata-inline-src=/i.test(openTag)) { out += src.slice(start, closeEnd); }
        else {
