 ...
 *   node tools/oracle-ecg-firmware-rr.mjs [--root <captures root>] [--min-beats 3600] [--limit N] [--json]
 * ═══════════════════════════════════════════════════════════════════════════════════════════════
 */
...
export function scoreNight(ECGDSP, ecgText, rrText, opts = {}) {
  ...
  const selfNN = rawNN.filter((x) => Number.isFinite(x) && x >= 200 && x <= 3000);
...
function report(rows, P, cov, roots) {
  ...
  L.push('');
  L.push('night        stamp           nSelf  nDev   excl  Δn%    med|Δ| ms  LoA ms  emp95   bias ms  dMean%  dRMSSD%  dSDNN%   decay  bands');
  for (const r of rows) {
    if (!r.ok) {
...
function selftest() {
  ...
  console.log(fails.length ? fails.length + ' failed of 14' : 'all 14 selftests passed');
...
async function main(argv) {
  ...
  for (const p of chosen) {
    let s;
    try {
      s = scoreNight(ECGDSP, readFileSync(p.ecg, 'utf8'), readFileSync(p.rr, 'utf8'), { minBeats });
    } catch (e) {
