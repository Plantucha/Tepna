...
export const TOOLS = [
  {
    ...
    function: {
      name: 'read_file',
      description: 'Read a file from the Tepna repo (path relative to repo root). Large files are truncated; pass start_line to page.',
      parameters: { type: 'object', properties: { path: { type: 'string' }, start_line: { type: 'number' } }, required: ['path'] }
    }
  },
  {
    type: 'function',
    function: {
      name: 'doc_search',
      description: 'Semantic (bge) search over the repo docs/briefs/code. Returns top hits with snippets. Use for INTENT: what a thing is documented to do.',
  ...
  },
  {
    ...
    function: {
      name: 'grep_repo',
      description: 'git grep -n a fixed string (not regex) across the repo. Use for occurrences of an identifier.',
      parameters: { type: 'object', properties: { text: { type: 'string' } }, required: ['text'] }
    }
  },
  {
    type: 'function',
    function: { name: 'list_dir', description: 'List a repo directory (names + sizes).', parameters: { type: 'object', properties: { path: { type: 'string' } }, required: ['path'] } }
  }
];
...
export function runTool(name, args) {
  try {
      ...
      return readdirSync(p)
        ...
        .map((f) => {
          try {
            ...
            return `${st.isDirectory() ? 'd' : '-'} ${st.size}\t${f}`;
          } catch {
            return '? ?\t' + f;
          }
        })
    ...
    return 'TOOL ERROR: ' + String(e.message || e).slice(0, 200);
  }
}
...
function distilledClaudeMd() {
  try {
    ...
    return (grab('## 🔒 THE CLOCK CONTRACT', 1500) + grab('### 5. Display — ALWAYS `getUTC*`', 300) + grab('## 📏 Units', 400) + grab('## 🎫 Evidence badges', 400)).slice(0, 3000);
  } catch {
    return '(CLAUDE.md unavailable — apply the Clock Contract and honest-null rules from memory)';
  }
}
...
async function main() {
  ...
  if (!task) {
    console.error('usage: qwen-agent.mjs "task"');
    process.exit(2);
  }
