...
function realm(files) {
  ...
  sb.navigator = { userAgent: 'roundtrip' }; sb.localStorage = { getItem: () => null, setItem() {}, removeItem() {} };
...
const iqr = (a) => { const s = [...a].sort((x, y) => x - y); const q = (p) => s[Math.min(s.length - 1, Math.floor(p * s.length))]; return q(0.75) - q(0.25); };
...
const ringHR = [];
{
  ...
  const iT = hdr.indexOf('Time'), iP = hdr.indexOf('Pulse Rate');
...
const [p0, p1] = span(ppgBeats), [e0, e1] = span(ecgBeats), [r0, r1] = span(ringHR);
const lo = Math.max(p0, e0, r0), hi = Math.min(p1, e1, r1);
...
console.log('  site=' + pres.site, '· channels=' + prec.ch.length, '· fs=' + prec.fs.toFixed(1) + ' Hz',
  '· ledAgreementPct=' + pres.ledAgreementPct, '· ledSingleChannel=' + pres.ledSingleChannel);
...
const pW = win(ppgBeats), eW = win(ecgBeats), rW = win(ringHR);
const mP = median(pW), mE = median(eW), mR = median(rW);
