...
export function walk(art) {
  ...
  // envelope
  const ev = b.evidence || {};
  if (ev.envelopeRef) push('envelope', true, 'envelopeRef ' + ev.envelopeRef + ' (join on the .dat session_id)');
  ...
  if (art.ledgerRec) {
    ...
    if (art.inputSha16) {
      const f = Object.keys(rec.inputHashes || {})[0];
      push('raw.inputHash', f && rec.inputHashes[f] === art.inputSha16, `ledger inputHashes[${f}] ${f && rec.inputHashes[f]} ≡ sha256[0:16](input bytes) ${art.inputSha16}`);
...
async function gather(fx, ctx) {
  ...
  if (inPath) {
    ...
    const night = ctx.OxyDex.computeNight({ text: bytes.toString('utf8') }, null);
...
async function main() {
  ...
  for (const fx of FIXTURES) {
    ...
    const hops = walk(art);
    report.push({ fixture: fx.name, input: art.inputPath, hops });
  ...
  if (has('--json')) {
    ...
    bad = report.reduce((n, r) => n + r.hops.filter((h) => h.ok === false && !/absent/.test(h.detail)).length, 0);
  }
  console.log(bad ? `\n✗ ${bad} hop(s) do not walk back — the chain is broken` : '\n✓ every derived hop walks back to the artifacts on disk');
