...
(function (root) {
  ...
  // DEEP-AUDIT-II §1.1-1.9 — the presence gate for DERIVED metrics. Every objective column parses
  // absent -> null (numOrNull, below), and JS coercion then makes absence look like a measurement:
  // `null >= 0` is TRUE, `null + 1` is 1, `null / x` is 0. So an expression that gates SOME of its
  // factors and consumes others ungated returns a CONFIDENT NUMBER for data never recorded.
  // Measured on a real export with no `hrv` block: d_welfare read 4200 (vs 91.3), d_otr saturated
  // at 500, d_abs returned +/-1, d_dfa_proxy returned 1. Those are fabrications, not defaults.
  // _all(...) is the missing half of every such gate: NaN unless EVERY factor is really present.
  // Note it rejects non-numbers too, so a stray string column cannot arithmetic its way to a value.
  function _all() {
    for (var i = 0; i < arguments.length; i++) {
      var v = arguments[i];
      if (v == null || typeof v !== 'number' || !isFinite(v)) return false;
    }
    return true;
  }
  ...
  function _hrvSig(r) {
    return [Math.round(r._tMs), _hrvNum(r._hr), _hrvNum(r._meanRR), _hrvNum(r._sdnn), _hrvNum(r._rmssd), _hrvNum(r._pnn50), _hrvNum(r._hf), _hrvNum(r._lf), _hrvNum(r._vlf), _hrvNum(r._stress)].join(
      '|'
    );
  }
  function _seedFromRow(r) {
    ...
    HRV_SEED_FIELDS.forEach((k) => {
      const v = r[k];
      s[k] = typeof v === 'number' && isFinite(v) ? v : null;
    });
    return s;
  }
  function _rowFromSeed(s) {
    ...
    r._date = isFinite(r._tMs) ? new Date(r._tMs) : null;
    HRV_SEED_FIELDS.forEach((k) => {
      r[k] = typeof s[k] === 'number' && isFinite(s[k]) ? s[k] : null;
    });
    return r;
  }
  ...
  function restoreHRVRows() {
    let seeds = null;
    try {
      const raw = localStorage.getItem(HRV_STORE_KEY);
  ...
  function _hrvRefreshChrome() {
    ...
    (function () {
      ...
      if (sdc && sdi && allRows && allRows.length) {
        ...
        sdi.innerHTML = allRows.length + ' sessions &bull; 85 metrics<br>' + (allRows[0] && fmtDate(allRows[0]._tMs) + ' &rarr; ' + fmtDate(allRows[allRows.length - 1]._tMs));
      }
    })();
  }
  ...
  function _envToSeed(env) {
    ...
    return {
      ...
      _psns: null,
      _coherence: null,
      _hrv: n(env.personalization && env.personalization.ansReadinessScore),
      _cv: n(tm.cv)
    };
  }
  function ingestGangliorJSON(text, opts) {
    ...
    if (Array.isArray(data))
      envs = data; // bare array of envelopes
    else if (data && Array.isArray(data.recordings))
      envs = data.recordings; // multiRecording payload
  ...
  function computeDerived(rowsArg) {
    ...
    _rows.forEach((r, i, arr) => {
      ...
      r.d_rmssd_sdnn = _all(r._rmssd, r._sdnn) && r._sdnn ? r._rmssd / r._sdnn : NaN;
      ...
      var _baev = typeof DexUnits !== 'undefined' && DexUnits && DexUnits.guardBaevsky ? DexUnits.guardBaevsky(r._mode, r._mxdmn) : null;
      if (_baev) {
      ...
      r.d_hfnu = _hasNu && _all(r._hf) ? (r._hf / (r._totalPow - r._vlf)) * 100 : NaN;
      r.d_lfnu = _hasNu && _all(r._lf) ? (r._lf / (r._totalPow - r._vlf)) * 100 : NaN;
      ...
      r.d_ans_load = r._sns > 0 && r._stress > 0 && r._psns > 0 && r._energy > 0 ? (r._sns * r._stress) / (r._psns * r._energy + 1) : NaN;
      r.d_coh_energy = _hasSubj ? (r._coherence * r._energy) / 100 : NaN;
      r.d_pti = _hasSubj && _all(r._rmssd) ? (r._psns * r._rmssd) / 100 : NaN;
      r.d_incoherent_stress = r._coherence > 0 && _all(r._stress) ? r._stress * (100 / r._coherence) : NaN;
      r.d_vei = r._hr > 0 && _all(r._rmssd) ? r._rmssd / r._hr : NaN;
      ...
      r.d_sdi = _hasLfHf && _all(r._sns, r._psns) ? (r._lf + r._sns * 10) / (r._hf + r._psns * 10) : NaN;
      // ── Poincaré SD1 / SD2 ──
      r.d_sd1 = _all(r._rmssd) ? r._rmssd / Math.sqrt(2) : NaN;
      const sd2sq = _all(r._sdnn) ? Math.max(0, 2 * r._sdnn * r._sdnn - r.d_sd1 * r.d_sd1) : NaN;
      ...
      r.d_efc = r._energy > 0 && r._focus > 0 && r._coherence > 0 ? (r._energy * 0.4 + r._focus * 0.3 + r._coherence * 0.3) / 100 : NaN;

      // ── Cardiac Resilience Score ──
      r.d_crs = r._stress > 0 && _all(r._coherence, r._rmssd, r._pnn50) ? (r._coherence * r._rmssd * r._pnn50) / (r._stress * 1000 + 0.001) : NaN;
      ...
      if (r._vlf > 0 && r._lf > 0 && r._hf > 0) {
        ...
        const mxf = (lf_arr[0] + lf_arr[1] + lf_arr[2]) / 3,
          mxp = (lp_arr[0] + lp_arr[1] + lp_arr[2]) / 3;
      ...
      const _vo2win7 = [];
      {
        ...
        for (let j = i; j >= 0 && _vo2win7.length < 7; j--) {
          ...
          if (_dk && !_seenvo2.has(_dk)) {
            _seenvo2.add(_dk);
            _vo2win7.unshift(arr[j]);
          }
        }
      }
      ...
      r.d_bap = !_all(r._hr) ? NaN : 0.011 * r._hr + 0.014 * AGE + 0.008 * _p.sbp + 0.014 * _p.dbp - 0.009 * _p.weight - 0.009 * _p.height - 0.27;
      // d_bap uses full Baevsky AP formula (all 7 terms) when SBP/DBP/weight/height set in profile
      r.d_ortho = r._sdnn > 0 && _all(r._hr) ? r._hr * (1 / r._sdnn) : NaN;
      ...
      r.d_cai = !isNaN(r.d_sd1) && !isNaN(r.d_sd2) && r.d_sd1 > 0 && r.d_sd2 > 0 ? Math.sqrt(r.d_sd1 * r.d_sd2) : NaN;
      ...
      r.d_welfare = r._energy > 0 && r._coherence > 0 && _all(r._stress) ? (r._energy * r._coherence) / (r._stress + 1) : NaN;
    ...
    allRows.forEach((r, i, arr) => {
      ...
      const window7 = [];
      {
        ...
        for (let j = i; j >= 0 && window7.length < 7; j--) {
          const _dk = isFinite(arr[j]._tMs) ? utcDayKey(arr[j]._tMs) : '';
          if (_dk && !_seen7.has(_dk)) {
            _seen7.add(_dk);
            window7.unshift(arr[j]);
          }
        }
      }
      ...
      const _win14ac = [];
      {
        ...
        for (let j = i; j >= 0 && _win14ac.length < 14; j--) {
          const _dk = isFinite(arr[j]._tMs) ? utcDayKey(arr[j]._tMs) : '';
          if (_dk && !_seen14ac.has(_dk)) {
            _seen14ac.add(_dk);
            _win14ac.unshift(arr[j]);
          }
        }
      }
      ...
      const window14 = [];
      {
        ...
        for (let j = i; j >= 0 && window14.length < 14; j--) {
          const _dk = isFinite(arr[j]._tMs) ? utcDayKey(arr[j]._tMs) : '';
          if (_dk && !_seen14.has(_dk)) {
            _seen14.add(_dk);
            window14.unshift(arr[j]);
          }
        }
      }
  ...
  function computeCAMQ(r) {
    ...
    var paraScore = 0,
      paraCount = 0;
    if (r._rmssd > 0) {
      paraScore += Math.min(100, r._rmssd * 2.5);
      paraCount++;
    }
    if (_all(r._pnn50) && r._pnn50 >= 0) {
      paraScore += Math.min(100, r._pnn50 * 4);
      paraCount++;
    }
    if (r._hf > 0) {
  ...
  function hrvEventsFromRows(rows) {
    ...
    (rows || []).forEach(function (r) {
      ...
      var tMs = r._tMs;
      if (isFinite(r._rmssd) && r._rmssd > 0 && r._rmssd < 20)
        // parasympathetic-low measurement (measured)
        ev.push({
          tMs: tMs,
          t: _hrvClockS(tMs),
          impulse: 'hrv_low',
      ...
      if (isFinite(r._stress) && r._stress >= 70)
        // high-stress: Welltory black-box composite (heuristic)
        ev.push({
          tMs: tMs,
          t: _hrvClockS(tMs),
          impulse: 'stress_high',
  ...
  function hrvBuildNodeExport(rows, opts) {
    ...
    var dated = (rows || [])
      .filter(function (r) {
        return r && isFinite(r._tMs);
      })
      .sort(function (a, b) {
        return a._tMs - b._tMs;
      });
    ...
    return {
      ...
      recording: {
        source: 'welltory',
        contentId:
          typeof SignalFrame !== 'undefined' && SignalFrame && SignalFrame.computeContentId && dated.length
            ? SignalFrame.computeContentId({
                signalType: 'hrv',
                kind: 'samples',
                samples: dated.map(function (r) {
                  return { t: r._tMs, rmssd: r._rmssd, sdnn: r._sdnn, hr: r._hr };
                }),
                t0Ms: t0,
                usable: true
              })
      ...
      measurements: dated.map(function (r) {
        ...
        var m = {
          tMs: r._tMs,
          offsetMin: r._offsetMin == null ? null : r._offsetMin,
          hr: r._hr,
          meanRR: r._meanRR,
          sdnn: r._sdnn,
          rmssd: r._rmssd,
          pnn50: r._pnn50,
          mxdmn: r._mxdmn,
          sd1: sd1 == null ? null : +sd1.toFixed(2),
          sd2: sd2 == null ? null : +sd2.toFixed(2)
        };
        var comp = {};
        if (r._stress > 0) comp.stress = r._stress;
        if (r._energy > 0) comp.energy = r._energy;
        if (r._focus > 0) comp.focus = r._focus;
        if (r._sns > 0) comp.sns = r._sns;
        if (r._psns > 0) comp.psns = r._psns;
        if (r._coherence > 0) comp.coherence = r._coherence;
        if (r._hrv > 0) comp.hrvScore = r._hrv;
        if (Object.keys(comp).length) {
          comp.meta = { derived: true, tier: 'heuristic' };
          m.composites = comp;
        }
        return m;
      }),
  ...
  HRVDex.derive = function (rows) {
    computeDerived(rows);
    return rows;
  };
  HRVDex.rowFromNodeExport = function (env) {
    var s = _envToSeed(env);
    return s ? _rowFromSeed(s) : null;
  };
