...
(function (global) {
  ...
  // ── LIVE-vs-SD COMPARATOR (CPAPDEX-LIVE-SD-COMPARATOR brief) ──────────────────────────────────────
  // Compare a BLE-live-captured channel to the device SD-card channel of the same night. Refusal-first;
  ...
  function compareChannel(a, b, opts) {
    opts = opts || {};
    var maxLagSec = opts.maxLagSec == null ? 5 : opts.maxLagSec; // FINE correction only. A WIDE xcorr to align through a BROKEN clock is a v2 opt-in
    // diagnostic ONLY: respiratory flow is quasi-periodic, so a wide search can lock onto the
    // WRONG breath (the beat-trains-align-only-mod-RR failure, respiratory form). Refusal-first
    // is the permanent default — a large clock disagreement is a finding, not a thing to align out.
    var tol = opts.tolerance == null ? 0 : opts.tolerance; // 0 => derive from residual SD
    if (!a || !b || !a.values || !b.values || !a.values.length || !b.values.length) {
      return { ok: false, reason: 'a channel is absent in one file — nothing to compare' };
    }
    var fs = a.fs || b.fs || 1;
    // The header-clock disagreement — the FINDING. Positive => the live header starts LATER than the SD.
    var clockOffsetSec = ((a.t0Ms || 0) - (b.t0Ms || 0)) / 1000;
    // Align on the device clock: the overlapping device-time window.
    var startMs = Math.max(a.t0Ms || 0, b.t0Ms || 0);
    var endMs = Math.min((a.t0Ms || 0) + (a.values.length / fs) * 1000, (b.t0Ms || 0) + (b.values.length / fs) * 1000);
    if (endMs - startMs < 1000) {
      // A large delta between a live capture and its SD twin is defect DETECTION, not an alignment
      // problem — the comparator is the instrument that screams when the EdfSink clock-bug class
      // reappears; aligning through it would launder the scream into a rosy scale. Quantify it.
      return {
        ok: false,
        clockOffsetSec: clockOffsetSec,
        reason:
          'no overlapping device-clock span — the headers disagree by ' +
          Math.round(clockOffsetSec) +
          ' s (' +
          (clockOffsetSec / 3600).toFixed(2) +
          ' h). Same-night live+SD files MUST overlap, so a\n' +
          'large delta is a clock defect (EdfSink UTC-vs-local-civil class), not a different night.'
      };
    }
    var ai0 = Math.round(((startMs - (a.t0Ms || 0)) / 1000) * fs);
    ...
    for (var i = 0; i < overlapN; i++) {
      var ia = ai0 + i,
        ib = bi + i;
      if (ia < 0 || ia >= a.values.length || ib < 0 || ib >= b.values.length) continue;
      A.push(a.values[ia]);
      B.push(b.values[ib]);
    }
    if (A.length < 2) return { ok: false, reason: 'too few paired samples after alignment' };
    var reg = ols(A, B); // B ≈ slope·A + intercept
    var resid = [],
      diffs = [];
    for (var k = 0; k < A.length; k++) {
      resid.push(B[k] - ((reg.slope || 0) * A[k] + (reg.intercept || 0)));
      diffs.push(B[k] - A[k]);
    }
    ...
    return {
      ...
      alignmentOffsetSec: clockOffsetSec + lag, // total device-clock offset the comparator applied
  ...
  function cpapCompare(liveSet, sdSet, opts) {
    ...
    for (var lab in labels) {
      channels[lab] = compareChannel(liveSet[lab], sdSet[lab], opts);
      if (channels[lab].ok) anyOk = true;
    }
    return anyOk ? { ok: true, channels: channels } : { ok: false, reason: 'no channel could be compared', channels: channels };
  }
