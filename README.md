...
[![Suite v2.15.0](https://img.shields.io/badge/suite-v2.15.0-2a6fdb?style=for-the-badge)](CHANGELOG.md)
...
[![Tests green](https://img.shields.io/badge/tests-9.1k_assertions_green-1f8a5b)](#-the-privacy-claim-is-a-test-not-a-promise)
...
![Releases](https://img.shields.io/badge/releases-41_shipped-555)

**[tepna.net](https://tepna.net)**  ·  [github.com/Plantucha/Tepna](https://github.com/Plantucha/Tepna)

<!-- readme-synced: cdae44a5 2026-09-06 -->
...
**Suite version:** 2.15.0 &nbsp;—&nbsp; **41** ledger-backed releases, each computed from a green tree, with **0 changesets** pending since (the tree is exactly the release).
...
| 🧪 **Tests** | **10,418 assertions** across **667 groups**, run as **6 partitioned CI shards** — green on every push |
...
>
> **In `2.13.0` (2026-09-17 → 2026-09-21, 181 commits, 143 changesets — four days).** The theme is
> **absence as a value, at every layer.** OxyDex's primary stats builder had counted a dropout as a
> reading (`null < 90` is true in JavaScript, so every absent second scored as desaturated); it now
> rates over measured seconds only, and the NSRR adapter reads SHHS1's own `OX stat` validity channel
> instead of discarding it. On the wearables, PpgDex reads the capture host's constant-run sidecar as a
> **second population** rather than a second sentinel, a *saturated* span is labelled distinctly from
> an *absent* one (owner ruling), and ECGDex's rail check — which had keyed on a global constant and
> **fired on nothing** across 112 real saturation runs — now finds each file's own rail. Measurements
> now travel with their provenance: OxyDex emits per-instance **measurement blocks** (value, window,
> channel, code identity, input lineage; schema 2.1) and the Integrator consumes them behind a
> fail-closed adapter; a published table can carry a `TABLE-PROVENANCE` stamp whose output hash reds
> a hand-edited cell; and two withheld corrections are out — a shipped bundle had carried retracted
> validation figures for **thirteen days**, the published site its uncorrected claims for **over a
> month**. ECGDex's Pan–Tompkins was scored against the strap's firmware detector on **52 real nights**
> with bands stated in advance: **98.7 %** of beats paired, RR |Δ| **0.45 ms**. The capture box asks a spare radio for an HCI round trip before
> failing over onto it, sends the passive-scan filter BlueZ actually requires, resolves the DST
> fall-back hour on its naive stamps, and keeps the **30 most recent nights** local while the rest are
> tiered to the NAS after a byte-for-byte check. capture-host's mypy count fell **100 → 41** with the
> ratchet banked at each step, and a five-round brief drain re-stamped dozens of status headers
> against the tree rather than against memory. Full history in the **[changelog](CHANGELOG.md)**.
...
| **PpgDex** | Wrist PPG | Polar Verity Sense · O2Ring (`_PPG2W`) | 🟢 **Live** | PPI → HRV + pulse-wave morphology |
...
| **tests** | 9,124 assertions over the real DSP modules, run as 6 parallel shards that provably *partition* the suite — so the union of the shards is the whole gate, and no group can quietly go unrun |
...
| **browser-gates** | Render-coverage — boots each real app bundle headless in an iframe and asserts computed values reach the DOM (`Dex-Test-Suite.html?full`, run on every PR) |
| **stale-file** | A brief cannot be overwritten by an edit that never read the upstream version — required, with no escape hatch but rebasing |
| **capture-host** | The Python capture lane on its own runners, py3.12 **and** py3.13 — `ruff` · `shellcheck` (strictest level, every `.sh`) · `pytest` at a **100 % statement and branch** coverage floor · `find_unwired --check` (no published stream without a consumer); `mypy` advisory, ratcheting down from a 103-error baseline |
| **CodeQL** | Static security analysis on every push |
| **mutation** *(advisory)* | Diff-scoped: mutates only the lines a PR touched and reports which of them the suite would not notice |

The `tests` workflow also carries a **`static`** job that re-derives every ledger the tree claims — bundle
hashes, served `docs/` copies, the analysis tools, brief and release ledgers, commit shape — so a green
test shard with a stale artefact still reds the PR.
...
the live BLE links (Polar H10 ECG, Verity PPG/ACC, the O2Ring's SpO₂ and raw PPG, and the AirSense 11
CPAP's live flow) and writes **existing vendor layouts** into
...
`capture-host/check.sh` does, and CI runs the same jobs: `ruff`, `shellcheck` at its strictest
level over every shell script, `find_unwired` (every stream written has a reader), and `pytest` with a
**100 % statement *and* branch** coverage floor across **6,500+ tests**, on two Python versions. That floor is the point: this code runs unattended overnight against
...
| an O2Ring vs the host, worst case | **−3035 ppm apparent** — and the mechanism is BLE link stalls, not the crystal (which holds sub-ppm between dropouts), so a single ppm figure is the wrong model, not merely an imprecise one |
...
Every non-trivial number traces to a published method, and each of the **22 working preprints** in
...
- **[One phone is not one clock](papers/wearable-clock-drift.html)** — a single-subject methods study,
  twice self-corrected: the negative result stands (beat-level cross-device PAT is not recoverable,
  0 of 54 pairings) but v1's "47.7 ppm drift" was one mis-attributed RR interval — the real
  inter-device rate is ~1.5 ppm, and the failure is beat-slip, not clock drift.
...
| You want… | Look at |
|---|---|
| **A map of every doc** | [`DOCS-INDEX.md`](DOCS-INDEX.md) — the single entry path over all ~500 briefs, audits, and READMEs, grouped by topic. Start here before opening any individual brief. |
...
| **The papers** | `papers/` — 22 working preprints, each regenerated from its tool. |
...
| **Testing the tests** | `tools/mutate.mjs` — breaks the code on purpose and reports which gates fail to notice. The honest measure of whether a green suite means anything; results in [`docs/MUTATION-INVENTORY.md`](docs/MUTATION-INVENTORY.md). |
| **Every tool, indexed** | [`docs/TOOLS-INDEX.md`](docs/TOOLS-INDEX.md) — the ~190 scripts under `tools/`, each with its purpose; gate-backed so a new tool cannot land undocumented. |
...
| **House rules** | `CLAUDE.md` (constitution) · `CONTRIBUTING.md` (on-ramp) · `ARCHITECTURE-PRINCIPLES.md` · [`docs/LEXICON.md`](docs/LEXICON.md). Open verified defects: [`briefs/RESIDUE.md`](briefs/RESIDUE.md), one row each. |

---

## On the metric count

The eight node registries define **520 graded metrics** (counted from the `evidence` field of every
registry entry at `cdae44a5`). That number describes **surface area, not confidence** — which is
exactly why the suite publishes the breakdown rather than the headline:

| Tier | Count | Share |
|---|---:|---:|
| **measured** — read off the device | 149 | 29 % |
| **validated** — anchored to a published standard | 86 | 17 % |
| **emerging** — published, device-dependent | 85 | 16 % |
| **experimental** — internally consistent, not externally confirmed | 143 | 27 % |
| **heuristic** — directional only | 57 | 11 % |

So **just under half** the surface is `measured` or `validated`, and **nearly two-fifths** sits at
`experimental` or `heuristic` — the share at the bottom of the ladder *grew* since the last count
(404 metrics, 31 %), because the metrics added were surfaced honestly at the tier the evidence
supports and two existing ones were re-tiered *down* on measurement. A count of 520 would be a
boast; the distribution is the honest version of the same fact, and it is why the ladder leads and
the count follows.
