...
function evBadge(label, fallback){
  try { return (window.GlucoRegistry && window.GlucoRegistry.badgeForLabel(label, fallback!==false)) || ''; }
  catch(e){ return ''; }
}
...
try { if (typeof localStorage!=='undefined' && localStorage.getItem('glucodex_dispUnit')==='mmol') _gluDispUnit='mmol'; } catch(e){}
const GluDisp = {
  get unit(){ return _gluDispUnit; },
  isMmol(){ return _gluDispUnit==='mmol'; },
  set(u){ _gluDispUnit = (u==='mmol') ? 'mmol' : 'mgdl'; try{ localStorage.setItem('glucodex_dispUnit', _gluDispUnit); }catch(e){} return _gluDispUnit; },
  label(){ return _gluDispUnit==='mmol' ? 'mmol/L' : 'mg/dL'; },
  // concentration value (stored mg/dL) -> display number (mg/dL integer · mmol/L 1 dp)
  val(mgdl){ if(mgdl==null || !isFinite(+mgdl)) return mgdl; return _gluDispUnit==='mmol' ? +(+mgdl/_GLU_MMOL).toFixed(1) : +mgdl; },
  // dispersion value (SD / MAGE / CONGA / MODD — same linear scale)
  spread(mgdl){ return GluDisp.val(mgdl); },
  // signed delta (dawn rise, sensor bias) — sign preserved
  delta(mgdl){ if(mgdl==null || !isFinite(+mgdl)) return mgdl; const sg=(+mgdl<0)?-1:1; return sg*GluDisp.val(Math.abs(+mgdl)); },
  // canonical band-edge / axis-tick label (standardized mmol cutoffs)
  tick(mgdl){ return _gluDispUnit==='mmol' ? (_GLU_MMOL_LABEL[mgdl] || (+mgdl/_GLU_MMOL).toFixed(1)) : String(mgdl); },
  range(lo,hi){ return GluDisp.tick(lo)+'–'+GluDisp.tick(hi); },
  cmp(op,mgdl){ return op+GluDisp.tick(mgdl); }
};
...
class GlucoScope {
  constructor(canvas, mini){
    ...
    this.data=null; this.light=false;
    this.view={ start:0, span:0 };          // cell-domain viewport
    this._bind();
  }
  setData(r){
    const s=r.series;
    this.s=s; this.N=s.N; this.cad=s.cadence;
    this.gT=s.gT; this.gV=s.gV; this.gF=s.gF; this.FLAG=s.FLAG;
    this.yLo=40; this.yHi=Math.max(260, r.max+20);
  ...
  draw(){
    ...
    const padL=44*dpr, padR=10*dpr, padT=8*dpr, padB=20*dpr;
    ...
    const bands=[[this.yLo,CUT.vlow,BAND.vlow],[CUT.vlow,CUT.low,BAND.low],[CUT.low,CUT.high,BAND.tir],[CUT.high,CUT.vhigh,BAND.high],[CUT.vhigh,this.yHi,BAND.vhigh]];
    for(const [lo,hi,col] of bands){
      const y0=sy(Math.min(hi,this.yHi)), y1=sy(Math.max(lo,this.yLo));
      ctx.fillStyle=col; ctx.globalAlpha=this.light?0.10:0.07; ctx.fillRect(padL,y0,plotW,y1-y0); ctx.globalAlpha=1;
    }
    // target lines 70 / 180
    ctx.strokeStyle='rgba(57,217,138,.45)'; ctx.lineWidth=1; ctx.setLineDash([4*dpr,4*dpr]);
    for(const t of [CUT.low,CUT.high]){ const y=sy(t); ctx.beginPath(); ctx.moveTo(padL,y); ctx.lineTo(padL+plotW,y); ctx.stroke(); }
    ctx.setLineDash([]);

    // y labels
    ctx.fillStyle=C.dim; ctx.font=(10*dpr)+'px IBM Plex Mono, monospace'; ctx.textAlign='right';
    for(const v of [54,70,180,250]){ if(v<this.yLo||v>this.yHi)continue; ctx.fillText(GluDisp.tick(v), padL-5*dpr, sy(v)+3*dpr); }

    // x time grid (hour ticks)
    ctx.textAlign='center';
    const secStart=this.gT[Math.max(0,Math.floor(start))], spanMin=span*this.cad;
    ...
    const _sd=new Date(secStart), _step=tickMin*60000;
    ...
    for(let tm=_mid+Math.ceil((secStart-_mid)/_step)*_step; ; tm+=_step){
      ...
      if(cell>start+span) break; if(cell<start) continue;
      const x=sxCell(cell);
      ctx.strokeStyle=C.grid; ctx.lineWidth=1; ctx.beginPath(); ctx.moveTo(x,padT); ctx.lineTo(x,padT+plotH); ctx.stroke();
      const d=new Date(tm);
      const lbl = spanMin>1440 ? (d.getUTCMonth()+1)+'/'+d.getUTCDate()+(tickMin>=360?' '+String(d.getUTCHours()).padStart(2,'0')+'h':'') : String(d.getUTCHours()).padStart(2,'0')+':'+String(d.getUTCMinutes()).padStart(2,'0');
      ctx.fillStyle=C.dim; ctx.fillText(lbl, x, H-6*dpr);
    }
    ...
    for(let i=i0;i<=i1;i+=stepDraw){
      const x=sxCell(i), y=sy(this.gV[i]);
      const isGap=this.gF[i]===this.FLAG.WARMUP;
      if(isGap){ if(pen){ ctx.stroke(); pen=false; } continue; }
      const col=colOf(this.gV[i]);
      if(!pen){ ctx.beginPath(); ctx.strokeStyle=col; ctx.moveTo(x,y); pen=true; prevCol=col; }
      else if(col!==prevCol){ ctx.lineTo(x,y); ctx.stroke(); ctx.beginPath(); ctx.strokeStyle=col; ctx.moveTo(x,y); prevCol=col; }
      else ctx.lineTo(x,y);
    }
    if(pen) ctx.stroke();
  }
  drawMini(){
    ...
    // tir band
    ctx.fillStyle='rgba(57,217,138,.10)'; ctx.fillRect(0,sy(CUT.high),W,sy(CUT.low)-sy(CUT.high));
    ctx.strokeStyle='rgba(61,224,208,.5)'; ctx.lineWidth=1; ctx.beginPath();
    const step=Math.max(1,Math.floor(this.N/W));
    let pen=false;
    for(let px=0;px<W;px++){ const i=Math.floor(px/W*this.N); if(this.gF[i]===this.FLAG.WARMUP){ pen=false; continue; }
      const y=sy(this.gV[i]); if(!pen){ ctx.moveTo(px,y); pen=true; } else ctx.lineTo(px,y); }
    ctx.stroke();
  ...
  zoom(factor,frac){ const c=this.view.start+this.view.span*(frac==null?0.5:frac);
    ...
    this.view={start,span}; this.draw(); this.drawMini(); this._emit(); }
  fitAll(){ this.view={start:0,span:this.N}; this.draw(); this.drawMini(); this._emit(); }
  setSpanMin(min){ const c=this.view.start+this.view.span/2; let span=Math.max(2,Math.min(this.N,Math.round(min/this.cad)));
    let start=Math.max(0,Math.min(this.N-span,c-span/2)); this.view={start,span}; this.draw(); this.drawMini(); this._emit(); }
  _emit(){ if(this.onView) this.onView(this.view,this.cad,this.N,this.gT); }
  _bind(){
    const cv=this.canvas;
    cv.addEventListener('wheel',e=>{ if(!this.gV)return; e.preventDefault(); const r=cv.getBoundingClientRect(); const frac=(e.clientX-r.left)/r.width; this.zoom(e.deltaY>0?1.25:0.8,frac); },{passive:false});
    let drag=null;
    ...
    cv.addEventListener('pointermove',e=>{ if(!drag||!this.gV)return; const r=cv.getBoundingClientRect(); const dSamp=-(e.clientX-drag.x)/r.width*this.view.span; this.view.start=Math.max(0,Math.min(this.N-this.view.span,drag.start+dSamp)); this.draw(); this.drawMini(); this._emit(); });
    const end=()=>{ drag=null; cv.style.cursor='grab'; }; cv.addEventListener('pointerup',end); cv.addEventListener('pointercancel',end);
    if(this.mini){ const jump=e=>{ const r=this.mini.getBoundingClientRect(); const frac=(e.clientX-r.left)/r.width; this.view.start=Math.max(0,Math.min(this.N-this.view.span,frac*this.N-this.view.span/2)); this.draw(); this.drawMini(); this._emit(); };
      let md=false; this.mini.addEventListener('pointerdown',e=>{md=true;jump(e);}); this.mini.addEventListener('pointermove',e=>{if(md)jump(e);}); window.addEventListener('pointerup',()=>md=false); }
    window.addEventListener('resize',()=>{ if(this.gV){ this.resize(); this.draw(); this.drawMini(); } });
  }
}
...
function agpChart(hourly, opts){
  opts=opts||{}; const W=opts.W||720, H=opts.H||230, P={l:42,r:14,t:14,b:24};
  ...
  const ymn=40, ymx=Math.max(260, Math.ceil((Math.max(...pts.map(b=>b.p90))+20)/20)*20);
  const sx=h=>P.l+h/24*(W-P.l-P.r);
  const sy=v=>H-P.b-(Math.min(ymx,Math.max(ymn,v))-ymn)/(ymx-ymn)*(H-P.t-P.b);
  const band=(key0,key1)=>{ let d='M'; const up=[],dn=[];
    for(const b of pts){ up.push(sx(b.h).toFixed(1)+' '+sy(b[key1]).toFixed(1)); }
    for(let i=pts.length-1;i>=0;i--){ dn.push(sx(pts[i].h).toFixed(1)+' '+sy(pts[i][key0]).toFixed(1)); }
  ...
  return `<svg viewBox="0 0 ${W} ${H}" preserveAspectRatio="none" style="width:100%;height:auto">
    ...
    ${[54,70,180,250].filter(v=>v>=ymn&&v<=ymx).map(v=>`<text x="${P.l-6}" y="${(sy(v)+3).toFixed(1)}" fill="${C.dim}" font-size="9" text-anchor="end" font-family="IBM Plex Mono,monospace">${GluDisp.tick(v)}</text>`).join('')}
...
function tirBar(tir){
  ...
  const legend = segs.map(s=>`<div class="tir-leg"><span class="tir-dot" style="background:${s.col}"></span><span class="tir-leg-l">${s.lbl}</span><span class="tir-leg-s">${s.sub} ${GluDisp.label()}</span><b>${s.v}%</b></div>`).join('');
...
function lineChart(pts, color, opts){
  ...
  const sx=x=>P.l+(x-xmn)/(xmx-xmn)*(W-P.l-P.r), sy=y=>H-P.b-(y-ymn)/(ymx-ymn)*(H-P.t-P.b);
  ...
  const bands=opts.tirBands?`<rect x="${P.l}" y="${sy(Math.min(ymx,180)).toFixed(1)}" width="${W-P.l-P.r}" height="${Math.max(0,sy(Math.max(ymn,70))-sy(Math.min(ymx,180))).toFixed(1)}" fill="${C.green}" opacity=".07"/>`:'';
  return `<svg viewBox="0 0 ${W} ${H}" preserveAspectRatio="none" style="width:100%;height:auto">
...
function dayOverlay(daysCells, opts){
  opts=opts||{}; const W=opts.W||720,H=opts.H||200,P={l:42,r:14,t:12,b:22};
  const ymn=40, ymx=opts.ymx||300;
  const sx=h=>P.l+h/24*(W-P.l-P.r), sy=v=>H-P.b-(Math.min(ymx,Math.max(ymn,v))-ymn)/(ymx-ymn)*(H-P.t-P.b);
  const traces=daysCells.map(d=>{ if(d.pts.length<3) return '';
    return `<path d="${d.pts.map((p,k)=>(k?'L':'M')+sx(p.h).toFixed(1)+' '+sy(p.v).toFixed(1)).join(' ')}" fill="none" stroke="${C.teal}" stroke-width="1" opacity=".18"/>`; }).join('');
  return `<svg viewBox="0 0 ${W} ${H}" preserveAspectRatio="none" style="width:100%;height:auto">
    <rect x="${P.l}" y="${sy(180).toFixed(1)}" width="${W-P.l-P.r}" height="${(sy(70)-sy(180)).toFixed(1)}" fill="${C.green}" opacity=".06"/>
...
function distribution(vals, opts){
  opts=opts||{}; const W=opts.W||340,H=opts.H||200,P={l:34,r:10,t:10,b:24};
  const lo=40, hi=Math.max(260,Math.ceil(Math.max(...vals.slice(0,1),...[opts.max||280])/20)*20);
  const nb=28, bins=new Array(nb).fill(0);
  for(const v of vals){ const b=Math.min(nb-1,Math.max(0,Math.floor((v-lo)/(hi-lo)*nb))); bins[b]++; }
  ...
  const bars=bins.map((c,i)=>{ const v=lo+(i+0.5)/nb*(hi-lo); const h=(c/mx)*(H-P.t-P.b); const x=P.l+i*bw; const y=H-P.b-h;
    return `<rect x="${x.toFixed(1)}" y="${y.toFixed(1)}" width="${(bw-1).toFixed(1)}" height="${h.toFixed(1)}" fill="${colOf(v)}" opacity=".85"/>`; }).join('');
  ...
  return `<svg viewBox="0 0 ${W} ${H}" style="width:100%;height:auto">
    ...
    ${[54,100,180,250].filter(v=>v<=hi).map(v=>`<text x="${sx(v).toFixed(1)}" y="${H-7}" fill="${C.dim}" font-size="8.5" text-anchor="middle" font-family="IBM Plex Mono,monospace">${GluDisp.tick(v)}</text>`).join('')}
  </svg>`;
}

// ── FUSION scatter: autonomic-risk vector (x) vs glycemic variability (y) ──
function fusionScatter(items, opts){
  opts=opts||{}; const W=opts.W||340,H=opts.H||300,P=38;
  // items: {x,y,label,col}
  const xs=items.map(i=>i.x), ys=items.map(i=>i.y);
  const xmn=Math.min(...xs,opts.xmn??0), xmx=Math.max(...xs,opts.xmx??1);
  const ymn=Math.min(...ys,opts.ymn??0), ymx=Math.max(...ys,opts.ymx??50);
  const sx=v=>P+(v-xmn)/(xmx-xmn||1)*(W-2*P), sy=v=>H-P-(v-ymn)/(ymx-ymn||1)*(H-2*P);
  const dots=items.map(i=>`<g><circle cx="${sx(i.x).toFixed(1)}" cy="${sy(i.y).toFixed(1)}" r="6" fill="${i.col||C.teal}" opacity=".9"/>${i.label?`<text x="${sx(i.x).toFixed(1)}" y="${(sy(i.y)-10).toFixed(1)}" fill="${C.dim}" font-size="9" text-anchor="middle" font-family="IBM Plex Mono,monospace">${i.label}</text>`:''}</g>`).join('');
