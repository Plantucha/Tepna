    ...
    parse: function (text, ctx) {
      ...
      if (!parseFn)
        return root.SignalFrame.toSignalFrame(
          'spo2',
          { usable: false, reason: 'oxydex-spo2: no parseCSV in scope (load OxyDex DSP in isolation)' },
          { adapter: 'oxydex-spo2', vendor: VENDOR, device: DEVICE, files: ctx.files || null }
        );
      var rows = parseFn(text, { fname: (ctx.files && ctx.files[0]) || null, file: null });
      if (!rows || !rows.length)
        return root.SignalFrame.toSignalFrame(
          'spo2',
          { usable: false, reason: 'oxydex-spo2: no usable SpO₂ rows parsed (need time + SpO₂ + pulse columns)' },
          { adapter: 'oxydex-spo2', vendor: VENDOR, device: DEVICE, files: ctx.files || null }
        );
