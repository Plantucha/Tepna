...
(function (root) {
  ...
  REG.registerAdapter({
    ...
    parse: function (text, ctx) {
      ...
      var parseFn = ecg && typeof ecg.parseECG === 'function' ? ecg.parseECG : typeof root.ECGDSP !== 'undefined' && typeof root.ECGDSP.parseECG === 'function' ? root.ECGDSP.parseECG : null;
      if (!parseFn) return root.SignalFrame.toSignalFrame('ecg', { usable: false, reason: 'polar-h10-ecg: ECGDex/ECGDSP not in scope (load ecgdex-dsp.js before this adapter)' }, prov);
      ...
      if (!rec || !rec.int16 || !rec.int16.length)
        return root.SignalFrame.toSignalFrame(
          'ecg',
          { usable: false, reason: 'polar-h10-ecg: no usable ECG samples parsed (expected Polar Sensor Logger `*_ECG.txt`: Phone timestamp;sensor ns;timestamp [ms];ecg [uV])' },
          prov
        );
      ...
      var frame = root.SignalFrame.toSignalFrame(
        'ecg',
        {
          ...
          reason: rec.int16.length >= minN ? null : 'only ' + rec.int16.length + ' ECG sample' + (rec.int16.length === 1 ? '' : 's') + ' (need ≥' + minN + ' ≈ 8 s for R-peak detection)'
        },
        prov
      );
      ...
      if (comp && typeof comp === 'object') {
        try {
          if (comp.rr && typeof ecg.parseDeviceRR === 'function') {
            var dr = ecg.parseDeviceRR(comp.rr);
            if (dr && dr.length) frame.deviceRR = dr;
          }
        } catch (e0) {
          prov.warnings.push('RR companion parse failed: ' + ((e0 && e0.message) || e0));
        }
        try {
          if (comp.hr && typeof ecg.parseDeviceHR === 'function') {
            var dh = ecg.parseDeviceHR(comp.hr);
            if (dh && dh.length) frame.deviceHR = dh;
          }
        } catch (e1) {
          prov.warnings.push('HR companion parse failed: ' + ((e1 && e1.message) || e1));
        }
        try {
          if (comp.acc && typeof ecg.parseDeviceACC === 'function') {
            ...
            if (da && da.acc && da.acc.length) {
              ...
              if (da.acc._relBase && frame.t0Ms != null) {
                for (var ai = 0; ai < da.acc.length; ai++) da.acc[ai].tsMs += frame.t0Ms;
                da.acc._relBase = false;
              }
              frame.deviceACC = da.acc;
              frame.accFs = da.accFs;
            }
          }
        } catch (e2) {
          prov.warnings.push('ACC companion parse failed: ' + ((e2 && e2.message) || e2));
        }
      }
      return frame;
    }
