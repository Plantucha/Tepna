...
function classifyModeLongitudinal(nights) {
  var q = (nights || [])
    .map(function (n) { return n && n.metrics ? n.metrics.pressureEnvIqr : null; })
...
function buildLongitudinal(nights) {
  ...
  return { compliancePct: compliancePct, nights: nights.length, usageTrend7d: usageTrend7d, ahiTrend30d: ahiTrend30d, crossNight: crossNight,
           mode: lm.mode, modeEnvIqr: lm.envIqr, modeNights: lm.nights };
}
...
function selfTest() {
  ...
  ok('§1 night mode is ALWAYS null — even when every session agrees (a night cannot name a device setting)',
     nmAgree && nmAgree.mode === null, nmAgree && nmAgree.mode);
  ok('§1 night mode is null when sessions disagree too', nmSplit && nmSplit.mode === null, nmSplit && nmSplit.mode);
  ok('§1 a night still reports pressureEnvIqr (the measurement survives; only the CLAIM is retired)',
     nmAgree && nmAgree.pressureEnvIqr != null && isFinite(nmAgree.pressureEnvIqr), nmAgree && nmAgree.pressureEnvIqr);

  /* ── the LONGITUDINAL call — where the evidence actually lives ── */
  var mkNight = function (envIqr) { return { metrics: { pressureEnvIqr: envIqr, usageHours: 7, residualAHI: 2 }, t0Ms: null }; };
  var runOf = function (v, n) { var a = []; for (var z = 0; z < n; z++) a.push(mkNight(v)); return a; };

  ok('§1 longitudinal: < 7 nights ⇒ null (not enough evidence for a SETTING)',
     classifyModeLongitudinal(runOf(1.4, 6)).mode === null, classifyModeLongitudinal(runOf(1.4, 6)).mode);
  ok('§1 longitudinal: a run of auto-titrating nights ⇒ APAP',
     classifyModeLongitudinal(runOf(1.4, 10)).mode === 'APAP', classifyModeLongitudinal(runOf(1.4, 10)).mode);
  ok('§1 longitudinal: a run of flat-envelope nights ⇒ CPAP',
     classifyModeLongitudinal(runOf(0.2, 10)).mode === 'CPAP', classifyModeLongitudinal(runOf(0.2, 10)).mode);
  ok('§1 longitudinal: a run in the dead-band ⇒ null (still refuses to guess)',
     classifyModeLongitudinal(runOf(0.75, 10)).mode === null, classifyModeLongitudinal(runOf(0.75, 10)).mode);
  ...
  ok('§1 longitudinal: outlier nights do NOT flip the setting (the per-night label would have)',
     classifyModeLongitudinal(noisy).mode === 'APAP', classifyModeLongitudinal(noisy).mode);

  var lb = buildLongitudinal(runOf(1.4, 10));
  ok('§1 buildLongitudinal surfaces mode + modeEnvIqr + modeNights',
     lb.mode === 'APAP' && lb.modeEnvIqr != null && lb.modeNights === 10,
     lb.mode + '/' + lb.modeEnvIqr + '/' + lb.modeNights);
  ok('§1 buildLongitudinal on ONE night ⇒ mode null (a night is not a setting)',
     buildLongitudinal(runOf(1.4, 1)).mode === null);
